...
Функция СформироватьТаблицуТоваровДокументаДляЧека(ДокументСсылка, КлючСтроки = Неопределено, СтавкаНДСПоУмолчанию = Неопределено, СписаниеСДепозитаУказыватьВОплатах = Истина, СуммаВзносаНаДепозит = 0, ВидДепозитнойОперации = Неопределено, ПравилоПечатиАртикулаНМУВЧеке = Неопределено, ЧекБезОплаты = Ложь) Экспорт
	...
	|	Выбор
	|	 Когда ОказаниеУслугТовары.Номенклатура.ВидМаркированнойПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыМаркированнойПродукции.МДЛП)  Тогда
	...
	Для Каждого СтрокаТоваров Из ТаблицаТоваров Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТоваров.СтавкаНДС) Тогда
			// Установим ставку для УСН если вдруг в карточке товара стоит 20 или 10
			РегистрыСведений.СтавкиНДСПриУСН.СкорректироватьСтавкуТовараДляУСН(СтрокаТоваров.СтавкаНДС, СтавкаНДСПоУмолчанию);
	...
	Если УчетнаяПолитика.ЧекККМПродажаПечататьУслугиОднойСтрокой Тогда
		...
		Для Сч = 0 По СтаршийИндекс Цикл
			НоваяСтрока = ТаблицаТоваров.Вставить(Сч);
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Услуги[Сч]);
			РассчитатьСуммаНДСПозицииЧека(НоваяСтрока);
		КонецЦикла;		
...
Функция ПолучитьЭквайринговыйТерминал(РабочееМесто = Неопределено, Филиал = Неопределено, Касса = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	ЭквайринговыеТерминалы.Тип КАК Тип,
		|	ЭквайринговыеТерминалы.ИспользоватьЭТАвтономно КАК ИспользоватьЭТАвтономно
		|ПОМЕСТИТЬ Терминалы
		|ИЗ
		...
		|			ИЛИ ЭквайринговыеТерминалы.Касса = ЗНАЧЕНИЕ(Справочник.Филиалы.ПустаяСсылка))
		|	И (НЕ ЭквайринговыеТерминалы.Касса ССЫЛКА Справочник.Кассы
		|			ИЛИ ЭквайринговыеТерминалы.Касса = &Касса
		|			ИЛИ ЭквайринговыеТерминалы.Касса = ЗНАЧЕНИЕ(Справочник.Кассы.ПустаяСсылка))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА ЭквайринговыеТерминалы.Касса = &Касса
		|			ТОГДА 10
		|		КОГДА ЭквайринговыеТерминалы.Касса = &Филиал
		|			ТОГДА 20
		...
		|ГДЕ
		|	ЭквайринговыеТерминалы.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыСпособовОплат.Эквайринг)
		|	И ЭквайринговыеТерминалы.ИспользоватьЭТАвтономно = ЛОЖЬ
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА ЭквайринговыеТерминалы.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыСпособовОплат.Эквайринг)
		|				И ЭквайринговыеТерминалы.ИспользоватьЭТАвтономно = ИСТИНА
		|			ТОГДА ВЫБОР
		|					КОГДА ЭквайринговыеТерминалы.Касса = &Касса
		|						ТОГДА 11
		|					КОГДА ЭквайринговыеТерминалы.Касса = &Филиал
		|						ТОГДА 21
		|					КОГДА ЭквайринговыеТерминалы.Касса В (&ПустаяКасса)
		|						ТОГДА 31
		|					ИНАЧЕ 999
		|				КОНЕЦ
		|		КОГДА ЭквайринговыеТерминалы.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыСпособовОплат.СБП)
		|			ТОГДА ВЫБОР
		|					КОГДА ЭквайринговыеТерминалы.Касса = &Касса
		|						ТОГДА 12
		|					КОГДА ЭквайринговыеТерминалы.Касса = &Филиал
		...
		|					ИНАЧЕ 999
		|				КОНЕЦ
		|		КОГДА ЭквайринговыеТерминалы.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыСпособовОплат.Прочее)
		|			ТОГДА ВЫБОР
		|					КОГДА ЭквайринговыеТерминалы.Касса = &Касса
		|						ТОГДА 13
		|					КОГДА ЭквайринговыеТерминалы.Касса = &Филиал
		...
		|ГДЕ
		|	ТаблицаТерминалов.Приоритет < 100
		|
		|СГРУППИРОВАТЬ ПО
	...
	ПустаяКасса = Новый Массив;
	ПустаяКасса.Добавить(Неопределено);
	ПустаяКасса.Добавить(Справочники.Кассы.ПустаяСсылка());
	ПустаяКасса.Добавить(Справочники.Филиалы.ПустаяСсылка());
	Запрос.УстановитьПараметр("ПустаяКасса", ПустаяКасса);
