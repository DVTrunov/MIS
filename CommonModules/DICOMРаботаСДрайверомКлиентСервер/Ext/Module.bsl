...
Функция ПолучитьИнтерфейсDICOM() Экспорт
	...
	Исключение
		...
		Попытка
			...
			ОК = ПодключитьВнешнююКомпоненту(
				ИмяМакета, "AddInNative", ТипВнешнейКомпоненты.Native
			);
...
Процедура ПриНачалеРаботыСистемы() Экспорт
	...
	Если ИспользоватьDICOM Тогда
		...
		// Запуск StorageSCP
		НастройкиDICOM = DICOMРаботаСДрайвером.ПолучитьНастройки("DCMTK_STORESCP_АвтоматическийЗапуск");
		Если НастройкиDICOM.DCMTK_STORESCP_АвтоматическийЗапуск = Истина Тогда
...
Процедура ОстановкаStorageSCP() Экспорт
	...
	#Если НаСервере Тогда
		...
		Попытка
			НастройкиDICOM.DCMTK_STORESCP_ИспользуемыйAETitle = Константы.DCMTK_STORESCP_ИспользуемыйAETitle.Получить();
		Исключение
		КонецПопытки;
		
		Попытка
			НастройкиDICOM.DCMTK_STORESCP_КаталогФайлов = Константы.DCMTK_STORESCP_КаталогФайлов.Получить();
		Исключение
		КонецПопытки;
	...
	AE = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(
				НастройкиDICOM.DCMTK_STORESCP_ИспользуемыйAETitle, "Port, Наименование, Host"
			);
...
Функция ПолучитьСтатусStorageSCP() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(НастройкиDICOM.DCMTK_STORESCP_ИспользуемыйAETitle) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не указано значение DCMTK_STORESCP_ИспользуемыйAETitle");
		Возврат Ложь;
	ИначеЕсли НЕ ЗначениеЗаполнено(НастройкиDICOM.DCMTK_STORESCP_КаталогФайлов) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не указано значение DCMTK_STORESCP_КаталогФайлов");
		Возврат Ложь;		
	КонецЕсли;
...
Функция ЗапуститьСлужбуStoreSCP() Экспорт
	...
	AE = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(
				НастройкиDICOM.DCMTK_STORESCP_ИспользуемыйAETitle, "Port, Наименование"
			);
	Port = Формат(AE.Port, "ЧГ=0");
	...
	Если КаталогНаДиске.Существует() Тогда
		...
		СтрокаЗапуска = ПутьКФайлуСлужбы 
							...
							+ AETitle
							+ " -od "
							+ НастройкиDICOM.DCMTK_STORESCP_КаталогФайлов
							+ " " + Port;
