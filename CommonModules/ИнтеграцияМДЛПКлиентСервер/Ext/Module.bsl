...
Процедура РазобратьСтрокуШтрихкодаГС1СоСкобками(Штрихкод, КодыГС1, РезультатРазбора)
	...
	//+ОЛИМП
	ШтрихкодBase64 = Base64Строка(ПолучитьДвоичныеДанныеИзСтроки(ШтрихкодСРазделителями));
	ШтрихкодBase64 = СтрЗаменить(СтрЗаменить(ШтрихкодBase64, Символы.ВК, ""), Символы.ПС, "");
	РезультатРазбора.ШтрихкодBase64 = ШтрихкодBase64;
