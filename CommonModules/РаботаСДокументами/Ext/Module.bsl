...
Процедура дкЗаполнитьШапкуДокумента(ДокументОбъект,фМетаданные) Экспорт
	...
	ИмяРеквизитаСклад = СкладыБИТ.ИмяРеквизитаСкладОтправитель(фМетаданные);
	Если  ИмяРеквизитаСклад <> Неопределено
		И ДокументОбъект[ИмяРеквизитаСклад] = Справочники.Склады.ПустаяСсылка()
	Тогда
		ДокументОбъект[ИмяРеквизитаСклад] = СкладыБИТПовтИсп.ЕдинственныйСкладВБазе();
		
		Если фМетаданные.Имя = "ОказаниеУслуг"
			И ЗначениеЗаполнено(ДокументОбъект[ИмяРеквизитаСклад])
			И Не ЗначениеЗаполнено(ДокументОбъект.СкладМатериалов)
		Тогда
			ДокументОбъект.СкладМатериалов = ДокументОбъект[ИмяРеквизитаСклад];
		КонецЕсли;
	КонецЕсли;
...
Функция дкОбработкаЗаполнения_Заявка_ОказаниеУслуг(ЭтотОбъект,Основание,Копирование, Знач Заявки = Неопределено)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Заявка.омг_Прейскурант КАК омг_Прейскурант,
	               |	Заявка.Филиал КАК Филиал,
	               |	Заявка.омг_СтраховаяПрограмма КАК омг_СтраховаяПрограмма
	               |ИЗ
	...
	ЭтотОбъект.Прейскурант = РеквизитыЗаявки.омг_Прейскурант;
	// )
	////////////////////////////////////////////////////
	// ОМГ 	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		...
		|				ТОГДА омг_НоменклатураЗаявокДляОказаний.Количество 
		|			ИНАЧЕ омг_НоменклатураЗаявокДляОказаний.Количество * -1
		|		КОНЕЦ) КАК Продано,
		...
		|ГДЕ
		| омг_НоменклатураЗаявокДляОказаний.Заявка = &Заявка
		|
		|СГРУППИРОВАТЬ ПО
	...
	ВыборкаПродано = РезультатЗапроса.Выгрузить();
	...
	Для Каждого Заявка Из Заявки Цикл
		Для Каждого ТекСтрока Из Заявка.Работы Цикл
			...
			Если ЗначениеЗаполнено(ТекСтрока.Номенклатура)
				И Не ТекСтрока.Номенклатура.ТолькоДляПланирования Тогда   
				//ОсталосьПродать = ТекСтрока.Количество; 
				ОсталосьПродать = 0;
				СтрПоиска = Новый Структура();
				СтрПоиска.Вставить("Номенклатура",ТекСтрока.Номенклатура);  
				Сотрудник = ?(ЗначениеЗаполнено(ТекСтрока.Сотрудник),ТекСтрока.Сотрудник,ЭтотОбъект.Сотрудник);
				СтрПоиска.Вставить("Врач",Сотрудник);
				НайденныеОстатки = ВыборкаПродано.НайтиСтроки(СтрПоиска);
				Если НайденныеОстатки.Количество() > 0 и НайденныеОстатки[0].Продано > 0 Тогда
					ОсталосьПродать =  Мин(НайденныеОстатки[0].Продано,ТекСтрока.Количество); 
					НайденныеОстатки[0].Продано = НайденныеОстатки[0].Продано - ОсталосьПродать; 
				КонецЕсли;
				Если ОсталосьПродать <= 0 Тогда
					Продолжить	
				КонецЕсли;	
				// ОМГ 
				////////////////////////////////////////////////////
				...
				////////////////////////////////////////////////////
				// ОМГ Количество берем из заявки
				//Если НоваяСтрока.Количество = 0 Тогда
				//НоваяСтрока.Количество = 1;  
				//КонецЕсли;  
				НоваяСтрока.Количество = ОсталосьПродать;
				// ОМГ Количество берем из заявки
				////////////////////////////////////////////////////

				// Добавлено Макеев (
				Прейскурант = ?(РеквизитыЗаявки.омг_СтраховаяПрограмма.Пустая(), ЭтотОбъект.Прейскурант, РеквизитыЗаявки.омг_СтраховаяПрограмма);
				// )
				// Изменено Макеев (
				НоваяСтрока.Цена	= Ценообразование.ПолучитьЦену(Прейскурант, НоваяСтрока.Номенклатура,,,,,РеквизитыЗаявки.Филиал, ТекСтрока.омг_Набор,ТекСтрока.омг_НаборУровень).Цена;	
				// )
	...
	Для Каждого Свойство Из СписокСвойств Цикл
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "ПервичныйПрием") Тогда
				...
				Иначе
					Если Не ЗначениеЗаполнено(ЭтотОбъект.ПервичныйПрием) Тогда
						// Изменено Макеев (
						// Отключено, так как первичный прием будет заполняться иначе
						//ЭтотОбъект.ПервичныйПрием = МедицинскаяДеятельность.ПолучитьТекущийПервичныйПриемМедкарты(ЭтотОбъект.МедицинскаяКарта);
						// )
						// Добавлено Макеев (
						омг_МедицинскаяДеятельность.ЗаполнитьВторичныйПрием(ЭтотОбъект, Основание);
		...
		ИначеЕсли Свойство.Значение = "Работы" Тогда
			...
			Если ТипЗнч(Основание) = Тип("ДокументСсылка.Заявка") Тогда
				...
				|		КОНЕЦ) КАК Продано
				|ИЗ
				|	РегистрНакопления.омг_НоменклатураЗаявокДляПриемов КАК омг_НоменклатураЗаявокДляОказаний
				|ГДЕ
				| омг_НоменклатураЗаявокДляОказаний.Заявка = &Заявка
				|
				|СГРУППИРОВАТЬ ПО
			...
			////////////////////////////////////////////////////////// 
					НоваяСтрока.Количество = ОсталосьПродать; 
			// ОМГ   // Количество в заявке и приеме должно совпадать в любом случае
					//Если ТекСтрока.Номенклатура.ПродолжительностьЕдиницыУслуги <> 0 Тогда
		...
		ИначеЕсли Свойство.Значение = "Врач" Тогда   
			...
			Если РольДоступна("омг_Врач") и ЗначениеЗаполнено(ПараметрыСеанса.ТекущийПользователь.Сотрудник) Тогда
...
Функция омг_ПолучитьСтраховуюПрограммуЗаявки(Заявка)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	Заявка.омг_СтраховаяПрограмма КАК омг_СтраховаяПрограмма,
	               |	Заявка.Ссылка КАК Ссылка,
	               |	Заявка.омг_СтраховаяПрограмма.Владелец КАК Контрагент
	               |ИЗ
	               |	Документ.Заявка КАК Заявка
	               |ГДЕ
	               |	Заявка.Ссылка = &Ссылка";
	...
	Возврат Новый Структура("Контрагент, Договор", ВыборкаОснование.Контрагент, ВыборкаОснование.омг_СтраховаяПрограмма);
...
Функция дкОбработкаЗаполнения_Приемы_ОказаниеУслуг(ЭтотОбъект, Основание, Копирование, Знач Заявки = Неопределено)
	...
	Иначе
		...
		ЭтотОбъект.ДокументОснование = Основание.Ссылка;
		// Добавлено Макеев (
		омг_СтраховаяПрограмма = омг_ПолучитьСтраховуюПрограммуЗаявки(Основание);
		// )
		ЭтотОбъект.Клиент = Основание.Клиент;
		ЭтотОбъект.Сотрудник = Основание.Сотрудник;
		ЭтотОбъект.ДатаНачала = Основание.ДатаНачала;
		...
		Пока Выборка.Следующий() Цикл
			...
			Для Каждого СтрокаРаботы Из Работы Цикл 
				...
				// Добавлено Макеев (
				Прейскурант = ?(ЭтотОбъект.омг_СтраховаяПрограмма.Пустая(), ЭтотОбъект.Прейскурант, омг_СтраховаяПрограмма.Контрагент);
				// )
				// Изменено Макеев (
				НоваяСтрока.Цена	= Ценообразование.ПолучитьЦену(Прейскурант, НоваяСтрока.Номенклатура).Цена;	
				// )
				НоваяСтрока.Сумма	= НоваяСтрока.Цена*НоваяСтрока.Количество;
				НоваяСтрока.КлючСтроки = НоваяСтрока.НомерСтроки;
...
Процедура дкРассчитатьСуммуСкидкиСтрокиТабЧасти(ЭтотОбъект,ИмяТЧ, ТекСтрока) Экспорт
	...
	Если не ТекСтрока.Цена = Неопределено Тогда
