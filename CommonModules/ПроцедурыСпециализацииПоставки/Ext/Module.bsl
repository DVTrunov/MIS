...
Функция ПолучитьРеквизитыОрганизации(ИсточникЗапроса = Неопределено, ДатаЛицензии = Неопределено) Экспорт 
	...
	СтруктураРеквизитов = Новый Структура("НазваниеОрганизации, ИНН, КПП, ОГРН, ОКПО,
										  |Адрес, Телефон, АдресФактический,
										  |Лицензия, ЛицензияКемВыдана, ЛицензияДата, ЛицензияПредставление");
	...
	Если ИсточникЗапроса <> Неопределено Тогда
		...
		КИ.УстановитьПараметр("Тип", пчТипыКИ.Адрес);
		...
		КонецЕсли;
		// Телефон
		КИ.УстановитьПараметр("Тип", пчТипыКИ.Телефон);
		...
		Если ЗначениеЗаполнено(Филиал) Тогда
			СтруктураРеквизитов.АдресФактический = Справочники.Филиалы.ПолучитьКИ(Филиал, пчТипыКИ.Адрес, спрВидыКИ.АдресФактический);
		Иначе
			СтруктураРеквизитов.АдресФактический = СтруктураРеквизитов.Адрес;
		КонецЕсли;
	КонецЕсли;
...
//  ОтборМодельКабинет		 - Булево - Истина - Только кабинеты, Ложь - только модели, Неопределено - без отбора.
// 
// Возвращаемое значение:
//  Структура - оборудование.
//
Функция ПолучитьЗначенияОборудования(Номенклатура, Филиал, ОтборМодельКабинет = Ложь) Экспорт
	...
	Для Каждого СтрокаОборудования Из Номенклатура.Оборудования Цикл
		...
		Если ОтборМодельКабинет = Истина И ТипЗнч(СтрокаОборудования.Оборудование) <> Тип("СправочникСсылка.Оборудование")
		 Или ОтборМодельКабинет = Ложь	 И ТипЗнч(СтрокаОборудования.Оборудование) <> Тип("СправочникСсылка.МоделиОборудования")
		Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ДобавитьКомандуСтоматологииВФормуУчетнойПолитики(Форма) Экспорт
	
	Элементы = Форма.Элементы;
	
	НавигационнаяСсылка = ПолучитьНавигационнуюСсылку(РегистрыСведений.стомУчетнаяПолитика.СоздатьКлючЗаписи(Новый Структура));
	КнопкаОткрытьНастройки = Элементы.Добавить("ОткрытьНастройкиСтоматологии", Тип("ДекорацияФормы"), Элементы.МедицинскаяДеятельность);
	КнопкаОткрытьНастройки.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Настройки стоматологии'"),,,, НавигационнаяСсылка);
	КнопкаОткрытьНастройки.Шрифт = Новый Шрифт(КнопкаОткрытьНастройки.Шрифт, , 12);
