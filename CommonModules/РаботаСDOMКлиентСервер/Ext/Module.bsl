...
// Функция - Получить элементы по имени узла и атрибуту
//
// Параметры:
//  ДокументDOM		 - ДокументDOM			 - документ DOM, в котором производится поиск элементов
//  ИмяУзла			 - Строка				 - имя узла искомого элемента
//  ИмяАтрибута		 - Строка				 - наименование атрибута искомого элемента
//  ЗначениеАтрибута - Строка, Неопределено	 - значение указанного атрибута искомого элемента
// 
// Возвращаемое значение:
//  Массив - Найденные элементы DOM
//
Функция ПолучитьЭлементыПоИмениУзлаИАтрибуту(ДокументDOM, ИмяУзла, ИмяАтрибута, ЗначениеАтрибута = Неопределено) Экспорт
	
	НайденныеЭлементы = ПолучитьЭлементыПоИмени(ДокументDOM, ИмяУзла);
	
	ЭлементыПоАтрибуту = Новый Массив;
	Для Каждого НайденныйЭлемент Из НайденныеЭлементы Цикл
		Попытка
			Если (ЗначениеАтрибута = Неопределено
					И НайденныйЭлемент.ЕстьАтрибут(ИмяАтрибута))
				Или (ЗначениеАтрибута <> Неопределено
					И НайденныйЭлемент.ПолучитьАтрибут(ИмяАтрибута) = Строка(ЗначениеАтрибута))
			Тогда
				ЭлементыПоАтрибуту.Добавить(НайденныйЭлемент);
			КонецЕсли;
...
//
Функция ПолучитьЭлементыПоИмени(ДокументDOM, Знач ИмяЭлемента, ВернутьМассив1С = Ложь) Экспорт
	
	НайденныеЭлементы = ДокументDOM.ПолучитьЭлементыПоИмени(ИмяЭлемента);
	
	Если ВернутьМассив1С Тогда
		ЭлементыПоИмени = Новый Массив;
		Для Каждого НайденныйЭлемент Из НайденныеЭлементы Цикл
			ЭлементыПоИмени.Добавить(НайденныйЭлемент);
		КонецЦикла;
	Иначе
		ЭлементыПоИмени = НайденныеЭлементы;
	КонецЕсли;
	
	Возврат ЭлементыПоИмени;
