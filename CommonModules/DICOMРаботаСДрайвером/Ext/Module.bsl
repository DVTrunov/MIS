...
Функция ПолучитьЗначениеКонстантыИспользоватьDICOM() Экспорт 
	// Считаем, что достаточно быть управляющим. Для возможных отладок
	Если РольДоступна("DICOM_Администрирование_ДополнительныеПрава") Или РольДоступна("ПолныеПрава") Тогда
...
Функция ПолучитьPACS() Экспорт
	Настройки  =  DICOMРаботаСДрайвером.ПолучитьНастройки("DCMTK_ЗапросыВызываемыйAETitle");
	
	Если Не ЗначениеЗаполнено(Настройки.DCMTK_ЗапросыВызываемыйAETitle) Тогда
		Попытка
			DCMTK_ЗапросыВызываемыйAETitle = Константы.DCMTK_ЗапросыВызываемыйAETitle.Получить();
		Исключение
			DCMTK_ЗапросыВызываемыйAETitle = Неопределено; 
		КонецПопытки;
		
		Возврат DCMTK_ЗапросыВызываемыйAETitle;
	КонецЕсли;
	
	Возврат Настройки.DCMTK_ЗапросыВызываемыйAETitle;
КонецФункции

Функция ПолучитьAETitleДляЗапросовDICOM() Экспорт
	Настройки  =  DICOMРаботаСДрайвером.ПолучитьНастройки("DCMTK_STORESCP_ИспользуемыйAETitle");
	Возврат Настройки.DCMTK_STORESCP_ИспользуемыйAETitle;
КонецФункции

Функция ПолучитьХранилищеDICOM() Экспорт
	Настройки  =  DICOMРаботаСДрайвером.ПолучитьНастройки("DCMTK_STORESCP_ИспользуемыйAETitle,STORESCP_ИспользоватьКлиент");
	
	Если Настройки.STORESCP_ИспользоватьКлиент = Истина Тогда
		Возврат Настройки.DCMTK_STORESCP_ИспользуемыйAETitle;
	Иначе
		Попытка
			Возврат Константы.DCMTK_STORESCP_ИспользуемыйAETitle.Получить();
		Исключение
...
Функция ПолучитьПутьКНастройкамЛогов() Экспорт
	LogCatalog = Константы.DCMTK_LogCatalog.Получить();
...
Функция ПолучитьКаталогФайловWorkList() Экспорт
	БазовыйКаталог = Константы.DCMTK_MWLSCP_КаталогФайлов.Получить();
	...
	//AETitle = Константы.DCMTK_MWLSCP_ИспользуемыйAETitle.Получить();
...
Процедура ОстановкаMWLCSP() Экспорт
	...
	Попытка
		...
		Рез = ВнешняяКомпонента.StopServiceIn1C("-aec", "1C", "-aet", Строка(Константы.DCMTK_MWLSCP_ИспользуемыйAETitle.Получить().Наименование)
			,"-host", Формат(Константы.DCMTK_MWLSCP_ИспользуемыйAETitle.Получить().Host, "ЧГ=0")
			, "-port", Формат(Константы.DCMTK_MWLSCP_ИспользуемыйAETitle.Получить().Port, "ЧГ=0"));
...
Функция ПолучитьЗначениеКонстантыMWLSCP_АвтоматическийЗапуск() Экспорт
	Возврат Константы.DCMTK_MWLSCP_АвтоматическийЗапуск.Получить();
КонецФункции
...
Функция ЗапуститьСлужбуMWLSCP() Экспорт
	...
	СтрокаЗапуска = "";
	КаталогЗапускаСлужб = Константы.DCMTK_КаталогЗапускаСлужб.Получить();
	...
	Если КаталогНаДиске.Существует() Тогда
		...
		СтрокаЗапуска = ПутьКФайлуСлужбы 
							+ " -s "
			 				+ " -dfp "
							+ Константы.DCMTK_MWLSCP_КаталогФайлов.Получить()
							+ " " 
							+ Формат(Константы.DCMTK_MWLSCP_ИспользуемыйAETitle.Получить().Port, "ЧГ=0");
...
Функция ЗапуститьСлужбуStoreSCP() Экспорт
		...
		#Если НаСервере Тогда
			НастройкиDICOM = Новый Структура("DCMTK_STORESCP_ИспользуемыйAETitle,DCMTK_STORESCP_КаталогФайлов,STORESCP_ОчищатьКаталогЧерез");
			Попытка
				НастройкиDICOM.ХранилищеDICOM_ИспользуемыйAETitle = Константы.DCMTK_STORESCP_ИспользуемыйAETitle.Получить();
			Исключение
			КонецПопытки;
			
			Попытка
				НастройкиDICOM.ХранилищеDICOM_КаталогФайлов = Константы.DCMTK_STORESCP_КаталогФайлов.Получить();
			Исключение
			КонецПопытки;
...
Функция ПолучитьКаталогЗапускаСлужб() Экспорт
	Возврат Константы.DCMTK_КаталогЗапускаСлужб.Получить()	;
КонецФункции
