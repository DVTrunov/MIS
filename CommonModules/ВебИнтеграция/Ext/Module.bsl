...
Функция ПолучитьГрафикиРаботыXML(ДатаНачала, ДатаОкончания, ОписаниеОшибки, ИнтеграцияССайтом = Ложь, ДопПараметры = Неопределено, ОписаниеОшибкиParams = "") Экспорт
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(ОписаниеОшибки, ОписаниеОшибки(),Символы.ПС) ;
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'В процедуре ВебИнтеграция.ПолучитьГрафикиРаботыXML(ДатаНачала, ДатаОкончания) возникла ошибка. %1%2'"),Символы.ПС,ОписаниеОшибки);
...
//  Строка - xml-текст.
//
Функция ПолучитьУкрупненыеГрафикиРаботыXML(ДатаНачала, ДатаОкончания, ОписаниеОшибки, ДопПараметры = Неопределено) Экспорт
	Попытка
		
		СтрокаСотрудникXDTO	= ПолучитьXDTOТекстУкрупненыеГрафикиРаботы(ДатаНачала, ДатаОкончания, ДопПараметры);
...
//  Строка - ответ сайту.
//
Функция СоздатьЗаявку(Сотрудник, Фамилия, Имя, Отчество, ДатаРождения = '00010101', Дата, ВремяНачалаЗаписи, ВремяОкончанияЗаписи, Комментарий, Телефон, Email, Адрес, Клиника, ОписаниеОшибки, УИД = "", Клиент = Неопределено, Услуга = Неопределено, ДопПараметры = Неопределено) Экспорт
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
			...
			Если Клиент = Неопределено Тогда
				...
				Клиент = ОбработатьДанныеКлиента(ДанныеКлиента, Клиника, ?(ДопПараметры.Свойство("НастройкиОбмена"),ДопПараметры.НастройкиОбмена,Неопределено));
			КонецЕсли;
			
			Если ЗначениеЗаполнено(УИД) Тогда
				Попытка
					Бронь = ?(ТипЗнч(УИД) = Тип("ДокументСсылка.Заявка"), УИД, Документы.Заявка.ПолучитьСсылку(Новый УникальныйИдентификатор(УИД)) );
					Бронь = Бронь.ПолучитьОбъект();
					
					Если Бронь = Неопределено Тогда
						ОписаниеОшибки = СтрШаблон(НСтр("ru='Не найден документ Заявки с GUID: %1.'"), Строка(УИД));
						Возврат ОписаниеОшибки;
					КонецЕсли;
				Исключение
			...
			Если ДопПараметры <> Неопределено И ДопПараметры.Свойство("EquipmentCabinet") И ЗначениеЗаполнено(ДопПараметры.EquipmentCabinet) Тогда
				Бронь.Кабинет		= ДопПараметры.EquipmentCabinet;	
			Иначе
				Бронь.Кабинет		= Неопределено;
			КонецЕсли;
			...
			ВебИнтеграцияПереопределяемый.СоздатьЗаявку_ПередЗаписью(Бронь, ДопПараметры);
			...
			Попытка
				...
				Попытка
					СоздатьЛистОжиданияПоЗаявкеНовогоКлиента(Бронь.Ссылка, ДопПараметры);
				Исключение КонецПопытки;
				ОписаниеОшибки = " ";
				Результат = Истина;
			Исключение
				ОписаниеОшибки = НСтр("ru='Не удалось записать документ.'");
		...
		// Для веб-сервисов ДопПараметры = Неопределено, для HTTP-сервиса надо возвращать xml
		Если ЗначениеЗаполнено(Услуга) И ДопПараметры = Неопределено Тогда
			Если Результат Тогда
				Возврат "Ok";
			Иначе
				Возврат "Error";
	...
	Исключение
		...
		Возврат СтрШаблон(НСтр("ru='В процедуре ВебИнтеграция.СоздатьЗаявку(Имя, УИДЗаявки, ДатаРождения, Фамилия, Комментарий, Телефон, Email, ОписаниеОшибки) возникла ошибка.%1'"), ОписаниеОшибки);
	КонецПопытки;
...
//  Строка - ответ сайту.
//
Функция СоздатьЛистОжидания(Специализация, Фамилия, Имя, Отчество, Дата, ВремяНачалаЗаписи, Комментарий, Телефон, Email, Адрес, Клиника, ОписаниеОшибки, Клиент = Неопределено, ДопПараметры = Неопределено) Экспорт
	Попытка 
		Результат = Ложь;
		УИД = "";
		Если НЕ ЗначениеЗаполнено(ОписаниеОшибки) Тогда
			...
			// Источник рекламы
			Если ДопПараметры <> Неопределено И ДопПараметры.Свойство("НастройкиОбмена") Тогда
            	НастройкиОбмена = ДопПараметры.НастройкиОбмена;
			Иначе
			...
			// Дописываем перечень работ, если был задан
			ВремяОкончанияЗаписи = Неопределено;
			Если ДопПараметры <> Неопределено И ДопПараметры.Свойство("ВремяОкончанияЗаписи",ВремяОкончанияЗаписи) И ЗначениеЗаполнено(ДопПараметры.Service) Тогда
				
				Если ТипЗнч(ДопПараметры.Service) = Тип("ТаблицаЗначений") Тогда	
					ЛистОжиданияОбъект.Работы.Загрузить(ДопПараметры.Service);
				ИначеЕсли ТипЗнч(ДопПараметры.Service) = Тип("Массив") Тогда
					
					Для каждого Услуга Из ДопПараметры.Service Цикл
						СтрокаРаботы = ЛистОжиданияОбъект.Работы.Добавить();
						СтрокаРаботы.Номенклатура = Услуга;		
					КонецЦикла;					
					ЛистОжиданияОбъект.Работы[0].Продолжительность = ВремяОкончанияЗаписи;
					
				Иначе
					СтрокаРаботы = ЛистОжиданияОбъект.Работы.Добавить();
					СтрокаРаботы.Номенклатура = ДопПараметры.Service;
					СтрокаРаботы.Продолжительность = ВремяОкончанияЗаписи;
				КонецЕсли;				
			КонецЕсли;		
			
			ВебИнтеграцияПереопределяемый.СоздатьЛистОжидания_ПередЗаписью(ЛистОжиданияОбъект, ДопПараметры);
...
//  Строка - ответ сайту.
//
Функция СоздатьБронь(Сотрудник, Дата, ВремяНачалаЗаписи, ВремяОкончанияЗаписи, Клиника, ОписаниеОшибки, ДопПараметры = Неопределено) Экспорт
	...
	Попытка 
		...
		Если НЕ ЗначениеЗаполнено(ОписаниеОшибки) Тогда
			...
			Если ТипЗнч(Сотрудник) = Тип("СправочникСсылка.Сотрудники") Тогда			
				Бронь.Сотрудник		= Сотрудник;
			Иначе
				// Имеет смысл только при настройке политики - запись в кабинет
				Бронь.Кабинет		= Сотрудник;
			КонецЕсли;
			Если ДопПараметры <> Неопределено И ДопПараметры.Свойство("EquipmentCabinet") И ЗначениеЗаполнено(ДопПараметры.EquipmentCabinet) Тогда
				Бронь.Кабинет		= ДопПараметры.EquipmentCabinet;	
			КонецЕсли;
			...
			Исключение		
				ОписаниеОшибки = НСтр("ru = 'Не удалось записать документ.'");
				ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(ОписаниеОшибки, Строка(Бронь.Дата), Символы.Таб);
				ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(ОписаниеОшибки, ОписаниеОшибки(), " - ");
				Результат = Ложь;
			КонецПопытки;
		...
		ВебИнтеграцияПереопределяемый.СоздатьБронь_ПередВозвратом(Бронь, Результат, ОписаниеОшибки, ДопПараметры);
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(ОписаниеОшибки, ОписаниеОшибки(),Символы.ПС) ;
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'В процедуре ВебИнтеграция.СоздатьБронь(Сотрудник, Дата, ВремяНачалаЗаписи, ВремяОкончанияЗаписи, Клиника, ОписаниеОшибки) возникла ошибка. %1%2'"),Символы.ПС,ОписаниеОшибки);
		Возврат ОписаниеОшибки;
...
//  Строка - текст XML.
//
Функция ПолучитьСписокСотрудниковXML(НастройкиОбмена = Неопределено, ДопПараметры = Неопределено) Экспорт
	ОбъектXDTO = ПолучитьСотрудниковXDTO(НастройкиОбмена, ДопПараметры);
	ВебИнтеграцияПереопределяемый.ПолучитьСписокСотрудников_ПередВозвратом(ОбъектXDTO, ДопПараметры);
...
//  Строка - ответ сайту.
//
Функция ПолучитьСписокНоменклатурыИЦен(Клиника, ОписаниеОшибки = "", ДопПараметры = Неопределено) Экспорт
	...
	Попытка 
		...
		Если Не ЗначениеЗаполнено(ОписаниеОшибки) Тогда
			...
			Если Результат Тогда
				ОбъектXDTO = ВебИнтеграция.ПолучитьКаталогУслугXDTO(НастройкиОбмена, ДопПараметры);
				ВебИнтеграцияПереопределяемый.ПолучитьСписокНоменклатурыИЦен_ПередВозвратом(ОбъектXDTO, ДопПараметры);
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(ОписаниеОшибки, ОписаниеОшибки(),Символы.ПС) ;
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'В процедуре ВебИнтеграция.ПолучитьСписокНоменклатурыИЦен(Клиника, ОписаниеОшибки = "") возникла ошибка:'"),Символы.ПС,ОписаниеОшибки);
		Возврат ОписаниеОшибки;
	КонецПопытки;
...
//  Строка - ответ сайту.
//
Функция ОтменитьЗаявку(ЗаявкаСсылка, ДополнительнаяИнформация, ПричинаОтмены, ОписаниеОшибки, ДопПараметры = Неопределено) Экспорт
	...
	Попытка 
		...
		ВебИнтеграцияПереопределяемый.ОтменитьЗаявку_ПослеПометкиНаУдаление(ЗаявкаСсылка, Результат, ОписаниеОшибки, ДопПараметры);
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(ОписаниеОшибки, ОписаниеОшибки(),Символы.ПС) ;
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'В процедуре ВебИнтеграция.ОтменитьЗаявку(Имя, Заявка, ДополнительнаяИнформация, ПричинаОтмены, ОписаниеОшибки) возникла ошибка.'"),Символы.ПС,ОписаниеОшибки);
		Возврат ОписаниеОшибки;
	КонецПопытки;
...
Функция ПолучитьXDTOТекстУкрупненыеГрафикиРаботы(ДатаНачала, ДатаОкончания, ДопПараметры = Неопределено)
	                     
	Если ДопПараметры <> Неопределено Тогда
		СтруктураТаблиц	= УправлениеЗаявками.СформироватьТаблицыДляОбщегоГрафика(ДатаНачала, КонецДня(ДатаОкончания),
							?(ДопПараметры.Свойство("Clinic"),ДопПараметры.Clinic,Неопределено),
							?(ДопПараметры.Свойство("Employees"),ДопПараметры.Employees,Неопределено), , , , Истина);
	Иначе
	...
	ВебИнтеграцияПереопределяемый.ПолучитьУкрупненныйГрафик_ПередВозвратом(ТаблицаГрафиков, ТаблицаСотрудников, ДопПараметры);
...
//
Функция ОбработатьДанныеКлиента(ДанныеКлиента, Филиал = Неопределено, НастройкиОбмена = Неопределено) Экспорт
	...
	Если Не Филиал = Неопределено Тогда
		КлиентВБазе = НайтиКлиента(СтруктураПоиска, Филиал, НастройкиОбмена);
	Иначе
...
Функция СформироватьТаблицыДляОбщегоГрафикаСУчетомНаложенияПериодов(ДатаНачала, ДатаОкончания, ИнтеграцияССайтом = Ложь, ДопПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(ДопПараметры) = Тип("Структура")
		...
		// Передаётся либо структура, либо сама ссылка на справочник (HTTP-сервис)
		Если ТипЗнч(ДопПараметры.НастройкиОбмена) = Тип("Структура") Тогда
	...
	Для Каждого СтрокаТаблицы Из ОбработаннаяТаблица Цикл
		Если НаименованияСотрудников.Получить(СтрокаТаблицы.Сотрудник) = Неопределено Тогда
			НаименованияСотрудников.Вставить(СтрокаТаблицы.Сотрудник, Строка(СтрокаТаблицы.Сотрудник));
		КонецЕсли;
...
Функция ПолучитьСотрудниковXDTO(НастройкиОбменаССайтом = Неопределено, ДопПараметры = Неопределено) Экспорт
	
	ТаблицаСотрудников = ПолучитьТаблицуСотрудников(НастройкиОбменаССайтом, ДопПараметры); 
...
Функция ПолучитьТаблицуСотрудников (НастройкиОбменаССайтом, ДопПараметры = Неопределено) Экспорт
	...
	Если Не НастройкиОбменаССайтом = Неопределено Тогда	
		...
		// Переданные дополнительные отборы
		Если ДопПараметры <> Неопределено Тогда
			ОтборКомпоновщикаНастроек = КомпоновщикНастроек.Настройки.Отбор; 			
			
			// Сравнение ссылки с переданным массивом ссылок
			Если ДопПараметры.Свойство("EmployeeID") И ЗначениеЗаполнено(ДопПараметры.EmployeeID) Тогда			
				ЭлементОтбора = ОтборКомпоновщикаНастроек.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				ЭлементОтбора.Использование = Истина;
				ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Ссылка");
				ЭлементОтбора.ПравоеЗначение = ДопПараметры.EmployeeID;
				Если  Тип(ДопПараметры.EmployeeID) = Тип("Массив") Тогда
			...
			// Сравнение по наименованию ссылки с переданной строкой
			Если ДопПараметры.Свойство("SearchName") И Не ПустаяСтрока(ДопПараметры.SearchName) Тогда
				ЭлементОтбора = ОтборКомпоновщикаНастроек.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				ЭлементОтбора.Использование = Истина;
				ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Ссылка.Наименование");
				ЭлементОтбора.ПравоеЗначение = ДопПараметры.SearchName;
				ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Содержит;			
			КонецЕсли;
			
			// Сравнение по принадлежности к клинике (Подразделению), нежелательный отбор
			Если ДопПараметры.Свойство("Clinic") И Не ПустаяСтрока(ДопПараметры.Clinic) Тогда
				ЭлементОтбора = ОтборКомпоновщикаНастроек.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				ЭлементОтбора.Использование = Истина;
				ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Ссылка.Подразделение.Филиал");
				ЭлементОтбора.ПравоеЗначение = ДопПараметры.Clinic;
				Если  Тип(ДопПараметры.Clinic) = Тип("Массив") Тогда
					ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
				Иначе
					ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				КонецЕсли;			
...
//
Функция ПолучитьКаталогУслуг(НастройкаИнтеграции, ДатаЦен, Знач Прейскурант = Неопределено, ДопПараметры = Неопределено) Экспорт
...
Функция ПолучитьКаталогУслугXDTO(НастройкаОбмена, ДопПараметры = Неопределено) Экспорт
	
	Если ТипЗнч(НастройкаОбмена) = Тип("СправочникВыборка.НастройкиОбменаССайтом")
		Или ТипЗнч(НастройкаОбмена) = Тип("СправочникСсылка.НастройкиОбменаССайтом") Тогда
		ОбменССайтом = Истина;	
	Иначе
		ОбменССайтом = Ложь;
	КонецЕсли;
	...
	ТаблицаКаталога = ПолучитьКаталогУслуг(НастройкаОбмена, ТекущаяДата(), НастройкаОбмена.Прейскурант, ДопПараметры);
...
Функция ПолучитьГрафикиРаботыXDTO(ДатаНачала, ДатаОкончания, ОписаниеОшибки, ИнтеграцияССайтом = Ложь, ДопПараметры = Неопределено, ОписаниеОшибкиParams = "") Экспорт
	...
	ВебИнтеграцияПереопределяемый.ПолучитьГрафикиРаботы_ПередВозвратом(ТаблицаГрафиков, ДопПараметры);
...
//
Функция ПолучитьСтатусЗаявкиДляСайта(Заявка, ОписаниеОшибки, ДопПараметры = Неопределено) Экспорт
	...
	Попытка
		...
		ВебИнтеграцияПереопределяемый.ПолучитьСтатусЗаявки_ПередВозвратом(ОписаниеРезультата,КодСтатуса,ДопПараметры);
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(ОписаниеОшибки, ОписаниеОшибки(),Символы.ПС) ;
		ОписаниеОшибки = СтрШаблон(НСтр("ru = 'В процедуре ВебИнтеграция.ПолучитьСтатусЗаявкиДляСайта(Заявка, ОписаниеОшибки) возникла ошибка.'"),Символы.ПС,ОписаниеОшибки);
		Возврат ОписаниеОшибки;	
	КонецПопытки;
...
Процедура СоздатьЛистОжиданияПоЗаявкеНовогоКлиента(Заявка, ДопПараметры = Неопределено)
	
	Если ДопПараметры = Неопределено Тогда
		НастройкаОбменаССайтом = ПолучитьНастройкиОбменаССайтом();
	Иначе
		НастройкаОбменаССайтом = ДопПараметры.НастройкиОбмена;	
	КонецЕсли;
