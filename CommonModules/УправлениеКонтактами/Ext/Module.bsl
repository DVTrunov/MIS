...
// Периодичность обновления задач
// 
// Возвращаемое значение:
//  Число - секунды.
//
Функция ПериодичностьОбновленияЗадач() Экспорт
...
Процедура СформироватьАвтоматическиеЗадачиПользователя()  Экспорт
	...
	// Требуется ли формирование с учетом времени последнего выполнения актуализации задач
	Если Не ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		СледующаяАктуализация = Константы.ЗадачиПользователей_ПоследняяАктуализация.Значение() + ПериодичностьОбновленияЗадач();
		
		Если СледующаяАктуализация < ТекущаяДата()
			И НачалоДня(СледующаяАктуализация) = НачалоДня(ТекущаяДата())
		Тогда
			Возврат; // Уже обновлено другим пользователем.
		КонецЕсли;
	КонецЕсли;
	...
	// Фиксация времени последнего выполнения актуализации задач
	Если Не ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		УстановитьПривилегированныйРежим(Истина);
		Константы.ЗадачиПользователей_ПоследняяАктуализация.Установить(ТекущаяДата());
