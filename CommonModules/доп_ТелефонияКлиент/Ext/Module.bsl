...
Процедура ОтправитьSMS(Контакт) ЭКСПОРТ
	Если ЗначениеЗаполнено(Контакт) Тогда
		доп_ТелефонияКлиентПереопределяемый.ВыбратьНомерКонтактаИОповестить(Контакт, "Обработка.доп_БитфонОбщее.Форма.ОтправкаСМС", "БитФон_ОтправитьSMS");
	Иначе
		ФормаСМС = ПолучитьФорму("Обработка.доп_БитфонОбщее.Форма.ОтправкаСМС");
		ФормаСМС.Открыть();
	КонецЕсли;
КонецПроцедуры
...
Функция СформироватьНомерСПрефиксами(стрНомерИсх, флагПрямойНабор, стрПрефиксВнешЛинии) ЭКСПОРТ
	стрНомер = доп_ТелефонияКлиентСервер.ОчиститьНомерЦифрыЗвРеш(стрНомерИсх);
	Если НЕ ЗначениеЗаполнено(стрНомер) Тогда
		Возврат "";
	КонецЕсли;
	индексЗвРеш = доп_ТелефонияКлиентСервер.ПолучитьИндексЗвРеш(стрНомер);
	Если (НЕ флагПрямойНабор) И (индексЗвРеш = 0) Тогда
		стрНомер = доп_ТелефонияКлиентСервер.СократитьНомерДляПоиска(стрНомер);
...
#КонецЕсли
	...
	доп_ТелефонияКлиент.ВывестиСообщение("Обновление внешней компоненты '" + стрИмяФайлаСокр + "'...");
...
//
// Параметры:
//  версияСЛ - Число - задает версию ОЛИМП Сервера Лицензий, варианты значения 0 (СЛ 1.6) или 1 (СЛ 2.0).
//  флагСофтфон - Булево - признак вызова для компоненты софтфона или монитора/панели управления ОЛИМП.АТС.
//
Функция ПолучитьПутьБиблиотекиКоннектораСЛ(версияСЛ, флагСофтфон) Экспорт
	...
	Если версияСЛ = 1 Тогда // СЛ 2.0
		...
		стрПутьКоннектора = доп_ТелефонияСервер.ПолучитьПараметрСеансаПутьБиблиотекиКоннектораСЛ20(флагСофтфон);
		...
		Если Не Файл.Существует() Тогда          
			БиблиотекаКоннектораДвоичныеДанные = доп_ТелефонияСервер.ПолучитьБиблиотекуКоннектораСЛ20(Клиент64бит());
			БиблиотекаКоннектораДвоичныеДанные.Записать(стрПутьКоннектора);
			доп_ТелефонияСервер.УстановитьПараметрСеансаПутьБиблиотекиКоннектораСЛ20(флагСофтфон, стрПутьКоннектора);
		КонецЕсли;
...
Функция ПолучитьВерсиюПлатформы() ЭКСПОРТ
	...
	списЗначВерс = доп_ТелефонияКлиентСервер.СтрРазбить(стрВерсияПлатформы, ".");
	Для й=0 По 2 Цикл
...
Процедура ОткрытьФайлИлиСсылку(стрПуть) ЭКСПОРТ
	Если ЗначениеЗаполнено(стрПуть) Тогда
		...
		#Иначе
			доп_ТелефонияКлиентПереопределяемый.ЗапускПрограммы(стрПуть);
		#КонецЕсли
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьФормуВводаНомера(формаРодитель, флагРежимПеревода, ссылкаВнешняяКомпонента, флагПроверкаСтатусаЧерезSIP, стрСвойНомер) ЭКСПОРТ
	...
	фрмВводаНомера = ПолучитьФорму("Обработка.доп_БитАТС.Форма.НаборНомера", парам, формаРодитель);
...
Функция ПолучитьФормуОбработкиВызова(формаРодитель, флагСкрыватьКнопкиОбработкиВызова, стрНомерВходящий, контрагентНайден, контрагентСсылка, контактноеЛицоСсылка, стрНабранныйНомер, стрСвойНомер) ЭКСПОРТ
	...
	//+ОЛИМП.Медицина
	//фрмОбработкаВызова = ПолучитьФорму("Обработка.доп_БитфонОбщее.Форма.ОбработкаВызова", , формаРодитель);
	фрмОбработкаВызова = ПолучитьФорму("Обработка.доп_Битфон.Форма.ОбработкаВызова", , формаРодитель);
	//-ОЛИМП.Медицина
...
Процедура ОткрытьФормуОбработкиДублейНомера(Знач стрНомерТелефонаДубл, формаРодитель) ЭКСПОРТ
	...
	Возврат;
	//-Переопределенное
	стрНомерТелефонаДубл = доп_ТелефонияКлиентСервер.СократитьНомерДляПоиска(доп_ТелефонияКлиентСервер.ОчиститьНомерТолькоЦифры(стрНомерТелефонаДубл));
	доп_ТелефонияКлиент.ВывестиСообщение("Обнаружено несколько контрагентов с номером '" + стрНомерТелефонаДубл + "'. Список дублей открыт в отдельном окне.", формаРодитель);
	ОткрытьФорму("Обработка.доп_БитфонОбщее.Форма.ПоискКонтрагентовПоНомеру", Новый Структура("НомерТелефона", стрНомерТелефонаДубл));
КонецПроцедуры

//
Процедура ОткрытьФормуОтчетаИсторииЗвонков(формаРодитель) ЭКСПОРТ
	формаОтчетПоИсторииЗвонков = ПолучитьФорму("Отчет.доп_ИсторияЗвонков.Форма.ФормаОтчета", , формаРодитель, Ложь);
	...
	Если ПараметрПользователь <> Неопределено Тогда
		ПараметрПользователь.Значение = доп_ТелефонияСервер.ПолучитьТекущегоПользователя();
...
Процедура ОткрытьФормуВыбораНомера(формаРодитель, ссылкаАТС = Неопределено) ЭКСПОРТ
	...
	КонецЕсли;
	формаВыбораТелефона = ПолучитьФорму("Обработка.доп_БитфонОбщее.Форма.ФормаВыбораНомера", парам, формаРодитель);
...
Процедура ОткрытьФормуПросмотраЗвонка(структураПараметровЗвонка) ЭКСПОРТ
	Ключ = доп_ТелефонияСервер.ИсторияЗвонковПолучитьКлюч(структураПараметровЗвонка);
	ПарамЗвонок = Новый Структура("Ключ", Ключ);
	ОткрытьФорму("РегистрСведений.доп_ИсторияЗвонков.Форма.ФормаПросмотраЗвонка", ПарамЗвонок);
КонецПроцедуры
...
Процедура ИзбранноеСоздать(стрИмяСобытияОповещения) ЭКСПОРТ
	фрмНовИзбранное = ПолучитьФорму("Справочник.доп_БитфонИзбранное.Форма.ФормаЭлементаБезПользователя");
	фрмНовИзбранное.ИмяСобытияОповещенияПриИзменении = стрИмяСобытияОповещения;
	доп_ТелефонияКлиентПереопределяемый.ОткрытьФормуСБлокировкойВладельца(фрмНовИзбранное);
КонецПроцедуры
...
Процедура ИзбранноеСкопировать(форма, стрИмяСобытияОповещения) ЭКСПОРТ
	...
	Если текущСтрокаИзбр <> Неопределено Тогда
		...
		нф = ПолучитьФорму("Справочник.доп_БитфонИзбранное.Форма.ФормаЭлементаБезПользователя", ПарамИзбр);
		нф.ИмяСобытияОповещенияПриИзменении = стрИмяСобытияОповещения;
		доп_ТелефонияКлиентПереопределяемый.ОткрытьФормуСБлокировкойВладельца(нф);
	КонецЕсли;
КонецПроцедуры
...
Процедура ИзбранноеИзменить(форма, стрИмяСобытияОповещения) ЭКСПОРТ
	...
	Если текущСтрокаИзбр <> Неопределено Тогда
		...
		формаТекущ = ПолучитьФорму("Справочник.доп_БитфонИзбранное.Форма.ФормаЭлементаБезПользователя", ПарамИзбр); 
		формаТекущ.ИмяСобытияОповещенияПриИзменении = стрИмяСобытияОповещения;
		доп_ТелефонияКлиентПереопределяемый.ОткрытьФормуСБлокировкойВладельца(формаТекущ);
	КонецЕсли;
КонецПроцедуры
...
Процедура ИзбранноеУдалить(форма, стрИмяСобытияОповещения) ЭКСПОРТ
	...
	Если текущСтрокаИзбр <> Неопределено Тогда
		...
		КонецЕсли;
		доп_ТелефонияКлиентПереопределяемый.ПоказВопрос(стрВопрос, РежимДиалогаВопрос.ДаНет, , , , стрИмяСобытияОповещения, текущ);
	КонецЕсли;
КонецПроцедуры
