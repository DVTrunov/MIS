...
Функция ПолучитьПредставлениеИнфекционныеЗаболевания(ТаблицаДиагнозов) Экспорт
	
	Результат = "";
	Шаблон= "%2: %1";
	
	Для Каждого СтрокаРезультат Из ТаблицаДиагнозов Цикл 
		
		ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(Результат, СтрШаблон(Шаблон, Формат(СтрокаРезультат.Дата, "ДФ=yyyy"), СтрокаРезультат.Диагноз), Символы.ПС);
...
Функция ПолучитьПредставлениеСведенияОПривиках(ТаблицаПрививок) Экспорт
	
	Результат = "";
	ШаблонСНомером = "%2 [%3]: %1";
	ШаблонБезНомера = "%2: %1";
	
	Для Каждого СтрокаРезультат Из ТаблицаПрививок Цикл 
		
		Если ЗначениеЗаполнено(СтрокаРезультат.ПорядковыйНомер) Тогда
			Шаблон = ШаблонСНомером;
			ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(Результат, СтрШаблон(Шаблон, Формат(СтрокаРезультат.ДатаВакцинации, "ДФ=yyyy"), СтрокаРезультат.ГруппаЗаболеваний, СтрокаРезультат.ПорядковыйНомер), Символы.ПС);
		Иначе
			Шаблон = ШаблонБезНомера;
			ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(Результат, СтрШаблон(Шаблон, Формат(СтрокаРезультат.ДатаВакцинации, "ДФ=yyyy"), СтрокаРезультат.ГруппаЗаболеваний), Символы.ПС);
		КонецЕсли;
...
Функция ТекстЗапросаДляВыбораПрофессии() Экспорт
	...
	ТекстЗапроса = 
		...
		|ГДЕ
		|	СправочникКлассификаторыМинЗдрава.Ссылка В
		|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|				СправочникКлассификаторыМинЗдрава.Ссылка КАК Ссылка
		|			ИЗ
		|				Справочник.КлассификаторыМинЗдрава КАК СправочникКлассификаторыМинЗдрава
		...
		|					ПО
		|						СправочникКлассификаторыМинЗдрава.ВидКлассификатора = ЗНАЧЕНИЕ(Перечисление.ВидыКлассификаторовМинЗдрава.ПрофессииРабочихИДолжностейСлужащих)
		|							И УникальныеПрофессии.УИДЕГИСЗ = СправочникКлассификаторыМинЗдрава.УИДЕГИСЗ
		|							И НЕ СправочникКлассификаторыМинЗдрава.Архив)";
