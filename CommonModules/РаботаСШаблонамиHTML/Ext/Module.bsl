...
Функция ДобавитьРисункиКартинок(ТекстДляПечати, Документ, ИндексШаблона) Экспорт
	...
	Для Каждого СтрокаРисунка Из НЗ Цикл
		Попытка
			...
			ДДРисунка = доп_РаботаСФайлами.ПолучитьДвоичныеДанныеФайлаРедактируемогоЭлемента(СтрокаРисунка.ПутьКФайлу, СтрокаРисунка.Файл);
...
Функция ВставитьДобавленныеИзображения(ТекстДляПечати, Документ, ИндексШаблона) Экспорт
	...
	Для Каждого СтрокаДобавленногоИзображения Из НЗ Цикл
		Попытка
			...
			ДДИзображения = доп_РаботаСФайлами.ПолучитьДвоичныеДанныеФайлаРедактируемогоЭлемента(СтрокаДобавленногоИзображения.ПутьКФайлу, СтрокаДобавленногоИзображения.Файл);
...
//  ТаблицаПараметровРеквизит		 - ТаблицаЗначений	 - таблица параметров на форме для использования в расчете.
// 
// Возвращаемое значение:
//   Произвольный.
//
Функция ВычислитьЗначениеБлокаHTML(ИдВычисляемогоЗначения, Знач Объект, Знач ПланЛеченияРеквизит = Неопределено, Знач ТаблицаПараметровРеквизит = Неопределено) Экспорт
	
	Перем ПланЛечения, ТаблицаПараметров;
	...
	Если ТипЗнч(ТаблицаПараметровРеквизит) = Тип("ТаблицаЗначений") Тогда
		ТаблицаПараметров = ТаблицаПараметровРеквизит;
	ИначеЕсли ТипЗнч(ТаблицаПараметровРеквизит) = Тип("ДанныеФормыКоллекция") Тогда
		ТаблицаПараметров = ТаблицаПараметровРеквизит.Выгрузить();
	Иначе
	...
	Попытка
		...
		Возврат ВычислитьЗначениеБлокаHTMLШаблона(ВычисляемоеЗначениеБлока, Объект, ПланЛечения, ТаблицаПараметров);
	Исключение
...
Процедура ВычислитьЗначениеПараметраРекурсивно(СтрокаПараметр, ТаблицаПараметров, ЗависимыеПараметры, РасчитанныеПараметры, Объект, ПланЛечения = Неопределено, НеРасчитыватьРодительские = Ложь)
	...
	Если Не НеРасчитыватьРодительские Тогда
		...
		Для Каждого РодПараметр Из РодительскиеПараметры Цикл
			...
			СтрокаРодПараметр = ОбщегоНазначенияБИТКлиентСервер.ПерваяСтрокаОтбораТаблицы(ТаблицаПараметров, Отбор);
	...
	Для Каждого СтрокаЗависимыйПараметр Из ЗависимыеПараметры.НайтиСтроки(Новый Структура("Параметр", СтрокаПараметр.Параметр)) Цикл
		...
		СтрокаЗависимый = ОбщегоНазначенияБИТКлиентСервер.ПерваяСтрокаОтбораТаблицы(ТаблицаПараметров, Отбор);
...
Функция ВычислитьЗначениеПараметра(ПараметрОперации_Отказ, Знач ВычисляемоеЗначение, Знач СтрокаПараметр, Знач ТаблицаПараметров, Знач Объект, Знач ПланЛечения)
	...
	Для Каждого ЗависимыйПараметр Из СписокПеременных Цикл
		...
		ЗначениеПараметра = ОбщегоНазначенияБИТКлиентСервер.ПерваяСтрокаОтбораТаблицы(ТаблицаПараметров, Отбор);
	...
	Попытка
		Алгоритм = Справочники.ВычисляемыеЗначенияПараметров.ПолучитьАлгоритмВычисленияЗначения(ВычисляемоеЗначение);
 		Выполнить(Алгоритм);
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки());
	КонецПопытки;
...
Процедура ВыгрузитьТаблицуПараметров(ВладелецПараметровОбъект, Знач ТаблицаПараметров, Отказ = Ложь, ШаблоныПриема = Неопределено, ЭтоАнамнез = Ложь, ПараметрыЗаписи = Неопределено, ПараметрыТаблицШаблонов = Неопределено) Экспорт
	...
	Для Каждого СтрокаПараметра Из ТаблицаПараметров Цикл
		...
		Если ВладелецПараметровОбъект.ДополнительныеСвойства.Свойство("ЭтоАвтосохранение") Тогда
			Продолжить; 
		КонецЕсли;
		// Конец добавления
			...
			Тогда
				...
				КонецЕсли;
				Отказ = Истина;
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru='Параметр %1 не заполнен. Данный параметр обязателен к заполнению'"), ПредставлениеПараметраШаблона(ПараметрСсылка)));
			Иначе
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьЗначенияПараметровПоУмолчанию(Знач ТаблицаПараметров, Знач ТаблицаЗависимыхПараметров, Знач Объект, Параметр = Неопределено, ИдентификаторСтрокиШаблона = Неопределено, ДополнительныйОтборПараметров = Неопределено, РассчитатьИсходныйПараметр = Ложь) Экспорт
	...
	Если ТипЗнч(ДополнительныйОтборПараметров) = Тип("Структура") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ОтборПараметров, ДополнительныйОтборПараметров)
	КонецЕсли;
...
Функция ПараметрыЭталонныхСтрокВсехТаблицШаблона(ТекстШаблона) Экспорт
	...
	Для Каждого ЭлементТаблицы Из ЭлементыТаблиц Цикл
		...
		ЭлементыПараметров = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ЭталоннаяСтрока, , ТегиПараметров);
...
// Возвращает представление парамера HTML 
//
// Параметры:
//  Параметр - ПланВидовХарактеристикСсылка.ПараметрыHTML	 - Параметр
// 
// Возвращаемое значение:
// Строка  -  Представление параметра.
//
Функция ПредставлениеПараметраШаблона(Параметр) Экспорт
	
	Возврат ?(ЗначениеЗаполнено(Параметр.НаименованиеДляВрача), СокрЛП(Параметр.НаименованиеДляВрача), Строка(Параметр));
...
Процедура СохранитьРисункиДокумента(ДокументОбъект, РисункиКартинок, ШаблоныПриема) Экспорт
	...
	Для Каждого СтрокаРисунка Из РисункиКартинок Цикл
		...
		СтруктураСохраненияФайла = доп_РаботаСФайлами.ПолучитьСтруктуруСохраненияФайлаРедактируемогоЭлемента(ПолучитьИзВременногоХранилища(СтрокаРисунка.АдресРисунка), НоваяЗапись);
...
Процедура СохранитьДобавленныеИзображенияДокумента(ДокументОбъект, ДобавленныеИзображения, ШаблоныПриема) Экспорт
	...
	Для Каждого СтрокаИзображения Из ДобавленныеИзображения Цикл
		...
		СтруктураСохраненияФайла = доп_РаботаСФайлами.ПолучитьСтруктуруСохраненияФайлаРедактируемогоЭлемента(ПолучитьИзВременногоХранилища(СтрокаИзображения.АдресИзображения), НоваяЗапись);
		ЗаполнитьЗначенияСвойств(НоваяЗапись, СтруктураСохраненияФайла, "Файл, ПутьКФайлу");
	КонецЦикла;
...
Функция СформироватьКолонтитулыЭЦПШаблонов(ПриемСсылка, Знач ТаблицаПодписей = Неопределено, Знач ШаблоныОсмотра = Неопределено, Знач ЭМД = Неопределено, Знач РолиИПодписиЭМД = Неопределено) Экспорт
	...
	Если ТаблицаПодписей = Неопределено Тогда
		...
		тзПодписей = доп_СертификатыЭЦП.ПолучитьЭлектронныеПодписи(ПриемСсылка,,,,, Истина);
		...
		доп_СертификатыЭЦП.ЗаполнитьУИДыЭМД(ТаблицаПодписей, ПриемСсылка);
...
Функция ЭталонныеСтрокиТаблиц(ТекстШаблона) Экспорт
	...
	ЭлементыИнтерфейсов = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(РаботаСDOMКлиентСервер.СоздатьДокументDOM(ТекстШаблона), , , "data-bit-interface-id");
	Для Каждого ЭлементИнтерфейса Из ЭлементыИнтерфейсов Цикл
		...
		Для Каждого СтруктураНастройкиТаблицы Из СтруктурыНастроекТаблиц Цикл
			...
			СтруктураТаблицы.Вставить("ИдентификаторТаблицы",	 СтруктураНастройкиТаблицы.Идентификатор);
			СтруктураТаблицы.Вставить("ИдентификаторИнтерфейса", ИдентификаторИнтерфейса);
			СтруктураТаблицы.Вставить("ИмяТаблицы",				 СтруктураНастройкиТаблицы.ИмяТаблицы);
			СтруктураТаблицы.Вставить("ПараметрыТаблицы",		 ПараметрыТаблицы);
			ЭталонныеСтрокиТаблиц.Добавить(СтруктураТаблицы);
		КонецЦикла;
	КонецЦикла;
...
Функция ПараметрыЭталонныхСтрокТаблиц(ЭталонныеСтрокиТаблиц, ТекстШаблона) Экспорт
	...
	СтруктурыИнтерфейсов = Новый Массив;
	СтруктурыТаблиц = Новый Массив;
	НомераЭталонныхСтрок = Новый Соответствие;
	
	Для Каждого СтруктураЭталоннойСтроки Из ЭталонныеСтрокиТаблиц.ВыгрузитьЗначения() Цикл
		Если СтруктураЭталоннойСтроки.Свойство("НомерЭталоннойСтроки") Тогда
			СтруктурыТаблиц.Добавить(СтруктураЭталоннойСтроки);
			НомераЭталонныхСтрок.Вставить(СтруктураЭталоннойСтроки.ИдентификаторТаблицы, СтруктураЭталоннойСтроки.НомерЭталоннойСтроки);
		ИначеЕсли СтруктураЭталоннойСтроки.Свойство("ИдентификаторИнтерфейса") Тогда
			СтруктурыИнтерфейсов.Добавить(СтруктураЭталоннойСтроки);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого СтруктураИнтерфейса Из СтруктурыИнтерфейсов Цикл
		ПараметрыЭталонныхСтрокТаблиц.Добавить(Новый Структура("ИдентификаторТаблицы, ПараметрыЭталоннойСтроки", СтруктураИнтерфейса.ИдентификаторТаблицы, СтруктураИнтерфейса.ПараметрыТаблицы));
	КонецЦикла;
	
	Для Каждого СтруктураТаблицы Из СтруктурыТаблиц Цикл
		ДокументHTML = РаботаСDOMКлиентСервер.СоздатьДокументDOM(ТекстШаблона);
		ЭлементТаблицы = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML, СтруктураТаблицы.ИдентификаторТаблицы, "table", "data-bit-dyntable-id", Истина);
		Если ЭлементТаблицы = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ОбработанныеВложенныеТаблицы = Новый Массив;
		ВложеннаяТаблица = ПолучитьСледующуюВложеннуюТаблицу(ЭлементТаблицы, ОбработанныеВложенныеТаблицы);
		
		Пока ВложеннаяТаблица <> Неопределено Цикл
			
			ИдентификаторВложеннойТаблицы = ВложеннаяТаблица.ПолучитьАтрибут("data-bit-dyntable-id");
			
			НомерЭталоннойСтрокиВложеннойТаблицы = НомераЭталонныхСтрок.Получить(ИдентификаторВложеннойТаблицы);
			Если НомерЭталоннойСтрокиВложеннойТаблицы <> Неопределено Тогда
				ТелоВложеннойТаблицы = ВложеннаяТаблица.ПолучитьТелоТаблицы()[0];
				Для Каждого СтрокаВложеннойТаблицы Из ТелоВложеннойТаблицы.ДочерниеУзлы Цикл
					Если СтрокаВложеннойТаблицы.ИндексСтроки = НомерЭталоннойСтрокиВложеннойТаблицы - 1 Тогда
						Попытка
							СтрокаВложеннойТаблицы.РодительскийУзел.УдалитьДочерний(СтрокаВложеннойТаблицы);
						Исключение КонецПопытки;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			ОбработанныеВложенныеТаблицы.Добавить(ИдентификаторВложеннойТаблицы);
			ВложеннаяТаблица = ПолучитьСледующуюВложеннуюТаблицу(ЭлементТаблицы, ОбработанныеВложенныеТаблицы);
		КонецЦикла;
		
		ЭлементЭталоннойСтроки = ЭлементТаблицы.ПолучитьТелоТаблицы()[0].ДочерниеУзлы[СтруктураТаблицы.НомерЭталоннойСтроки-1];
		...
		ПараметрыЭталонныхСтрокТаблиц.Добавить(Новый Структура("ИдентификаторТаблицы, ПараметрыЭталоннойСтроки", СтруктураТаблицы.ИдентификаторТаблицы, ПараметрыЭталоннойСтроки));
	КонецЦикла;
...
Функция ПолучитьСтруктуруЗначенийВсехПараметровПредыдущегоПриема(Знач Объект, Знач ТаблицаПараметров, Знач ВерсииШаблонов) Экспорт
	
	СтруктураЗначений = Новый Структура;
	СтруктураЗначений.Вставить("ЗначенияПредыдущие", ПолучитьЗначенияПараметровПредыдущегоПриема(ТаблицаПараметров, Объект));
	СтруктураЗначений.Вставить("ЗначенияТаблицПредыдущие", СформироватьЗначенияДанныхТаблицПредыдущегоОсмотра(Объект.МедицинскаяКарта, Объект.Ссылка, Объект.Дата, ВерсииШаблонов, Объект.ШаблоныПриема));
...
Функция СформироватьЗначенияДанныхТаблицПредыдущегоОсмотра(МедКарта, 
															ЗаполняемыйПрием, 
															ДатаДокумента, 
															ВерсииШаблонов, 
															ШаблоныПриема, 
															ПриемОснование=Неопределено, 
															ШаблонОснование=Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(ПриемОснование) Тогда
		Запрос = ПолучитьЗапросЗначениеДаннхыТаблицОсмотраПоПриему(ПриемОснование, ШаблонОснование, ДатаДокумента, ВерсииШаблонов);
	Иначе
		Запрос = ПолучитьЗапросЗначенияДанныхТаблицОсмотра(МедКарта, ЗаполняемыйПрием, ДатаДокумента, ВерсииШаблонов);
	КонецЕсли;
	...
	Пока ВыборкаШаблонов.Следующий() Цикл
		...
		Пока ВыборкаТаблиц.Следующий() Цикл
			...
			ВыборкаСтрокТаблицы = ВыборкаТаблиц.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаСтрокТаблицы.Следующий() Цикл
				
				ИдентификаторСтроки = ВыборкаСтрокТаблицы.ИдентификаторСтроки;
				Выборка = ВыборкаСтрокТаблицы.Выбрать();
				...
				Пока Выборка.Следующий() Цикл
					...
					Если Не СтрокаОбработана Тогда
						...
						Если ПараметрыЭталоннойСтрокиТаблицы.Количество() > 0 Тогда
							Для Каждого ПараметрЭталоннойСтроки Из ПараметрыЭталоннойСтрокиТаблицы Цикл
								...
								ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка, , "Значение,Параметр");
					...
					Если Выборка.Параметр <> ПланыВидовХарактеристик.ПараметрыHTML.ПустаяСсылка() Тогда
						СтруктураПоиска = Новый Структура("НомерСтрокиТаблицы, Параметр, ИдентификаторСтроки, ИдентификаторРодительскойСтроки");
						ЗаполнитьЗначенияСвойств(СтруктураПоиска, Выборка);
						СтруктураПоиска.Вставить("ИдентификаторТаблицы", ИдентификаторТаблицы);
						
						СтрокиПараметраВыборки = тзПараметрыТаблицТекущегоШаблона.НайтиСтроки(СтруктураПоиска);
						Если СтрокиПараметраВыборки.Количество() > 0 Тогда
							ТекущаяСтрока = СтрокиПараметраВыборки[0];
							Если ТипЗнч(Выборка.Значение) = Тип("СправочникСсылка.СтрокиЭМКНеограниченнойДлины") Тогда
								ТекущаяСтрока.Значение = Выборка.Значение.Значение;
							Иначе
								ТекущаяСтрока.Значение = Выборка.Значение;
							КонецЕсли;
							ТекущаяСтрока.Представление = РаботаСШаблонамиHTMLКлиентСервер.ПолучитьПредставлениеПараметра(Выборка, Выборка.ФорматнаяСтрока);
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
...
Процедура ЗаполнитьЗначенияПараметровИзСтрокиВнутр(ОбновленныеЗначенияПараметров, ЗначенияДляПолученияИзСтрокиВнутр) Экспорт
	...
	Для Каждого КлючЗначения Из ЗначенияДляПолученияИзСтрокиВнутр Цикл
		Для Каждого СтруктураПараметра Из ОбновленныеЗначенияПараметров Цикл
			Если СтруктураПараметра.Значение = КлючЗначения.Ключ Тогда
				...
				СтруктураПараметра.Представление = РаботаСШаблонамиHTMLКлиентСервер.ПолучитьПредставлениеПараметра(СтруктураПараметра, КлючЗначения.Значение.ФорматнаяСтрока);
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
// Преобразовывает значения из массива значений во внутренние строки, возвращая результаты в виде соответствия
//
// Параметры:
//  ЗначенияПараметров	 - Массив	 - Значения параметров
// 
// Возвращаемое значение:
//  Соответствие - Значения параметров строкой. Ключ - значение параметра, Значение - значение параметра строкой
//
Функция ПреобразоватьЗначенияПараметровВСтрокуВнутр(ЗначенияПараметров) Экспорт
	
	ЗначенияСтрокой = Новый Соответствие;
	Для Каждого Значение Из ЗначенияПараметров Цикл
		ЗначенияСтрокой.Вставить(Значение, ПреобразоватьЗначениеВСтрокуВнутр(Значение));
	КонецЦикла;
...
// Структуру значений параметров приема по шаблону или индексу шаблона.
//
// Параметры:
//  Объект				 - ДанныеФормыСтруктура, ДокументОбъект.Прием	 - объект документа Прием.
//  ТаблицаПараметров	 - Массив	 - заполняемые строки таблицы параметров, например на форме.
//  ВерсииШаблонов		 - Массив - ссылки на шаблон или его версию
//  ПриемОснование		 - ДокументСсылка.Прием	 - прием-основание.
//  ШаблонОснование		 - Число, СправочникСсылка.ШаблоныHTML	 - индекс строки шаблона в приеме-основании или ссылка на шаблон.
// 
// Возвращаемое значение:
//  Структура. 
//
Функция ЗначенияПараметровДругогоПриемаДляЗаполнения(Знач Объект, Знач ТаблицаПараметров, Знач ВерсииШаблонов, Знач ПриемОснование, Знач ШаблонОснование) Экспорт
	
	СтруктураЗначений = Новый Структура;
	СтруктураЗначений.Вставить("ЗначенияПредыдущие", ПолучитьЗначенияПараметровПредыдущегоПриема(ТаблицаПараметров, Объект, ПриемОснование, ШаблонОснование));
	СтруктураЗначений.Вставить("ЗначенияТаблицПредыдущие", СформироватьЗначенияДанныхТаблицПредыдущегоОсмотра(Объект.МедицинскаяКарта, 
																												Объект.Ссылка, 
																												Объект.Дата, 
																												ВерсииШаблонов, 
																												Объект.ШаблоныПриема,
																												ПриемОснование, 
																												ШаблонОснование));
...
Функция ПолучитьЗначенияПараметровПредыдущегоПриема(ТаблицаПараметров, Объект, ПриемОснование = Неопределено, ШаблонОснование = Неопределено)
	
	Если ЗначениеЗаполнено(ПриемОснование) Тогда
		ПредыдущиеЗначения = СформироватьЗначенияОсмотраПоПриему(ПриемОснование, ШаблонОснование, Истина);
	Иначе
		ПредыдущиеЗначения = СформироватьЗначенияПредыдущегоОсмотра(Объект.МедицинскаяКарта, Объект.Ссылка, ТаблицаПараметров.Выгрузить().ВыгрузитьКолонку("Параметр"), Истина);
	КонецЕсли;
	...
	Для Каждого СтрокаПараметра Из ТаблицаПараметров Цикл
		...
		СтрокаШаблона = Объект.ШаблоныПриема.НайтиПоИдентификатору(СтрокаПараметра.ИдентификаторСтрокиШаблона);
		
		ПараметрыЭталонныхСтрокТаблицШаблона = ПараметрыЭталонныхСтрокТаблицШаблонов.Получить(СтрокаШаблона.ШаблонПриема);
		Если ПараметрыЭталонныхСтрокТаблицШаблона = Неопределено Тогда
			ПараметрыЭталонныхСтрокТаблицШаблона = Новый Массив;
			...
			Для Каждого ЭлементПараметровТаблицы Из ПараметрыЭталонныхСтрокТаблиц Цикл
				Для Каждого ПараметрЭталоннойСтроки Из ЭлементПараметровТаблицы.Значение.ПараметрыЭталоннойСтроки Цикл
					ПараметрыЭталонныхСтрокТаблицШаблона.Добавить(ПараметрЭталоннойСтроки);
				КонецЦикла;
			КонецЦикла;
			ПараметрыЭталонныхСтрокТаблицШаблонов.Вставить(СтрокаШаблона.ШаблонПриема, ПараметрыЭталонныхСтрокТаблицШаблона);
		КонецЕсли;
		
		Если ПараметрыЭталонныхСтрокТаблицШаблона.Найти(СтрокаПараметра.Параметр) <> Неопределено Тогда
			// Параметр из скрытой строки многострочной таблицы
			Продолжить;
		КонецЕсли;
...
Функция СформироватьЗначенияОсмотраПоПриему(Прием, ШаблонПриема, ТолькоПараметрыЗаполняемыеПредыдущими = Ложь)
	...
	Запрос.Текст =
		...
		|	РегистрСведений.ЗначенияПараметровHTML КАК ЗначенияПараметровHTML
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Прием.ШаблоныПриема КАК ПриемШаблоныПриема
		|		ПО ПриемШаблоныПриема.Ссылка = &Прием
		|			И ЗначенияПараметровHTML.Документ = &Прием
		|			И (&ШаблонПриема = НЕОПРЕДЕЛЕНО ИЛИ ПриемШаблоныПриема.ШаблонПриема = &ШаблонПриема)
		|			И (&ИндексШаблона = НЕОПРЕДЕЛЕНО ИЛИ ПриемШаблоныПриема.НомерСтроки = (&ИндексШаблона + 1))
		|			И (ЗначенияПараметровHTML.ИндексШаблона = ПриемШаблоныПриема.НомерСтроки - 1)
		| 			И ЗначенияПараметровHTML.Документ = ПриемШаблоныПриема.Ссылка
		|			И (ЗначенияПараметровHTML.ИндексШаблона = ПриемШаблоныПриема.НомерСтроки - 1)
		|ГДЕ
		|	НЕ(&ТолькоПараметрыЗаполняемыеПредыдущими
		|				И ЗначенияПараметровHTML.Параметр.НеЗаполняетсяПредыдущимиЗначениями)";
	
	Если ТипЗнч(ШаблонПриема) = Тип("Число") Тогда
		ПараметрИндексШаблона = ШаблонПриема;
		ПараметрШаблон = Неопределено;
	Иначе
		ПараметрНомерШаблона = Неопределено;
		ПараметрШаблон = ШаблонПриема;
	КонецЕсли;
	
	Запрос.УстановитьПараметр("Прием", Прием);
	Запрос.УстановитьПараметр("ИндексШаблона", ПараметрИндексШаблона);
	Запрос.УстановитьПараметр("ШаблонПриема", ПараметрШаблон);
	Запрос.УстановитьПараметр("ТолькоПараметрыЗаполняемыеПредыдущими", ТолькоПараметрыЗаполняемыеПредыдущими);
...
Функция ПолучитьСледующуюВложеннуюТаблицу(ЭлементТаблицы, ОбработанныеВложенныеТаблицы)
	...
	Для Каждого ВложеннаяТаблица Из ВложенныеТаблицы Цикл
		Если ОбработанныеВложенныеТаблицы.Найти(ВложеннаяТаблица.ПолучитьАтрибут("data-bit-dyntable-id")) = Неопределено Тогда
...
Функция ПолучитьЗапросЗначенияДанныхТаблицОсмотра(МедКарта, ЗаполняемыйПрием, ДатаДокумента, ВерсииШаблонов)
...
Функция ПолучитьЗапросЗначениеДаннхыТаблицОсмотраПоПриему(Прием, ШаблонПриема, ДатаДокумента, ВерсииШаблонов)
	...
	Запрос.Текст = 
		...
		|ВЫБРАТЬ
		|	ДатыДействияВерсийШаблонов.ШаблонПриема КАК ШаблонПриема,
		|	тчШаблоныПриема.Ссылка КАК ПредыдущийПрием,
		|	МАКСИМУМ(тчШаблоныПриема.НомерСтроки) - 1 КАК ИндексШаблона
		|ПОМЕСТИТЬ ИндексыШаблоновВТаблицеПриемов
		|ИЗ
		|	ДатыДействияВерсийШаблонов КАК ДатыДействияВерсийШаблонов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Прием.ШаблоныПриема КАК тчШаблоныПриема
		|		ПО (тчШаблоныПриема.ШаблонПриема = ДатыДействияВерсийШаблонов.ШаблонПриема)
		|			И (тчШаблоныПриема.Ссылка = &Прием)
		|			И (тчШаблоныПриема.ШаблонПриема = &ШаблонПриема
		|				ИЛИ &ШаблонПриема = НЕОПРЕДЕЛЕНО)
		|
		|СГРУППИРОВАТЬ ПО
		|	ДатыДействияВерсийШаблонов.ШаблонПриема,
		|	тчШаблоныПриема.Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ДатаДокумента",	ДатаДокумента);
	Запрос.УстановитьПараметр("Прием",	Прием);
	Запрос.УстановитьПараметр("ШаблонПриема", ШаблонПриема);
