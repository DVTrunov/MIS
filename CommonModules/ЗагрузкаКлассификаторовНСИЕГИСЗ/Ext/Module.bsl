...
Функция ПолучитьСоединениеССервисом(АдресСервера, ОписаниеОшибки = "")
	...
	Исключение
		...
		ОписаниеОшибки = ИнформацияОбОшибке.Причина;
		ОбщиеМеханизмы.ОбработатьОшибку(ИнформацияОбОшибке, Ложь);
		Возврат Неопределено;
	КонецПопытки;
...
Функция ЗапроситьКлассификаторЦеликом(OID, Версия, МенеджерСправочника, ВидКлассификатора, ТестовыйКонтур)
	...
	Результат = ВыполнитьОбменДанными(ТекстОперации, ИмяОперации, ТестовыйКонтур);
	Если ТипЗнч(Результат) = Тип("ОбъектXDTO") Тогда
		ТаблицаДанных = ПрочитатьДанныеКлассификатораИзXDTO(МенеджерСправочника, Результат, ВидКлассификатора, Версия);
	КонецЕсли;
...
Функция ПрочитатьДанныеКлассификатораИзXDTO(МенеджерСправочника, Результат, ВидКлассификатора, Версия)
	...
	Если ЗначениеЗаполнено(ВидКлассификатора) Тогда
		СоответствияПолей = МенеджерСправочника.ПолучитьСопоставленийРеквизитовИXMLСправочникаЕГИСЗ(ВидКлассификатора, Версия);
	Иначе
		Попытка
			СоответствияПолей = МенеджерСправочника.ПолучитьСопоставленийРеквизитовИXMLСправочникаЕГИСЗ(Версия);
		Исключение
	...
	Попытка
		Если ЗначениеЗаполнено(ВидКлассификатора) Тогда
			ДопАтрибуты = МенеджерСправочника.ПолучитьДополнительныеАтрибуты(ВидКлассификатора, Версия);
		Иначе
			Попытка
				ДопАтрибуты = МенеджерСправочника.ПолучитьДополнительныеАтрибуты(Версия);
			Исключение
...
Функция ПолучитьКлассификаторПоОтбору(OID, МенеджерСправочника, ПараметрыОтбора = Неопределено,
	...
	Результат = ПрочитатьДанныеКлассификатораИзXDTO(МенеджерСправочника, XDTOРезультат, ВидКлассификатора, Версия);
...
Функция ДополнитьТаблицуИПолучитьНеактуальные(ПараметрыСправочника, ТаблицаКлассификатор, ОпределятьАрхивные = Истина, ВыполняетсяЗагрузкаОбновлений = Ложь) Экспорт
	...
	Если Не ВыполняетсяЗагрузкаОбновлений Тогда
	...
	// Заполнение структуры по реквизитам
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Ссылка,
	...
	// Для поиска в базе по значению атрибута
	...
	Для Каждого СтрокаКлассификатора Из ТаблицаКлассификатор Цикл
		...
		Если НайденныеЭлементы.Количество() <> 0 Тогда
			...
			// Для обновления следует обновить все, пропускем алгоритм определения
			Если Не ВыполняетсяЗагрузкаОбновлений Тогда
				...
				КонецЦикла;
				Если ЕстьДопАтрибуты Тогда
					Если СтрокаКлассификатора["Атрибуты"].Количество() <> ДанныеЭлементаБД["Атрибуты"].Количество() Тогда
						СтрокаКлассификатора.СледуетОбновить = Истина;
					Иначе
						Для Каждого Атрибут Из СтрокаКлассификатора["Атрибуты"] Цикл
							...
							Если СтарыйАтрибут = Неопределено
								Или СтарыйАтрибут.Значение <> Атрибут.Значение И ОдинИсточник
							Тогда
								СтрокаКлассификатора.СледуетОбновить = Истина;
								Прервать;
							КонецЕсли;
	...
	Если ОпределятьАрхивные Тогда
		// Определяем по передаваемым тегам OPER = "d"
		Если ВыполняетсяЗагрузкаОбновлений Тогда
			АрхивныеСтроки = ТаблицаКлассификатор.НайтиСтроки(Новый Структура("OPER", "d"));
			ТаблицаАрхивные = ТаблицаКлассификатор.Скопировать(АрхивныеСтроки);
			
			// Если не удалить, то архивный элемент загрузится повторно при обходе дерева
			Для Каждого АрхивнаяСтрока Из АрхивныеСтроки Цикл
				ТаблицаКлассификатор.Удалить(АрхивнаяСтрока);
			КонецЦикла;
			
			Возврат ТаблицаАрхивные;
		Иначе
			// При полной загрузке смотрим запросом на те элементы, которых нет в новой версии справочника
			Запрос.Текст = 
				"ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура ЗаписатьНовыеЭлементыСправочника(Параметры, ТаблицаКлассификатор, ДеревоКлассификатора = Неопределено, 
	Знач ДополнительныеСвойства = Неопределено, РежимОбновления = Неопределено, ТестовыйКонтур = Ложь, ВыполняетсяЗагрузкаОбновлений = Ложь) Экспорт
	...
	Если ВыполняетсяЗагрузкаОбновлений Тогда 
		ДополнитьТаблицуКлассификаторРодителями(ТаблицаКлассификатор, Параметры);
	КонецЕсли;
	...
	Если ДеревоКлассификатора = Неопределено Тогда
		...
		ТЗНеактуальное = ДополнитьТаблицуИПолучитьНеактуальные(Параметры, ТаблицаКлассификатор, , ВыполняетсяЗагрузкаОбновлений);
...
Процедура ДополнитьТаблицуКлассификаторРодителями(ТаблицаКлассификатор, Параметры)
	
	Если Параметры.Свойство("ВидКлассификатора") Тогда
		ВидКлассификатора = Параметры.ВидКлассификатора;
	Иначе
		ПараметрыКлассификатора = ИнтеграцияЕГИСЗСерверПовтИсп.ПолучитьПараметрыСправочникаЕГИСЗ(Параметры.OID);
		Если ПараметрыКлассификатора.Свойство("ВидКлассификатора") Тогда 
			ВидКлассификатора = ПараметрыКлассификатора.ВидКлассификатора;
		Иначе
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	// Ищем строки, у которых нет родителя в таблице. Рекурсивно добавляем для корректного формирования дерева
	Для Каждого СтрокаТЗ Из ТаблицаКлассификатор Цикл
		Если СтрокаТЗ.УИДЕГИСЗРодителя = 0 
			Или ТаблицаКлассификатор.Найти(СтрокаТЗ.УИДЕГИСЗРодителя, "УИДЕГИСЗ") <> Неопределено 
		Тогда
			Продолжить;
		КонецЕсли; 
		
		ДополнитьРодителемЭлемента(СтрокаТЗ.УИДЕГИСЗРодителя, ТаблицаКлассификатор, ВидКлассификатора);
		
	КонецЦикла;
	
КонецПроцедуры

// Рекурсивная процедура, дополняющая всеми отсутствующими родителями оригинального элемента
Процедура ДополнитьРодителемЭлемента(УИДЕГИСЗРодителя, ТаблицаКлассификатор, ВидКлассификатора)
	
	РодительЭлемента = ИнтеграцияЕГИСЗОбщегоНазначенияПовтИсп.ЭлементКлассификатораПоУИДЕГИСЗ(УИДЕГИСЗРодителя, ВидКлассификатора);
			
	НоваяСтрокаТЗ = ТаблицаКлассификатор.Добавить();
	НоваяСтрокаТЗ.Архив = РодительЭлемента.Архив;
	НоваяСтрокаТЗ.Наименование = РодительЭлемента.Наименование;
	НоваяСтрокаТЗ.НаименованиеПолное = РодительЭлемента.НаименованиеПолное;
	НоваяСтрокаТЗ.УИДЕГИСЗ = РодительЭлемента.УИДЕГИСЗ;
	НоваяСтрокаТЗ.УИДЕГИСЗРодителя = РодительЭлемента.УИДЕГИСЗРодителя;
	НоваяСтрокаТЗ.Архив = РодительЭлемента.Архив;
	
	НоваяСтрокаТЗ.Атрибуты.Колонки.Добавить("Ключ",Новый ОписаниеТипов("Строка"));
	НоваяСтрокаТЗ.Атрибуты.Колонки.Добавить("Значение",Новый ОписаниеТипов("Строка"));
	// Скопировать таблицу атрибутов. Без этого они потеряются
	Для Каждого Атрибут Из РодительЭлемента.Атрибуты Цикл
		НоваяСтрока = НоваяСтрокаТЗ.Атрибуты.Добавить();
		НоваяСтрока.Ключ = Атрибут.Ключ;
		НоваяСтрока.Значение = Атрибут.Значение;
	КонецЦикла;
	
	// Если дошли до корня или нашли родителя в таблице, прерываемся
	Если РодительЭлемента.УИДЕГИСЗРодителя = 0 
		Или ТаблицаКлассификатор.Найти(РодительЭлемента.УИДЕГИСЗРодителя, "УИДЕГИСЗ") <> Неопределено 
	Тогда
		Возврат;
	Иначе
		ДополнитьРодителемЭлемента(РодительЭлемента.УИДЕГИСЗРодителя, ТаблицаКлассификатор, ВидКлассификатора);
	КонецЕсли;
		...
		Попытка
			...
			Если Не Отказ Тогда
				...
				Если ЗначениеЗаполнено(СтрокаДереваКлассификатора.Ссылка) Тогда
					...
					Если МетаданныеСправочника.ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов
				...
				Если ТаблицаКлассификатор.Колонки.Найти("Атрибуты") <> Неопределено
					И (МетаданныеСправочника.ТабличныеЧасти.Атрибуты.Использование <> Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента
						Или Не ОбъектСправочника.ЭтоГруппа)
				Тогда
				...
				// Если элемент был в корне, а обновлением перенесли в группу или наоборот
...
Процедура ПроверитьДоступностьНСИ() Экспорт
	
	АдресНСИ = АдресСервераREST();
	ОписаниеОшибкиУстановкиСоединения = "";
	СоединениеHTTP = ПолучитьСоединениеССервисом(АдресНСИ, ОписаниеОшибкиУстановкиСоединения);
	
	Если СоединениеHTTP = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Не удалось установить соединение с сервером: %1'"), ОписаниеОшибкиУстановкиСоединения));
		Возврат;
	КонецЕсли;
	
	Ответ = СоединениеHTTP.Получить(Новый HTTPЗапрос("/"));
	Если Ответ.КодСостояния < 200
		Или Ответ.КодСостояния >= 300
	Тогда
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru='Ошибка обращения к ресурсу: %1 (код %2)'"), АдресНСИ, Ответ.КодСостояния));
		Возврат;
	КонецЕсли;
	
	ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Проверка подключения успешно пройдена'"));
