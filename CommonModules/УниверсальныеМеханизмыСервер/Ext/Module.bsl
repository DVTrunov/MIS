...
Функция ПолучитьСтрокуДереваМакетовПоУмолчанию(ИмяМетаданных,ДеревоКнопок) Экспорт
	...
	Иначе
		Если ТипЗнч(ИмяМакетаПоУмолчанию) = Тип("Строка") Тогда
			ИмяПоУмолчанию = ИмяМакетаПоУмолчанию;
		ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ИмяМакетаПоУмолчанию)) Тогда
			ИмяПоУмолчанию = ИмяМакетаПоУмолчанию.УникальныйИдентификатор();
		Иначе
			ИмяПоУмолчанию = Null;
		КонецЕсли;
		
		СтрокаПоУмолчанию = ДеревоКнопок.Строки.Найти(ИмяПоУмолчанию,"Имя");
		Если СтрокаПоУмолчанию = Неопределено Тогда
			Если ДеревоКнопок.Колонки.Найти("Ссылка") <> Неопределено Тогда
				ИмяКолонкиСсылки = "Ссылка";
			ИначеЕсли ДеревоКнопок.Колонки.Найти("СсылкаНаВнешнююОбработку") <> Неопределено Тогда
				ИмяКолонкиСсылки = "СсылкаНаВнешнююОбработку";
			Иначе
				ИмяКолонкиСсылки = Неопределено;
			КонецЕсли;
			
			Если ИмяКолонкиСсылки <> Неопределено Тогда
				СтрокаПоУмолчанию = ДеревоКнопок.Строки.Найти(ИмяПоУмолчанию, ИмяКолонкиСсылки);
			КонецЕсли;
		КонецЕсли;
		Если СтрокаПоУмолчанию = Неопределено Тогда
			СтрокаПоУмолчанию = ДеревоКнопок.Строки.Найти(ИмяПоУмолчанию,"Текст"); // Устарело. Для совместимости со старыми значениями в пользовательском хранилище настроек.
		КонецЕсли;
	...
	Пока ВыборкаЗаменяемыхФорм.Следующий() Цикл
		...
		Пока ВыборкаСсылок.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				ДобавитьСтрокуВДеревоКнопок(ДеревоМакетов,, Тип("КнопкаФормы"), ПолноеНаименованиеПечатнойФормы, Действие, СтруктураРасшифровки, , Подсказка, , );
...
Функция ДобавитьСтрокуВДеревоКнопок(СтрокаПодменю, Имя = Неопределено, Знач ТипКнопки = Неопределено ,Текст = "", Действие = Неопределено, Расшифровка = Неопределено, Пометка = Ложь, Подсказка = "", Картинка = Неопределено, Позиция = Неопределено, ИзменяетДанные = Ложь)
	...
	Если Имя = Неопределено Тогда
		Если ЗначениеЗаполнено(СтрокаКнопки.СсылкаНаВнешнююОбработку) Тогда
			СтрокаКнопки.Имя = СтрокаКнопки.СсылкаНаВнешнююОбработку.УникальныйИдентификатор();
		Иначе
			СтрокаКнопки.Имя = Новый УникальныйИдентификатор;
		КонецЕсли;
	Иначе
...
Функция НапечататьВнешнююФорму(Ссылка, ИсточникМакета) Экспорт
	...
	Иначе
		ДополнительныеПараметры = Справочники.ВнешниеОбработки.ПолучитьДополнительныеПараметрыВнешнейОбработки(ИсточникМакета, ИсточникМакета.ОбъектПечати);
...
Функция РежимЗапускаОбработки(Ссылка, ИсточникМакета) Экспорт 
...
// Синоним макета по умолчанию для оформления отчетов на СКД
// 
// Возвращаемое значение:
//  Строка.
//
Функция ПредставлениеОсновногоМакетаОформленияОтчетовНаСКД() Экспорт
	
	ИмяМакета = ОсновнойМакетОформленияОтчетовНаСКД();
	
	Возврат Метаданные.ОбщиеМакеты.Найти(ИмяМакета).Синоним;
...
//
Функция ОпределитьПол(Знач Имя = "", Знач Фамилия = "", Знач Отчество = "", Знач ИИН = "") Экспорт
	
	//Р Конради 19.06.2025 +
	// Определение по отчеству
    Если СокрЛП(Отчество) <> "" Тогда
		
		ОкончаниеОтчества = НРег(Прав(СокрЛП(Отчество), 2));
		
		ОкончанияМужскихОтчеств = ",ch,ly,ич,лы,";
		ОкончанияЖенскихОтчеств = ",na,zy,зы,на,";
		
		Если Найти(ОкончанияМужскихОтчеств, "," + ОкончаниеОтчества + ",") > 0 Тогда
			Возврат Перечисления.ПолФизическихЛиц.Мужской;
		ИначеЕсли Найти(ОкончанияЖенскихОтчеств, "," + ОкончаниеОтчества + ",") > 0 Тогда
	...
	// Определение по ИИН
    Если СокрЛП(ИИН) <> "" И СтрДлина(СокрЛП(ИИН)) = 12 Тогда
        СедьмаяЦифра = Сред(СокрЛП(ИИН), 7, 1);
      
        Если СедьмаяЦифра % 2 <> 0 Тогда
            Возврат Перечисления.ПолФизическихЛиц.Мужской;
        Иначе
            Возврат Перечисления.ПолФизическихЛиц.Женский;
		КонецЕсли;
    ...
    // Определение по имени (без исключений)
    Если СокрЛП(Имя) <> "" Тогда

		ОкончаниеИмени = НРег(Прав(СокрЛП(Имя), 1));
        
        Если ОкончаниеИмени = "а" 
            Или ОкончаниеИмени = "я" 
            Или ОкончаниеИмени = "ь" Тогда
            Возврат Перечисления.ПолФизическихЛиц.Женский;
        Иначе
            Возврат Перечисления.ПолФизическихЛиц.Мужской;
		КонецЕсли;
...
Процедура НастроитьПараметрыWkhtmltopdfПриСозданииНаСервере(Форма, РодительЭлементаКнопки, ПараметрыЭлемента = Неопределено) Экспорт
	
	РеквизитыWkhtmltopdf = Новый Массив;
	РеквизитыWkhtmltopdf.Добавить(Новый РеквизитФормы("АдресWkhtmltopdfНаКлиенте",		Новый ОписаниеТипов("Строка")));
	РеквизитыWkhtmltopdf.Добавить(Новый РеквизитФормы("МожноФормироватьPDFНаКлиенте",	Новый ОписаниеТипов("Булево")));
	РеквизитыWkhtmltopdf.Добавить(Новый РеквизитФормы("МожноФормироватьPDFНаСервере",	Новый ОписаниеТипов("Булево")));
	РеквизитыWkhtmltopdf.Добавить(Новый РеквизитФормы("ИмяТипаОбъектаДляWkhtmltopdf",	Новый ОписаниеТипов("Строка")));
	
	КомандаWkhtmltopdf = Форма.Команды.Добавить("СохранитьВPDF");
	КомандаWkhtmltopdf.Заголовок = НСтр("ru='Сохранить в PDF'");
	КомандаWkhtmltopdf.Действие = "СохранитьВPDF";
	
	ЭлементWkhtmltopdf = Форма.Элементы.Добавить("СохранитьВPDF", Тип("КнопкаФормы"), РодительЭлементаКнопки);
	ЭлементWkhtmltopdf.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
	ЭлементWkhtmltopdf.ИмяКоманды = "СохранитьВPDF";
	ЭлементWkhtmltopdf.Картинка = БиблиотекаКартинок.Сертификат;
	
	Если ПараметрыЭлемента <> Неопределено Тогда
		Для Каждого ПараметрЭлемента Из ПараметрыЭлемента Цикл
			ЭлементWkhtmltopdf[ПараметрЭлемента.Ключ] = ПараметрЭлемента.Значение;
		КонецЦикла;
	КонецЕсли;
	
	Форма.ИзменитьРеквизиты(РеквизитыWkhtmltopdf);
	
	Форма.ИмяТипаОбъектаДляWkhtmltopdf = ИмяТипаОбъектаДляWkhtmltopdf(Форма);
	
	Если Не ЗначениеЗаполнено(Форма.ИмяТипаОбъектаДляWkhtmltopdf) Тогда
		Возврат;
	КонецЕсли;
	
	Адресwkhtmltopdf = УправлениеНастройкамиПереопределяемый.ПолучитьПараметрУчета("Адресwkhtmltopdf");
	Форма.МожноФормироватьPDFНаСервере = РаботаСШаблонамиHTMLКлиентСервер.УтилитаWkhtmltopdfУстановленаВСистеме(Адресwkhtmltopdf);
	...
	Если УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "ФормироватьPDFНаКлиенте") Тогда
		Форма.АдресWkhtmltopdfНаКлиенте = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь, "РасположениеПрограммыWkhtmltopdfНаКлиенте");
	КонецЕсли;
	
КонецПроцедуры

Функция ПолучитьHTMLДляПреобразованияВPDF(ПараметрыФормированияHTML) Экспорт
	
	Если ПараметрыФормированияHTML.ИмяТипаОбъектаДляWkhtmltopdf = "Обработка.ПросмотрМедКарт" Тогда
		Возврат РаботаСШаблонамиHTMLКлиентСервер.КодироватьВДвоичныйКодИзображенияHTMLТекста(ПараметрыФормированияHTML.ИсходныйHTML,,, Истина);
	КонецЕсли;
...
Функция ПолучитьДанныеPDFФайлаПоHTML(ПараметрыФормированияHTML, ТекстHTML) Экспорт
	
	Если ТекстHTML = Неопределено Тогда
		ТекстHTML = ПолучитьHTMLДляПреобразованияВPDF(ПараметрыФормированияHTML);
	КонецЕсли;
	
	Возврат РаботаСШаблонамиHTML.ПреобразоватьHTMLвPDF(ТекстHTML, Ложь, Истина);
	
КонецФункции

Функция ИмяТипаОбъектаДляWkhtmltopdf(Форма)
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "Объект") Тогда
		ТипОбъекта = ТипЗнч(Форма.РеквизитФормыВЗначение("Объект"));
