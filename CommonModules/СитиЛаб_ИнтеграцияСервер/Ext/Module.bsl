...
Процедура ПолучитьФайлыРезультатовАнализов(Лаборатория, СообщенияОбОшибках) 
	...
	ТаблицаКодыПроб = ТаблицаЗаказы.Скопировать(, "Ссылка, НомерЗаявки, КодПробы");
	ТаблицаКодыПроб.Свернуть("Ссылка, НомерЗаявки, КодПробы");
	...
	Для Каждого Заказ Из ТаблицаЗаказы Цикл
		...
		СтрокиКодыПроб = ТаблицаКодыПроб.НайтиСтроки(Новый Структура("Ссылка, НомерЗаявки", ЗаказСсылка, НомерЗаявки));
		Для Каждого СтрокаКодыПроб Из СтрокиКодыПроб Цикл
			НоваяСтрокаТЗПМО = ТЗДляПростановкиОтметокПМО.Добавить();
			НоваяСтрокаТЗПМО.КодПробы = СтрокаКодыПроб.КодПробы;
