...
//  ПараметрыВвода				 - Структура					 - параметры, передаваемые в форму ввода, например, НачальноеЗначение.
//  ДополнительныеПараметрыВвода - Структура					 - Дополнительные параметры для передачи в форму ввода.
//
Процедура ВвестиЗначениеКодируемогоПоля(ОповещениеОВыборе, Форма, ЭлементФормы, КодировкаПоля, ПараметрыВвода, ДополнительныеПараметрыВвода = Неопределено) Экспорт
	...
	ПараметрыФормы = Новый Структура("КодировкаПоля", КодировкаПоля);
	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыФормы, ПараметрыВвода);
...
Процедура ПередНачаломРаботыСистемы() Экспорт
	...
	Кеши = МедицинскаяДеятельность.ПередНачаломРаботыСистемы_ПодготовитьКлиентскиеМедицинскиеКеши();
	
	МедицинскаяДеятельностьКлиентПереопределяемый.ПередНачаломРаботыСистемы(Кеши);
