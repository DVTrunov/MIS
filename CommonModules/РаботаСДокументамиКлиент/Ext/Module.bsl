...
Процедура РассчитатьСуммуСкидкиСтрокиТабЧасти(ЭтотОбъект, ИмяТЧ, ТекСтрока)
	...
	// Изменение Макеев 28-01-2026 (
	РаботаСДокументамиКлиентСервер.омг_РассчитатьСуммуСкидкиСтрокиТабЧасти(ЭтотОбъект, ИмяТЧ, ТекСтрока); 
...
Функция дкОбработкаРеквизитаКлиент(ЭтотОбъект,Имя,ТекСтрока=Неопределено,ЭтаФорма=Неопределено,ДопПараметры=Неопределено) Экспорт
	...
	ИначеЕсли Имя="Работы.Номенклатура" Тогда
		...
		Тогда
			// Добавлено Макеев (
			Если Не ДопПараметры = Неопределено И ДопПараметры.Свойство("омг_Набор") Тогда
				СтруктураШапкиДокумента.Вставить("омг_Набор", ДопПараметры.омг_Набор); 
				Если ДопПараметры.Свойство("омг_НаборУровень") Тогда
				СтруктураШапкиДокумента.Вставить("омг_НаборУровень", ДопПараметры.омг_НаборУровень);
				КонецЕсли;
			КонецЕсли;
			// )
			СтрЦенаСумма = РаботаСДокументамиСервер.дкУстановитьЦенуСтрокиТабЧастиСервер(СтруктураШапкиДокумента, ИмяТЧ, СтруктураСтроки);
...
Процедура ТоварыЕдиницаИзмерения_ОбработкаШтрихкодаСтрокиДокумента(ЭтотОбъект, ТекСтрока)
	...
	Тогда
		...
		Тогда
			...
			ДробнаяЧасть = РаботаСДокументамиСервер.ПолучитьДробнуюЧасть(ТекСтрока.Номенклатура, ТекСтрока.Количество, ТекСтрока.ЕдиницаИзмерения);
...
// Обработчик для формы денежного документа (ПКО, РКО, ОПК, ВПК) при изменении номенклаты таб.части "Кредитные данные".
//
// Параметры:
//  Форма	 - ФормаКлиентскогоПриложения - форма денежного документа с табличной частью "Кредитные денные".
//
Процедура КредитныеДанныеНоменклатураПриИзменении(Форма) Экспорт
	
	НастройкаПечатиЧеков = Форма.НастройкаПечатиЧеков;
	
	ТекущиеКредитныеДанные = Форма.Элементы.КредитныеДанные.ТекущиеДанные;
	Если ТекущиеКредитныеДанные <> Неопределено И ЗначениеЗаполнено(ТекущиеКредитныеДанные.Номенклатура) Тогда
		...
		Если ПустаяСтрока(ТекущиеКредитныеДанные.Наименование) Тогда 
			ТекущиеКредитныеДанные.Наименование = Строка(ТекущиеКредитныеДанные.Номенклатура);
		КонецЕсли;
		
		// Заполним Ставку НДС
		СтавкаНДСНоменклатуры = РаботаСДокументамиСервер.ПолучитьСтавкуНДС(НастройкаПечатиЧеков, ТекущиеКредитныеДанные.Номенклатура);
		...
		Иначе
			...
			Иначе
				СтавкаНДСПоУмолчанию = ПредопределенноеЗначение("Перечисление.СтавкиНДС.БезНДС");
			КонецЕсли;
	...
	РаботаСДокументамиКлиент.РассчитатьЗаполнитьПризнакиДопСНОКредитнойЧасти(Форма.Объект, НастройкаПечатиЧеков);
	
КонецПроцедуры

// Обработчик для формы денежного документа (ПКО, РКО, ОПК, ВПК) при изменении таб.части "Кредитные данные".
//
// Параметры:
//  Форма	 - ФормаКлиентскогоПриложения - форма денежного документа с табличной частью "Кредитные денные".
//
Процедура КредитныеДанныеПриИзменении(Форма) Экспорт
	
	НастройкаПечатиЧеков = Форма.НастройкаПечатиЧеков;
	
	ТекущиеКредитныеДанные = Форма.Элементы.КредитныеДанные.ТекущиеДанные;
	Если ТекущиеКредитныеДанные <> Неопределено
		И Не ЗначениеЗаполнено(ТекущиеКредитныеДанные.СтавкаНДС)
		И Не ЗначениеЗаполнено(ТекущиеКредитныеДанные.Номенклатура)
	Тогда
		// Заполним Ставку НДС
		Если ЗначениеЗаполнено(НастройкаПечатиЧеков) И ЗначениеЗаполнено(НастройкаПечатиЧеков.СтавкаНДСДляОсновнойСистемыНалогообложения) Тогда
			СтавкаНДСПоУмолчанию = НастройкаПечатиЧеков.СтавкаНДСДляОсновнойСистемыНалогообложения;
...
Функция ПроверкаНоменклатурыКредитаНаНесколькоСНО(Объект, НастройкаПечатиЧеков) Экспорт
	...
	Если Не ЗначениеЗаполнено(Объект.НомерЧекаККМ) Тогда
		...
		РассчитатьЗаполнитьПризнакиДопСНОКредитнойЧасти(Объект, НастройкаПечатиЧеков);
...
// Заполняет признаки Патентной СНО в кредитной части документа чека.
// Также сбрасывает Ставку НДС до ставки этой дополнительной СНО.
//
// Параметры:
//  Объект				 - ДокументОбъект	 - документ чека.
//  НастройкаПечатиЧеков - Структура		 - настройки работы с чеками.
//
Процедура РассчитатьЗаполнитьПризнакиДопСНОКредитнойЧасти(Объект, НастройкаПечатиЧеков) Экспорт
	
	ТаблицаПродажПрименение = РаботаСНДССервер.ЗаполнитьПризнакПатентДляНоменклатуры(Объект, НастройкаПечатиЧеков);
...
Процедура ПлатежныйДокументПослеЗаписиОбработкаНоменклатурыДоплатыНаНесколькоСНО(ЭтаФорма) Экспорт
	...
	Если ЭтаФорма.РаспределитьНоменклатуруПоРазнымСНО Тогда
		РаботаСДокументамиКлиент.РассчитатьЗаполнитьПризнакиДопСНОКредитнойЧасти(ЭтаФорма.Объект, ЭтаФорма.НастройкаПечатиЧеков);
		РезультатРаспределения = РаботаСНДССервер.ПлатежныйДокументРаспределитьНоменклатуруПоРазнымСНО(ЭтаФорма.Объект);
...
Процедура ПечатьКопииЧекаПоИсторииПечати(ДанныеXML, ДокументСсылка, УникальныйИдентификатор) Экспорт
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		Организация = РаботаСДокументамиКлиентСервер.ОрганизацияОбъекта(ДокументСсылка);
		ВыбратьККТСОтборомОрганизации(ОповещениеПродолжения, Организация);
	КонецЕсли;
...
Процедура НапечататьКопиюФормированиеДанных(РезультатОперации, Контекст) Экспорт
	...
	Если РезультатОперации.Результат Тогда
		...
		//	
		ДанныеЧека = ""; 
		НомерЧекаККМ = омг_WebKassa.ПолучитьНомерЧекаККМОказанияУслуг(Контекст.Документ);
		...
		Иначе
			...
			ПараметрыОперации.Вставить("НомерЧека", НомерЧекаККМ);
...
Процедура ВыполнитьОперациюПКОРКО(ЭтаФорма)
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		// Оповещение перехода к действиям после выбора кассы в зависимости от операции (чек или инкассация)
		Если ЭтоОперацияИнкассации Тогда
			
			// Напечатаем чек инкассации
		...
		ЭтаФорма.Доступность = Ложь; // Блокировка формы на время операции.
		Организация = РаботаСДокументамиКлиентСервер.ОрганизацияОбъекта(Объект.Филиал);
		ВыбратьККТСОтборомОрганизации(ОповещениеПриЗавершении, Организация);
	КонецЕсли;
...
Процедура ПечатьЧекаФРСПредВыбором(ЭтаФорма, ДополнительныеПараметрыОперации = Неопределено)
	...
	Организация = РаботаСДокументамиКлиентСервер.ОрганизацияОбъекта(ЭтаФорма.Объект.Филиал);
	
	ВыбратьККТСОтборомОрганизации(ОповещениеПриЗавершении, Организация);
	
КонецПроцедуры

Функция ПолучитьШаблонЧека(ЭтаФорма, ОборудованиеСсылка, ШиринаСтроки = 0, ТекстСообщения)
	...
	ОбщиеПараметры.Вставить("Организация"			  , РаботаСДокументамиКлиентСервер.ОрганизацияОбъекта(Объект.Филиал)); // Ссылка,Обязательно. Организация по которому пробивается чек.
	...
	Тогда
		...
		Для Каждого Позиция Из Объект.КредитныеДанные Цикл
			...
			СтрокаТаблицыТоваров.Добавить(0);						// 11 - Номер налога (1)
			ПроверитьСтавкуНДС(НастройкаПечатиЧеков, Позиция.СтавкаНДС, ТекстСообщения); 
			...
			ПризнакПредметаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.Услуга");
			Если ВидНоменклатурыСоотв <> Неопределено Тогда
				...
				ИначеЕсли ВидНоменклатурыСоотв.ВидНоменклатуры = ПредопределенноеЗначение("Перечисление.ВидыНоменклатуры.Материал") Тогда
					ПризнакПредметаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.Товар");
				КонецЕсли;
			КонецЕсли;
			СтрокаТаблицыТоваров.Добавить(ПризнакПредметаРасчета);
...
Процедура ПроверитьСтавкуНДС(НастройкиПечатиЧеков, СтавкаНДС, ТекстСообщения)
	
	Если ЗначениеЗаполнено(ТекстСообщения) Тогда
		Возврат;
	КонецЕсли;
	
	ОсновнаяСтавкаНДС = НастройкиПечатиЧеков.СтавкаНДСДляОсновнойСистемыНалогообложения;
	ОсновнаяСНО	= НастройкиПечатиЧеков.ОсновнаяСистемаНалогообложения;
	ЭтоУСН = УчетНДСКлиентСервер.ЭтоСНО_УСН(ОсновнаяСНО);
	
	СтавкиУСН5_7 = Новый Массив;
	СтавкиУСН5_7.Добавить(ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС5"));
	СтавкиУСН5_7.Добавить(ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС7"));
	СтавкиУСН5_7.Добавить(ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС5_105"));
	СтавкиУСН5_7.Добавить(ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС7_107"));
	
	ШаблонТекстаСообщения = СтрШаблон("Для некоторых из товарных позиций установлена неверная ставка НДС, на данный момент действует ставка %1. Продолжить пробитие чека?",
									ОсновнаяСтавкаНДС);
	
	Если ЭтоУСН Тогда
										
		Если СтавкиУСН5_7.Найти(ОсновнаяСтавкаНДС) <> Неопределено Тогда
			
			Если	СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС10") 
				Или СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС10_110")
				Или СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС20")
				Или СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС20_120") 
			Тогда
				ТекстСообщения = ШаблонТекстаСообщения;
				Возврат;
			КонецЕсли;
			
			Если ОсновнаяСтавкаНДС <> СтавкаНДС 
				И (		СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС5")	Или СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС5_105")
					Или СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС7")	Или СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС7_107"))
			Тогда
				ТекстСообщения = ШаблонТекстаСообщения;
			КонецЕсли;
	...
	// Если дошли до этого места и это ставка НДС 5% или 7%
	Если СтавкиУСН5_7.Найти(СтавкаНДС) <> Неопределено Тогда
		Если ЭтоУСН Тогда
			ТекстСообщения = ШаблонТекстаСообщения;
		Иначе
			ТекстСообщения = "Для некоторых из товарных позиций установлена неверная ставка НДС, ставка НДС 5% и 7% применяются только для УСН, продолжить пробитие чека?";
		КонецЕсли;
	КонецЕсли;
...
Функция ВыбратьККТСОтборомОрганизации(ОповещениеПродолжения, Организация)
...
Процедура НачалоПечатиЧекаФР_ПослеПолученияШириныСтрокиПечатающегоУстройства(РезультатОперации, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.Вставить("ШиринаСтроки", ШиринаСтроки);
	
	НачатьПечатьЧекаФР(Истина, ДополнительныеПараметры); 
	
КонецПроцедуры

Процедура ПечатьЧекаФР(ЭтаФорма, РезультатВыполнения, ОбщиеПараметры, ОповещениеПриЗавершении)
...
Процедура ЗавершениеПечатиЧекаНаФР(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ru = '%ДополнительноеОписание%
		|Чек не фискализирован на кассовом устройстве.'");
	...
	ПродолжитьПечатьЧека(ДополнительныеПараметры);
...
Процедура ЗакрытиеФормыПроверкиКМНаККТ(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Если НЕ Результат.Результат.ЕстьОшибки Тогда  
			ПечатьЧекаФР(ДополнительныеПараметры.ЭтаФорма, ОбщиеПараметрыЧека.ПодключаемоеОборудование, ОбщиеПараметрыЧека, ДополнительныеПараметры.ОповещениеПриЗавершении);
		Иначе
...
Процедура ПослеЗакрытияВопросаОПечатиКМНеПрошедшихПроверку(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ПечатьЧекаФР(ДополнительныеПараметры.ЭтаФорма, ОбщиеПараметрыЧека.ПодключаемоеОборудование, ОбщиеПараметрыЧека, ОповещениеПриЗавершении);
	КонецЕсли;
...
Процедура УстановитьНомерЧекаДокументаВФорме(ЭтаФорма, НомерЧека, ИмяРеквизита = "НомерЧекаККМ", Объект = Неопределено)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "КредитныеДанные") Тогда
		НастройкаПечатиЧеков = ЭтаФорма.НастройкаПечатиЧеков;
		РассчитатьЗаполнитьПризнакиДопСНОКредитнойЧасти(Объект, НастройкаПечатиЧеков.Ссылка);
	КонецЕсли;
...
Асинх Процедура НачатьПечатьЧекаФР(Результат, Контекст) Экспорт
	
	ЭтаФорма = Контекст.ЭтаФорма;
	
	Объект = ЭтаФорма.Объект;
	НастройкаПечатиЧеков = ЭтаФорма.НастройкаПечатиЧеков;
	
	ДопПараметрыОповещения = Новый Структура;
	ДопПараметрыОповещения.Вставить("ЭтаФорма", ЭтаФорма);
	ДопПараметрыОповещения.Вставить("Контекст", Контекст);
	
	ОповещениеПриЗавершении = Новый ОписаниеОповещения("ЗавершениеПечатиЧекаНаФР", РаботаСДокументамиКлиент, ДопПараметрыОповещения);
	
	Если ЗначениеЗаполнено(Контекст.Оборудование)
		И РаботаСДокументамиКлиент.НастройкиПечатиЧековКорректны(Контекст.Оборудование, НастройкаПечатиЧеков)
	Тогда
		Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ОказаниеУслуг") Тогда
			Оборудование = Контекст.Оборудование;
			Если Объект.Чеки.Количество() = 0 Тогда
				Возврат;
			КонецЕсли;
			Если Не Контекст.Свойство("НесколькоЧеков") Тогда
				Контекст.Вставить("НесколькоЧеков", Истина);                     
				Контекст.Вставить("ЧекиНомерСтроки", 0);
				Контекст.Вставить("ЧекиКоличество", Объект.Чеки.Количество()); 
			КонецЕсли;
			СтрокаЧек = Объект.Чеки[Контекст.ЧекиНомерСтроки];
			Если Не ЗначениеЗаполнено(СтрокаЧек.НомерЧека) Тогда
				ОбщиеПараметрыЧека = ПолучитьШаблонЧекаОказаниеУслуг(Объект, ЭтаФорма, Оборудование, СтрокаЧек, Контекст.ШиринаСтроки);
				...
				Если Не ЗначениеЗаполнено(ОбщиеПараметрыЧека) Тогда
					...
					КонецЕсли;
					// 
					ПродолжитьПечатьЧека(Новый Структура("Контекст", Контекст));
					Возврат;
				КонецЕсли;
				
				Если СтрокаЧек.КлючСтроки <> Неопределено Тогда
					ДопПараметрыОповещения.Вставить("ЧекиКлючСтроки",СтрокаЧек.КлючСтроки);	
				КонецЕсли;
				...
				Если ОбщиеПараметрыЧека.СодержитМаркированныеТовары Тогда
					...
					Если ККТПоддерживаетПроверкуКМ Тогда
						...
						Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОказанияУслуг.ПродажаКлиенту")
							И Не Объект.Коррекция
						Тогда
						
							ТекстОшибкиРазрешительныйРежим = "";
							МаркировкаРазрешительныйРежим.ПолучитьРазрешение(ОбщиеПараметрыЧека.ПараметрыПроверкиНаККТ.ПроверяемыеКМ, 
																			Объект.Филиал, 
																			Контекст.Оборудование, 
																			ОбщиеПараметрыЧека.ПозицииЧека, 
																			ТекстОшибкиРазрешительныйРежим);
																			
							Если ЗначениеЗаполнено(ТекстОшибкиРазрешительныйРежим) Тогда
								ПоказатьПредупреждение(,"Невозможно пробить чек из-за ограничительного режима: " + ТекстОшибкиРазрешительныйРежим);
								ЭтаФорма.Доступность = Истина;
								Возврат;
							КонецЕсли;
						...
						ПараметрыОповещения = Новый Структура("ОбщиеПараметрыЧека, ЭтаФорма, ОповещениеПриЗавершении", ОбщиеПараметрыЧека, ЭтаФорма, ОповещениеПриЗавершении); 
					...
					КонецЕсли;
				Иначе
					ПечатьЧекаФР(ЭтаФорма, Оборудование, ОбщиеПараметрыЧека, ОповещениеПриЗавершении);
				КонецЕсли;
			Иначе
				ПродолжитьПечатьЧека(Новый Структура("Контекст", Контекст));
			КонецЕсли;
		Иначе
			ТекстСообщения = "";
			ОбщиеПараметры = ПолучитьШаблонЧека(ЭтаФорма, Контекст.Оборудование, Контекст.ШиринаСтроки, ТекстСообщения);
			Если ЗначениеЗаполнено(ТекстСообщения) Тогда 
				...
				КонецЕсли;
			КонецЕсли;
			ПечатьЧекаФР(ЭтаФорма, Контекст.Оборудование, ОбщиеПараметры, ОповещениеПриЗавершении);
		КонецЕсли;	
	Иначе
		ЭтаФорма.Доступность = Истина;
	КонецЕсли;
...
Процедура ПродолжитьПечатьЧека(ДополнительныеПараметры)
	Если ДополнительныеПараметры.Свойство("Контекст") И ДополнительныеПараметры.Контекст.Свойство("НесколькоЧеков") Тогда
		ДополнительныеПараметры.Контекст.ЧекиНомерСтроки = ДополнительныеПараметры.Контекст.ЧекиНомерСтроки + 1;
		Если ДополнительныеПараметры.Контекст.ЧекиНомерСтроки < ДополнительныеПараметры.Контекст.ЧекиКоличество Тогда
			ОписаниеОповещения = Новый ОписаниеОповещения("НачатьПечатьЧекаФР", ЭтотОбъект, ДополнительныеПараметры.Контекст);
...
Процедура ВыполнитьРедактированиеРеквизитаККТ(Форма, НазваниеРеквизитаККТ, Клиент) Экспорт
	...
	Если СтрНайти(Форма.ИмяФормы, "Документ.РасходныйКассовыйОрдер") Тогда
		ИмяДокумента = "РКО";
	ИначеЕсли СтрНайти(Форма.ИмяФормы, "Документ.ПриходныйКассовыйОрдер") Тогда
		ИмяДокумента = "ПКО";
	Иначе
		ИмяДокумента = "";
	КонецЕсли;
	...
	Если НазваниеРеквизитаККТ = "ПерсональныеДанныеПокупателя" Тогда
		...
		пчВидыПКО = ДопСерверныеФункцииПовтИсп.ПолучитьСтруктуруДляПеречисления("ВидыОперацийПКО");
		пчВидыРКО = ДопСерверныеФункцииПовтИсп.ПолучитьСтруктуруДляПеречисления("ВидыОперацийРКО");
		ВидыОперацийЮЛ = Новый Массив;
		ВидыОперацийЮЛ.Добавить(пчВидыПКО.ПолучениеНаличныхВБанке);
		ВидыОперацийЮЛ.Добавить(пчВидыПКО.ПриходДенежныхСредствОтКонтрагента);
		ВидыОперацийЮЛ.Добавить(пчВидыПКО.ПрочееПоступлениеДенежныхСредств);
		ВидыОперацийЮЛ.Добавить(пчВидыРКО.ВзносНаличнымиВБанк);
		ВидыОперацийЮЛ.Добавить(пчВидыРКО.ПрочийРасходДенежныхСредств);
		ВидыОперацийЮЛ.Добавить(пчВидыРКО.РасчетыСКонтрагентами);
		
		Если (ИмяДокумента = "ПКО" Или ИмяДокумента = "РКО")
			И ВидыОперацийЮЛ.Найти(Форма.Объект.ВидОперации) <> Неопределено
		Тогда
			ПерсональныеДанные.Вставить("ЭтоФизЛицо", Ложь);
		Иначе
			ПерсональныеДанные.Вставить("ЭтоФизЛицо", Истина);
		КонецЕсли;
