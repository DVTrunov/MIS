...
Процедура ПриПолученииКомандПечатиОбъекта(КомандыПечати, СведенияОбОбъекте, ДобавлятьДополнительные) Экспорт
	...
	ДеревоМакетов = УниверсальныеМеханизмыСервер.ПолучитьДеревоМакетовПечати(ПустаяСсылка, ПечатьДокументовСервер.ПолучитьСтруктуруПечатныхФормСервер(ПустаяСсылка),,,Ложь);
