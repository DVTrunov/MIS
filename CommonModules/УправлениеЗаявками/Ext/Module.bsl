...
Функция ПолучитьПараметрыЗаписиПоЛистуОжидания(ПараметрКоманды, СообщениеОбОшибке = "") Экспорт
	...
	Если ПараметрКоманды.Метаданные().Реквизиты.Найти("Филиал") <> Неопределено И ЗначениеЗаполнено(ПараметрКоманды.Филиал) Тогда
		ПараметрыЗаписи.Вставить("Филиал", ПараметрКоманды.Филиал);
	Иначе
		ПараметрыЗаписи.Вставить("Филиал", Справочники.Филиалы.ПустаяСсылка());
	КонецЕсли;
	...
	Если ПараметрКоманды.Метаданные().Реквизиты.Найти("ВидПриема") <> Неопределено Тогда
		Если не ПараметрКоманды.ВидПриема = ПредопределенноеЗначение("Перечисление.ВидыПриемов.Первичный") 
			и не ПараметрКоманды.ВидПриема = ПредопределенноеЗначение("Перечисление.ВидыПриемов.Вторичный") Тогда
			ЕстьОшибки = Истина; 
			ТекстЕстьОшибки = "Заполните вид приема!" + Символы.ПС;
		КонецЕсли;   
	КонецЕсли; 	
		
	Если ПараметрКоманды.Метаданные().Реквизиты.Найти("НаправлениеНовый") <> Неопределено Тогда
		Если не ПараметрКоманды.НаправлениеНовый = ПредопределенноеЗначение("Перечисление.ВидыНаправлений.Платно") 
			и не ПараметрКоманды.НаправлениеНовый = ПредопределенноеЗначение("Перечисление.ВидыНаправлений.Страховка") Тогда
			ЕстьОшибки = Истина; 
			ТекстЕстьОшибки = ТекстЕстьОшибки + "Заполните направление!" + Символы.ПС;
		КонецЕсли;   
	КонецЕсли;
	
    Если ЕстьОшибки Тогда	
		СообщениеОбОшибке = ТекстЕстьОшибки;
		Возврат Неопределено;
	КонецЕсли; 	
