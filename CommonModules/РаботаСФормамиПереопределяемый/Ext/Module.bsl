...
Процедура НастроитьВидФормыОбъекта(ЭтаФорма, МетаданныеОбъекта) Экспорт
	...
	ИначеЕсли Метаданные.ОпределяемыеТипы.ОснованиеФискальнойОперацииБПО.Тип.СодержитТип(ТипЗнч(ЭтаФорма.Объект.Ссылка)) Тогда
		
		ЕстьПравоИзменятьСтавкуНДС = УчетНДСВызовСервера.ЕстьПравоИзменятьСтавкуНДС_ДенежныеДокументы();
		
		// Чековые документы за исключением "ОказаниеУслуг"
		// Запрет ручного редактирования ставки НДС для таб. части предоплат и постоплат.
		Если Не ЕстьПравоИзменятьСтавкуНДС Тогда
			РаботаСФормамиКлиентСервер.УстановитьДоступностьЭлементаФормы(ЭтаФорма.Элементы, "КредитныеДанныеНДС", Ложь);
		КонецЕсли;
	КонецЕсли;
