...
// Возвращает признак передачи данных организации через определенную настройку EnterpriseData
//
// Параметры:
//  Организация - СправочникСсылка - организация, для которой нужно определить участие в плане обмена
//  ВариантНастройки - Строка - наименование настройки EnterpriseData
//  ОтборПоОрганизации - Булево - результат будет Истина только при наличии в настройках обмена отбора по выбранной организации
//
//  Возвращаемое значение:
//    Булево
//
Функция ОрганизацияИспользуетОбменЧерезУниверсальныйФормат(Организация, ВариантНастройки, ОтборПоОрганизации = Ложь) Экспорт
	...
	Запрос.УстановитьПараметр("Узлы", РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Ссылка"));
	Запрос.Текст = 
	"ВЫБРАТЬ
