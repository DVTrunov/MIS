...
#КонецОбласти
	...
	|	И НЕ Анализы.Архив
	|	И Анализы.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Услуга)
	|	И (Анализы.Анализ ИЛИ НЕ НоменклатураАнализов.Лаборатория ЕСТЬ NULL) // Либо используется в интеграции, либо является аналиом
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция СформироватьЭтикеткиКонтейнеров(ДокументЗабора, Лаборатории, Клиент, КодыПроб) Экспорт
	...
	// Может быть распределено несколько исследований на один контейнер.
	// Поэтому исключим дублирование контейнеров при печати этикеток.
	ТаблицаКонтейнеров = ДокументЗабора.Контейнеры.Выгрузить();
	
	УдалитьКонтейнерыБезДопустимогоКодаПробы(ТаблицаКонтейнеров, КодыПроб);
	
	ТаблицаКонтейнеров.Свернуть("Лаборатория, НомерКонтейнера, КодПробы, Биоматериал, БиоматериалПредставление,
								|Контейнер, КонтейнерПредставление, ЦветКонтейнера, УсловияХранения, УсловияТранспортировки");
...
Процедура УдалитьКонтейнерыБезДопустимогоКодаПробы(Контейнеры, КодыПроб)
	
	Если КодыПроб = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Счетчик = Контейнеры.Количество() - 1;
	Пока Счетчик >= 0 Цикл 
		Если КодыПроб.Найти(Контейнеры[Счетчик].КодПробы) = Неопределено Тогда
			Контейнеры.Удалить(Контейнеры[Счетчик]);
		КонецЕсли;
		Счетчик = Счетчик - 1;
	КонецЦикла;
...
//	
Функция ПолучитьСтруктуруПечатиЭтикеток(Знач ДанныеКонтейнеров, Лаборатория, ДокументСсылка, Клиент, КодыПроб) Экспорт
	...
	Если МодульОбработчик <> Неопределено Тогда
		Попытка
			ОписаниеОшибки = "";
			УдалитьКонтейнерыБезДопустимогоКодаПробы(ДанныеКонтейнеров, КодыПроб);
