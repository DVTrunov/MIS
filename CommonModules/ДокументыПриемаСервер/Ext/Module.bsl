...
Функция ПолучитьТаблицуПараметровПриема(Документ) Экспорт
	...
	ТипыНазначений = Новый Массив;
	ТипыНазначений.Добавить(Тип("ПеречислениеСсылка.КодировкаПолейЭМД"));
	ТипыНазначений.Добавить(Тип("СправочникСсылка.КлассификаторыМинЗдрава"));
	...
	ТаблицаПараметров.Колонки.Добавить("Значение",						Новый ОписаниеТипов(Метаданные.ПланыВидовХарактеристик.ПараметрыHTML.Тип, , , , Новый КвалификаторыСтроки()));
	ТаблицаПараметров.Колонки.Добавить("ИдентификаторСтрокиШаблона",	Новый ОписаниеТипов("Число"));
	ТаблицаПараметров.Колонки.Добавить("СтрокаУидПараметра",			Новый ОписаниеТипов(Метаданные.ОпределяемыеТипы.УникальныйИдентификаторСтрокой.Тип));
	...
	ТаблицаПараметров.Колонки.добавить("НаименованиеДляВрача",			Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки()));
	...
	Для Каждого СтрокаШаблона Из Документ.ШаблоныПриема Цикл
		...
		Пока ВыбПараметры.НайтиСледующий(ОтборШаблон) Цикл
			...
			Если СтрокиПараметра.Количество() = 0 Тогда
				...
				НоваяСтрока.НаименованиеДляВрача = ВыбПараметры.НаименованиеДляВрача;
...
Функция ПолучитьВыборкуПараметровШаблоновПриема(ШаблоныПриема) Экспорт
	...
	Запрос.Текст = 
		...
		|	ПараметрыШаблонов.Параметр.НаименованиеДляВрача КАК НаименованиеДляВрача,
		//|	ВЫБОР КОГДА ПараметрыШаблонов.Параметр.НаименованиеДляВрача = """"
		//|		ТОГДА ПРЕДСТАВЛЕНИЕ(ПараметрыШаблонов.Параметр)
		//|		ИНАЧЕ ПараметрыШаблонов.Параметр.НаименованиеДляВрача КОНЕЦ КАК НаименованиеДляВрача,
...
Функция СформироватьДанныеИзображенийПоШаблонам(Знач ШаблоныПриема, Знач ВерсииШаблонов, ДобавлятьШапкуШаблона, Дата, Филиал, РежимДляКлиента) Экспорт
	
	ДанныеИзображенийПоШаблонам = Новый Соответствие;
	
	Для Каждого СтрокаШаблона Из ШаблоныПриема Цикл
		
		СтруктурыИзображений = Новый Массив;
		...
		Если СтрокиВерсии.Количество() > 0 Тогда
			ДополнитьМассивСтруктурИзображенийШаблона(СтруктурыИзображений, СтрокиВерсии[0].ВерсияШаблона);
		Иначе
			ДополнитьМассивСтруктурИзображенийШаблона(СтруктурыИзображений, СтрокаШаблона.ШаблонПриема);
		КонецЕсли;
		
		Если ДобавлятьШапкуШаблона И Не СтрокаШаблона.НеИспользоватьШапкуШаблона Тогда
			НастройкиШаблонов = РаботаСШаблонамиHTMLПовтИсп.ПолучитьНастройкиШаблоновHTML(Дата, Филиал);
			Если РежимДляКлиента = Истина Тогда
				Если ЗначениеЗаполнено(НастройкиШаблонов.ШаблонКлиентскогоЗаголовка) Тогда
					ДополнитьМассивСтруктурИзображенийШаблона(СтруктурыИзображений, НастройкиШаблонов.ШаблонКлиентскогоЗаголовка);
				КонецЕсли;
			Иначе
				Если ЗначениеЗаполнено(НастройкиШаблонов.ШаблонМедицинскогоЗаголовка) Тогда
					ДополнитьМассивСтруктурИзображенийШаблона(СтруктурыИзображений, НастройкиШаблонов.ШаблонМедицинскогоЗаголовка);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		ДанныеИзображенийПоШаблонам.Вставить(ШаблоныПриема.Индекс(СтрокаШаблона), СтруктурыИзображений);
	КонецЦикла;
	
	Возврат ДанныеИзображенийПоШаблонам;
	
КонецФункции

Функция ДанныеВерсииШаблонаДляСтрокиШаблонов(ВерсияШаблона, ШаблонПриема) Экспорт
	
	ДанныеВерсии = Новый Структура;
	
	ТекстHTML = ?(ПустаяСтрока(ВерсияШаблона.ТекстHTML), РаботаСШаблонамиHTMLКлиентСервер.ПолучитьТекстШаблонаHTMLПоУмолчанию(), ВерсияШаблона.ТекстHTML);
	ДанныеВерсии.Вставить("ТекстШаблона",							ТекстHTML);
	ДанныеВерсии.Вставить("ВидШаблона",								ВерсияШаблона.ВидШаблона);
	ДанныеВерсии.Вставить("ВидДокумента",							ВерсияШаблона.ВидДокумента);
	ДанныеВерсии.Вставить("НеИспользоватьШапкуШаблона",				ВерсияШаблона.НеИспользоватьШапкуШаблона);
	ДанныеВерсии.Вставить("ПечатаетсяБезПереходаНаНовуюСтраницу",	ШаблонПриема.ПечатаетсяБезПереходаНаНовуюСтраницу);
	ДанныеВерсии.Вставить("ЭталонныеСтрокиТаблиц",					РаботаСШаблонамиHTML.ЭталонныеСтрокиТаблиц(ТекстHTML));
...
Процедура ДополнитьМассивСтруктурИзображенийШаблона(СтруктурыИзображений, Шаблон)
	
	ВерсииШаблонаДляКеша = Новый Массив;
	ВерсииШаблонаДляКеша.Добавить(Шаблон);
	соотИзображенияВерсии = РаботаСШаблонамиHTML.ПолучитьКешИзображенийПриема(ВерсииШаблонаДляКеша, Новый УникальныйИдентификатор);
	Для Каждого ДанныеИзображения Из соотИзображенияВерсии Цикл
		...
		СтруктураИзображения.Идентификатор = ДанныеИзображения.Значение.Идентификатор;
		СтруктураИзображения.Расширение = ДанныеИзображения.Значение.Расширение;
		...
		СтруктураИзображения.ИзображениеBase64 = ИзображениеBase64;
		
		СтруктурыИзображений.Добавить(СтруктураИзображения);
	КонецЦикла;
