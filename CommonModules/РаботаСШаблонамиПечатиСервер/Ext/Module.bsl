...
Функция ВычислитьЗначенияПараметровШаблонаПечати(ОбъектПечати, ШаблонПечати, СодержаниеОшибки = "", ИмяПараметраШаблона = Неопределено) Экспорт
	...
	// Вычисление параметров печати по алгоритму из реквизита ПередФормированием 
	ПараметрыПередФормированием = Справочники.ШаблоныПечати.ПолучитьПараметрыПередФормированием(ШаблонПечати, ОбъектПечати);
	
	УстановитьБезопасныйРежим(Истина);
	СтандартныеАлгоритмы = Справочники.ШаблоныПечати.ПолучитьСтандартныеАлгоритмыПараметров(Истина);
	Для Каждого ПараметрШаблона Из ПараметрыПечати Цикл
		...
		Если ПараметрШаблона.СтандартныйАлгоритм Тогда
			Алгоритм = СтандартныеАлгоритмы[ПараметрШаблона.Значение];	
		Иначе
			Алгоритм = ПараметрШаблона.Значение;	
		КонецЕсли;
		
		Попытка
			Если ТипЗнч(ОбъектПечати) = Тип("ДокументОбъект.ДоговорСКлиентом") Тогда
				ОбъектПечати.ВыполнитьФормулуДляПечатиШаблона(Алгоритм, ПараметрыПередФормированием, Результат);
			ИначеЕсли ТипЗнч(ОбъектПечати) = Тип("ДокументСсылка.ДоговорСКлиентом") Тогда
				ДокументОбъект = ОбъектПечати.ПолучитьОбъект();
				ДокументОбъект.ВыполнитьФормулуДляПечатиШаблона(Алгоритм, ПараметрыПередФормированием, Результат);
			Иначе
				ВыполнитьФормулуДляПечатиШаблона(Алгоритм, ПараметрыПередФормированием, Результат, ОбъектПечати);
			КонецЕсли;
		Исключение
...
Функция ВыполнитьФормулуДляПечатиШаблона(ФормулаРасчета, ОбщиеПеременныеАлгоритмов, Результат, Знач ОбъектПечати)
