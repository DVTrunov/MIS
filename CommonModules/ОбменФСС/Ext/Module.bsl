...
Функция СформироватьДанныеДляПодписания(ПараметрыЗапроса, ТекстОшибки = "") Экспорт
	...
	Попытка
		...
		ПараметрыЗапроса.Вставить("УИДМЧД", Справочники.МашиночитаемыеДоверенности.ПолучитьУИДМЧД(ПараметрыЗапроса.ogrn, Перечисления.ПолномочияЭЦП.ФСС));
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Отправка запроса в ФСС'"), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(Информация));
...
Функция ПолучитьСертификатыМО(Филиал = Неопределено, ОГРН = Неопределено) Экспорт
	
	Если Филиал = Неопределено Тогда
		Возврат ЭЦП_УМЦ_Сервер.ПолучитьСертификатыМОПользователя(ОГРН, Перечисления.ПолномочияЭЦП.ФСС);
	Иначе
		Возврат ЭЦП_УМЦ_Сервер.ПолучитьСертификатыМОПользователя(Филиал, Перечисления.ПолномочияЭЦП.ФСС);
	КонецЕсли;
...
Функция ПослеОтправки(Результат, ПараметрыЗапроса, ТекстОшибки)
	...
	ИначеЕсли ИмяЗапроса = "GetLNListBySnilsRequest" Тогда
		...
		Если Результат <> Неопределено И Результат.Свойства().Получить("Data") <> Неопределено Тогда
			...
			Возврат МассивСтрок;
		Иначе
			Если Результат <> Неопределено И Результат.Свойства().Получить("mess") <> Неопределено Тогда
				ТекстОшибки = Результат.mess;
			Иначе
				ТекстОшибки = "Произошла неизвестная ошибка";
			КонецЕсли;
			Возврат Ложь;
		КонецЕсли;
