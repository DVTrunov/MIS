...
Процедура ЗагрузитьТранспортныйСправочник (СтруктураНастроек)
	...
	Исключение
		...
		ОбщегоНазначения.СообщитьПользователю("Не удалось прочитать полученный ответ сервера по причине:" + Символы.ПС + ИнформацияОбОшибке.Причина);
		Возврат;
	КонецПопытки;
...
Функция ПолучитьСтруктуруПечатиЭтикеток(ДанныеКонтейнеров, Лаборатория, ДокументСсылка, Клиент, ОписаниеОшибки) Экспорт
	...
	Для каждого  Стр Из ДанныеКонтейнеров Цикл
		Если Стр.Лаборатория.ВнешняяЛаборатория = Перечисления.ВнешниеЛаборатории.Гемотест Тогда
			...
			Область.Параметры.Биоматериал = Стр.БиоматериалПредставление;
			Область.Параметры.Дата = Формат(ДокументСсылка.Дата, "ДЛФ=D");
			Область.Параметры.Лаборатория = Стр.ИдПравила;
			Область.Параметры.Локализация = Стр.Контейнер;
			Область.Параметры.Проба = Стр.Биоматериал;
	...
	ТабличныйДокумент.ШиринаСтраницы  = 31;
