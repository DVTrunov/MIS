...
Функция ВыполнитьВыгрузкуИзмененийНаАТС(УзелОбмена, Сообщение, Ошибка, флагВыгружатьВсе) ЭКСПОРТ
	...
	контИнформВРегистреСведений = доп_ТелефонияСерверПереопределяемый.КонтактнаяИнформацияВРегистреСведений();
...
Процедура ПроверитьНомерНаАТС(УзелОбмена, НомерКонтрагента, Сообщение, Ошибка) ЭКСПОРТ
	...
	стрСокрНомер = доп_ТелефонияКлиентСервер.СократитьНомерДляПоиска(доп_ТелефонияКлиентСервер.ОчиститьНомерТолькоЦифры(НомерКонтрагента));
...
Функция ПолучитьВерсиюВебСервиса(УзелОбмена, Ошибка) ЭКСПОРТ
	...
	Попытка
		...
		всСсылка = доп_АТСВебСервисы.ПолучитьВебСервисКлиентМенеджер(стрХост);
...
Процедура СинхронизацияСАТС() ЭКСПОРТ
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	доп_ОбменАТС.Ссылка
	               |ИЗ
	               |	ПланОбмена.доп_ОбменАТС КАК доп_ОбменАТС
	               |ГДЕ
	               |	доп_ОбменАТС.ПометкаУдаления = ЛОЖЬ";
	...
	Для Каждого запись Из табл Цикл
		...
		Тогда
			...
			Если ЗначениеЗаполнено(стрОшибка) Тогда
				ЗаписьЖурналаРегистрации("ОЛИМП.АТС", УровеньЖурналаРегистрации.Ошибка,
					, "Ошибка выгрузки данных на ОЛИМП.АТС", стрОшибка);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла
КонецПроцедуры
...
Функция ПолучитьНомерПользователя(ПользовательСсылка)
	...
	Если НЕ ЗначениеЗаполнено(стрНомер) Тогда
		// получение из настроек ОЛИМП.Phone
		менеджЗапись = РегистрыСведений.доп_БитфонНастройки.СоздатьМенеджерЗаписи();
	...
	//
	Если НЕ ЗначениеЗаполнено(стрНомер) Тогда
		// получение из настроек подключения к ОЛИМП.АТС
		менеджЗапись = РегистрыСведений.доп_БитАТСНастройки.СоздатьМенеджерЗаписи();
		менеджЗапись.Пользователь = ПользовательСсылка;
		менеджЗапись.Прочитать();
		Если менеджЗапись.Выбран() Тогда
...
Функция ПроверитьНаличиеКонтрагентаВБазеДанных(всСсылка, идПодключения, базаИд, контрагентСсылка, Ошибка)
	...
	//
	менеджерСсылка = доп_ТелефонияСерверПереопределяемый.ПолучитьОсновногоМенеджераКонтрагента(контрагентСсылка);
	стрМенеджерИд = ПолучитьИдентификаторМенеджера(менеджерСсылка);
	стрМенеджерНомер = ПолучитьНомерПользователя(менеджерСсылка);
	//
	найден = Ложь;
...
Функция ПроверитьНаличиеНомераТелефонаВБазеДанных(всСсылка, идПодключения, базаИд, стрНомерКонтрагента, контрагентСсылка, Сообщение, Ошибка)
	...
	КонецЕсли;
	//
	стрНомер = доп_ТелефонияКлиентСервер.ОчиститьНомерТолькоЦифры(стрНомерКонтрагента);
...
Функция ОбновитьИзмененияМенеджеров(Узел, всСсылка, идПодключения, Сообщение, Ошибка, флагВыгружатьВсе)
	...
	метаданныеНомера.Добавить(Метаданные.РегистрыСведений.доп_БитфонНастройки);
	метаданныеНомера.Добавить(Метаданные.РегистрыСведений.доп_БитАТСНастройки);
		...
		Если ТипЗнч(объект) <> Тип("УдалениеОбъекта") Тогда
			Если НЕ объект.ЭтоГруппа Тогда
				...
				Если контрагентСсылка.ПометкаУдаления Тогда
					массивНомеров = доп_ТелефонияСерверПереопределяемый.НайтиНомераКонтрагента(контрагентСсылка);
					Если флагВыгружатьВсе Тогда
						Сообщение = Сообщение + Символы.ПС;
					КонецЕсли;
	...
	Пока выборка.Следующий() Цикл
		...
		Если ТипЗнч(объект) <> Тип("УдалениеОбъекта") Тогда
			Если НЕ объект.ЭтоГруппа Тогда
				...
				контрагентСсылка = доп_ТелефонияСерверПереопределяемый.ПолучитьКонтрагентаКонтактногоЛица(контактноеЛицоСсылка);
				Если контактноеЛицоСсылка.ПометкаУдаления Тогда
					массивНомеров = доп_ТелефонияСерверПереопределяемый.НайтиНомераКонтактногоЛица(контактноеЛицоСсылка);
					Если флагВыгружатьВсе Тогда
						Сообщение = Сообщение + Символы.ПС;
					КонецЕсли;
	...
	Пока выборка.Следующий() Цикл
		...
		Для Каждого записьРС Из наборЗаписейРС Цикл
			Если записьРС.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
				...
				Если ТипЗнч(записьРС.Объект) = Тип("СправочникСсылка." + доп_ТелефонияСерверПереопределяемый.ПолучитьИмяСправочникаКонтрагентов()) Тогда
					контрагентСсылка = записьРС.Объект;
					Если НЕ контрагентСсылка.ПометкаУдаления Тогда
						стрНомерКонтрагента = записьРС.Представление;
					КонецЕсли;
				ИначеЕсли ТипЗнч(записьРС.Объект) = Тип("СправочникСсылка." + доп_ТелефонияСерверПереопределяемый.ПолучитьИмяСправочникаКонтактныхЛиц()) Тогда
