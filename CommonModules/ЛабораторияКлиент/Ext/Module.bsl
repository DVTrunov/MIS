...
//
Процедура НапечататьЭтикеткиПоДокументу(ДокументСсылка, ДанныеКонтейнеров, Клиент = Неопределено, КодыПроб = Неопределено) Экспорт 
	
	Состояние(НСтр("ru='Фомирование этикеток...'"));
	
	Лаборатории = Новый Массив;
	Для Каждого Контейнер Из ДанныеКонтейнеров Цикл
		Если Лаборатории.Найти(Контейнер.Лаборатория) = Неопределено
			И (КодыПроб = Неопределено Или КодыПроб.Найти(Контейнер.КодПробы) <> Неопределено)
		Тогда
	...
	ПараметрыЗабора = Новый Структура("ДокументСсылка, Лаборатории, Клиент, ДанныеКонтейнеров, КодыПроб", ДокументСсылка, Лаборатории, Клиент, ДанныеКонтейнеров, КодыПроб);  
	ОповещениеПриЗавершении = Новый ОписаниеОповещения("НапечататьЭтикеткиПоДокументуНачалоПечати", ЭтотОбъект, ПараметрыЗабора);
	НапечататьЭтикеткиПоДокументуПредВыбором(ОповещениеПриЗавершении);
		
КонецПроцедуры

Процедура НапечататьЭтикеткиПоДокументуПредВыбором(ОповещениеПриЗавершении) Экспорт
...
Процедура НапечататьЭтикеткиПоДокументуНачалоПечати(Результат, ДопПараметры) Экспорт
	...
	Если Результат.Результат Тогда 
		...
		ДанныеЭтикеток = ЛабораторияСервер.СформироватьЭтикеткиКонтейнеров(ДопПараметры.ДокументСсылка, ДопПараметры.Лаборатории, ДопПараметры.Клиент, ДопПараметры.КодыПроб);
	...
	Иначе
		НапечататьЭтикеткиПоДокументуНаПринтере(ДопПараметры.ДокументСсылка, ДопПараметры.ДанныеКонтейнеров, ДопПараметры.Клиент, ДопПараметры.Лаборатории, ДопПараметры.КодыПроб);
	КонецЕсли;
...
//
Процедура НапечататьЭтикеткиПоДокументуНаПринтере(ДокументСсылка, ДанныеКонтейнеров, Клиент, Лаборатории, КодыПроб)
	...
	Для Каждого Лаборатория Из Лаборатории Цикл
		Если ЛабораторияСервер.ИспользуетсяПечатьЭтикеток(Лаборатория) Тогда
			СтруктураПечати = ЛабораторияСервер.ПолучитьСтруктуруПечатиЭтикеток(ДанныеКонтейнеров, Лаборатория, ДокументСсылка, Клиент, КодыПроб);	
