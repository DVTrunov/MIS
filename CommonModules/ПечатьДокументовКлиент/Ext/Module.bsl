...
Процедура УстановитьЗаголовокПечатнойФормы(ЭтаФорма, ПоказыватьКнопкуПечатьПоУмолчанию = Истина) Экспорт
	...
	ЧастиИмениФормы	= СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ЭтаФорма.ИмяФормы,".");
	ПолноеИмяОбъекта = ЧастиИмениФормы[0] + "." + ЧастиИмениФормы[1];
	
	Если ЭтаФорма.Элементы.Найти("ФормаОбщаяКомандаПечатьПоУмолчанию") <> Неопределено Тогда
		Заголовок = ПолучитьИмяПечатнойФормыДокументаПоУмолчаниюИзКэша(ПолноеИмяОбъекта);
...
Функция ПолучитьИмяПечатнойФормыДокументаПоУмолчаниюИзКэша(ИмяОбъекта)
	...
	Если глКэшПечатныхФорм = Неопределено Тогда
		Возврат Неопределено;
	Иначе
		Для Каждого ОписаниеПечатнойФормы Из глКэшПечатныхФорм Цикл
			Если ИмяОбъекта = ОписаниеПечатнойФормы.ИмяОбъекта Тогда
				Возврат ОписаниеПечатнойФормы.Представление;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
