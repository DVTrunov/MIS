...
Функция СоздатьСтруктуруЗаписиРегистра(ПолноеИмяИлиМетаданные, ВключатьИзмерения = Истина, ВключатьРесурсы = Истина, ВключатьРеквизиты = Истина, ВключатьСтандартныеРеквизиты = Истина) Экспорт 
	
	СтруктураЗаписи = Новый Структура;
	
	МетаданныеРегистра = ПолноеИмяИлиМетаданные;
	Если ТипЗнч(ПолноеИмяИлиМетаданные) = Тип("Строка") Тогда 
		МетаданныеРегистра = Метаданные.НайтиПоПолномуИмени(ПолноеИмяИлиМетаданные);
	КонецЕсли;
	
	Если ВключатьИзмерения Тогда 
		Для Каждого Измерение Из МетаданныеРегистра.Измерения Цикл 
			СтруктураЗаписи.Вставить(Измерение.Имя, Измерение.Тип.ПривестиЗначение(Неопределено));
		КонецЦикла;
		Если МетаданныеРегистра.ПериодичностьРегистраСведений <> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
			СтруктураЗаписи.Вставить("Период", Дата(1,1,1));
		КонецЕсли;
		Если МетаданныеРегистра.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда
			СтруктураЗаписи.Вставить("Регистратор", МетаданныеРегистра.СтандартныеРеквизиты.Регистратор.Тип.ПривестиЗначение(Неопределено));
		КонецЕсли;
	КонецЕсли;
	
	Если ВключатьРесурсы Тогда 
		Для Каждого Ресурс Из МетаданныеРегистра.Ресурсы Цикл
			СтруктураЗаписи.Вставить(Ресурс.Имя, Ресурс.Тип.ПривестиЗначение(Неопределено));
		КонецЦикла;
	КонецЕсли;
	
	Если ВключатьРеквизиты Тогда 
		Для Каждого Реквизит Из МетаданныеРегистра.Реквизиты Цикл
			СтруктураЗаписи.Вставить(Реквизит.Имя, Реквизит.Тип.ПривестиЗначение(Неопределено));
		КонецЦикла;
	КонецЕсли;
	
	Если ВключатьСтандартныеРеквизиты Тогда 
