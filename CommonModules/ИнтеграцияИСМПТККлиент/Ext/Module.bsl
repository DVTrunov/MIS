...
Процедура ПодписатьДокументыИСЦЭДМ(КоллекцияСгруппированныхДокументовИСЦЭДМ)
	...
	Для Каждого ДанныеДляПодписи Из СоответствиеДляПодписиТитулов Цикл
		...
		Если Не ДанныеДляПодписи.Значение.МассивДокументов.Количество() = 0 Тогда 
			...
			ПодписанныеДокументы = СоздатьЭЦП_CMS_NCA_Layer(МассивДанныхДляПодписи, 2, Ложь, Ложь, ИдентификаторОрганизации, ТипДокумента);
			
			Если Не ПодписанныеДокументы = "НеВыбран" Тогда 
...
Функция ПодписатьДокументы(КоллекцияДокументовДляПодписи, Организация, ТипДокумента = Неопределено)
	...
	МассивПодписанныхДокументов = СоздатьЭЦП_CMS_NCA_Layer(МассивДокументовДляПодписи, 1, Ложь, Истина, ИдентификаторОрганизации, ТипДокумента);
...
Процедура ОтправитьДокументыОтчетОПередачеКИОтНерезидентаРКИСЦЭДМ(ДополнительныеПараметры)
	...
	ОповеститьФормы(РаботаСДокументамиИСМПТККлиентСервер.ИмяСобытияЗаписьОтчетОПередачеКИОтНерезидентаРКИСЦЭДМ(), МассивИсходящихДокументов);
...
Процедура ПодтвердитьВходящиеДокументыИСМПТЗавершение(ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Для Каждого ЭлементКоллекции Из КоллекцияДанныхДокументов Цикл
			...
			ПодписанныйТитул = СоздатьЭЦП_CMS_NCA_Layer(ТитулБезПодписи, 2, Ложь, Ложь, ИдентификаторОрганизации, ТипДокумента, Ложь);
...
Процедура ПодтвердитьВходящиеДокументыИСЦЭДМЗавершение(ДополнительныеПараметры) Экспорт
...
Процедура ОтклонитьВходящиеДокументыИСЦЭДМЗавершение(ДополнительныеПараметры) Экспорт
...
Процедура ОтозватьИсходящиеДокументыИСЦЭДМЗавершение(ДополнительныеПараметры) Экспорт
...
Функция СоздатьЭЦП_CMS_NCA_Layer(Знач ДанныеXML, ТипВходящихДанных, Знач ВключатьДанныеВПодпись, КодироватьВbase64 = Ложь, ИдентификаторКлиента = "", ТипДокумента = Неопределено, Отправка = Истина) Экспорт
	...
	МассивДляПодписи 	 = ОбработатьДанныеДляПодписи(ДанныеXML, КодироватьВbase64, ПодписьСписка);
	ДанныеДляПодписиJSON = ЗаписатьЗначениеJSON_Маркировка(МассивДляПодписи);
	
	Попытка
		
		Криптопровайдер = ИнтеграцияИСМПТККлиентСерверПереопределяемый.НовыйКриптопровайдерWSSocket();
		...
		ОтветОтКриптопровайдера = Криптопровайдер.GetCMSesSign(ИдентификаторКлиента, "1CService", ПараметрыExtras, ДанныеДляПодписиJSON, ВключатьДанныеВПодпись, ВключатьШтампВремениВПодпись);
		
	Исключение
		
		Ошибка = ОшибкаКриптопровайдера(Криптопровайдер, ИнформацияОбОшибке());
		ТекстОшибки = НСтр("ru = 'Произошла ошибка при работе криптопровайдера.'")
					+ Символы.ПС
					+ Ошибка.ПодробноеПредставление;
		ИмяСобытия  = РаботаСТекстамиИСМПТККлиентСервер.ТекстСообщенияОшибкаВФункцииСПараметром();
		ИмяСобытия  = СтрЗаменить(ИмяСобытия, "%ИмяФункции%", "ИнтеграцияИСМПТККлиент.СоздатьЭЦП_CMS_NCA_Layer");
		ОбщегоНазначенияИСМПТКВызовСервера.СоздатьЗаписьЖурналаРегистрации(ИмяСобытия, "Ошибка",,, ТекстОшибки);
		
		ТекстИсключения = НСтр("ru = 'Не удалось сформировать ЭЦП в формате CMS.'")
						+ Символы.ПС
						+ Ошибка.КраткоеПредставление;
	...
	Попытка
		...
		Если ПодписанныеДанныеСтруктура.Свойство("responseobject") И ПодписанныеДанныеСтруктура.code = "200" Тогда
			...
			Если Не ПодписьСписка Тогда 
				ПодписанныеДанные = ПодписанныеДанные[0];
			КонецЕсли;
	...
	Исключение
		...
		ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(НСтр("ru = 'Не удалось прочитать ответ криптопровайдера: '") + СокрЛП(ОтветОтКриптопровайдера));
		...
		Иначе
			ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(НСтр("ru = 'Криптопровайдер недоступен.'"));
		КонецЕсли;
...
Процедура ВыборФайлаCSVЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если Не ВыбранныеФайлы = Неопределено Тогда
		...
		Если ИнтеграцияИСМПТКВызовСервера.ИспользоватьАвтоОпределениеЛогистическихКодовИСМПТК() 
			И НЕ ДополнительныеПараметры.ИмяФормы = ОбщегоНазначенияИСМПТКВызовСервера.ПолучитьИмяФормыСверкаВходящийАктИСМПТ() //Форма сверки - не поддерживает запрос состава упаковок 
			И СтрНайти(ДополнительныеПараметры.ИмяФормы, "ИСЦЭДМ") = 0 Тогда //В Документах ИС ЦЭДМ не используем 
			...
			Если ДополнительныеПараметры.ИмяФормы = ОбщегоНазначенияИСМПТКВызовСервера.ПолучитьИмяФормыРабочееМестоПолучениеИнформацииОВладельцеИСостоянииКМ() Тогда
...
Процедура ЗагрузитьКодыМаркировкиИзФайлаЗавершение(РезультатВыполнения, СтруктураПараметров) Экспорт
	...
	ДлительнаяОперация = РаботаСДокументамиИСМПТКВызовСервера.ВыполнитьДобавлениеКМИзФайлаВФоне(ПараметрыДобавленияКодов, ПараметрыСканирования, ТокенАвторизации);
...
Процедура ПолучитьКлючАвторизации(СобытиеПослеАвторизации, СтруктурныеЕдиницы)
    ...
    ИнтерфейсИСМПТККлиент.ПроверитьТокенИВыбратьКлючПриНебходимостиИСМПТ(СобытиеПослеАвторизации, ПараметрыФормы, СтруктурныеЕдиницы);
