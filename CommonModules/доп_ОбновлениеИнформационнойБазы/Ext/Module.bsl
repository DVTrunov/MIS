...
Функция ВыполнитьОбновление() Экспорт
	...
	#Область КОРП
	Если РелизБолееРанний(ТекущаяВерсияИБ,"2.1.0.0") Тогда  
		ВыполнитьОбновлениеС20(ТекущаяВерсияИБ);
		Возврат Истина;
	КонецЕсли;
	#КонецОбласти
	...
	ЕСЛИ РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.23") ТОГДА // Глобальная отсечка проверок между 1.0.14.1 и 2.1.25.23.
	...
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
		...
		ДвоичныеДанныеВыгрузки = Справочники.ВидыМедицинскихКарт.ПолучитьМакет("корпПараметрыМедкарты003у");
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки);
		
		ДвоичныеДанныеВыгрузки = Справочники.ВидыМедицинскихКарт.ПолучитьМакет("корпПараметрыМедкарты111у");
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки);
	...
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
		...
		стомОбновлениеИБ.ЗаполнитьСтандартныеЗубныеКарты();
		стомОбновлениеИБ.ЗаполнитьВычисляемыеБлокиЗубнойКарты();
		
		ЗавершитьТранзакциюОбновлиенияИБ(НоваяВерсияИБ); // Завершение транзакции.
	КонецЕсли;
		
	НоваяВерсияИБ ="2.1.3.1";
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
		...
		// Действия обновления, выполняемые в транзакции.
		// заполнение предопределенных элементов нового справочника драйверов.
	...
	СтандартноеОбновление("2.1.5.10",	"21510"	 ,	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений);
		
	НоваяВерсияИБ ="2.1.6.1";
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	НоваяВерсияИБ ="2.1.9.1"; // 2.0.32.13
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
		...
		стомОбновлениеИБ.РеструктуризоватьКонструкцииЗубовКлиентов();
		#КонецОбласти
	...
	НоваяВерсияИБ ="2.1.9.2"; // 2.0.32.23
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	НоваяВерсияИБ ="2.1.9.3"; // 2.0.32.27
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
		...
		#Область Стоматология
		стомОбновлениеИБ.ЗаменитьНеиспользуемыеМакетыМедкарт();
		#КонецОбласти
		
		ЗаменитьНеиспользуемыеМакетыМедкарт025и043();
		
		// Загрузка параметров медкарт
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Справочники.ВидыМедицинскихКарт.ПолучитьМакет("ПараметрыМедкарты025Приказ834н"), Истина);
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Справочники.ВидыМедицинскихКарт.ПолучитьМакет("стомПараметрыМедкарты043У"), Истина);
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Справочники.ВидыМедицинскихКарт.ПолучитьМакет("корпПараметрыМедкарты003у"), Истина);
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Справочники.ВидыМедицинскихКарт.ПолучитьМакет("корпПараметрыМедкарты111у"), Истина);
	...
	НоваяВерсияИБ ="2.1.10.1"; // 2.0.32.32
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
		НачатьТранзакциюОбновленияИБ(ВыполненныеОбновления); // Начало транзакции обновления.
	...
	НоваяВерсияИБ ="2.1.10.2"; // 2.0.32.35
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	СтандартноеОбновление("2.1.10.5",	"2110"	 ,	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений); // 2.0.32.42
	
	НоваяВерсияИБ ="2.1.11.1"; //2.0.33.1
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	НоваяВерсияИБ ="2.1.11.2"; //2.0.33.2
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	НоваяВерсияИБ ="2.1.11.3"; //2.0.33.8
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	НоваяВерсияИБ ="2.1.11.5"; //2.0.33.12
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	НоваяВерсияИБ ="2.1.12.1"; // 2.0.34.16
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	НоваяВерсияИБ ="2.1.12.2"; // 2.0.34.17
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	НоваяВерсияИБ ="2.1.13.1"; // 2.0.36.2
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда

		НачатьТранзакциюОбновленияИБ(ВыполненныеОбновления); // Начало транзакции обновления.
		
		//УправлениеНастройками.УстановитьЗначениеПараметраУчетнойПолитики("ИспользоватьХозяйственныйБлок", Истина);
		//УправлениеНастройками.УстановитьЗначениеПараметраУчетнойПолитики("ИспользоватьАнкетирование", Истина);
		ОбновитьДанныеВариантовОтчетов();
		
		ПервоначальноеЗаполнениеВопросаАнкетированияNPS();
		
		СнятьПометкуУдаленияРанееПредопределенныхСостоянийЗубов();
	...
	НоваяВерсияИБ ="2.1.13.5"; // УМЦ 2.0.36.5
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	НоваяВерсияИБ ="2.1.14.1"; // УМЦ 2.0.37.7
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	НоваяВерсияИБ ="2.1.14.5"; // УМЦ 2.0.37.10
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
	...
	НоваяВерсияИБ ="2.1.15.1"; // 2.0.38.2
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
		...
		ЗаполнитьЭтоВечерВВыполненииДействийСтационара();
		ПеренестиОсновнойШаблонОсмотраВРегистрШаблоновМедицинскихДокументов();
		ОбновитьДанныеВариантовОтчетов();
	...
	СтандартноеОбновление("2.1.16.1",	"2116", 	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, Истина);	// 2.0.39.5.
	СтандартноеОбновление("2.1.16.4",	, 			ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина);
	СтандартноеОбновление("2.1.16.7",	,			ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , Истина,);
	СтандартноеОбновление("2.1.16.8",	"2116_8",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );			// 2.0.39.15.
	СтандартноеОбновление("2.1.16.9",	"2116_9",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина);		// 2.0.39.18.
	СтандартноеОбновление("2.1.16.10",	"2116_10",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина);		// 2.0.39.19.
	
	// Дополнительно к 2.1.16.13 (2.0.39.25)
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.16.13") Тогда
		ДвоичныеДанныеВыгрузки = Справочники.РазделыМедицинскихКарт.ПолучитьМакет("РазделыМедкарты025у");
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки);
		
		ДвоичныеДанныеВыгрузки = Справочники.РазделыМедицинскихКарт.ПолучитьМакет("РазделыМедкарты003у");
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки);
		
		ПерезагрузитьСправочникВредныхФакторов();
	КонецЕсли;
	СтандартноеОбновление("2.1.16.13",	"2116_13",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // УМЦ 2.0.39.25.
	
	НоваяВерсияИБ ="2.1.16.15"; // 2.0.38.2
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.16.15") Тогда // УМЦ 2.0.39.28.
		НачатьТранзакциюОбновленияИБ(ВыполненныеОбновления); // Начало транзакции обновления.
		ПерезагрузитьСправочникВредныхФакторов();
		ЗавершитьТранзакциюОбновлиенияИБ(НоваяВерсияИБ); // Завершение транзакции.
	КонецЕсли;
	...
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.17.1") Тогда
		ДвоичныеДанныеВыгрузки = Справочники.ПоказателиПланирования.ПолучитьМакет("ПервоначальноеЗаполнение");
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки);
	КонецЕсли;
	СтандартноеОбновление("2.1.17.10",	"2117_1",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);		
	
	// Дополнительно к 2.1.18.1 // 2.0.40.2
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.18.1") Тогда
		УстановитьПривилегированныйРежим(Истина);
		...
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
	СтандартноеОбновление("2.1.18.1",	 "2118_1",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.19.1") Тогда
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.19.1",	"2119_1"	 ,	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // УМЦ 2.0.41.3.
	СтандартноеОбновление("2.1.19.3",	"2119_3"	 ,	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // УМЦ 2.0.41.6.
	СтандартноеОбновление("2.1.19.4",	"2119_4"	 ,	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // УМЦ 2.0.41.8.

	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.19.5") Тогда
		СтруктураПараметров = Новый Структура("АвтоподстановкаИмениОтчества", Истина);
		ЗаполнениеНовыхПараметровУчета(СтруктураПараметров, Истина);
	КонецЕсли;
	СтандартноеОбновление("2.1.19.5", "2119_5",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, Истина, Истина); // УМЦ 2.0.41.9.
	СтандартноеОбновление("2.1.20.1", "2120",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // УМЦ 2.0.42.3.
	
	// Обовление стоматологической карты.
	НоваяВерсияИБ ="2.1.20.3";
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
		
		НачатьТранзакциюОбновленияИБ(ВыполненныеОбновления); // Начало транзакции обновления.
		
		стомОбновлениеИБ.ПеренестиОбозначенияЗубовВСписокВариантов();
		стомОбновлениеИБ.ПеренестиНастройкиУчетнойПолитики();
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Справочники.стомВидыЗубныхКарт.ПолучитьМакет("ЗубнаяКартаОбновление231"));
		
		ЗавершитьТранзакциюОбновлиенияИБ(НоваяВерсияИБ); // Завершение транзакции.
	КонецЕсли;
	СтандартноеОбновление("2.1.20.3",	"21203",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	СтандартноеОбновление("2.1.20.4", 	"2120_4",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // УМЦ 2.0.42.7.
	СтандартноеОбновление("2.1.20.8", 	"2120_8",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // УМЦ 2.0.42.12.
	СтандартноеОбновление("2.1.20.9", 	"2120_9",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // УМЦ 2.0.42.14.
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.20.10") Тогда
		ОбновитьНаименованияРегЗаданийПоСинонимам();
		ПерезагрузитьСправочникВредныхФакторов();
	КонецЕсли;
	СтандартноеОбновление("2.1.20.10", "2120_10",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // УМЦ 2.0.42.15
	СтандартноеОбновление("2.1.20.11", "2120_11",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // УМЦ 2.0.42.18
	СтандартноеОбновление("2.1.21.1",  "2121_1",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // УМЦ 2.0.43.6
	СтандартноеОбновление("2.1.21.7",  "2121_7",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	СтандартноеОбновление("2.1.21.8",  "2121_8",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	СтандартноеОбновление("2.1.21.9",  "2121_9",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	СтандартноеОбновление("2.1.21.10", "2121_10",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	СтандартноеОбновление("2.1.21.11", "2121_11",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	СтандартноеОбновление("2.1.21.12", "2121_12",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.1") Тогда // 2.0.46.1
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.22.1", "2122_1",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, Истина, Истина);
	СтандартноеОбновление("2.1.22.4", "2122_4",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, Истина, Истина);
	СтандартноеОбновление("2.1.22.5", "2122_5",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	СтандартноеОбновление("2.1.22.7", "2122_7",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.12") Тогда // 2.0.46.20.
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.22.12",	,	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	СтандартноеОбновление("2.1.22.14",	"2122_14",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	СтандартноеОбновление("2.1.22.15",	"2122_15",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.0.46.25") Тогда // 2.0.46.26
		Helix_ОбновитьПараметрыЗабора();
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.22.16",	"2122_16",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, Истина, Истина);
	СтандартноеОбновление("2.1.22.17",	"2122_17",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.19") Тогда // 2.0.46.30
		МедосмотрыОбновитьНа29н();
	КонецЕсли;
	СтандартноеОбновление("2.1.22.19",	"2122_19",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	СтандартноеОбновление("2.1.22.21",	"2122_21",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.46.33
	СтандартноеОбновление("2.1.22.22",	"2122_22",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.46.35
	СтандартноеОбновление("2.1.22.24",	"2122_24",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // 2.0.46.37
	СтандартноеОбновление("2.1.22.25",	"2122_25",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , ); // 2.0.46.38
	СтандартноеОбновление("2.1.22.27",	"2122_27",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , ); // 2.0.46.39
	СтандартноеОбновление("2.1.22.28",	"2122_28",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.46.43
	СтандартноеОбновление("2.1.22.30",	"2122_30",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , ); // 2.0.46.47
	
	// Сегментация в 2.0.46.49
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.31") Тогда
		ПереносВОтдельныйРегистрИсторииСегментации_Этап1();
	КонецЕсли;
	СтандартноеОбновление("2.1.22.31",, ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.32") Тогда
		ПереносВОтдельныйРегистрИсторииСегментации_Этап2();
	КонецЕсли;
	СтандартноеОбновление("2.1.22.32", "2122_32", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); //  2.0.46.49
	СтандартноеОбновление("2.1.22.34", "2122_34", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); //  2.0.46.52
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.35") Тогда // 2.0.46.53
		Helix_ОбновитьПараметрыЗабора();
	КонецЕсли;
	СтандартноеОбновление("2.1.22.35", "2122_35", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	СтандартноеОбновление("2.1.22.36", "2122_36", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.46.54
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.38") Тогда
		Справочники.МедицинскиеОтчеты.ЗаполнитьСписокМедицинскихОтчетов();
	КонецЕсли;
	СтандартноеОбновление("2.1.22.38", "2122_38", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // 2.0.46.57
	
	СтандартноеОбновление("2.1.22.39", "2122_39", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина); // 2.0.46.59
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.40") Тогда // 2.0.46.60, вкл. ЭЛН ФСС
		ОбновитьДанныеОРодственникахВЛисткахНетрудоспособности();
	КонецЕсли;
	СтандартноеОбновление("2.1.22.40", "2122_40", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина); // 2.0.46.60
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.41") Тогда // 2.0.46.62
		Helix_ОбновитьПараметрыЗабора();
	КонецЕсли;
	СтандартноеОбновление("2.1.22.41", "2122_41", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина); // 2.0.46.62
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.42") Тогда
		ОбменФСС.ОбновитьСертификатыКриптографииФСС();
	КонецЕсли;
	СтандартноеОбновление("2.1.22.42", "2122_42", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина); // 2.0.46.63
	СтандартноеОбновление("2.1.22.43", "2122_43", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // 2.0.46.64
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.44") Тогда // 2.0.46.65
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.22.44", "2122_44", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , Истина, Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.22.45") Тогда // 2.0.46.66
		ПереименоватьВидКИНомерICQ();
	КонецЕсли;
	СтандартноеОбновление("2.1.22.45", "2122_45", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , Истина, Истина);
	СтандартноеОбновление("2.1.22.46", "2122_46", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.23.1") Тогда // 2.0.47.5
		ПервоначальноеОбновлениеЕГИСЗ();
	КонецЕсли;
	СтандартноеОбновление("2.1.23.1",	"2123",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, 		, Истина);
	СтандартноеОбновление("2.1.23.2", , ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , Истина, Истина); // 2.0.47.6
	
	СтандартноеОбновление("2.1.23.3",	"2123_3",	ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , 		, ); // 2.0.47.7 (46.69)
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.23.4") Тогда // 2.0.46.70
		ОчиститьВерсииКлассификаторовМинздрава();
		СитиЛаб_ИнтеграцияСервер.ПервоначальнаяЗагрузкаПараметровЗабора();	
	КонецЕсли;
	СтандартноеОбновление("2.1.23.4", "2123_4", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.47.8 (46.70, 22.50)
	СтандартноеОбновление("2.1.23.5", "2123_5", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.47.9 (46.71, 22.51)
	СтандартноеОбновление("2.1.23.6", "2123_6", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.47.10(46.72, 22.52)
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.23.9") Тогда // 2.0.46.77 (22.57, 47.15)
		ОчиститьВерсииКлассификаторовМинздрава();
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.23.9", "2123_9", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.46.77
	СтандартноеОбновление("2.1.23.10", "2123_10", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.46.78, 47.16, 22.58	
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.23.11") Тогда
		УстановитьТипыРегламентированныхСтандартов();
		ОбновитьСтарыеСобственныеЭЛННаВерхнийРегистр();
	КонецЕсли;
	СтандартноеОбновление("2.1.23.11", "2123_11", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	СтандартноеОбновление("2.1.23.12", "2123_12", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.23.15") Тогда // 47.21
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.23.15", "2123_15", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , );  // 47.21
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.23.16") Тогда // 47.22
		ОбменФСС.ОбновитьСертификатыКриптографииФСС();
	КонецЕсли;
	СтандартноеОбновление("2.1.23.16", "2123_16", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , );  // 47.22
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.23.17") Тогда // 47.23
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.23.17", "2123_17", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);  // 47.23
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.23.19") Тогда // 47.25
		ОчиститьВерсиюКлассификатораРолиПриПодписиВитальныеПараметры();
	КонецЕсли;
	СтандартноеОбновление("2.1.23.19", "2123_19", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );	// 47.25
	СтандартноеОбновление("2.1.23.20", "2123_20", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , );	// 47.26
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.1") Тогда
		ПредупреждениеСовместимостиШаблоновWORD_2048();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.1", "2124_1", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина); 
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.2") Тогда // 48.5
		РазархивироватьГруппыНМУ();
		ОчиститьВерсиюКлассификатораНМУ();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.2", , ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.5") Тогда // 48.8
		ПроставитьАвтономныеЭТЗапрашиватьПодтверждение();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.5", "2124_5", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.6") Тогда // 48.11
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.24.6", "2124_6", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений,  , Истина, Истина); // 48.9
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.7") Тогда // 48.12
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Справочники.ВидыМедицинскихКарт.ПолучитьМакет("корпПараметрыМедкарты111у"), Истина); // Обновление до 111/у-20.
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.24.7", "2124_7", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	СтандартноеОбновление("2.1.24.8", "2124_8", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	СтандартноеОбновление("2.1.24.9", "2124_9", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, Истина, Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.11") Тогда // 48.14 повторно
		...
		ОчиститьВерсиюКлассификатораКаналовГоспитализации();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.11", "2124_11", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, Истина, Истина);
	
	// Релиз 2.0.48.16. Проведена унификация имён метаданных в рамках линейки решений "ОЛИМП.Медицина и Красота".
	...
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.12") Тогда
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.24.12", "2124_12", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.13") Тогда
		ОчиститьВерсиюКлассификатораНМУ();
		ОбновитьСтандартыЛечения();
		Обновить1СДрайверСШК_COM_на_Native();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.13", "2124_13", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, Истина, Истина);
	СтандартноеОбновление("2.1.24.14", "2124_14", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	СтандартноеОбновление("2.1.24.15", "2124_15", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.16") Тогда // УМЦ 2.0.48.31
		НоваяНастройкаЗаСчетФирмыВидОплатыИспользуется();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.16", "2124_16", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина); // УМЦ 2.0.48.31
	СтандартноеОбновление("2.1.24.17", "2124_17", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина); // УМЦ 2.0.48.32
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.18") Тогда
		РегистрыСведений.АдресныеОбъекты.ОбновитьСоставСубъектовРФПоКлассификатору();
		ПечатьЭтикетокИЦенниковБПО.ЗаполнитьПредопределенныеЭлементы();
		ЗаполнитьТипСпособовБезналичнойОплаты();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.18", "2124_18", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	СтандартноеОбновление("2.1.24.21", "2124_21", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);
	СтандартноеОбновление("2.1.24.22", "2124_22", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.23") Тогда
		Стоматология_ПереименоватьПараметрПрорезаниеЗубов();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.23", , ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.24") Тогда		
		КОРП_ОбновитьСтационарПод530н();
		Файлы_ЗаполнитьРеквизитКонечныйВладелецХранилищеДопИнформации();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.24", "2124_24", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина , Истина, Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.25") Тогда
		Справочники.удалитьПричиныОтказаВГоспитализации.ЗаполнитьДанныеСправочника(Ложь, Ложь);
		ЗаполнитьРоссиюПоКлассификатору();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.25", , ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина , Истина, Истина);
	СтандартноеОбновление("2.1.24.26", "2124_26", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.27") Тогда
		Справочники.МедицинскиеОтчеты.ЗаполнитьСписокМедицинскихОтчетов();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.27", "2124_27", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.48.41-
	
    Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.28") Тогда
      	ЭЛН_ЗаполнитьРеквизитДатаОкончания();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.28", "2124_28", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );  // 2.0.48.41-,42-
	СтандартноеОбновление("2.1.24.29", "2124_29", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.30") Тогда
		Справочники.МедицинскиеОтчеты.ЗаполнитьСписокМедицинскихОтчетов();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.30", "2124_30", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.31") Тогда
		КОРП_ПростановкаКоекВРегистрСостоянияЗанятости();
		КОРП_ПроставитьПодразделениеВРегистрКлиентыСтационара();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.31", "2124_31", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.33") Тогда
		Справочники.МедицинскиеОтчеты.ЗаполнитьСписокМедицинскихОтчетов();
		КОРП_СтационарУдалениеВыполненияПланаЛеченияУслугДляДействияМедсестрыЗаСутки();
		КОРП_ПростановкаКоекВРегистрСостоянияЗанятости();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.33", "2124_33", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.36") Тогда  // 2.0.48.47
		ОбменФСС.ОбновитьСертификатыКриптографииФСС();
		ЗаполнитьЗначениеКоличествоПлатежныхДокументов();
		ОбновлениеИнформационнойБазыБПО.УстановитьСрокХраненияОпераций(); // БПО 3.1.8.1.
	КонецЕсли;
	СтандартноеОбновление("2.1.24.36", "2124_36", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.48.47
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.38") Тогда
		стомОбновлениеИБ.УстановитьНастройкуКэшированияЗК();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.38", "2124_38", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.39") Тогда
		КОРП_ЗаполнитьДанныеРегистраОтчетности007у();	
	КонецЕсли;
	СтандартноеОбновление("2.1.24.39", "2124_39", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.40") Тогда // 2.0.48.50
		ПараметрыЗабораАнализовОбновитьАлгоритмамиСрокамиПовторности();
	КонецЕсли;
	СтандартноеОбновление("2.1.24.40", "2124_40", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.48.50
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.24.42") Тогда // 2.0.48.52
		ПерезаписатьВидыРабочегоВремениГрафика(); // Появление флаша "РабочееВремяТабеля".
	КонецЕсли;
	СтандартноеОбновление("2.1.24.42", "2124_42", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.1") Тогда // 2.0.49.5
		...
		ОрганизацииПерезаполнениеВидовПолисов();
		...
		ОчиститьВерсиюКлассификатораЛабораторныхТестов();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.1", "2125_1", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // 2.0.49.5
	СтандартноеОбновление("2.1.25.3", "2125_3", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.49.8
	СтандартноеОбновление("2.1.25.4", "2125_4", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина);  // 2.0.49.10
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.5") Тогда // 2.0.49.11
		ЗаполнитьВредныеФакторыВПМОНеЗавершенныхСПМО();
		ЗагрузитьПараметрыHTMLПрофпатолога();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.5", "2125_5", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина);

	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.7") Тогда // 2.0.49.13
		ЗаполнитьИспользуемуюВерсиюСервераЛицензированияПриОбновлении();
		ОбновитьРекизитОрганизацияПодключаемоеОборудование();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.7", "2125_7", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, Истина, Истина);

	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.8") Тогда // 2.0.49.14
		УстановитьДатуНачалаРаботыСАбонементамиПоСхемеАБ_4_20_13041();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.8", "2125_8", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина);
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.9") Тогда // 2.0.49.16
		УстановитьВидДокументаДляПриказа29н();
		УстановитьТипФайловРедактируемыхЭлементовШаблонаПриема();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.9", "2125_9", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.49.16
	СтандартноеОбновление("2.1.25.10", "2125_10", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина); // 2.0.49.16 + физ.лица.
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.11") Тогда // 2.0.49.18
		ОчиститьВерсиюКлассификатораЛьготныхКатегорийНаселения();
		ЗагрузитьПараметрыHTMLПрофпатолога();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.11", "2125_11", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина); // 2.0.49.18
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.12") Тогда // 2.0.49.19
		ЗаполнитьВидИсследованийУДействийМедосмотра();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.12", "2125_12", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // 2.0.49.19
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.13") Тогда // 2.0.49.20
		ОбновитьСтандартыЛечения();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.13", "2125_13", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );
	СтандартноеОбновление("2.1.25.14", "2125_14", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );  // 2.0.49.21
	СтандартноеОбновление("2.1.25.15", "2125_15", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , );  // 2.0.49.22
	СтандартноеОбновление("2.1.25.16", "2125_16", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , );  // 2.0.49.24
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.17") Тогда // 2.0.49.25
	...
	КонецЕсли;
	СтандартноеОбновление("2.1.25.17", "2125_17", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );  // 2.0.49.25
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.18") Тогда // 2.0.49.26
		УстановитьВШаблонахЗаполнениеДиагнозовИзТабличнойЧасти();
		УстановитьРежимЗагрузкиКлассификаторМО_2049_26();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.18", "2125_18", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.49.26
	СтандартноеОбновление("2.1.25.19", "2125_19", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // 2.0.49.27
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.20") Тогда // 2.0.49.28
		УстановитьРежимЗагрузкиКлассификаторДолжностейФРМР_2049_28();
		Справочники.МедицинскиеОтчеты.ЗаполнитьСписокМедицинскихОтчетов();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.20", "2125_20", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // 2.0.49.28
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.21") Тогда // 2.0.49.29
		// Принудительная дозагрузка регистров сведений медосмотров 29н
		ДвоичныеДанныеВыгрузки = Справочники.ПереченьВредныхФакторовИРабот.ПолучитьМакет("Обновление20_49_29");
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки, Истина, Истина);
	КонецЕсли;
	СтандартноеОбновление("2.1.25.21", "2125_21", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // 2.0.49.29
	СтандартноеОбновление("2.1.25.22", "2125_22", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , ); // 2.0.49.31
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.23") Тогда
		ДвоичныеДанныеВыгрузки = Справочники.РегистрируемыеЭлектронныеМедДокументы.ПолучитьМакет("НаименованияФиксированныхВерсий");
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки, Истина, Истина);
	КонецЕсли;
	СтандартноеОбновление("2.1.25.23", "2125_23", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.49.32
	
	КОНЕЦЕСЛИ; // Конец глобальной отсечки Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.23") Тогда
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.24") Тогда
		ОбновитьСтандартныеАлгоритмыШаблоновWord();
		ОбновитьСтандартныеАлгоритмыВычисляемыхЗначенийПараметров();
		
		ОбновитьБлокЗП_20_49_33();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.24", "2125_24", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // 2.0.49.33
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.25") Тогда
		УстановитьВидМаркированнойПродукцииДляМДЛП();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.25", "2125_25", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина); // 2.0.49.35
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.26") Тогда
		Справочники.ШаблоныЭтикетокИЦенниковБПО.ПриНачальномЗаполненииЭлементов(Истина);
		УправлениеНастройкамиПереопределяемый.УстановитьПараметрУчета("ЭтикеткаВнутреннейЛаборатории", Справочники.ШаблоныЭтикетокИЦенниковБПО.ЭтикеткаВнутреннейЛаб);
	КонецЕсли;
	СтандартноеОбновление("2.1.25.26", "2125_26", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.49.36 (? 49.37).
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.27") Тогда
		Медосмотры_ЗагрузитьКлассификаторВидовПолисовОМС();
		Медосмотры_ЗагрузитьПриказ1252нИПриказыИДействияМедосмотров230();
		Медосмотры_РазделениеДействийКишечныеИнфекцииИБрюшнойТифНаДва();
		Медосмотры_ОбновитьМакетДляПриказа29н();
		
		Медосмотры_СПМОДополнитьРеквизитыЧисленности();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.27", "2125_27", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , ); // 2.0.49.40
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.28") Тогда // 2.0.49.42
		Медосмотры_ИсправлениеФактора26Приказа29н_20_49_41();

		РольКонтактнаяИнформацияПросмотр_ПростановкаУтратившимПраво(); // Роль КонтактнаяИнформацияПросмотр проставится ролям, которым в релизе забрано это право.
		ОбменФСС.ОбновитьСертификатыКриптографииФСС();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.28", "2125_28", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина); // 2.0.49.42
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.29") Тогда // 2.0.49.44
		ОчиститьВерсиюКлассификатора("1.2.643.5.1.13.13.11.1078");
	КонецЕсли;
	СтандартноеОбновление("2.1.25.29", "2125_29", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , Истина); // 2.0.49.44
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.30") Тогда // 2.0.49.45
		ДвоичныеДанныеВыгрузки = Справочники.РегистрируемыеЭлектронныеМедДокументы.ПолучитьМакет("НаименованияФиксированныхВерсий");
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки, Истина, Истина);
		
		Попытка
			DICOMОбщий.Обновление_ПереходС_DCMTK_ПереносОборудованияВМедОборудование();
		Исключение КонецПопытки;
		
		ОчиститьВерсиюКлассификатора("1.2.643.5.1.13.13.11.1497"); // Причины отказа в госпитализации.
		
		стомОбновлениеИБ.ЗаполнитьСтандартныеЗубныеКарты();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.30", "2125_30", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);  // 2.0.49.45
	СтандартноеОбновление("2.1.25.31", "2125_31", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );  // 2.0.49.46
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.32") Тогда // 2.0.49.48
		ПеренестиМашиночитаемыеДоверенностиВСправочникИзРегистра();
		ОчиститьРегистрАдресныхОбъектов();
		
		КОРП_Стационар_ПервоначальноеЗаполнениеНоменклатура_ИсполнительВСтационаре();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.32", "2125_32", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);  // 2.0.49.48
	СтандартноеОбновление("2.1.25.33", "2125_33", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , Истина);  // 2.0.49.49
	СтандартноеОбновление("2.1.25.34", "2125_34", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , );  // 2.0.49.50
	СтандартноеОбновление("2.1.25.35", "", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , , );  // 2.0.49.50 + add 18-20.09.2024
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.36") Тогда // 2.0.49.51
		ОбновлениеИнформационнойБазыБПО.ОбновитьПодключаемоеОборудование();
		...
		Справочники.ВнешниеКомпоненты.ОбработатьДанныеДляПереходаНаНовуюВерсию(Неопределено);
		
		ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Справочники.РазделыМедицинскихКарт.ПолучитьМакет("ОбновлениеКОРП25_36"));
	КонецЕсли;
	СтандартноеОбновление("2.1.25.36", "2125_36", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, , Истина, Истина); // 2.0.49.51
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.37") Тогда // Стом.2.0.25.16
		// стомОбновлениеИБ.СоздатьВидМедкартыОртодонта_043_1У_СПараметрами();
		стомОбновлениеИБ.Стоматология_ОбновитьСегментыЗубныхКарт_29_1();
	КонецЕсли;
	СтандартноеОбновление("2.1.25.37", "2125_37", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , );
	...
	Если РелизБолееРанний(ТекущаяВерсияИБ, "2.1.25.38") Тогда // 2.0.49.55
		ДобавитьОтщепленнуюРольПользователямПрофилям(Метаданные.Роли.РасчетчикЗарплаты, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Метаданные.Роли.Бухгалтер));
	КонецЕсли;
	СтандартноеОбновление("2.1.25.38", "2125_38", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , );  // 2.0.49.55
	
	Если РелизБолееРанний(ТекущаяВерсияИБ, "3.0.0.21") Тогда
		УдалениеНеактуальныхРегламентныхЗаданий();
	КонецЕсли;
	СтандартноеОбновление("3.0.0.21", "300_21", ТекущаяВерсияИБ, ВыполненныеОбновления, ДокументОписаниеОбновлений, МакетОписаниеОбновлений, Истина, , );  // 2.0.49.55
	
	Если ВыполненныеОбновления.Свойство("НоваяВерсияИБ") Тогда // Обновление было.
		...
		Результат.Вставить("ТекущаяВерсияИБ", ТекущаяВерсияИБ);
		Результат.Вставить("НоваяВерсияИБ", НоваяВерсияИБ);
		Возврат Результат;
	КонецЕсли;
	
	// Добавлено Макеев (
	НоваяВерсияИБ ="3.0.0.63";
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               |	омг_ПриложенияКЛицензии.Ссылка КАК Ссылка,
		               |	омг_ПриложенияКЛицензии.Код КАК Код
		               |ИЗ
		               |	Справочник.омг_ПриложенияКЛицензии КАК омг_ПриложенияКЛицензии
		               |ГДЕ
		               |	омг_ПриложенияКЛицензии.НомерПриложения = """"
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Код";
		...
		Пока Выборка.Следующий() Цикл
			...
			СправочникОбъект.НомерПриложения = СправочникОбъект.Код;
			СправочникОбъект.Код = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(Код, 9, "0");
...
Процедура УдалениеНеактуальныхРегламентныхЗаданий(Параметры = Неопределено)
	
	РеглЗадания = РегламентныеЗадания.ПолучитьРегламентныеЗадания();
	Для Каждого РеглЗадание Из РеглЗадания Цикл
		Если РеглЗадание.Ключ = "омг_ОбновлениеАнализовЛИС" Тогда
			РеглЗадание.Удалить();
		КонецЕсли;
		Если РеглЗадание.Ключ = "омг_ОбновлениеБиоматериаловЛИС" Тогда
			РеглЗадание.Удалить();
		КонецЕсли;
		Если РеглЗадание.Ключ = "омг_ОбновлениеЦенЛИС" Тогда
			РеглЗадание.Удалить();
		КонецЕсли;
	КонецЦикла;
...
Процедура СтандартноеОбновление(НоваяВерсияИБ, ОписаниеОбновлений = "", ТекущаяВерсияИБ, 
	...
	Если РелизБолееРанний(ТекущаяВерсияИБ,НоваяВерсияИБ) Тогда
		...
		Если ОбновитьИдентификаторыМетаданных Тогда
			ОбновитьИдентификаторыМетаданных(ВыполненныеОбновления);
		КонецЕсли;
		
		СтандартноеОбновление_Медицина(ВыполненныеОбновления);
		
		ЗавершитьТранзакциюОбновлиенияИБ(НоваяВерсияИБ, ВыполненныеОбновления); // Завершение транзакции.
...
Процедура ОбновитьИдентификаторыМетаданных(ВыполненныеОбновления)
	
	Если  Не ВыполненныеОбновления.ИдентификаторыМетаданных
...
Процедура СтандартноеОбновление_Медицина(ВыполненныеОбновления)
	
	Если Не ВыполненныеОбновления.Свойство("КлассификаторыЕГИСЗ")
		Или ВыполненныеОбновления.КлассификаторыЕГИСЗ <> Истина
	Тогда
		ОбновитьНастройкиОбновленияКлассификаторовЕГИСЗ();
		ВыполненныеОбновления.Вставить("КлассификаторыЕГИСЗ", Истина);
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбновитьНастройкиОбновленияКлассификаторовЕГИСЗ()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	КОЛИЧЕСТВО(ЕГИСЗНастройкиИнтеграции.МедицинскаяОрганизация) КАК Количество
		|ИЗ
		|	РегистрСведений.ЕГИСЗНастройкиИнтеграции КАК ЕГИСЗНастройкиИнтеграции
		|ГДЕ
		|	ЕГИСЗНастройкиИнтеграции.ИспользоватьРЭМД";
	
	Если Запрос.Выполнить().Выгрузить()[0].Количество = 0 Тогда
		// РЭМД не используется
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВЫРАЗИТЬ(СписокСправочников.oid КАК СТРОКА(50)) КАК oid
		|ПОМЕСТИТЬ СписокСправочников
		|ИЗ
		|	&СписокСправочников КАК СписокСправочников
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СписокСправочников.oid КАК oid
		|ИЗ
		|	СписокСправочников КАК СписокСправочников
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиОбновленияСправочниковЕГИСЗ КАК НастройкиОбновленияСправочниковЕГИСЗ
		|		ПО СписокСправочников.oid = НастройкиОбновленияСправочниковЕГИСЗ.Классификатор
		|ГДЕ
		|	НастройкиОбновленияСправочниковЕГИСЗ.Классификатор ЕСТЬ NULL";
	
	Запрос.УстановитьПараметр("СписокСправочников", СписокСправочников);
	
	OIDыНовыхКлассификаторов = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("oid");
	
	Если OIDыНовыхКлассификаторов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	СписокНовыхСправочников = СписокСправочников.СкопироватьКолонки();
	Для Каждого OID Из OIDыНовыхКлассификаторов Цикл
		СтрокаТЗ = СписокСправочников.Найти(СокрЛП(OID), "oid");
		Если СтрокаТЗ <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(СписокНовыхСправочников.Добавить(), СтрокаТЗ);
		КонецЕсли;
	КонецЦикла;
	
	Если СписокНовыхСправочников.Количество() > 0 Тогда
		РегистрыСведений.НастройкиОбновленияСправочниковЕГИСЗ.ПервоначальноеЗаполнение(1, СписокНовыхСправочников);
	КонецЕсли;
...
Процедура ЗавершитьТранзакциюОбновлиенияИБ(НоваяВерсияИБ, ВыполненныеОбновления = Неопределено)
	...
	Если ТипЗнч(ВыполненныеОбновления) = Тип("Структура") Тогда
		ВыполненныеОбновления.Вставить("НоваяВерсияИБ", НоваяВерсияИБ);
	КонецЕсли;
...
Процедура ВыполнитьДействияПриПервомЗапускеМедицина()
	...
	МедосмотрыПриказыМО_ПервоначальноеЗаполнение();
	...
	#Область Стоматология
	ДействияПриПервомЗапускеСтоматология();
	#КонецОбласти
	
	#Область КОРП
	ДействияПриПервомЗапускеКОРП();
	#КонецОбласти
...
Процедура ОбновитьНасройкиБитФон2_0_21_5()
	...
	УстановитьПривилегированныйРежим(Истина);
	НаборЗаписей = РегистрыСведений.доп_БитфонНастройки.СоздатьНаборЗаписей();
	НаборЗаписей.Прочитать();
...
Процедура ДействияПриПервомЗапускеСтоматология()
	
	стомОбновлениеИБ.ЗаполнитьСтандартныеЗубныеКарты();
	стомОбновлениеИБ.ЗаполнитьВычисляемыеБлокиЗубнойКарты();
	стомОбновлениеИБ.ЗаполнитьСостоянияПараметрыЗубов();
	
	ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Справочники.ВидыМедицинскихКарт.ПолучитьМакет("стомПараметрыМедкарты043У"), Истина);
	
	стомОбновлениеИБ.ПеренестиОбозначенияЗубовВСписокВариантов();
	
	// стомОбновлениеИБ.СоздатьВидМедкартыОртодонта_043_1У_СПараметрами(); // 043-1/у.
	
КонецПроцедуры

#КонецОбласти

Процедура ДействияПриПервомЗапускеКОРП()
	...
	ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Справочники.КаналыГоспитализации.ПолучитьМакет("ПервоначальноеЗаполнениеПутиПоступленияПациентаВСтационар"), Истина);
	
	// Заполнение реквизитов действий медсестры стационара.
	
	Выб = Справочники.ДействияМедсестрыСтационара.Выбрать();
	Пока Выб.Следующий() Цикл
		
		Если Выб.Ссылка <> Справочники.ДействияМедсестрыСтационара.Ванна Тогда
			Объект = Выб.ПолучитьОбъект();
			Объект.ВыполнениеВводомЗначения = Истина;
			
			Если Выб.Ссылка = Справочники.ДействияМедсестрыСтационара.АртериальноеДавление Тогда
				Объект.БазовоеЗначениеДиаграммы = 40;
			ИначеЕсли Выб.Ссылка = Справочники.ДействияМедсестрыСтационара.Вес Тогда
				Объект.БазовоеЗначениеДиаграммы = 40;
			ИначеЕсли Выб.Ссылка = Справочники.ДействияМедсестрыСтационара.Температура Тогда
				Объект.БазовоеЗначениеДиаграммы = 36;
			КонецЕсли;
			Объект.Записать();
		КонецЕсли;
	КонецЦикла;
	
	// Заполнение предопределенных показателей планирования
	ДвоичныеДанныеВыгрузки = Справочники.ПоказателиПланирования.ПолучитьМакет("ПервоначальноеЗаполнение");
	ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки);
	
	КОРП_ЗагрузитьКлассификаторыДляВыписногоЭпикриза();
	
	УстановитьПривилегированныйРежим(Ложь);
	
КонецПроцедуры

// Возвращает релиз, который нужно установить при переходе с редакци 2.0 на 2.1.
Функция ВыполнитьОбновлениеС20(ТекущаяВерсияИБ)
	
	ОбновитьПравилаОбменаПланаОбменаСФилиалами();
	
	ДействияПриПервомЗапускеСтоматология();
	ДействияПриПервомЗапускеКОРП();
	
	НачатьТранзакцию();
	ЗавершитьТранзакциюОбновлиенияИБ(Метаданные.Версия);
...
Процедура ЗаполнитьСостояниеЧековККМОказанийУслуг()
	...
	ПредельноДопустимоеВремяВыполнения = 750; // Секунд.
...
Процедура ПерезаполнитьИсториюЗвонков()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	доп_ИсторияЗвонков.Пользователь,
	...
	НачатьТранзакцию();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ЗаписьРС = РегистрыСведений.доп_ИсторияЗвонков.СоздатьМенеджерЗаписи();
...
Процедура ПервоначальноеОбновлениеЕГИСЗ()
	...
	// Наименования фиксированных версий классификаторов.
	ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Справочники.РегистрируемыеЭлектронныеМедДокументы.ПолучитьМакет("НаименованияФиксированныхВерсий"), Истина, Истина);
	...
	Если РежимПечатиПоШаблонуWORD <> Перечисления.РежимыПечатиПоШаблонуWORD.ФормироватьНаКлиентеВOpenOffice Тогда
		...
		Если ПроблемныеШаблоны.Количество() <> 0 Тогда
			...
			Для Каждого ИмяШаблона Из ПроблемныеШаблоны Цикл
				ОбщегоНазначенияБИТКлиентСервер.КонкатенацияСтрок(ТекстСообщения, ИмяШаблона, Символы.ПС + " - ");
			КонецЦикла;
...
Процедура ЗаменитьРолиПодписейНаЭлементыКлассификатора()
	...
	Пока НеВсеСтрокиИзменены Цикл
		...
		// Врачебная комиссия
		ЭлементыКлассификаторов.Вставить(Перечисления.РолиПодписей.ПредседательВрачебнойКомиссии,	ИнтеграцияЕГИСЗСервер.ЭлементКлассификатораПоID(14,	"РолиПриПодписиМедДокументов"));
		ЭлементыКлассификаторов.Вставить(Перечисления.РолиПодписей.ЧленВрачебнойКомиссии,			ИнтеграцияЕГИСЗСервер.ЭлементКлассификатораПоID(16,	"РолиПриПодписиМедДокументов"));
...
Процедура ЗаполнениеНовогоРеквизитаОрганизацииВДоговорах()
	...
	Если Выборка.Количество() = 1 Тогда
		...
		Возврат; // Проставили организацию во все договоры, дальнейшая отработка частных случаев не требуется.
	КонецЕсли;
...
Процедура ОрганизацииПерезаполнениеВидовПолисов()
	
	Выб = Справочники.ВидыПолисов.Выбрать();
	Пока Выб.Следующий() Цикл
		Если Не Выб.ЭтоГруппа
			И ЗначениеЗаполнено(Выб.СоглашениеСтрахования)
			И Выб.Договоры.Количество() = 0
			И ЗначениеЗаполнено(Выб.СоглашениеСтрахования.Организация)
		Тогда
			ВидПолисаОбъект = Выб.ПолучитьОбъект();
			ВидПолисаОбъект.Договоры.Добавить().Договор = ВидПолисаОбъект.СоглашениеСтрахования;
			ВидПолисаОбъект.Записать();
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьПредопределенныеВидыМедосмотров()
	...
	НовыйПриказ.ИспользоватьБезВФ = Истина;
...
Функция МедосмотрыПриказыМО_ПервоначальноеЗаполнение()
...
Процедура ЗаполнитьИспользуемуюВерсиюСервераЛицензированияПриОбновлении()
	...
	Если ПустаяСтрока(Константы.ИспользуемаяВерсияСистемыЛицензирования.Получить()) Тогда
		Константы.ИспользуемаяВерсияСистемыЛицензирования.ЗаполнитьИспользуемуюВерсиюСервераЛицензирования();
	КонецЕсли;
...
Процедура ОбновитьСтандартныеАлгоритмыШаблоновWord()

	Выборка = Справочники.ШаблоныПечати.Выбрать();
	
	ПрефиксСтандартныхАлгоритмов = РаботаСШаблонамиПечатиКлиентСервер.ПрефиксСтандартныхАлгоритмов();
	Пока Выборка.Следующий() Цикл 
		
		Объект = Выборка.Ссылка.ПолучитьОбъект();
		ЗаписатьОбъект = Ложь; 
		Для Каждого СтрокаПараметра Из Объект.ПараметрыПечати Цикл 
			Если Не СтрокаПараметра.СтандартныйАлгоритм Тогда
				СтандартныйПараметр = Справочники.ШаблоныПечати.ПолучитьПараметрПоСтандартномуАлгоритму(СтрокаПараметра.Значение); 
				Если Не ЗначениеЗаполнено(СтандартныйПараметр) Тогда
					СтандартныйПараметр = ПолучитьПараметрПоСтандартномуАлгоритмуПриОбновлении(СтрокаПараметра.Значение);
				КонецЕсли;
				Если ЗначениеЗаполнено(СтандартныйПараметр) Тогда
					СтрокаПараметра.Значение = ПрефиксСтандартныхАлгоритмов + СтандартныйПараметр;
					СтрокаПараметра.СтандартныйАлгоритм = Истина;
...
Функция ПолучитьПараметрПоСтандартномуАлгоритмуПриОбновлении(Алгоритм)
	
	СокрАлгоритм = СтрЗаменить(СтрЗаменить(СокрЛП(Алгоритм), Символы.ПС, ""), Символы.ВК, "");
	
	ТабДокОбразцы = Справочники.ШаблоныПечати.ПолучитьМакет("ОбразцыПараметровШаблонаПечати_ДляОбновления20_49_33");
...
Процедура ОбновитьСтандартныеАлгоритмыВычисляемыхЗначенийПараметров()
	
	Выборка = Справочники.ВычисляемыеЗначенияПараметров.Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		Если Не Выборка.ЭтоГруппа Тогда
			Ключ =  Справочники.ВычисляемыеЗначенияПараметров.ПолучитьКлючСтандартногоАлгоритмаПоТекстуАлгоритма(Выборка.АлгоритмВычисления, Истина, Выборка.ПолучательВычисляемогоЗначения);
			Если Ключ <> Неопределено Тогда
				Объект = Выборка.Ссылка.ПолучитьОбъект();
				Объект.АлгоритмВычисления = Ключ;
				Объект.Записать();
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбновитьБлокЗП_20_49_33()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ПорядокРасчета = 0 Тогда
			
			ОбъектБД = Выборка.ПолучитьОбъект();
			
			ЭтоВторичное = Ложь;
			Если Выборка.ФормулаРасчета = Перечисления.ФормулыРасчетаЗаработнойПлаты.МинимальныйОклад Тогда
				ЭтоВторичное = Истина;
			Иначе
				Для Сч = 1 По 3 Цикл
					Если Выборка["ВидПараметра" + Сч] = Перечисления.ВидыПараметровРасчетаНачислений.ВидНачисления Тогда
						ЭтоВторичное = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			...
			ОбъектБД.ПорядокРасчета = ?(ЭтоВторичное, 2, 1);
			Если ЭтоВторичное Тогда
				ОбъектБД.ИсточникБазы = Перечисления.ИсточникБазыВторичногоНачисленияПоСтруктуре.ВсеДанные;
			КонецЕсли;
	...
	// Документы начисления з/п: простановка флаго 3-го и 4-го порядка для включающих вторичные.
	Выборка = Документы.НачислениеЗарплаты.Выбрать(ДобавитьМесяц(ТекущаяДата(), -6));
	Пока Выборка.Следующий() Цикл
		Если Выборка.ВключатьВторичныеНачисления
			И Не Выборка.ВключатьНачисленияТретьегоПорядка
		Тогда
			ОбъектБД = Выборка.ПолучитьОбъект();
			ОбъектБД.ВключатьНачисленияТретьегоПорядка = Истина;
			ОбъектБД.ВключатьНачисленияЧетвертогоПорядка = Истина;
			ОбъектБД.ДополнительныеСвойства.Вставить("НеДелатьПроверокПриЗаписи", Истина);
...
Процедура УстановитьВидМаркированнойПродукцииДляМДЛП()
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПустойВидМаркированнойПродукции", Перечисления.ВидыМаркированнойПродукции.ПустаяСсылка());
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ГДЕ
		|	УведомлениеОПриемкеМДЛПТовары.Номенклатура.ВидМаркированнойПродукции = &ПустойВидМаркированнойПродукции
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	УведомлениеОПриемкеМДЛПСоставТранспортныхУпаковок.Номенклатура
		|ИЗ
		|	Документ.УведомлениеОПриемкеМДЛП.СоставТранспортныхУпаковок КАК УведомлениеОПриемкеМДЛПСоставТранспортныхУпаковок
		|ГДЕ
		|	УведомлениеОПриемкеМДЛПСоставТранспортныхУпаковок.Номенклатура.ВидМаркированнойПродукции = &ПустойВидМаркированнойПродукции
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	Справочник.Номенклатура КАК Номенклатура
		|ГДЕ
		|	Номенклатура.Упаковка <> ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмерения.ПустаяСсылка)
		|	И Номенклатура.ВидМаркированнойПродукции = &ПустойВидМаркированнойПродукции";
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.Номенклатура <> Справочники.Номенклатура.ПустаяСсылка() Тогда
			...
			НоменклатураОбъект.ВидМаркированнойПродукции = Перечисления.ВидыМаркированнойПродукции.МДЛП;
			НоменклатураОбъект.ОбменДанными.Загрузка = Истина;
			Попытка
				НоменклатураОбъект.Записать();
			Исключение
				Шаблон = "Для номенклатуры %1 не удалось установить вид маркировки, необходимо проверить вручную!";
				ОбщегоНазначения.СообщитьПользователю(СтрШаблон(Шаблон, Строка(Выборка.Номенклатура)));
			КонецПопытки;
		КонецЕсли;
	КонецЦикла;
...
Процедура Медосмотры_ЗагрузитьПриказ1252нИПриказыИДействияМедосмотров230()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Количество(ПереченьВредныхФакторовИРабот.Ссылка) КАК КоличествоСсылка
		|ИЗ
		|	Справочник.ПереченьВредныхФакторовИРабот КАК ПереченьВредныхФакторовИРабот
		|ГДЕ
		|	НЕ ПереченьВредныхФакторовИРабот.ПометкаУдаления";
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.КоличествоСсылка > 3 Тогда
			// 1252н
			ДвоичныеДанныеВыгрузки = Справочники.ПереченьВредныхФакторовИРабот.ПолучитьМакет("Обновление20_49_38");
			ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки, Истина);
			
			// Действия медосмотров 230
			ДвоичныеДанныеВыгрузки = Справочники.ПереченьВредныхФакторовИРабот.ПолучитьМакет("ОбновлениеДля230");
			ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки, Истина);
		КонецЕсли;
	КонецЦикла;
...
Процедура Медосмотры_РазделениеДействийКишечныеИнфекцииИБрюшнойТифНаДва()
	
	Если ИнтеграцияЕГИСЗСерверПовтИсп.ИспользуетсяИнтеграцияРЭМД() Тогда
		
		СтароеДействиеДляЗамены = Справочники.ДействияМедосмотра.НайтиПоНаименованию("Исследования на носительство возбудителей кишечных инфекций и серологическое обследование на брюшной тиф", Истина);
		
		НовоеДействиеКишечныеИнфекции = Справочники.ДействияМедосмотра.НайтиПоНаименованию("Исследования на носительство возбудителей кишечных инфекций", Истина);
		НовоеДействиеБрюшнойТиф = Справочники.ДействияМедосмотра.НайтиПоНаименованию("Серологическое обследование на брюшной тиф", Истина);
		
		Если Не ЗначениеЗаполнено(СтароеДействиеДляЗамены) 
			Или Не ЗначениеЗаполнено(НовоеДействиеКишечныеИнфекции) 
			Или Не ЗначениеЗаполнено(НовоеДействиеБрюшнойТиф) 
		Тогда
			Возврат;
		КонецЕсли;
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	Документ.ПрохождениеМедосмотра КАК ПрохождениеМедосмотра
		               |ГДЕ
		               |	ПрохождениеМедосмотра.Дата МЕЖДУ &ДатаОтсечки И &ТекущаяДата
		               |	И НЕ ПрохождениеМедосмотра.ПодписанЭП
		               |	И НЕ ПрохождениеМедосмотра.ПометкаУдаления";
		Запрос.УстановитьПараметр("ТекущаяДата", КонецДня(ТекущаяДата()));
		Запрос.УстановитьПараметр("ДатаОтсечки", ДобавитьМесяц(НачалоДня(ТекущаяДата()), -2));
		
		Результат = Запрос.Выполнить().Выбрать();
		
		Пока Результат.Следующий() Цикл
			
			ПМО = Результат.Ссылка;
			ПМООбъект = ПМО.ПолучитьОбъект();
			// Это справка. Нет вредных факторов
			Если ЗначениеЗаполнено(ПМО.Номенклатура.ВредныйФакторСправки.ШаблонОсмотраСправки) Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокиСтарогоДействия = ПМООбъект.ДействияМедосмотра.НайтиСтроки(Новый Структура("Действие", СтароеДействиеДляЗамены));
			Для Каждого СтрокаСтарогоДействия Из СтрокиСтарогоДействия Цикл
				НоваяСтрока = ПМООбъект.ДействияМедосмотра.Добавить();
				НоваяСтрока.Действие = НовоеДействиеКишечныеИнфекции;
				НоваяСтрока.НазначенДополнительно = СтрокаСтарогоДействия.НазначенДополнительно;
				
				НоваяСтрока = ПМООбъект.ДействияМедосмотра.Добавить();
				НоваяСтрока.Действие = НовоеДействиеБрюшнойТиф;
				НоваяСтрока.НазначенДополнительно = СтрокаСтарогоДействия.НазначенДополнительно;
				
				// Удаляем старую строку из ТЧ
				ПМООбъект.ДействияМедосмотра.Удалить(СтрокаСтарогоДействия);
				
				Если ПМООбъект.Проведен Тогда
					ПМООбъект.Записать(РежимЗаписиДокумента.Проведение);
				Иначе
					ПМООбъект.Записать(РежимЗаписиДокумента.Запись);
				КонецЕсли;
				
				// Находим старую строку в регистре ОтметкиВыполненияДействий
				НаборСтарый = РегистрыСведений.ОтметкиВыполненияДействий.СоздатьНаборЗаписей();
				НаборСтарый.Отбор.Действие.Установить(СтароеДействиеДляЗамены);
				НаборСтарый.Отбор.ПрохождениеМедОсмотра.Установить(ПМО);
				НаборСтарый.Прочитать();
				
				Для Каждого ЗаписьСтарая Из НаборСтарый Цикл
					
					// В регистр отметок вносим два новых действия с теми же отметками
					
					МЗ = РегистрыСведений.ОтметкиВыполненияДействий.СоздатьМенеджерЗаписи();
					МЗ.Действие = НовоеДействиеКишечныеИнфекции;
					МЗ.ПрохождениеМедОсмотра = ЗаписьСтарая.ПрохождениеМедОсмотра;
					МЗ.Период = ЗаписьСтарая.Период;
					МЗ.Выполнено = ЗаписьСтарая.Выполнено;
					МЗ.ДокументРегистратор = ЗаписьСтарая.ДокументРегистратор;
					МЗ.АктивностьЗаписи = ЗаписьСтарая.АктивностьЗаписи;
					МЗ.Записать();
					
					МЗ = РегистрыСведений.ОтметкиВыполненияДействий.СоздатьМенеджерЗаписи();
					МЗ.Действие = НовоеДействиеБрюшнойТиф;
					МЗ.ПрохождениеМедОсмотра = ЗаписьСтарая.ПрохождениеМедОсмотра;
					МЗ.Период = ЗаписьСтарая.Период;
					МЗ.Выполнено = ЗаписьСтарая.Выполнено;
					МЗ.ДокументРегистратор = ЗаписьСтарая.ДокументРегистратор;
					...
					// Удаляем найденную строку из регистра
					НаборСтарый.Удалить(ЗаписьСтарая);
					
				КонецЦикла;
				
				НаборСтарый.Записать();
				
				// Находим старую строку в регистре ЗаключенияПоДействиямМедосмотра
				НаборСтарый = РегистрыСведений.ЗаключенияПоДействиямМедосмотра.СоздатьНаборЗаписей();
				НаборСтарый.Отбор.Действие.Установить(СтароеДействиеДляЗамены);
				НаборСтарый.Отбор.ПрохождениеМедОсмотра.Установить(ПМО);
				НаборСтарый.Прочитать();
				...
				Для Каждого ЗаписьСтарая Из НаборСтарый Цикл
					...
					МенеджерЗаписи.Действие = НовоеДействиеКишечныеИнфекции;
					МенеджерЗаписи.ПрохождениеМедОсмотра = ЗаписьСтарая.ПрохождениеМедОсмотра;
					МенеджерЗаписи.Заключение = ЗаписьСтарая.Заключение;
					МенеджерЗаписи.ДатаВыполнения = ЗаписьСтарая.ДатаВыполнения;
					МенеджерЗаписи.Сотрудник = ЗаписьСтарая.Сотрудник;
					МенеджерЗаписи.ВыявленыПротивопоказания = ЗаписьСтарая.ВыявленыПротивопоказания;
					МенеджерЗаписи.ТитрСерологическогоОбследования = ЗаписьСтарая.ТитрСерологическогоОбследования;
					МенеджерЗаписи.Записать();
					
					МенеджерЗаписи = РегистрыСведений.ЗаключенияПоДействиямМедосмотра.СоздатьМенеджерЗаписи();
					МенеджерЗаписи.Действие = НовоеДействиеБрюшнойТиф;
					МенеджерЗаписи.ПрохождениеМедОсмотра = ЗаписьСтарая.ПрохождениеМедОсмотра;
					МенеджерЗаписи.Заключение = ЗаписьСтарая.Заключение;
					МенеджерЗаписи.ДатаВыполнения = ЗаписьСтарая.ДатаВыполнения;
					МенеджерЗаписи.Сотрудник = ЗаписьСтарая.Сотрудник;
...
Процедура Медосмотры_ЗагрузитьКлассификаторВидовПолисовОМС()
	
	УстановитьПривилегированныйРежим(Истина);
	ДвоичныеДанныеВыгрузки = Справочники.КлассификаторыМинЗдрава.ПолучитьМакет("ВидыПолисаОМС");
	ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки, Истина);
	
КонецПроцедуры

Процедура Медосмотры_ОбновитьМакетДляПриказа29н()
	
	Если Не ЗначениеЗаполнено(Справочники.ПриказыМедосмотров.Приказ29н.МакетДляПодписанияЭЦП) Тогда
		ОбъектПриказ29н = Справочники.ПриказыМедосмотров.Приказ29н.ПолучитьОбъект();
		ОбъектПриказ29н.МакетДляПодписанияЭЦП = "Заключение";
		ОбъектПриказ29н.Записать();
	КонецЕсли;
	
КонецПроцедуры

Процедура Медосмотры_СПМОДополнитьРеквизитыЧисленности()
	...
	|	Документ.СписокНаПрохождениеМедосмотра КАК СписокНаПрохождениеМедосмотра
	|ГДЕ
	|	НЕ СписокНаПрохождениеМедосмотра.РучнаяКорректировка
	|	И СписокНаПрохождениеМедосмотра.РаботникиНаТяжРаботахВсего <> 0
	|	И СписокНаПрохождениеМедосмотра.ЧисленностьРаботниковПодлежМедосмотруВсего = 0"
	;
	Выб = Запрос.Выполнить().Выбрать();
	...
	Пока Выб.Следующий() Цикл
		...
		ДокОбъект.ЧисленностьРаботниковОбязМедосмотрВсего	 = ДокОбъект.РаботникиНаТяжРаботахВсего;
		ДокОбъект.ЧисленностьРаботниковОбязМедосмотрЖенщин	 = ДокОбъект.РаботникиНаТяжРаботахЖенщин;
		ДокОбъект.ЧисленностьРаботниковОбязМедосмотрДо18Лет	 = ДокОбъект.РаботникиНаТяжРаботахДо18Лет;
		ДокОбъект.ЧисленностьРаботниковОбязМедосмотрУстУтратаНетрудоспособности = ДокОбъект.РаботникиНаТяжРаботахУстУтратаНетрудоспособности;
		
		ДокОбъект.ЧисленностьРаботниковПодлежМедосмотруВсего	 = ДокОбъект.РаботникиНаТяжРаботахВсего;
		ДокОбъект.ЧисленностьРаботниковПодлежМедосмотруЖенщин	 = ДокОбъект.РаботникиНаТяжРаботахЖенщин;
		ДокОбъект.ЧисленностьРаботниковПодлежМедосмотруДо18Лет	 = ДокОбъект.РаботникиНаТяжРаботахДо18Лет;
		ДокОбъект.ЧисленностьРаботниковПодлежМедосмотруУстУтратаНетрудоспособности = ДокОбъект.РаботникиНаТяжРаботахУстУтратаНетрудоспособности;
...
Процедура Медосмотры_ИсправлениеФактора26Приказа29н_20_49_41()
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.КоличествоСсылка > 3 Тогда
			ДвоичныеДанныеВыгрузки = Справочники.ПереченьВредныхФакторовИРабот.ПолучитьМакет("Обновление20_49_41");
			ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(ДвоичныеДанныеВыгрузки, Истина);
		КонецЕсли;
	КонецЦикла;
...
Процедура РольКонтактнаяИнформацияПросмотр_ПростановкаУтратившимПраво()
	
	РольПросмотрКИ = Метаданные.Роли.КонтактнаяИнформацияПросмотр;
	
	// Роли, потерявшие право просмотра
	Роли = Новый Массив;
	Роли.Добавить(Метаданные.Роли.Администратор);
	Роли.Добавить(Метаданные.Роли.УправляющийФилиалом);
	Роли.Добавить(Метаданные.Роли.Кладовщик);
	Роли.Добавить(Метаданные.Роли.СоставлениеГрафиковРаботы);
	Роли.Добавить(Метаданные.Роли.ЛабораторныеИсследования);
	Роли.Добавить(Метаданные.Роли.Врач);    
	Роли.Добавить(Метаданные.Роли.Бухгалтер);
	Роли.Добавить(Метаданные.Роли.МедицинскаяСтатистика);
	Роли.Добавить(Метаданные.Роли.ОператорCallЦентра);
	Роли.Добавить(Метаданные.Роли.доп_ПользовательБИТфон);
	Роли.Добавить(Метаданные.Роли.ИсполнительЗаявокНаТехОбслуживание); 
	Роли.Добавить(Метаданные.Роли.ОтработкаОбращений);
	
	ДобавитьОтщепленнуюРольПользователямПрофилям(РольПросмотрКИ, Роли);
	
КонецПроцедуры

Процедура ДобавитьОтщепленнуюРольПользователямПрофилям(НоваяРоль, РолиПолучателей)

	// Пользователи
	Для Каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл
		Для Каждого РольПользователя Из ПользовательИБ.Роли Цикл
			Если РолиПолучателей.Найти(РольПользователя) <> Неопределено Тогда
	...
	Пока Выборка.Следующий() Цикл
		Если Не Выборка.ЭтоГруппа Тогда
			Для Каждого Роль Из РолиПолучателей Цикл
				Если Выборка.Роли.Найти(Роль.Имя) <> Неопределено Тогда
					ПрофильОбъект = Выборка.ПолучитьОбъект();
					ПрофильОбъект.Роли.Добавить().Роль = НоваяРоль.Имя;
...
Процедура ПеренестиМашиночитаемыеДоверенностиВСправочникИзРегистра()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	МашиночитаемыеДоверенности.Пользователь КАК Пользователь,
		|	МашиночитаемыеДоверенности.УИД КАК УИД,
		|	МашиночитаемыеДоверенности.Организация КАК Организация,
		|	МашиночитаемыеДоверенности.ДатаНачала КАК ДатаНачала,
		|	МашиночитаемыеДоверенности.ДатаКонца КАК ДатаКонца
		|ИЗ
		|	РегистрСведений.удалитьМашиночитаемыеДоверенности КАК МашиночитаемыеДоверенности
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.МашиночитаемыеДоверенности КАК МашиночитаемыеДоверенностиСправочник
		|		ПО (МашиночитаемыеДоверенностиСправочник.Пользователь = МашиночитаемыеДоверенности.Пользователь)
		|			И (МашиночитаемыеДоверенностиСправочник.УИД = МашиночитаемыеДоверенности.УИД)
		|			И (МашиночитаемыеДоверенностиСправочник.Организация = МашиночитаемыеДоверенности.Организация)";
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		НовыйЭлементМЧД = Справочники.МашиночитаемыеДоверенности.СоздатьЭлемент();
		НовыйЭлементМЧД.Пользователь	= Выборка.Пользователь;
		НовыйЭлементМЧД.УИД				= Выборка.УИД;
		НовыйЭлементМЧД.Организация		= Выборка.Организация;
		НовыйЭлементМЧД.ДатаВыдачи		= Выборка.ДатаНачала;
		НовыйЭлементМЧД.ДатаОкончания	= Выборка.ДатаКонца;
		СтрокаПолномочия = НовыйЭлементМЧД.Полномочия.Добавить();
		СтрокаПолномочия.Полномочие = Перечисления.ПолномочияЭЦП.ФСС;
		НовыйЭлементМЧД.Записать();
	КонецЦикла;
	
КонецПроцедуры

Процедура ОчиститьРегистрАдресныхОбъектов()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Количество(АдресныеОбъекты.Идентификатор) КАК КоличествоИдентификатор
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.КоличествоИдентификатор > 0 Тогда
...
#Область Стоматология
Процедура СнятьПометкуУдаленияРанееПредопределенныхСостоянийЗубов()
	
	// Предполагаем, что пользователь мог добавлять и помечать на удаление свои состояния.
	УстановитьПривилегированныйРежим(Истина);
	
	МассивНаименований = Новый Массив;
	МассивНаименований.Добавить("Кариес");
	МассивНаименований.Добавить("Пародонтоз");
	МассивНаименований.Добавить("Периодонтид");
	МассивНаименований.Добавить("Пломбированный");
	МассивНаименований.Добавить("Пульпит");
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	стомСостоянияЗубов.Ссылка КАК Ссылка,
	|	стомСостоянияЗубов.ПометкаУдаления КАК ПометкаУдаления
	|ИЗ
	|	Справочник.стомСостоянияЗубов КАК стомСостоянияЗубов
	|ГДЕ
	|	стомСостоянияЗубов.Наименование = &Наименование"
	;
	
	Для Каждого НаименованиеРанееПредопределенного Из МассивНаименований Цикл
		
		Запрос.УстановитьПараметр("Наименование", НаименованиеРанееПредопределенного);
		Выб = Запрос.Выполнить().Выбрать();
		Если Выб.Количество() = 1 Тогда
			Выб.Следующий();
			Если Выб.ПометкаУдаления Тогда
				СправочникОбъект = Выб.Ссылка.ПолучитьОбъект();
...
Процедура Стоматология_ПереименоватьПараметрПрорезаниеЗубов()
	
	Параметр = ПланыВидовХарактеристик.стомПараметрыЗубнойФормулы.НайтиПоНаименованию("Срок прорезания");
	Если ЗначениеЗаполнено(Параметр) Тогда
		ПараметрОбъект = Параметр.ПолучитьОбъект();
		ПараметрОбъект.Наименование = СтрЗаменить(Параметр.Наименование, "прорезания", "прорезывания");
...
#Область КОРП
Процедура ЗаполнитьЭтоВечерВВыполненииДействийСтационара()
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Для Каждого СтрДействиеСтационара Из ДокОбъект.ДействияСтационара Цикл
			СтрДействиеСтационара.ЭтоВечер = ?(СтрДействиеСтационара.ДатаВремяВыполнения - НачалоДня(СтрДействиеСтационара.ДатаВремяВыполнения)>= 50400, Истина, Ложь);
		КонецЦикла;
...
Процедура ПеренестиОсновнойШаблонОсмотраВРегистрШаблоновМедицинскихДокументов()
	
	Значение = УправлениеНастройкамиМедицинскогоУчета.ПолучитьПараметрУчетнойПолитики("УдалитьОсновнойШаблонОсмотра");
	Если ЗначениеЗаполнено(Значение) Тогда
		Запись = РегистрыСведений.ШаблоныМедицинскихДокументовСтационара.СоздатьМенеджерЗаписи();
		Запись.РольШаблона = Перечисления.РольШаблоновСтационара.ДневниковаяЗапись;
		Запись.Специализация = Справочники.КатегорииВыработки.ПустаяСсылка();
		Запись.ШаблонОсмотра = Значение;
		Запись.Записать(Истина);
	КонецЕсли;
	
КонецПроцедуры

Процедура ОчиститьВерсиюКлассификатораКаналовГоспитализации()
	
	УстановитьПривилегированныйРежим(Истина);
	ОчиститьВерсиюКлассификатора("1.2.643.5.1.13.2.1.1.281");
	
КонецПроцедуры

Процедура КОРП_ОбновитьСтационарПод530н()
	
	// Переход каналов госпитализации на классификатор ФР НСИ ЕГИСЗ "Пути поступления пациента на госпитализацию"
	Справочники.КаналыГоспитализации.ПоместитьВАрхивЭлементыПрошлыхКлассификаторов();
	ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Справочники.КаналыГоспитализации.ПолучитьМакет("ПервоначальноеЗаполнениеПутиПоступленияПациентаВСтационар"), Истина);
	
	Справочники.удалитьПричиныОтказаВГоспитализации.ЗаполнитьДанныеСправочника(Истина, Истина);
	
	// Коечный фонд
	КОРП_ПереносКоекВРегистрДвиженияФонда();
	КОРП_ПростановкаКоекВРегистрСостоянияЗанятости();
	
	// Создание вида медкарты 003/у 530н.
	ЕстьВидыМедкартСтационара = Ложь;
	ЕстьМедкарта003у530н = Ложь;
	Выб = Справочники.ВидыМедицинскихКарт.Выбрать();
	Пока Выб.Следующий() Цикл
		Если Выб.ТипКарты = Перечисления.ТипыМедицинскихКарт.Стационар Тогда
			ЕстьВидыМедкартСтационара = Истина;
		КонецЕсли;
		Если Выб.Макет = "МедицинскаяКарта003у_530н" Тогда
			ЕстьМедкарта003у530н = Истина
		КонецЕсли;
	КонецЦикла;
	Если ЕстьВидыМедкартСтационара И Не ЕстьМедкарта003у530н Тогда
		ВидМедкарты = Справочники.ВидыМедицинскихКарт.СоздатьЭлемент();
		ВидМедкарты.Наименование = ВидМедкарты.Метаданные().Макеты.МедицинскаяКарта003у_530н.Синоним;
		ВидМедкарты.Макет = "МедицинскаяКарта003у_530н";
		ВидМедкарты.ТипКарты = Перечисления.ТипыМедицинскихКарт.Стационар;
		ВидМедкарты.ДлинаНомера = 4;
		ВидМедкарты.ДвусторонняяПечать = ТипДвустороннейПечати.ПереворотВлево;
		ВидМедкарты.НачальныйНомер = 1;
		ВидМедкарты.Записать();
		
		ТекстСообщения = "Создан вид медкарты 003у по приказу 530н. Необходимо проверить настройки нумерации и двусторонные печати!";
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	
	// Показатели жизненно важных функций организма (обновление).
	Объект = Справочники.ДействияМедсестрыСтационара.Дыхание.ПолучитьОбъект();
	Объект.Наименование = "Частота дыхания";
	Объект.Записать();
	Объект = Справочники.ДействияМедсестрыСтационара.Вес.ПолучитьОбъект();
	Объект.Наименование = "Масса тела";
	Объект.Записать();
...
Процедура КОРП_ПереносКоекВРегистрДвиженияФонда()
	
	Выборка = Справочники.КойкоМеста.Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		Запись = РегистрыСведений.ДвижениеКоечногоФонда.СоздатьМенеджерЗаписи();
		Запись.Период = Дата(1900, 1, 1);
		Запись.Койка = Выборка.Ссылка;
		Запись.Палата = Выборка.Владелец;
		Запись.Подразделение = Выборка.Владелец.Подразделение;
		Запись.Записать();
...
Процедура КОРП_ПростановкаКоекВРегистрСостоянияЗанятости()
	
	// Простановка в движения, где койка пустая (выписка и смена).
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	УстановкаУсловийПребывания.Ссылка КАК Ссылка,
	               |	УстановкаУсловийПребывания.Дата КАК Дата,
	               |	УстановкаУсловийПребывания.Клиент КАК Клиент
	               |ИЗ
	               |	Документ.УстановкаУсловийПребывания КАК УстановкаУсловийПребывания
	               |ГДЕ
	               |	УстановкаУсловийПребывания.ВидОперации <> ЗНАЧЕНИЕ(Перечисление.ВидыДвиженияВСтационаре.Поступление)
	...
	Пока Выборка.Следующий() Цикл
		...
		СостояниеКойкоМеста = ?(ДокументОбъект.ВидОперации = Перечисления.ВидыДвиженияВСтационаре.Выписка, Справочники.ВидыСостоянийКойкоМест.Свободно, Справочники.ВидыСостоянийКойкоМест.ЗанятоПациентом);
		
		НаборДвижений = ДокументОбъект.Движения.СостояниеКойкоМеста;
		
		Койка = Стационар.ПолучитьОсвобождаемоеКойкоМесто(ДокументОбъект.Ссылка, ДокументОбъект.Клиент, ДокументОбъект.ДатаНачала);
		...
		НоваяЗапись.КойкоМесто 			= ?(ДокументОбъект.ВидОперации = Перечисления.ВидыДвиженияВСтационаре.Выписка, Койка, НоваяЗапись.КойкоМесто);
		НоваяЗапись.Период 				= ДокументОбъект.ДатаНачала;
		НоваяЗапись.Регистратор 		= ДокументОбъект.Ссылка;
		
		// Для изменения условий создаем выписку из предыдущей палаты
		Если (ДокументОбъект.ВидОперации = Перечисления.ВидыДвиженияВСтационаре.ИзменениеУсловий
		...
		Тогда
			ЗаписьВыписка = НаборДвижений.Добавить();
			...
			ЗаписьВыписка.Период 			= ДокументОбъект.ДатаНачала;
			ЗаписьВыписка.Регистратор 		= ДокументОбъект.Ссылка;
		КонецЕсли;
...
Процедура КОРП_ЗагрузитьКлассификаторыДляВыписногоЭпикриза()
	
	ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Обработки.доп_ВыписнойЭпикриз.ПолучитьМакет("КлассификаторВидовАнестезии"));
	ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Обработки.доп_ВыписнойЭпикриз.ПолучитьМакет("КлассификаторРезультатовОбращений"));
	ОбщегоНазначенияСервер.ВыполнитьЗагрузкуУниверсальнымОбменомXML(Обработки.доп_ВыписнойЭпикриз.ПолучитьМакет("КлассификаторИсходовГоспитализации"));
	
КонецПроцедуры

Процедура КОРП_ПроставитьПодразделениеВРегистрКлиентыСтационара()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И КлиентыСтационара.ДатаНачала >= ДАТАВРЕМЯ(2023, 1, 1, 0, 0, 0)
	               |	И КлиентыСтационара.Подразделение = ЗНАЧЕНИЕ(Справочник.Подразделения.ПустаяСсылка)
	               |	И КлиентыСтационара.Регистратор ССЫЛКА Документ.УстановкаУсловийПребывания";
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаДвижения = НаборДвижений.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаДвижения,ДокументОбъект);
		СтрокаДвижения.Подразделение = ?(ЗначениеЗаполнено(СтрокаДвижения.Подразделение), СтрокаДвижения.Подразделение, ДокументОбъект.ДокументОбращения.Подразделение);
		СтрокаДвижения.Подразделение = ?(ЗначениеЗаполнено(СтрокаДвижения.Подразделение), СтрокаДвижения.Подразделение, ДокументОбъект.Палата.Подразделение);
		СтрокаДвижения.ДокументПоступления = Выборка.ДокументПоступления;
		СтрокаДвижения.Период = Выборка.Период;
		СтрокаДвижения.Выписан = Ложь;
...
Процедура КОРП_СтационарУдалениеВыполненияПланаЛеченияУслугДляДействияМедсестрыЗаСутки()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ДействияЗаСуткиДействияСтационара.Ссылка КАК Ссылка,
	...
	|ГДЕ
	|	ДействияЗаСуткиДействияСтационара.ДействиеСтационара ССЫЛКА Справочник.Номенклатура
	|	И ВЫРАЗИТЬ(ДействияЗаСуткиДействияСтационара.ДействиеСтационара КАК Справочник.Номенклатура).ВидНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Материал)
	|	И ДействияЗаСуткиДействияСтационара.Ссылка.Проведен
	|ИТОГИ ПО
	|	Ссылка"
	;
	ОперацийВТранзакции = 25;
	Сч = 0;
	
	НЗ = РегистрыСведений.ВыполнениеПланаЛечения.СоздатьНаборЗаписей();
	Отбор = Новый Структура("ИдентификаторЭлемента");
	
	ВыбСсыка = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	НачатьТранзакцию();
	Пока ВыбСсыка.Следующий() Цикл
		
		ВыбНоменклатура = ВыбСсыка.Выбрать();
		
		НЗ.Отбор.Регистратор.Установить(ВыбСсыка.Ссылка);
		НЗ.Прочитать();
		
		Таб = НЗ.Выгрузить();
		мОбрИнд = Таб.Количество()-1;
		Для Сч = 0 По мОбрИнд Цикл
			
			ВыбНоменклатура.Сбросить();
			Отбор.ИдентификаторЭлемента = Таб[мОбрИнд-Сч].ИдентификаторЭлемента;
			Если ВыбНоменклатура.НайтиСледующий(Отбор) Тогда
				Таб.Удалить(мОбрИнд-Сч);
			КонецЕсли;
		КонецЦикла;
		НЗ.Загрузить(Таб);
		НЗ.Записать();
...
Процедура КОРП_ЗаполнитьДанныеРегистраОтчетности007у()
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	МедицинскийОтчет.Ссылка КАК Ссылка,
	               |	МедицинскийОтчет.ДанныеОтчета КАК ДанныеОтчета
	               |ИЗ
	               |	Документ.МедицинскийОтчет КАК МедицинскийОтчет
	               |ГДЕ
	               |	МедицинскийОтчет.ИсточникОтчета = ""МедицинскийОтчет007у""
	               |	И НЕ МедицинскийОтчет.ПометкаУдаления";
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		Движения = Выборка.Ссылка.ПолучитьОбъект().Движения.ДанныеМедицинскихОтчетов007у;
		ДанныеОтчета = Выборка.ДанныеОтчета.Получить();
		
		Если ТипЗнч(ДанныеОтчета) = Тип("Структура") Тогда
			
			Данные = ДанныеОтчета.ПоказателиОтчета.Титульный;	
			
			Если Данные.Свойство("Подразделение") И Данные.Свойство("Дата") Тогда 
				
				Если КОРП_СуществуетДругой007(Выборка.Ссылка, Данные.Подразделение, Данные.Дата) Тогда
				...
				Пока Данные.Свойство(ИмяСтроки) Цикл
					...
					ПереведеноИзДругихОтделений = ?(Данные.Свойство("СтрокаТаблицы__010_" + Строка), Данные["СтрокаТаблицы__010_" + Строка], 0);
					ПереведеноВДругиеОтделения  = ?(Данные.Свойство("СтрокаТаблицы__011_" + Строка), Данные["СтрокаТаблицы__011_" + Строка], 0);
					
					Если ПереведеноИзДругихОтделений = 0 И ПереведеноВДругиеОтделения = 0 Тогда
						...
						Запись.ПереведеноИзДругихОтделений 		= 0;
						Запись.ПереведеноВДругиеОтделения 		= 0;
						...
						Запись.ПереведеноИзДругихОтделенийСтаршеТрудоспособногоВозраста = 0;
						Запись.ПереведеноВДругиеОтделенияСтаршеТрудоспособногоВозраста = 0;
...
Функция КОРП_СуществуетДругой007(Объект, Подразделение, Дата)
...
Функция КОРП_Стационар_ПервоначальноеЗаполнениеНоменклатура_ИсполнительВСтационаре()
	...
	|	И Номенклатура.ИсполнительВСтационаре = ЗНАЧЕНИЕ(Перечисление.ИсполнителиВСтационаре.ПустаяСсылка)"
	;
	Выб = Запрос.Выполнить().Выбрать();
	...
	Пока Выб.Следующий() Цикл
		...
		Попытка
			НоменклатураОбъект = Выб.Ссылка.ПолучитьОбъект();
			// Аналогично старому поведению АРМ палатной медсестры
			НоменклатураОбъект.ИсполнительВСтационаре = Перечисления.ИсполнителиВСтационаре.ВыполняетсяВрачомОтображается; 
			НоменклатураОбъект.ОбменДанными.Загрузка = Истина;
			НоменклатураОбъект.Записать();
		Исключение КонецПопытки;
