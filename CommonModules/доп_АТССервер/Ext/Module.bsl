///////////////////////////////////////////////////////////////////////////////
// Общий серверный модуль работы с ОЛИМП.АТС
///////////////////////////////////////////////////////////////////////////////
...
Функция ПолучитьПараметрыОткрытияФормыНастроек() ЭКСПОРТ
	Возврат доп_ТелефонияСервер.ПолучитьПараметрыОткрытияФормыНастроек("доп_БитАТСНастройки", "доп_АТССервер");
КонецФункции
...
Функция ПроверитьПраваАдминистрировать() ЭКСПОРТ
	АдминистраторАТС = (РольДоступна(Метаданные.Роли.доп_АдминистраторБитАТС) ИЛИ РольДоступна("ПолныеПрава"));
	Возврат АдминистраторАТС;
КонецФункции

Функция ПроверитьПраваИспользовать() ЭКСПОРТ
	Возврат РольДоступна(Метаданные.Роли.доп_ПользовательБитАТС);
КонецФункции
...
Функция НайтиКонтрагентаПоНомеру(Знач НомерДляПоиска, ВнешнийВызов, КонтрагентСсылка, КонтактноеЛицоСсылка, флагЕстьДубли) ЭКСПОРТ
	...
	Если найден <> Истина Тогда
		Если ВнешнийВызов Тогда
			...
			найден = доп_ТелефонияСервер.НайтиКонтрагентаИКонтактноеЛицоПоНомеру(НомерДляПоиска, КонтрагентСсылка, КонтактноеЛицоСсылка,
...
Функция ПолучитьНомераАТС(АТССсылка, РодительНомеров) ЭКСПОРТ
	...
	ЗапросНомеров.Текст = "ВЫБРАТЬ
	                      |	доп_НомераБитАТС.Код,
	                      |	доп_НомераБитАТС.Наименование,
	                      |	доп_НомераБитАТС.Номер,
	                      |	доп_НомераБитАТС.ГруппаДоступа
	                      |ИЗ
	                      |	Справочник.доп_НомераБитАТС КАК доп_НомераБитАТС
	                      |ГДЕ
	                      |	доп_НомераБитАТС.ЭтоГруппа = ЛОЖЬ
	                      |	И доп_НомераБитАТС.Владелец = &Владелец
	                      |	И доп_НомераБитАТС.ПометкаУдаления = ЛОЖЬ";
	ЗапросНомеров.УстановитьПараметр("Владелец", АТССсылка);
	//
	Если РодительНомеров <> ПредопределенноеЗначение("Справочник.доп_НомераБитАТС.ПустаяСсылка") Тогда
		ЗапросНомеров.Текст = ЗапросНомеров.Текст + "
		                      |	И (доп_НомераБитАТС.Родитель В ИЕРАРХИИ (&Родитель)
		                      |			ИЛИ доп_НомераБитАТС.Родитель = &Родитель)";
...
Функция ПолучитьПараметрыАТС(АТССсылка, СписокНомеровХоста, СоответствиеНомеровХоста) ЭКСПОРТ
	...
	массивНомеров = ПолучитьНомераАТС(АТССсылка, ПредопределенноеЗначение("Справочник.доп_НомераБитАТС.ПустаяСсылка"));
...
Процедура ПроверитьНаличиеЭлементаНомеровАТС(ссылкаАТС, родительСсылка, стрНаименование, стрНомер, флагПометкаУдаления=Ложь) ЭКСПОРТ
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	доп_НомераБитАТС.Наименование,
	               |	доп_НомераБитАТС.Родитель,
	               |	доп_НомераБитАТС.ПометкаУдаления
	               |ИЗ
	               |	Справочник.доп_НомераБитАТС КАК доп_НомераБитАТС
	               |ГДЕ
	               |	доп_НомераБитАТС.Владелец = &Владелец
	               |	И доп_НомераБитАТС.ЭтоГруппа = ЛОЖЬ
	               |	И доп_НомераБитАТС.Номер = &Номер";
	Запрос.УстановитьПараметр("Владелец", ссылкаАТС);
	Запрос.УстановитьПараметр("Номер", стрНомер);
	табл = Запрос.Выполнить().Выгрузить();
	Если табл.Количество() = 0 Тогда
		// создание
		новОбъект = Справочники.доп_НомераБитАТС.СоздатьЭлемент();
...
Функция ПроверитьНаличиеГруппыНомеровАТС(ссылкаАТС, родительСсылка, стрНаименование, стрИдентификаторГруппы, флагПометкаУдаления=Ложь) ЭКСПОРТ
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	доп_НомераБитАТС.Владелец = &Владелец
	               |	И доп_НомераБитАТС.Код = &Код
	               |	И доп_НомераБитАТС.ЭтоГруппа = ИСТИНА";
	Запрос.УстановитьПараметр("Владелец", ссылкаАТС);
	...
	Иначе
		// создание
		новОбъект = Справочники.доп_НомераБитАТС.СоздатьГруппу();
		новОбъект.Наименование	= стрНаименование;
		новОбъект.Владелец		= ссылкаАТС;
		новОбъект.Родитель		= родительСсылка;
...
Процедура ЗагрузитьНомераСотрудников(АТСссылка, группаСотрудниковРодитель, загружено) ЭКСПОРТ
	...
	ЗапросСотрудники.Текст = "ВЫБРАТЬ
	                         ...
	                         |ИЗ
	                         |	Справочник." + доп_ТелефонияСерверПереопределяемый.ПолучитьИмяСправочникаСотрудников() + " КАК Сотрудники
	                         |ГДЕ
	...
	ЗагрузитьГруппуНомеровСотрудников(АТСссылка, Справочники.доп_НомераБитАТС.ПустаяСсылка(), дерСотрудники.Строки, загружено);
...
Процедура СинхронизацияНомеровСотрудников() ЭКСПОРТ
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	доп_ТелефонныеСтанцииБитАТС.Ссылка,
	               |	доп_ТелефонныеСтанцииБитАТС.ГруппаСинхронизацииСотрудников
	               |ИЗ
	               |	Справочник.доп_ТелефонныеСтанцииБитАТС КАК доп_ТелефонныеСтанцииБитАТС
	               |ГДЕ
	               |	доп_ТелефонныеСтанцииБитАТС.ПометкаУдаления = ЛОЖЬ
	               |	И доп_ТелефонныеСтанцииБитАТС.СинхронизацияССотрудниками = ИСТИНА";
...
Функция ВебСервисОткрытьСоединение(ссылкаАТС, стрАдрес, стрПользователь, стрПароль, стрСписокНомеров, флагМонитор) ЭКСПОРТ
	вс = доп_АТСВебСервисы.ПолучитьВебСервисУправлениеАТС(ссылкаАТС);
	стрКлиентИнфо = Строка(доп_ТелефонияСервер.ПолучитьТекущегоПользователя()) + " (" + ?(флагМонитор, "монитор", "панель управления") + ")";
...
Функция ВебСервисОбновитьНомер(ссылкаАТС, ИдКлиента, стрСписокНомеровПолученияЗвонков, стрСписокНомеровПолученияСтатуса) ЭКСПОРТ
	...
	Возврат доп_ТелефонияКлиентСервер.ПолучитьАТСИнфоИзJSON(Данные);
КонецФункции
...
Функция ВебСервисПолучитьИнформациюАТС(стрВебСервисАдрес, стрАдрес, стрПользователь, стрПароль, стрТипИнформации) ЭКСПОРТ
	вс = доп_АТСВебСервисы.ПолучитьВебСервисУправлениеЗвонками(стрВебСервисАдрес, стрАдрес);
...
Функция ПолучитьНастройкиТелефонии()
	...
	Возврат РегистрыСведений.доп_БитАТСНастройки.Получить(СтруктураОтбора);
КонецФункции

Процедура УстановитьНастройку(стрНастройка, значениеНастройки)
	доп_ТелефонияСервер.УстановитьНастройкуТекущегоПользователя("доп_БитАТСНастройки", "доп_АТССервер", стрНастройка, значениеНастройки);
КонецПроцедуры
...
Процедура ЗагрузитьГруппуНомеровСотрудников(ссылкаАТС, родительСсылка, строкиСотрудники, загружено)
	...
	Для Каждого строкаСотр Из строкиСотрудники Цикл
		...
		Иначе
			...
			массНомераСотрудникаИБ = доп_ТелефонияСерверПереопределяемый.НайтиНомераСотрудника(строкаСотр.Ссылка);
			Для Каждого номерСотрудника Из массНомераСотрудникаИБ Цикл
				списокНомеровИБ = доп_ТелефонияКлиентСервер.СтрРазбить(номерСотрудника, ",");
			...
			Для Каждого номерСотрудника Из массНомераСотрудника Цикл
				сокрНомерСотрудника = доп_ТелефонияКлиентСервер.СократитьНомерДляПоиска(доп_ТелефонияКлиентСервер.ОчиститьНомерТолькоЦифры(номерСотрудника));
...
Процедура ПроверкаДублейНомеров(ссылкаАТС)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |
	               |ИМЕЮЩИЕ
	               |	КОЛИЧЕСТВО(доп_НомераБитАТС.Номер) > 1";
	Запрос.УстановитьПараметр("Владелец", ссылкаАТС);
	...
	Для Каждого строкаДубли Из таблДубли Цикл
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ГДЕ
		               |	доп_НомераБитАТС.Владелец = &Владелец
		               |	И доп_НомераБитАТС.Родитель = &Родитель
		               |	И доп_НомераБитАТС.Наименование = &Наименование
		               |	И доп_НомераБитАТС.Номер = &Номер";
		Запрос.УстановитьПараметр("Владелец", ссылкаАТС);
