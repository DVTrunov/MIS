...
Функция АктуальныйВидРЭМДЗаключенияМО() Экспорт
	...
	Возврат РегистрируемыеЭлектронныеМедДокументы(230);
...
Функция ВидРЭМДРецепта(ФормаРецепта) Экспорт
	...
	ИначеЕсли ФормаРецепта = Перечисления.ФормыРецептов.Форма148_1_у_04_л Тогда
		Возврат РегистрируемыеЭлектронныеМедДокументы(141);
	КонецЕсли;
...
Функция ДолжностиФРМРПоДолжностямМЗ() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ДолжностиФРМР.Ссылка КАК ДолжностьФРМР,
		|	ИдентификаторыДолжностей.ИдентификаторДолжностиМЗ КАК ИдентификаторДолжностиМЗ
		|ИЗ
		|	(ВЫБРАТЬ
		|		МИНИМУМ(АтрибутыКлассификатора.Ссылка.УИДЕГИСЗ) КАК УИДЕГИСЗ,
		...
		|	ГДЕ
		|		АтрибутыКлассификатора.Ссылка.ВидКлассификатора = ЗНАЧЕНИЕ(Перечисление.ВидыКлассификаторовМинЗдрава.ФРМРДолжностиМедицинскогоПерсонала)
		|		И НЕ АтрибутыКлассификатора.Ссылка.ПометкаУдаления
		|		И АтрибутыКлассификатора.Ключ = ""federal_code""
		|	
		|	СГРУППИРОВАТЬ ПО
		|		ВЫРАЗИТЬ(АтрибутыКлассификатора.Значение КАК СТРОКА(10))) КАК ИдентификаторыДолжностей
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлассификаторыМинЗдрава КАК ДолжностиФРМР
		|		ПО ИдентификаторыДолжностей.УИДЕГИСЗ = ДолжностиФРМР.УИДЕГИСЗ
		|			И (ДолжностиФРМР.ВидКлассификатора = ЗНАЧЕНИЕ(Перечисление.ВидыКлассификаторовМинЗдрава.ФРМРДолжностиМедицинскогоПерсонала))
		|			И (НЕ ДолжностиФРМР.ПометкаУдаления)";
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	ИдентификаторыДолжностей = Новый ТаблицаЗначений;
	ИдентификаторыДолжностей.Колонки.Добавить("ДолжностьФРМР", Новый ОписаниеТипов("СправочникСсылка.КлассификаторыМинЗдрава"));
	ИдентификаторыДолжностей.Колонки.Добавить("ИдентификаторДолжностиМЗ", Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(10,0)));
	
	Пока Выборка.Следующий() Цикл
		НоваяСтрока = ИдентификаторыДолжностей.Добавить();
		НоваяСтрока.ДолжностьФРМР = Выборка.ДолжностьФРМР;
		НоваяСтрока.ИдентификаторДолжностиМЗ = Число(Выборка.ИдентификаторДолжностиМЗ);
	КонецЦикла;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ИдентификаторыДолжностей.ДолжностьФРМР КАК ДолжностьФРМР,
		|	ИдентификаторыДолжностей.ИдентификаторДолжностиМЗ КАК ИдентификаторДолжностиМЗ
		|ПОМЕСТИТЬ ИдентификаторыДолжностей
		|ИЗ
		|	&ИдентификаторыДолжностей КАК ИдентификаторыДолжностей
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КлассификаторыМинЗдрава.Ссылка КАК ДолжностьМЗ,
		|	ИдентификаторыДолжностей.ДолжностьФРМР КАК ДолжностьФРМР
		|ИЗ
		|	Справочник.КлассификаторыМинЗдрава КАК КлассификаторыМинЗдрава
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ИдентификаторыДолжностей КАК ИдентификаторыДолжностей
		|		ПО (КлассификаторыМинЗдрава.ВидКлассификатора = ЗНАЧЕНИЕ(Перечисление.ВидыКлассификаторовМинЗдрава.ДолжностиРаботниковМедОрганизаций))
		|			И КлассификаторыМинЗдрава.УИДЕГИСЗ = ИдентификаторыДолжностей.ИдентификаторДолжностиМЗ";
	
	Запрос.УстановитьПараметр("ИдентификаторыДолжностей", ИдентификаторыДолжностей);
	
	Возврат Запрос.Выполнить().Выгрузить();
	
КонецФункции

Функция ЕдиницаИзмененияНСИПоОКЕИ(ЕдиницаОКЕИ) Экспорт
	
	Если ЗначениеЗаполнено(ЕдиницаОКЕИ.Код) Тогда
		Если СокрЛП(ЕдиницаОКЕИ.Код) = "875" Тогда
			Код = "111";
		Иначе
			Код = СокрЛП(ЕдиницаОКЕИ.Код);
		КонецЕсли;
		Возврат ИнтеграцияЕГИСЗСервер.ПолучитьЭлементКлассификатораПоЗначениюАтрибута("OKEI_CODE", Код, Перечисления.ВидыКлассификаторовМинЗдрава.ЕдиницыИзмерения);
	КонецЕсли;
