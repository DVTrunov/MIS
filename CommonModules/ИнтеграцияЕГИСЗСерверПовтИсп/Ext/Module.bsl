...
Функция ИспользуетсяИнтеграцияРЭМД(Филиал = Неопределено, МедицинскаяОрганизация = Неопределено) Экспорт
	
	Возврат УправлениеНастройкамиПереопределяемый.ПолучитьПараметрУчета("ИспользоватьЕГИСЗ") И ИнтеграцияЕГИСЗСервер.ПолучитьЗначениеНастройки("ИспользоватьРЭМД", Филиал, МедицинскаяОрганизация);
	
КонецФункции

Функция ПолучитьИдентифакаторМИС_В_РЭМД(Филиал = Неопределено, МедицинскаяОрганизация = Неопределено) Экспорт
	
	Возврат ИнтеграцияЕГИСЗСервер.ПолучитьЗначениеНастройки("ИдентификаторМИСвРЭМД", Филиал, МедицинскаяОрганизация);
	
КонецФункции

Функция ПолучитьИдентификаторМИС_В_ИПС(Филиал = Неопределено, МедицинскаяОрганизация = Неопределено) Экспорт
	
	Возврат ИнтеграцияЕГИСЗСервер.ПолучитьЗначениеНастройки("ИдентификаторМИСвИПС", Филиал, МедицинскаяОрганизация);
...
Функция ПолучитьСертификатПодписанияСообщенийSOAP(Филиал = Неопределено, МедицинскаяОрганизация = Неопределено) Экспорт
	
	Возврат ИнтеграцияЕГИСЗСервер.ПолучитьЗначениеНастройки("СертификатЭЦППодписиSOAPСообщений", Филиал, МедицинскаяОрганизация);
...
Функция ПолучитьOIDИНаменованиеМедицинскойОрганизации(Филиал = Неопределено, МедицинскаяОрганизация = Неопределено) Экспорт
	
	Результат = Новый Структура("Наименование, КодOIDМедицинскойОрганизации", );
	
	МО = Неопределено;
	Если МедицинскаяОрганизация <> Неопределено Тогда
		МО = МедицинскаяОрганизация;
	ИначеЕсли Филиал <> Неопределено Тогда
		МО = Филиал.Организация.МедицинскаяОрганизация;
	Иначе
		ФилиалПользователя = УправлениеНастройками.ПолучитьФилиалПоУмолчаниюПользователя();
		МО = ФилиалПользователя.Организация.МедицинскаяОрганизация;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(МО) Тогда
		Результат.КодOIDМедицинскойОрганизации = МО.OIDОрганизации;
		Результат.Наименование = МО.Наименование;
	КонецЕсли;
...
Функция ПолучитьПутьКАрхивуРЭМД(Филиал = Неопределено, МедицинскаяОрганизация = Неопределено) Экспорт
	
	Возврат ИнтеграцияЕГИСЗСервер.ПолучитьЗначениеНастройки("ПутьКАрхивуРЭМД", Филиал, МедицинскаяОрганизация);
	
КонецФункции

Функция ПолучитьПутьКАрхивуЛогов(Филиал = Неопределено, МедицинскаяОрганизация = Неопределено) Экспорт
	
	Возврат ИнтеграцияЕГИСЗСервер.ПолучитьЗначениеНастройки("ПутьКАрхивуЛогов", Филиал, МедицинскаяОрганизация);
...
Функция ПолучитьДатуНачалаОбменаИнформациейСРЭМД(Филиал = Неопределено, МедицинскаяОрганизация = Неопределено) Экспорт
	
	Возврат ИнтеграцияЕГИСЗСервер.ПолучитьЗначениеНастройки("ДатаНачалаОбменаИнформациейСРЭМД", Филиал, МедицинскаяОрганизация);
...
//
Функция ПолучитьИменаРеквизитовКлассификатора(Справочник, Версия = Неопределено) Экспорт
	...
	Попытка
		СтруктураВозвращаемогоЗначения = Менеджер.ПолучитьИмяРеквизитаDisplayNameИCode(Версия);
	Исключение
		Попытка
...
Функция ПолучитьИменаРеквизитовКлассификатораПоВидуКлассификатора(ВидКлассификатора, Версия = Неопределено) Экспорт
	
	ПолноеИмя	= ВидКлассификатора.Метаданные().ПолноеИмя();
	Менеджер	= ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмя);
	
	Попытка
		СтруктураВозвращаемогоЗначения = Менеджер.ПолучитьИмяРеквизитаDisplayNameИCode(ВидКлассификатора, Версия);
	Исключение
		Попытка
...
Функция КэшОтветаНаЗапросРегистрацииРЭМД(МедицинскаяОрганизация) Экспорт
	
	Возврат ИнтеграцияЕГИСЗСервер.КэшОтветаЕГИСЗ(МедицинскаяОрганизация, Перечисления.ПодсистемыЕГИСЗ.РЭМД, "registerDocumentResult");
...
Функция КэшироватьОтветыРЭМД(Филиал = Неопределено, МедицинскаяОрганизация = Неопределено) Экспорт
	
	Возврат ИнтеграцияЕГИСЗСервер.ПолучитьЗначениеНастройки("КэшироватьОтветыРЭМД", Филиал, МедицинскаяОрганизация);
...
Функция ПолучитьСертификатыМОПользователя() Экспорт
	...
	Запрос.Текст = 
		...
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЕГИСЗНастройкиИнтеграции КАК ЕГИСЗНастройкиИнтеграции
		|			ПО (ЕГИСЗНастройкиИнтеграции.МедицинскаяОрганизация = Организации.МедицинскаяОрганизация)
		|				И (НЕ Организации.ПометкаУдаления)
		|		ПО (Организации.Ссылка = Филиалы.Организация)
		|			И (НЕ Филиалы.ПометкаУдаления)
		|			И (ЕГИСЗНастройкиИнтеграции.ИспользоватьРЭМД)";
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		СертификатыМОПользователяПоФилиалу = ПолучитьСертификатыМОПользователяПоФилиалу(Выборка.Организация);
		Для Каждого СертификатПоФилиалу Из СертификатыМОПользователяПоФилиалу Цикл
			Если СертификатыМО.Найти(СертификатПоФилиалу) = Неопределено Тогда
				СертификатыМО.Добавить(СертификатПоФилиалу);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	Возврат СертификатыМО;
	
КонецФункции

Функция ПолучитьСертификатыМОПользователяПоФилиалу(Организация)
	
	СертификатыМО = Новый Массив;
	СертификатыМО.Добавить(ПолучитьСертификатПодписанияСообщенийSOAP(, Организация.МедицинскаяОрганизация));
	
	Для Каждого СертификатМОПользователя Из ЭЦП_УМЦ_Сервер.ПолучитьСертификатыМОПользователя(Организация, Перечисления.ПолномочияЭЦП.РЭМД) Цикл
		Если СертификатыМО.Найти(СертификатМОПользователя) = Неопределено Тогда
			СертификатыМО.Добавить(СертификатМОПользователя);
		КонецЕсли;
	КонецЦикла;
