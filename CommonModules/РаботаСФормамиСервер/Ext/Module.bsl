...
Процедура ЗапретИнтерактивногоОткрытия(Отказ) Экспорт
	...
	Если Отказ Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Объект конфигурации не предназначен для непосредственного использования.");
	КонецЕсли;	
	
КонецПроцедуры

#Область ФормыСправочников
...
Процедура ДокументСписокПриСозданииНаСервере(ЭтаФорма, ИмяЭлементаСписка = Неопределено) Экспорт
	...
	// Добавлено Макеев (
	ДокументыСОтборомПоОрганизации = Новый Массив;
	ДокументыСОтборомПоОрганизации.Добавить("ЖурналДокументов.КассовыеДокументы.Форма.ФормаСписка");
	ДокументыСОтборомПоОрганизации.Добавить("Документ.ВозвратПоПлатежнойКарте.Форма.ФормаСпискаУпр");
	ДокументыСОтборомПоОрганизации.Добавить("Документ.ПриходныйКассовыйОрдер.Форма.ФормаСпискаУпр");
	ДокументыСОтборомПоОрганизации.Добавить("Документ.РасходныйКассовыйОрдер.Форма.ФормаСпискаУпр");
	ДокументыСОтборомПоОрганизации.Добавить("Документ.ОплатаПлатежнойКартой.Форма.ФормаСпискаУпр");
	ДокументыСОтборомПоОрганизации.Добавить("Документ.ПлатежноеПоручениеВходящее.Форма.ФормаСпискаУпр");
	ДокументыСОтборомПоОрганизации.Добавить("Документ.ПлатежноеПоручениеИсходящее.Форма.ФормаСпискаУпр");
	ДокументыСОтборомПоОрганизации.Добавить("Документ.Заявка.Форма.ФормаСписка");
	ДокументыСОтборомПоОрганизации.Добавить("Документ.ОказаниеУслуг.Форма.ФормаСписка");
	Если Не ДокументыСОтборомПоОрганизации.Найти(ЭтаФорма.ИмяФормы) = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(омг_Организация) Тогда
			ОтборПоОрганизации = ЭтаФорма.Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		...
		КонецЕсли;
	КонецЕсли;       
	ДокументыСБлокировкойПорядка = Новый Массив;
	ДокументыСБлокировкойПорядка.Добавить("ЖурналДокументов.КассовыеДокументы");
	ДокументыСБлокировкойПорядка.Добавить("Документ.ВозвратПоПлатежнойКарте");
	ДокументыСБлокировкойПорядка.Добавить("Документ.ПриходныйКассовыйОрдер");
	ДокументыСБлокировкойПорядка.Добавить("Документ.РасходныйКассовыйОрдер");
	ДокументыСБлокировкойПорядка.Добавить("Документ.ОплатаПлатежнойКартой");
	ДокументыСБлокировкойПорядка.Добавить("Документ.ПлатежноеПоручениеВходящее");
	ДокументыСБлокировкойПорядка.Добавить("Документ.ПлатежноеПоручениеИсходящее");
	ДокументыСБлокировкойПорядка.Добавить("Документ.Заявка");
	ДокументыСБлокировкойПорядка.Добавить("Документ.ОказаниеУслуг");
	
	ИмяПоиска = Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "Форма") - 2);
	
	Если Не ДокументыСБлокировкойПорядка.Найти(ИмяПоиска) = Неопределено Тогда
		омг_ДополнительныеАлгоритмы.УстановитьЗапретСортировки(ЭтаФорма.Список);
	КонецЕсли;       
	// )
КонецПроцедуры
...
//	Структура.
//
Функция ФормаДокументаПриСозданииНаСервере(ЭтаФорма, ПовторноеОткрытие=Ложь) Экспорт 
...
// Устаревший синтаксис: Стандартный обработчик создания формы документа.
//
// Параметры:
//  ЭтаФорма			 - ФормаКлиентскогоПриложения	 - форма документа.
//  ПовторноеОткрытие	 - Булево						 - признак повторного открытия формы.
...
Функция ФормаДокументаПриОткрытииСервер(ЭтаФорма, ПовторноеОткрытие=Ложь) Экспорт 
	...
	Возврат ФормаДокументаПриСозданииНаСервере(ЭтаФорма, ПовторноеОткрытие);
	...
	Если ОбщегоНазначенияСервер.ЕстьРеквизитДокумента(ИмяФилиал, фМетаданные) Тогда
		...
		Если ПараметрыСеанса.УчетПоНесколькимФилиалам Тогда
			...
			Тогда
				...
				////////////////////////////////////////////////
				// ОМГ 
				Или РольДоступна("омг_ОтборПоОрганизацииВОтчетеПоВыработке")
				// ОМГ 
				////////////////////////////////////////////////
...
Процедура НастройкаПодбораПриСоздании(ЭтаФорма, ПоказыватьЦены = Ложь, ИмяТЧ = Неопределено, ИмяВидаНом = Неопределено, ИмяВидаНом2 = Неопределено) Экспорт 
	...
	Если ЭтаФорма.Элементы.ГруппаПодбор.ПодчиненныеЭлементы.Количество() = 0 Тогда
		...
		ЭтаФорма.Элементы.ГруппаПодбор.РастягиватьПоГоризонтали = Истина;
		//ЭтаФорма.Элементы.ГруппаПодбор.Ширина = 50;
		...
		//////////////////////////////////////////
		//омг
			МассивРеквизитов.Добавить(Новый РеквизитФормы("Омг_ТолькоСЦенами", ОписаниеТипаБулево,"","Только с ценами"));   
			Омг_ТолькоСЦенами = Истина;
		//омг
	    //////////////////////////////////////////
		КонецЕсли;
		...
		Если ПоказыватьЦены Тогда
			...
			Если ОбъектФормы <> Неопределено Тогда
				...
				//	ДатаЗначение = НачалоДня(ТекущаяДата());
				//КонецЕсли;              
				ЭтаФорма.Омг_ТолькоСЦенами = Истина;   
				Если ТипЗнч(ОбъектФормы.Ссылка) = Тип("ДокументСсылка.Заявка") И ЗначениеЗаполнено(ОбъектФормы.ДатаНачала) Тогда
					 ДатаЗначение = ОбъектФормы.ДатаНачала;
				ИначеЕсли ОбъектФормы.Свойство("Дата") И ЗначениеЗаполнено(ОбъектФормы.Дата) Тогда
				    ДатаЗначение = ОбъектФормы.Дата;
				Иначе 
					ДатаЗначение = НачалоДня(ТекущаяДата());
				КонецЕсли;
		...
		Если ПоказыватьЦены Тогда
			...
			|ПОМЕСТИТЬ ВТ_ПриостановленныеИсследования
			|ИЗ
			|	РегистрСведений.омг_ПриостановленныеИсследованияЛИС КАК омг_ПриостановленныеИсследованияЛИС
			|ГДЕ
			|	омг_ПриостановленныеИсследованияЛИС.Филиал = &омг_Филиал
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	омг_ПрочиеПриостановленныеИсследования.Номенклатура,
			|	омг_ПрочиеПриостановленныеИсследования.Причина
			|ИЗ
			|	РегистрСведений.омг_ПрочиеПриостановленныеИсследования КАК омг_ПрочиеПриостановленныеИсследования
			|ГДЕ
			|	омг_ПрочиеПриостановленныеИсследования.Филиал = &омг_Филиал
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|	СправочникНоменклатура.омг_Теги КАК омг_Теги,
			|	СправочникНоменклатура.Ссылка В (&ВыбранныеПозиции) КАК ПозицияВыбрана,
			|	НЕ ВТ_ПриостановленныеИсследования.Номенклатура ЕСТЬ NULL КАК Приостановлено,
			|	ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) КАК Цена,
			|	ЕСТЬNULL(ВТ_ПриостановленныеИсследования.Причина, """") КАК ПричинаБлокировки
			|ИЗ
			|	Справочник.Номенклатура КАК СправочникНоменклатура
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПриостановленныеИсследования КАК ВТ_ПриостановленныеИсследования
			|		ПО (ВТ_ПриостановленныеИсследования.Номенклатура = СправочникНоменклатура.Ссылка)
			...
			|						И ВЫБОР
			|							КОГДА &омг_Филиал = НЕОПРЕДЕЛЕНО
			|								ТОГДА ИСТИНА
			|							ИНАЧЕ омг_Филиал = &омг_Филиал
			|						КОНЕЦ
			...
			|ГДЕ
			|	ВЫБОР
			|			КОГДА &Омг_ТолькоСЦенами
			|				ТОГДА ЦеныНоменклатуры.Цена > 0
			|			ИНАЧЕ 0 = 0
			|		КОНЕЦ";   
			////////////////////////////////////////////////
			// ОМГ
			реквСписок.Параметры.УстановитьЗначениеПараметра("Омг_ТолькоСЦенами",Омг_ТолькоСЦенами); 
						
			Если ОбъектФормы.Свойство("омг_СтраховаяПрограмма") и ЗначениеЗаполнено(ОбъектФормы.омг_СтраховаяПрограмма) Тогда
				реквСписок.Параметры.УстановитьЗначениеПараметра("Прейскурант",ОбъектФормы.омг_СтраховаяПрограмма);	
			Иначе	
			...
			реквСписок.Параметры.УстановитьЗначениеПараметра("Прейскурант",РеквизитПрейскурант);
			// Добавлено Макеев (
			реквСписок.Параметры.УстановитьЗначениеПараметра("ВыбранныеПозиции", Новый Массив);
			Филиал = Неопределено;
			Если Не ОбъектФормы = Неопределено Тогда
				Если ОбъектФормы.Свойство("омг_СтраховаяПрограмма") И Не ОбъектФормы.омг_СтраховаяПрограмма.Пустая() Тогда
					реквСписок.Параметры.УстановитьЗначениеПараметра("Прейскурант",ОбъектФормы.омг_СтраховаяПрограмма);
				ИначеЕсли ОбъектФормы.Свойство("омг_Прейскурант") И Не ОбъектФормы.омг_Прейскурант.Пустая() Тогда
					реквСписок.Параметры.УстановитьЗначениеПараметра("Прейскурант",ОбъектФормы.омг_Прейскурант);
				КонецЕсли;
				ОбъектФормы.Свойство("Филиал", Филиал);
				реквСписок.Параметры.УстановитьЗначениеПараметра("омг_Филиал",Филиал);
		...
		// Добавлено Макеев (
		Если ПоказыватьЦены Тогда
			новКолонка = ЭтаФорма.Элементы.Добавить("КолонкаПозицияВыбрана", тип("ПолеФормы"), списокНоменклатуры);
			новКолонка.ПутьКДанным = "РеквизитСписокПодбор.ПозицияВыбрана";
			новКолонка.Заголовок = " ";
			новКолонка.Вид = ВидПоляФормы.ПолеФлажка;
		КонецЕсли;
		// )
		...
		//новКолонка.РастягиватьПоГоризонтали = Истина;
		...
		КонецЕсли;
		//Трунов
		новКолонка = Этаформа.Элементы.Добавить("КолонкаТеги", тип("ПолеФормы"), списокНоменклатуры); 
		новКолонка.Заголовок = "Теги";
		новКолонка.ПутьКДанным = "РеквизитСписокПодбор.омг_Теги";
		новКолонка.Ширина = 2;  
		//новКолонка.РастягиватьПоГоризонтали = Истина;
		новКолонка.ЦветТекста = WebЦвета.Белый; 
		//Трунов
		// Добавлено Макеев (
		реквСписок.УстановитьОбязательноеИспользование("ПозицияВыбрана", Истина);
		реквСписок.УстановитьОбязательноеИспользование("Приостановлено", Истина);
		реквСписок.УстановитьОбязательноеИспользование("ПричинаБлокировки", Истина);
		// )
		...
		Если ПоказыватьЦены Тогда
			...
			////////////////////////////////////////////////
			// ОМГ	
			ЭлементОмг_ТолькоСЦенами = ЭтаФорма.Элементы.Добавить("Омг_ТолькоСЦенами", Тип("ПолеФормы"));
			ЭлементОмг_ТолькоСЦенами.Вид = ВидПоляФормы.ПолеФлажка;
			ЭлементОмг_ТолькоСЦенами.ПутьКДанным = "Омг_ТолькоСЦенами";
			ЭлементОмг_ТолькоСЦенами.УстановитьДействие("ПриИзменении", "Подключаемый_Омг_ТолькоСЦенами");
			ЭтаФорма.Элементы.Переместить(ЭлементОмг_ТолькоСЦенами, ЭтаФорма.Элементы.ГруппаПодбор);	
			// ОМГ 
			////////////////////////////////////////////////
		КонецЕсли;
	...
	// Добавлено Макеев (
	омг_ДополнительныеАлгоритмы.УстановитьЗапретСортировки(реквСписок, "Наименование");
	// )
