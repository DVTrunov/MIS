...
Функция дкУстановитьЦенуСтрокиТабЧастиСервер(ЭтотОбъект, ИмяТЧ, СтруктураСтрока, Прейскурант = Неопределено, Знач фМетаданныеРеквизитыТЧ = Неопределено) Экспорт
	...
	Если Прейскурант = Неопределено Тогда
		...
		///////////////////////////////////////////////////////////////////////
		// ОМГ
		Если МетаданныеОбъекта.Имя = "ОказаниеУслуг" и ЗначениеЗаполнено(ЭтотОбъект.омг_СтраховаяПрограмма) Тогда
			Прейскурант = ЭтотОбъект.омг_СтраховаяПрограмма
		иначе
...
Функция СметаПоДокументу(Документ, ПараметрыСметы = Неопределено) Экспорт
	...
	Смета.Колонки.Добавить("Артикул", ТипСтрока);
	...
	Если ПараметрыСметы.Свойство("ПорядокКолонок") Тогда
		...
		Попытка
			...
			МассивПорядка = СтрРазделить(СтрЗаменить(ПараметрыСметы.ПорядокКолонок," ",""), ",", Ложь);
			
			// Свертка таблицы (удаление лишних колонок
			МассивСуммируемых = ОбщегоНазначенияБИТКлиентСервер.НовыйМассив("Количество", "Сумма", "СуммаСкидки");
			МассивСуммируемых = ОбщегоНазначенияБИТКлиентСервер.ПересечениеМассивов(МассивСуммируемых, МассивПорядка);
			МассивИзмерений = ОбщегоНазначенияКлиентСервер.РазностьМассивов(МассивПорядка, МассивСуммируемых);
			
			Смета.Свернуть(СтрСоединить(МассивИзмерений,","), СтрСоединить(МассивСуммируемых,","));
...
Функция ПолучитьСтавкуНДС(НастройкаПечатиЧеков, Номенклатура) Экспорт
	
	ОсновнаяСтавкаНДС = НастройкаПечатиЧеков.СтавкаНДСДляОсновнойСистемыНалогообложения;
	
	// Проверяем, не по пантенту ли эта номенклатура
	Если НастройкаПечатиЧеков.ЕстьУчетПоПатенту
		И ЗначениеЗаполнено(Номенклатура)
	Тогда
		ИсходныеТаблица = Новый ТаблицаЗначений; // См. также ОказаниеУслуг.ПолучитьТаблицуПродажДокументаДляЧеков().
		ИсходныеТаблица.Колонки.Добавить("Номенклатура");
		ИсходныеТаблица.Колонки.Добавить("ВидНоменклатуры");
		ИсходныеТаблица.Колонки.Добавить("ИмяТЧ");
		ИсходныеТаблица.Колонки.Добавить("НомерСтроки", Новый ОписаниеТипов("Число"));
		
		Позиция = ИсходныеТаблица.Добавить();
		Позиция.Номенклатура = Номенклатура;
		Позиция.ВидНоменклатуры = Номенклатура.ВидНоменклатуры;
		Позиция.ИмяТЧ = "КредитныеДанные";
		
		ТаблицаПатент = РаботаСНДССервер.ПолучитьПродажиСоответствующиеОтбору(ИсходныеТаблица, НастройкаПечатиЧеков);
		Если ТаблицаПатент.Количество() <> 0 Тогда
			
			СтавкаНДС = УчетНДСВызовСервера.ОсновныеСтавкиНДСпоСНО().Получить(ПредопределенноеЗначение("Перечисление.СистемыНалогообложения.ПатентнаяСистемаНалогообложения"));
	...
	СтавкаНДС = УчетНДСВызовСервера.СтавкаНДС(Номенклатура, ОбщегоНазначения.ТекущаяДатаПользователя());
	РегистрыСведений.СтавкиНДСПриУСН.СкорректироватьСтавкуТовараДляУСН(СтавкаНДС, ОсновнаяСтавкаНДС);
...
Функция ПроверитьПодготовитьДанныеПоМаркированнымТоварам(Ссылка, ТаблицаТоваров, ВидОперации) Экспорт
	...
	Если ВидОперации = Перечисления.ВидыОперацийОказанияУслуг.ПродажаКлиенту Тогда
		...
		ПланируемыйСтатусКМЧастично = Перечисления.ПланируемыйСтатусМаркируемогоТовара.МерныйТоварВСтадииРеализации;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийОказанияУслуг.ВозвратКлиенту Тогда
		...
		ПланируемыйСтатусКМЧастично = Перечисления.ПланируемыйСтатусМаркируемогоТовара.ЧастьТовараВозвращена;
	Иначе
		...
		Если ЗначениеЗаполнено(ШКТовара) Тогда
			...
			Если ДанныеШтрихкода.Разобран Тогда
				...
				Если ДанныеШтрихкода.ТипИдентификатораТовара = ПредопределенноеЗначение("Перечисление.ТипыИдентификаторовТовараККТ.КодТовараВФорматеDataMatrixGS1") Тогда
					...
					// Частичное выбытие маркированной продукции
					Если СтрокаТаблицыТоваров[25].Значение <> 0 Тогда
						
						// ставим принудительно 0, т.к. при частичном выбытии всегда должен быть 0
						ПараметрыКМ.МераКоличестваПредметаРасчета = 0;
						ПараметрыКМ.КодЕдиницыИзмерения = РаботаСТорговымОборудованием.КодЕдиницыИзмеренияПоМереКоличестваПредметаРасчета(ПредопределенноеЗначение("Перечисление.МераКоличестваПредметаРасчетаККТ.Штука"));
						ПараметрыКМ.ДробноеКоличество.Числитель = СтрокаТаблицыТоваров[24].Значение;
						ПараметрыКМ.ДробноеКоличество.Знаменатель = СтрокаТаблицыТоваров[25].Значение;
						ПараметрыКМ.Количество = 1;
						ПараметрыКМ.ПланируемыйСтатусТовара = ПланируемыйСтатусКМЧастично; 
						
						// добавляем в строку номенклатуры печатной формы чека информацию что частично продаем
						// заменяем количество на 1 по Приказу ФНС России от 14.09.2020 N ЕД-7-20/662@
						// подставляем сумму в цену
						СтрокаТаблицыТоваров[0].Значение = СтрокаТаблицыТоваров[0].Значение + " (" + СтрокаТаблицыТоваров[5].Значение + " шт., цена " + СтрокаТаблицыТоваров[4].Значение + " руб/шт.)";   
						СтрокаТаблицыТоваров[4].Значение = СтрокаТаблицыТоваров[4].Значение * СтрокаТаблицыТоваров[5].Значение;
						СтрокаТаблицыТоваров[5].Значение = 1;
						СтрокаТаблицыТоваров[18].Значение = СтрокаТаблицыТоваров[9].Значение;
...
Процедура ДокументыККМПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если Не Отказ Тогда 
		...
		Тогда
			Если РольДоступна("ПолныеПрава") или РольДоступна("омг_Регистратор") Тогда			
...
Процедура СвернутьДвиженияРегистра(НаборЗаписей, СуммируемыеКолонки = "") Экспорт
	...
	ДопСерверныеФункции.СвернутьТаблицуЗначений(ТаблицаДвижений, СуммируемыеКолонки);
...
Процедура ВыполнитьДвиженияПоРегиструОплаты(ДокументОбъект, ТаблицаОплат, СтруктураШапкиДокумента, Отказ) Экспорт
	...
	НаборДвижений.мПериод           = СтруктураШапкиДокумента.омг_ДатаОплаты;
	// ОМГ	
	////////////////////////////////////////////////////////////////////////////////
...
//
Функция ПолучитьДробнуюЧасть(Номенклатура, Количество, ЕдиницаИзмерения) Экспорт
	...
	Если ЗначениеЗаполнено(ЕдиницаИзмерения) И ДробнаяЧасть.Знаменатель > ЕдиницаИзмерения.Коэффициент Тогда
		ДробнаяЧасть.Числитель = ЕдиницаИзмерения.Коэффициент * Количество;
	Иначе
		ДробнаяЧасть.Числитель = 0;
		ДробнаяЧасть.Знаменатель = 0;
	КонецЕсли;
