...
Функция ПараметрыЭталонныхСтрокТаблиц(ШаблонПриема) Экспорт
	
	ПараметрыЭталонныхСтрокТаблиц = Новый СписокЗначений;
	
	СоответствияТаблицамШаблонов = ШаблонПриема.СоответствияТаблицамШаблонов.Выгрузить();
	Таблицы = СоответствияТаблицамШаблонов.Скопировать();
	Таблицы.Свернуть("ТаблицаШаблонов");
	Для Каждого Таблица Из Таблицы.ВыгрузитьКолонку("ТаблицаШаблонов") Цикл
		Если ЗначениеЗаполнено(Таблица) Тогда
			ПараметрыТаблицы = Новый Массив;
			Для Каждого СтрокаПараметра Из СоответствияТаблицамШаблонов.НайтиСтроки(Новый Структура("ТаблицаШаблонов", Таблица)) Цикл
				Если ЗначениеЗаполнено(СтрокаПараметра.Параметр) Тогда
					ПараметрыТаблицы.Добавить(СтрокаПараметра.Параметр);
				КонецЕсли;
			КонецЦикла;
			ПараметрыЭталонныхСтрокТаблиц.Добавить(Новый Структура("ИдентификаторТаблицы, ПараметрыЭталоннойСтроки", Строка(Таблица.УникальныйИдентификатор()), ПараметрыТаблицы));
		КонецЕсли;
	КонецЦикла;
