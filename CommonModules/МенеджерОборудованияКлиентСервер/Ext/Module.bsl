...
//
Функция ПараметрыПодключения(ДанныеДрайвера,Параметры = Неопределено) Экспорт
	
	Если ДанныеДрайвера = Неопределено и не Параметры = Неопределено Тогда	
	ДанныеДрайвераОборудования = МенеджерОборудованияВызовСервера.ПолучитьДанныеУстройства(Параметры.ИдентификаторУстройства);  
	ДанныеДрайвера = омг_WebKassa.ДанныеДрайвераОборудования(ДанныеДрайвераОборудования);
	КонецЕсли;
