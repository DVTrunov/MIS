...
Процедура ВыбратьЗначениеПараметраHTMLИнтерактивно(Форма, ИмяПоляШаблона, ЭлементHTML, СтрокаПараметра,
	...
	ПодсказкаВвода = ?(СтрокаПараметра.Свойство("НаименованиеДляВрача") И ЗначениеЗаполнено(СтрокаПараметра.НаименованиеДляВрача), СтрокаПараметра.НаименованиеДляВрача, Строка(СтрокаПараметра.Параметр));
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ДанныеОповещения, ДополнительныеПараметры);
	КонецЕсли;
	...
	Если ТегЭлемента = "A" Тогда
		...
		ИначеЕсли СтрокаПараметра.Свойство("ДеревоЗначенийВыбора")
			И (ТипЗнч(ДеревоЗначенийВыбора) = Тип("ПеречислениеСсылка.КодировкаПолейЭМД")
				Или ТипЗнч(ДеревоЗначенийВыбора) = Тип("СправочникСсылка.КлассификаторыМинЗдрава")) // Классификатор "Кодировка полей CDA документов" или "Витальные параметры".
		Тогда
			...
			ПараметрыВвода = Новый Структура("НачальноеЗначение, РедактируемыйПараметрШаблона", НачальноеЗначение, СтрокаПараметра.Параметр);
			МедицинскаяДеятельностьКлиент.ВвестиЗначениеКодируемогоПоля(ОповещениеОВыборе, Форма, ЭлементHTML, ДеревоЗначенийВыбора, ПараметрыВвода, ДополнительныеПараметрыВвода);
			
		ИначеЕсли ТипЗначенияПараметра.Типы().Количество() > 1 Тогда
			
			СписокТипов = Новый СписокЗначений;
			СписокТипов.ЗагрузитьЗначения(ТипЗначенияПараметра.Типы());
			ДопПараметры = Новый Структура("Клиент, Форма, ОповещениеОВыборе, НачальноеЗначение, Параметр, ЭтотОбъект, ПервичныйПрием, ТипЗначенияПараметра", 
					ОбъектДокумента.Клиент, Форма, ОповещениеОВыборе, НачальноеЗначение, СтрокаПараметра.Параметр, ЭтотОбъект, ПервичныйПрием, ТипЗначенияПараметра);
			СписокТипов.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ОбработкаВыбораСоставногоТипа", РаботаСШаблонамиHTMLКлиент, ДопПараметры), НСтр("ru='Выбор типа данных'"));
	...
	ИначеЕсли ТегЭлемента = "TEXTAREA"
		...
		Или ТегЭлемента = "SELECT"
	Тогда
		
		ЗначениеПараметра = Неопределено;
		Если ПолеВводаИзменено(ЭлементHTML) Тогда
			Если ТегЭлемента = "TEXTAREA"
				Или ТегЭлемента = "INPUT"
			Тогда
				ЗначениеПараметра = ЭлементHTML.value;
				Если СтрокаПараметра.ТипЗначения.СодержитТип(Тип("Число")) Тогда
					ЗначениеПараметра = СтрокаПараметра.ТипЗначения.ПривестиЗначение(СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ЗначениеПараметра));
				КонецЕсли;
...
Процедура ВыбораЗначенияПараметраHTMLЗавершение(Значение, ДополнительныеПараметры) Экспорт
	...
	СтрокиПараметров = Новый Массив; // Строки изменённых параметров.
	...
	// Определение ДокументHTML, на котором произошло редактирование
	...
	// Кеш для УстановитьЗначениеПредставлениеПараметраHTMLИнтерактивно()
	КешЗначенийВыбора = Неопределено;
	ДополнительныеПараметры.Свойство("КешЗначенийВыбора", КешЗначенийВыбора);
	
	// Обновляем значение параметра шаблона (значения, если параметров несколько).
	СтрокаПараметраИсх = ДополнительныеПараметры.СтрокаПараметра; // Строка того параметра, который непосредственно начали изменять.
	СтрокиИзмененныхПараметров = Новый Массив;
	...
	Если ТипЗнч(ВыбранноеЗначение) <> Тип("Соответствие") Тогда
		// Обычный выбор для одного параметра
		СтрокаПараметра = СтрокаПараметраИсх;
		...
		СтрокиИзмененныхПараметров.Добавить(СтрокаПараметра);
	Иначе
		// Выбраны значения сразу нескольких параметров, Значение в виде соответствия.
		Для Каждого кзЗначениеПараметра Из ВыбранноеЗначение Цикл
			// Определение строки шаблона
			Отбор = ОтборПоискаПараметровСтрокиВТаблицеПараметров(СтрокаПараметраИсх);
			Отбор.Параметр = кзЗначениеПараметра.Ключ;
			СтрокаПараметра = Форма.ТаблицаПараметров.НайтиСтроки(Отбор)[0];
			СтрокиИзмененныхПараметров.Добавить(СтрокаПараметра);
			
			// Установка нового значения
			СтрокаПараметра.Значение = кзЗначениеПараметра.Значение;
			СтрокаПараметра.Представление = РаботаСШаблонамиHTMLКлиентСервер.ПолучитьПредставлениеПараметра(СтрокаПараметра);
			// Обновляем все элементы шаблона, связанные с параметром
			УстановитьЗначениеПредставлениеПараметраHTMLИнтерактивно(СтрокаПараметра, ДокументHTML, КешЗначенийВыбора);
		КонецЦикла;
	...
	СтрокиИмеющиеЗависимыеПараметры = Новый Массив;
	Попытка
		Для Каждого СтрокаПараметра Из СтрокиИзмененныхПараметров Цикл
			ЗависимыеПараметры = Форма.КешЗависимыхПараметров.НайтиСтроки(Новый Структура("Параметр", СтрокаПараметра.Параметр));
			Если ЗависимыеПараметры.Количество() <> 0 Тогда
				СтрокиИмеющиеЗависимыеПараметры.Добавить(СтрокаПараметра);
			КонецЕсли;
		КонецЦикла;
	Исключение // У формы нет таблицы зависимых параметров
	КонецПопытки;
	
	Для Каждого СтрокаПараметра Из СтрокиИмеющиеЗависимыеПараметры Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаПараметра, "ИдентификаторСтроки") Тогда
			ДополнительныйОтборПараметров.Вставить("ИдентификаторСтроки", СтрокаПараметра.ИдентификаторСтроки);
		КонецЕсли;
		...
		Для Каждого СтрокаПараметраЗависимого Из ЗначенияЗависимыхПараметров Цикл
			УстановитьЗначениеПредставлениеПараметраHTMLИнтерактивно(СтрокаПараметраЗависимого, ДокументHTML, КешЗначенийВыбора);
		КонецЦикла;
		
	КонецЦикла;
	
	// Зависимые через назначение параметра
	Для Каждого СтрокаПараметра Из СтрокиИзмененныхПараметров Цикл
...
Функция ДополнительныеПараметрыВводаКодируемогоПоля(СтрокаПараметра, НазначениеПараметра, Форма)
	...
	// Передача значений параметров, от которых зависит вводимый параметр.
	...
	Если СтрокиЗависмыхПараметров.Количество() > 0 Тогда
		...
		Отбор = ОтборПоискаПараметровСтрокиВТаблицеПараметров(СтрокаПараметра);
		
		Для Каждого СтрокаЗависмыхПараметров Из СтрокиЗависмыхПараметров Цикл
			ДополнительныеПараметрыВводаКодируемогоПоля_ДобавитьСвязанныйПараметрЕслиПредставленВТаблицеПараметров(СтрокаЗависмыхПараметров.Параметр, Форма.ТаблицаПараметров, Отбор, СвязанныеПараметрыШаблона);
		КонецЦикла;
	КонецЕсли;
	
	// Параметры, связанные по назначениям для форм частных интерфейсов ввода. Например, форма "АртериальноеДавление" и систолическое + диастоличесткое давление.
	СвязанныеНазначенияПараметровШаблонов = МедицинскаяДеятельностьПовтИсп.СвязиНазначенийПараметровШаблонов();
	СвязанныеДляЭтогоНазначения = СвязанныеНазначенияПараметровШаблонов.Получить(НазначениеПараметра);
	Если ТипЗнч(СвязанныеДляЭтогоНазначения) = Тип("Массив") Тогда
		
		Если Отбор = Неопределено Тогда
			Отбор = ОтборПоискаПараметровСтрокиВТаблицеПараметров(СтрокаПараметра);
		КонецЕсли;
		
		Для Каждого СвязанноеНазначение Из СвязанныеДляЭтогоНазначения Цикл
			ПараметрыШаблоновСвязанногоНазначения = глКешПараметрыШаблоновСвязанныхНазначенийПараметров.Получить(СвязанноеНазначение);
			Если ПараметрыШаблоновСвязанногоНазначения <> Неопределено Тогда
				Для Каждого КадидатСвязанныйПараметрШаблона Из ПараметрыШаблоновСвязанногоНазначения Цикл
					ДополнительныеПараметрыВводаКодируемогоПоля_ДобавитьСвязанныйПараметрЕслиПредставленВТаблицеПараметров(КадидатСвязанныйПараметрШаблона, Форма.ТаблицаПараметров, Отбор, СвязанныеПараметрыШаблона);
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
...
Процедура ДополнительныеПараметрыВводаКодируемогоПоля_ДобавитьСвязанныйПараметрЕслиПредставленВТаблицеПараметров(КадидатПараметрШаблона, ТаблицаПараметров, Отбор, СвязанныеПараметрыШаблона)

	Отбор.Параметр = КадидатПараметрШаблона;
	СтрокиПараметров = ТаблицаПараметров.НайтиСтроки(Отбор);
	Если СтрокиПараметров.Количество() > 0 Тогда
		Для Каждого СтрокаДопПараметра Из СтрокиПараметров Цикл
			СвязанныеПараметрыШаблона.Вставить(СтрокаДопПараметра.Параметр, СтрокаДопПараметра.Значение); 		
		КонецЦикла;
	КонецЕсли;
	
КонецПроцедуры

Функция ОтборПоискаПараметровСтрокиВТаблицеПараметров(СтрокаПараметра)
...
Процедура ОбновитьЗначенияЗависимыхПараметровТаблицШаблонов(ЗначенияЗависимыхПараметровТаблицШаблонов, Форма, ИдентификаторСтрокиШаблона, ДокументHTML)
	...
	Для Каждого СтрокаПараметра Из ЗначенияЗависимыхПараметровТаблицШаблонов Цикл
		...
		Если МассивОбновленныхТаблиц.Найти(СтрокаПараметра.ИдентификаторТаблицы) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Таблица = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML, СтрокаПараметра.ИдентификаторТаблицы, , "data-bit-dyntable-id", Истина);
		Если Таблица <> Неопределено Тогда
			НайденныеЭлементы = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(Таблица, СтрокаПараметра.Параметр.УникальныйИдентификатор());
			ЭлементыТаблицы = РаботаСDOMКлиентСервер.ПреобразоватьОбходимуюКоллекциюВМассив(НайденныеЭлементы);
			Если ЭлементыТаблицы.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
			ПараметрыТаблицы = Новый Массив;
			ДополнитьПараметрамиТаблицы(ПараметрыТаблицы, Таблица, Форма.ТаблицаПараметров, ИдентификаторСтрокиШаблона);
...
Функция ПолеВводаИзменено(ЭлементHTML) Экспорт
	...
	Возврат ЭлементHTML.hasAttribute("data-bit-isOnChange");
...
Процедура УстановитьПризнакИзмененияПоляВвода(ЭлементHTML, Изменено = Истина) Экспорт
	...
	Если Изменено Тогда
		ЭлементHTML.setAttribute("data-bit-isOnChange", "true");
	Иначе
		ЭлементHTML.removeAttribute("data-bit-isOnChange");
	КонецЕсли;
...
Процедура КнопкаШаблонаНажатие(Форма, ЭлементHTMLКнопка, ИдентификаторСтрокиШаблона, соотвКешИзображенийШаблонов) Экспорт
	...
	Если ЗначениеЗаполнено(ИдентификаторИнтерфейсаЗаполненияПараметров) Тогда
		РаботаСИнтерфейсамиЗаполненияПараметровКлиент.ОбработатьНажатиеКнопкиОткрытияИнтерфейсаЗаполненияПараметров(ЭлементHTMLКнопка, ИдентификаторИнтерфейсаЗаполненияПараметров, Форма, ИдентификаторСтрокиШаблона);
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьНажатиеКнопкиРедактированияТаблицы(Форма, ИдентификаторСтрокиШаблона, ИдентификаторТаблицы)
	...
	ЭлементРедактируемойТаблицы = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML, ИдентификаторТаблицы,, "data-bit-dyntable-id", Истина);
	
	Если ЭлементРедактируемойТаблицы <> Неопределено Тогда
		...
		ВложенныеТаблицы = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ЭлементРедактируемойТаблицы,, "table", "data-bit-dyntable-id");
...
Процедура ОбработатьНажатиеКнопкиРедактированияВложеннойТаблицы(Форма, ИдентификаторСтрокиШаблона, ИдентификаторТаблицы, ИдентификаторСтроки) Экспорт
	...
	ДокументHTML = РаботаСDOMКлиентСервер.СоздатьДокументDOM(Форма.ТекстШаблона);
	ЭлементРедактируемойТаблицы = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML, ИдентификаторТаблицы, , "data-bit-dyntable-id", Истина);
	...
	ДопустимыеСтрокиПараметров = Новый Массив;
	ДополнитьДопустимымиСтрокамиПараметров(ДопустимыеСтрокиПараметров, Форма.ТаблицаПараметров, ИдентификаторСтроки);
	
	ДополнитьПараметрамиТаблицы(ПараметрыТаблицы, ЭлементРедактируемойТаблицы, Форма.ТаблицаПараметров, ИдентификаторСтрокиШаблона, ДопустимыеСтрокиПараметров);
	...
	Параметры.Вставить("МассивДанныхТаблицы",				МассивДанныхТаблицы);
	Параметры.Вставить("ИдентификаторТаблицыШаблонов",		ИдентификаторТаблицы);
	Параметры.Вставить("ТекстШаблона",						Форма.ТекстШаблона);
	Параметры.Вставить("КешЗначенийВыбора",					Форма.КешЗначенийВыбора);
	Параметры.Вставить("ИдентификаторСтрокиШаблона",		ИдентификаторСтрокиШаблона);
	Параметры.Вставить("КешЗависимыхПараметров",			Форма.КешЗависимыхПараметров);
	...
	Параметры.Вставить("ЭталонныеСтрокиТаблиц",				Форма.ЭталонныеСтрокиТаблиц);
...
Процедура РедактированиеВложеннойТаблицыШаблоновДокументаЗавершение(Результат, Параметры, ПолеHTMLОсмотр, ПараметрыТаблицШаблонов, ИдентификаторРодительскойСтроки, ТекстШаблона, ЭталонныеСтрокиТаблиц) Экспорт
	...
	ТаблицаИзШаблона = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(РаботаСDOMКлиентСервер.СоздатьДокументDOM(ТекстШаблона), ИдентификаторТаблицы, , "data-bit-dyntable-id", Истина);
	...
	РаботаСШаблонамиHTMLКлиентСервер.ЗаполнитьЭлементТаблицыШаблоновДокументаHTML(ПараметрыТаблицШаблонов, ТаблицаРезультат, ЭталонныеСтрокиТаблиц, Истина, ИдентификаторСтрокиШаблона, , ПараметрыТаблицы, ИдентификаторСтроки);
	...
	ЭлементСтроки = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML, ИдентификаторСтроки, "tr", "data-bit-dyntable-tr-id", Истина);
	ЭлементТаблицы = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ЭлементСтроки, ИдентификаторТаблицы, "table", "data-bit-dyntable-id", Истина);
...
Функция ПараметрНеэталоннойСтрокиМногострочнойТаблицыВерхнегоУровня(ЭлементТаблицы, ИдентификаторТаблицы, ИдентификаторТаблицыПараметра) Экспорт
	...
	ЭлементТаблицыПараметра = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ЭлементТаблицы, ИдентификаторТаблицыПараметра, "table", "data-bit-dyntable-id", Истина);
	
	Если ЭлементТаблицыПараметра = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
...
Асинх Функция ОбработатьНажатиеКнопкиИзмененияИзображения(УникальныйИдентификатор, Кнопка, ДанныеФормыПриема = Неопределено) Экспорт
	...
	Если УдалениеИзображения Тогда
		...
		Если ДанныеФормыПриема = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
		УдалитьВыбранноеИзображение(ДанныеФормыПриема, Контейнер);
	Иначе
		Если ДанныеФормыПриема = Неопределено Тогда
		...
		Если Ответ = "ФайлНаДиске" Тогда
			ДобавитьВыбранноеИзображение(Ждать ВыборФайлаНаДиске(Контейнер, УникальныйИдентификатор), ДанныеФормыПриема, Контейнер);
		ИначеЕсли Ответ = "ПрикрепленныйФайл" Тогда
			ВыборПрикрепленногоФайла(УникальныйИдентификатор, ДанныеФормыПриема, Контейнер);
		КонецЕсли;
	КонецЕсли;
	
КонецФункции

Процедура ОбработатьИзмененныеПоляВвода(ДокументHTML, Форма, ИмяЭлементаHTML, ИдентификаторСтрокиШаблона, ТаблицаПараметров, КешЗначенийВыбора, Объект = Неопределено, ДанныеТаблицы = Неопределено) Экспорт
	
	Для Каждого ИзмененныйЭлемент Из РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML,,,"data-bit-isOnChange") Цикл
		Если ПолеВводаИзменено(ИзмененныйЭлемент) Тогда
			
			ДополнительныеПараметры = Неопределено;
			СтруктураПоискаПараметра = Новый Структура("СтрокаУидПараметра, ИдентификаторСтрокиШаблона", ИзмененныйЭлемент.id, ИдентификаторСтрокиШаблона);
			Если ДанныеТаблицы <> Неопределено Тогда
				
				СтрокаПараметра = ПолучитьТекущуюСтрокуМногострочнойТаблицы(ИзмененныйЭлемент, ДанныеТаблицы.ИдентификаторТаблицыШаблонов);
				ИндексСтрокиПараметра = Число(СтрокаПараметра.rowIndex);
				
				ДополнительныеПараметры = Новый Структура("ИндексСтрокиМногострочнойТаблицы", ИндексСтрокиПараметра);
				СтруктураПоискаПараметра.Вставить("НомерСтрокиТаблицы", ИндексСтрокиПараметра - ДанныеТаблицы.ИндексЭталоннойСтроки);
			КонецЕсли;
			
			мсСтрокаПараметра = ТаблицаПараметров.НайтиСтроки(СтруктураПоискаПараметра);
			Если мсСтрокаПараметра.Количество() > 0 Тогда
				ВыбратьЗначениеПараметраHTMLИнтерактивно(Форма, ИмяЭлементаHTML, ИзмененныйЭлемент, мсСтрокаПараметра[0], КешЗначенийВыбора, Объект,, ДополнительныеПараметры);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьТекущуюСтрокуМногострочнойТаблицы(ЭлементHTML, ИдентификаторТаблицыШаблонов) Экспорт
	...
	Если ТаблицаТекущейСтроки.getAttribute("data-bit-dyntable-id") = ИдентификаторТаблицыШаблонов Тогда
		Возврат ТекущаяСтрокаТаблицы;
	Иначе
		Возврат ПолучитьТекущуюСтрокуМногострочнойТаблицы(ТаблицаТекущейСтроки.parentNode, ИдентификаторТаблицыШаблонов);
	КонецЕсли;
...
Процедура ДополнитьПараметрамиТаблицы(ПараметрыТаблицы, Таблица, ТаблицаПараметров, ИдентификаторСтрокиШаблона, ДопустимыеСтрокиПараметров = Неопределено)
	...
	Для Каждого СтрокаПараметра Из ТаблицаПараметров.НайтиСтроки(Новый Структура("ИдентификаторСтрокиШаблона", ИдентификаторСтрокиШаблона)) Цикл
		Если ДопустимыеСтрокиПараметров <> Неопределено
			И ДопустимыеСтрокиПараметров.Найти(СтрокаПараметра) = Неопределено
		Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОбработкаВыбораСоставногоТипа(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	
	Если ВыбранныйЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	// Очищаем начальное значение если другой тип
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДополнительныеПараметры, "НачальноеЗначение") 
		И ТипЗнч(ДополнительныеПараметры.НачальноеЗначение) = ВыбранныйЭлемент.Значение 
	Тогда
	...
	ИначеЕсли ВыбранныйЭлемент.Значение = Тип("Строка") Тогда
		МедицинскаяДеятельностьКлиент.ПоказатьФормуВводаТекста(Строка(НачальноеЗначение), ДополнительныеПараметры.Параметр, 
				ДополнительныеПараметры.ОповещениеОВыборе, ДополнительныеПараметры.ЭтотОбъект, ДополнительныеПараметры.ПервичныйПрием);
	ИначеЕсли ВыбранныйЭлемент.Значение = Тип("СправочникСсылка.НоменклатураМедицинскихУслуг") Тогда
		ПоказатьВводЗначения(ДополнительныеПараметры.ОповещениеОВыборе, НачальноеЗначение, "СправочникСсылка.НоменклатураМедицинскихУслуг", Тип("СправочникСсылка.НоменклатураМедицинскихУслуг"));
	КонецЕсли;
...
Процедура СкорректироватьДанныеЗначенияВитальногоПараметра(Форма, СтрокаПараметра, СтандартнаяОбработка, НачальноеЗначение, ТипЗначенияПараметра, ДеревоЗначенийВыбора, ВидКлассификатора)
	...
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаПараметра, "ИдентификаторСтроки") Тогда
		ОтборПараметра.Вставить("ИдентификаторСтроки", СтрокаПараметра.ИдентификаторСтроки);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ЗначениеПараметраОпределяющегоТипЗначения) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru='Параметр ""%1"" не заполнен'"), СтрокиПараметраОпределяющегоТипЗначения[0].НаименованиеДляВрача));
		Возврат;
	КонецЕсли;
...
Процедура ВыборПрикрепленногоФайла(УникальныйИдентификатор, ДанныеФормыПриема, Контейнер)
	...
	ДополнительныеПараметры = Новый Структура("ДанныеФормыПриема, Контейнер, УникальныйИдентификатор", ДанныеФормыПриема, Контейнер, УникальныйИдентификатор);
...
Процедура ПослеВыбораПрикрепленногоФайла(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ДобавитьВыбранноеИзображение(ДанныеВыбранногоИзображения, ДополнительныеПараметры.ДанныеФормыПриема, ДополнительныеПараметры.Контейнер);
	КонецЕсли;
...
Функция ДобавитьВыбранноеИзображение(ДанныеВыбранногоИзображения, ДанныеФормыПриема, Контейнер)
	...
	УдалитьРисункиИзображения(ДанныеФормыПриема, Изображение);
	
	ДанныеФормыПриема.Форма.Модифицированность = Истина;
	
КонецФункции

Процедура УдалитьВыбранноеИзображение(ДанныеФормыПриема, Контейнер)
...
Процедура УдалитьРисункиИзображения(ДанныеФормыПриема, Изображение)
...
Функция ДополнитьДопустимымиСтрокамиПараметров(ДопустимыеСтрокиПараметров, ТаблицаПараметров, ИдентификаторСтроки)
	
	Для Каждого СтрокаТаблицы Из ТаблицаПараметров.НайтиСтроки(Новый Структура("ИдентификаторСтроки", ИдентификаторСтроки)) Цикл
		
		ДопустимыеСтрокиПараметров.Добавить(СтрокаТаблицы);
		
		ИдентификаторыДочернихСтрок = Новый Массив;
		Для Каждого ДочерняяСтрока Из ТаблицаПараметров.НайтиСтроки(Новый Структура("ИдентификаторРодительскойСтроки", ИдентификаторСтроки)) Цикл
			Если ИдентификаторыДочернихСтрок.Найти(ДочерняяСтрока.ИдентификаторСтроки) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			ДополнитьДопустимымиСтрокамиПараметров(ДопустимыеСтрокиПараметров, ТаблицаПараметров, ДочерняяСтрока.ИдентификаторСтроки);
			ИдентификаторыДочернихСтрок.Добавить(ДочерняяСтрока.ИдентификаторСтроки);
		КонецЦикла;
	КонецЦикла;
