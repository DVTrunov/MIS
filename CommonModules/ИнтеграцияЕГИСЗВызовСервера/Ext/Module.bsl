...
Функция ИмеютсяОшибкиВалидацииПоДокументу(МедицинскийДокумент) Экспорт
	...
	ДолжностиСотрудника = ДолжностиМЗ(СотрудникУстановившийПодпись);
...
Функция ПолучитьСписокLocalidПоВидуДокумента(ВидДокумента) Экспорт
	...
	Если ЗначениеЗаполнено(МакетXML) Тогда
		Возврат ИнтеграцияЕГИСЗ_СЭМД.СписокLocalidМакета(МакетXML,, Истина);
	Иначе
...
Функция ДолжностиМЗ(Сотрудник) Экспорт
	...
	ДолжностиСотрудникаПоКлассификаторам = ДолжностиСотрудникаПоКлассификаторам(Сотрудник);
	Возврат ДолжностиСотрудникаПоКлассификаторам.ВыгрузитьКолонку("ДолжностьМинздрава");
	
КонецФункции

Функция ДолжностиСотрудникаПоКлассификаторам(Сотрудник) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Должности.ДолжностьМинздрава КАК ДолжностьМинздрава,
		|	Должности.ДолжностьФРМР КАК ДолжностьФРМР,
		|	0 КАК Приоритет
		|ИЗ
		...
		|	ДолжностиМинздрава.ДолжностьФРМР,
		|	1
		|ИЗ
...
Функция ПолучитьСтруктуруТипаЗначенияВитальногоПараметра(ВитальныйПараметр) Экспорт
	...
	Тогда
		...
		ЗначениеТипа = ИнтеграцияЕГИСЗСервер.ПолучитьЗначениеАтрибутаЭлементаКлассификатора(ВитальныйПараметр, "DATA_TYPE");
		Если ЗначениеТипа <> Неопределено Тогда
			СтруктураТипа.Тип = ИнтеграцияЕГИСЗ_СЭМД.ТипДанныхИзXML(ЗначениеТипа);
		КонецЕсли;
		
		ЗначениеАтрибутаВидаКлассификатора = ИнтеграцияЕГИСЗСервер.ПолучитьЗначениеАтрибутаЭлементаКлассификатора(ВитальныйПараметр, "OID");
		Если ЗначениеАтрибутаВидаКлассификатора <> 0 Тогда
			ПараметрыСправочника = ИнтеграцияЕГИСЗСерверПовтИсп.ПолучитьПараметрыСправочникаЕГИСЗ(ЗначениеАтрибутаВидаКлассификатора);
...
Функция ПолучитьДолжностьФРМРПоДолжностиМЗ(ДолжностьМинздрава, ВернутьПервое = Истина) Экспорт
	
	Возврат ИнтеграцияЕГИСЗСервер.ПолучитьЭлементКлассификатораПоЗначениюАтрибута("federal_code", ДолжностьМинздрава.УИДЕГИСЗ, Перечисления.ВидыКлассификаторовМинЗдрава.ФРМРДолжностиМедицинскогоПерсонала,, ВернутьПервое);
	
КонецФункции

Функция ДополнительныеПараметрыДляРаботыССЭМД() Экспорт
	
	Возврат ИнтеграцияЕГИСЗ_СЭМД.ДополнительныеПараметрыДляРаботыССЭМД();
	
КонецФункции

Функция ЭлементКлассификатораПоID(УИДЕГИСЗ, ИмяСправочника, ВидКлассификатора = Неопределено) Экспорт
	
	Возврат ИнтеграцияЕГИСЗСервер.ЭлементКлассификатораПоID(УИДЕГИСЗ, ИмяСправочника, ВидКлассификатора);
	
КонецФункции

Функция ПолучитьЛьготнуюКатегориюПоКатегорииЛьгот(КатегорияЛьготы) Экспорт
	
	СвязьКлассификаторовЛьгот = СвязьИдентификаторовКлассификаторовЛьгот();
	
	ИдентификаторЛьготнойКатегории = СвязьКлассификаторовЛьгот.Получить(КатегорияЛьготы.УИДЕГИСЗ);
	Если ИдентификаторЛьготнойКатегории = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат ИнтеграцияЕГИСЗОбщегоНазначенияПовтИсп.ЭлементКлассификатораПоУИДЕГИСЗ(ИдентификаторЛьготнойКатегории, Перечисления.ВидыКлассификаторовМинЗдрава.ЛьготныеКатегорииГраждан);
...
// Связь между идентификаторами элементов классификатора 1.2.643.5.1.13.13.99.2.457 и 1.2.643.5.1.13.13.99.2.541
// 
// Возвращаемое значение:
//  Соответствие - Ключ: УИДЕГИСЗ 1.2.643.5.1.13.13.99.2.457; Значение: УИДЕГИСЗ 1.2.643.5.1.13.13.99.2.541
//
Функция СвязьИдентификаторовКлассификаторовЛьгот()
	
	СвязьКлассификаторов = Новый Соответствие;
	
	СвязьКлассификаторов.Вставить(2, 1000000001);
	СвязьКлассификаторов.Вставить(3, 1000000009);
	СвязьКлассификаторов.Вставить(4, 1000000215);
	СвязьКлассификаторов.Вставить(6, 1000000026);
	СвязьКлассификаторов.Вставить(7, 1000000247);
	СвязьКлассификаторов.Вставить(8, 1000000210);
	СвязьКлассификаторов.Вставить(10, 1000000033);
	
	Возврат СвязьКлассификаторов;
