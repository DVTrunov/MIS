...
Функция НормализоватьОтступы(ИсходныйТекстHTML) Экспорт
	//Добавлено Трунов. Обход ограничений веб клиента
	  #Если ВебКлиент Тогда
        // Для браузера пробрасываем выполнение на сервер
        Возврат РаботаСШаблонамиHTMLПовтИсп.НормализоватьОтступыНаСервере(ИсходныйТекстHTML);
    #Иначе
	//
...
Функция ДекодироватьИзображенияHTMLТекста(ТекстHTML, СписокИзображений, ДанныеРисунков = Неопределено) Экспорт
    //Изменено Трунов. Ограничения веб клиента
	#Если ВебКлиент Тогда
		Возврат ТекстHTML;
	#Иначе
...
Процедура ДобавитьКнопкиСворачиваемости(ДокументHTML, ТаблицаПараметров)
	...
	Для Каждого ИдентификаторЭлемента Из СворачиваемыеЭлементы Цикл
		...
		Если Элемент.Атрибуты.ПолучитьИменованныйЭлемент("data-bit-collapse").Значение = "collapse" Тогда
			...
			//Изменено Трунов
			РаботаСШаблонамиHTMLПовтИсп.ПолучитьКоличествоЗаполненныхПараметров(Элемент, ТаблицаПараметров, КоличествоЗаполненныхПараметров); 
...
Функция ПреобразоватьHTMLвPDF(Знач HtmlНаПреобразование, ВыполнениеНаLinux, ВернутьПуть, АдресWkhtmltopdfНаКлиенте = Неопределено, РежимPDF_A = Ложь, ОбязателенPDF_A = Ложь) Экспорт
	//Изменено Трунов
	#Если ВебКлиент Тогда
	// В браузере запрещена работа с локальными файлами и запуск сторонних EXE-файлов (wkhtmltopdf).
	// Генерация PDF должна быть принудительно вызвана на стороне сервера!
	Возврат Неопределено;
#Иначе
	Попытка
