 ...
 Функция ПараметрыОперацииФискализацииЧека(Реквизиты, ДанныеПечатиЧека, ВерсияФормата, СообщениеОбОшибке) Экспорт
	...
	Если ИтогоСуммаЧеков <> Реквизиты.СуммаДокумента И ПозицииЧека.Количество() = 0  Тогда	
		...
		// ФИКС: Цена и Количество убраны из Свернуть - в платежных документах
		// (ОплатаПлатежнойКартой/ВозвратПоПлатежнойКарте/ПКО/РКО) РасшифровкаПлатежа
		// таких колонок не содержит (только суммы), в отличие от ОказаниеУслуг,
		// где есть построчные Цена и Скидки в табличной части "Работы" (но это
		// не эта таблица и не эта ветка кода).
		РасшифровкаПлатежа.Свернуть("СтавкаНДС, ССЫЛКА", "СуммаПлатежа, СуммаВзаиморасчетов, СуммаНДС");
		...
		Для Каждого ОплачиваемыйДокумент  Из РасшифровкаПлатежа Цикл
			...
			// ФИКС: Количество/Цена больше не берутся из ОплачиваемыйДокумент (там
			// таких колонок нет) - Количество=1, Цена=СуммаПлатежа (документ оплаты
			// не позиционный, одна строка на всю сумму платежа).
