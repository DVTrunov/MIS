...
Процедура ОчиститьНекорректныеСопоставления(ТаблицаLocalidМакета, ТаблицаКонстант, ТаблицаПараметров, СоответствияТаблицамШаблонов, ВидШаблона) Экспорт
	...
	НекорректныеТипыДанных = Ошибки.Получить("НекорректныеТипыДанных");
	Если НекорректныеТипыДанных <> Неопределено Тогда
		Для Каждого СтруктураОшибки Из НекорректныеТипыДанных.Ошибки Цикл
			Если ТаблицаКонстант.Индекс(СтруктураОшибки.СтрокаТаблицы) >= 0 Тогда
				ТаблицаКонстант.Удалить(СтруктураОшибки.СтрокаТаблицы);
				УстановитьФлагИспользованДляLocalid(СтруктураОшибки.СтрокаТаблицы.localid, Ложь, ТаблицаLocalidМакета);
				Продолжить;
			КонецЕсли;
			
			СтруктураОшибки.СтрокаТаблицы.localid = "";
			СтруктураОшибки.СтрокаТаблицы.localidПредставление = "";
			УстановитьФлагИспользованДляLocalid(СтруктураОшибки.СтрокаТаблицы.localid, Ложь, ТаблицаLocalidМакета);
		КонецЦикла;
	КонецЕсли;
	
	НесуществующиеГрупПоля = Ошибки.Получить("НесуществующиеГрупПоля");
	Если НесуществующиеГрупПоля <> Неопределено Тогда
		Для Каждого СтруктураОшибки Из НесуществующиеГрупПоля.Ошибки Цикл
			СтруктураОшибки.СтрокаТаблицы.ГруппирующееПоле = Неопределено;
			СтруктураОшибки.СтрокаТаблицы.ГруппирующееПолеПредставление = Неопределено;
		КонецЦикла;
	КонецЕсли;
...
Процедура ПолеТаблицыПриИзмененииПредставления(Элемент, ТаблицаLocalidМакета) Экспорт
	...
	ТекДанные = ТаблицаРодительЭлемента(Элемент).ТекущиеДанные;
...
Процедура ГруппирующееПолеПредставлениеНачалоВыбора(Элемент, ТаблицаПараметров, ТаблицаLocalidМакета, ТаблицаКонстант) Экспорт
	...
	ТекСтрока = ТаблицаРодительЭлемента(Элемент).ТекущиеДанные;
...
Процедура КлассификацияПоляНачалоВыбора(Элемент, СтандартнаяОбработка, ТаблицаLocalidМакета, ТаблицаПараметров) Экспорт
	...
	ТекущиеДанные = ТаблицаРодительЭлемента(Элемент).ТекущиеДанные;
...
Процедура ЗаполнитьСписокВыбораLocalid(Элемент, ТаблицаLocalidМакета) Экспорт
	...
	Элемент.СписокВыбора.Очистить();
	ТекущаяСтрока = ТаблицаРодительЭлемента(Элемент).ТекущиеДанные;
...
Функция ТаблицаРодительЭлемента(Элемент) Экспорт
	
	Если ТипЗнч(Элемент) = Тип("ТаблицаФормы") Тогда
		Возврат Элемент;
	Иначе
		Возврат ТаблицаРодительЭлемента(Элемент.Родитель);
	КонецЕсли;
...
Процедура ДополнитьНесуществующимиПолямиЭМД(Ошибки, ТаблицаLocalidМакета, ТаблицаКонстант, ТаблицаПараметров, СоответствияТаблицамШаблонов, ВидШаблона)
	...
	Если ВидШаблона <> ПредопределенноеЗначение("Перечисление.ВидыШаблонов.Обработка") Тогда
		ПроверитьСуществованиеПолейЭМДПараметровТаблицы(ТаблицаПараметров, НесуществующиеПоляЭМД, ТаблицаLocalidМакета);
	Иначе
		ПроверитьСуществованиеПолейЭМДПараметровТаблицы(ТаблицаПараметров, НесуществующиеПоляЭМД, ТаблицаLocalidМакета);
		ПроверитьСуществованиеПолейЭМДПараметровТаблицы(СоответствияТаблицамШаблонов, НесуществующиеПоляЭМД, ТаблицаLocalidМакета);
	КонецЕсли;
...
Процедура ПроверитьСуществованиеПолейЭМДПараметровТаблицы(ПроверяемаяТаблица, НесуществующиеПоляЭМД, ТаблицаLocalidМакета)
...
Процедура ДополнитьНекорректнымиТипамиПараметров(Ошибки, ТаблицаLocalidМакета, ТаблицаКонстант, ТаблицаПараметров, СоответствияТаблицамШаблонов, ВидШаблона)
	
	СтрокиНекорректныхТипов = Новый Соответствие;
	
	Для Каждого СтрокаКонстанты Из ТаблицаКонстант.НайтиСтроки(Новый Структура("Вычисляется", Ложь)) Цикл
		Если Не ЗначениеЗаполнено(СтрокаКонстанты.localid) Тогда
			Продолжить;
		КонецЕсли;
		...
		СтрокиLocalid = ТаблицаLocalidМакета.НайтиСтроки(Новый Структура("localid", СтрокаКонстанты.localid));
		Если СтрокиLocalid.Количество() > 0 Тогда
			Если Не ТипыДанныхСовпадают(СтрокиLocalid[0].ТипЗначения.Типы(), ТипЗнч(СтрокаКонстанты.Значение)) Тогда
				СтрокиНекорректныхТипов.Вставить(СтрокаКонстанты, СтрокаКонстанты.localid);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Если ВидШаблона <> ПредопределенноеЗначение("Перечисление.ВидыШаблонов.Обработка") Тогда
		ПроверитьКорректностьТиповПараметровТаблицы(ТаблицаПараметров, СтрокиНекорректныхТипов, ТаблицаLocalidМакета);
	Иначе
		ПроверитьКорректностьТиповПараметровТаблицы(ТаблицаПараметров, СтрокиНекорректныхТипов, ТаблицаLocalidМакета);
		ПроверитьКорректностьТиповПараметровТаблицы(СоответствияТаблицамШаблонов, СтрокиНекорректныхТипов, ТаблицаLocalidМакета);
	КонецЕсли;
	
	Если СтрокиНекорректныхТипов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	мНекорректныеТипы = Новый Массив;
	
	Для Каждого СтрокаНекорректногоТипа Из СтрокиНекорректныхТипов Цикл
		мНекорректныеТипы.Добавить(Новый Структура("СтрокаТаблицы, Представление", СтрокаНекорректногоТипа.Ключ, СтрокаНекорректногоТипа.Значение));
	КонецЦикла;
	
	СтруктураОшибки = Новый Структура;
	СтруктураОшибки.Вставить("ЗаголовокОшибки", НСтр("ru='Обнаружены параметры, чей тип данных не совпадает с типом поля ЭМД:'"));
	СтруктураОшибки.Вставить("Ошибки", мНекорректныеТипы);
	
	Ошибки.Вставить("НекорректныеТипыДанных", СтруктураОшибки);
	
КонецПроцедуры

Процедура ПроверитьКорректностьТиповПараметровТаблицы(ПроверяемаяТаблица, СтрокиНекорректныхТипов, ТаблицаLocalidМакета)
	...
	Для Каждого СтрокаПараметра Из ПроверяемаяТаблица Цикл
		...
		СтрокиLocalid = ТаблицаLocalidМакета.НайтиСтроки(Новый Структура("localid", ТекLocalid));
		Если СтрокиНекорректныхТипов.Получить(СтрокаПараметра) = Неопределено
			И СтрокиLocalid.Количество() > 0
			И Не ТипыДанныхСовпадают(СтрокиLocalid[0].ТипЗначения.Типы(), СтрокаПараметра.ТипЗначения.Типы())
		Тогда
			СтрокиНекорректныхТипов.Вставить(СтрокаПараметра, СтрокаПараметра.Параметр);
		КонецЕсли;
	КонецЦикла;
...
Процедура ДополнитьНезаполненнымиКонстантами(Ошибки, ТаблицаКонстант, ТаблицаLocalidМакета)
	...
	Для Каждого СтрокаКонстанты Из ТаблицаКонстант Цикл
		...
		Если СтрокаКонстанты.Вычисляется Тогда
			Если Не ЗначениеЗаполнено(СтрокаКонстанты.ВычисляемоеЗначение) Тогда
...
Процедура ДополнитьНесуществующимиГруппирующимиПолями(Ошибки, ТаблицаLocalidМакета, ТаблицаПараметров, ТаблицаКонстант)
	
	НесуществующиеГрупПоля = Новый Соответствие;
	...
	Для Каждого СтрокаLocalid Из ТаблицаLocalidМакета Цикл
		...
		Если ЗначениеЗаполнено(СтрокаLocalid.КлассификацияПоляКлассификаторМЗ) Тогда
			Для Каждого СтрокаТаблицы Из ТаблицаПараметров.НайтиСтроки(Новый Структура("localid", СтрШаблон("%1.КлассификацияПоля", СтрокаLocalid.localid))) Цикл
				Если НесуществующееГруппирующееПоле(СтрокаТаблицы.ГруппирующееПоле, ТаблицаПараметров, ТаблицаКонстант)
					И НесуществующиеГрупПоля.Получить(СтрокаТаблицы) = Неопределено
				Тогда
					НесуществующиеГрупПоля.Вставить(СтрокаТаблицы, СтрокаТаблицы.Параметр);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из ТаблицаКонстант.НайтиСтроки(Новый Структура("localid", СтрокаLocalid.localid)) Цикл
			Если НесуществующееГруппирующееПоле(СтрокаТаблицы.ГруппирующееПоле, ТаблицаПараметров, ТаблицаКонстант)
				И НесуществующиеГрупПоля.Получить(СтрокаТаблицы) = Неопределено
			Тогда
				НесуществующиеГрупПоля.Вставить(СтрокаТаблицы, ?(ЗначениеЗаполнено(СтрокаТаблицы.localidПредставление), СтрокаТаблицы.localidПредставление, СтрокаТаблицы.localid));
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	Если НесуществующиеГрупПоля.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	мНезаполненныеГрупПоля = Новый Массив;
	
	Для Каждого элНесуществующегоГрупПоля Из НесуществующиеГрупПоля Цикл
		мНезаполненныеГрупПоля.Добавить(Новый Структура("СтрокаТаблицы, Представление", элНесуществующегоГрупПоля.Ключ, элНесуществующегоГрупПоля.Значение));
	КонецЦикла;
	
	СтруктураОшибки = Новый Структура;
	СтруктураОшибки.Вставить("ЗаголовокОшибки", НСтр("ru='Обнаружены параметры с несуществующими группирующими полями:'"));
	СтруктураОшибки.Вставить("Ошибки", мНезаполненныеГрупПоля);
	
	Ошибки.Вставить("НесуществующиеГрупПоля", СтруктураОшибки);
...
Функция НесуществующееГруппирующееПоле(ГруппирующееПоле, ТаблицаПараметров, ТаблицаКонстант)
	
	Если ТипЗнч(ГруппирующееПоле) = Тип("Число") Тогда
		Возврат ГруппирующееПоле >= ТаблицаКонстант.Количество();
	Иначе
		Возврат ТаблицаПараметров.НайтиСтроки(Новый Структура("Параметр", ГруппирующееПоле)).Количество() = 0;
	КонецЕсли;
...
Функция ПолучитьОшибкиСЭМД(ТаблицаLocalidМакета, ТаблицаКонстант, ТаблицаПараметров, СоответствияТаблицамШаблонов, ВидШаблона)
	...
	ДополнитьНекорректнымиТипамиПараметров(Ошибки, ТаблицаLocalidМакета, ТаблицаКонстант, ТаблицаПараметров, СоответствияТаблицамШаблонов, ВидШаблона);
	...
	ДополнитьНесуществующимиГруппирующимиПолями(Ошибки, ТаблицаLocalidМакета, ТаблицаПараметров, ТаблицаКонстант);
...
Процедура УстановитьДоступныеТипыСтрокиКонстанты(Элемент, ТаблицаLocalidМакета, ТаблицаКонстант, ТекущаяСтрокаКонстанты)
	...
	ТекущийLocalid = ТаблицаРодительЭлемента(Элемент).ТекущиеДанные.localid;
