...
Процедура ЗаполнитьТаблицуКИОбъекта(КонтактнаяИнформация, Объект) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДобавитьСтроку Тогда
			...
			СтрокаКИ.МаскаВводаТелефонногоНомера = ?(Выборка.Вид.ИспользоватьМаскуВвода, Выборка.Вид.МаскаВводаТелефонногоНомера, "");
...
Функция ПолучитьПоляАдресаФилиала(Знач Филиал) Экспорт
	...
	Попытка
		ЗначенияПолей = КонтактнаяИнформацияСервер.ЗначенияПолейСГородомИзКэшаФилиала(Справочники.Клиенты.ПустаяСсылка(), Филиал);
		Если Не ПустаяСтрока(ЗначенияПолей) Тогда
			XDTOКонтактная = УправлениеКонтактнойИнформациейЛокализация.КонтактнаяИнформацияИзXML(ЗначенияПолей, Перечисления.ТипыКонтактнойИнформации.Адрес, Новый Структура);
			...
			СоответствиеПолей.Вставить("Поле1", "country");
			СоответствиеПолей.Вставить("Поле2", "area");
			СоответствиеПолей.Вставить("Поле4", "city");
			СоответствиеПолей.Вставить("Поле5", "locality");
			СоответствиеПолей.Вставить("ГородскойРайон", "district");
			
			
			Для Каждого кзСоответствие Из СоответствиеПолей Цикл
				Если ЗначениеЗаполнено(ДанныеАдреса[кзСоответствие.Значение]) Тогда
					ПоляАдреса.Вставить(кзСоответствие.Ключ, ДанныеАдреса[кзСоответствие.Значение]);
				КонецЕсли;
			КонецЦикла;
