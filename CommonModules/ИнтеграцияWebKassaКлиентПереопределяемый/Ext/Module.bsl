...
Функция ПолучитьСведенияОбОрганизации(ОбщиеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ОбщиеПараметры.ДокументОснование) Тогда 
		Если ТипЗнч(ОбщиеПараметры.ДокументОснование) = Тип("ДокументСсылка.ОказаниеУслуг") Тогда 
		омг_WebKassa.ДополнитьСведенияЧека(ОбщиеПараметры.ДокументОснование,Результат)
		ИначеЕсли ТипЗнч(ОбщиеПараметры.ДокументОснование) = Тип("ДокументСсылка.ОплатаПлатежнойКартой") Тогда 
		омг_WebKassa.ДополнитьСведенияЧекаОПК(ОбщиеПараметры.ДокументОснование,Результат)
		ИначеЕсли ТипЗнч(ОбщиеПараметры.ДокументОснование) = Тип("ДокументСсылка.ВозвратПоПлатежнойКарте") Тогда 
		омг_WebKassa.ДополнитьСведенияЧекаВПК(ОбщиеПараметры.ДокументОснование,Результат) 
		ИначеЕсли ТипЗнч(ОбщиеПараметры.ДокументОснование) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда 
		омг_WebKassa.ДополнитьСведенияЧекаПКО(ОбщиеПараметры.ДокументОснование,Результат)
		ИначеЕсли ТипЗнч(ОбщиеПараметры.ДокументОснование) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда 
		омг_WebKassa.ДополнитьСведенияЧекаРКО(ОбщиеПараметры.ДокументОснование,Результат)
		КонецЕсли;
	КонецЕсли;
