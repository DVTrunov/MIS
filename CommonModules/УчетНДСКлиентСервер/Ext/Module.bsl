...
//  ПеречислениеСсылка.СтавкиНДС.
//
Функция ПолучитьОсновнуюСтавкуНДСПоСНО(СНО, НастройкаПечатиЧеков, Период, Филиал) Экспорт
	...
	Если ЗначениеЗаполнено(СНО) И Не НастройкаПечатиЧеков.ПустаяСсылка Тогда
		Если НастройкаПечатиЧеков.ОсновнаяСистемаНалогообложения = СНО Тогда                         
			...
			Если ЭтоСНО_УСН(СНО) Тогда
				Организация = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Филиал, "Организация");
				Возврат УчетНДСВызовСервера.ПолучитьСтавкуНДСДляУСН(Организация, Период);
			Иначе
...
Функция ЭтоСНО_УСН(СНО) Экспорт
	
	Возврат 	СНО = ПредопределенноеЗначение("Перечисление.СистемыНалогообложения.УпрощеннаяДоход")
			Или	СНО = ПредопределенноеЗначение("Перечисление.СистемыНалогообложения.УпрощеннаяДоходМинусРасход");
