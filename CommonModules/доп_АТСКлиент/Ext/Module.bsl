///////////////////////////////////////////////////////////////////////////////
// Общий клиентский модуль работы с ОЛИМП.АТС
///////////////////////////////////////////////////////////////////////////////
...
Процедура ОткрытьМонитор() ЭКСПОРТ
	доп_ТелефонияКлиент.ОткрытьФормуВыполнитьДействие(ПолучитьИмяФормыМонитора(), "", "");
КонецПроцедуры
...
Процедура ОткрытьМониторНачатьРазговор(Контакт) ЭКСПОРТ
	доп_ТелефонияКлиентПереопределяемый.ВыбратьНомерКонтактаИОповестить(Контакт, ПолучитьИмяФормыМонитора(), "БитфонМонитор_НачатьЗвонок");
КонецПроцедуры

// Возвращает имя формы Монитора ОЛИМП.АТС
// Возвращаемое значение:
//   Строка - полное имя формы Монитора.
Функция ПолучитьИмяФормыМонитора() ЭКСПОРТ
	Возврат "Обработка.доп_БитАТС.Форма.Монитор";
КонецФункции
...
#Если ВебКлиент Тогда
	доп_ТелефонияКлиент.ВывестиСообщение("Работа внешней компоненты управления ОЛИМП.АТС в режиме веб-клиента не поддерживается");
	Возврат;
#КонецЕсли

	Попытка
		
		стрИмяФайлаВК = ? ( доп_ТелефонияКлиент.Клиент64бит(), "BITPBXControlNative64.dll", "BITPBXControlNative.dll");
		
		доп_ТелефонияКлиентПереопределяемый.ПодключениеВнешнейКомпоненты(
								"ОбщийМакет.доп_КомпонентаУправлениеАТС",
								стрИмяФайлаВК,
								"доп_АТССервер",
								"PBXControl",
								"ОЛИМП.АТС",
...
Процедура УстановитьПараметрыПроверкиЛицензии(обКонтроллерАТС, флагМонитор) ЭКСПОРТ
	...
	Если (обКонтроллерАТС <> Неопределено) Тогда
		...
		серверЛицензийВерсия		= доп_АТССервер.ПолучитьВерсиюСервераЛицензий();
		стрСерверЛицензийАдрес		= доп_АТССервер.ПолучитьАдресСервераЛицензий();
		серверЛицензийПорт			= доп_АТССервер.ПолучитьПортСервераЛицензий();
		серверЛицензийНеИспПрокси	= доп_АТССервер.ПолучитьФлагСервераЛицензийНеИспользоватьПрокси();
		стрСерверЛицензийCID		= доп_АТССервер.ПолучитьCIDСервераЛицензий();
		стрСерверЛицензийПутьКоннектора	= доп_ТелефонияКлиент.ПолучитьПутьБиблиотекиКоннектораСЛ(серверЛицензийВерсия, Ложь);
...
Функция СформироватьНомерСПрефиксами(стрНомерИсх) ЭКСПОРТ
	...
	стрПрефиксВнешЛинии	= доп_АТССервер.ПолучитьПрефиксВыходаНаВнешнююЛинию();
...
//
// Параметры:
//  СкрыватьПараметрыПанели  - Булево, флаг скрывания параметров, не используемых в Панели ОЛИМП.АТС.
//
Процедура ОткрытьФормуНастроек(СкрыватьПараметрыПанели) ЭКСПОРТ
	ПараметрыФормыНастроек = доп_АТССервер.ПолучитьПараметрыОткрытияФормыНастроек();
	фрмНастройки = ПолучитьФорму("РегистрСведений.доп_БитАТСНастройки.Форма.ФормаЗаписи", ПараметрыФормыНастроек);
	...
	доп_ТелефонияКлиентПереопределяемый.ОткрытьФормуСБлокировкойВладельца(фрмНастройки, "БитфонМониторПанель_ОбновлениеНастроек");
КонецПроцедуры
...
Функция СократитьНомер(НомерСПрефиксами, флагПрямойНабор, ВнешнийВызовВозвращ) ЭКСПОРТ
	...
	Если индСИП = 0 Тогда
		...
		СокращНомер = доп_ТелефонияКлиентСервер.ОчиститьНомерЦифрыЗвРеш(НомерСПрефиксами);
		...
		Если НЕ флагПрямойНабор Тогда
			СокращНомер = доп_ТелефонияКлиентСервер.СократитьНомерДляПоиска(СокращНомер);
		КонецЕсли;
		//
		ДлинаСоединен = СтрДлина(СокращНомер);
...
Функция НайтиКонтрагентаПоСокращНомеру(кэшКонтактов, СоответствиеВнутреннихНомеров, СокращНомер, ВнешнийВызов, КонтрагентСсылка, КонтактноеЛицоСсылка, ЕстьДубли=Ложь) ЭКСПОРТ
	...
	Если СтруктураКонтакт = Неопределено Тогда
		...
		найден = доп_АТССервер.НайтиКонтрагентаПоНомеру(СокращНомер, ВнешнийВызов, КонтрагентСсылка, КонтактноеЛицоСсылка, ЕстьДубли);
...
Функция ПолучитьТаймаутЗвонка() ЭКСПОРТ
	таймаут = доп_АТССервер.ПолучитьТаймаутЗвонка();
	Если таймаут = 0 Тогда
		доп_ТелефонияКлиент.ВывестиСообщение("Не задан таймаут звонка в настройках, принимается 15 сек");
		таймаут = 15;
	КонецЕсли;
...
Процедура ОткрытьМониторВоспроизвестиЗаписьРазговора(стрПутьЗаписьРазговора) ЭКСПОРТ
	доп_ТелефонияКлиент.ОткрытьФормуВыполнитьДействие(ПолучитьИмяФормыМонитора(), "БитфонМонитор_ВоспроизвестиЗаписьРазговора", стрПутьЗаписьРазговора);
КонецПроцедуры

//-----------------------------------------------------------------------------
Процедура ОткрытьАналитикуБИТАТС(АТССсылка) ЭКСПОРТ
	доп_ТелефонияКлиент.ОткрытьФайлИлиСсылку(доп_АТССервер.ПолучитьАдресАналитикиАТС(АТССсылка));
КонецПроцедуры
...
Процедура ПеревестиЗвонокВыборНомера(ФормаСсылка, стрИмяТаблицыОжидающих, ссылкаКонтроллерАТС, стрСвойНомер, стрИмяОповещения) ЭКСПОРТ
	...
	формаВводаНомера = доп_ТелефонияКлиент.ПолучитьФормуВводаНомера(ФормаСсылка, Истина, ссылкаКонтроллерАТС, Ложь, стрСвойНомер);
	доп_ТелефонияКлиентПереопределяемый.ОткрытьФормуСБлокировкойВладельца(формаВводаНомера, стрИмяОповещения);
КонецПроцедуры
...
Процедура ПеревестиОжидающийЗвонокНаНомер(ФормаСсылка, стрИмяТаблицыОжидающих, стрСвойНомер, номерПеревода, ссылкаКонтроллерАТС, флагВебСервис, ИдКлиентаВебСервиса) ЭКСПОРТ
	...
	Если ТекущаяСтрокаОжидающих = Неопределено Тогда
		...
		Возврат;
	КонецЕсли;
	номерПереводаПолн = доп_АТСКлиент.СформироватьНомерСПрефиксами(номерПеревода);
	...
	Попытка
		Если флагВебСервис Тогда
			ссылкаАТС = доп_АТССервер.ПолучитьАТС();
			доп_АТССервер.ВебСервисБезусловныйПеревод(ссылкаАТС, ИдКлиентаВебСервиса, КаналОжид, стрСвойНомер, номерПереводаПолн);
		Иначе
			ссылкаКонтроллерАТС.Redirect2(КаналОжид, стрСвойНомер, номерПереводаПолн);
		КонецЕсли;
	Исключение
		доп_ТелефонияКлиент.ВывестиСообщение(ОписаниеОшибки(), ФормаСсылка);
	КонецПопытки;
КонецПроцедуры
...
Функция ПолучитьНомерВнеОфисаПолный() ЭКСПОРТ
	...
	стрНомерВнеОфиса = доп_АТСКлиент.СформироватьНомерСПрефиксами(стрНомерВнеОфиса);
	Если НЕ ЗначениеЗаполнено(стрНомерВнеОфиса) Тогда
		доп_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("Не задан номер для работы вне офиса", , "Ошибка");
	КонецЕсли;
...
Процедура НачатьЗвонок(Подключен, СвязанныйНомер, ВызываемыйНомер, ссылкаКонтроллерАТС, флагВебСервис, ИдКлиентаВебСервиса, ФормаСсылка, ДокументСобытие = Неопределено, СобытияПоНомерам = Неопределено) ЭКСПОРТ
	...
	Если НЕ Подключен Тогда
		доп_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("Нет подключения к АТС", , "Ошибка");
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ВызываемыйНомер) Тогда
		доп_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("Не задан номер вызова", , "Ошибка");
		Возврат;
	КонецЕсли;
	...
	ВызываемыйНомер = доп_АТСКлиент.СформироватьНомерСПрефиксами(ВызываемыйНомер);
...
Процедура ОбработкаОшибки(ссылкаАТС, стрИсточник, стрЗаголовок, стрОшибка, ФормаСсылка) ЭКСПОРТ
	...
	Тогда
			доп_ТелефонияКлиент.ВывестиСообщение(стрОписаниеОшибкиПодр, ФормаСсылка);
	КонецЕсли;
	
	доп_ТелефонияСервер.ЗаписатьОшибкуВЖурналРегистрации(стрИсточник, стрЗаголовок, стрОписаниеОшибкиПодр);
...
Функция ДатаИзВебФормата(стрДата) ЭКСПОРТ
	...
	Исключение
		доп_ТелефонияКлиент.ВывестиСообщение("Ошибка преобразования строки '" + стрДата + "' в дату");
	КонецПопытки;
	Возврат дтДата;
КонецФункции
...
Функция ПроверитьРезультатЗапросаАТС(соотвОтветЗапроса, стрПрефиксОшибки, ФормаСсылка) ЭКСПОРТ
	...
	Если резтЗапроса <> Истина Тогда
		...
		КонецЕсли;
		ОбработкаОшибки(Неопределено, "ОЛИМП.АТС_API", "Ошибка вызова функции API ОЛИМП.АТС", стрОшибка, ФормаСсылка);
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
КонецФункции
...
Процедура НачатьЗвонокВебСервис(ИдКлиента, стрВызывающийНомер, стрНомерКонтекста, стрВызываемыйНомер, флагЗаголовокАвтоподнятия, ТаймаутЗвонка)
	ссылкаАТС = доп_АТССервер.ПолучитьАТС();
	доп_АТССервер.ВебСервисНачатьРазговор(ссылкаАТС, ИдКлиента, стрВызывающийНомер, стрНомерКонтекста, стрВызываемыйНомер, флагЗаголовокАвтоподнятия, ТаймаутЗвонка);
КонецПроцедуры
