///////////////////////////////////////////////////////////////////////////////
// Общий клиентский модуль телефонии ОЛИМП
...
Процедура ЗадатьНомерТелефонаКонтрагента(Контрагент, НомерТел) ЭКСПОРТ
	...
	КонецЕсли;
	//
	массВидовКонтактнойИнформации = доп_ТелефонияСерверПереопределяемый.ПолучитьВидыКонтактнойИнформацииКонтрагента();
...
Процедура ЗадатьНомерТелефонаКонтрагентаПослеВыбораВидаКонтактнойИнформации(ВидКонтактнойИнформации, Параметр) ЭКСПОРТ
	...
	текущийНомерТелефона = доп_ТелефонияСерверПереопределяемый.НайтиНомерКонтрагента(Контрагент, видКонтИнф);
	//
...
Процедура ЗадатьНомерТелефонаКонтрагентаЗавершение(Заменить, Параметр) ЭКСПОРТ
	...
	Контрагент	= Параметр.Контрагент;
	НомерТел	= Параметр.Номер;
	доп_ТелефонияСерверПереопределяемый.ЗаписатьНомерТелефонаКонтрагента(Контрагент, видКонтИнф, НомерТел);
КонецПроцедуры
...
Процедура УдалениеИсторииЗвонковКонтрагентаЗавершение(Результат, Параметры) ЭКСПОРТ
	...
	Если ЗначениеЗаполнено(контрагент) Тогда
		записейУдалено = доп_ТелефонияСервер.УдалитьИсториюЗвонковКонтрагента(контрагент);
		доп_ТелефонияКлиент.ВывестиСообщение("Удалено записей: " + Строка(записейУдалено));
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыбратьНомерКонтактаИОповестить(Контакт, ИмяФормы, ИмяСобытияОповещения) ЭКСПОРТ
	...
	//Если ТипЗнч(Контакт) = Тип("СправочникСсылка." + доп_ТелефонияСерверПереопределяемый.ПолучитьИмяСправочникаКонтрагентов()) Тогда
	Если ИмяТипа = "Справочник." + доп_ТелефонияСерверПереопределяемый.ПолучитьИмяСправочникаКонтрагентов() Тогда
		массивНомераТелефонов = доп_ТелефонияСерверПереопределяемый.НайтиНомераКонтрагента(Контакт);
	//ИначеЕсли ТипЗнч(Контакт) = Тип("СправочникСсылка." + доп_ТелефонияСерверПереопределяемый.ПолучитьИмяСправочникаКонтактныхЛиц()) Тогда
	ИначеЕсли ИмяТипа = "Справочник." + доп_ТелефонияСерверПереопределяемый.ПолучитьИмяСправочникаКонтактныхЛиц() Тогда
		массивНомераТелефонов = доп_ТелефонияСерверПереопределяемый.НайтиНомераКонтактногоЛица(Контакт);
	//ИначеЕсли ТипЗнч(Контакт) = Тип("ДокументСсылка." + доп_ТелефонияСерверПереопределяемый.ПолучитьИмяДокументаСобытие()) Тогда
	ИначеЕсли ИмяТипа = "Документ." + доп_ТелефонияСерверПереопределяемый.ПолучитьИмяДокументаСобытие() Тогда
	//-Переопределенное
		стрСообщениеОбОшибке = "";
		контактСобытия = доп_ТелефонияСерверПереопределяемый.ПолучитьКонтактСобытия(Контакт, стрСообщениеОбОшибке);
		Если ЗначениеЗаполнено(стрСообщениеОбОшибке) Тогда
			доп_ТелефонияКлиент.ВывестиСообщение(стрСообщениеОбОшибке);
		Иначе
...
Процедура ВыбратьНомерКонтактаИОповеститьЗавершение(выбранныйЭлементСпискаНомеров, Параметры) ЭКСПОРТ
	...
	доп_ТелефонияКлиент.ОткрытьФормуВыполнитьДействие(Параметры.ИмяФормы, Параметры.ИмяСобытияОповещения, НомерВызова);
КонецПроцедуры
...
Процедура ПодключениеВнешнейКомпоненты(стрМакетВК, стрИмяФайлаВК, стрИмяМодуля, стрИмяКлассаВК, стрЗаголовокСообщения, ИмяСобытияОповещения, ПараметрОповещения=Неопределено) ЭКСПОРТ
	...
	Если стрИмяМодуля = "доп_БитфонСервер" Тогда
		стрВерсияКомпонентыОжидаемая = доп_БитфонСервер.ПолучитьВерсиюВнешнейКомпоненты();
		стрВерсияКомпонентыПользователя = доп_БитфонСервер.ПолучитьВерсиюВнешнейКомпонентыТекущегоПользователя();
	ИначеЕсли стрИмяМодуля = "доп_АТССервер" Тогда
		стрВерсияКомпонентыОжидаемая = доп_АТССервер.ПолучитьВерсиюВнешнейКомпоненты();
		стрВерсияКомпонентыПользователя = доп_АТССервер.ПолучитьВерсиюВнешнейКомпонентыТекущегоПользователя();
	КонецЕсли;
...
Процедура ПодключениеВнешнейКомпонентыЗавершение(Подключено, Параметры) ЭКСПОРТ
	...
	Если НЕ Подключено Тогда
		Если Параметры.ПовторноеПодключение Тогда
			доп_ТелефонияКлиент.ВывестиСообщение(Параметры.ЗаголовокСообщения + " - Ошибка! Не удалось подключить внешнюю компоненту " + Параметры.ИмяФайлаВК + ".");
		Иначе
	...
	Попытка
		...
		Если стрВерсияКомпоненты <> Параметры.ВерсияКомпонентыОжидаемая Тогда
			доп_ТелефонияКлиент.ВывестиСообщение(Параметры.ЗаголовокСообщения + " - Ошибка! Неправильная версия внешней компоненты " + Параметры.ИмяФайлаВК + ". Перезапустите 1С для установки нужной версии.");
			УстановитьВерсиюВнешнейКомпонентыТекущегоПользователяПереопр(Параметры.ОбщийМодуль, стрВерсияКомпоненты);
		Иначе
			стрРазрядностьПлатформы = ? (доп_ТелефонияКлиент.Клиент64бит(), "x64", "x86");
		...
		КонецЕсли;
	Исключение
		доп_ТелефонияКлиент.ВывестиСообщение(Параметры.ЗаголовокСообщения + " - Ошибка! Не удалось получить версию внешней компоненты " + Параметры.ИмяФайлаВК + ". " + ОписаниеОшибки());
...
Функция ПроверкаНовыйИнтерфейсТакси() ЭКСПОРТ
	...
	массВерсПлатф = доп_ТелефонияКлиент.ПолучитьВерсиюПлатформы();
	
	// проверка версии платформы
	платформаБольшеРавна838 = доп_ТелефонияКлиент.ПроверкаВерсий(массВерсПлатф, минВерсия);
	...
	массРежимСовм = доп_ТелефонияСервер.ПолучитьРежимСовместимостиКонфигурации();
	...
	Иначе
		проверитьВариантИнтерфейсаКл = доп_ТелефонияКлиент.ПроверкаВерсий(массРежимСовм, минВерсия);
	КонецЕсли;
...
Процедура ОткрытьМодальноФормуВыбораКонтрагента(ИмяПроцедурыОповещения, ПараметрыПроцедурыОповещения)
	форма = ПолучитьФорму("Справочник." + доп_ТелефонияСерверПереопределяемый.ПолучитьИмяСправочникаКонтрагентов() + ".ФормаВыбора");
...
Процедура УстановитьВерсиюВнешнейКомпонентыТекущегоПользователяПереопр(стрИмяМодуля, стрВерсия)
	Если стрИмяМодуля = "доп_БитфонСервер" Тогда
		доп_БитфонСервер.УстановитьВерсиюВнешнейКомпонентыТекущегоПользователя(стрВерсия);
	ИначеЕсли стрИмяМодуля = "доп_АТССервер" Тогда
		доп_АТССервер.УстановитьВерсиюВнешнейКомпонентыТекущегоПользователя(стрВерсия);
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановкаКомпонентыТелефонии(Знач стрВК, стрИмяФайлаСокр, Параметры, флагОбновлениеПутиВК, ИмяПроцедурыОповещения)
	...
	стрПолноеИмяФайлаВК = доп_ТелефонияКлиент.УдалениеФайлаВнешнейКомпонентыТелефонии(стрИмяФайлаСокр);
...
Процедура СоздатьНовогоКонтрагента(стрНомер, Событие = Неопределено) Экспорт
	...
	стрИмяСправочника = доп_ТелефонияСерверПереопределяемый.ПолучитьИмяСправочникаКонтрагентов();
...
Функция ОткрытьКарточкуКонтрагента(Клиент, НомерТелефона = Неопределено) Экспорт
	...
	ИначеЕсли НомерТелефона <> Неопределено Тогда 	
		...
		фрмОбработкаНеизвНомера = ПолучитьФорму("Обработка.доп_БитфонОбщее.Форма.ОбработкаНомерНеопределен", обработкаНеизвНомераПарам);
...
Процедура ОткрытьФормуОбработкиНеизвестногоНомера(НомерНеОпределен, НомерВходящий, Событие) Экспорт
	...
	Если НомерНеОпределен Тогда
		НеизвестныйНомер = доп_ТелефонияКлиентСервер.СократитьНомерДляПоиска(НомерВходящий);
...
Процедура доп_ИсторияЗвонковОбработкаРасшифровки(ДанныеРасшифровки, Расшифровка, СтандартнаяОбработка) Экспорт
	
	стрЗапись = доп_ТелефонияСерверПереопределяемый.ПолучитьСсылкуНаЗаписьРазговораИзДанныхРасшифровкиВоВременномХранилище(ДанныеРасшифровки, Расшифровка);
	Если ЗначениеЗаполнено(стрЗапись) Тогда
		СтандартнаяОбработка = Ложь;
		доп_БитфонКлиент.ВоспроизвестиЗаписьРазговора(стрЗапись);
	КонецЕсли;
...
Функция Позвонить(НомерТелефона, ДокументЗвонок = Неопределено, ОснованиеСобытия = Неопределено) Экспорт
	...
	Попытка
		//Изменение для 8.5 Карсаков
		//ФормаОповещ = ПолучитьФорму("Обработка.доп_Битфон.Форма.ОсновнаяФорма");
		  ФормаОповещ = ПолучитьФорму("Обработка.доп_Битфон.Форма.ОсновнаяФорма85");
		//Конец изменений для 8.5
...
Функция ПозвонитьМонитор(НомерТелефона, ДокументЗвонок = Неопределено, ОснованиеСобытия = Неопределено) Экспорт
	...
	Попытка
		ФормаОповещ = ПолучитьФорму("Обработка.доп_БитАТС.Форма.Монитор");
		Если ФормаОповещ.Открыта() Тогда
			ФормаОповещ.Активизировать();		
...
Функция ПрекратитьЗвонок(ДокЗвонка) Экспорт
	ФормаБитФон = ПолучитьФорму("Обработка.доп_Битфон.Форма.ОсновнаяФорма");
