...
Функция ПроверкаНаложенияЗаявокОборудования(табРаботы,ВремяНачала,ВремяОкончания,Объект,ИспользованиеКабинетов) Экспорт
	...
	Для Каждого СтрокаРабота Из табРаботы Цикл
		...
		Тогда
			...
			ОборудованиеНоменклатуры = ПроцедурыСпециализацииПоставки.ПолучитьЗначенияОборудования(СтрокаРабота.Номенклатура,Объект.Филиал,Неопределено);
...
Функция ПолучитьУчастниковМоделиРасписания(МодельРасписания, ВключатьПомеченныхНаУдаление = Ложь) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("МодельРасписания", МодельРасписания);
	Запрос.УстановитьПараметр("ВключатьПомеченныхНаУдаление", ВключатьПомеченныхНаУдаление);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Субъекты.Субъект
	|ИЗ
	...
	|	    И (&ВключатьПомеченныхНаУдаление = ИСТИНА ИЛИ НЕ МоделиРасписанияСотрудники.Сотрудник.ПометкаУдаления)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	    И (&ВключатьПомеченныхНаУдаление = ИСТИНА ИЛИ НЕ МоделиРасписанияКабинеты.Кабинет.ПометкаУдаления)
	|	
	|	ОБЪЕДИНИТЬ
	|	
	|	ВЫБРАТЬ
	...
	|	    И (&ВключатьПомеченныхНаУдаление = ИСТИНА ИЛИ НЕ СотрудникиСпециализации.Ссылка.ПометкаУдаления)
