...
//  Массив.
//
Процедура ОбновитьКешАдресаФилиалов() Экспорт
	...
	ВыбОбъекты = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыбОбъекты.Следующий() Цикл
		Выб = ВыбОбъекты.Выбрать();
		Выб.Следующий();
		ОписаниеАдреса = Новый Структура("Вид, ЗначенияПолей", Выб.Вид, Выб.ЗначенияПолей);
		Адреса.Вставить(Выб.Объект, Новый ФиксированнаяСтруктура(ОписаниеАдреса));
	КонецЦикла;
	
	УстановитьПривилегированныйРежим(Истина);
	ПараметрыСеанса.КешАдресовФилиалов = Новый ФиксированноеСоответствие(Адреса);
...
Процедура ПривестиНомераТелефоновПоВидуКМаскеВФоне(Параметры, Адрес) Экспорт
	
	МаксЗаписей = 200;
	ТекущаяЗапись = 0;
	Маска = Параметры.Маска;
	ЗначащихЦифрМаски = СтрЧислоВхождений(Маска, "9");
		
	Отбор = Новый Структура("Вид", Параметры.ВидКИ);
	
	// Первая цифра в маске
	Если ЗначащихЦифрМаски <> 0 Тогда
		ПерваяЦифраМаски = Лев(СтроковыеФункцииКлиентСервер.ПолучитьЦифрыСтроки(Маска), 1);
	Иначе
		ПерваяЦифраМаски = "";
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл             
		...
		Номер = Выборка.Поле12;
		ДлинаНомера = СтрДлина(Номер);
		
		Если ДлинаНомера = ЗначащихЦифрМаски 
			Или ДлинаНомера = ЗначащихЦифрМаски + 1
		Тогда
		
			Если ДлинаНомера = ЗначащихЦифрМаски + 1 
				И (ПерваяЦифраМаски  = "7" Или ПерваяЦифраМаски  = "8")
			Тогда
			 	Номер = Прав(Номер, ДлинаНомера - 1);
			Иначе
				Продолжить;
			КонецЕсли;

			Представление = ПолучитьПредставлениеНомераПоМаске(Номер, Маска);			
		...
		Если ТекущаяЗапись = МаксЗаписей Тогда
...
Функция ПолучитьПредставлениеНомераПоМаске(Номер, Маска)
	...
	Для ИндМаски = 1 По ДлинаМаски Цикл
		Символ = Сред(Маска, ИндМаски, 1); 
		Если Символ = "9" Тогда
			Представление = Представление + Сред(Номер, ИндНомера, 1);
			ИндНомера = ИндНомера + 1;
		Иначе
...
Функция ЗаписатьКонтактнуюИнформацию(Объект, Тип, Вид, ЗначенияПолей, Значение) Экспорт
	...
	ОбъектКИ = УправлениеКонтактнойИнформациейЛокализация.КонтактнаяИнформацияИзXML(ЗначенияПолей, Вид);
...
// Обновить КИ под формат ФИАС. Адерс в регистре есть, но, с пустым ЗначенияПолей или ввод нового адреса с ВозвращатьЗначенияПолей = Истина.
//
// Параметры:
//  Объект					 - ЛюбаяСсылка						 - владелец.
//  Вид						 - СправочникСсылка.ВидыКонтактнойИнформации	 - вид КИ.
...
//  Структура.
//
Функция ОбновитьКИПодФорматФИАС(Объект, Вид, ВозвращатьЗначенияПолей = Ложь) Экспорт
	...
	ИначеЕсли ВозвращатьЗначенияПолей Тогда
		// Добавление нового адреса, а не 
		Возврат ЗначенияПолейСГородомИзКэшаФилиала(Объект);
...
//  Структура.
//
Функция ЗначенияПолейСГородомИзКэшаФилиала(Объект, Знач Филиал = Неопределено) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("СправочникСсылка.Клиенты") Тогда
		...
		УстановитьПривилегированныйРежим(Истина);
		КешАдресовФилиалов = ПараметрыСеанса.КешАдресовФилиалов; 
		...
		АдресФилиала = КешАдресовФилиалов.Получить(Филиал);
		Если АдресФилиала <> Неопределено Тогда
			Если АдресФилиала.ЗначенияПолей <> "" Тогда
				ДанныеАдресаФилиала = ПолучитьПредставлениеЭлементовАдресов(АдресФилиала.ЗначенияПолей, Истина);
			Иначе
				ЗначенияПолей = ОбновитьКИПодФорматФИАС(Филиал, АдресФилиала.Вид, Истина);
...
Функция ПолучитьПредставлениеЭлементовАдресов(ЗначенияПолей, ВсеЭлементы = Ложь) Экспорт
	...
	ДанныеАдреса = УправлениеКонтактнойИнформациейЛокализация.КонтактнаяИнформацияИзXML(ЗначенияПолей);
