Процедура ВыполнитьОбработкуИФормированиеАвтоматическихСообщений() Экспорт
	...
	Если Не доп_Защита_Конфигурации.ЛицензияДоступна() Тогда
		Возврат;
	КонецЕсли;
...
Функция ОтобратьКлиентовПоВидуСообщения(ВидСообщения)
	...
	Если ВидСообщения.СпособОтправки = Перечисления.СпособыОтправкиСообщений.SMS Тогда 
		мсВидыКИ.Добавить(Справочники.ВидыКонтактнойИнформации.ТелефонСотовый);
	ИначеЕсли ВидСообщения.СпособОтправки = Перечисления.СпособыОтправкиСообщений.EMAIL Тогда 
...
Процедура СобытияДокументов_ПриЗаписи(Источник, Отказ) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("ФормированиеСообщений.СобытияДокументов_ПриЗаписи", УровеньЖурналаРегистрации.Ошибка, Источник.Метаданные(), Источник.Ссылка, ТекстОшибки);
		ОбщегоНазначенияКлиентСервер.СообщитьОбОшибке("Ошибка при обработке рассылок по документу:" + Символы.ПС + ТекстОшибки);
	КонецПопытки;
