...
Функция СоздатьВерсиюШаблона(Владелец, Дата = Неопределено) Экспорт
	...
	НоваяВерсия.ЗаполнятьДиагнозыИзТабличнойЧасти = Владелец.ЗаполнятьДиагнозыИзТабличнойЧасти;
...
Функция ПолучитьВерсиюШаблона(Шаблон, Дата = Неопределено, УчетнаяПолитика = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	И &Дата <= ВерсииШаблоновHTML.Дата  И НЕ ВерсииШаблоновHTML.ПометкаУдаления
		|
		|УПОРЯДОЧИТЬ ПО
