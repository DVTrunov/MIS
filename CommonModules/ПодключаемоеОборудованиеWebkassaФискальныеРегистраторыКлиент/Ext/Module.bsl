...
Процедура НачатьФискализациюЧека(ОповещениеПриЗавершении, ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры) Экспорт
	...
	Если не ТипЗнч(ОбщиеПараметры.ТипРасчета) = Тип("Число") Тогда	
	ТипРасчета = ИнтеграцияWebKassaКлиентПереопределяемый.КодТипаРасчета(ОбщиеПараметры.ТипРасчета)
	иначе
	ТипРасчета = ОбщиеПараметры.ТипРасчета 
	КонецЕсли;
	...
	//Заполним данные первичного чека
	Если ТипЧека Тогда
		//Если не заполнены ключевые поля то нет смысла передовать структуру первичного чека
		//позволит пробивать чеки возврата в версии ОФД ниже 2.0.3 без указания информации по первияному чеку
		Если Не ЗначениеЗаполнено(ВходныеПараметры.ДанныеПервичногоЧека.РегистрационныйНомерКассы)
			И Не ЗначениеЗаполнено(ВходныеПараметры.ДанныеПервичногоЧека.ДатаВремяЧека)
			И Не ЗначениеЗаполнено(ВходныеПараметры.ДанныеПервичногоЧека.НомерЧека) Тогда 
			
			ДанныеДляЗапроса.Удалить("ReturnBasisDetails");
			
		Иначе
			ЧекОснование = ДанныеДляЗапроса.ReturnBasisDetails;
			ЧекОснование.Total 					= ВходныеПараметры.ДанныеПервичногоЧека.ОбщаяСуммаЧека;
			ЧекОснование.RegistrationNumber	 	= ВходныеПараметры.ДанныеПервичногоЧека.РегистрационныйНомерКассы;
			ЧекОснование.DateTime 				= ВходныеПараметры.ДанныеПервичногоЧека.ДатаВремяЧека;
			ЧекОснование.IsOffline 				= ВходныеПараметры.ДанныеПервичногоЧека.АвтономныйРежимРаботыКассы;
			ЧекОснование.CheckNumber 			= ВходныеПараметры.ДанныеПервичногоЧека.НомерЧека;
		КонецЕсли;
...
Функция ОбщаяИнформацияПоЧеку(СерийныйНомер, ЧекВозврата, ФискальныйЧек, ВыходныеПараметры)
	...
	//Данные По чеку возврата протокол ОФД версии 2.0.3
	Если ЧекВозврата Тогда
		
		СтруктураВозврата = Новый Структура();
		СтруктураВозврата.Вставить("CheckNumber", "");
		СтруктураВозврата.Вставить("DateTime", ТекущаяДата());
		СтруктураВозврата.Вставить("RegistrationNumber", "");
		СтруктураВозврата.Вставить("Total", 0);
		СтруктураВозврата.Вставить("IsOffline", Ложь);
		
		ДанныеДляЗапроса.Вставить("ReturnBasisDetails", СтруктураВозврата);
...
Функция ДобавитьПозициюВЧек(Наименование, Количество, Цена, Сумма,
	...
	СтавкаНДС = Расш4_НормализоватьСтавкуНДС(СтавкаНДС);
	...
	//Код нац. каталога - заглушка
	Если Ложь Тогда 
		ПозицияВЧеке.Вставить("NTIN", "");
	КонецЕсли;
...
Процедура НачатьЗакрытиеЧека_ПослеАутентификации(Результат, ДополнительныеПараметры) Экспорт
	...
	Попытка
		...
		Если ОбщиеПараметры.Свойство("ДокументОснование") Тогда
			...
			ДокСсылка = ОбщиеПараметры.ДокументОснование;
			
			Если ТипЗнч(ДокСсылка) = Тип("ДокументСсылка.ОказаниеУслуг") Тогда
				
				СуммаПредоплатыСписать = омг_ДополнительныеАлгоритмы.ПолучитьСуммуПредоплатыСписать(ДокСсылка);
				Если СуммаПредоплатыСписать > 0 Тогда
					ДобавитьМодификаторПредоплаты(ДополнительныеПараметры.ПараметрыЗапроса, СуммаПредоплатыСписать);
				КонецЕсли;
				
				РеальныеСтроки = омг_ДополнительныеАлгоритмы.ПолучитьРеальныеЦеныСтрок(ДокСсылка);
				ВосстановитьЦеныВПозициях(ДополнительныеПараметры.ПараметрыЗапроса, РеальныеСтроки);
				
				// ДОБАВЛЕНО: гасим остаточную разницу округления между
				// Positions (построчное копеечное округление) и Payments
				// (СуммаДокумента округляется до целых тенге в омг_ПроверкаСуммыОплаты).
				// На документах с большим числом строк и "кривыми" процентами
				// скидки (наборы/профили) эти округления могут разойтись на
				// несколько тыйын/тенге - гасим разницу отдельным модификатором.
				СверитьИСкорректироватьОкругление(ДополнительныеПараметры.ПараметрыЗапроса);
	...
	Исключение
		// Не блокируем пробитие чека при сбое патча
	КонецПопытки;
...
Процедура НачатьЗакрытиеЧека_ПослеОтветаВебСервиса(РезультатЗапроса, ДополнительныеПараметры) Экспорт
	...
	//Передадим информацию по чеку со стороны WebKassa
	ВыходныеПараметры.Добавить(РезультатЗапроса.ДополнительныеПараметры.ПараметрыЗапроса);
...
Процедура НачатьАутентификациюПользователя(ОповещениеПриЗавершении) Экспорт
	...
	//////////////////////////////////////////////////////////////////////////////////////
	// ОМГ   
	Если ЛогинПарольТокен = Неопределено и ОповещениеПриЗавершении.ИмяПроцедуры = "НачатьЗакрытиеЧека_ПослеАутентификации" Тогда
	ЛогинПарольТокен = омг_WebKassa.ПолучитьПарольИзТекПользователя()	
	КонецЕсли;	
	// ОМГ
	//////////////////////////////////////////////////////////////////////////////////////
	...
	Если Параметры.ИспользоватьПринтерЧеков Тогда
		...
		Если не ДополнительныеПараметры = Неопределено Тогда
			Если ДополнительныеПараметры.Свойство("ОповещениеПриЗавершении") Тогда
				Если ДополнительныеПараметры.ОповещениеПриЗавершении.ДополнительныеПараметры.Свойство("ВходныеПараметры") Тогда
					Если ДополнительныеПараметры.ОповещениеПриЗавершении.ДополнительныеПараметры.ВходныеПараметры.Свойство("ДокументОснование") Тогда
						Если ЗначениеЗаполнено(ДополнительныеПараметры.ОповещениеПриЗавершении.ДополнительныеПараметры.ВходныеПараметры.ДокументОснование) Тогда
							ДокОснование = ДополнительныеПараметры.ОповещениеПриЗавершении.ДополнительныеПараметры.ВходныеПараметры.ДокументОснование	
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		ТекстЧека = СформироватьТекстФискальногоЧека(СтруктураДанных, Параметры, ДополнительныеДанные,ДокОснование);  
		////////////////////////////////////////////////////////
		// ОМГ

		ДополнительныеПараметры.Вставить("ТекстЧека",ТекстЧека);
		// ОМГ   
		////////////////////////////////////////////////////////
...
Процедура ПослеЗавершенияПечатиНаПринтереЧеков(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ОповещениеПриЗавершении <> Неопределено Тогда
		...
		Если РезультатВыполнения.ВыходныеПараметры.Количество() > 5 и ДополнительныеПараметры.Свойство("ТекстЧека")Тогда
			 Результат.ВыходныеПараметры.Добавить(ДополнительныеПараметры.ТекстЧека); 
			 Результат.ВыходныеПараметры.Добавить(РезультатВыполнения.ВыходныеПараметры[5]);
		КонецЕсли;
		КонецЕсли;
		// ОМГ	 
		////////////////////////////////////////////////////////
...
//
Функция СформироватьТекстФискальногоЧека(СтруктураДанных, Параметры, ДополнительныеДанные = Неопределено,ДокОснование = Неопределено) Экспорт
	
	Если не ДополнительныеДанные.Свойство("Филиал") Тогда
		
		ДокументПродажи = омг_WebKassa.НайтиДокументПоЧеку(СтруктураДанных.ФискальныйНомер);
		
		Если не ДокументПродажи = Неопределено Тогда
			омг_WebKassa.ДополнитьСведенияЧека(ДокументПродажи,ДополнительныеДанные)
		иначе
			Текст = "";	
			Возврат Текст; 
		КонецЕсли;
	...
	Если не ДокОснование = Неопределено Тогда
		Если ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ОказаниеУслуг") Тогда
			ТекстЗаявок = омг_WebKassa.ДополнитьЗаявками(ДокОснование);
		КонецЕсли;
	КонецЕсли;
	
	Если не СтруктураДанных.OperationType = 2 Тогда
				СтрВозврата = омг_WebKassa.ДополнитьСведенияВовзврата(ДокОснование)
	КонецЕсли;
	...
	Попытка  
		...
		//	ШиринаСтроки = ?(Параметры.ШиринаЛенты = Неопределено, 58, Число(Параметры.ШиринаЛенты));
		//Иначе
		//	ШиринаСтроки = 58;
		//КонецЕсли;
		ШиринаСтроки = 58;
		...
		КонецЕсли;
		Если ДополнительныеДанные.Кассир = Неопределено Тогда
		Кассир = "Кассир 1"
		иначе
		Кассир = ДополнительныеДанные.Кассир
        КонецЕсли;
		
		Если СтруктураДанных.Свойство("TicketModifiers") И СтруктураДанных.TicketModifiers.Количество() > 0 Тогда
			
			СуммаМодификаторов = 0;
			Для Каждого Модификатор Из СтруктураДанных.TicketModifiers Цикл
				Если Модификатор.Свойство("Type") И Модификатор.Type = 1 Тогда
					СуммаМодификаторов = СуммаМодификаторов + Модификатор.Sum;
				ИначеЕсли Модификатор.Свойство("Type") И Модификатор.Type = 2 Тогда
					СуммаМодификаторов = СуммаМодификаторов - Модификатор.Sum;
				КонецЕсли;
			КонецЦикла;
			
			Если СуммаМодификаторов <> 0 И СтруктураДанных.Свойство("Positions") Тогда
				
				СтруктураДанных = ОбщегоНазначения.СкопироватьРекурсивно(СтруктураДанных);
				
				ВиртуальнаяПозиция = Новый Структура;
				ВиртуальнаяПозиция.Вставить("Count",        1);
				ВиртуальнаяПозиция.Вставить("Price",        -СуммаМодификаторов);
				ВиртуальнаяПозиция.Вставить("Tax",          0);
				ВиртуальнаяПозиция.Вставить("TaxType",      0);
				ВиртуальнаяПозиция.Вставить("PositionName", "");
				ВиртуальнаяПозиция.Вставить("PositionCode", СтруктураДанных.Positions.Количество()+1);
				ВиртуальнаяПозиция.Вставить("Discount",     0);
				ВиртуальнаяПозиция.Вставить("Markup",       0);
				ВиртуальнаяПозиция.Вставить("SectionCode",  1);
				ВиртуальнаяПозиция.Вставить("UnitCode",     796);
				
				СтруктураДанных.Positions.Добавить(ВиртуальнаяПозиция);
		...
		Текст = Текст + ДополнительныеДанные.Организация + Символы.ПС;
		Текст = Текст + ВыравнитьПоле(СтруктураНаименованияПолей.БИН + " " + ДополнительныеДанные.ИИН, ШиринаСтроки) + Символы.ПС; 
		Текст = Текст + ВыравнитьПоле(СтруктураНаименованияПолей.ЗНМ + ": " + СтруктураДанных.СерийныйНомерККМ,ШиринаСтроки) + Символы.ПС;
        Текст = Текст + ВыравнитьПоле(СтруктураНаименованияПолей.ИНК + ": " + Формат(СтруктураДанных.ИНК, ФорматБезПробела),ШиринаСтроки) + Символы.ПС; 
		Текст = Текст + ВыравнитьПоле(Формат(Дата(СтруктураДанных.ДатаЧека),"ДФ='dd.MM.yyyy hh:mm'"),ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ВыравнитьПоле(СтруктураНаименованияПолей.НомерПродажи  + СтруктураДанных.НомерВРамкахСмены,ШиринаСтроки) + Символы.ПС; 
		Текст = Текст + ВыравнитьПоле(ДополнительныеДанные.Филиал,ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ВыравнитьПоле("Ф-л: "+ ДополнительныеДанные.Филиал,ШиринаСтроки) + Символы.ПС;
		
		//Текст = Текст + ВыравнитьПоле("Лицензия на занятие медицинской деятельностью:" ,ШиринаСтроки) + Символы.ПС;
		//Текст = Текст + ВыравнитьПоле("№ "+ ДополнительныеДанные.НомерЛицензии + " от " +Формат(ДополнительныеДанные.ДатаВыдачиЛицензии,"ДФ=dd.MM.yyyy") ,ШиринаСтроки) + Символы.ПС;
        Текст = Текст + "Лицензия на занятие медицинской деятельностью № "+ ДополнительныеДанные.НомерЛицензии + " от " +Формат(ДополнительныеДанные.ДатаВыдачиЛицензии,"ДФ=dd.MM.yyyy")+ Символы.ПС;
        Текст = Текст + ВыравнитьПоле(ДополнительныеДанные.ФактическийАдрес,ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ВыравнитьПоле("тел. "+ДополнительныеДанные.РабочийТелефон,ШиринаСтроки) + Символы.ПС; 
		Текст = Текст + ВыравнитьПоле(ТипЧека + " " + ДополнительныеДанные.НомерДокумента,ШиринаСтроки) + Символы.ПС;
		//Текст = Текст + РазделительВнут;
		//Текст = Текст + ВыравнитьПоле(ДополнительныеДанные.НомерДокумента,ШиринаСтроки) + Символы.ПС; 
		Текст = Текст + ВыравнитьПоле("Дата: "+ Формат(Дата(СтруктураДанных.ДатаЧека),"ДФ='dd.MM.yyyy'"),ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ВыравнитьПоле("ИИН пациента: "+ ДополнительныеДанные.КлиентИНН,ШиринаСтроки) + Символы.ПС;
		Текст = Текст + ВыравнитьПоле("Пациент: "+ ДополнительныеДанные.Клиент,ШиринаСтроки) + Символы.ПС; 
		Если ЗначениеЗаполнено(ДополнительныеДанные.КлиентДатаРождения) Тогда  
		Лет =ОбщегоНазначенияКлиентСервер.ПолучитьПолноеЧислоЛетСДаты(Дата(ДополнительныеДанные.КлиентДатаРождения), Дата(СтруктураДанных.ДатаЧека));
		//Лет =  Цел((Дата(СтруктураДанных.ДатаЧека) - ДополнительныеДанные.КлиентДатаРождения)/(60*60*24*365));
		Текст = Текст + ВыравнитьПоле("Дата рождения: "+ Формат(Дата(ДополнительныеДанные.КлиентДатаРождения),"ДФ='dd.MM.yyyy'") + "(" +Лет+" л.)",ШиринаСтроки) + Символы.ПС;
		иначе
		Текст = Текст + ВыравнитьПоле("Дата рождения: ",ШиринаСтроки)+ Символы.ПС;
        КонецЕсли;
		Текст = Текст + РазделительВнут;
		Если не ТекстЗаявок = "" Тогда
		ТекстКДЛ =" Вы сможете отслеживать выполнения заявки на сайте kdl.kz и скачать результаты, когда они будут готовы.Данные для доступа на сайт kdl.kz:"; 
		//Текст = Текст + ПостроитьПолеПереносом(ТекстКДЛ,ШиринаСтроки,,Истина)+ Символы.ПС;
		Текст = Текст + ТекстКДЛ+ Символы.ПС;
		Текст = Текст + ТекстЗаявок+ Символы.ПС;
		КонецЕсли;
		ТекстИнстаграм =  "Nazar audaryñyz! Ağymdağy aksialar turaly aqparatty www.divera.kz saitynan nemese Instagram jelisindegi "+ ДополнительныеДанные.Инстаграм +" paraqşasynan köruge bolady. Внимание! Информацию по текущим акциям можно просмотреть на сайте www.divera.kz или на страничке инстраграмм " + ДополнительныеДанные.Инстаграм;
		 //Текст = Текст + ПостроитьПолеПереносом(ТекстИнстаграм,ШиринаСтроки,,Истина)+ Символы.ПС;
		 Текст = Текст + ТекстИнстаграм + Символы.ПС;
		//QR код Инстаграм
		Текст = Текст  +"QRInstagarm";
		
		Текст = Текст + РазделительВнут;
		Если СтруктураДанных.OperationType = 2 Тогда
		ТекстПредупреждение = "Оплатив настоящие услуги, Пациент соглашается и присоединяется к условиям Публичного договора, размещенного на информационных стендах и сайте www.divera.kz";
		//Текст = Текст + ПостроитьПолеПереносом(ТекстПредупреждение,ШиринаСтроки,,Истина)+ Символы.ПС; 
		Текст = Текст + ТекстПредупреждение + Символы.ПС; 
		Текст = Текст + РазделительВнут;
		
		// Тело чека
		
		Текст = Текст + ВыстроитьПоля("ЗАКАЗАННЫЕ УСЛУГИ:", , ШиринаСтроки) + Символы.ПС; 
		иначе
		Текст = Текст + ВыстроитьПоля("ОТМЕНЕННЫЕ УСЛУГИ:", , ШиринаСтроки) + Символы.ПС;
		...
		Для Каждого Позиция Из СтруктураДанных.Positions Цикл
			...
			Текст = Текст + НаименованиеТовара + Символы.ПС;
			...
			Если Позиция.Свойство("Mark") Тогда 
				...
				//Текст = Текст + КодМаркировки + Символы.ПС;
			КонецЕсли;
			
			Текст = Текст + ВыравнитьПоле(КоличествоЦена,78,"Право") + Символы.ПС;
		...
		Если не СтруктураДанных.OperationType = 2 Тогда
			Текст = Текст + "Чек основания:" + Символы.ПС;
			Текст = Текст + ВыравнитьПоле("ФП: " + СтрВозврата.ФП,ШиринаСтроки)+ Символы.ПС;
            Текст = Текст + ВыравнитьПоле("Дата: " + СтрВозврата.ДатамВремя,ШиринаСтроки)+ Символы.ПС;
			Текст = Текст + ВыравнитьПоле("РНК: " + СтрВозврата.РНК,ШиринаСтроки)+ Символы.ПС;
            Текст = Текст + ВыравнитьПоле("Итого: " + СтрВозврата.ОбщаяСумма,ШиринаСтроки)+ Символы.ПС;
			Текст = Текст + ВыравнитьПоле("Автономный: " + СтрВозврата.ПризнакАвтономности,ШиринаСтроки)+ Символы.ПС;
			Текст = Текст + РазделительВнут;
		КонецЕсли;
		...
		Для Каждого Платеж Из СтруктураДанных.Payments Цикл
			...
			ТекстБанка = "";
			Если Платеж.PaymentType = 1 Тогда
			ТекстБанка = " ("+ ДополнительныеДанные.омг_ИспользуемыйТерминал + "):";	
			КонецЕсли;	
			//ТекстВрем = ПостроитьПолеПереносом(ТипОплаты + ТекстБанка,ШиринаСтроки, , Ложь);
			ТекстВрем = ТипОплаты + ТекстБанка;
		...
		Если СуммаСкидки <> 0 Тогда
			...
			ИтогоБезСкидка = Формат(СуммаСкидки + СуммаПоЧеку, ФорматДенежный);
			//ТекстВрем = ПостроитьПолеПереносом("Jeñildik somasy/Сумма скидки:",ШиринаСтроки, , Ложь);
			//Текст = Текст + ВыравнитьПоле("Jeñildik somasy/",ШиринаСтроки) + Символы.ПС; 
			Текст = Текст + ВыстроитьПоля("Jeñildiksiz/Без скидки:", ИтогоБезСкидка, ШиринаСтроки) + Символы.ПС;
			Текст = Текст + ВыстроитьПоля("Jeñildik/Скидка:","- " + Скидка, ШиринаСтроки) + Символы.ПС; 
			//Текст = Текст + ВыравнитьПоле("Jeñildik somasy/Сумма скидки:",ШиринаСтроки) + Символы.ПС;
			//Текст = Текст + ВыравнитьПоле(Скидка,ШиринаСтроки,"Право") + Символы.ПС;		КонецЕсли;
		КонецЕсли;
		...
		Текст = Текст + ВыстроитьПоля(ТипОплаты, ИтогоСумма, ШиринаСтроки) + Символы.ПС;
		...
		Текст = Текст + ВыстроитьПоля(ТипОплаты, НДС, ШиринаСтроки) + Символы.ПС;
		...
		Текст = Текст + ФискальныйЧек + Символы.ПС;
		Текст = Текст + КодККМ + Символы.ПС;
		Текст = Текст + ВремяЧека + Символы.ПС;
		Текст = Текст + АдресККМ + Символы.ПС;
		...
		Если ЗначениеЗаполнено(ОписаниеОФД) Тогда
			...
			Текст = Текст + СтрШаблон(СтруктураНаименованияПолей.ОператорФискальныхДанных+": %1", ОписаниеОФД.Name) + Символы.ПС;
			Текст = Текст + СтруктураНаименованияПолей.НадписьДляПроверкиЧекаЗайдитеНаСайт + ":" + Символы.ПС;
			Текст = Текст + ОписаниеОФД.Host + Символы.ПС;
		...
		Текст = Текст + СтруктураНаименованияПолей.НадписьФискальныйЧекФП + Символы.ПС;
		Текст = Текст + НСтр("ru='WEBKASSA.KZ'")+ Символы.ПС;

		
		//QR код
		Текст = Текст  + "НачалоСсылки" + СтруктураДанных.СсылкаНаЧек + "КонецСсылки" + Символы.ПС;   
...
//Процедура Расш1_ДобавитьМодификаторПредоплаты(ПараметрыЗапроса, Знач СуммаПредоплатыСписать)
//	
//	Если Не ПараметрыЗапроса.Свойство("TicketModifiers") Тогда
//		ПараметрыЗапроса.Вставить("TicketModifiers", Новый Массив);
//	КонецЕсли;
//	
//	Модификатор = Новый Структура;
//	Модификатор.Вставить("Sum",        СуммаПредоплатыСписать);
//	Модификатор.Вставить("Text",       "Бронь");
//	Модификатор.Вставить("Type",       1);
//	Модификатор.Вставить("TaxType",    0);
//	Модификатор.Вставить("TaxPercent", 0);
//	Модификатор.Вставить("Tax",        0);
//	
//	ПараметрыЗапроса.TicketModifiers.Добавить(Модификатор);
//	
//КонецПроцедуры

//// Добавляет в запрос v4/check модификатор скидки на весь чек - аналог
//// Расш1_ДобавитьМодификаторПредоплаты, но с реальным налогом (для облагаемых
//// НДС позиций), чтобы ОФД корректно показывал скидку и налог от суммы после скидки.
////
//// Параметры:
////  ПараметрыЗапроса - Структура - тело запроса /api/v4/check (по ссылке).
////  СуммаСкидки       - Число   - сумма скидки в тенге (положительное число).
////  СтавкаНДС         - Число   - ставка НДС позиций, к которым относится скидка
////                                (0, если позиции без НДС/освобождены).
////
//Процедура Расш1_ДобавитьМодификаторСкидки(ПараметрыЗапроса, Знач СуммаСкидки, Знач СтавкаНДС = 0) Экспорт
//	
//	Если СуммаСкидки = 0 Тогда
//		Возврат;
//	КонецЕсли;
...
//	
//	ТипНалога         = ?(СтавкаНДС = 0, 0, 100);
//	СуммаНалогаСкидки = ?(СтавкаНДС = 0, 0, Окр(СуммаСкидки * СтавкаНДС / (100 + СтавкаНДС), 2));
//	
//	Модификатор = Новый Структура;
//	Модификатор.Вставить("Sum",        СуммаСкидки);
//	Модификатор.Вставить("Text",       "Скидка");
//	Модификатор.Вставить("Type",       1); // 1 - скидка
//	Модификатор.Вставить("TaxType",    ТипНалога);
//	Модификатор.Вставить("TaxPercent", СтавкаНДС);
//	Модификатор.Вставить("Tax",        СуммаНалогаСкидки);
//	
//	ПараметрыЗапроса.TicketModifiers.Добавить(Модификатор);
//	
//КонецПроцедуры

//Процедура Расш2_ПрименитьСкидкуКПозициям(ПараметрыЗапроса, Знач МассивСкидокПоСтрокам)
//	
//	Если МассивСкидокПоСтрокам = Неопределено Или МассивСкидокПоСтрокам.Количество() = 0 Тогда
//		Возврат;
//	КонецЕсли;
//	
//	Если Не ПараметрыЗапроса.Свойство("Positions") Тогда
//		Возврат;
//	КонецЕсли;
//	
//	Позиции = ПараметрыЗапроса.Positions;
//	
//	Если Позиции = Неопределено Или Позиции.Количество() = 0 Тогда
//		Возврат;
//	КонецЕсли;
//	
//	Для Каждого СтрокаСкидки Из МассивСкидокПоСтрокам Цикл
//		
//		ИндексПозиции = СтрокаСкидки.НомерСтроки - 1;
//		
//		Если ИндексПозиции < 0 Или ИндексПозиции >= Позиции.Количество() Тогда
//			Продолжить;
//		КонецЕсли;
//		
//		Позиция = Позиции[ИндексПозиции];
//		
//		Если Не Позиция.Свойство("Discount") Тогда
//			Позиция.Вставить("Discount", 0);
//		КонецЕсли;
//		
//		Позиция.Discount = Позиция.Discount + СтрокаСкидки.СуммаСкидки;
//		
//		// Уменьшаем налог позиции пропорционально скидке, чтобы итоговый налог
//		// в чеке остался положительным и соответствовал сумме после скидки.
//		Если СтрокаСкидки.СтавкаНДС <> 0 И Позиция.Свойство("Tax") Тогда
//			СуммаНалогаСкидки = Окр(СтрокаСкидки.СуммаСкидки * СтрокаСкидки.СтавкаНДС / (100 + СтрокаСкидки.СтавкаНДС), 2);
//			Позиция.Tax = Макс(Позиция.Tax - СуммаНалогаСкидки, 0);
//		КонецЕсли;
//		
//	КонецЦикла;
//	
...
Функция Расш4_НормализоватьСтавкуНДС(Знач СтавкаНДС) Экспорт
	...
	// Справочник со свойством "Ставка" (числовой реквизит)
	Попытка
		Если СтавкаНДС.Свойство("Ставка") Тогда
			Возврат СтавкаНДС.Ставка;
		КонецЕсли;
	Исключение
	КонецПопытки;
	
	// Перечисление - извлекаем число из имени значения (НДС18, НДС18_118,
	// НДС10, НДС10_110, НДС0, БезНДС, НДС20, НДС20_120, НДС5, НДС5_105,
	// НДС7, НДС7_107, НДС12, НДС16 и подобные)
	Попытка
		ТекстСтавки = Строка(СтавкаНДС);
	Исключение
		Возврат 0;
	КонецПопытки;
	
	ТекстЦифр = "";
	Для Индекс = 1 По СтрДлина(ТекстСтавки) Цикл
		Символ = Сред(ТекстСтавки, Индекс, 1);
		Если Символ >= "0" И Символ <= "9" Тогда
			ТекстЦифр = ТекстЦифр + Символ;
		ИначеЕсли ТекстЦифр <> "" Тогда
			Прервать; // берём только первую группу цифр (до знака "_", если он есть)
		КонецЕсли;
	КонецЦикла;
	
	Если ТекстЦифр = "" Тогда
		Возврат 0;
	КонецЕсли;
	
	Возврат Число(ТекстЦифр);
	
КонецФункции

Процедура ДобавитьМодификаторПредоплаты(ПараметрыЗапроса, Знач СуммаПредоплатыСписать)
...
Процедура ВосстановитьЦеныВПозициях(ПараметрыЗапроса, Знач РеальныеСтроки)
	
	Если РеальныеСтроки = Неопределено Или РеальныеСтроки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Если Позиции = Неопределено Или Позиции.Количество() <> РеальныеСтроки.Количество() Тогда
		Возврат; // защита: количество не совпало (например, Свернуть схлопнула
		         // одинаковые номенклатуры) - безопасный отказ
	КонецЕсли;
	
	// Строим очередь реальных строк по каждому имени номенклатуры - позволяет
	// сопоставлять по имени, а не по (ненадёжному) индексу массива, так как
	// омг_WebKassa строит Positions через Свернуть(), не гарантирующую 
	// сохранение исходного порядка строк документа.
	мсОчередиПоИменам = Новый Соответствие;
	
	Для Каждого стр Из РеальныеСтроки Цикл
		Очередь = мсОчередиПоИменам[стр.Наименование];
		Если Очередь = Неопределено Тогда
			Очередь = Новый Массив;
			мсОчередиПоИменам.Вставить(стр.Наименование, Очередь);
		КонецЕсли;
		Очередь.Добавить(стр);
	КонецЦикла;
	
	Для Каждого Позиция Из Позиции Цикл
		
		Если Не Позиция.Свойство("Price") Или Не Позиция.Свойство("Discount") Или Не Позиция.Свойство("PositionName") Тогда
			Продолжить;
		КонецЕсли;
		
		Очередь = мсОчередиПоИменам[Позиция.PositionName];
		Если Очередь = Неопределено Или Очередь.Количество() = 0 Тогда
			Продолжить; // имя позиции не найдено среди строк документа - пропускаем
		КонецЕсли;
		
		СтрокаДок = Очередь[0];
		Очередь.Удалить(0); // "съедаем" из очереди - следующая позиция с тем же 
		                     // именем (если есть) получит следующую строку
		
		Если Позиция.Свойство("Count") И СтрокаДок.Свойство("Количество") Тогда
			Если Позиция.Count <> СтрокаДок.Количество Тогда
				Позиция.Count = СтрокаДок.Количество;
			КонецЕсли;
		КонецЕсли;
		
		Если Окр(Позиция.Price - СтрокаДок.Цена, 2) = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		Позиция.Price    = СтрокаДок.Цена;
		Позиция.Discount = СтрокаДок.СуммаСкидки;
...
Процедура СверитьИСкорректироватьОкругление(ПараметрыЗапроса)
	
	Если Не ПараметрыЗапроса.Свойство("Positions") Или Не ПараметрыЗапроса.Свойство("Payments") Тогда
		Возврат;
	КонецЕсли;
	
	СуммаПозиций = 0;
	Для Каждого Позиция Из ПараметрыЗапроса.Positions Цикл
		Цена     = ?(Позиция.Свойство("Price"),    Позиция.Price,    0);
		Количество = ?(Позиция.Свойство("Count"),    Позиция.Count,    1);
		Скидка   = ?(Позиция.Свойство("Discount"), Позиция.Discount, 0);
		Наценка  = ?(Позиция.Свойство("Markup"),   Позиция.Markup,   0);
		СуммаПозиций = СуммаПозиций + Окр(Цена * Количество, 2) - Скидка + Наценка;
	КонецЦикла;
	
	Если ПараметрыЗапроса.Свойство("TicketModifiers") Тогда
		Для Каждого Модификатор Из ПараметрыЗапроса.TicketModifiers Цикл
			Если Модификатор.Свойство("Type") И Модификатор.Type = 1 Тогда
				СуммаПозиций = СуммаПозиций - Модификатор.Sum;
			ИначеЕсли Модификатор.Свойство("Type") И Модификатор.Type = 2 Тогда
				СуммаПозиций = СуммаПозиций + Модификатор.Sum;
			КонецЕсли;
		КонецЦикла;
	...
	СуммаПлатежей = 0;
	Для Каждого Платеж Из ПараметрыЗапроса.Payments Цикл
		СуммаПлатежей = СуммаПлатежей + ?(Платеж.Свойство("Sum"), Платеж.Sum, 0);
	КонецЦикла;
	
	РазницаОкругления = Окр(СуммаПозиций - СуммаПлатежей, 2);
	...
	Модификатор = Новый Структура;
	Если РазницаОкругления > 0 Тогда
		// Позиции больше платежей - нужна дополнительная скидка на разницу
		Модификатор.Вставить("Sum",  РазницаОкругления);
		Модификатор.Вставить("Type", 1);
	Иначе
		// Позиции меньше платежей - нужна наценка на разницу
		Модификатор.Вставить("Sum",  -РазницаОкругления);
		Модификатор.Вставить("Type", 2);
	КонецЕсли;
	Модификатор.Вставить("Text",       "Округление");
	Модификатор.Вставить("TaxType",    0);
	Модификатор.Вставить("TaxPercent", 0);
	Модификатор.Вставить("Tax",        0);
