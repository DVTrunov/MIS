...
//Добавлено Карсаков. Функция возвращает основную организацию текущего пользователя
Функция ПолучитьОсновнуюОрганизациюПользователя() Экспорт
...
Функция ПолучитьПериодическийПараметрУчетнойПолитики(ИмяПараметра, Знач Дата = Неопределено) Экспорт
	...
	Параметр = Перечисления.ПараметрыУчетнойПолитики[ИмяПараметра];
	Отбор = Новый Структура("Параметр", Параметр);
	СрезПоследних = РегистрыСведений.УчетнаяПолитикаПериодическая.СрезПоследних(Дата, Отбор);
	
	Если СрезПоследних.Количество() = 0 Тогда
		Возврат ПолучитьЗначениеПараметраУчетнойПолитикиРасширеннойПоУмолчанию(ИмяПараметра);
	Иначе
		Если УправлениеНастройкамиПовтИсп.ПолучитьПараметрыУчетнойПолитикиРасширеннойХранилищеЗначений().Найти(ИмяПараметра) = Неопределено Тогда
			Возврат СрезПоследних[0].Значение;
		Иначе
			Возврат СрезПоследних[0].ХранилищеЗначения;
		КонецЕсли;
	КонецЕсли;
