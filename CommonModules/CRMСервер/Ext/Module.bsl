...
Функция ДобавитьСобытие(Знач ВидСобытия, Знач Дата = Неопределено, Знач Филиал = Неопределено, Знач СтруктураСобытия = Неопределено) Экспорт
	...
	КонецЕсли;
	Организация = УправлениеНастройками.ПолучитьОсновнуюОрганизациюПользователя();
	...
	//Добавлено Карсаков
	НовоеСобытие.омг_Организация = Организация;
...
Процедура ОбращениеПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	ИзменилсяИсходЗвонка		 = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Источник, "ИсходЗвонка") И Источник.ИсходЗвонка <> Источник.Ссылка.ИсходЗвонка;
