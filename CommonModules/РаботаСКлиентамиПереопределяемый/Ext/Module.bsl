...
Функция ЭтоГрупповаяЗаявка(ЗаявкаКлиент, ЭтоКлиент = Неопределено) Экспорт
	...
	Если ЭтоКлиент = Истина
		Или ЭтоКлиент = Неопределено И ТипЗнч(ЗаявкаКлиент) = Тип("СправочникСсылка.Клиенты")
	Тогда
		Клиент = ЗаявкаКлиент;
	Иначе
		Клиент = ЗаявкаКлиент.Клиент;
	КонецЕсли;
	
	Возврат Справочники.Клиенты.ПолучитьИмяПредопределенного(Клиент) = "Групповая_заявка";
...
Процедура ОказаниеУслугПриСозданииНаСервере(Объект,СтруктураПараметров,Параметры) Экспорт
	...
	Если Объект.Ссылка.Пустая()
		...
		И ЭтоГрупповаяЗаявка(Объект.ДокументОснование, Ложь)
	Тогда
		СтруктураПараметров.Вставить("ВводОУПоГрупповойЗаявке", Истина);
		Если Параметры.Свойство("ЗначенияЗаполнения")
			И Параметры.ЗначенияЗаполнения.Свойство("Клиент")
		Тогда
			Объект.Клиент = Параметры.ЗначенияЗаполнения.Клиент;
		ИначеЕсли Объект.Клиент = Справочники.Клиенты.Групповая_заявка Тогда
...
Функция ПолучитьПосещенияЗаявки(Заявка, ВернутьНаСторонуКлиента = Истина) Экспорт 
	
	ТаблицаПосещений = Новый ТаблицаЗначений;
	ТаблицаПосещений.Колонки.Добавить("Клиент", Новый ОписаниеТипов("СправочникСсылка.Клиенты"));
	ТаблицаПосещений.Колонки.Добавить("Документ", Новый ОписаниеТипов("ДокументСсылка.ОказаниеУслуг"));
	
	Если ЭтоГрупповаяЗаявка(Заявка, Ложь)
		И Заявка.Клиенты.Количество() <> 0 
	Тогда
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Клиент", ?(ТипЗнч(Заявка) = Тип("ДанныеФормыСтруктура"), Заявка.Клиенты.Выгрузить().ВыгрузитьКолонку("Клиент"), Заявка.Клиенты.ВыгрузитьКолонку("Клиент")));
		...
		|ГДЕ
		|	ОказаниеУслуг.Клиент В(&Клиент)
		|	И ОказаниеУслуг.ДокументОснование = &Заявка"
		;
		Выб = Запрос.Выполнить().Выбрать();
		Пока Выб.Следующий() Цикл
			ЗаполнитьЗначенияСвойств(ТаблицаПосещений.Добавить(), Выб);
		КонецЦикла;
	КонецЕсли;
	
	Если ВернутьНаСторонуКлиента Тогда
		Возврат ОбщегоНазначения.ТаблицаЗначенийВМассив(ТаблицаПосещений);
	Иначе
...
Процедура ПриемПриСозданииНаСервере(Объект,СтруктураПараметров,Параметры) Экспорт
	...
	Если Объект.Ссылка.Пустая()
		И ЗначениеЗаполнено(Объект.ДокументОснование) 
		И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.Заявка")
		И ЭтоГрупповаяЗаявка(Объект.Клиент, Истина)
	Тогда
		СтруктураПараметров.Вставить("ВводОУПоГрупповойЗаявке", Истина);
		Если Параметры.Свойство("ЗначенияЗаполнения")
			И Параметры.ЗначенияЗаполнения.Свойство("Клиент")
		Тогда
			Объект.Клиент = Параметры.ЗначенияЗаполнения.Клиент;
		Иначе
			Объект.Клиент = Справочники.Клиенты.ПустаяСсылка();
		КонецЕсли;
	КонецЕсли;
...
Процедура ЖурналЗаписиПолучитьВизуальныеНастройки(ОсновнаяНастройкаЯчеек, Результат) Экспорт
	...
	Результат.Вставить("ВыводитьФИОГрупповых", ОсновнаяНастройкаЯчеек.фВыводГрупповых);
	Результат.Вставить("ЕстьПереопределяемоеФормированиеПодстрокиЯчейкиКлиента", Истина); 
...
Функция ЖурналЗаписиСформироватьПодстрокуЯчейкиКлиента(ПараметрыВызова, СтандартнаяОбработка) Экспорт
	...
	ДанныеКлиента	= ПараметрыВызова.ДанныеКлиента;
	Ячейка			= ПараметрыВызова.Ячейка;
	ВыбРаботыБлока	= ПараметрыВызова.ВыборкаРаботыБлока;
	ВыводитьФИОГрупповых = ПараметрыВызова.СтруктураВизуальныхНастроек.ВыводитьФИОГрупповых;
	
	Значение = "";
	
	Если ЭтоГрупповаяЗаявка(ДанныеКлиента.Клиент, Истина) Тогда
		Если Ячейка = Справочники.ЯчейкиКалендаря.НаименованиеКлиента Тогда

			СтандартнаяОбработка = Ложь;
			Если ВыводитьФИОГрупповых Тогда
				ВыбРаботыБлока.Сбросить();
				ВыбРаботыБлока.Следующий();
				ТабКлиенты = ВыбРаботыБлока.ДокументСсылка.Клиенты;
				Для Каждого Строка Из ТабКлиенты Цикл
					Значение = Значение + Строка(Строка.Клиент.Наименование) + Символы.ВК;	
				КонецЦикла;					
			КонецЕсли;
			
		ИначеЕсли Ячейка = Справочники.ЯчейкиКалендаря.ФамилияИОКлиента Тогда
			...
			Если ВыводитьФИОГрупповых Тогда
				...
				ТабКлиенты = ВыбРаботыБлока.ДокументСсылка.Клиенты;
				Для Каждого Строка Из ТабКлиенты Цикл
					Значение = Значение + Символы.ВК + ОбщегоНазначения.ФамилияИнициалыФизЛица(Строка.Клиент);	
				КонецЦикла;					
			КонецЕсли;
			
		ИначеЕсли Ячейка = Справочники.ЯчейкиКалендаря.ФамилияКлиента Тогда
			...
			Если ВыводитьФИОГрупповых Тогда
				...
				ТабКлиенты = ВыбРаботыБлока.ДокументСсылка.Клиенты;
				Для Каждого Строка Из ТабКлиенты Цикл
					Значение = Значение + Символы.ВК + Строка.Клиент.Фамилия;	
				КонецЦикла;					
			КонецЕсли;
			
		ИначеЕсли Ячейка = Справочники.ЯчейкиКалендаря.ГодРождения Тогда
