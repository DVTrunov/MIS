...
Функция ЗаменитьСсылкиОбщая(Знач ПарыЗамен, Знач Параметры = Неопределено) Экспорт
	...
	// +ОЛИМП
	ОбъединениеКлиентов = Ложь;
	Для Каждого КлючИЗначение Из ПарыЗамен Цикл
		Если ТипЗнч(КлючИЗначение.Ключ) = Тип("СправочникСсылка.Клиенты") Тогда
			ОбъединениеКлиентов = Истина;
		Иначе
			Прервать;
		КонецЕсли;
		Если КлючИЗначение.Значение <> КлючИЗначение.Ключ Тогда 
			доп_ПолныеПрава.ОбъединитьДвухПациентов(КлючИЗначение.Значение, КлючИЗначение.Ключ);
		КонецЕсли;
	КонецЦикла; 
	Если ОбъединениеКлиентов Тогда
	...
	Тогда
		...
		Для Каждого Дубль Из Дубли Цикл
			доп_ПолныеПрава.ПереместитьФайлыОбъекта(ПарыЗамен[Дубль],Дубль,УчетнаяПолитика.БазовыйКаталогФайлов);		
		КонецЦикла;	
