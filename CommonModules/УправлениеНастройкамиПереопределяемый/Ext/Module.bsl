...
Процедура ПриПолученииУчетнойПолитики(УчетнаяПолитика) Экспорт
	...
	ПолучитьУчетнуюПолитику(УчетнаяПолитика);
...
// Дополняет структуру полученной из базы учетной политики
//
// Параметры:
//  мУчетнаяПолитика - Структура - структура учетной политики.
//
Процедура ПолучитьУчетнуюПолитику(мУчетнаяПолитика)
	
	нзУП = РегистрыСведений.НастройкиМедицинскогоУчета.СоздатьНаборЗаписей();
	нзУП.Прочитать();
	Запись = ?(нзУП.Количество() > 0, нзУП[0], Неопределено);
	
	РесурсыРегистра = нзУП.Метаданные().Ресурсы;
	Для Каждого Ресурс Из РесурсыРегистра Цикл
		
		Если Не мУчетнаяПолитика.Свойство(Ресурс.Имя) Тогда
			Если Запись <> Неопределено Тогда
				Значение = Запись[Ресурс.Имя];
			Иначе
				МетаданныеРесурса = РесурсыРегистра[Ресурс.Имя];
				Значение = МетаданныеРесурса.Тип.ПривестиЗначение(Неопределено);
			КонецЕсли;
			
			мУчетнаяПолитика.Вставить(Ресурс.Имя, Значение);
		КонецЕсли;
	КонецЦикла;
