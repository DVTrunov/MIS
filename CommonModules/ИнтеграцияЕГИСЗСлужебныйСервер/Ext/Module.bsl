...
Функция СформироватьИОтправитьSOAPЗапрос(Адрес, ТелоСообщения, ИдентификаторКлиента, ТипПакета = Неопределено, MSG_ID = Неопределено, СообщениеРЭМД = Неопределено) Экспорт
	...
	ТекстЗапроса = ПоместитьСообщениеВSOAPКонверт(Адрес, ТелоСообщения, ИдентификаторКлиента, ТипПакета);
	
	Если СообщениеРЭМД <> Неопределено Тогда
		МедицинскаяОрганизация = СообщениеРЭМД.Документ.Филиал.Организация.МедицинскаяОрганизация;
	Иначе
		МедицинскаяОрганизация = Неопределено;
	КонецЕсли;
	
	Сертификат = ИнтеграцияЕГИСЗСерверПовтИсп.ПолучитьСертификатПодписанияСообщенийSOAP(, МедицинскаяОрганизация);
	Если ЗначениеЗаполнено(Сертификат) Тогда
		ТекстЗапроса = ИнтеграцияЕГИСЗ_ЭЦП.ПодписатьSoapСообщение(ТекстЗапроса, Сертификат);
	КонецЕсли;
	
	Возврат ОтправитьПоHTTP(Хост, ПутьНаСервере, ТекстЗапроса, ИспользоватьHttps, MSG_ID, СообщениеРЭМД);
...
Процедура ЗаписатьВАрхивЛогов(СтруктураДляЛогов) Экспорт
	...
	Филиал = ОбщегоНазначенияБИТКлиентСервер.СвойствоОбъекта(СтруктураДляЛогов, "Филиал");
	МедицинскаяОрганизация = ОбщегоНазначенияБИТКлиентСервер.СвойствоОбъекта(СтруктураДляЛогов, "МедицинскаяОрганизация");
	ПутьКАрхивуЛогов = ИнтеграцияЕГИСЗСерверПовтИсп.ПолучитьПутьКАрхивуЛогов(Филиал, МедицинскаяОрганизация);
...
Функция ЕстьОшибкиЗаполненностиЭлементаКлассификатора(Элемент, ТекстОшибки)
	...
	Если ОбщегоНазначенияСервер.РежимРасширенныхВозможностейРедактированияДанных()
		Или ОбщегоНазначенияБИТКлиентСервер.СвойствоОбъекта(Элемент, "Архив", Элемент.ПометкаУдаления)
	Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ПоместитьСообщениеВSOAPКонверт(URI, ТелоСообщения, ИдентификаторКлиента, ТипПакета)
	
	Результат = ТекстXMLSOAPКонверт(Не ЗначениеЗаполнено(ТипПакета));
	
	Результат = стрШаблон(Результат,
					ИдентификаторКлиента, // Идентификатор клиента
					ТипПакета,
					Строка(Новый УникальныйИдентификатор), // Идентификатор сообщения
					URI,
					ТелоСообщения);
...
Функция ОтправитьПоHTTP(Хост, АдресРесурса, ТекстЗапроса, ИспользоватьHttps, MSG_ID, СообщениеРЭМД)
	...
	Если СообщениеРЭМД <> Неопределено Тогда
		ИдентификаторЗапроса = Строка(СообщениеРЭМД.УникальныйИдентификатор());
	ИначеЕсли MSG_ID <> Неопределено Тогда
		ИдентификаторЗапроса = MSG_ID;
	КонецЕсли;
	
	Если ИдентификаторЗапроса <> Неопределено Тогда
		ЗаписатьВАрхивЛогов(Новый Структура("УИДСообщения, ТекстЗапроса", ИдентификаторЗапроса, HTTPЗапрос.ПолучитьТелоКакСтроку()));
	...
	Если ИдентификаторЗапроса <> Неопределено Тогда
		ЗаписатьВАрхивЛогов(Новый Структура("УИДСообщения, ТекстЗапроса", ИдентификаторЗапроса, HTTPОтвет.ПолучитьТелоКакСтроку()));
	ИначеЕсли ИнтеграцияЕГИСЗСерверПовтИсп.СохранятьЛогСинхронныхСообщенийЕГИСЗ() Тогда
...
Функция ПолучитьСтрокуПоXPath(ДокументDOM, Знач Разыменователь = Неопределено, XPath, ТипРезультата = Неопределено) Экспорт
	...
	Если ТипРезультата = ТипРезультатаDOMXPath.Число Тогда
		Возврат РезультатXPath.ЧисловоеЗначение;
	ИначеЕсли ТипРезультата = ТипРезультатаDOMXPath.Булево Тогда
		Возврат РезультатXPath.БулевоЗначение;
	Иначе
