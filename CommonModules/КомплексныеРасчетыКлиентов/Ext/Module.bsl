...
// Для документа Оказание услуг выполняет движения по регистру "Номенклатура комплексных расчетов клиентов".
//
// Параметры:
//  Отказ			 - Булево						 - признак отказа проведения документа
//  ДокументОбъект	 - ДокументОбъект.ОказаниеУслуг	 - проводимый документ
//  ПозицииДокумента - ТаблицаЗначений				 - таблица с колонками "Номенклатура, ХарактеристикаНоменклатуры, ЕдиницаИзмерения, Цена, Количество".
//
Процедура ОказаниеУслуг_ДвиженияПоРегиструНоменклатураКомплексныхРасчетовКлиентов(Отказ, ДокументОбъект, ПозицииДокумента) Экспорт
	
	ЭтоПродажа = ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийОказанияУслуг.ПродажаКлиенту;
	
	// Отказ не передается из расчета, что единственная проверка - нехватка,
	// которая в данном документе усложнена пост-учетом коррекции.
	ТаблицаДвижений = КомплексныеРасчетыКлиентов.ПроверитьВозможностьСписанияПозицийКомплексногоРасчета(ДокументОбъект, 
																										ДокументОбъект.МоментВремени(),
																										ДокументОбъект.КомплексныйРасчет,
																										Ложь, ПозицииДокумента, , ЭтоПродажа);
	
	// Учет сторно-движений при коррекции
	Если ДокументОбъект.Коррекция Тогда
		СторноДвижения = ДокументОбъект.Движения.НоменклатураКомплексныхРасчетовКлиентов;
		Отбор = Новый Структура("Номенклатура, ХарактеристикаНоменклатуры, ЕдиницаИзмерения, Цена");
		Для Каждого СторноДвижение Из СторноДвижения Цикл
			Если СторноДвижение.ВидДвижения = ВидДвиженияНакопления.Расход
				И (ЭтоПродажа И СторноДвижение.Количество < 0 Или Не ЭтоПродажа И СторноДвижение.Количество > 0)
			Тогда
				ЗаполнитьЗначенияСвойств(Отбор, СторноДвижение.КлючСтроки);
				ПозицииПодСторно = ТаблицаДвижений.НайтиСтроки(Отбор);
				Остаток = ?(ЭтоПродажа, -СторноДвижение.Количество, СторноДвижение.Количество);
				Для Каждого СтрокаПозиции Из ПозицииПодСторно Цикл
					НаСколькоУменьшитьНехватку = Мин(СтрокаПозиции.Нехватка, Остаток);
					СтрокаПозиции.Нехватка = СтрокаПозиции.Нехватка - НаСколькоУменьшитьНехватку;
					Остаток = Остаток - НаСколькоУменьшитьНехватку;
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если ТаблицаДвижений.Итог("Нехватка") <> 0 Тогда
		ОбщегоНазначения.СообщитьОбОшибке(КомплексныеРасчетыКлиентов.СформироватьСообщениеОшибкиОстаткаПоКомплексномуРасчету(ТаблицаДвижений, ЭтоПродажа), Отказ);
	КонецЕсли;
	
	Если Не Отказ Тогда
		Для Каждого СтрокаТаблицы Из ТаблицаДвижений Цикл
			Движение = ДокументОбъект.Движения.НоменклатураКомплексныхРасчетовКлиентов.ДобавитьРасход();
			Движение.Период = ДокументОбъект.Дата;
			Движение.КомплексныйРасчет = ДокументОбъект.КомплексныйРасчет;
