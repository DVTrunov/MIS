...
Функция КонтактнаяИнформация(Источник, Отбор) Экспорт
	...
	Если КонтактнаяИнформацияСсылки.Количество() > 0 Тогда
		...
		//
		Запрос.УстановитьПараметр("КонтактнаяИнформацияСсылки",КонтактнаяИнформацияСсылки);
		...
		|ГДЕ КонтактнаяИнформация.Объект В (&КонтактнаяИнформацияСсылки)
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеГдеТипКонтактнойИнформации",
			?(ТипыКонтактнойИнформации = Неопределено, "ИСТИНА", "КонтактнаяИнформация.Тип В (&ТипыКонтактнойИнформации)"));
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеГдеВидКонтактнойИнформации",
			?(ВидыКонтактнойИнформации = Неопределено, "ИСТИНА", "КонтактнаяИнформация.Вид В (&ВидыКонтактнойИнформации)"));
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Форма, Объект, Отказ) Экспорт
	...
	//+ОЛИМП не используется //ПараметрыСеанса.ИнтерактивнаяПроверкаЗаполненияКонтактнойИнформации = Истина;
...
Процедура УстановитьСвойстваВидаКонтактнойИнформации(Параметры) Экспорт
	...
	Если ПустаяСтрока(Объект.ИдентификаторДляФормул) Тогда
		//+ОЛИМП
		Объект.ИдентификаторДляФормул = "ВКИ"+СокрЛП(Объект.Код);
