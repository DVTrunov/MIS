...
Функция ПолучитьДвоичныеДанныеФайлаЭМД(Ссылка)
	...
	ДанныеФайлов = доп_РаботаСФайлами.ПолучитьДанныеФайловЭМД(Ссылка);
...
Функция ИмеютсяПодписиФайлов(МедицинскийДокумент)
	...
	Тогда
		Возврат доп_РаботаСФайлами.ФайлЭМДСохранен(МедицинскийДокумент);
	КонецЕсли;
...
Процедура ПроверитьПодписиПослеПроверкиСтроки(Результат, ДополнительныеПараметры) Экспорт
	...
	доп_СертификатыЭЦП.ОбновитьСтатусПроверкиПодписи(ДанныеПроверкиПодписи);
	// -ОЛИМП
...
Процедура ПроверитьПодписиПослеПодготовкиДанных(Данные, ДополнительныеПараметры)
	...
	Если ИспользоватьЭлектроннуюПодписьВМоделиСервиса
	 Или Не МодульЭлектроннаяПодписьКлиент.ПроверятьЭлектронныеПодписиНаСервере() Тогда
		...
		// ОЛИМП.Начало.Изменили, чтобы всегда использовался алгоритм модели сервиса с получением алгоритма подписи перед получением менеджера криптографии.
