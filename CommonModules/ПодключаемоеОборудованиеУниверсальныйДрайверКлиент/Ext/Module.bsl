...
Функция ПодключитьУстройство(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	// Добавлено Макеев (
	// Меняем ширину ленты на ту, которая указана для печати направления в ЛИС
	ЭтоПечатьНаправления = омг_ПечатьНаправленияСервер.ПрочитатьВременныеНастройкиПечати(Параметры.Идентификатор);
	Если ЭтоПечатьНаправления Тогда
		ШиринаЛенты = омг_ПечатьНаправленияСервер.ПолучитьШиринуЛенты();
		Параметры.P_LineLength = ШиринаЛенты;
	КонецЕсли;
	// )
...
Функция ЗакрытьСмену(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	////////////////////////////////////////////////////////////////////////
	// ОМГ
	Если не ВходныеПараметры.Свойство("КассирИНН") Тогда 	
	ВходныеПараметры.Вставить("КассирИНН","")	
	КонецЕсли;
	// ОМГ
	////////////////////////////////////////////////////////////////////////	
	...
	ИначеЕсли ПараметрыПодключения.ТипОборудования = "ККТ" Или ПараметрыПодключения.ТипОборудования = "ПринтерЧеков" Тогда
		...
		Попытка
			Для Каждого ТекстПечати Из ДанныеЧека Цикл 
				...
				Если СтрНайти(ТекстПечати,"WEBKASSA.KZ") = 0 Тогда
					Результат = ОбъектДрайвера.НапечататьТекстовыйДокумент(ПараметрыПодключения.ИДУстройства, ТекстПечати);
				Иначе
					ТабДок = омг_WebKassa.ПолучитьТабличныйДокументДляПечати(Истина,СтрокаТекста); 
					ТабДок.ИмяПринтера = Параметры.P_WindowsPrinterName;
				...
				Иначе
					...
					ВыходныеПараметры.Добавить(0);
					ВыходныеПараметры.Добавить(МенеджерОборудованияКлиентПереопределяемый.ДатаСеанса());
					ВыходныеПараметры.Добавить(ТекстПечати);
...
Функция ОткрытьЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТипЧека, ФискальныйЧек, ВыходныеПараметры, ПараметрыОткрытияЧека = Неопределено) Экспорт
	...
	Если ПараметрыОткрытияЧека <> Неопределено И ПараметрыОткрытияЧека.Количество() > 0 Тогда
		...
		ДанныеПервичногоЧека = ПараметрыОткрытияЧека[5];
		
		Если ДанныеПервичногоЧека = Неопределено Тогда
			
			НомерЧекаОснования 			= "";
			ДатаВремяЧекаОснования 		= Дата("00010101");
			РНК 						= "";
			ИтогоОснования 				= 0;
			АвтономныйРежимОснования 	= Ложь;
			
		Иначе
			
			НомерЧекаОснования			= ДанныеПервичногоЧека.НомерЧека;
			ДатаВремяЧекаОснования 		= ДанныеПервичногоЧека.ДатаВремяЧека;
			РНК							= ДанныеПервичногоЧека.РегистрационныйНомерКассы;
			ИтогоОснования				= ДанныеПервичногоЧека.ОбщаяСуммаЧека;
			АвтономныйРежимОснования	= ДанныеПервичногоЧека.АвтономныйРежимРаботыКассы;
	...
	ИначеЕсли ПараметрыОткрытияЧека = Неопределено Тогда
		...
		Если Параметры.Свойство("ДанныеПервичногоЧека") Тогда 
			НомерЧекаОснования			= ?(Параметры.ДанныеПервичногоЧека.Свойство("НомерЧека"), 					Параметры.ДанныеПервичногоЧека.НомерЧека, 					"");
			ДатаВремяЧекаОснования 		= ?(Параметры.ДанныеПервичногоЧека.Свойство("ДатаВремяЧека"), 				Параметры.ДанныеПервичногоЧека.ДатаВремяЧека, 				Дата("00010101"));
			РНК							= ?(Параметры.ДанныеПервичногоЧека.Свойство("РегистрационныйНомерКассы"), 	Параметры.ДанныеПервичногоЧека.РегистрационныйНомерКассы, 	"");
			ИтогоОснования				= ?(Параметры.ДанныеПервичногоЧека.Свойство("ОбщаяСуммаЧека"), 				Параметры.ДанныеПервичногоЧека.ОбщаяСуммаЧека, 				0);
			АвтономныйРежимОснования	= ?(Параметры.ДанныеПервичногоЧека.Свойство("АвтономныйРежимРаботыКассы"), 	Параметры.ДанныеПервичногоЧека.АвтономныйРежимРаботыКассы, 	Ложь);
		КонецЕсли;
	КонецЕсли;

	Попытка
		
		Если ПараметрыПодключения.Свойство("РевизияИнтерфейса") И ПараметрыПодключения.РевизияИнтерфейса >= 2010 Тогда
			Ответ = ОбъектДрайвера.ОткрытьЧек(ПараметрыПодключения.ИДУстройства, ФискальныйЧек, ТипЧека, Истина, НомерЧека, НомерСмены, 
			ПолучательИИН, ПокупательEmail, ПокупательНомер, НомерЧекаОснования, ДатаВремяЧекаОснования, РНК, ИтогоОснования, АвтономныйРежимОснования);
		ИначеЕсли ПараметрыПодключения.Свойство("РевизияИнтерфейса") И ПараметрыПодключения.РевизияИнтерфейса >= 2008 Тогда
			Ответ = ОбъектДрайвера.ОткрытьЧек(ПараметрыПодключения.ИДУстройства, ФискальныйЧек, ТипЧека, Истина, НомерЧека, НомерСмены, 
...
Функция ЗакрытьЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТаблицаОплат, ВыходныеПараметры) Экспорт
	...
	//Выходные параметры ревизия интерфеёса 2010
	ФискальныйНомер 		= "";
	ЧекВАвтономномРежиме 	= Ложь;
	РНК 					= "";
	ДатаВремяЧека 			= Дата("00010101");
	...
	Попытка  
		Если ПараметрыПодключения.РевизияИнтерфейса >=2010 Тогда
			Ответ = ОбъектДрайвера.ЗакрытьЧек(ПараметрыПодключения.ИДУстройства, СуммаНаличнойОплаты, СуммаБезналичнойОплаты1, СуммаБезналичнойОплаты2, СуммаБезналичнойОплаты3, СуммаБезналичнойОплаты4, 
												ФискальныйНомер, ДатаВремяЧека, РНК, ЧекВАвтономномРежиме);
												
			СтруктураДанныхЧекаККМ = Новый Структура;
			СтруктураДанныхЧекаККМ.Вставить("ФискальныйНомер", ФискальныйНомер);
			СтруктураДанныхЧекаККМ.Вставить("АвтономныйРежимРаботы", ЧекВАвтономномРежиме);
			СтруктураДанныхЧекаККМ.Вставить("РегистрационныйНомерККМ", РНК);
			СтруктураДанныхЧекаККМ.Вставить("ДатаЧека", ДатаВремяЧека);
			
			ВыходныеПараметры.Добавить(СтруктураДанныхЧекаККМ);
		ИначеЕсли ПараметрыПодключения.РевизияИнтерфейса >=2009 Тогда
			Ответ = ОбъектДрайвера.ЗакрытьЧек(ПараметрыПодключения.ИДУстройства, СуммаНаличнойОплаты, СуммаБезналичнойОплаты1, СуммаБезналичнойОплаты2, СуммаБезналичнойОплаты3, СуммаБезналичнойОплаты4);
		Иначе
...
Функция НапечататьФискальнуюСтроку(ОбъектДрайвера, Параметры, ПараметрыПодключения, Наименование, Количество, Цена, Сумма,
	...
	Если ПараметрыПодключения.Свойство("РевизияИнтерфейса") И ПараметрыПодключения.РевизияИнтерфейса >= 2010 Тогда
			...
			Сумма, НомерСекции, СтавкаНДС, ОсвобожденныйОборотНДС, КодЕдиницыИзмерения, КодМаркировки, ИдентификаторТовара, ШтрихКод, КодНКТ);
