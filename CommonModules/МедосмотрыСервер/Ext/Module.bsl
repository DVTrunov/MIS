...
Процедура ЗаписатьОтметкиВыполненияДействияМедосмотра(ДокументРегистратор, ПрохождениеМедОсмотра, ТаблицаОтметок, Период = Неопределено) Экспорт
	...
	Для Каждого СтрокаОтметки Из ТаблицаОтметок Цикл
		...
		Если ТаблицаОтметки.Количество() > 0 Тогда
			...
			Тогда
				// Прием может делать запись только если ставит отметку выполнения
				Если ТипЗнч(ДокументРегистратор) = Тип("ДокументСсылка.Прием") 
					И СтрокаОтметки.Выполнено
				Тогда
...
Функция ПолучитьМассивПриказовПоВидуМО(ВидМедосмотра, ВернутьЗначениеДопПараметра = Ложь) Экспорт
	...
	Запрос.Текст = 
		...
		|	ВидыМедосмотровПриказы.ИспользоватьБезВФ КАК ИспользоватьБезВФ
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		Если ВернутьЗначениеДопПараметра Тогда
			СтрДанные = Новый Структура("ВредныйФактор, ИспользоватьБезВФ", Выборка.Приказ, Выборка.ИспользоватьБезВФ);
...
Функция ПриемыПоДействиямМедосмотров(ПрохождениеМедОсмотра, СписокДействий) Экспорт
	...
	Запрос.Текст = 
		...
		|	И ОтметкиВыполненияДействийСрезПоследних.Действие В(&СписокДействий)
		|	И ТИПЗНАЧЕНИЯ(ОтметкиВыполненияДействийСрезПоследних.ДокументРегистратор) = ТИП(Документ.Прием)";
...
Функция ДанныеЗаполненияЧисленностиИзСПМО(СПМО) Экспорт
	
	Результат = Новый Структура("Кол, КолЖ, Кол18", 0, 0, 0);
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СПМОКлиенты.Клиент КАК Клиент,
	|	СПМОКлиенты.ТипМедосмотра КАК ТипМедосмотра
	|ПОМЕСТИТЬ СПМО_Клиенты
	|ИЗ
	|	Документ.СписокНаПрохождениеМедосмотра.Клиенты КАК СПМОКлиенты
	|ГДЕ
	|	СПМОКлиенты.Ссылка = &СПМО
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СПМО_Клиенты.Клиент КАК Клиент,
	|	ВЫБОР
	|		КОГДА СПМО_Клиенты.Клиент.Пол = ЗНАЧЕНИЕ(Перечисление.ПолФизическихЛиц.Женский)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ПолЖ,
	|	ВЫБОР
	|		КОГДА РАЗНОСТЬДАТ(СПМО_Клиенты.Клиент.ДатаРождения, &ДатаСписка, ГОД) < 18
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК До18
	|ИЗ
	|	СПМО_Клиенты КАК СПМО_Клиенты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПрохождениеМедосмотра КАК ПМО
	|		ПО (ПМО.СписокНаПрохождениеМедосмотра = &СПМО)
	|			И (НЕ ПМО.ПометкаУдаления)
	|			И СПМО_Клиенты.Клиент = ПМО.Клиент
	|ГДЕ
	|	ЕСТЬNULL(ПМО.ТипМедосмотра, СПМО_Клиенты.ТипМедосмотра) <> &ТипМедосмотраПредварительный
	|ИТОГИ ПО
	|	Клиент";
	
	Запрос.УстановитьПараметр("ТипМедосмотраПредварительный", Перечисления.ТипыМедосмотров.Предварительный);
	Запрос.УстановитьПараметр("СПМО", СПМО);
	Запрос.УстановитьПараметр("ДатаСписка", СПМО.Дата);
	
	ВыбКлиенты = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыбКлиенты.Следующий() Цикл
		Выб = ВыбКлиенты.Выбрать();
		Выб.Следующий();
		
		Результат.Кол = Результат.Кол + 1;
		
		Если Выб.ПолЖ Тогда
			Результат.КолЖ = Результат.КолЖ + 1;
		КонецЕсли;
		
		Если Выб.До18 Тогда
			Результат.Кол18 = Результат.Кол18 + 1;
		КонецЕсли;
...
Функция ПолучитьПриказыВредныеФакторыДляТаблицыИдентификаторовПоСсылке(ПМО) Экспорт
	
	Возврат ПолучитьПриказыВредныеФакторыДляТаблицыИдентификаторов(ПМО, ПМО.Филиал, ПМО.Дата, ПМО.Номенклатура, ПМО.ФакторыИУсловияРаботы);
	
КонецФункции

Функция ПолучитьПриказыВредныеФакторыДляТаблицыИдентификаторов(Ссылка, Филиал, Дата, Номенклатура, Знач ФакторыИУсловияРаботы) Экспорт
	
	ДополнительныеПараметры = Новый Структура("Филиал, Дата, Номенклатура, ТаблицаФакторовИУсловийРабот", Филиал, Дата, Номенклатура, ФакторыИУсловияРаботы);
	Возврат ПолучитьМассивПриказовИВредныхФакторовПрохожденияМедосмотра(Ссылка, ДополнительныеПараметры);
...
Процедура ОбновитьСписокПараметровДокументаМедосмотраСправки(Знач ДействияМедосмотра, ПараметрыПриема, Знач Документ, Номенклатура = Неопределено, 
	...
	ПараметрыДействий = ПараметрыПриемаДействийМедосмотра(ДействияМедосмотра, Номенклатура, ПрохождениеМедОсмотра);
...
Функция ОбновитьСоставСправокПоСтрокеСервер(Номенклатура, Клиент, ДатаДокумента, Прейскурант) Экспорт
	...
	Если ОбщегоНазначения.ПолучитьЗначениеРеквизита(Номенклатура, "МедосмотрСправка") = Истина Тогда
		...
		Тогда
			...
			ТЗДействий = ПолучитьСписокУслугПоКлиенту(Неопределено, Перечисления.ТипыМедосмотров.ПустаяСсылка(), Клиент, ВредныеФакторы, ДатаДляСправки, Прейскурант, Истина);
...
Процедура УстановитьОтметкиДляДействийПМОПриСозданииДокументаИзВнЛаборатории(ТЗДляПростановкиОтметокПМО) Экспорт
	...
	Запрос.Текст = 
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПрохождениеМедосмотра.ДействияМедосмотра КАК ПрохождениеМедосмотраДействияМедосмотра
		|		ПО  ПрохождениеМедосмотраДействияМедосмотра.Ссылка.Дата >= &ДатаАктуальностиСтарогоМедосмотра
		|			И ДанныеЗаказовНеобходимыхОбработать.Ссылка.Клиент = ПрохождениеМедосмотраДействияМедосмотра.Ссылка.Клиент
	...
	ДатаАктуальностиСтарогоМедосмотра = ДобавитьМесяц(ТекущаяДата(), -12);
	Запрос.УстановитьПараметр("ДатаАктуальностиСтарогоМедосмотра", ДатаАктуальностиСтарогоМедосмотра);
...
Процедура УстановитьОтметкиДляДействийПМОИзЛаборатоныхИследований(КлиентИсследования, ИсследованияКлиента, ПриемСсылка) Экспорт
	...
	Запрос.Текст = 
		...
		|	Документ.ПрохождениеМедосмотра.ДействияМедосмотра КАК ПрохождениеМедосмотраДействияМедосмотра
		|ГДЕ
		|	ПрохождениеМедосмотраДействияМедосмотра.Ссылка.Дата >= &ДатаАктуальностиСтарогоМедосмотра
		|	И ПрохождениеМедосмотраДействияМедосмотра.Ссылка.Клиент = &Клиент
		|	И ПрохождениеМедосмотраДействияМедосмотра.Действие.Номенклатура В(&ИсследованияКлиента)
		|	И ПрохождениеМедосмотраДействияМедосмотра.Ссылка.ДатаЗавершения = ДАТАВРЕМЯ(1, 1, 1)
		|
		|УПОРЯДОЧИТЬ ПО
		|	ПрохождениеМедосмотраДействияМедосмотра.Ссылка.Дата";
	...
	ДатаАктуальностиСтарогоМедосмотра = ДобавитьМесяц(ТекущаяДата(), -12);
	Запрос.УстановитьПараметр("ДатаАктуальностиСтарогоМедосмотра", ДатаАктуальностиСтарогоМедосмотра);
	Запрос.УстановитьПараметр("Клиент", КлиентИсследования);
	Запрос.УстановитьПараметр("ИсследованияКлиента", ИсследованияКлиента);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		МЗ = РегистрыСведений.ОтметкиВыполненияДействий.СоздатьМенеджерЗаписи();
		МЗ.Период = ПриемСсылка.Дата;
		МЗ.Выполнено = Истина;
		МЗ.АктивностьЗаписи = Истина;
		МЗ.Действие = Выборка.Действие;
		МЗ.ПрохождениеМедОсмотра = Выборка.СсылкаПМО;
		МЗ.ДокументРегистратор = ПриемСсылка;
		МЗ.Записать();
		УстановитьПриемуПервичныйПриемПМО(ПриемСсылка, Выборка.СсылкаПМО);
	КонецЦикла;
...
Процедура УстановитьОтметкиДляДействийПМОПоВыборке(Выборка) Экспорт
...
Функция ЗначенияПараметровИзДокументовПоМедосмотру(ДокументМедосмотра, Знач Клиент = Неопределено) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Если Клиент = Неопределено Тогда
		Клиент = ДокументМедосмотра.Клиент;
	КонецЕсли;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ Ссылка КАК Прием 
	|ПОМЕСТИТЬ ПриемыМедосмотра
	|ИЗ Документ.Прием КАК Приемы ГДЕ Клиент = &Клиент И ПервичныйПрием = &ДокументМедосмотра; 
	|
	|ВЫБРАТЬ
	|	ЗначенияПараметровHTML.Параметр КАК Параметр,
	|	ЗначенияПараметровHTML.Значение КАК Значение
	|ИЗ
	|	РегистрСведений.ЗначенияПараметровHTML КАК ЗначенияПараметровHTML
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПриемыМедосмотра 
	|	ПО ЗначенияПараметровHTML.Документ = ПриемыМедосмотра.Прием
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
//
// Параметры:
//  ПриказВредныйФактор		 - СправочникСсылка.ПриказыМедосмотров, СправочникСсылка.ПереченьВредныхФакторовИРабот	 - приказ или вредный фактор медосмотра.
...
//
Функция ВидДокументаПоПриказуВредномуФактору(ПриказВредныйФактор, ПрохождениеМедОсмотра, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Приказ = Справочники.ПриказыМедосмотров.Приказ29н Тогда
		Возврат ВидДокументаПоПриказу29н(ПрохождениеМедосмотра, ПриказВредныйФактор, ДополнительныеПараметры);
	КонецЕсли;
...
// 
// Возвращаемое значение:
//  СправочникСсылка.РегистрируемыеЭлектронныеМедДокументы - вид документа.
//
Функция ВидДокументаПоПриказу29н(ПрохождениеМедосмотра, ПриказВредныйФактор, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если ТипЗнч(ПриказВредныйФактор) = Тип("СправочникСсылка.ПереченьВредныхФакторовИРабот")
		И ПунктыВредныхФакторовДляМедкнижки().Найти(ПриказВредныйФактор.НомерПП) <> Неопределено
	Тогда
		Возврат ИнтеграцияЕГИСЗОбщегоНазначенияПовтИсп.РегистрируемыеЭлектронныеМедДокументы(230);
	Иначе
...
// По медосмотру формирует таблицу с приказами/вредными факторами медосмотров и видами СЭДМ.
//
// Параметры:
//  ПрохождениеМедОсмотра	 - ДокументСсылка.ПрохождениеМедосмотра - медосмотр.
...
// 
// Возвращаемое значение:
//  ТаблицаЗначений - см. ПриказыВредныеФакторыМедосмотра(), а также с колонкой ВидДокумента - СправочникСсылка.РегистрируемыеЭлектронныеМедДокументы.
//
Функция ПриказыВредныеФакторыМедосмотраСВидамиДокументов(ПрохождениеМедОсмотра, ДополнительныеПараметры = Неопределено) Экспорт
	
	// TODO: Возможно стоит просто сразу обходить ТЧ ИдентификаторыПриказов, когда она не пустая
	ПриказыВредныеФакторы = ПриказыВредныеФакторыМедосмотра(ПрохождениеМедОсмотра, ДополнительныеПараметры);
	...
	Для Каждого СтрокаФактора Из ПриказыВредныеФакторы Цикл
		...
		Если ЭтоПриказСправки(Приказ) Тогда
			СтрокаФактора.ВидДокумента = СтрокаФактора.ПриказВредныйФактор.ШаблонОсмотраСправки.ВидДокумента;
		Иначе
			
			СтрокаЭМД = ПрохождениеМедОсмотра.ЭМД.Найти(СтрокаФактора.ПриказВредныйФактор, "ПриказВредныйФактор");
			...
			Иначе
				СтрокаФактора.ВидДокумента = ВидДокументаПоПриказуВредномуФактору(СтрокаФактора.ПриказВредныйФактор, ПрохождениеМедОсмотра, ДополнительныеПараметры);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
//
Функция ПриказыВредныеФакторыМедосмотра(ПрохождениеМедОсмотра, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ФилиалДокумента				 = ЗначениеСвойстваДляФормированияПриказовВредныхФакторов(ПрохождениеМедОсмотра, ДополнительныеПараметры, "Филиал",					"Филиал");
	ДатаДокумента				 = ЗначениеСвойстваДляФормированияПриказовВредныхФакторов(ПрохождениеМедОсмотра, ДополнительныеПараметры, "Дата",					"Дата");
	НоменклатураДокумента		 = ЗначениеСвойстваДляФормированияПриказовВредныхФакторов(ПрохождениеМедОсмотра, ДополнительныеПараметры, "Номенклатура",			"Номенклатура");
	ТаблицаФакторовИУсловийРабот = ЗначениеСвойстваДляФормированияПриказовВредныхФакторов(ПрохождениеМедОсмотра, ДополнительныеПараметры, "ФакторыИУсловияРаботы",	"ТаблицаФакторовИУсловийРабот");
	
	Если ФилиалДокумента = Неопределено
		Или ДатаДокумента = Неопределено
		Или НоменклатураДокумента = Неопределено
		Или ТаблицаФакторовИУсловийРабот = Неопределено
	Тогда
		Возврат ПриказыВредныеФакторы;
	КонецЕсли;
	
	ИспользуетсяРЭМД = ИнтеграцияЕГИСЗСерверПовтИсп.ИспользуетсяИнтеграцияРЭМД(ФилиалДокумента) И ДатаДокумента >= ИнтеграцияЕГИСЗСерверПовтИсп.ПолучитьДатуНачалаОбменаИнформациейСРЭМД(ФилиалДокумента);
	
	Если ЗначениеЗаполнено(НоменклатураДокумента.ВредныйФакторСправки) Тогда
		НоваяСтрокаТаблицы = ПриказыВредныеФакторы.Добавить();
		НоваяСтрокаТаблицы.ПриказВредныйФактор = НоменклатураДокумента.ВредныйФакторСправки;
		НоваяСтрокаТаблицы.Ссылка = ПрохождениеМедОсмотра;
	Иначе
		
		// В уже сформированных заключениях по 194 разделение на факторы не производится
		РазделениеПриказа29нНаФакторыЭЛМК = ПрохождениеМедосмотра.ЭМД.НайтиСтроки(Новый Структура("ВидДокументаЭМД", ИнтеграцияЕГИСЗОбщегоНазначенияПовтИсп.РегистрируемыеЭлектронныеМедДокументы(194))).Количество() = 0;
		
		Для Каждого СтрокаФактора Из ТаблицаФакторовИУсловийРабот Цикл
			НоваяСтрокаТаблицы = ПриказыВредныеФакторы.Добавить();
			НоваяСтрокаТаблицы.Ссылка = ПрохождениеМедОсмотра;
			
			Фактор = СтрокаФактора.Фактор;
			Приказ = Фактор.Приказ;
			Если ЭтоПриказСправки(Приказ)
				Или Приказ = Справочники.ПриказыМедосмотров.Приказ29н
					И ПунктыВредныхФакторовДляМедкнижки().Найти(Фактор.НомерПП) <> Неопределено
					И ИспользуетсяРЭМД
					И РазделениеПриказа29нНаФакторыЭЛМК
			Тогда
				НоваяСтрокаТаблицы.ПриказВредныйФактор = Фактор;
			Иначе
				НоваяСтрокаТаблицы.ПриказВредныйФактор = Приказ;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ЗначениеСвойстваДляФормированияПриказовВредныхФакторов(ПрохождениеМедОсмотра, ДополнительныеПараметры, ИмяРеквизита, ИмяСвойства)
	
	Если ДополнительныеПараметры <> Неопределено
		И ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		И ДополнительныеПараметры.Свойство(ИмяСвойства)
	Тогда
		Возврат ДополнительныеПараметры[ИмяСвойства];
	ИначеЕсли ЗначениеЗаполнено(ПрохождениеМедОсмотра) Тогда
		Возврат ОбщегоНазначенияБИТКлиентСервер.СвойствоОбъекта(ПрохождениеМедОсмотра, ИмяРеквизита);
	КонецЕсли;
...
Функция ПолучитьМассивПриказовИВредныхФакторовПрохожденияМедосмотра(ПрохождениеМедОсмотра, Знач ДополнительныеПараметры) Экспорт
	
	ПриказыВредныеФакторы = ПриказыВредныеФакторыМедосмотра(ПрохождениеМедОсмотра, ДополнительныеПараметры);
	Возврат ПриказыВредныеФакторы.ВыгрузитьКолонку("ПриказВредныйФактор");
	
КонецФункции

Функция ТребуетсяПодписаниеПечатнойФормыЗаключения(ПрохождениеМедОсмотра) Экспорт
	
	ПриказыВредныеФакторы = ПриказыВредныеФакторыМедосмотраСВидамиДокументов(ПрохождениеМедОсмотра);
	
	ЗаключениеДляЭЛМК = Ложь;
	ЗаключениеНеДляЭЛМК = Ложь;
	...
	Для Каждого СтрокаПриказаВредногоФактора Из ПриказыВредныеФакторы Цикл
		...
		Если Приказ = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПриказыМедосмотров.Приказ29н") Тогда
			Если ЭтоСтрокаФактора Тогда
				ЗаключениеДляЭЛМК = Истина;
			Иначе
				ЗаключениеНеДляЭЛМК = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	// Если в заключении использованы только факторы для ЭЛМК - помимо СЭМДа 230 формируется и подписывается ПФ заключения
	Возврат ЗаключениеДляЭЛМК И Не ЗаключениеНеДляЭЛМК;
...
Функция ДополнитьОбязательныеДействияКлиентов(ВФКлиентов, СписокКлиентов, ВидМедосмотра, ТаблицаПриказовКлиентов)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	(Приказы.ИспользоватьБезВФ = Истина
