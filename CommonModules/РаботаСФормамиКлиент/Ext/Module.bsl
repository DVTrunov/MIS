...
// Обработчик изменения даты в форме документа
//
// Параметры:
//  ЭтаФорма	 - ФормаКлиентскогоПриложения - форма документа.
//  СтараяДата	 - Дата	 - значение из переменная на форме, хранящей значение даты перед изменением (обычно Перем мТекущаяДатаДокумента).
//
Процедура ДокументПриИзмененииДаты(ЭтаФорма, СтараяДата) Экспорт
	
	// Сброс номера документа
	...
	// НДС: обновление основной ставки НДС у реквизита формы НастройкаПечатиЧеков чекового документа.
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтаФорма, "НастройкаПечатиЧеков") Тогда
		Типы = Новый Массив; // См. ОпределяемыеТипы.ОснованиеФискальнойОперацииБПО.
		Типы.Добавить(Тип("ДокументСсылка.ВозвратПоПлатежнойКарте"));
		Типы.Добавить(Тип("ДокументСсылка.ОплатаПлатежнойКартой"));
		Типы.Добавить(Тип("ДокументСсылка.ПриходныйКассовыйОрдер"));
		Типы.Добавить(Тип("ДокументСсылка.РасходныйКассовыйОрдер"));
		Типы.Добавить(Тип("ДокументСсылка.ОказаниеУслуг"));
		
		Если Типы.Найти(ТипЗнч(ЭтаФорма.Объект.Ссылка)) <> Неопределено Тогда
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтаФорма, "ДатаПечатиЧека") Тогда
				ДатаНДС = ОбщегоНазначенияБИТКлиентСервер.ЗаполненноеЗначение(ЭтаФорма.Объект.ДатаПечатиЧека, ЭтаФорма.Объект.Дата);
			Иначе
				ДатаНДС = ЭтаФорма.Объект.Дата;
			КонецЕсли;
			
			ЭтаФорма.НастройкаПечатиЧеков = МенеджерОборудованияВызовСервераПереопределяемый.ПолучитьНастройкуПечатиЧековФилиала(ЭтаФорма.Объект.Филиал, Истина, ДатаНДС); 
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьСписокДоступныхСтавокНДС(ДанныеВыбора, СтандартнаяОбработка, Знач НастройкаПечатиЧеков, Филиал) Экспорт
	
	Если УчетНДСКлиентСервер.ЭтоСНО_УСН(НастройкаПечатиЧеков.ОсновнаяСистемаНалогообложения)
		И НастройкаПечатиЧеков.СтавкаНДСДляОсновнойСистемыНалогообложения <> ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС20") 
	Тогда
		СтандартнаяОбработка = Ложь;
		
		СтавкиДляВыбора = Новый Массив;
		СтавкиДляВыбора.Добавить(НастройкаПечатиЧеков.СтавкаНДСДляОсновнойСистемыНалогообложения);
		ОбщегоНазначенияКлиентСервер.ДобавитьВМассивБезДублей(СтавкиДляВыбора, ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС0"));
		
		ДанныеВыбора = Новый СписокЗначений;
		ДанныеВыбора.ЗагрузитьЗначения(СтавкиДляВыбора);
	КонецЕсли;
