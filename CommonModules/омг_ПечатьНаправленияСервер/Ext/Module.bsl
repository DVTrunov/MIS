...
Функция ВыровнятьТекст(Знач Текст, МаксШирина, ВыравниватьВправо = Ложь) Экспорт
    ...
    НедостающиеПробелы = МаксШирина - ДлинаТекста;

    Если ВыравниватьВправо Тогда
        Возврат ПовторСтроку(" ", НедостающиеПробелы) + Текст;
    Иначе
        ЛевыеПробелы  = Цел(НедостающиеПробелы / 2);
        ПравыеПробелы = НедостающиеПробелы - ЛевыеПробелы;
        Возврат ПовторСтроку(" ", ЛевыеПробелы) + Текст + ПовторСтроку(" ", ПравыеПробелы);
    КонецЕсли;
...
Функция СформироватьШаблон(Заявка, Шаблон, ШиринаСтроки) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	омг_ЗаявкаНаЛабораторноеИсследованиеВЛИСБиоматериалы.Биоматериал.Наименование КАК БиоматериалНаименование,
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	омг_ЗаявкаНаЛабораторноеИсследованиеВЛИСБиоматериалы.Контейнер.Наименование КАК КонтейнерНаименование,
	               |	омг_ЗаявкаНаЛабораторноеИсследованиеВЛИСБиоматериалы.Ссылка КАК Ссылка,
	               |	КОЛИЧЕСТВО(омг_ЗаявкаНаЛабораторноеИсследованиеВЛИСБиоматериалы.Контейнер) КАК Количество
	               |ИЗ
	               |	Документ.омг_ЗаявкаНаЛабораторноеИсследованиеВЛИС.Биоматериалы КАК омг_ЗаявкаНаЛабораторноеИсследованиеВЛИСБиоматериалы
	               |ГДЕ
	               |	омг_ЗаявкаНаЛабораторноеИсследованиеВЛИСБиоматериалы.Ссылка В(&Ссылка)
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	омг_ЗаявкаНаЛабораторноеИсследованиеВЛИСБиоматериалы.Ссылка,
	               |	омг_ЗаявкаНаЛабораторноеИсследованиеВЛИСБиоматериалы.Контейнер.Наименование";
	...
	ВыборкаКонтейнеров = ПакетРезультатов.Получить(2).Выбрать();
	...
	Пока ВыборкаКонтейнеров.Следующий() Цикл
		СтрокаКонтейнеров = СтрокаКонтейнеров + 
			?(СтрокаКонтейнеров = "", "", Символы.ПС) + ВыборкаКонтейнеров.КонтейнерНаименование + Символы.ПС +
			ВыровнятьТекст(ВыборкаКонтейнеров.Количество, ШиринаСтроки, Истина);
	КонецЦикла;
	
	Текст = СтрЗаменить(Текст, "&ПереченьПробирок", СтрокаКонтейнеров);
	
	Пока ВыборкаИсследований.Следующий() Цикл
		СтрокаИсследований = СтрокаИсследований + 
			?(СтрокаИсследований = "", "", Символы.ПС) + 
			ВыборкаИсследований.НомерСтроки + ": " + ВыборкаИсследований.НоменклатураНаименование + " " + 
			"(" + ВыборкаИсследований.БиоматериалНаименование + ")";
	КонецЦикла;
	...
	Для НСтроки = 1 По СтрЧислоСтрок(Текст) Цикл
		...
		Если Не СтрНайти(ТекущаяСтрока, "#Центр") = 0 Тогда
			...
			ТекущаяСтрока = ВыровнятьТекст(ТекущаяСтрока, ШиринаСтроки);
		КонецЕсли;
