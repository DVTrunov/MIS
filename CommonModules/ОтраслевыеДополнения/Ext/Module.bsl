...
Функция СотрудникиПользователяПоФилиалу(Пользователь, Филиал) Экспорт
	
	СотрудникиПользователяПоФилиалу = Новый Массив;
	...
	Тогда
		...
		Для Каждого ПравилоПоиска Из ПравилаПоиска Цикл
			Для Каждого Найденный Из СотрудникиПоФизЛицу.НайтиСтроки(ПравилоПоиска) Цикл
				СотрудникиПользователяПоФилиалу.Добавить(Найденный.Сотрудник);
			КонецЦикла;
		КонецЦикла;
	Иначе
		ФилиалСотрудника = Пользователь.Сотрудник.Подразделение.Филиал;
		Если ФилиалСотрудника = Филиал
			Или ФилиалСотрудника.Организация = Филиал.Организация
			Или ФилиалСотрудника = Справочники.Филиалы.ПустаяСсылка()
		Тогда
			СотрудникиПользователяПоФилиалу.Добавить(Пользователь.Сотрудник);
		КонецЕсли;
	КонецЕсли;
	
	Возврат СотрудникиПользователяПоФилиалу;
...
Процедура УбратьПробелыФИО(ФИО) Экспорт
	
	ФИО.Вставить("Фамилия",	 СокрЛП(ФИО["Фамилия"]));
	ФИО.Вставить("Имя",		 СокрЛП(ФИО["Имя"]));
	ФИО.Вставить("Отчество", СокрЛП(ФИО["Отчество"]));
