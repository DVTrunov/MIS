...
Процедура ЗаполнитьСписокПодписейФайла(ТекущийОбъект, ЭлементыДерева, КоличествоПодписей,
	...
	Если ТекущийОбъект.ПодписанЭП Тогда
		...
		Выборка = ПолучитьЭлектронныеПодписи(ТекущийОбъект.Ссылка);
		...
		Пока Выборка.Следующий() Цикл
			...
			НоваяСтрока.Статус = доп_СертификатыЭЦПКлиентСервер.ПолучитьОбщийСтатусПроверкиПодписи(
...
Процедура ЗаполнитьСписокПодписейДокумента(ТекущийОбъект, ЭлементыДерева, КоличествоПодписей,
	...
	Если ТекущийОбъект.ПодписанЭП Тогда
		...
		Выборка = ПолучитьЭлектронныеПодписи(ТекущийОбъект.Ссылка,,,, ПодписиЭМД);
...
Процедура ЗаменитьИнформациюОПодписяхОбъекта(МассивДанныхДляЗанесенияВРегистр, ОбъектСсылка,
	...
	Попытка
		...
		Если ОбъектСТЧЭП Тогда
			...
			КонецЦикла;
		Иначе
			УдалитьПодписиОбъекта(ПодписываемыйОбъектСсылка);
			Для Каждого Данные Из МассивДанныхДляЗанесенияВРегистр Цикл
				...
				ЗанестиИнформациюОПодписи(ПодписываемыйОбъектСсылка, СвойстваПодписи);
			КонецЦикла;
		КонецЕсли;
...
Процедура ЗанестиИнформациюОПодписи(ПодписанныеДанные, СвойстваПодписиАргумент) Экспорт
	...
	МенеджерЗаписи.Версия = ЭЦП_УМЦ_Сервер.ПолучитьАктуальныйНомерВерсииПодписываемыхДанных(ТипЗнч(ПодписанныеДанные));
...
Процедура УстановитьСтатусПроверкиДляФайла(ВерсияФайла) Экспорт
	...
	ЭПФайла = ПолучитьЭлектронныеПодписи(ВерсияФайла);
...
Процедура УстановитьСтатусПроверкиДляДокумента(Документ) Экспорт
	...
	Для Каждого Файл Из ПодчиненныеФайлы Цикл
		...
		ЭПФайла = ПолучитьЭлектронныеПодписи(Файл.ТекущаяВерсия);
...
Процедура УстановитьСтатусПроверкиДляДокументаПриЗаписиЭлектроннойПодписи(Источник, Отказ, Замещение) Экспорт
	...
	Если ЗначениеЗаполнено(ОбъектИсточника) Тогда
		...
		Если Метаданные.ОпределяемыеТипы.ДокументРЭМД.Тип.СодержитТип(ТипЗнч(ОбъектИсточника)) Тогда
			...
			УстановитьСтатусПроверкиДляДокумента(ОбъектИсточника);
...
Функция ПолучитьСодержимоеТелаHTMLСводки(ОбъектСсылка, ЭлементыДерева, ДополнительныеПараметры)
	...
	ДокументДоРЭМД = ДатаМД < ИнтеграцияЕГИСЗСерверПовтИсп.ПолучитьДатуНачалаОбменаИнформациейСРЭМД(ОбъектСсылка.Филиал);
	...
	Тогда
		ДополнительныеДанные = Новый Структура("МинКолПодписей", ДанныеТребуемыхПодписей.МинКолПодписейДокумента);
		ЧастиТекстаСводки.Добавить(ПолучитьСводкуПоДаннымЭПДокумента(ОбъектСсылка, ЭлементыДерева, НСтр("ru='Подписи документа:'"), ВсеПодписиУстановлены, ДополнительныеДанные));
	...
	ИначеЕсли ДанныеТребуемыхПодписей.ТребуютсяПодписиФайлов Тогда
		ДополнительныеДанные = Новый Структура("МинКолПодписей, ПодписьФайлов", ДанныеТребуемыхПодписей.МинКолПодписейФайлов, Истина);
		ЧастиТекстаСводки.Добавить(ПолучитьСводкуПоДаннымЭПДокумента(ОбъектСсылка, ЭлементыДерева, НСтр("ru='Подписи файлов:'"), ВсеПодписиУстановлены, ДополнительныеДанные));
	КонецЕсли;
	
	Если ДанныеТребуемыхПодписей.ТребуютсяПодписиРЭМД Тогда
		ДополнительныеДанные = Новый Структура("ДанныеРЭМД", ДанныеТребуемыхПодписей.ДанныеРЭМД);
		ЧастиТекстаСводки.Добавить(ПолучитьСводкуПоДаннымЭПДокумента(ОбъектСсылка, ЭлементыДерева, НСтр("ru='Подписи РЭМД:'"), ВсеПодписиУстановлены, ДополнительныеДанные));
		
		ДополнительныеДанные.Вставить("ПодписьМО", Истина);
		Для Каждого СтруктураДанныхРЭМД Из ДанныеТребуемыхПодписей.ДанныеРЭМД Цикл
			Если СтруктураДанныхРЭМД.ТребуетсяПодписьМО Тогда
				ЧастиТекстаСводки.Добавить(ПолучитьСводкуПоДаннымЭПДокумента(ОбъектСсылка, ЭлементыДерева, НСтр("ru='Подписи МО РЭМД:'"), ВсеПодписиУстановлены, ДополнительныеДанные));
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ЗаполнитьДанныеПоШаблонамПриема(ОбъектСсылка, ДанныеТребуемыхПодписей, ЭлементыДерева, Ошибки)
	...
	ЕстьПодписиНеРЭМД = ПолучитьЭлектронныеПодписи(ОбъектСсылка,,,, Ложь).Количество() > 0;
	Если Не ИнтеграцияЕГИСЗСерверПовтИсп.ИспользуетсяИнтеграцияРЭМД()
		Или Выборка.Следующий()
	Тогда
		Если ЕстьПодписиНеРЭМД
			И Не доп_РаботаСФайлами.ФайлЭМДСохранен(ОбъектСсылка)
		Тогда
...
Процедура ЗаполнитьДанныеПоДаннымПМО(ОбъектСсылка, ДанныеТребуемыхПодписей, ЭлементыДерева, Ошибки, ДополнительныеПараметры)
	...
	ПриказыВредныеФакторы = МодульМедосмотрыСервер.ПриказыВредныеФакторыМедосмотраСВидамиДокументов(ОбъектСсылка, ДополнительныеПараметры);
	
	ПриказыВредныеФакторыРЭМД = ПриказыВредныеФакторы.СкопироватьКолонки();
	Для Каждого СтрокаПриказаВредногоФактора Из ПриказыВредныеФакторы Цикл
		Если ЗначениеЗаполнено(СтрокаПриказаВредногоФактора.ВидДокумента) Тогда
			ЗаполнитьЗначенияСвойств(ПриказыВредныеФакторыРЭМД.Добавить(), СтрокаПриказаВредногоФактора);
		КонецЕсли;
	КонецЦикла;
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(ПрохождениеМедосмотраЭМД.УИД, ИдентификаторыПриказовПМО.УИД) КАК УИД,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПрохождениеМедосмотра.ИдентификаторыПриказов КАК ИдентификаторыПриказовПМО
		|		ПО ДанныеПМО.Ссылка = ИдентификаторыПриказовПМО.Ссылка
		|			И ДанныеПМО.ПриказВредныйФактор = ИдентификаторыПриказовПМО.ПриказВредныйФактор
	...
	Запрос.УстановитьПараметр("Ссылка", ОбъектСсылка);
	Запрос.УстановитьПараметр("ПриказыВредныеФакторы", ПриказыВредныеФакторыРЭМД);
	Выборка = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
...
Функция ПолучитьСводкуПоДаннымЭПДокумента(ОбъектСсылка, ЭлементыДерева, Заголовок, ВсеПодписиУстановлены, ДополнительныеДанные)
	
	ДанныеРЭМД		= ?(ДополнительныеДанные.Свойство("ДанныеРЭМД"),	 ДополнительныеДанные.ДанныеРЭМД,		Неопределено);
	ПодписьМО		= ?(ДополнительныеДанные.Свойство("ПодписьМО"),		 ДополнительныеДанные.ПодписьМО,		Ложь);
	МинКолПодписей	= ?(ДополнительныеДанные.Свойство("МинКолПодписей"), ДополнительныеДанные.МинКолПодписей,	0);
	ПодписьФайлов	= ?(ДополнительныеДанные.Свойство("ПодписьФайлов"),	 ДополнительныеДанные.ПодписьФайлов,	Ложь);
	...
	Иначе
		ТаблицаHTMLПодписей = СформироватьТаблицуHTMLПодписей(ОбъектСсылка, ЭлементыДерева, ДанныеЗаголовков, МинКолПодписей, ПодписьФайлов);
	КонецЕсли;
...
Функция СформироватьТаблицуHTMLПодписейРЭМД(ЭлементыДерева, ДанныеЗаголовков, ДанныеРЭМД, ПодписьМО)
	...
	ШаблонСтрокиШаблона = "<tr style='font-weight: bold'><td colspan='%1'>%2</td></tr>";
	...
	Если СтрокиТаблицыПодписей.Количество() > 0 Тогда
		СтрокиТаблицыПодписей.Вставить(0, СформироватьСтрокуПодписи(ДанныеЗаголовка, КолонкиПодписи, "<tr style='text-align:center; font-weight: bold'>%1</tr>"));
	КонецЕсли;
...
Функция СформироватьТаблицуHTMLПодписей(ОбъектСсылка, ЭлементыДерева, ДанныеЗаголовков, МинКолПодписей, ПодписьФайлов)
	
	УстановитьПривилегированныйРежим(Истина);
	
	ОтображатьПодписиИРоли = ПодписьФайлов;
	
	РолиДляПодписания = МедицинскаяДеятельность.РолиДляПодписанияМедицинскогоДокумента(ОбъектСсылка);
	Для Каждого ДанныеРоли Из РолиДляПодписания Цикл
		Если Не (ТипЗнч(ДанныеРоли.ПараметрыРоли) = Тип("Структура")
				И ДанныеРоли.ПараметрыРоли.Свойство("УИД_ЭМД")
				И ДанныеРоли.ПараметрыРоли.Свойство("ПредставлениеЭМД")
				И ДанныеРоли.ПараметрыРоли.Свойство("РолиШаблона"))
		Тогда
			ОтображатьПодписиИРоли = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	ПодписиДокумента = ЭлементыДерева.Выгрузить(Новый Структура("ПодписанФайлЭМД", Ложь));
	ДанныеЗаголовка = Новый Структура("УстановившийПодпись, ДатаПодписи, Статус", "Пользователь", "Дата подписи", "Статус");
	
	Если ОтображатьПодписиИРоли Тогда
		ДанныеРолейЭМД = Новый ТаблицаЗначений;
		ДанныеРолейЭМД.Колонки.Добавить("УИД_ЭМД");
		ДанныеРолейЭМД.Колонки.Добавить("ПредставлениеЭМД");
		ДанныеРолейЭМД.Колонки.Добавить("Роль");
		ДанныеРолейЭМД.Колонки.Добавить("ИдентификаторРоли");
		ДанныеРолейЭМД.Колонки.Добавить("Обязательная");
		
		Для Каждого ДанныеРоли Из РолиДляПодписания Цикл
			Для Каждого РольШаблона Из ДанныеРоли.ПараметрыРоли.РолиШаблона Цикл
				СтрокаРоли = ДанныеРолейЭМД.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаРоли, ДанныеРоли.ПараметрыРоли, "УИД_ЭМД, ПредставлениеЭМД");
				ЗаполнитьЗначенияСвойств(СтрокаРоли, РольШаблона, "ИдентификаторРоли, Обязательная");
				СтрокаРоли.Роль = ДанныеРоли.Роль;
			КонецЦикла;
		КонецЦикла;
		
		ДобавитьСтрокиТаблицыHTMLПодписейСРолями(СтрокиТаблицыПодписей, КолонкиПодписи, ПодписиДокумента, ДанныеЗаголовков, МинКолПодписей, ОбъектСсылка, ДанныеЗаголовка, ДанныеРолейЭМД);
	Иначе
		ДобавитьСтрокиТаблицыHTMLПодписейБезРолей(СтрокиТаблицыПодписей, КолонкиПодписи, ПодписиДокумента, ДанныеЗаголовков, МинКолПодписей);
	КонецЕсли;
...
Процедура ДобавитьСтрокиТаблицыHTMLПодписейБезРолей(СтрокиТаблицыПодписей, КолонкиПодписи, ПодписиДокумента, ДанныеЗаголовков, МинКолПодписей)
...
Процедура ДобавитьСтрокиТаблицыHTMLПодписейСРолями(СтрокиТаблицыПодписей, КолонкиПодписи, ПодписиДокумента, ДанныеЗаголовков, МинКолПодписей, ОбъектСсылка, ДанныеЗаголовка, ДанныеРолейЭМД)
	...
	Запрос.УстановитьПараметр("УИДыЭЦП", ПодписиДокумента.ВыгрузитьКолонку("УникальныйИдентификатор"));
	Запрос.УстановитьПараметр("Документ", ОбъектСсылка);
	
	РолиИПодписиЭМД = Запрос.Выполнить().Выгрузить();
	
	ДанныеРолейЭМД.Колонки.Добавить("УстановившийПодпись");
	ДанныеРолейЭМД.Колонки.Добавить("ДатаПодписи", Новый ОписаниеТипов("Дата"));
	ДанныеРолейЭМД.Колонки.Добавить("Статус");
	ДанныеРолейЭМД.Колонки.Добавить("СертификатДействителен");
	ДанныеРолейЭМД.Колонки.Добавить("ПодписьВерна");
	
	СтрокиКУдалению = Новый Массив;
	Для Каждого СтрокаДанных Из ДанныеРолейЭМД Цикл
		
		СтрокиРолиИПодписи = РолиИПодписиЭМД.НайтиСтроки(Новый Структура("РольПодписи, УИД_ЭМД, ИдентификаторРоли", СтрокаДанных.Роль, СтрокаДанных.УИД_ЭМД, СтрокаДанных.ИдентификаторРоли));
		Если СтрокиРолиИПодписи.Количество() = 0 Тогда
			Если Не СтрокаДанных.Обязательная Тогда
				СтрокиКУдалению.Добавить(СтрокаДанных);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
		
		СтрокиПодписи = ПодписиДокумента.НайтиСтроки(Новый Структура("УникальныйИдентификатор", СтрокиРолиИПодписи[0].УИД_ЭЦП));
		Если СтрокиПодписи.Количество() = 0 Тогда
			Если Не СтрокаДанных.Обязательная Тогда
				СтрокиКУдалению.Добавить(СтрокаДанных);
			КонецЕсли;
		...
		ЗаполнитьЗначенияСвойств(СтрокаДанных, СтрокиПодписи[0], "УстановившийПодпись, ДатаПодписи, Статус, СертификатДействителен, ПодписьВерна");
	КонецЦикла;
	
	Для Каждого СтрокаКУдалению Из СтрокиКУдалению Цикл
		ДанныеРолейЭМД.Удалить(СтрокаКУдалению);
	КонецЦикла;
	
	ДанныеРолейЭМД.Сортировать("УИД_ЭМД, ДатаПодписи УБЫВ");
	
	ЭМД = ДанныеРолейЭМД.Скопировать();
	ЭМД.Свернуть("УИД_ЭМД, ПредставлениеЭМД");
	
	Для Каждого СтрокаЭМД Из ЭМД Цикл
		СтрокиРолейЭМД = ДанныеРолейЭМД.НайтиСтроки(Новый Структура("УИД_ЭМД", СтрокаЭМД.УИД_ЭМД));
		ОбщегоНазначенияСервер.ВосстановитьПорядокНайденныхСтрок(СтрокиРолейЭМД, ДанныеРолейЭМД);
		
		Если ЗначениеЗаполнено(СтрокаЭМД.УИД_ЭМД) Тогда
			ШаблонСтрокиШаблона = "<tr style='font-weight: bold'><td colspan='%1'>%2</td></tr>";
			СтрокиТаблицыПодписей.Добавить(СтрШаблон("<tr style='font-weight: bold'><td colspan='%1'>%2</td></tr>", КолонкиПодписи.Количество(), СтрокаЭМД.ПредставлениеЭМД));
		КонецЕсли;
		
		Для Каждого СтрокаРоли Из СтрокиРолейЭМД Цикл
			Если ЗначениеЗаполнено(СтрокаРоли.ДатаПодписи) Тогда
				ДанныеЗаголовков.КоличествоПодписейВсего = ДанныеЗаголовков.КоличествоПодписейВсего + 1;
				Если СтрокаРоли.ПодписьВерна
					И СтрокаРоли.СертификатДействителен
				Тогда
					ДанныеЗаголовков.КоличествоДействительныхПодписей = ДанныеЗаголовков.КоличествоДействительныхПодписей + 1;
					ШаблонСтроки = "<tr>%1</tr>";
				Иначе
					ШаблонСтроки = "<tr style='background-color:rgb(255,240,240)'>%1</tr>";
				КонецЕсли;
				СтрокиТаблицыПодписей.Добавить(СформироватьСтрокуПодписи(СтрокаРоли, КолонкиПодписи, ШаблонСтроки));
			Иначе
				ДанныеЗаголовков.КоличествоНедостающихПодписей = ДанныеЗаголовков.КоличествоНедостающихПодписей + 1;
				ДанныеПодписи = Новый Структура("Роль, УстановившийПодпись, ДатаПодписи, Статус", СтрокаРоли.Роль);
				СтрокиТаблицыПодписей.Добавить(СформироватьСтрокуПодписи(ДанныеПодписи, КолонкиПодписи, "<tr style='background-color:rgb(255,240,240)'>%1</tr>"));
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
