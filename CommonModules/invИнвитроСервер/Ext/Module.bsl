...
Процедура ИмпортВыполненнойЗаявки(Подразделение = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл	
		...
		МассивФайловВЗагрузке = НайтиФайлыПоРасширениюБезУчетаРегистра(ПапкаЗагрузки, ".xml");
		МассивВзятыхФайлов = НайтиФайлыПоРасширениюБезУчетаРегистра(ПапкаСФайлами + "\", ".xml", Истина);
		...
		Попытка
			МассивФайловВЗагрузкеPDF = НайтиФайлыПоРасширениюБезУчетаРегистра(ПутьКРезультатамPDF, ".pdf");
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ошибка: "+ ОписаниеОшибки());
			Продолжить;
		КонецПопытки;
		
		МассивВзятыхФайловPDF = НайтиФайлыПоРасширениюБезУчетаРегистра(ПапкаСФайлами + "\", ".pdf");
...
Функция НайтиФайлыПоРасширениюБезУчетаРегистра(Путь, Расширение, ИскатьВПодкаталогах = Ложь)
	
	ФайлыПоРасширению = Новый Массив;
	
	ФайлыПапки = НайтиФайлы(Путь, ПолучитьМаскуВсеФайлы(), ИскатьВПодкаталогах);
	Для Каждого Файл Из ФайлыПапки Цикл
		Если НРег(Файл.Расширение) = НРег(Расширение) Тогда
			ФайлыПоРасширению.Добавить(Файл);
		КонецЕсли;
	КонецЦикла;
	
	Возврат ФайлыПоРасширению;
...
Процедура СформироватьФайлЗаявки(ПутьКФайлу, Токен, КодМедОфиса, ДействиеНадАнализамиСсылка, CovidOrder)
	...
	Иначе
		...
		// +ОЛИМП. Передадим адрес пациента, согласно законодательству, если среди продуктов есть ВИЧ / Гепатит В/С.
		Найден = Ложь;
