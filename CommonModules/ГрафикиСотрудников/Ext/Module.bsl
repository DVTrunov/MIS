...
Функция ДобавитьСобытие(Знач ОбъектыКОбработке, ВремяНач, ВремяКон, Знач ВидВремени, Знач Название = "", Комментарий = "") Экспорт
	...
	Для Каждого СтруктураПолучателя Из ОбъектыКОбработке Цикл
		...
		СообщениеОбОшибке = доп_ПолныеПрава.ЗаписатьОбъект(ДокГрафик, РежимЗаписиДокумента.Проведение);
...
Функция УдалитьСобытие(ИдентификаторСобытия) Экспорт
	...
	Если ЗначениеЗаполнено(СтруктураДанныхГрафика.ДанныеГрафика) И СтруктураДанныхГрафика.ДокументУГ.Проведен Тогда
		...
		доп_ПолныеПрава.ЗаписатьОбъект(ДокГрафик, РежимЗаписиДокумента.Проведение);
...
Функция ПолучитьРабочееВремяЗаПериод(ДатаНачала, ДатаОкончания, ВернутьВремяБезДаты, РазличатьВидыВремени = Ложь,
	...
	ДобавленныеСтрокиГрафика = ТаблицаГрафиков.СкопироватьКолонки();
	...
	// Цикл по уникальным сочетаниям на день сотрудника.
	Для Каждого СтрокаИзмерения Из ТаблицаИзмеренийГрафика Цикл
		
		ЗаполнитьЗначенияСвойств(Отбор, СтрокаИзмерения);
		ДобавленныеСтрокиГрафика.Очистить();
		
		СтрокиГрафикаПоОтбору = ТаблицаГрафиков.НайтиСтроки(Отбор);
		ОбщегоНазначенияСервер.ВосстановитьПорядокНайденныхСтрок(СтрокиГрафикаПоОтбору, ТаблицаГрафиков);
		
		Для Каждого СтрокаГрафика Из СтрокиГрафикаПоОтбору Цикл
			
			Если СтрокаГрафика.ВремяНачала = СтрокаГрафика.ВремяОкончания Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокиНаУделение = Новый Массив;
			
			// Вырезаем текущую строку из уже добавленных
			Для Каждого ДобавленнаяСтрока Из ДобавленныеСтрокиГрафика Цикл
				ВычестьИнтервалИзПериода(ДобавленнаяСтрока, СтрокаГрафика, ДобавленныеСтрокиГрафика, СтрокиНаУделение);
			КонецЦикла;
			
			Для Каждого СтрокаНаУдаление Из СтрокиНаУделение Цикл
				ДобавленныеСтрокиГрафика.Удалить(СтрокаНаУдаление);
			КонецЦикла;
			
			// Добавляем строку
			ЗаполнитьЗначенияСвойств(ДобавленныеСтрокиГрафика.Добавить(), СтрокаГрафика);
		КонецЦикла;
		
		Для Каждого ДобавленнаяСтрока Из ДобавленныеСтрокиГрафика Цикл
			Если ДобавленнаяСтрока.ВремяНачала < ДобавленнаяСтрока.ВремяОкончания Тогда
				ЗаполнитьЗначенияСвойств(ТаблицаГрафиковОбъединенная.Добавить(), ДобавленнаяСтрока);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	Для Каждого НаборИзмерений Из ИзмеренияСклейки Цикл
		...
		ТаблицаГрафиковПромежуточная.Очистить();
...
Функция ПолнаяТаблицаГрафиковЗаПериод(ДатаНачала, ДатаОкончания, ВернутьВремяБезДаты, 
	...
	ТаблицаГрафиков.Колонки.Добавить("ЭтоРабочееВремя",	Новый ОписаниеТипов("Булево")); // Этот вид времени отвечает за рабочее время
	ТаблицаГрафиков.Колонки.Добавить("ЭтоВидРабВремя",	Новый ОписаниеТипов("Булево")); // Это предопределенный вид времени "Рабочее время"
...
Функция ТекстЗапросаПолнаяТаблицаГрафиковЗаПериод()
	...
	Возврат "ВЫБРАТЬ
	        ...
	        |	И (&УсловиеСотрудники
	        |			ИЛИ &УсловиеКабинеты)
	        |	И (&ФилиалНеВыбран = ИСТИНА
	        |			ИЛИ ГрафикиРаботы.Филиал В (&Филиал))
	        ...
	        |	ГрафикиРаботы.Филиал КАК Филиал,
	        |	ГрафикиРаботы.ВидВремени КАК ВидВремени,
	        |	ГрафикиРаботы.ВидВремени = ЗНАЧЕНИЕ(Справочник.ВидыВремениГрафика.РабочееВремя)
	        |		ИЛИ ГрафикиРаботы.ВидВремени = ЗНАЧЕНИЕ(Справочник.ВидыВремениГрафика.ПустаяСсылка) КАК ЭтоВидРабВремя,
	        ...
	        |	ГрафикиРаботы.ВидВремени = ЗНАЧЕНИЕ(Справочник.ВидыВремениГрафика.РабочееВремя)
	        |		ИЛИ ГрафикиРаботы.ВидВремени = ЗНАЧЕНИЕ(Справочник.ВидыВремениГрафика.ПустаяСсылка),
...
Функция ПолучитьГрафикСотрудника(Филиал, Подразделение, Сотрудник, Дата, ДокументУГ, ФилиалПоПриоритету, ПодразделениеПоПриоритету)
	...
	Если Не Рез.Пустой() Тогда
		...
		Иначе
			Если мДокументов.Количество() >= 2 Тогда
				// Если сотрудник есть в графиках 2+ филиалов, а филиал целевой пользователем не задан,
				// то операцию не делаем - ни в существующий документ, ни в новый.
				Если Не ЗначениеЗаполнено(Филиал) Тогда
					ФилиалыДокументов = Новый Массив;
					Для Каждого СтрокаДокумент из мДокументов Цикл
						ОбщегоНазначенияКлиентСервер.ДобавитьВМассивБезДублей(ФилиалыДокументов ,СтрокаДокумент.Филиал);
					КонецЦикла;
					Если ФилиалыДокументов.Количество() > 1 Тогда
						ВызватьИсключение "Сотрудник работает более чем в одном филиале в этом месяце.
										  |Без отбора по филиалу операцию выполнить нельзя.";
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(Филиал) Тогда
		...
		// Нет подходящего документа, где этот Сотрудник есть.
		// Ищем подходящий документ без него (по филиалу, по его подразделению или пустому подразделению).
	...
	// Не нашли подходящего или результат запроса пустой
