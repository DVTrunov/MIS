...
Функция ИсточникДанныхАдресногоКлассификатора()
	...
	УстановитьПривилегированныйРежим(Истина);
	Возврат Константы.ИсточникДанныхАдресногоКлассификатора.Получить();
	//-ОЛИМП
...
Процедура ЗаполнитьВерсиюПоставщикаДанныхСервис1С(Результат)
	...
	//+ОЛИМП Не требуется подгрузка на ходу из веб-сервиса 1С. И тормозит при недоступности.
...
//+ОЛИМП
Процедура ФоноваяЗагрузитьДанныеКлассификатораАдресовИзФайла(ПараметрПроцедуры) Экспорт
	
	СчетчикПрогресса = 0;
	КаталогВФоновомЗадании = КаталогВременногоХранилищаФайлов();
	
	Для Каждого Эл Из ПараметрПроцедуры Цикл
		Прогресс = СчетчикПрогресса/ПараметрПроцедуры.Количество() * 100;
		ДлительныеОперации.СообщитьПрогресс(Прогресс, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Загружается %1'"), Эл.Ключ));
		Эл.Значение[2].КаталогФайлов = КаталогВФоновомЗадании;
		ЗагрузитьДанныеКлассификатораАдресовИзФайла(Эл.Значение[0], Эл.Значение[1], Эл.Значение[2]);
		СчетчикПрогресса = СчетчикПрогресса + 1;
	КонецЦикла;
	
КонецПроцедуры

Процедура ФоноваяЗагрузкаРегионаАдресногоКлассификатораССайта(ПараметрПроцедуры) Экспорт
	...
	Для Каждого Эл Из ПараметрПроцедуры Цикл
		...
		ДлительныеОперации.СообщитьПрогресс(Прогресс, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Загружается %1'"), Эл.Ключ));
		Эл.Значение[1].КаталогФайлов = КаталогВФоновомЗадании;
		ЗагрузкаРегионаАдресногоКлассификатораССайта(Эл.Значение[0], Эл.Значение[1]);
		СчетчикПрогресса = СчетчикПрогресса + 1;
	КонецЦикла;
...
Функция КаталогВременногоХранилищаФайлов() Экспорт
	...
	//+ОЛИМП Такого механизма в УМЦ нет
	//Было:
