...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	//+ОЛИМП
	Форма.Элементы.доп_ОткрытьФормуСпискаВариантовОтчета.Видимость = ПравоДоступа("Редактирование", Метаданные.Справочники.ВариантыОтчетов);
	...
	Для НомерУровня = 1 По 10 Цикл
		...
		Команда.Действие = "Подключаемый_доп_ИзменитьУровеньГруппировки";
...
Процедура ПередЗагрузкойВариантаНаСервере(Форма, НовыеНастройкиКД) Экспорт
	...
	//+ОЛИМП Удаление из пользовательских настроек всех отборов по Филиалу/Организации, если учет по филиалам выключен.
	
	// Только для внутренних отчетов
	Если СтрНачинаетсяС(Форма.ИмяФормы, "Отчет.") Тогда
		...
		Если Не УчетПоНесколькимФилиалам Тогда
			Для Каждого ЭлементОтбора Из НовыеНастройкиКД.Отбор.Элементы Цикл
				Если ЗначениеЗаполнено(ЭлементОтбора.ИдентификаторПользовательскойНастройки) Тогда
					Путь = Строка(ЭлементОтбора.ЛевоеЗначение);
					
					// Стандартное для встроенных отчётов именование Филиала
					
					Если Путь = "Филиал" Или СтрНачинаетсяС(Путь, "Филиал.") Тогда
