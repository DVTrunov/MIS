...
Функция ПараметрыФискализацииЧека() Экспорт
	...
	//+ОЛИМП
	ПараметрыФискализации.Вставить("доп_Сторно", Ложь);
	ПараметрыФискализации.Вставить("НеСторнировать", Ложь);
	ПараметрыФискализации.Вставить("доп_ПодключаемоеОборудование", );
	//-ОЛИМП
...
Процедура ЗаписатьФискальнуюОперацию(ПараметрыФискализации) Экспорт
	...
	КонецЕсли;
	Если ПараметрыФискализации.Свойство("доп_Сторно") Тогда
		НоваяСтрока.доп_Сторно = ПараметрыФискализации.доп_Сторно;
	КонецЕсли;
	Если ПараметрыФискализации.Свойство("доп_ПодключаемоеОборудование") Тогда
		НоваяСтрока.доп_ПодключаемоеОборудование = ПараметрыФискализации.доп_ПодключаемоеОборудование;
	КонецЕсли;
	//-ОЛИМП
...
Процедура ОбновитьПоставляемыеДрайвера(ДрайвераОборудования) Экспорт
	...
	//+ОЛИМП исключен только для Android
	//Драйвер = ДрайвераОборудования.Добавить();
	//Драйвер.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ККТ;
...
Функция ПодготовитьДанныеОперации(ПараметрыПодключения, Команда, ПараметрыОперации) Экспорт
	...
	Если Команда = "CheckFiscalization" Тогда
		...
		Если ПараметрыПодключения.ТипыОборудования.ККТ Тогда
			...
			Если ПараметрыОперации.Свойство("ПодключаемоеОборудование") Тогда
				ПараметрыФискализации.доп_ПодключаемоеОборудование = ПараметрыОперации.ПодключаемоеОборудование;
			КонецЕсли;
			//-ОЛИМП
	...
	ИначеЕсли Команда = "PrintReceiptCorrection" Тогда
		...
		Иначе
			...
			//+ОЛИМП Получаем все фискальные операции для которых нужно создать чек сторнирования
			...
			//+ОЛИМП Добавляем цикл чтобы обойти все фискальные операции
			...
			//+ОЛИМП
				...
				ПараметрыФискализацииСторно.доп_Сторно = Истина;
