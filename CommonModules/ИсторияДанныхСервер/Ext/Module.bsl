...
//  ПолноеИмяМетаданных	 - Строка - Полное имя метаданных объекта конфигурации.
//
Процедура УдалитьВерсииДанных(ПолноеИмяМетаданных, ДатаСреза = Неопределено) Экспорт
	...
	Если МетаданныеОбъекта <> Неопределено Тогда
		...
		Иначе
			ИсторияДанных.УдалитьВерсии(МетаданныеОбъекта, ДатаСреза);
		КонецЕсли;
	КонецЕсли;       
	
КонецПроцедуры

// Процедура для регламентного задания по очистке истории по сроку хранения
Процедура УдалитьВерсииПоСрокуХранения() Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	СрокиХраненияИсторииДанных.СрокХранения > 0";
	...
	Пока Выборка.Следующий() Цикл
		ДатаОтчистки = ТекущаяДата - Выборка.СрокХранения * 60 * 60 * 24;
		УдалитьВерсииДанных(Выборка.ИмяМетаданных, ДатаОтчистки);
	КонецЦикла;
...
// Срок хранения версий по умолчанию для объектов, не имеющих индивидуального срока по умолчанию.
// 
// Возвращаемое значение:
//  Число - срок в днях.
//
Функция СрокХраненияВерсийПоУмолчанию() Экспорт
	Возврат 90;
КонецФункции
