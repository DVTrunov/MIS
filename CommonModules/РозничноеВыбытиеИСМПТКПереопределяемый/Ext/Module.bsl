...
Функция ПолучитьНоменклатуруПоШтрихкодуВРозничномКонтуре(ШтрихКод, НужноВернутьМассив = Ложь) Экспорт
	...
	Если Не Результат.Пустой() Тогда 
		...
		Если Выборка.Количество() = 1 Тогда
			...
			ДанныеНоменклатуры.Вставить("ОсобенностьУчета",        Выборка.ОсобенностьУчета);
...
#Область Элементы
	...
	Если Форма.Элементы.Найти("ТоварыКоличествоКодовМаркировкиИСМПТК") = Неопределено Тогда
		...
		КолонкаИнтерфейса = Форма.Элементы.Вставить("ТоварыКоличествоКодовМаркировкиИСМПТК", Тип("ПолеФормы"), ЭлементыФормы.Товары, ТоварыГруппаНоменклатура);
		КолонкаИнтерфейса.ПутьКДанным 		 = ПутьКРеквизиту;
...
#Область Инициализация
	...
	ЭлементыФормы.ТоварыКоличествоКодовМаркировкиИСМПТК.Видимость = РозничноеВыбытиеИСМПТКВызовСервера.ЕстьПравоЧтенияОбработкаРозничноеВыбытиеИСМПТК()
														 И Форма.ИспользоватьАкцизныеМаркиИСМПТК;
...
Функция ПроверитьДокументНаПоддержкуИСМП(Форма) Экспорт
	...
	//ПЕРЕОПРЕДЕЛЕНИЕ//
	Если ИмяФормы    = "Документ.ЧекККМ.Форма.ФормаДокумента" Тогда
	///////////////////
		Возврат Истина;
	Иначе 
		Возврат Ложь;
...
Функция ДанныеМаркированнойПродукции(Номенклатура) Экспорт
	...
	|	Номенклатура.ОсобенностьУчета КАК ОсобенностиУчетаНоменклатуры,
	|	Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ТабачнаяПродукция) КАК ЭтоТабачнаяПродукцияИСМПТК,
	|	Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ОбувнаяПродукция) КАК ЭтоОбувнаяПродукцияИСМПТК,
	|	Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ЛекарственныеПрепараты) КАК ЭтоЛекарственныеПрепаратыИСМПТК
	|ИЗ
...
Процедура ЗаполнитьПризнакКодаМаркировкиВТоварах(Форма) Экспорт
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		Если ЗначениеЗаполнено(СтрокаТовары.Номенклатура) Тогда
			...
			Если ВидПродукции.Пустая() И КоличествоМарокПоТовару = 0 Тогда
				СтрокаТовары.СтатусПроверкиИСМПТК = 0;
			ИначеЕсли Не ВидПродукции.Пустая() И Не КоличествоМарокПоТовару = 0 Тогда
				СтрокаТовары.СтатусПроверкиИСМПТК = 1;
			ИначеЕсли Не ВидПродукции.Пустая() И КоличествоМарокПоТовару = 0 Тогда
				СтрокаТовары.СтатусПроверкиИСМПТК = 2;
			ИначеЕсли ВидПродукции.Пустая() И Не КоличествоМарокПоТовару = 0 Тогда
