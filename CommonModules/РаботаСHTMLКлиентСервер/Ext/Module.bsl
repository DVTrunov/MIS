...
//
// Параметры:
//  ОбъектHTML			 - ДокументHTML, ЭлементHTML, ВнешнийОбъект	 - любой HTML объект, в котором производится поиск элементов
//  ЗначениеАтрибута	 - Строка									 - значение указанного атрибута искомого элемента
//  ОграничениеПоТегам	 - Строка, Массив							 - перечень имен тегов, по которым будут отфильтрованы найденные элементы. Указывается либо массивом, либо строкой через ","
//  ИмяАтрибута			 - Строка									 - наименование атрибута искомого элемента
//  ВернутьПервое		 - Булево									 - Вернуть первое
// 
// Возвращаемое значение:
//  Массив, Неопределено - Если ВернутьПервое = Ложь, возвращается коллекция элементов HTML, найденных по значению атрибута;
//  иначе возвращается первый найденный объект, либо Неопределено, если ничего не найдено.
//
Функция ПолучитьЭлементыПоЗначениюАтрибута(ОбъектHTML, Знач ЗначениеАтрибута = Неопределено, ОграничениеПоТегам = Неопределено, ИмяАтрибута = "id", ВернутьПервое = Ложь) Экспорт
	...
	ИсходныйМассивТегов = Неопределено;
	Если ОграничениеПоТегам <> Неопределено Тогда
		Если ТипЗнч(ОграничениеПоТегам) = Тип("Массив") Тогда
			ИсходныйМассивТегов = ОграничениеПоТегам;
		ИначеЕсли ТипЗнч(ОграничениеПоТегам) = Тип("Строка") Тогда
			ИсходныйМассивТегов = СтрРазделить(ОграничениеПоТегам, ",");
		КонецЕсли;
	КонецЕсли;
	
	Если ИсходныйМассивТегов <> Неопределено Тогда
		МассивТегов = Новый Массив;
		Для Каждого ИсходноеИмяТега Из ИсходныйМассивТегов Цикл
			МассивТегов.Добавить(НРег(СокрЛП(ИсходноеИмяТега)));
		КонецЦикла;
	КонецЕсли;
	
	ПоискПоВладельцу = Ложь;
	Если РаботаСHTMLКлиентСервер.ЭтоДокументDOM(ОбъектHTML)
		И ОбъектHTML <> ОбъектHTML.ДокументВладелец
	Тогда
		ДокументHTML = ОбъектHTML.ДокументВладелец;
		ПоискПоВладельцу = Истина;
	Иначе
		ДокументHTML = ОбъектHTML;
	КонецЕсли;
	
	Попытка
		СписокНайденныхЭлементовПоФильтру = ПолучитьСписокДочернихЭлементовПоФильтру(ДокументHTML, ЗначениеАтрибута, ИмяАтрибута);
	Исключение
		СписокНайденныхЭлементовПоФильтру = ОбщегоНазначенияБИТКлиентСервер.НовыйМассив();
	КонецПопытки;
	
	Если ПоискПоВладельцу Тогда
		// Сохраняем только элементы, найденные внутри исходного элемента
		СписокДочернихЭлементовПоФильтру = Новый Массив;
		Для Каждого НайденныйЭлемент Из СписокНайденныхЭлементовПоФильтру Цикл
			Если НайденныйЭлемент = ОбъектHTML Тогда
				Продолжить;
			КонецЕсли;
			ТекущийУзел = НайденныйЭлемент;
			Пока ТекущийУзел <> Неопределено Цикл
				Если ТекущийУзел = ОбъектHTML Тогда
					СписокДочернихЭлементовПоФильтру.Добавить(НайденныйЭлемент);
					Прервать;
				КонецЕсли;
				ТекущийУзел = ТекущийУзел.ParentNode;
			КонецЦикла;
		КонецЦикла;
	Иначе
		СписокДочернихЭлементовПоФильтру = СписокНайденныхЭлементовПоФильтру;
	КонецЕсли;
	
	Если МассивТегов <> Неопределено Тогда
		НайденныеЭлементыHTML = Новый Массив;
		Для Каждого Узел Из СписокДочернихЭлементовПоФильтру Цикл
			Если МассивТегов.Найти(Нрег(ПолучитьСвойствоЭлемента(Узел, "tagName"))) <> Неопределено Тогда
				НайденныеЭлементыHTML.Добавить(Узел);
			КонецЕсли;
		КонецЦикла;
	Иначе
		НайденныеЭлементыHTML = СписокДочернихЭлементовПоФильтру;
	КонецЕсли;
	
	Если Не ВернутьПервое Тогда
		Возврат НайденныеЭлементыHTML;
	КонецЕсли;
	
	Если ЭтоВнешнийОбъект(НайденныеЭлементыHTML)
			И НайденныеЭлементыHTML.length > 0
		Или Не ЭтоВнешнийОбъект(НайденныеЭлементыHTML)
			И НайденныеЭлементыHTML.Количество() > 0
	Тогда
		Возврат НайденныеЭлементыHTML[0];
	КонецЕсли;
...
// Возвращает текст HTML-документа, получаемого из исходного переносом стилей из элемента заголовка в элемент тела документа.
//
// Параметры:
//  HTML					 - ДокументHTML, Строка	 - исходный HTML
//  СоздатьТелоПриОтсутствии - Булево				 - Создавать элемент body при отсутствии в исходном HTML
// 
// Возвращаемое значение:
//  ДокументHTML, Строка - HTML с перенесенными стилями из head в body.
//
Функция ПеренестиСтилиИзHeadВТелоДокумента(HTML, СоздатьТелоПриОтсутствии = Ложь) Экспорт
	
	Результат = ПеренестиСтилиИзИсточниковВПриемник(HTML, СоздатьТелоПриОтсутствии, "head", "body", Истина);
	Возврат ?(Результат = Неопределено, HTML, Результат);
	
КонецФункции

// Возвращает текст HTML-документа, получаемого из исходного переносом стилей из элемента тела в элемент заголовка документа.
//
// Параметры:
//  HTML							 - ДокументHTML, Строка	 - исходный HTML
//  СоздатьЗаголовокПриОтсутствии	 - Булево				 - Создавать элемент head при отсутствии в исходном HTML
// 
// Возвращаемое значение:
//  ДокументHTML, Строка - HTML с перенесенными стилями из body в head.
//
Функция ПеренестиСтилиИзТелаВHeadДокумента(HTML, СоздатьЗаголовокПриОтсутствии = Ложь) Экспорт
	
	Результат = ПеренестиСтилиИзИсточниковВПриемник(HTML, СоздатьЗаголовокПриОтсутствии, "body", "head", Ложь);
	Возврат ?(Результат = Неопределено, HTML, Результат);
...
Функция ПолучитьБлижайшийУзел(ЭлементHTML, ИмяУзла) Экспорт
	...
	Пока ТекущийУзел <> Неопределено
		И ВРег(ТекущийУзел.NodeName) <> "BODY"
	Цикл
		Если ВРег(ТекущийУзел.NodeName) = ВРег(ИмяУзла) Тогда
			Возврат ТекущийУзел;
		КонецЕсли;
		ТекущийУзел = ТекущийУзел.ParentNode;
	КонецЦикла;
...
// Возвращает элемент, следующий непосредственно за указанным элементом, на том же уровне дерева, если такой есть. Соответствует свойству HTML "nextElementSibling"
//
// Параметры:
//  ЭлементHTML	 - ЭлементHTML, ВнешнийОбъект - Объект HTML
// 
...
//  ЭлементHTML, ВнешнийОбъект, Неопределено
//
Функция СледующийСоседнийЭлемент(ЭлементHTML) Экспорт
	...
	Если ЭтоВнешнийОбъект(ЭлементHTML) Тогда
		Возврат ЭлементHTML.nextElementSibling;
	КонецЕсли;
	
	// Так как в объектной модели отсутствует свойство, соответствующее свойству nextElementSibling, циклично используем свойство СледующийСоседний, соответствующий свойству nextSibling
	СледующийЭлемент = ЭлементHTML.СледующийСоседний;
	Пока СледующийЭлемент <> Неопределено Цикл
		Если ТипЗнч(СледующийЭлемент) <> Тип("ТекстDOM")
			И ТипЗнч(СледующийЭлемент) <> Тип("КомментарийDOM")
		Тогда
...
Функция ПолучитьСписокДочернихЭлементовПоФильтру(ДокументHTML, ЗначениеАтрибута, ИмяАтрибута)
	
	Если ЭтоДокументDOM(ДокументHTML) Тогда
		ЗначенияФильтра = Новый Массив;
		ЗначенияФильтра.Добавить(Новый Структура("type, value", "attribute", Новый Структура("value, operation", ИмяАтрибута, "nameequals")));
		Если ЗначениеАтрибута <> Неопределено Тогда
			ЗначенияФильтра.Добавить(Новый Структура("type, value", "attribute", Новый Структура("value, operation", Строка(ЗначениеАтрибута), "valueequals")));
		КонецЕсли;
		
		СтруктураФильтра = Новый Структура("type, value", "intersection", ЗначенияФильтра);
		Фильтр = ОбщегоНазначенияБИТКлиентСервер.СформироватьСтрокуJSON(СтруктураФильтра);
	...
	Иначе
		Если ЗначениеАтрибута <> Неопределено Тогда
			Фильтр = СтрШаблон("[%1='%2']", ИмяАтрибута, Строка(ЗначениеАтрибута));
		Иначе
			Фильтр = СтрШаблон("[%1]", ИмяАтрибута);
		КонецЕсли;
		
		ЭлементыПоАтрибуту = ДокументHTML.querySelectorAll(Фильтр);
	КонецЕсли;
	
	Возврат ЭлементыПоАтрибуту;
	
КонецФункции

Функция ПеренестиСтилиИзИсточниковВПриемник(HTML, СоздатьУзелИсточникаПриОтсутствии, ИмяИсточника, ИмяПриемника, ДобавитьВКонец)
	...
	ЭлементыИсточниковИПриемников = ПолучитьЭлементыИсточниковИПриемников(ДокументHTML, СоздатьУзелИсточникаПриОтсутствии, ИмяИсточника, ИмяПриемника, ДобавитьВКонец);
	
	Если ЭлементыИсточниковИПриемников = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Источники = ЭлементыИсточниковИПриемников.Источники;
	Приемник = ЭлементыИсточниковИПриемников.Приемник;
	
	// Перенести все стили в приемник
	Для Каждого ЭлементИсточника Из Источники Цикл
		ЭлементПередКоторымДобавлять = Приемник.ПервыйДочерний;
		Для Каждого ЭлементСтиляИсточника Из ЭлементИсточника.ПолучитьЭлементыПоИмени("style") Цикл
			Если ЭлементПередКоторымДобавлять = Неопределено Тогда
				Приемник.ДобавитьДочерний(ЭлементСтиляИсточника);
				ЭлементПередКоторымДобавлять = ЭлементСтиляИсточника;
			Иначе
				Приемник.ВставитьПеред(ЭлементСтиляИсточника, ЭлементПередКоторымДобавлять);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция ПолучитьЭлементыИсточниковИПриемников(ДокументHTML, СоздатьУзелИсточникаПриОтсутствии, ИмяИсточника, ИмяПриемника, ДобавитьВКонец)
	
	ЭлементыИсточника = ДокументHTML.ПолучитьЭлементыПоИмени(ИмяИсточника);
	Если ЭлементыИсточника.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ЭлементИсточника = ЭлементыИсточника[0];
	
	ЭлементыПриемника = ДокументHTML.ПолучитьЭлементыПоИмени(ИмяПриемника);
	Если ЭлементыПриемника.Количество() = 0 Тогда
		Если Не СоздатьУзелИсточникаПриОтсутствии Тогда
			Возврат Неопределено;
		КонецЕсли;
		
		Попытка
			ЭлементПриемника = ДокументHTML.СоздатьЭлемент(ИмяПриемника);
			Если ДобавитьВКонец Тогда
				ЭлементИсточника.РодительскийУзел.ДобавитьДочерний(ЭлементПриемника);
			Иначе
				ЭлементИсточника.РодительскийУзел.ВставитьПеред(ЭлементПриемника, ЭлементИсточника);
			КонецЕсли;
		Исключение
			Возврат Неопределено;
		КонецПопытки;
	Иначе
		ЭлементПриемника = ЭлементыПриемника[0];
	КонецЕсли;
	
	Возврат Новый Структура("Источники, Приемник", ЭлементыИсточника, ЭлементПриемника);
