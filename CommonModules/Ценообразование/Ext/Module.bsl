...
Функция ПересчитатьЦенуПриИзмененииЕдиницы(Цена, ЕдиницаЦены, НоваяЕдиница) Экспорт
	...
	НовоеЗначениеЦены = Окр(НовоеЗначениеЦены, 2, РежимОкругления.Окр15как20);
...
//
// Изменено Макеев (
Функция ПолучитьЦену(Прейскурант, Знач Номенклатура,Знач НаМомент=Неопределено, 
	ХарактеристикаНоменклатуры=Неопределено, ЕдиницаИзмерения=Неопределено, 
	ПересчитыватьЦенуИзДругихЕдиниц = Истина, Филиал = Неопределено, Набор = Неопределено,омг_НаборУровень= Неопределено) Экспорт
// )
	...
	//////////////////////////////////////////////////////////////////////////////
	// ОМГ
	Если ЗначениеЗаполнено(НаМомент) и не Набор = Неопределено Тогда
		 Если не омг_МедицинскаяДеятельность.ПроверитьПериодДействияНабора(НаМомент,Набор) Тогда
		 Возврат Новый Структура("Цена",0); 
		 КонецЕсли;
	КонецЕсли;	
	// ОМГ
	...
	Если Не Филиал = Неопределено Тогда
		СтруктураОтбора.Вставить("омг_Филиал", Филиал);
	КонецЕсли;
	Если Набор = Неопределено Тогда
		СтруктураОтбора.Вставить("омг_Набор", Справочники.Номенклатура.ПустаяСсылка());
	Иначе
		СтруктураОтбора.Вставить("омг_Набор", Набор);
	КонецЕсли;
	// )
	...
	///////////////////////////////////////////////////////////////////////////////
	// ОМГ временно - не правильно забиты единицы в установках цен
    ...
    ///////////////////////////////////////////////////////////////////////////////
		...
		// Пробуем получить из набора
		Если СрезЦен.Количество() = 0 и не омг_НаборУровень = Неопределено и ЗначениеЗаполнено(омг_НаборУровень) Тогда
			// Цену надо получать. формируем структуру отбора.
			СтруктураОтбора=Новый Структура("Прейскурант, Номенклатура",Прейскурант, омг_НаборУровень);
			// Добавлено Макеев (
			Если Не Филиал = Неопределено Тогда
				СтруктураОтбора.Вставить("омг_Филиал", Филиал);
			КонецЕсли;
			...
			СрезЦен = РегистрыСведений.ЦеныНоменклатуры.СрезПоследних(НаМомент,СтруктураОтбора);
			Если не СрезЦен.Количество() = 0 Тогда 
				    СрезЦен[0].Цена = СрезЦен[0].Цена / омг_НаборУровень.КомплектующиеНоменклатуры.итог("Количество")
			КонецЕсли;
		КонецЕсли;	
...
// Применяется только в блоке расчета з/п для метода "Оценка по прейскуранту".
// 
// Возвращаемое значение:
//  Число.
//
Функция КонстантаОтмененнаяЦена() Экспорт 
	Возврат -999999.0;
КонецФункции
