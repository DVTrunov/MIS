...
Функция ПолучитьДополнительныеСведенияОНоменклатуреДляИмпорта(Номенклатура, EAN) Экспорт
	...
	ДанныеПоШтрихкоду = ОбщегоНазначенияИСМПТКПереопределяемый.ПолучитьДанныеНоменклатурыИзРегистраШтрихкодовПоШтрихкоду(, EAN, Номенклатура); 
...
Процедура ПолучитьНоменклатуруПоШтрихкодам(Штрихкоды, ДанныеПоШтрихкодам) Экспорт
	...
	ДанныеПоШтрихкодам = ОбщегоНазначенияИСМПТКПереопределяемый.ПолучитьДанныеНоменклатурыИзРегистраШтрихкодовПоШтрихкоду(Истина, Штрихкоды);
...
Функция ПолучитьРеквизитыОрганизацииКонтрагента(Ссылка, Реквизит) Экспорт
	...
	ИначеЕсли Не ЭтоТорговоеРешение
		И ТипЗнч(Ссылка) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда	
...
Функция ПроверитьКонтрагентаДляСхемыВзаимногоПризнанияИСМПТ(Основание) Экспорт
	...
	//ПЕРЕОПРЕДЕЛЕНИЕ//
	Нерезидент = КонтрагентНерезидент(Основание);
	Контрагент = Основание.Контрагент;
	СтранаВзаимногоПризнания = Справочники.КлассификаторСтранМира.НайтиПоКоду("643");  // Россия;
	
	// У контрагента-нерезидента заполнена "Страна резидентства"
	Возврат Нерезидент
		И Контрагент.ЮрФизЛицо = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ЮрЛицо")
		И Контрагент.СтранаРезидентства = СтранаВзаимногоПризнания;	
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Процедура = "ИнтеграцияИСМПТКПереопределяемый.ЗаполнитьЭтапыОформленияДокументовИСМПТК";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение значений текущего этапа оформления у документов ИС МПТ.'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НаборЗаписей.Выбран() Тогда
			...
			Попытка
				...
				ЭлементБлокировки = Блокировка.Добавить(Метаданные.РегистрыСведений.СтатусыЗаказовИСМПТК.ПолноеИмя());
				ЭлементБлокировки.УстановитьЗначение("ЗаказНаЭмиссию", Выборка.ЗаказНаЭмиссию);
				Блокировка.Заблокировать();
					
				Для Каждого Запись Из НаборЗаписей Цикл
					Запись.Организация = Выборка.ЗаказНаЭмиссию.Организация;				
				КонецЦикла;
...
Функция ПолучитьДанныеРегистраПрофильДоступаКЭДО(СписокОрганизаций) Экспорт
	...
	|	Организации.ИдентификационныйНомер КАК БИНОрганизации,
