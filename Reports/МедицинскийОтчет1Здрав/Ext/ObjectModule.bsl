...
Процедура УстановитьДоступностьОбластей(ВыбраннаяФорма, Форма, ИмяСтраницыПанели = "") Экспорт
	...
	ЗапрещенныеОбласти.Добавить("ОргПочтАдрес");
	//ЗапрещенныеОбласти.Добавить("ИнГраждГр3");
...
Функция ПолучитьКоличествоПосещенийВрачей(ПараметрыОтчета)
	...
	Возврат ? (Выборка.Следующий(), Выборка.ВсегоПриемов, 0);
...
Функция ПолучитьТаблицуРаздела8(ПараметрыОтчета)
	...
	Запрос.Текст = 
		...
		|	МИНИМУМ(Диагнозы.Период) КАК Период
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	Диагнозы.Регистратор.Клиент,
		|	Диагнозы.Регистратор.ПервичныйПрием,
		|	Диагнозы.Диагноз
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА СлучаиЗаболевания.Диагноз.КодДиагноза ПОДОБНО ""U%""
		|			ТОГДА СлучаиЗаболевания.Диагноз.КодДиагноза
		|		ИНАЧЕ ВЫРАЗИТЬ(ЛЕВ(СлучаиЗаболевания.Диагноз.КодДиагноза, 3) КАК СТРОКА(3))
		|	КОНЕЦ КАК РубрикаМКБ,
		|	ВЫБОР
		|		КОГДА ДОБАВИТЬКДАТЕ(СлучаиЗаболевания.Клиент.ДатаРождения, ГОД, 18) < СлучаиЗаболевания.Период
		|			ТОГДА 0
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК Меньше17,
		|	ВЫБОР
		|		КОГДА ДОБАВИТЬКДАТЕ(СлучаиЗаболевания.Клиент.ДатаРождения, ГОД, 1) < СлучаиЗаболевания.Период
		|			ТОГДА 0
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК МеньшеГода
		|ПОМЕСТИТЬ РубрикиИГруппы
		|ИЗ
		|	СлучаиЗаболевания КАК СлучаиЗаболевания
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ СлучаиЗаболевания
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	РубрикиИГруппы.РубрикаМКБ КАК КодДиагноза,
		|	КОЛИЧЕСТВО(РубрикиИГруппы.Меньше17) КАК Всего,
		|	СУММА(РубрикиИГруппы.Меньше17) КАК Меньше17,
		|	СУММА(РубрикиИГруппы.МеньшеГода) КАК МеньшеГода
		|ИЗ
		|	РубрикиИГруппы КАК РубрикиИГруппы
		|
		|СГРУППИРОВАТЬ ПО
		|	РубрикиИГруппы.РубрикаМКБ
		|
		|УПОРЯДОЧИТЬ ПО
		|	КодДиагноза";
...
Функция УстановитьПараметрыРасшифровки(ВыбраннаяФорма, ПараметрыОтчета, ИмяЯчейки) Экспорт
	ИмяФормыРасшифровкиОтчета = Неопределено;
	ПараметрыРасшифровки = Новый Структура();	
	
	// Страница 2. Ячейка "ЧислоПосещений"
	Если ИмяЯчейки = "ЧислоПосещений" И ПараметрыОтчета.ТекущаяСтрокаРазделовОтчета = 2 Тогда
		ИмяФормыРасшифровкиОтчета = "Отчет.РасшифровкаМедицинскогоОтчета1ЗдравЧислоПосещений.ФормаОбъекта";
	КонецЕсли;	

	// Страница 6. Раздел 8 с диагнозами
	ГруппыДиагнозов = ПолучитьГруппуЯчейки(ИмяЯчейки);
	Если ЗначениеЗаполнено(ГруппыДиагнозов) И ПараметрыОтчета.ТекущаяСтрокаРазделовОтчета = 6 Тогда
		...
		ПараметрыРасшифровки.Вставить("ОтборПоВозрасту", ПолучитьОтборЛет(ИмяЯчейки));
	КонецЕсли;

	Если ИмяФормыРасшифровкиОтчета = Неопределено Тогда
...
Функция ПолучитьГруппуЯчейки(ИмяЯчейки) Экспорт
	...
	ГруппыДиагнозов.Добавить("A00-T98,U07.1,U07.2");
