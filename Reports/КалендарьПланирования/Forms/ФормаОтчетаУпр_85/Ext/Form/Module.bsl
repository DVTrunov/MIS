...
&НаКлиенте
Перем мПланировщикАктивныйЭлемент; //Карсаков - текущий элемент планировщика для контекст меню
&НаКлиенте
Перем мПланировщикПараметрыКоманд; //Карсаков - Данные для создания заявки
&НаКлиенте
Перем мТаймерЗаявка;//Карсаков. 8.5 не различает 1 или 2 клика было. Надо чтобы было 2  
&НаКлиенте
Перем мСписокОбъектовПланировщика;//Карсаков. ДЛя планировщика

&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	
	ПодробныйРежимТаблиц = Ложь;
	
	Если Параметры.Свойство("Данные") Тогда
		ФормаВызоваАРМ = Параметры.Данные.ФормаВызова;
	КонецЕсли;
	
	// Добавлено Макеев (
	Если РольДоступна("омг_МедсестраПЗБ") Тогда
		Отказ = Истина;
	КонецЕсли;
	// )
	...
	// Изменено Макеев (
	мУчетнаяПолитика.Вставить("ИспользоватьОбменДанными", Константы.ИспользоватьСинхронизациюДанных.Получить() И ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ОшибкиОбменовДанными));
	// )
	...
	КонецЕсли;
	// Добавлено Макеев (  
	омг_Клиника 			= УправлениеНастройками.ПолучитьФилиалПоУмолчаниюПользователя();
	Отчет.омг_Организация = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(
	Пользователи.ТекущийПользователь(),
	"омг_ОсновнаяОрганизация");
	Если Отчет.омг_Организация.Пустая() Тогда 
		Элементы.Филиал.СвязиПараметровВыбора = Новый ФиксированныйМассив(Новый Массив);
		ОтборПоОрганизации = ДокументыОчереди.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборПоОрганизации.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Филиал.Организация");
		ОтборПоОрганизации.ПравоеЗначение = Отчет.омг_Организация;
		ОтборПоОрганизации.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	КонецЕсли;
	
	Если Отчет.Сотрудники.Количество() = 0 И омг_РольДоступна() Тогда
		Запрос = Новый Запрос;
		...
		Если Выборка.Следующий() Тогда
			Отчет.Сотрудники.Добавить(Выборка.Сотрудник);
		КонецЕсли;
	КонецЕсли;
	// )  
	...
	////Карсаков. Листы ожидания в журнале не нужны
	...
	//Для 8.5 Карсаков
	Если РольДоступна("омг_АРМОператор") Тогда
		Элементы.ГруппаВозвратаВАРМ.Видимость = Ложь;
	Иначе
		Элементы.ГруппаПереходаПоВкладкам.Видимость = Ложь;
	КонецЕсли;
...
Процедура ИнициализироватьПланировщикНаСервере()
	...
	Планировщик = ЭтаФорма.ПланировщикКалендарь;
	Планировщик.Измерения.Очистить();
	
	// Читаем из учетной политики — те же настройки, что и у таблицы значений
	КвантВСекундах = ОбщегоНазначения.ВремяВСекунды(мУчетнаяПолитика.ПериодичностьПланирования);
	ВремяНач       = мУчетнаяПолитика.ВремяНачалаРаботы;
	ВремяОконч     = мУчетнаяПолитика.ВремяОкончанияРаботы;
	
	// Защита от незаполненных значений
	Если Не ЗначениеЗаполнено(КвантВСекундах) Или КвантВСекундах = 0 Тогда
		КвантВСекундах = 900; // 15 минут по умолчанию
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ВремяНач) Тогда
		ВремяНач = '00010101090000';
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ВремяОконч) Тогда
		ВремяОконч = '00010101200000';
	КонецЕсли;
	
	// Настройка шкалы времени
	ЭлементШкалы = Планировщик.ШкалаВремени.Элементы[0];
	ЭлементШкалы.Единица    = ТипЕдиницыШкалыВремени.Минута;
	ЭлементШкалы.Кратность  = КвантВСекундах / 60;
	
	// Период отображения — всегда один день (ДатаНачала отчёта)
	РабочаяДата = НачалоДня(?(ЗначениеЗаполнено(Отчет.ДатаНачала), Отчет.ДатаНачала, ТекущаяДата()));
	
	ДатаНачалаОтображения    = РабочаяДата + (Час(ВремяНач)   * 3600 + Минута(ВремяНач)   * 60);
	ДатаОкончанияОтображения = РабочаяДата + (Час(ВремяОконч) * 3600 + Минута(ВремяОконч) * 60);
	
	Планировщик.ТекущиеПериодыОтображения.Очистить();
	Планировщик.ТекущиеПериодыОтображения.Добавить(ДатаНачалаОтображения, ДатаОкончанияОтображения);
	
	// Создание измерения врачей
	ИзмерениеВрачи = Планировщик.Измерения.Добавить("Врач");
КонецПроцедуры


&НаСервере
Процедура ЗаполнитьДанныеПланировщикаНаСервере(СписокОбъектов, ДанныеЗаявок)
	...
	// --- Цвета ---
	ЦветНераб = мУчетнаяПолитика.ОсновнойЦветНерабочегоВремени.Получить();
	Если ТипЗнч(ЦветНераб) <> Тип("Цвет") Тогда
		ЦветНераб = Новый Цвет(245, 230, 235);
	КонецЕсли;
	ЦветСлот          = Новый Цвет(235, 248, 238); // фон свободного слота
	ЦветВидаУмолч     = Новый Цвет(235, 235, 235); // если у вида не задан цвет
	ЦветЗаявкиПоУмолч = Новый Цвет(200, 220, 255);
	
	// --- Параметры времени ---
	КвантВСекундах = ОбщегоНазначения.ВремяВСекунды(мУчетнаяПолитика.ПериодичностьПланирования);
	ВремяНач       = мУчетнаяПолитика.ВремяНачалаРаботы;
	ВремяОконч     = мУчетнаяПолитика.ВремяОкончанияРаботы;
	Если Не ЗначениеЗаполнено(КвантВСекундах) Или КвантВСекундах = 0 Тогда КвантВСекундах = 900 КонецЕсли;
	Если Не ЗначениеЗаполнено(ВремяНач)   Тогда ВремяНач   = '00010101090000' КонецЕсли;
	Если Не ЗначениеЗаполнено(ВремяОконч) Тогда ВремяОконч = '00010101200000' КонецЕсли;
	
	ЭлементШкалы = Планировщик.ШкалаВремени.Элементы[0];
	ЭлементШкалы.Кратность = КвантВСекундах / 60;
	
	ПустаяДата       = Дата(1, 1, 1);
	СекундаНачала    = Час(ВремяНач)   * 3600 + Минута(ВремяНач)   * 60;
	СекундаОкончания = Час(ВремяОконч) * 3600 + Минута(ВремяОконч) * 60;
	
	// --- Список отображаемых дней ---
	ДатаНач = НачалоДня(?(ЗначениеЗаполнено(Отчет.ДатаНачала), Отчет.ДатаНачала, ТекущаяДата()));
	
	Если Отчет.РежимФормирования = 2 Тогда
		КоличествоДней = ?(ЗначениеЗаполнено(ДлинаПериода), ДлинаПериода, 1);
		Если КоличествоДней < 1 Тогда КоличествоДней = 1 КонецЕсли;
	Иначе
		КоличествоДней = 1;
	КонецЕсли;
	...
	Для сч = 1 По КоличествоДней Цикл
		МассивДней.Добавить(ТекДень);
		ТекДень = ТекДень + 86400;
	КонецЦикла;
	ДатаКон = МассивДней[МассивДней.Количество() - 1];
	
	Планировщик.ТекущиеПериодыОтображения.Очистить();
	Для Каждого День Из МассивДней Цикл
		Планировщик.ТекущиеПериодыОтображения.Добавить(День + СекундаНачала, День + СекундаОкончания);
	КонецЦикла;
	
	// --- Колонки врачей ---
	ИзмерениеВрачи = Планировщик.Измерения.Найти("Врач");
	Если ИзмерениеВрачи = Неопределено Тогда
		ИзмерениеВрачи = Планировщик.Измерения.Добавить("Врач");
	КонецЕсли;
	ИзмерениеВрачи.Элементы.Очистить();
	
	МассивОбъектов   = Новый Массив;
	ДобавленныеВрачи = Новый Соответствие;
	Если СписокОбъектов <> Неопределено Тогда
		Для Каждого Объект Из СписокОбъектов Цикл
			Если ЗначениеЗаполнено(Объект) И ДобавленныеВрачи.Получить(Объект) = Неопределено Тогда
				НовыйВрач = ИзмерениеВрачи.Элементы.Добавить(Объект);
				НовыйВрач.Текст = Строка(Объект);
				ДобавленныеВрачи.Вставить(Объект, Истина);
	...
	// --- Длительность приёма по сотрудникам (для разбиения свободных слотов) ---
	ДлительностьПоОбъекту = Новый Соответствие;
	МассивСотрудников = Новый Массив;
	Для Каждого Объект Из МассивОбъектов Цикл
		Если ТипЗнч(Объект) = Тип("СправочникСсылка.Сотрудники") Тогда
			МассивСотрудников.Добавить(Объект);
		КонецЕсли;
	КонецЦикла;
	Если МассивСотрудников.Количество() > 0 Тогда
		ЗапросДлит = Новый Запрос;
		ЗапросДлит.Текст =
		"ВЫБРАТЬ
		|	Сотрудники.Ссылка КАК Ссылка,
		|	Сотрудники.ДлительностьПриема КАК ДлительностьПриема
		|ИЗ
		|	Справочник.Сотрудники КАК Сотрудники
		|ГДЕ
		|	Сотрудники.Ссылка В (&Сотрудники)";
		ЗапросДлит.УстановитьПараметр("Сотрудники", МассивСотрудников);
		Для Каждого СтрД Из ЗапросДлит.Выполнить().Выгрузить() Цикл
			ДлитСек = ?(ЗначениеЗаполнено(СтрД.ДлительностьПриема), СтрД.ДлительностьПриема - ПустаяДата, 0);
			ДлительностьПоОбъекту.Вставить(СтрД.Ссылка, ДлитСек);
		КонецЦикла;
	КонецЕсли;
	
	// --- Заявки: элементы планировщика + интервалы по объектам ---
	ЗаявкиПоОбъекту = Новый Соответствие; // Объект -> Массив структур {Нач, Кон}
	
	Если ДанныеЗаявок <> Неопределено И ДанныеЗаявок.Количество() > 0 Тогда

		МассивЗаявок = Новый Массив;
		Для Каждого КЗ Из ДанныеЗаявок Цикл
			МассивЗаявок.Добавить(КЗ.Ключ);
		КонецЦикла;
		...
		|	Заявка.Клиент.омг_РискПадения КАК РискПадения,
		|	Заявка.Состояние.Цвет КАК СостояниеЦвет
		|ИЗ
		|	Документ.Заявка КАК Заявка
		|ГДЕ
		|	Заявка.Ссылка В (&СписокЗаявок)
		|	И Заявка.ДатаНачала >= &ДатаНач
		|	И Заявка.ДатаНачала < &ГраницаКонца";
		Запрос.УстановитьПараметр("СписокЗаявок", МассивЗаявок);
		Запрос.УстановитьПараметр("ДатаНач",      ДатаНач);
		Запрос.УстановитьПараметр("ГраницаКонца", ДатаКон + 86400);

		ЗапросУслуги = Новый Запрос;
		ЗапросУслуги.Текст =
		"ВЫБРАТЬ
		|	Работы.Ссылка КАК Ссылка,
		|	Работы.Номенклатура.Наименование КАК Услуга
		|ИЗ
		|	Документ.Заявка.Работы КАК Работы
		|ГДЕ
		|	Работы.Ссылка В (&СписокЗаявок)
		|УПОРЯДОЧИТЬ ПО
		|	Работы.НомерСтроки";
		ЗапросУслуги.УстановитьПараметр("СписокЗаявок", МассивЗаявок);

		УслугиПоЗаявкам = Новый Соответствие;
		Для Каждого СтрУсл Из ЗапросУслуги.Выполнить().Выгрузить() Цикл
			СписокУслуг = УслугиПоЗаявкам.Получить(СтрУсл.Ссылка);
			Если СписокУслуг = Неопределено Тогда
				СписокУслуг = Новый Массив;
				УслугиПоЗаявкам.Вставить(СтрУсл.Ссылка, СписокУслуг);
			КонецЕсли;
			Если Не ПустаяСтрока(СтрУсл.Услуга) Тогда
				СписокУслуг.Добавить(СтрУсл.Услуга);
			КонецЕсли;
		КонецЦикла;

		// Строки Работы для мультизаявок (Сотрудник + время построчно)
		ЗапросРаботы = Новый Запрос;
		ЗапросРаботы.Текст =
		"ВЫБРАТЬ
		|	Работы.Ссылка КАК Ссылка,
		|	Работы.Сотрудник КАК Сотрудник,
		|	Работы.ДатаНачала КАК ДатаНачала,
		|	Работы.ДатаОкончания КАК ДатаОкончания,
		|	Работы.Номенклатура КАК Номенклатура
		|ИЗ
		|	Документ.Заявка.Работы КАК Работы
		|ГДЕ
		|	Работы.Ссылка В(&СписокЗаявок)
		|	И Работы.Сотрудник <> ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
		|	И Работы.ДатаНачала <> ДАТАВРЕМЯ(1, 1, 1)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Работы.НомерСтроки";
		ЗапросРаботы.УстановитьПараметр("СписокЗаявок", МассивЗаявок);

		СтрокиРаботПоЗаявкам = Новый Соответствие;
		Для Каждого СтрР Из ЗапросРаботы.Выполнить().Выгрузить() Цикл
			СписокСтрок = СтрокиРаботПоЗаявкам.Получить(СтрР.Ссылка);
			Если СписокСтрок = Неопределено Тогда
				СписокСтрок = Новый Массив;
				СтрокиРаботПоЗаявкам.Вставить(СтрР.Ссылка, СписокСтрок);
			КонецЕсли;
			СписокСтрок.Добавить(СтрР);
		КонецЦикла;

		Для Каждого Стр Из Запрос.Выполнить().Выгрузить() Цикл

			Если Не ЗначениеЗаполнено(Стр.ДатаНачала) Тогда
				Продолжить;
			КонецЕсли;

			// Список приёмов заявки: {Сотрудник, Нач, Кон}
			Приемы = Новый Массив;
			
			ЭтоВрач = РольДоступна("омг_Врач");
			
			Если ЭтоВрач Тогда
				 ТекСотрудникВрач = ПараметрыСеанса.ТекущийПользователь.Сотрудник;
			КонецЕсли;
			
			ТекстУслугПоВрачу = "";
			
			Если Стр.НесколькоСотрудников Тогда
				// Мультизаявка — по строкам табличной части Работы
				СтрокиРабот = СтрокиРаботПоЗаявкам.Получить(Стр.Ссылка);
				Если СтрокиРабот <> Неопределено Тогда
					Для Каждого СтрР Из СтрокиРабот Цикл
						КонецПриема = ?(ЗначениеЗаполнено(СтрР.ДатаОкончания),
						СтрР.ДатаОкончания, СтрР.ДатаНачала);
						Если КонецПриема <= СтрР.ДатаНачала Тогда
							Продолжить;
						КонецЕсли;
						Приемы.Добавить(Новый Структура("Сотрудник, Нач, Кон",
						СтрР.Сотрудник, СтрР.ДатаНачала, КонецПриема)); 
						
						Если ЭтоВрач и ТекСотрудникВрач = СтрР.Сотрудник Тогда
							ТекстУслугПоВрачу = ТекстУслугПоВрачу + Символы.ПС + "  - " + СтрР.Номенклатура;
						КонецЕсли;
						
					КонецЦикла;
				КонецЕсли;
			Иначе
				// Обычная заявка — из шапки
				ДеньЗаявки    = НачалоДня(Стр.ДатаНачала);
				ДатаНачЗаявки = ДеньЗаявки + (Стр.ВремяНачала    - ПустаяДата);
				ДатаКонЗаявки = ДеньЗаявки + (Стр.ВремяОкончания - ПустаяДата);
				Если ДатаКонЗаявки > ДатаНачЗаявки Тогда
					Приемы.Добавить(Новый Структура("Сотрудник, Нач, Кон",
						Стр.Сотрудник, ДатаНачЗаявки, ДатаКонЗаявки));
				КонецЕсли;
			КонецЕсли;

			// Текст услуг (один список на всю заявку)
			СписокУслугЗаявки = УслугиПоЗаявкам.Получить(Стр.Ссылка);
			ТекстУслуг = "";
			Если СписокУслугЗаявки <> Неопределено Тогда  
				Если ЭтоВрач и Стр.НесколькоСотрудников и не ТекстУслугПоВрачу = "" Тогда
					
					ТекстУслуг = ТекстУслугПоВрачу;
					
				иначе	
					
					Для Каждого Услуга Из СписокУслугЗаявки Цикл
						ТекстУслуг = ТекстУслуг + Символы.ПС + "  - " + Услуга;
					КонецЦикла; 
			...
			// Цвет блока (один на всю заявку)
			Попытка
				ЦветБазы = Стр.СостояниеЦвет;
				ЦветЗ = ?(ТипЗнч(ЦветБазы) = Тип("ХранилищеЗначения"), ЦветБазы.Получить(), ЦветБазы);
				ЦветЗаявки = ?(ТипЗнч(ЦветЗ) = Тип("Цвет"), ЦветЗ, ЦветЗаявкиПоУмолч);
			Исключение
				ЦветЗаявки = ЦветЗаявкиПоУмолч;
			КонецПопытки;

			// Рисуем блок на каждый приём
			Для Каждого Прием Из Приемы Цикл

				Если Не ЗначениеЗаполнено(Прием.Сотрудник) Тогда
					Продолжить;
				КонецЕсли;

				НоваяЗапись = Планировщик.Элементы.Добавить(Прием.Нач, Прием.Кон);
				НоваяЗапись.Текст = Строка(Стр.Клиент);
				Если Стр.РискПадения = ПредопределенноеЗначение("Перечисление.омг_РискиПадения.Высокий") Тогда
					НоваяЗапись.Картинка = БиблиотекаКартинок.ОформлениеЗнакВосклицательныйЗнак;
				КонецЕсли;

				НоваяЗапись.Подсказка = СтрШаблон(
					"Пациент: %1%2%3 - %4%5Услуги:%6",
					Строка(Стр.Клиент),
					Символы.ПС,
					Формат(Прием.Нач, "ДФ=ЧЧ:мм"),
					Формат(Прием.Кон, "ДФ=ЧЧ:мм"),
					Символы.ПС,
					?(ПустаяСтрока(ТекстУслуг), Символы.ПС + "  Не указаны", ТекстУслуг));

				СоотвИзм = Новый Соответствие;
				СоотвИзм.Вставить("Врач", Прием.Сотрудник);
				НоваяЗапись.ЗначенияИзмерений = Новый ФиксированноеСоответствие(СоотвИзм);

				НоваяЗапись.ЦветФона = ЦветЗаявки;
				НоваяЗапись.Значение = Стр.Ссылка;

				// Интервал приёма — для расчёта свободных слотов
				СписокИнт = ЗаявкиПоОбъекту.Получить(Прием.Сотрудник);
				Если СписокИнт = Неопределено Тогда
					СписокИнт = Новый Массив;
					ЗаявкиПоОбъекту.Вставить(Прием.Сотрудник, СписокИнт);
				КонецЕсли;
				СписокИнт.Добавить(Новый Структура("Нач, Кон", Прием.Нач, Прием.Кон));

			КонецЦикла;

		КонецЦикла;
	КонецЕсли;	
	// --- Графики работы: ВСЕ виды времени ---
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА Г.Сотрудник <> ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
	|			ТОГДА Г.Сотрудник
	|		ИНАЧЕ Г.Кабинет
	|	КОНЕЦ КАК Объект,
	|	Г.Дата КАК Дата,
	|	Г.ВремяНачала КАК ВремяНачала,
	|	Г.ВремяОкончания КАК ВремяОкончания,
	|	Г.ВидВремени КАК ВидВремени,
	|	Г.ВидВремени.Наименование КАК НаименованиеВида,
	|	Г.ВидВремени.Цвет КАК ЦветВида,
	|	Г.ВидВремени.РабочееВремя КАК ВидРабочее,
	|	ВЫБОР
	|		КОГДА Г.ВидВремени = ЗНАЧЕНИЕ(Справочник.ВидыВремениГрафика.РабочееВремя)
	|				ИЛИ Г.ВидВремени = ЗНАЧЕНИЕ(Справочник.ВидыВремениГрафика.ПустаяСсылка)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоПростоеРабочее
	|ИЗ
	|	РегистрСведений.ГрафикиРаботы КАК Г
	|ГДЕ
	|	Г.Дата МЕЖДУ &ДатаНач И &ДатаКон
	|	И (Г.Сотрудник В (&Объекты) ИЛИ Г.Кабинет В (&Объекты))
	|УПОРЯДОЧИТЬ ПО
	|	Объект, Дата, Г.ВремяНачала";
	Запрос.УстановитьПараметр("ДатаНач", ДатаНач);
	Запрос.УстановитьПараметр("ДатаКон", КонецДня(ДатаКон));
	Запрос.УстановитьПараметр("Объекты", МассивОбъектов);
	ТаблицаГрафика = Запрос.Выполнить().Выгрузить();
	
	// --- Построение по каждому объекту и дню ---
	Для Каждого Объект Из МассивОбъектов Цикл
		
		ДлитСлотаСек = ДлительностьПоОбъекту.Получить(Объект);
		Если Не ЗначениеЗаполнено(ДлитСлотаСек) Или ДлитСлотаСек <= 0 Тогда
			ДлитСлотаСек = КвантВСекундах;
		КонецЕсли;
		
		СоотвИзмОбъекта = Новый Соответствие;
		СоотвИзмОбъекта.Вставить("Врач", Объект);
		ФиксИзмОбъекта = Новый ФиксированноеСоответствие(СоотвИзмОбъекта);
		
		ИнтервалыЗаявокОбъекта = ЗаявкиПоОбъекту.Получить(Объект);
		Если ИнтервалыЗаявокОбъекта = Неопределено Тогда
			ИнтервалыЗаявокОбъекта = Новый Массив;
		КонецЕсли;
		
		Для Каждого День Из МассивДней Цикл
			
			ДатаНачалаДня    = День + СекундаНачала;
			ДатаОкончанияДня = День + СекундаОкончания;
			
			СтрокиГрафика = ТаблицаГрафика.НайтиСтроки(Новый Структура("Объект, Дата", Объект, День));
			
			ИнтервалыВсегоГрафика = Новый Массив; // для расчёта нерабочего времени
			ИнтервалыРабочие      = Новый Массив; // простое рабочее время
			ИнтервалыИменованные  = Новый Массив; // обед / резерв / прочие виды
			
			Для Каждого СтрГраф Из СтрокиГрафика Цикл
				
				СекНач = СтрГраф.ВремяНачала - ПустаяДата;
				СекКон = ?(СтрГраф.ВремяОкончания = ПустаяДата, СекундаОкончания, СтрГраф.ВремяОкончания - ПустаяДата);
				
				ИнтНач = Макс(День + СекНач, ДатаНачалаДня);
				ИнтКон = Мин(День + СекКон, ДатаОкончанияДня);
				Если ИнтНач >= ИнтКон Тогда
					Продолжить;
				КонецЕсли;
				
				ИнтервалыВсегоГрафика.Добавить(Новый Структура("Нач, Кон", ИнтНач, ИнтКон));
				
				Если СтрГраф.ЭтоПростоеРабочее Тогда
					
					ИнтервалыРабочие.Добавить(Новый Структура("Нач, Кон", ИнтНач, ИнтКон));
					
				Иначе
					
					// Именованный вид времени (обед, резерв и т.п.)
					ИмяВида = ?(ПустаяСтрока(СтрГраф.НаименованиеВида), "Вид времени", СтрГраф.НаименованиеВида);
					
					ЦветВида = Неопределено;
					Попытка
						Если ТипЗнч(СтрГраф.ЦветВида) = Тип("ХранилищеЗначения") Тогда
							ЦветВида = СтрГраф.ЦветВида.Получить();
						КонецЕсли;
					Исключение
						ЦветВида = Неопределено;
					КонецПопытки;
					Если ТипЗнч(ЦветВида) <> Тип("Цвет") Или ЦветВида = Новый Цвет(0, 0, 0) Тогда
						ЦветВида = ЦветВидаУмолч;
					КонецЕсли;
					
					ИнтервалыИменованные.Добавить(Новый Структура("Нач, Кон", ИнтНач, ИнтКон));
					
					Элем = Планировщик.Элементы.Добавить(ИнтНач, ИнтКон);
					Элем.ЦветФона = ЦветВида;
					Элем.Текст = ИмяВида;
					Элем.Подсказка = ИмяВида + ": " + Формат(ИнтНач, "ДФ=ЧЧ:мм")
					+ "-" + Формат(ИнтКон, "ДФ=ЧЧ:мм");
					Элем.ЗначенияИзмерений = ФиксИзмОбъекта;
					
					Если СтрГраф.ВидРабочее = Истина Тогда
						// Бронируемый вид (резерв и т.п.) — клик создаёт заявку.
						// ДлительностьСек нужна, т.к. блок вида может быть длиннее одного слота.
						Элем.Значение = Новый Структура("Тип, ДлительностьСек", "Слот", ДлитСлотаСек);
					Иначе
						// Информационный вид (обед и т.п.) — клик ничего не делает.
						Элем.Значение = "ВидВремениИнфо";
					КонецЕсли;
					
				КонецЕсли;
			КонецЦикла;
			
			// --- Нерабочее время = день минус все строки графика ---
			ОбъединГрафик      = мОбъединитьИнтервалы(ИнтервалыВсегоГрафика);
			НерабочиеИнтервалы = мВычестьИнтервалы(ДатаНачалаДня, ДатаОкончанияДня, ОбъединГрафик);
			Для Каждого Инт Из НерабочиеИнтервалы Цикл
				Элем = Планировщик.Элементы.Добавить(Инт.Нач, Инт.Кон);
				Элем.ЦветФона = ЦветНераб;
				Элем.Текст    = "Нерабочее время";
				Элем.Значение = "НерабочееВремя";
				Элем.ЗначенияИзмерений = ФиксИзмОбъекта;
			КонецЦикла;
			
			// --- Свободные слоты = рабочее минус заявки минус именованные виды ---
			ЗанятыеИнтервалы = Новый Массив;
			Для Каждого Инт Из ИнтервалыИменованные Цикл
				ЗанятыеИнтервалы.Добавить(Инт);
			КонецЦикла;
			Для Каждого Инт Из ИнтервалыЗаявокОбъекта Цикл
				Если Инт.Кон > ДатаНачалаДня И Инт.Нач < ДатаОкончанияДня Тогда
					ЗанятыеИнтервалы.Добавить(Новый Структура("Нач, Кон",
					Макс(Инт.Нач, ДатаНачалаДня), Мин(Инт.Кон, ДатаОкончанияДня)));
				КонецЕсли;
			КонецЦикла;
			ОбъединЗанятые = мОбъединитьИнтервалы(ЗанятыеИнтервалы);
			
			ОбъединРабочие = мОбъединитьИнтервалы(ИнтервалыРабочие);
			Для Каждого РабИнт Из ОбъединРабочие Цикл
				СвободныеУчастки = мВычестьИнтервалы(РабИнт.Нач, РабИнт.Кон, ОбъединЗанятые);
				Для Каждого Уч Из СвободныеУчастки Цикл
					// Бьём свободный участок на слоты по длительности приёма
					СлотНач = Уч.Нач;
					Пока СлотНач < Уч.Кон Цикл
						СлотКон = Мин(СлотНач + ДлитСлотаСек, Уч.Кон);
						Элем = Планировщик.Элементы.Добавить(СлотНач, СлотКон);
						Элем.ЦветФона   = ЦветСлот;
						Элем.ЦветТекста = Новый Цвет(120, 120, 120);
						// В ячейке 15 минут длинный текст переносится и обрезается целиком.
						// Поэтому в ячейке — короткая метка, точный интервал — в подсказке.
						Элем.Текст      = "";
						Элем.Подсказка  = "Свободно для записи: " + Формат(СлотНач, "ДФ=ЧЧ:мм")
							+ "-" + Формат(СлотКон, "ДФ=ЧЧ:мм");
						Элем.ЗначенияИзмерений = ФиксИзмОбъекта;
						Элем.Значение   = Новый Структура("Тип", "Слот");
						СлотНач = СлотКон;
					КонецЦикла;
				КонецЦикла;
			КонецЦикла;		
...
Функция мОбъединитьИнтервалы(СписокИнтервалов)

ТЗ = Новый ТаблицаЗначений;
ТЗ.Колонки.Добавить("Нач");
ТЗ.Колонки.Добавить("Кон");
Для Каждого Инт Из СписокИнтервалов Цикл
	Если Инт.Нач < Инт.Кон Тогда
		Стр = ТЗ.Добавить();
		Стр.Нач = Инт.Нач;
		Стр.Кон = Инт.Кон;
	КонецЕсли;
КонецЦикла;
ТЗ.Сортировать("Нач Возр, Кон Возр");

Результат = Новый Массив;
Для Каждого Стр Из ТЗ Цикл
	Если Результат.Количество() = 0 Тогда
		Результат.Добавить(Новый Структура("Нач, Кон", Стр.Нач, Стр.Кон));
	Иначе
		Последний = Результат[Результат.Количество() - 1];
		Если Стр.Нач <= Последний.Кон Тогда
			Если Стр.Кон > Последний.Кон Тогда
				Последний.Кон = Стр.Кон;
			КонецЕсли;
		Иначе
			Результат.Добавить(Новый Структура("Нач, Кон", Стр.Нач, Стр.Кон));
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция мВычестьИнтервалы(Знач Нач, Знач Кон, ОбъединённыеЗанятые)
...
Курсор = Нач;
Для Каждого Зан Из ОбъединённыеЗанятые Цикл
	Если Зан.Кон <= Курсор Тогда
		Продолжить;
	КонецЕсли;
	Если Зан.Нач >= Кон Тогда
		Прервать;
	КонецЕсли;
	Если Зан.Нач > Курсор Тогда
		Результат.Добавить(Новый Структура("Нач, Кон", Курсор, Мин(Зан.Нач, Кон)));
	КонецЕсли;
	Если Зан.Кон > Курсор Тогда
		Курсор = Зан.Кон;
	КонецЕсли;
	Если Курсор >= Кон Тогда
		Прервать;
	КонецЕсли;
КонецЦикла;
Если Курсор < Кон Тогда
	Результат.Добавить(Новый Структура("Нач, Кон", Курсор, Кон));
КонецЕсли;
...
&НаСервере
Процедура ДобавитьНерабочийЭлемент(Планировщик, ДатаНач, ДатаКон, Объект, ЦветНераб)

Если ДатаНач >= ДатаКон Тогда Возврат КонецЕсли;

Элем = Планировщик.Элементы.Добавить(ДатаНач, ДатаКон);
Элем.ЦветФона = ЦветНераб;
Элем.Текст    = "Нерабочее время";
Элем.Значение = "НерабочееВремя"; // маркер для обработчика клика

СоотвИзм = Новый Соответствие;
СоотвИзм.Вставить("Врач", Объект);
Элем.ЗначенияИзмерений = Новый ФиксированноеСоответствие(СоотвИзм);
...
омг_ЗаполнитьЗаявки(); 
омг_ЗаполнитьПриемы();
Если омг_РольДоступна() Тогда
	Элементы.омг_МедСестра.Видимость = Истина; 
	Элементы.омг_Лаборант.Видимость = Истина; 
	Элементы.Омг_Врач.Видимость = Ложь;
иначе
	Элементы.омг_МедСестра.Видимость = Ложь; 
	Элементы.омг_Лаборант.Видимость = Ложь;
	Элементы.Омг_Врач.Видимость = Истина;
КонецЕсли;  
Скрытие = Ложь;	
// ОМГ
////////////////////////////////////////////////// 
ОбновитьРежимОтображениеВсехЗаписей ();
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьРежимОтображениеВсехЗаписей ()
Если ПодробныйРежимТаблиц Тогда
	Элементы.ГруппаКомпактныхТаблиц.Видимость = Ложь;
	Элементы.ГруппаПодробныхТаблиц.Видимость = Истина;
Иначе
	Элементы.ГруппаКомпактныхТаблиц.Видимость = Истина;
	Элементы.ГруппаПодробныхТаблиц.Видимость = Ложь;
КонецЕсли;
...
ИначеЕсли ИмяСобытия = "ЗаписанПрием" Тогда
	Если Не СоздаватьПовторныеПриемыПоЗаявкеИзЖурналаЗаписи Тогда
		Если мДанныеЗаявок <> Неопределено Тогда
...
//
&НаКлиенте
Процедура ОбновитьОтчет(УстановитьФокусВводаНаТаблице = Истина,НажатиеКнопки = Ложь)

////Карсаков (
//Проверка1 = АктивноеОкно();
//Проверка2 = АктивноеОкно().НачальнаяСтраница;
//)
////Карсаков. Комментирую старый код, ниже добавлен новый код для работы на 8.5
//Если не НажатиеКнопки Тогда 
//	Если не АктивноеОкно() = Неопределено Тогда
//		Если (АктивноеОкно().Заголовок = "Календарь планирования" ИЛИ 
//			АктивноеОкно().Заголовок = "Кабинет ""Регистратор""" ИЛИ 
//			АктивноеОкно().Заголовок = "Кабинет ""Администратор""" ИЛИ 
//			АктивноеОкно().Заголовок = "Кабинет ""Врач""" ИЛИ 
//			АктивноеОкно().Заголовок = "Кабинет ""Оператор""" ИЛИ 
//			АктивноеОкно().Заголовок = "Кабинет ""Окно работы с WhatsApp""" ИЛИ 
//			АктивноеОкно().Заголовок = "Кабинет ""ОЛИМП.Phone""") И ЭтаФорма.Элементы.Панель.ТекущаяСтраница.Имя = "ЖурналЗаписи" Тогда
//		иначе	
//		ПодключитьАвтообновлениеОтчета();
//		Возврат;
//	КонецЕсли;
...
Если Не НажатиеКнопки Тогда
	...
	Если ТекОкно <> Неопределено Тогда
		...
		ЗаголовокОкна = ТекОкно.Заголовок;
		
		ЭтоНужноеОкно = (ЗаголовокОкна = "Календарь планирования" ИЛИ 
		ЗаголовокОкна = "Кабинет ""Регистратор""" ИЛИ 
		ЗаголовокОкна = "Кабинет ""Администратор""" ИЛИ 
		ЗаголовокОкна = "Кабинет ""Врач""" ИЛИ 
		ЗаголовокОкна = "Кабинет ""Оператор""" ИЛИ 
		ЗаголовокОкна = "Кабинет ""Окно работы с WhatsApp""" ИЛИ
		ЗаголовокОкна = "Журнал записей пациентов");
		
		Если Не ЭтоНужноеОкно Тогда
			ЭтоНужноеОкно = СтрНачинаетсяС(ЗаголовокОкна, "Окно телефонии");
		КонецЕсли;
		
		Если ЭтоНужноеОкно И (ЭтаФорма.Элементы.Панель.ТекущаяСтраница.Имя = "ЖурналЗаписи"
			Или ЭтаФорма.Элементы.Панель.ТекущаяСтраница.Имя = "Планировщик") Тогда
	...
	КонецЕсли;
КонецЕсли;
////Карсаков. Конец изменений для 8.5
...
// Добавлено Макеев (
ПараметрыФормирования.Вставить("омг_Организация",			Отчет.омг_Организация);
// )

///////////////////////////////////////
// ОМГ не разрешаем всем
ПараметрыФормирования.Вставить("мСоздаватьПовторныеПриемыПоЗаявкеИзЖурналаЗаписи", Ложь);
...
///Карсаков. Для планировщика 8.5
мСписокОбъектовПланировщика = РезультатФормирования.СписокОбъектовПланировщика;
///Карсаков. Конец

////Карсаков. Скрыто, используется планировщик. Этот код нужен для старой таблицы
...
КонецЦикла;
////////////////////////////////////
////Карсаков: таблица скрыта, цикл не нужен. Используется планировщик
...
//	КонецЦикла;	
//КонецЦикла;
////Карсаков: таблица скрыта, цикл выше не нужен. Используется планировщик
...
ПодключитьАвтообновлениеОтчета();
// Добавлено Карсаков: обновление планировщика
//ЗаполнитьДанныеПланировщикаНаСервере(мРасшифровкиКолонок, мДанныеЗаявок);
ЗаполнитьДанныеПланировщикаНаСервере(мСписокОбъектовПланировщика, ПолучитьОтфильтрованныеЗаявки());
//Конец добавления Карсаков
...
////Карсаков. Код закомментирован, так как не используется для планировщика 8.5
...
////////////////////////////////////////////////////
//// ДОбавлено Карсаков: Параметры для планировщика
//НастройкиПланировщика = Новый Структура;
//НастройкиПланировщика.Вставить("КвантВСекундах", 
//?(ЗначениеЗаполнено(ОтчетОбъект.мКвантПланирования), ОтчетОбъект.мКвантПланирования, 900));
//НастройкиПланировщика.Вставить("ВремяНачала", 
//?(ЗначениеЗаполнено(ОтчетОбъект.мВремяНачалаДня), ОтчетОбъект.мВремяНачалаДня, '00010101080000'));
//НастройкиПланировщика.Вставить("ВремяОкончания", 
//?(ЗначениеЗаполнено(ОтчетОбъект.мВремяОкончанияДня), ОтчетОбъект.мВремяОкончанияДня, '00010101200000'));
//
//РезультатФормирования.Вставить("НастройкиПланировщика", НастройкиПланировщика);
////Конец добавления Карсаков
//
...
///Карсаков. Конец. Действующий код для 8.5 ниже
УстановитьПривилегированныйРежим(Истина);

ОтчетОбъект = Отчеты.КалендарьПланирования.Создать();
ЗаполнитьЗначенияСвойств(ОтчетОбъект, ПараметрыФормирования);

ДанныеПланировщика = ОтчетОбъект.СформироватьДанныеДляПланировщика();

РезультатФормирования = Новый Структура;

РезультатФормирования.Вставить("ДокументРезультат", Новый ТабличныйДокумент);

РезультатФормирования.Вставить("ДанныеЗаявок",               ДанныеПланировщика.ДанныеЗаявок);
РезультатФормирования.Вставить("СписокОбъектовПланировщика", ДанныеПланировщика.Объекты);

РезультатФормирования.Вставить("ПовторяющиесяРасшифровкиСвободногоВремени", Новый Соответствие);
РезультатФормирования.Вставить("РасшифровкиКолонок",      Новый Соответствие);
РезультатФормирования.Вставить("РасшифровкиСтрок",        Новый Соответствие);
РезультатФормирования.Вставить("ПредставленияСубъектов",  Новый Соответствие);
РезультатФормирования.Вставить("ИндексыФилиалов",         Новый Массив);
РезультатФормирования.Вставить("ОтдельныеКолонкиГрафика", Новый Массив);

Если ДанныеПланировщика.Свойство("ДанныеПриемовЗаявок") Тогда
	РезультатФормирования.Вставить("ДанныеПриемовЗаявок", ДанныеПланировщика.ДанныеПриемовЗаявок);
КонецЕсли;

Если ДанныеПланировщика.Свойство("МестонахождениеМедКарт") Тогда
	РезультатФормирования.Вставить("МестонахождениеМедКарт",
	ОбщегоНазначенияСервер.ПреобразоватьТаблицуЗначенийВМассивСтруктур(ДанныеПланировщика.МестонахождениеМедКарт));
КонецЕсли;

РезультатФормирования.Вставить("ЕстьЗаявкиХочуРаньше",
?(ДанныеПланировщика.Свойство("ЕстьЗаявкиХочуРаньше"), ДанныеПланировщика.ЕстьЗаявкиХочуРаньше, Ложь));
...
&НаСервереБезКонтекста
Функция ПолучитьДанныеЗаявокДляПланировщика(СписокЗаявок)

Если СписокЗаявок = Неопределено 
	Или СписокЗаявок.Количество() = 0 
	Тогда
	Возврат Новый Массив;
КонецЕсли;

МассивЗаявок = Новый Массив;
Для Каждого КЗ Из СписокЗаявок Цикл
	МассивЗаявок.Добавить(КЗ.Ключ);
КонецЦикла;
...
Иначе 
	...
	Если не ЗначениеЗаполнено(Область.Текст) Тогда
		ОбластьВерх = Омг_ВернутьЗаполненнуюОбласть(Область)
	иначе
		ОбластьВерх = Область
	КонецЕсли;
	
	Возврат ПолучитьРасшифровкуПустойЯчейки(ОбластьВерх.Верх, ОбластьВерх.Лево, ОбластьВерх.Низ, Расшифровка);
...
Если Истина Тогда // мУчетнаяПолитика.ИспользоватьСостоянияЗаявок
	...
	Пока ВыбСостояния.Следующий() Цикл
		...
		Если ВыбСостояния.ПометкаУдаления Тогда
			Продолжить;
		КонецЕсли;
...
Если ЭтоЗаявка Или ЭтоСвободноеВремя Тогда
	...
	Если ЭтоЗаявка Тогда
		// омг Разрешаем всем
		//Если ЕстьПравоДоступаПриемВрача Тогда
		КомандаПриема = КомандыКонтекстногоМенюЖурнала.НайтиПоЗначению("СоздатьПрием");
		УстановитьПредставлениеКомандеСозданияПриема(Расшифровка, КомандаПриема);
		ДобавитьЭлементСпискаЗначенийКопированием(СписокКнопокМеню, КомандаПриема);
		//КонецЕсли; 
...
ИначеЕсли ТипЗнч(РасшифровкаЯчейки)= Тип("ДокументСсылка.Заявка") Тогда 
	...
	/////////////////////////////////////////////////
	// ОМГ по умолчанию открываем заявку  
...
&НаКлиенте
Функция НайтиПриемыПоЗаявке(ДанныеПриемовЗаявки, Объект)
...
Возврат НайденныеАналогичныеПриемы;

КонецФункции

&НаКлиенте
Процедура УстановитьПредставлениеКомандеСозданияПриема(Заявка, КомандаПриема)

//Если Не СоздаватьПовторныеПриемыПоЗаявкеИзЖурналаЗаписи
//	И мДанныеПриемовЗаявок <> Неопределено
//Тогда
////////////////////////////////////
// ОМГ разрешаем в любом случае 
Если мДанныеПриемовЗаявок <> Неопределено Тогда
	ДанныеПриемовЗаявки = мДанныеПриемовЗаявок.Получить(Заявка);
	Объект = ПолучитьОбъектТекущейКолонки();
	Если ДанныеПриемовЗаявки <> Неопределено Тогда
		НайденныеАналогичныеПриемы = НайтиПриемыПоЗаявке(ДанныеПриемовЗаявки, Объект);
		Если НайденныеАналогичныеПриемы.Количество() > 0 Тогда
			КомандаПриема.Представление = "Открыть прием";
			Возврат;
		КонецЕсли;
	КонецЕсли;
КонецЕсли;

КомандаПриема.Представление = "Создать прием";
...
Если не Заявка = Неопределено Тогда
	...
	СписокОказаний = омг_ДополнительныеАлгоритмы.ВернутьОказанияУслугПоЗаявке(Заявка);
	
	Если СписокОказаний.Количество() = 1 Тогда
		ОткрытьФорму("Документ.ОказаниеУслуг.ФормаОбъекта", Новый Структура("Ключ", СписокОказаний[0]));
		Возврат;
	ИначеЕсли СписокОказаний.Количество() > 1 Тогда
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Заявка", Заявка);
		ПараметрыФормы.Вставить("ОказанияЗаявки", СписокОказаний);
		ОткрытьФорму("Документ.Заявка.Форма.ПриемыЗаявки", ПараметрыФормы);
		Возврат;
	КонецЕсли;  
...
ОткрытьФорму("ОбщаяФорма.ФормаПоискаКлиентаУпр_85",,Элементы.Клиент);
КонецПроцедуры
...
Пока ШагПрохода <= Элементы.ЗадачиИНапоминания.ПолучитьСтрокиДляОбработки().Количество() - 1 Цикл
	ВыбЗадача = Элементы.ЗадачиИНапоминания.ТекущиеДанные.ЗадачаСсылка;
...
#Область КопироватьВырезатьВставитьЗаявку // Команды формы "Копировать", "Вырезать", "Вставить" заявку.
...
// 0 - копировать
// 1 - вырезать
// 2 - вставить после вырезания/копирования.
&НаКлиенте
...
Если БуферОбменаПоследняяОперацияВырКоп = 0 Тогда
	ЗначенияЗаполнения.Вставить("Дата", ТекущаяДата());
	СоздатьНовуюЗаявку(БуферОбменаЗаявка,ЗначенияЗаполнения);
ИначеЕсли БуферОбменаПоследняяОперацияВырКоп = 1 Тогда
	РедактироватьЗаявку(БуферОбменаЗаявка,ЗначенияЗаполнения);
...
&НаСервереБезКонтекста
Процедура СоздатьНовуюЗаявку(ЗаявкаСсылка,ЗначенияЗаполнения)

ВидыСостояний = Справочники.ВидыСостоянийЗаявок;

НоваяЗаявка = ЗаявкаСсылка.Скопировать();
НоваяЗаявка.Ответственный = Пользователи.ТекущийПользователь();

Если УправлениеЗаявками.СостоянияЗаявкиДоПриходаКлиента().Найти(НоваяЗаявка.Состояние) = Неопределено Тогда
	НоваяЗаявка.Состояние = ВидыСостояний.Запланирована;
КонецЕсли;

ДополнитьИЗаписатьЗаявкуПриВставке(НоваяЗаявка, ЗначенияЗаполнения);

КонецПроцедуры

&НаСервереБезКонтекста
Процедура РедактироватьЗаявку(ЗаявкаСсылка, ЗначенияЗаполнения)

ЗаявкаОбъект = ЗаявкаСсылка.ПолучитьОбъект();
ДополнитьИЗаписатьЗаявкуПриВставке(ЗаявкаОбъект, ЗначенияЗаполнения);

КонецПроцедуры

&НаСервереБезКонтекста
Процедура ДополнитьИЗаписатьЗаявкуПриВставке(ЗаявкаОбъект, ЗначенияЗаполнения)

ЗначенияЗаполнения.Вставить("ДатаОкончания", ЗаявкаОбъект.ДатаОкончания + (ЗначенияЗаполнения.ДатаНачала - ЗаявкаОбъект.ДатаНачала));
ЗначенияЗаполнения.Вставить("ВремяОкончания", ЗначенияЗаполнения.ДатаОкончания);

// Если в Заявке заполнен сотрудник/кабинет, а в ЗначенияЗапонения (в ячейке ЖЗ) кого-то нет, то не затираем их
ПоляИсключения = "";
ПоляПроверкиИсключения = ОбщегоНазначенияБИТКлиентСервер.НовыйМассив("Сотрудник", "Кабинет");
Для Каждого ПолеПроверки Из ПоляПроверкиИсключения Цикл
	Если ЗначениеЗаполнено(ЗаявкаОбъект[ПолеПроверки]) 
		И ЗначенияЗаполнения.Свойство(ПолеПроверки)
		И Не ЗначениеЗаполнено(ЗначенияЗаполнения[ПолеПроверки]) 
		Тогда
		ОбщегоНазначенияБИТКлиентСервер.КонкатенацияСтрок(ПоляИсключения, ПолеПроверки);
	КонецЕсли;
КонецЦикла;

// Обновление значений реквизитов Заявки
ЗаполнитьЗначенияСвойств(ЗаявкаОбъект, ЗначенияЗаполнения,, ПоляИсключения);
...
&НаКлиенте
Процедура омг_ОбновитьОтчет(Команда) 
...
&НаКлиенте
Функция Омг_ВернутьЗаполненнуюОбласть(ПереданнаяОбласть)

ЛинияПустая = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии, 1); 
ОбластьВерх = ПереданнаяОбласть;
Если не ЗначениеЗаполнено(ПереданнаяОбласть.Текст) и ПереданнаяОбласть.ГраницаСверху = ЛинияПустая Тогда 
	Сч = ПереданнаяОбласть.Низ -1;	
	Пока Сч > 1 Цикл 
		ОбластьВерх = ДокументРезультат.Область("R"+Сч+"C"+ПереданнаяОбласть.Лево);
		Сч = Сч-1;
		Если ЗначениеЗаполнено(ОбластьВерх.Текст) и не ОбластьВерх.ГраницаСверху = ЛинияПустая Тогда
			Сч = 1
		КонецЕсли;
	КонецЦикла;		
КонецЕсли;
...
&НаСервере
Процедура омг_ЗаполнитьЗаявки()

Если Не ЗначениеЗаполнено(омг_ПериодФормированияПериод.ДатаНачала) и
	Не ЗначениеЗаполнено(омг_ПериодФормированияПериод.ДатаОкончания)
	Тогда
	
	ДатаНачала = НачалоДня(ТекущаяДата());
	ДатаОкончания = КонецДня(ТекущаяДата());
	омг_ПериодФормированияПериод.ДатаНачала = НачалоДня(ТекущаяДата());	 
	омг_ПериодФормированияПериод.ДатаОкончания = КонецДня(ТекущаяДата());
	омг_ПериодФормирования = омг_ПериодФормированияПериод.ДатаНачала;
КонецЕсли;

ДатаНачала = омг_ПериодФормированияПериод.ДатаНачала;
ДатаОкончания = омг_ПериодФормированияПериод.ДатаОкончания;


Запрос = Новый Запрос;
Сотрудник = Справочники.Сотрудники.ПустаяСсылка();
Условия = "";
Если омг_РольДоступна() и ЗначениеЗаполнено(ПараметрыСеанса.ТекущийПользователь.Сотрудник) Тогда
	
	Сотрудник = ПараметрыСеанса.ТекущийПользователь.Сотрудник; 
	
	Условия = "
	|
	|    И (Заявка.Сотрудник = &Врач или Заявка.Ссылка.Сотрудник = &Врач
	|        ИЛИ Заявка.Ссылка В (
	|            ВЫБРАТЬ
	|                МедПерсонал.Ссылка
	|            ИЗ
	|                Документ.Заявка.омг_МедПерсонал КАК МедПерсонал
	|            ГДЕ
	|                МедПерсонал.Сотрудник = &Врач
	|        )
	|    )";
	
	Запрос.УстановитьПараметр("Врач",Сотрудник); 
	
иначеЕсли ЗначениеЗаполнено(ЭтотОбъект.Омг_Врач) Тогда
	Условия	="
	|
	|	И (Заявка.Сотрудник = &Врач или Заявка.Ссылка.Сотрудник = &Врач)
	|";
	Запрос.УстановитьПараметр("Врач",ЭтотОбъект.Омг_Врач);
КонецЕсли;		

Если ЗначениеЗаполнено(ЭтотОбъект.омг_ИИН) Тогда
	
	Условия	= Условия + "
	|
	|	И Заявка.Ссылка.Клиент.ИНН = &омг_ИИН
	|";
	Запрос.УстановитьПараметр("омг_ИИН",ЭтотОбъект.омг_ИИН);  
	
КонецЕсли;   
Если ЗначениеЗаполнено(ЭтотОбъект.омг_Клиника) Тогда
	
	Условия	= Условия + "
	|
	|	И Заявка.Ссылка.Филиал = &омг_Клиника
	|";
	Запрос.УстановитьПараметр("омг_Клиника",ЭтотОбъект.омг_Клиника);  
	
КонецЕсли;

Если ЗначениеЗаполнено(ЭтотОбъект.омг_Клиент) Тогда
	
	Условия	= Условия + "
	|
	|	И Заявка.Ссылка.Клиент = &омг_Клиент
	|";
	Запрос.УстановитьПараметр("омг_Клиент",ЭтотОбъект.омг_Клиент);
	
КонецЕсли;   

Если ЗначениеЗаполнено(ЭтотОбъект.омг_МедПерсонал) Тогда
	...
	|                Документ.Заявка.омг_МедПерсонал КАК МедПерсонал
	|            ГДЕ
	|                МедПерсонал.Сотрудник = &МедПерсонал
	|        )
	|";
	Запрос.УстановитьПараметр("МедПерсонал",ЭтотОбъект.омг_МедПерсонал);
	
КонецЕсли;  

Если ЗначениеЗаполнено(ЭтотОбъект.Омг_СтраховаяПрограмма) Тогда
	
	Условия	= Условия + "
	|
	|	И Заявка.Ссылка.омг_СтраховаяПрограмма = &Омг_СтраховаяПрограмма
	|";
	Запрос.УстановитьПараметр("Омг_СтраховаяПрограмма",ЭтотОбъект.Омг_СтраховаяПрограмма);
	
КонецЕсли;

Если ЗначениеЗаполнено(ЭтотОбъект.омг_Состояние) Тогда
	
	Условия	= Условия + "
	|
	|	И Заявка.Ссылка.Состояние = &омг_Состояние
	|";
	Запрос.УстановитьПараметр("омг_Состояние",ЭтотОбъект.омг_Состояние);
	
КонецЕсли;   

Если ЗначениеЗаполнено(ЭтотОбъект.омг_Автор) Тогда
	
	Условия	= Условия + "
	|
	|	И Заявка.Ссылка.Ответственный = &омг_Автор
	|";
	Запрос.УстановитьПараметр("омг_Автор",ЭтотОбъект.омг_Автор);
...
"ВЫБРАТЬ
|	ВЫБОР
|		КОГДА Заявка.Ссылка.НесколькоСотрудников
|			ТОГДА Заявка.ДатаНачала
|		ИНАЧЕ Заявка.Ссылка.ДатаНачала
|	КОНЕЦ КАК Время,
|	ВЫБОР
|		КОГДА Заявка.Ссылка.НесколькоСотрудников
|			ТОГДА Заявка.ДатаНачала
...
|	Заявка.Ссылка.Клиент КАК Клиент,
|	Заявка.Ссылка.Клиент.ИНН КАК ИИН,
|	Заявка.Ссылка.омг_СтраховаяПрограмма КАК СтраховаяПрограмма,
|	ВЫБОР
|		КОГДА Заявка.Сотрудник = ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
|			ТОГДА Заявка.Ссылка.Сотрудник
|		ИНАЧЕ Заявка.Сотрудник
|	КОНЕЦ КАК Врач,
|	Заявка.Ссылка КАК Ссылка,
|	Заявка.Ссылка.Ответственный КАК Ответственный,
|	Заявка.Ссылка.Омг_СостояниеПоПриемам КАК СтатусПриема,
|	ВЫБОР
|		КОГДА Заявка.Ссылка.Клиент.омг_РискПадения = ЗНАЧЕНИЕ(Перечисление.омг_РискиПадения.Высокий)
|			ТОГДА ""Высокий""
|		ИНАЧЕ ""Низкий""
| 	КОНЕЦ КАК РискПадения,
|	Заявка.Ссылка.Состояние КАК Состояние
|ИЗ
|	Документ.Заявка.Работы КАК Заявка
|ГДЕ
|	ВЫБОР
|			КОГДА Заявка.Ссылка.НесколькоСотрудников
|				ТОГДА Заявка.ДатаНачала МЕЖДУ &ДатаНачала И &ДатаОкончания
|			ИНАЧЕ Заявка.Ссылка.ДатаНачала МЕЖДУ &ДатаНачала И &ДатаОкончания
|		КОНЕЦ
|	И Заявка.Ссылка.омг_БезЗаписи = ЛОЖЬ
|   И Заявка.омг_БезЗаписиНесколькоВрачей = ЛОЖЬ "	
+Условия+
"
|
|СГРУППИРОВАТЬ ПО
|	Заявка.Ссылка,
|	Заявка.Ссылка.Клиент,
|	Заявка.Ссылка.Клиент.ИНН,
|	Заявка.Ссылка.омг_СтраховаяПрограмма,
|	ВЫБОР
|		КОГДА Заявка.Ссылка.Клиент.омг_РискПадения = ЗНАЧЕНИЕ(Перечисление.омг_РискиПадения.Высокий)
|			ТОГДА ""Высокий""
|		ИНАЧЕ ""Низкий""
| 	КОНЕЦ,
|	Заявка.Ссылка.Ответственный,
|	Заявка.Ссылка.Омг_СостояниеПоПриемам,
|	Заявка.Ссылка.Состояние, 
|	Выбор 
|	когда Заявка.Ссылка.Клиент.омг_РискПадения = Значение(Перечисление.омг_РискиПадения.Высокий) 
|	Тогда
|	""Высокий"" 
...
|			ИНАЧЕ Заявка.Ссылка.ДатаНачала МЕЖДУ &ДатаНачала И &ДатаОкончания
|		КОНЕЦ
|	И Заявка.омг_БезЗаписиНесколькоВрачей"	
+Условия+
"
|
|СГРУППИРОВАТЬ ПО
...
омг_Заявки.Очистить();
омг_ЗявкиБезЗаписи.Очистить(); 
омг_Заявки.Загрузить(РезультатЗапроса[0].Выгрузить());
омг_ЗявкиБезЗаписи.Загрузить(РезультатЗапроса[1].Выгрузить());

Медсестра = омг_МедицинскаяДеятельность.ВернутьМедПерсоналПоВрачу(Сотрудник,Перечисления.омг_РолиПерсонала.Медсестра);
Лаборант = омг_МедицинскаяДеятельность.ВернутьМедПерсоналПоВрачу(Сотрудник,Перечисления.омг_РолиПерсонала.Лаборант);

Если не ЗначениеЗаполнено(омг_МедСестра) и не Медсестра = Неопределено Тогда
	омг_МедСестра = Медсестра	
КонецЕсли;  

Если не ЗначениеЗаполнено(омг_Лаборант) и не Лаборант = Неопределено Тогда
	омг_Лаборант = Лаборант	
КонецЕсли;
Если ЗначениеЗаполнено(ЭтотОбъект.ТекущееПолеСортировки)
	И ЗначениеЗаполнено(ЭтотОбъект.ТекущееНаправлениеСортировки) Тогда
	
	Омг_СортироватьТаблицыНаСервере(
	ЭтотОбъект.ТекущееПолеСортировки,
	ЭтотОбъект.ТекущееНаправлениеСортировки
	);
КонецЕсли;   
...
&НаСервере
Процедура омг_ЗаполнитьПриемы()

//Если Не ЗначениеЗаполнено(омг_ПериодФормированияПриемы)  Тогда
//	
//	ДатаНачала = НачалоДня(ТекущаяДата());
//	ДатаОкончания = КонецДня(ТекущаяДата());
//    омг_ПериодФормированияПриемы = ТекущаяДата();	
//КонецЕсли;

Если Не ЗначениеЗаполнено(омг_ПериодФормированияПриемыПериод.ДатаНачала) и
	Не ЗначениеЗаполнено(омг_ПериодФормированияПриемыПериод.ДатаОкончания)
	Тогда
	
	ДатаНачала = НачалоДня(ТекущаяДата());
	ДатаОкончания = КонецДня(ТекущаяДата());
	омг_ПериодФормированияПриемыПериод.ДатаНачала = НачалоДня(ТекущаяДата());	 
	омг_ПериодФормированияПриемыПериод.ДатаОкончания = КонецДня(ТекущаяДата());
	омг_ПериодФормированияПриемы = омг_ПериодФормированияПриемыПериод.ДатаНачала;
КонецЕсли;

ДатаНачала = омг_ПериодФормированияПриемыПериод.ДатаНачала;
ДатаОкончания = омг_ПериодФормированияПриемыПериод.ДатаОкончания;


Сотрудник = Справочники.Сотрудники.ПустаяСсылка();

Если омг_РольДоступна() и ЗначениеЗаполнено(ПараметрыСеанса.ТекущийПользователь.Сотрудник) и не ЗначениеЗаполнено(Омг_ВрачПриемы) Тогда
	
	Сотрудник = ПараметрыСеанса.ТекущийПользователь.Сотрудник;
	Омг_ВрачПриемы = Сотрудник;
	
ИначеЕсли ЗначениеЗаполнено(Омг_ВрачПриемы)  Тогда
	
	Сотрудник = Омг_ВрачПриемы   
	
КонецЕсли;	
//Добавлено Карсаков
//Добавляем отборы по календарю
Этаформа.омг_СписокПриемов.Отбор.Элементы.Очистить();
ОтборПометка = Этаформа.омг_СписокПриемов.Отбор.Элементы.Добавить(тип("ЭлементОтбораКомпоновкиДанных")); 
ОтборПометка.ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно;
ОтборПометка.ЛевоеЗначение = омг_СписокПриемов.Отбор.ДоступныеПоляОтбора.Элементы.Найти("Дата").Поле;
ОтборПометка.ПравоеЗначение = ДатаНачала;
ОтборПометка.Использование = Истина;

ОтборПометка = Этаформа.омг_СписокПриемов.Отбор.Элементы.Добавить(тип("ЭлементОтбораКомпоновкиДанных"));
ОтборПометка.ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно;
ОтборПометка.ЛевоеЗначение = омг_СписокПриемов.Отбор.ДоступныеПоляОтбора.Элементы.Найти("Дата").Поле;
ОтборПометка.ПравоеЗначение = ДатаОкончания;
ОтборПометка.Использование = Истина;
...
Если не Сотрудник = Справочники.Сотрудники.ПустаяСсылка() Тогда 
	...
	//Этаформа.омг_СписокПриемов.Отбор.Элементы.Очистить();
	ОтборПометка = Этаформа.омг_СписокПриемов.Отбор.Элементы.Добавить(тип("ЭлементОтбораКомпоновкиДанных")); 
	ОтборПометка.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборПометка.ЛевоеЗначение = омг_СписокПриемов.Отбор.ДоступныеПоляОтбора.Элементы.Найти("Врач").Поле;
	ОтборПометка.ПравоеЗначение = Сотрудник; 
	ОтборПометка.Использование = Истина; 
КонецЕсли;		
...
&НаКлиенте
Процедура омг_ПериодФормированияПриИзменении(Элемент)

омг_ПериодФормированияПериод.ДатаНачала = НачалоДня(омг_ПериодФормирования);
омг_ПериодФормированияПериод.ДатаОкончания = КонецДня(омг_ПериодФормирования);
омг_ЗаполнитьЗаявки();

КонецПроцедуры  
&НаКлиенте
Процедура омг_ПериодФормированияПриемовПриИзменении(Элемент)

омг_ПериодФормированияПриемыПериод.ДатаНачала = НачалоДня(омг_ПериодФормированияПриемы);
омг_ПериодФормированияПриемыПериод.ДатаОкончания = КонецДня(омг_ПериодФормированияПриемы);
омг_ЗаполнитьПриемы();
...
&НаКлиенте
Процедура омг_ЗаполнитьЗаявкиНаФорме(Команда) 
...
&НаКлиенте
Процедура омг_КлиентПриИзменении(Элемент)
омг_ЗаполнитьЗаявки() 
КонецПроцедуры
...
&НаКлиенте
Процедура ИИНПриИзменении(Элемент)
омг_ЗаполнитьЗаявки() 
КонецПроцедуры

&НаСервере
Функция омг_РольДоступна()   

Возврат омг_МедицинскаяДеятельность.ЭтоВрач() 

КонецФункции	

&НаКлиенте
Процедура омг_СоздатьПрием(Команда)

ПараметрыФормы = Новый Структура("Основание", Элементы.омг_Заявки.ТекущиеДанные.Ссылка);
ДополнительныеПараметрыФормы = Новый Структура; 
ДополнительныеПараметрыФормы.Вставить("омг_МедСестра",омг_МедСестра);
ДополнительныеПараметрыФормы.Вставить("омг_Лаборант",омг_Лаборант); 
ДополнительныеПараметрыФормы.Вставить("омг_Врач",Элементы.омг_Заявки.ТекущиеДанные.Врач);

ПараметрыФормы.Вставить("ДополнительныеПараметры",ДополнительныеПараметрыФормы);
...
&НаКлиенте
Процедура омг_ОткрытьЗаявку(Команда)

омг_ОткрытьЗаявкуОбщ()

КонецПроцедуры

&НаСервере
Процедура омг_МедСестраПриИзмененииНаСервере(ЭтоМедсестра = Истина)

Если омг_РольДоступна() и ЗначениеЗаполнено(ПараметрыСеанса.ТекущийПользователь.Сотрудник)  Тогда
	
	Если ЭтоМедсестра Тогда
		
		УстановитьПривилегированныйРежим(Истина);
		
		Сотрудник = ПараметрыСеанса.ТекущийПользователь.Сотрудник;
		ЗаписьВрегистр = РегистрыСведений.омг_СвязиМедПерсонала.СоздатьМенеджерЗаписи();
		ЗаписьВрегистр.Врач = Сотрудник;
		ЗаписьВрегистр.Медсестра = Омг_Медсестра;
		ЗаписьВрегистр.Период = ТекущаяДата(); 
		ЗаписьВрегистр.омг_РольПерсонала = Перечисления.омг_РолиПерсонала.Медсестра;
	...
	иначе 
		...
		ЗаписьВрегистр = РегистрыСведений.омг_СвязиМедПерсонала.СоздатьМенеджерЗаписи();
		ЗаписьВрегистр.Врач = Сотрудник;
		ЗаписьВрегистр.Медсестра = Омг_Лаборант;
		ЗаписьВрегистр.Период = ТекущаяДата(); 
		ЗаписьВрегистр.омг_РольПерсонала = Перечисления.омг_РолиПерсонала.Лаборант;
		ЗаписьВрегистр.Записать();        
...
&НаКлиенте
Процедура омг_МедСестраПриИзменении(Элемент)
омг_МедСестраПриИзмененииНаСервере(Истина);
КонецПроцедуры

&НаКлиенте
Процедура Омг_ВрачПриемыПриИзменении(Элемент)
омг_ЗаполнитьПриемы()
КонецПроцедуры

&НаКлиенте
Процедура омг_СоздатьПриемБезЗаписи(Команда)

ПараметрыФормы = Новый Структура("Основание", Элементы.омг_ЗявкиБезЗаписи.ТекущиеДанные.Ссылка);
ДополнительныеПараметрыФормы = Новый Структура; 
ДополнительныеПараметрыФормы.Вставить("омг_МедСестра",омг_МедСестра);
ДополнительныеПараметрыФормы.Вставить("омг_Лаборант",омг_Лаборант);
...
&НаКлиенте
Процедура омг_ОткрытьЗаявкуБезЗаписи(Команда)

омг_ОткрытьЗаявкуБезЗаписиОбщ()

КонецПроцедуры

&НаКлиенте
Процедура омг_ЗаявкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)  
...
&НаКлиенте
Процедура омг_ОткрытьЗаявкуОбщ()

ПараметрыФормы = Новый Структура("Ключ", Элементы.омг_Заявки.ТекущиеДанные.Ссылка);
ОткрытьФорму("Документ.Заявка.ФормаОбъекта", ПараметрыФормы);

КонецПроцедуры 
&НаКлиенте
Процедура омг_ОткрытьЗаявкуКомпактОбщ()

ПараметрыФормы = Новый Структура("Ключ", Элементы.омг_ЗаявкиКомпакт.ТекущиеДанные.Ссылка);
ОткрытьФорму("Документ.Заявка.ФормаОбъекта", ПараметрыФормы);
...
&НаКлиенте
Процедура омг_ОткрытьЗаявкуБезЗаписиОбщ()

ПараметрыФормы = Новый Структура("Ключ", Элементы.омг_ЗявкиБезЗаписи.ТекущиеДанные.Ссылка);
ОткрытьФорму("Документ.Заявка.ФормаОбъекта", ПараметрыФормы);


КонецПроцедуры
&НаКлиенте
Процедура омг_ОткрытьЗаявкуБезЗаписиКомпактОбщ()

ПараметрыФормы = Новый Структура("Ключ", Элементы.омг_ЗявкиБезЗаписиКомпакт.ТекущиеДанные.Ссылка);
ОткрытьФорму("Документ.Заявка.ФормаОбъекта", ПараметрыФормы);
...
&НаКлиенте
Процедура омг_ЗявкиБезЗаписиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
омг_ОткрытьЗаявкуБезЗаписиОбщ()
КонецПроцедуры

&НаКлиенте
Процедура Омг_ВрачПриИзменении(Элемент)
омг_ЗаполнитьЗаявки()
КонецПроцедуры

&НаКлиенте
Процедура омг_ЛаборантПриИзменении(Элемент)
омг_МедСестраПриИзмененииНаСервере(Ложь);
КонецПроцедуры

&НаКлиенте
Процедура ПанельПриСменеСтраницы(Элемент, ТекущаяСтраница)

Если ТекущаяСтраница.Имя = "ЖурналЗаписи" Тогда       
	ОбновитьОтчет(,Истина); 
ИначеЕсли ТекущаяСтраница.Имя = "омг_ЗаявкиСтраница" Тогда		
	омг_ЗаполнитьЗаявки();  
ИначеЕсли ТекущаяСтраница.Имя = "омг_Приемы" Тогда	
	омг_ЗаполнитьПриемы(); 	
КонецЕсли; 
...
&НаКлиенте
Процедура омг_АвторПриИзменении(Элемент)
омг_ЗаполнитьЗаявки()
КонецПроцедуры

&НаСервере
Процедура Омг_СортироватьТаблицыНаСервере(ПолеСортировки, НаправлениеСортировки)

Если ПолеСортировки = "Имя" Тогда
	омг_Заявки.Сортировать("Клиент " + НаправлениеСортировки);
	омг_ЗявкиБезЗаписи.Сортировать("Клиент " + НаправлениеСортировки);
	
ИначеЕсли ПолеСортировки = "Время" Тогда
	// Для основной таблицы - время приема
	омг_Заявки.Сортировать("Время " + НаправлениеСортировки);
	
	// Для таблицы без записи - дата документа
	омг_ЗявкиБезЗаписи.Сортировать("ДатаДокумента " + НаправлениеСортировки);
КонецЕсли;

КонецПроцедуры 

//&НаСервере
//Процедура Омг_СортироватьТаблицыНаСервереБезЗаписи(ПолеСортировки, НаправлениеСортировки)
//    
//            // Для таблицы без записи - дата документа
//        омг_ЗявкиБезЗаписи.Сортировать("ДатаДокумента " + НаправлениеСортировки);
//        
//КонецПроцедуры
&НаКлиенте
Процедура ОткрытьКарточкуКлиентаЗаявки(Команда)

// Определяем активную таблицу
Если Элементы.омг_Заявки.ТекущаяСтрока <> Неопределено Тогда
	ДанныеСтроки = Элементы.омг_Заявки.ТекущиеДанные;
Иначе
	Возврат;
КонецЕсли;

// Получаем клиента
Клиент = ДанныеСтроки.Клиент;
Пациент = Новый Структура ("Ключ", Клиент);

// Открываем форму
Если ЗначениеЗаполнено(Клиент) Тогда
	ОткрытьФорму("Справочник.Клиенты.ФормаОбъекта", Пациент);
КонецЕсли;

КонецПроцедуры 
&НаКлиенте
Процедура ОткрытьКарточкуКлиентаЗаявкиБезЗаписи(Команда)

// Определяем активную таблицу
Если Элементы.омг_ЗявкиБезЗаписи.ТекущаяСтрока <> Неопределено Тогда
	ДанныеСтроки = Элементы.омг_ЗявкиБезЗаписи.ТекущиеДанные;
Иначе
	Возврат;
КонецЕсли;
...
КонецПроцедуры
&НаКлиенте
Процедура СортировкаПоИмениНажатие(Кнопка)
Омг_СортироватьТаблицыНаСервере("Имя", "Возр");
ОбновитьТаблицы();
КонецПроцедуры

&НаКлиенте
Процедура СортировкаПоВремениНажатие(Кнопка)
Омг_СортироватьТаблицыНаСервере("Время", "Убыв");
//Омг_СортироватьТаблицыНаСервереБезЗаписи ("ДатаДокумента ", "Убыв");
...
// Обновляем обе табличные части
Элементы.омг_Заявки.Обновить();
Элементы.омг_ЗявкиБезЗаписи.Обновить();

КонецПроцедуры

&НаКлиенте
Процедура омг_ПериодФормированияПериодПриИзменении(Элемент)
...
&НаКлиенте
Процедура омг_ПериодФормированияПриемыПериодПриИзменении(Элемент)
...
&НаКлиенте
Процедура СкрытьОтборКалендарь(Команда)


Элементы.ГруппаНавигацияЖурнала.Видимость = Скрытие;
Элементы.Группа3.Видимость = Скрытие;
Элементы.Группа9.Видимость = Скрытие;

Скрытие = ?(Скрытие=Истина,Ложь,Истина);	
...
ОткрытьФорму("Обработка.ПросмотрМедКарт.Форма.ФормаАмбулаторнаяМК85",,ЭтаФорма); 
КонецПроцедуры
...
&НаКлиенте
Процедура ВернутьсяВАРМ(Команда)
Если ЗначениеЗаполнено(ФормаВызоваАРМ) Тогда
	ОткрытьФорму(ФормаВызоваАРМ);
КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура КомпактныйРежимТаблицПриИзменении(Элемент)
ОбновитьРежимОтображениеВсехЗаписей ();
КонецПроцедуры

&НаКлиенте
Процедура омг_ОткрытьЗаявкуКомпакт(Команда)
ПараметрыФормы = Новый Структура("Ключ", Элементы.омг_ЗаявкиКомпакт.ТекущиеДанные.Ссылка);
ОткрытьФорму("Документ.Заявка.ФормаОбъекта", ПараметрыФормы);
...
&НаКлиенте
Процедура омг_ОткрытьЗаявкуБезЗаписиКомпакт(Команда)
ПараметрыФормы = Новый Структура("Ключ", Элементы.омг_ЗявкиБезЗаписиКомпакт.ТекущиеДанные.Ссылка);
ОткрытьФорму("Документ.Заявка.ФормаОбъекта", ПараметрыФормы);
КонецПроцедуры

&НаКлиенте
Процедура омг_ОткрытьКарточкуКлиентаКомпакт(Команда)
// Определяем активную таблицу
Если Элементы.омг_ЗаявкиКомпакт.ТекущаяСтрока <> Неопределено Тогда
	ДанныеСтроки = Элементы.омг_ЗаявкиКомпакт.ТекущиеДанные;
Иначе
	Возврат;
КонецЕсли;
...
&НаКлиенте
Процедура омг_ОткрытьКарточкуКлиентаБезЗаписиКомпакт(Команда)
// Определяем активную таблицу
Если Элементы.омг_ЗявкиБезЗаписиКомпакт.ТекущаяСтрока <> Неопределено Тогда
	ДанныеСтроки = Элементы.омг_ЗявкиБезЗаписиКомпакт.ТекущиеДанные;
Иначе
	Возврат;
КонецЕсли;
...
&НаКлиенте
Процедура омг_СоздатьПриемКомпакт(Команда)

ПараметрыФормы = Новый Структура("Основание", Элементы.омг_ЗаявкиКомпакт.ТекущиеДанные.Ссылка);
ДополнительныеПараметрыФормы = Новый Структура; 
ДополнительныеПараметрыФормы.Вставить("омг_МедСестра",омг_МедСестра);
ДополнительныеПараметрыФормы.Вставить("омг_Лаборант",омг_Лаборант); 
ДополнительныеПараметрыФормы.Вставить("омг_Врач",Элементы.омг_ЗаявкиКомпакт.ТекущиеДанные.Врач);
...
&НаКлиенте
Процедура омг_СоздатьПриемБезЗаписиКомпакт(Команда)

ПараметрыФормы = Новый Структура("Основание", Элементы.омг_ЗявкиБезЗаписиКомпакт.ТекущиеДанные.Ссылка);
ДополнительныеПараметрыФормы = Новый Структура; 
ДополнительныеПараметрыФормы.Вставить("омг_МедСестра",омг_МедСестра);
ДополнительныеПараметрыФормы.Вставить("омг_Лаборант",омг_Лаборант);
...
&НаКлиенте
Процедура омг_ОткрытьСофтФон(Команда)
...
&НаКлиенте
Процедура омг_ОткрытьWhatsApp(Команда)
ОткрытьФорму("Обработка.iga_ChatBot.Форма.АРМОператора");
КонецПроцедуры

&НаКлиенте
Процедура омг_ОткрытьКабинетОператора(Команда)
ОткрытьФорму("Обработка.омг_АРМОператора.Форма.Форма_85");
КонецПроцедуры

&НаКлиенте
Процедура ПланировщикКалендарьПриСменеТекущегоПериодаОтображения(Элемент, ТекущиеПериодыОтображения, СтандартнаяОбработка)
СтандартнаяОбработка = Ложь;
ВернутьПериодыОтображенияНаСервере();
КонецПроцедуры 
&НаСервере
Процедура ВернутьПериодыОтображенияНаСервере()
Планировщик = ЭтаФорма.ПланировщикКалендарь;
...
РабочаяДата + Час(ВремяНач)   * 3600 + Минута(ВремяНач)   * 60,
РабочаяДата + Час(ВремяОконч) * 3600 + Минута(ВремяОконч) * 60);
КонецПроцедуры
...
// ============================================================
&НаКлиенте
Процедура ПланировщикКалендарьОбработкаФормированияКоманд(Элемент, ПараметрыКоманд, Команды, КомандаПоУмолчанию)  
Элементы.ОткрытьЗаявку.Видимость        = Ложь;
Элементы.СоздатьОказаниеУслуг.Видимость = Ложь;
Элементы.СоздатьПрием.Видимость         = Ложь;
Элементы.ОткрытьКлиента.Видимость       = Ложь;
Элементы.СоздатьЗаявку.Видимость        = Ложь;

мПланировщикАктивныйЭлемент = Неопределено;
мПланировщикПараметрыКоманд = ПараметрыКоманд;

Если ПараметрыКоманд.Источник = "Пустая область" Тогда
	Элементы.СоздатьЗаявку.Видимость = Истина;
	Возврат;
КонецЕсли;

Если ТипЗнч(ПараметрыКоманд.Элементы) <> Тип("Массив") Тогда Возврат КонецЕсли;
Если ПараметрыКоманд.Элементы.Количество() = 0 Тогда Возврат КонецЕсли;

мПланировщикАктивныйЭлемент = ПараметрыКоманд.Элементы[0];
Значение = мПланировщикАктивныйЭлемент.Значение;

Если ТипЗнч(Значение) = Тип("ДокументСсылка.Заявка") Тогда
	
	Элементы.ОткрытьЗаявку.Видимость        = Истина;
	Элементы.СоздатьОказаниеУслуг.Видимость = Истина;
	Элементы.СоздатьПрием.Видимость         = Истина;
	Элементы.ОткрытьКлиента.Видимость       = Истина;
	
	// Первый клик — запускаем таймер ожидания второго
	Если мТаймерЗаявка = Неопределено Тогда
		мТаймерЗаявка = Значение;
		ПодключитьОбработчикОжидания("ПланировщикСбросТаймера", 0.4, Истина);
	Иначе
		// Второй клик пришёл до таймера — это двойной клик
		ОтключитьОбработчикОжидания("ПланировщикСбросТаймера");
		мТаймерЗаявка = Неопределено;
		ОткрытьФорму("Документ.Заявка.ФормаОбъекта", Новый Структура("Ключ", Значение));
	КонецЕсли;
	
ИначеЕсли ТипЗнч(Значение) = Тип("Структура")
		И Значение.Свойство("Тип") И Значение.Тип = "Слот" Тогда

		Элементы.СоздатьЗаявку.Видимость = Истина;

		Если мТаймерЗаявка = Неопределено Тогда
			мТаймерЗаявка = мПланировщикАктивныйЭлемент;
			ПодключитьОбработчикОжидания("ПланировщикСбросТаймера", 0.4, Истина);
		Иначе
			ОтключитьОбработчикОжидания("ПланировщикСбросТаймера");
			ЭлементДляСоздания = мПланировщикАктивныйЭлемент;
			мТаймерЗаявка = Неопределено;
			мОткрытьСозданиеЗаявкиПоЭлементу(ЭлементДляСоздания);
		КонецЕсли;
...
&НаКлиенте
Процедура ПланировщикСбросТаймера()
мТаймерЗаявка = Неопределено;
КонецПроцедуры


&НаКлиенте
Процедура ПланировщикКМОткрытьЗаявку(Команда)
Если мПланировщикАктивныйЭлемент = Неопределено Тогда 
	Возврат 
КонецЕсли;
ОткрытьФорму("Документ.Заявка.ФормаОбъекта",
Новый Структура("Ключ", мПланировщикАктивныйЭлемент.Значение));
КонецПроцедуры

&НаКлиенте
Процедура ПланировщикКМСоздатьОказаниеУслуг(Команда)
Если мПланировщикАктивныйЭлемент = Неопределено Тогда Возврат КонецЕсли;
ДобавитьОказаниеУслуги(мПланировщикАктивныйЭлемент.Значение);
КонецПроцедуры

&НаКлиенте
Процедура ПланировщикКМСоздатьПрием(Команда)
Если мПланировщикАктивныйЭлемент = Неопределено Тогда Возврат КонецЕсли;

Заявка       = мПланировщикАктивныйЭлемент.Значение;
КлиентЗаявки = мДанныеЗаявок.Получить(Заявка);

//// Получаем врача из измерений — ФиксированноеСоответствие, используем Получить()
//ВыбранныйВрач = Неопределено;
//Если мПланировщикАктивныйЭлемент.ЗначенияИзмерений <> Неопределено Тогда
//	ВыбранныйВрач = мПланировщикАктивныйЭлемент.ЗначенияИзмерений.Получить("Врач");
//КонецЕсли;

//СоздатьНовыйПриёмПоЯчейкеЗаписи(Заявка, ВыбранныйВрач, КлиентЗаявки); 

РасшифровкаОбластиЯчеек	= Заявка; 
ТекущийКлиент           = КлиентЗаявки;
	Объект = мПланировщикАктивныйЭлемент.ЗначенияИзмерений.Получить("Врач");
	Если Не СоздаватьПовторныеПриемыПоЗаявкеИзЖурналаЗаписи
		И мДанныеПриемовЗаявок <> Неопределено
	Тогда
		// Для кабинетов не реализовано автоопределение наличия приёма этого врача. Создается новый приём.
...
&НаКлиенте
Процедура ПланировщикКМОткрытьКлиента(Команда)
Если мПланировщикАктивныйЭлемент = Неопределено Тогда Возврат КонецЕсли;
КлиентЗаявки = мДанныеЗаявок.Получить(мПланировщикАктивныйЭлемент.Значение);
Если ЗначениеЗаполнено(КлиентЗаявки) Тогда
	ОткрытьЗначение(КлиентЗаявки);
КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПланировщикКМСоздатьЗаявку(Команда)
// Вариант 1: создание по элементу-слоту / резерву
Если мПланировщикАктивныйЭлемент <> Неопределено
	И ТипЗнч(мПланировщикАктивныйЭлемент.Значение) = Тип("Структура") Тогда
	мОткрытьСозданиеЗаявкиПоЭлементу(мПланировщикАктивныйЭлемент);
	Возврат;
КонецЕсли;

// Вариант 2: создание по клику на пустую область
ЗначенияЗаполнения = Новый Структура;

Если мПланировщикПараметрыКоманд <> Неопределено Тогда
	
	Попытка
			ДатаКлика = мПланировщикПараметрыКоманд.Дата;
			Если ЗначениеЗаполнено(ДатаКлика) Тогда

				// Длительность приёма врача той колонки, иначе — квант
				ДлитСек = ОбщегоНазначения.ВремяВСекунды(мУчетнаяПолитика.ПериодичностьПланирования);
				Если Не ЗначениеЗаполнено(ДлитСек) Или ДлитСек = 0 Тогда ДлитСек = 900 КонецЕсли;
				Попытка
					ВрачКолонки = мПланировщикПараметрыКоманд.ЗначенияИзмерений.Получить("Врач");
					Если ЗначениеЗаполнено(ВрачКолонки) Тогда
						ДлитВрача = ПолучитьДлительностьПриемаНаСервере(ВрачКолонки);
						Если ЗначениеЗаполнено(ДлитВрача) И ДлитВрача > 0 Тогда
							ДлитСек = ДлитВрача;
						КонецЕсли;
					КонецЕсли;
				Исключение 
				...
				ЗначенияЗаполнения.Вставить("ДатаНачала",    ДатаКлика);
				ЗначенияЗаполнения.Вставить("ДатаОкончания", ДатаКлика + ДлитСек);
				ЗначенияЗаполнения.Вставить("СредняяПродолжительность", Дата(1, 1, 1) + ДлитСек);
			КонецЕсли;
		Исключение 
		КонецПопытки;
	
	Попытка
		ВыбранныйВрач = мПланировщикПараметрыКоманд.ЗначенияИзмерений.Получить("Врач");
		Если ЗначениеЗаполнено(ВыбранныйВрач) Тогда
			ЗначенияЗаполнения.Вставить("Сотрудник", ВыбранныйВрач);
		КонецЕсли;
	Исключение 
	КонецПопытки;
...
&НаКлиенте
Процедура ПланировщикКалендарьВыбор(Элемент, СтандартнаяОбработка)
СтандартнаяОбработка = Ложь;
КонецПроцедуры

&НаКлиенте
Процедура ПланировщикКалендарьПередСозданием(Элемент, Начало, Конец, ЗначенияИзмерений, Текст, СтандартнаяОбработка)
СтандартнаяОбработка = Ложь;

Если мТаймерЗаявка = Неопределено Тогда
	// Первый клик — запускаем таймер
	мТаймерЗаявка = "ПустаяОбласть";
	мПланировщикПараметрыКоманд = Новый Структура("Начало, Конец, ЗначенияИзмерений", Начало, Конец, ЗначенияИзмерений);
	ПодключитьОбработчикОжидания("ПланировщикСбросТаймера", 0.4, Истина);
Иначе
	// Двойной клик
	ОтключитьОбработчикОжидания("ПланировщикСбросТаймера");
	...
	ВыбранныйВрач = ЗначенияИзмерений.Получить("Врач");
	Если ЗначениеЗаполнено(ВыбранныйВрач) Тогда
		ЗначенияЗаполнения.Вставить("Сотрудник", ВыбранныйВрач);
	КонецЕсли;
	Если ЗначениеЗаполнено(Клиент) Тогда
...
&НаКлиенте
Процедура ПланировщикКалендарьПередНачаломРедактирования(Элемент, НовыйЭлемент, СтандартнаяОбработка)
СтандартнаяОбработка = Ложь;

Если НовыйЭлемент = Неопределено Тогда Возврат КонецЕсли;

Попытка
	Значение = НовыйЭлемент.Значение;
Исключение
	Возврат;
КонецПопытки;
...
ИначеЕсли ТипЗнч(Значение) = Тип("Структура")
	И Значение.Свойство("Тип") И Значение.Тип = "Слот" Тогда
	// Двойной клик по свободному слоту / резерву — сразу создаём заявку
	мОткрытьСозданиеЗаявкиПоЭлементу(НовыйЭлемент);
КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПланировщикКалендарьНажатиеНаДействиеПланировщика(Элемент, ЭлементПланировщика, Действие)
Если Действие = "ПланировщикСоздатьЗаявку" Тогда
	// Сюда попадаем из динамического меню пустой области
	ПланировщикКМСоздатьЗаявку(Неопределено);
	Возврат;
КонецЕсли;

Если ЭлементПланировщика = Неопределено Тогда Возврат КонецЕсли;
Значение = ЭлементПланировщика.Значение;

Если Действие = "ОткрытьЗаявку" Тогда
	ОткрытьФорму("Документ.Заявка.ФормаОбъекта", Новый Структура("Ключ", Значение));
ИначеЕсли Действие = "СоздатьОУ" Тогда
	ДобавитьОказаниеУслуги(Значение);
ИначеЕсли Действие = "СоздатьПрием" Тогда
	КлиентЗаявки = мДанныеЗаявок.Получить(Значение);
	ВыбранныйВрач = Неопределено;
	Если ЭлементПланировщика.ЗначенияИзмерений <> Неопределено Тогда
		ВыбранныйВрач = ЭлементПланировщика.ЗначенияИзмерений.Получить("Врач");
	КонецЕсли;
	СоздатьНовыйПриёмПоЯчейкеЗаписи(Значение, ВыбранныйВрач, КлиентЗаявки);
ИначеЕсли Действие = "ОткрытьКлиента" Тогда
	КлиентЗаявки = мДанныеЗаявок.Получить(Значение);
	Если ЗначениеЗаполнено(КлиентЗаявки) Тогда ОткрытьЗначение(КлиентЗаявки) КонецЕсли;
КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПланировщикКалендарьПередНачаломБыстрогоРедактирования(Элемент, СтандартнаяОбработка) 
СтандартнаяОбработка = Ложь;
	...
	//ИначеЕсли ТипЗнч(Значение) = Тип("Структура")
	//	И Значение.Свойство("Тип") И Значение.Тип = "Слот" Тогда
	//	// Создаём заявку, только если клик был не одиночный (таймер ещё не сброшен)
	//	Если мТаймерЗаявка <> Неопределено Тогда
	//		ОтключитьОбработчикОжидания("ПланировщикСбросТаймера");
	//		мТаймерЗаявка = Неопределено;
	//		мОткрытьСозданиеЗаявкиПоЭлементу(мПланировщикАктивныйЭлемент);
...
&НаКлиенте
Процедура ПланировщикКалендарьПриАктивизации(Элемент)

КонецПроцедуры

&НаКлиенте
Функция ПолучитьОтфильтрованныеЗаявки()
Если Не ЗначениеЗаполнено(Клиент) Тогда
	Возврат мДанныеЗаявок;
КонецЕсли;

Результат = Новый Соответствие;
Если мДанныеЗаявок <> Неопределено Тогда
	Для Каждого КЗ Из мДанныеЗаявок Цикл
		Если КЗ.Значение = Клиент Тогда
...
КонецФункции
&НаКлиенте
Процедура мОткрытьСозданиеЗаявкиПоЭлементу(ЭлементПланировщика)

Значение = ЭлементПланировщика.Значение;

	ДатаНачала    = ЭлементПланировщика.Начало;
	ДатаОкончания = ЭлементПланировщика.Конец;

	// Для резерва блок может быть длиннее одного приёма — ограничиваем длительностью
	Если ТипЗнч(Значение) = Тип("Структура")
		И Значение.Свойство("ДлительностьСек")
		И ЗначениеЗаполнено(Значение.ДлительностьСек) Тогда
		ДатаОкончания = Мин(ЭлементПланировщика.Конец, ЭлементПланировщика.Начало + Значение.ДлительностьСек);
	КонецЕсли;

	ДлительностьСлотаСек = ДатаОкончания - ДатаНачала;

	ЗначенияЗаполнения = Новый Структура;
	ЗначенияЗаполнения.Вставить("ДатаНачала",    ДатаНачала);
	ЗначенияЗаполнения.Вставить("ДатаОкончания", ДатаОкончания);
	ЗначенияЗаполнения.Вставить("ВремяОкончания", Дата(1, 1, 1) + (ДатаОкончания - НачалоДня(ДатаОкончания)));
	ЗначенияЗаполнения.Вставить("КоличествоВыбранныхЯчеек", 1);

	// Объект колонки — сотрудник или оборудование
	ВыбранныйОбъект = Неопределено;
	Если ЭлементПланировщика.ЗначенияИзмерений <> Неопределено Тогда
		ВыбранныйОбъект = ЭлементПланировщика.ЗначенияИзмерений.Получить("Врач");
	КонецЕсли;
	Оборудование = Неопределено;
	Если ТипЗнч(ВыбранныйОбъект) = Тип("СправочникСсылка.Сотрудники") Тогда
		ЗначенияЗаполнения.Вставить("Сотрудник", ВыбранныйОбъект);
	ИначеЕсли ТипЗнч(ВыбранныйОбъект) = Тип("СправочникСсылка.Оборудование") Тогда
		Оборудование = ВыбранныйОбъект;
		ЗначенияЗаполнения.Вставить("Оборудование", ВыбранныйОбъект);
	КонецЕсли;
	...
	// Связь с документом-основанием (событие и т.п.)
	Если ЗначениеЗаполнено(ТекущийДокументЗаписи) Тогда
		ЗначенияЗаполнения.Вставить("Основание", ТекущийДокументЗаписи);
		Если ЗначениеЗаполнено(ТекущийКлиентДокументаЗаписи) Тогда
			ЗначенияЗаполнения.Вставить("Клиент", ТекущийКлиентДокументаЗаписи);
	...
	// Режим длительности (от сотрудника) и продолжительность = длине слота
	Если ЗначенияЗаполнения.Свойство("Сотрудник") И ЗначениеЗаполнено(ЗначенияЗаполнения.Сотрудник) Тогда
		РежимИПродолжительность = ПолучитьРежимДлительностиЗаявкиИПродолжительностьПоСотруднику(ЗначенияЗаполнения.Сотрудник);
		ЗначенияЗаполнения.Вставить("РежимДлительностиЗаявки", РежимИПродолжительность["РежимДлительностиЗаявки"]);
	КонецЕсли;
	Если ДлительностьСлотаСек > 0 Тогда
		ЗначенияЗаполнения.Вставить("СредняяПродолжительность", Дата(1, 1, 1) + ДлительностьСлотаСек);
	КонецЕсли;

	ПараметрыФормы = Новый Структура("ЗначенияЗаполнения, Оборудование", ЗначенияЗаполнения, Оборудование);
	Если ТекущиеПараметрыЗаписи <> Неопределено Тогда
		ПараметрыФормы.Вставить("ПараметрыЗаписиПоДокументу", ТекущиеПараметрыЗаписи);
	КонецЕсли;
...
КонецПроцедуры
&НаСервереБезКонтекста
Функция ПолучитьДлительностьПриемаНаСервере(Сотрудник)
	Если ТипЗнч(Сотрудник) <> Тип("СправочникСсылка.Сотрудники") Тогда
		Возврат 0;
	КонецЕсли;
	ДлитПриема = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Сотрудник, "ДлительностьПриема");
	Если ЗначениеЗаполнено(ДлитПриема) Тогда
		Возврат ДлитПриема - Дата(1, 1, 1);
	КонецЕсли;
	Возврат 0;
КонецФункции
...
&НаКлиенте
Процедура омг_ЗявкиБезЗаписиКомпактВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	омг_ОткрытьЗаявкуБезЗаписиКомпактОбщ()
КонецПроцедуры


&НаКлиенте
Процедура омг_ЗаявкиКомпактВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	омг_ОткрытьЗаявкуКомпактОбщ()
КонецПроцедуры


&НаКлиенте
Процедура омг_СоздатьЗаявкуБезЗаписи(Команда)

    ФормаДокумента = ПолучитьФорму("Документ.Заявка.Форма.ФормаДокумента_85");
    
    ФормаДокумента.Объект.омг_БезЗаписи = Истина;
