...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "НамерениеЗаписиПоДокументу" Тогда 
		...
		Если ЗначениеЗаполнено(Параметр.Филиал) Тогда
		Отчет.Филиал = Параметр.Филиал; 
		КонецЕсли;
...
Процедура ОбновитьОтчет(УстановитьФокусВводаНаТаблице = Истина,НажатиеКнопки = Ложь)
	...
	Если не НажатиеКнопки Тогда 
		Если не АктивноеОкно() = Неопределено Тогда
			Если АктивноеОкно().НачальнаяСтраница и ЭтаФорма.Элементы.Панель.ТекущаяСтраница.Имя = "ЖурналЗаписи" Тогда
			иначе	
			ПодключитьАвтообновлениеОтчета();
			Возврат;
		КонецЕсли;
...
Процедура ДобавитьПриёмПоЯчейкеЗаписи(РасшифровкаОбластиЯчеек,ТекущийКлиент)
	...
	//Если Не СоздаватьПовторныеПриемыПоЗаявкеИзЖурналаЗаписи
	//	И мДанныеПриемовЗаявок <> Неопределено     
	Если мДанныеПриемовЗаявок <> Неопределено 
	////////////////////////////////////////////////////////////	
	// ОМГ -	
	Тогда
...
Процедура омг_МедСестраПриИзмененииНаСервере(ЭтоМедсестра = Истина)
	...
	Если ЗначениеЗаполнено(Омг_Медсестра) и ЭтоМедсестра Тогда
	...
	иначеЕсли ЗначениеЗаполнено(Омг_Лаборант) и не ЭтоМедсестра Тогда
...
Процедура ПросмотрМедицинскихКарт(Команда)
	ОткрытьФорму("Обработка.ПросмотрМедКарт.Форма.ФормаАмбулаторнаяМК",,ЭтаФорма); 
КонецПроцедуры
