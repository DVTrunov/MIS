...
Перем мВремяНачалаДня Экспорт; // Время, с которого начинается вывод календаря планирования.

// Конечное значение диапазона выводимого времени в календарь записи.
Перем мВремяОкончанияДня Экспорт; 
...
Процедура ЗаполнитьКонстантыОтчета()
	...
	Тогда
		...
		Если ТекущийИнтервал = Неопределено Тогда
			...
			ТекущийИнтервал = мУчетнаяПолитика.омг_ПериодичностьПланированияКалендаря;
		КонецЕсли;
	...
	Иначе
		...
		мКвантПланирования = ОбщегоНазначения.ВремяВСекунды(мУчетнаяПолитика.омг_ПериодичностьПланированияКалендаря);
...
Функция ПолучитьТаблицуОбъектовЖурналаЗаписи(МенеджерВТ)
	...
	///////////////////////////////////////////////////////
	// ОМГ
	|	и ЗаявкаРаботы.омг_БезЗаписиНесколькоВрачей = Ложь
	// ОМГ
	///////////////////////////////////////////////////////
	|;
	|
	...
	// Добавлено Макеев (
	Запрос.УстановитьПараметр("омг_Организация", омг_Организация);
	// )
			...
			Иначе
				...
				Тогда
					Если МодельРасписания.Количество() <> 0 Тогда
						...
						СписокОбъектовДляОтбора.ЗагрузитьЗначения(КалендарьПланирования.ПолучитьУчастниковМоделиРасписания(МодельРасписания, Истина));
					Иначе
				...
				ИначеЕсли СписокОбъектовДляОтбора = Неопределено Тогда
					СписокОбъектовДляОтбора = Новый СписокЗначений;
					СписокОбъектовДляОтбора.ЗагрузитьЗначения(КалендарьПланирования.ПолучитьУчастниковМоделиРасписания(ПереченьМоделей, Истина));
				КонецЕсли;
	...
	Если Истина Тогда
		Если ДляВыборкиОбъектов Тогда
			Возврат "ВЫБРАТЬ
			        ...
			        |					)
					// Добавлено Макеев (
					|		И ВЫБОР КОГДА &омг_Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) ТОГДА ИСТИНА ИНАЧЕ ЗаявкаРаботы.Филиал.Организация = &омг_Организация КОНЕЦ
					// )
		...
		Иначе
			Возврат "ВЫБРАТЬ
					...
					//////////////////////////////////////////
					// ОМГ
					|	Выбор когда ЗаявкиВТ.КлиентСсылка.омг_РискПадения = Значение(Перечисление.омг_РискиПадения.Высокий) Тогда ""ВЫСОКИЙ!"" иначе ""Низкий"" КОНЕЦ КАК омг_РискПадения,
					// ОМГ  
					//////////////////////////////////////////
			        |	ЗаявкиВТ.Кабинет,
			        |	ЗаявкиВТ.Примечание
					...
					// Добавлено Макеев (
					|		И ВЫБОР КОГДА &омг_Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) ТОГДА ИСТИНА ИНАЧЕ ЗаявкиВТ.Филиал.Организация = &омг_Организация КОНЕЦ
					// )
			        |
			        |УПОРЯДОЧИТЬ ПО
			        |	ОБЪЕКТ,
...
Функция СформироватьОтчет() Экспорт
	...
	ТабДокументы = ПолучитьЕдинуюТаблицуДокументов(ДанныеДокументов, Ложь);
	...
	//////////////////////////////////////////////////////////////////////////////////
	// ОМГ 
	Если не СтрГрафика = Неопределено Тогда
		Если ЗначениеЗаполнено(СтрГрафика.Сотрудник) Тогда
			Если ЗначениеЗаполнено(СтрГрафика.Сотрудник.ДлительностьПриема) Тогда
				мКвантПланирования = Мин(мКвантПланирования,(СтрГрафика.Сотрудник.ДлительностьПриема - Дата("00010101")));	
			//иначе
			//	мКвантПланирования = ОбщегоНазначения.ВремяВСекунды(мУчетнаяПолитика.ПериодичностьПланирования);
			КонецЕсли;
		КонецЕсли;
...
////Карсаков. Функция для плданировщика. В 8.5 она заменяет предыдущую функцию СформироватьОтчет()
Функция СформироватьДанныеДляПланировщика() Экспорт
	...
	// Соответствие Заявка -> Клиент (аналог ВозвращаемыеДанныеЗаявок).
	ДанныеЗаявок     = Новый Соответствие;
	ТабЗаявокКлиенты = ТабДокументы.Скопировать(, "ДокументСсылка, КлиентСсылка");
	ТабЗаявокКлиенты.Свернуть("ДокументСсылка, КлиентСсылка");
	Для Каждого СтрокаДокумента Из ТабЗаявокКлиенты Цикл
		Если ЗначениеЗаполнено(СтрокаДокумента.ДокументСсылка) Тогда
			ДанныеЗаявок.Вставить(СтрокаДокумента.ДокументСсылка, СтрокаДокумента.КлиентСсылка);
		КонецЕсли;
	КонецЦикла;
	
	// Уточнение кванта по длительности приёма сотрудников (как в СформироватьОтчет).
	КвантВСекундах = мКвантПланирования;
	Для Каждого Объект Из Объекты Цикл
		Если ТипЗнч(Объект) = Тип("СправочникСсылка.Сотрудники")
			И ЗначениеЗаполнено(Объект.ДлительностьПриема)
		Тогда
			КвантВСекундах = Мин(КвантВСекундах, Объект.ДлительностьПриема - Дата("00010101"));
		КонецЕсли;
	КонецЦикла;
	
	Результат = Новый Структура;
	Результат.Вставить("Объекты",        Объекты);
	Результат.Вставить("ДанныеЗаявок",   ДанныеЗаявок);
	Результат.Вставить("КвантВСекундах", КвантВСекундах);
	Результат.Вставить("ВремяНачала",    мВремяНачалаДня);
	Результат.Вставить("ВремяОкончания", мВремяОкончанияДня);
	
	// Данные приёмов заявок — при тех же условиях, что в СформироватьОтчет.
	Если мСоздаватьПовторныеПриемыПоЗаявкеИзЖурналаЗаписи = Ложь Тогда
		ДанныеПриемовЗаявок = СформироватьДанныеПриемовЗаявок(ДанныеЗаявок);
		Если ДанныеПриемовЗаявок <> Неопределено Тогда
			Результат.Вставить("ДанныеПриемовЗаявок", ДанныеПриемовЗаявок);
		КонецЕсли;
	КонецЕсли;
	
	// Местонахождение медкарт — только если ведётся учёт.
	Если УправлениеНастройками.ПолучитьПараметрУчетнойПолитики("ВестиУчетМестонахожденияМедкарт") Тогда
		Результат.Вставить("МестонахождениеМедКарт",
			УстановитьМестонахожденияМедКарт(ТабДокументы.ВыгрузитьКолонку("ДокументСсылка")));
	КонецЕсли;
	
	// Признак заявок «хочу раньше».
	Если мУчетнаяПолитика.ИспользоватьХочуРаньшеВЗаявках Тогда
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		Результат.Вставить("ЕстьЗаявкиХочуРаньше", ?(Выборка.Следующий(), Выборка.Значение, Ложь));
	КонецЕсли;
...
Функция ПолучитьЕдинуюТаблицуДокументов(ДанныеДокументов,безЗаписи = Истина)
	
	ТабДокументы = ПолучитьТабицуДокументов(ДанныеДокументов.РезультатДокументыСотрудников,безЗаписи);
...
Функция ПолучитьВыборкуДокументовОтчета(СписокОбъектов, МенеджерВТ)
	...
	мсТаб.Добавить(омг_МедицинскаяДеятельность.ДополнитьСтрокамиИзЧекапов(Результат.Выгрузить(),СписокОбъектов,ДатаНачала,КонецДня(ДатаОкончания),?(ЗначениеЗаполнено(Филиал), Филиал, Неопределено)));
	...
	//Если стрДок.ДокументСсылка.НесколькоСотрудников Тогда
	//текДатаКон = стрДок.ДатаОкончания;	
	//КонецЕсли;	
	// ОМГ
    //////////////////////////////////////////////////////////////// 
...
Процедура ВывестиОбъектВШапкуСтраницы(Объект, ДокументРезультат, Дата, гПоз, этоСотрудник, ЛинияШапки, КомментарииДня, СтруктураЦветаЯчейкиОбъекта)
	...
	Если ЗначениеЗаполнено(Объект) Тогда
		...
		Если Комментарии.Количество() <> 0 Тогда
			...
			ОбластьСотрудник.Примечание.Шрифт = мШрифтРаботы;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВывестиЛинииРазделенияЧасовВСеткеОтчета(ДокументРезультат, ТаблицаДиапазонов)
	...
	Если мСтруктураВизуальныхНастроек.ТипЛинииСеткиВнутриЧаса = 0 Тогда 
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////
		// ОМГ
		ЛинияМеждуЧасами = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии, 1);
		...
		Для Сч = 2 По ТаблицаДиапазонов.Количество() - 1 Цикл
			...
			Если Диапазон.Дата <> НачалоЧаса(Диапазон.Дата) Тогда
				...
				////////////////////////////////////////////////////////////////////////////////////////////////////////
				// ОМГ    
				ЛинияМеждуЧасамиПунктир = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.РедкийПунктир, 1);
				ДокументРезультат.Область(вПоз, НачПунктира, вПоз, 3).ГраницаСверху = ЛинияМеждуЧасамиПунктир;	

				ДокументРезультат.Область(вПоз-1, НачПунктира, вПоз, ДокументРезультат.ШиринаТаблицы).ГраницаСнизу = ЛинияМеждуЧасами;
				
				Для счк = 4 по ДокументРезультат.ШиринаТаблицы Цикл
					Если ЗначениеЗаполнено(ДокументРезультат.Область(вПоз, счк, вПоз, счк).Текст) и не ДокументРезультат.Область(вПоз, счк, вПоз, счк).Текст = "00" Тогда 					
					ДокументРезультат.Область(вПоз, счк, вПоз, счк).ГраницаСверху = ЛинияМеждуЧасамиПунктир;	
					КонецЕсли;	
				КонецЦикла;
		...
		ДокументРезультат.Область(вПоз, 1, вПоз,ДокументРезультат.ШиринаТаблицы).ГраницаСнизу = ЛинияСплошная;	
...
Процедура ВывестиИнтервалГрафика(ДокументРезультат, ВремяНачала, ВремяОкончания, ВремяНачалаДня,ВремяОкончанияДня, гПоз, этоСотрудник, ТаблицаДиапазонов, ВыбГрафики)
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	// ОМГ
	ИндивидуальнаяДлительность = Ложь;
	Если ЗначениеЗаполнено(ВыбГрафики.Сотрудник.ДлительностьПриема) и не ВыбГрафики.Сотрудник.ДлительностьПриема - Дата("00010101") = мКвантПланирования 
		или не ЗначениеЗаполнено(ВыбГрафики.Сотрудник.ДлительностьПриема) Тогда 
		Если ЗначениеЗаполнено(ВыбГрафики.Сотрудник.ДлительностьПриема) Тогда
		ДлительностьПриема = ВыбГрафики.Сотрудник.ДлительностьПриема
		иначе
		ДлительностьПриема = мУчетнаяПолитика.ПериодичностьПланирования
		КонецЕсли;
		ИндивидуальнаяДлительность = Истина;
		ПустаяДата = Дата(1,1,1); 
		ИтогоСменаСек = ВыбГрафики.ВремяОкончания - ВыбГрафики.ВремяНачала; 				
		НачалоСмены = ВыбГрафики.ВремяНачала;
		ВремяНачала = ВыбГрафики.ВремяНачала; 
		ОкончаниеСмены = ВыбГрафики.ВремяОкончания; 
		ОкончаниеСменыСотрудника = ПустаяДата;
		ТаблицаДиапазоновСотрудника = ТаблицаДиапазонов.Скопировать();
		ДлительностьСменыСотрудника = ДлительностьПриема - Дата("00010101");
		ТаблицаДиапазоновСотрудника.Очистить();
		Сч = 0;
		сДата = 0;  
		Пока ОкончаниеСменыСотрудника <= ВыбГрафики.ВремяОкончания Цикл 
			СтрокаТабСотрудника = ТаблицаДиапазоновСотрудника.Добавить();
			Если  Сч = 0 Тогда
				Сч = 1;
				//ЗаполнитьЗначенияСвойств(СтрокаТабСотрудника, ТаблицаДиапазонов[0]);
				СтрокаТабСотрудника.Дата = ВремяНачала;
				СтрокаТабСотрудника.Продолжительность = СтрокаТабСотрудника.Дата - ПустаяДата;
			иначе 
				СтрокаТабСотрудника.Дата = сДата + ДлительностьСменыСотрудника;
				СтрокаТабСотрудника.Продолжительность = СтрокаТабСотрудника.Дата - ПустаяДата;
			КонецЕсли;  
			сДата = СтрокаТабСотрудника.Дата;
			ОкончаниеСменыСотрудника = сДата + ДлительностьСменыСотрудника;
		КонецЦикла;
		КонецЕсли;
		// ОМГ
		//////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	Тогда
		...
		СтароеЗначениеНайдВремяСотрудника = Неопределено;				
		Для текВертПоз = НачПоз По КонПоз Цикл 
			
		////////////////////////////////////////////////////////////////////////////////////
		//// ОМГ    
       	Если  ИндивидуальнаяДлительность Тогда
		НайдВремяСотрудника = ТаблицаДиапазоновСотрудника.Найти(Дата("01.01.0001 " + мПредставлениеВремениСтрок[текВертПоз]+":00"),"Дата");  
		Если НайдВремяСотрудника = Неопределено или СтароеЗначениеНайдВремяСотрудника = НайдВремяСотрудника Тогда
			Продолжить
		КонецЕсли; 
		КонецЕсли;  
	    СтароеЗначениеНайдВремяСотрудника = НайдВремяСотрудника;
		//// ОМГ
		////////////////////////////////////////////////////////////////////////////////////	
		...
		// ОМГ
		ЛинияМеждуЧасами = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.РедкийПунктир, 1); 
		ОбластьЯчейки.ГраницаСнизу = ЛинияМеждуЧасами;
		// ОМГ
		//////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Процедура КлиентУслугиРаздельно(ВыбРаботыБлока,Расшифровка,НачВПоз,КонВПоз,врНачСек,гПоз,ДокументРезультат,ЦветФонаТекДокумента)
	...
	Если НастройкиДПР.КоличествоДПР > 0 Тогда
		ОбластьПоля.Текст = ТекстовыеДанныеЯчейки.ТекстДопЯчейки;
		Если Не ПустаяСтрока(ТекстовыеДанныеЯчейки.ТекстДопЯчейки) Тогда
			ОбластьПоля.Шрифт = мШрифтРаботы;
		КонецЕсли;
		Если Не ПустаяСтрока(ТекстовыеДанныеЯчейки.ПримечаниеДопЯчейки) Тогда
			ОбластьПоля.Примечание.Текст = ТекстовыеДанныеЯчейки.ПримечаниеДопЯчейки;
			ОбластьПоля.Примечание.Шрифт = мШрифтРаботы;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ТекстовыеДанныеЯчейки.ПримечаниеОсновнойЯчейки) Тогда
		ОбластьРабота.Примечание.Текст = ТекстовыеДанныеЯчейки.ПримечаниеОсновнойЯчейки;
		ОбластьРабота.Примечание.Шрифт = мШрифтРаботы;
	КонецЕсли;
...
Процедура КлиентУслугиВместе(ВыбРаботыБлока,Расшифровка,НачВПоз,КонВПоз,врНачСек,гПоз,ДокументРезультат,ЦветФонаТекДокумента)
	...
	/////////////////////////////////////////////////////////////
	// ОМГ
	Если ЗначениеЗаполнено(ВыбРаботыБлока.омг_РискПадения) Тогда
		
		ОбластьКлиент.Текст = ОбластьКлиент.Текст + Символы.ПС + "Риск падения: " + ВыбРаботыБлока.омг_РискПадения;
	...
	Если Не ПустаяСтрока(ТекстовыеДанныеЯчейки.ПримечаниеОсновнойЯчейки) Тогда
		...
		ОбластьКлиент.Примечание.Шрифт = мШрифтРаботы; 
	КонецЕсли;
...
Функция ПолучитьТаблицуГрафиков(ВыбГрафики)
	...
	// Сортировка по ЭтоРабВремя: при прочих равных нерабочее время должно перекрыть рабочее
	ТабГрафики.Сортировать("ЭтоВидРабВремя УБЫВ, ВремяНачала, ВремяОкончания УБЫВ, ЭтоРабВремя УБЫВ, ДатаСоздания");
...
Функция	ПолучитьТабицуДокументов(резДокументы,безЗаписи = Истина)
	...
	Для сч = 0 по мОбр Цикл
		...
		//////////////////////////////////////////////////////
		// ОМГ                             
		ИначеЕсли безЗаписи = Ложь и ОбщегоНазначения.ЕстьРеквизитДокумента("омг_БезЗаписи", СтрокаТаб.ДокументСсылка.Метаданные()) и СтрокаТаб.ДокументСсылка.омг_БезЗаписи Тогда
		табДокументы.Удалить(СтрокаТаб);
		// ОМГ  
		//////////////////////////////////////////////////////
		КонецЕсли;
