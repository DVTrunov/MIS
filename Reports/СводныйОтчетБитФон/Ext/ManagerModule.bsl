...
Функция СформироватьСтатистикаПоОператорамСервер(Макет, Знач ДатаНачала, Знач ДатаОкончания) Экспорт
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.доп_ИсторияЗвонков КАК ИсторияЗвонков
		|ГДЕ
		|	ИсторияЗвонков.Дата <= &ДатаОкончания
		|	И ИсторияЗвонков.Дата >= ДОБАВИТЬКДАТЕ(&ДатаНачала, ДЕНЬ, -1)
		|	И ИсторияЗвонков.Успешность
		|	И ИсторияЗвонков.ТипЗвонка = ЗНАЧЕНИЕ(Перечисление.доп_ТипЗвонка.Входящий)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|		И ИсторияЗвонков.ТипЗвонка = ЗНАЧЕНИЕ(Перечисление.доп_ТипЗвонка.Входящий)) КАК Неуспешные
		...
		|			И ИсторияЗвонков.Дата >= &ДатаНачала
		|			И ИсторияЗвонков.Успешность
		|			И ИсторияЗвонков.ТипЗвонка = ЗНАЧЕНИЕ(Перечисление.доп_ТипЗвонка.Входящий)) КАК Успешные
		...
		|	И ИсторияЗвонков.Дата >= ДОБАВИТЬКДАТЕ(&ДатаНачала, ДЕНЬ, -1)
		|	И ИсторияЗвонков.Успешность
		|	И ИсторияЗвонков.ТипЗвонка = ЗНАЧЕНИЕ(Перечисление.доп_ТипЗвонка.Исходящий)
		|
		|СГРУППИРОВАТЬ ПО
		|	ИсторияЗвонков.Пользователь,
		|	ИсторияЗвонков.Дата,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	доп_СобытияБитФон.Пользователь КАК Пользователь,
		|	доп_СобытияБитФон.Дата КАК Дата,
		|	НАЧАЛОПЕРИОДА(доп_СобытияБитФон.Дата, ДЕНЬ) КАК День,
		|	доп_СобытияБитФон.Событие КАК Событие,
		|	ВЫБОР
		|		КОГДА доп_СобытияБитФон.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияБитФон.НаЛинии)
		|			ТОГДА 0
		|		КОГДА доп_СобытияБитФон.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияБитФон.Логаут)
		|			ТОГДА 1
		|		ИНАЧЕ 2
		|	КОНЕЦ КАК Приоритет
		|ИЗ
		...
		|ГДЕ
		|	доп_СобытияБитФон.Дата <= &ДатаОкончания
		|	И доп_СобытияБитФон.Дата >= &ДатаНачала
		|
		|УПОРЯДОЧИТЬ ПО
		|	Пользователь,
		|	Дата,
...
Процедура ЗаполнитьСобытияНаГраницахПоЗвонкам(тзСобытия, ИмеютсяПроблемыССобытиями)
	...
	Запрос.Текст = 
		...
		|			ПервыеСтроки.Пользователь КАК Пользователь,
		|			ПервыеСтроки.Дата КАК Дата,
		|			МИНИМУМ(доп_ИсторияЗвонков.Дата) КАК ДатаНачалаЗвонка
		|		ИЗ
		|			РегистрСведений.доп_ИсторияЗвонков КАК доп_ИсторияЗвонков
		|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПервыеСтроки КАК ПервыеСтроки
		|				ПО (ПервыеСтроки.Пользователь = доп_ИсторияЗвонков.Пользователь)
		|					И (доп_ИсторияЗвонков.Дата МЕЖДУ НАЧАЛОПЕРИОДА(ПервыеСтроки.Дата, ДЕНЬ) И КОНЕЦПЕРИОДА(ПервыеСтроки.Дата, ДЕНЬ))
		|					И доп_ИсторияЗвонков.Дата < ПервыеСтроки.Дата
		|		
		|		СГРУППИРОВАТЬ ПО
		...
		|			ПоследниеСтроки.Пользователь КАК Пользователь,
		|			ПоследниеСтроки.Дата КАК Дата,
		|			МАКСИМУМ(ДОБАВИТЬКДАТЕ(доп_ИсторияЗвонков.Дата, СЕКУНДА, доп_ИсторияЗвонков.ДлительностьЗвонка)) КАК ДатаКонцаЗвонка
		|		ИЗ
		|			РегистрСведений.доп_ИсторияЗвонков КАК доп_ИсторияЗвонков
		|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПоследниеСтроки КАК ПоследниеСтроки
		|				ПО (ПоследниеСтроки.Пользователь = доп_ИсторияЗвонков.Пользователь)
		|					И (доп_ИсторияЗвонков.Дата МЕЖДУ НАЧАЛОПЕРИОДА(ПоследниеСтроки.Дата, ДЕНЬ) И КОНЕЦПЕРИОДА(ПоследниеСтроки.Дата, ДЕНЬ))
		|					И доп_ИсторияЗвонков.Дата > ПоследниеСтроки.Дата
		|		
		|		СГРУППИРОВАТЬ ПО
...
Функция ПолучитьДатуПоследнегоЗвонка(Пользователь, ВремяНачалаРаботы, ВремяОкончанияРаботы)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	доп_ИсторияЗвонков.ДлительностьЗвонка КАК ДлительностьЗвонка,
		|	доп_ИсторияЗвонков.Дата КАК Дата
		|ИЗ
		|	РегистрСведений.доп_ИсторияЗвонков КАК доп_ИсторияЗвонков
		|ГДЕ
		|	доп_ИсторияЗвонков.Дата МЕЖДУ &ДатаНач И &ДатаКон
...
Функция ПолучитьТаблицуНедоступности(Пользователи, ДатаНачала, ДатаОкончания)
	...
	Запрос.Текст = 
		...
		|	РегистрСведений.доп_СобытияБитФон КАК доп_СобытияБитФон
		|ГДЕ
		|	доп_СобытияБитФон.Пользователь В (&Пользователи)
		|	И доп_СобытияБитФон.Дата МЕЖДУ ДОБАВИТЬКДАТЕ(&ДатаНачала, МЕСЯЦ, -2) И &ДатаОкончания
		|	И (доп_СобытияБитФон.Событие = ЗНАЧЕНИЕ(ПеречислениЕ.СобытияБитФон.Доступен)
		|			ИЛИ доп_СобытияБитФон.Событие = ЗНАЧЕНИЕ(ПеречислениЕ.СобытияБитФон.Недоступен))
		|
		|УПОРЯДОЧИТЬ ПО
		|	доп_СобытияБитФон.Пользователь,
		|	доп_СобытияБитФон.Дата";
