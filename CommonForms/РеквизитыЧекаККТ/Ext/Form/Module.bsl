...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПерсональныеДанные") Тогда
		...
		Если Параметры.ПерсональныеДанные.Свойство("ЭтоФизЛицо") Тогда
			ЭтоФизЛицо = Параметры.ПерсональныеДанные.ЭтоФизЛицо;
			
		ИначеЕсли ЗначениеЗаполнено(ДатаРождения)
			Или ЗначениеЗаполнено(ВидДокумента) 
			Или ЗначениеЗаполнено(ДанныеДокумента)
			Или Не (ЗначениеЗаполнено(ИНН) Или СтрДлина(ИНН) <> 10 Или ЗначениеЗаполнено(Адрес))
		Тогда
			ЭтоФизЛицо = Истина;
		Иначе
			ЭтоФизЛицо = Ложь;
