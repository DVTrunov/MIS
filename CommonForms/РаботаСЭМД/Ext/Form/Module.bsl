...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьРеквизитыПоМедицинскомуДокументу();
	...
	РасширеннаяФормаПодписания = Параметры.РасширеннаяФормаПодписания;
	Если Не РасширеннаяФормаПодписания Тогда
		Для Каждого Команда Из Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы Цикл
			Если Команда <> Элементы.ФормаПодписатьВсе Тогда
				Команда.Видимость = Ложь;
			КонецЕсли;
		КонецЦикла;
		Элементы.ДанныеНаФорме.ТолькоПросмотр = Истина;
	...
	Элементы.ФормаВключаяПодписиМО.Видимость = Ложь;
	Если Не РасширеннаяФормаПодписания Тогда
		ДоступныПодписиМО = Ложь;
		Для Каждого СтрокаПодписиМО Из Данные.НайтиСтроки(Новый Структура("РольСотрудника", Перечисления.РолиПодписей.МедицинскаяОрганизация)) Цикл
			Если СтрокаПодписиМО.ДоступноРолей > 0 Тогда
				Элементы.ФормаВключаяПодписиМО.Видимость = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура УдалитьВсеПодписи(Команда)
	
	УдалитьПодписи(Истина);
...
Процедура УдалитьПодписиПродолжение(Результат, Параметры) Экспорт
	...
	УдалитьПодписиИОбновитьСписок(Ложь, ИдентификаторФормыДляБлокировки, Параметры.УдалитьВсе);
	...
	Для Каждого РезультатПоУИД Из РезультатСохранения.УИДы Цикл
		...
		Для Каждого СтрокаДанных Из СтрокиДанныхРЭМД Цикл
			...
			Тогда
				...
				СтрокаДанных.СпециальностьМинздрава	= РезультатПоУИД.СпециальностьМинздрава;
...
&НаСервере
Процедура ЗаполнитьРеквизитыПоМедицинскомуДокументу()
	
	РеквизитДатыМедицинскогоДокумента = "Дата";
	РеквизитФилиалаМедицинскогоДокумента = "Филиал";
	
	Если ИмяТипаДокумента = "МедицинскиеКарты" Тогда
		РеквизитДатыМедицинскогоДокумента = "ДатаРегистрации";
	КонецЕсли;
	
	ДатаМедицинскогоДокумента	= ОбщегоНазначенияБИТКлиентСервер.СвойствоОбъекта(ЭтотОбъект["МедицинскийДокумент"], РеквизитДатыМедицинскогоДокумента);
	ФилиалМедицинскогоДокумента	= ОбщегоНазначенияБИТКлиентСервер.СвойствоОбъекта(ЭтотОбъект["МедицинскийДокумент"], РеквизитФилиалаМедицинскогоДокумента);
...
Процедура ЗаполнитьДаннымиРЭМД()
	...
	Для Каждого УИДШаблона Из УИДыШаблонов Цикл
		...
		ПодписанОбязательнойРолью = Ложь;
		ЕстьОбязательнаяРоль = Ложь;
		...
		Для Каждого СтрокаДанныхРЭМД Из ДанныеРЭМД Цикл
			Если СтрокаДанныхРЭМД.УИД_Шаблона = УИДШаблона Тогда
				...
				Если СтрокаДанныхРЭМД.МинКолПодписей > 0 Тогда
					ЕстьОбязательнаяРоль = Истина;
					Если СтрокаДанныхРЭМД.ПодписьУстановлена Тогда
						ПодписанОбязательнойРолью = Истина;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого СтрокаДанных Из СтрокиДанныхРЭМД Цикл
			...
			Если СтрокаДанных.МинКолПодписей = 0
				И Не ПодписанОбязательнойРолью
				И ЕстьОбязательнаяРоль
			Тогда
				СтрокаДанных.ДоступноРолей = 0;
			КонецЕсли;
	...
	Для Каждого РольЭМД Из ЭМДРолей Цикл
		...
		Для Каждого СтрокаРоли Из СтрокиРолей Цикл
			...
			Тогда
				...
				НоваяСтрока.Выбран = Не РольМО(НоваяСтрока.РольСотрудника) Или РасширеннаяФормаПодписания Или Элементы.ФормаВключаяПодписиМО.Пометка;
				ИспользованныеРоли.Добавить(НоваяСтрока.РольСотрудника);
			КонецЕсли;
...
&НаКлиенте
Функция РольМО(РольСотрудника)
	
	Возврат РольСотрудника = ПредопределенноеЗначение("Перечисление.РолиПодписей.МедицинскаяОрганизация");
...
Функция ПолучитьПредставлениеТипаПодписанияНеРЭМД()
	...
	Если ИмяТипаДокумента = "Документ.ПрохождениеМедосмотра"
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭтотОбъект["МедицинскийДокумент"], "ВидПрохожденияМО") <> ОбщегоНазначения.ЗначениеПеречисленияПоИмени("СправкаПрочий", "ВидыМедосмотров")
	Тогда
...
Процедура ДополнитьДаннымиДокумента(МассивОбъектов)
	...
	Для Каждого СтрокаПоТипу Из СтрокиПоТипу Цикл
		...
		СтруктураОбъекта.Вставить("ПодписываемыеДанные", Новый ОписаниеОповещения("ПриЗапросеДвоичныхДанныхОбъекта", ОбщегоНазначенияКлиент.ОбщийМодуль("доп_СертификатыЭЦПКлиент"), ТекущиеПараметрыВыполнения));
...
Процедура ДополнитьДаннымиРЭМД(МассивОбъектов, СтруктурыДанных, СтруктураОповещенияПередВыполнением, ОтсутствующиеФайлы)
	...
	Для Каждого СтрокаПоТипу Из СтрокиПоТипу Цикл
		...
		Тогда
			ПолныйПуть = ИнтеграцияЕГИСЗ_РЭМД.ПолучитьПолныйПутьПоОтносительномуПутиВХранилищеЭМД(СтрокаПоТипу.ПутьКФайлуВАрхиве, ФилиалМедицинскогоДокумента);
	...
	Если СтрокиКПодписанию.Количество() > 0 Тогда
		...
		Для Каждого СтрокаКПодписанию Из СтрокиКПодписанию Цикл
			...
			ПолныйПуть = ИнтеграцияЕГИСЗ_РЭМД.ПолучитьПолныйПутьПоОтносительномуПутиВХранилищеЭМД(СтрокаКПодписанию.ПутьКФайлуВАрхиве, ФилиалМедицинскогоДокумента);
...
Процедура ЗаполнитьСтруктуруОповещенияПередВыполнением(СтруктураОповещенияПередВыполнением, ДополнительныеПараметры)
	...
	Иначе
		...
		СтруктураОповещенияПередВыполнением.Вставить("Модуль",					ОбщегоНазначенияКлиент.ОбщийМодуль("доп_РаботаСФайламиКлиент"));
...
Процедура ПодписатьМассивОбъектов(МассивОбъектов, СтруктурыДанных, СтруктураОповещенияПередВыполнением)
	...
	Для Каждого ОбъектМ Из МассивОбъектов Цикл
		...
		Если ОбъектМ.ТипПодписания = 3 Тогда
			ПодписаниеРЭМД = Истина;
		Иначе
			ПодписаниеНеРЭМД = Истина;
		КонецЕсли;
	...
	Если ПодписаниеПодписьюМО Тогда
		ОписаниеДанных.Вставить("ОтборСертификатов", ПолучитьСертификатыМОПользователя(ЭтотОбъект["МедицинскийДокумент"], ПодписаниеРЭМД И ПодписаниеНеРЭМД));
		ОписаниеДанных.Операция = НСтр("ru = 'Подпись медицинской организации'");
	Иначе
		ОписаниеДанных.Вставить("ОтборСертификатов", ПолучитьСертификатыТекущегоПользователя());
	КонецЕсли;
...
Процедура ЗаполнитьТаблицыДанных()
	...
	ИспользуетсяРЭМД = ИнтеграцияЕГИСЗСерверПовтИсп.ИспользуетсяИнтеграцияРЭМД(ФилиалМедицинскогоДокумента) И ДатаМедицинскогоДокумента >= ИнтеграцияЕГИСЗСерверПовтИсп.ПолучитьДатуНачалаОбменаИнформациейСРЭМД(ФилиалМедицинскогоДокумента);
	...
	Если ИмяТипаДокумента = "Документ.Прием" Тогда
		Если ИспользуетсяРЭМД Тогда
			ЕстьПодписиНеРЭМД = доп_СертификатыЭЦП.ПолучитьЭлектронныеПодписи(ЭтотОбъект["МедицинскийДокумент"],,,, Ложь).Количество() > 0;
			Для Каждого Шаблон Из ЭтотОбъект["МедицинскийДокумент"].ШаблоныПриема Цикл
				...
				Иначе
					Если ЕстьПодписиНеРЭМД
						И Не доп_РаботаСФайлами.ФайлЭМДСохранен(ЭтотОбъект["МедицинскийДокумент"])
					Тогда
	...
	ИначеЕсли ИмяТипаДокумента = "Документ.ПрохождениеМедосмотра" Тогда
		...
		Иначе
			...
			ПриказыВредныеФакторы = МодульМедосмотрыСервер.ПриказыВредныеФакторыМедосмотраСВидамиДокументов(ЭтотОбъект["МедицинскийДокумент"]);
			...
			Для Каждого СтрокаПриказаВредногоФактора Из ПриказыВредныеФакторы Цикл
				...
				Тогда
					Если ИспользуетсяРЭМД Тогда
						Если ЗначениеЗаполнено(ПриказВредныйФактор.ШаблонОсмотраСправки.ВидДокумента) Тогда
							ПодписаниеДляРЭМД = Истина;
						ИначеЕсли Не ПустойТабДокШаблонаСправки(ПриказВредныйФактор.ШаблонОсмотраСправки) Тогда
							ПодписаниеНеДляРЭМД = Истина;
						КонецЕсли;
					ИначеЕсли Не ПустойТабДокШаблонаСправки(ПриказВредныйФактор.ШаблонОсмотраСправки) Тогда
						ПодписаниеНеДляРЭМД = Истина;
				...
				Иначе
					Если ИспользуетсяРЭМД Тогда
						Если ЗначениеЗаполнено(Приказ.ВидДокумента)
							Или Приказ = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПриказыМедосмотров.Приказ29н")
						Тогда
							ПодписаниеДляРЭМД = Истина;
						ИначеЕсли ЗначениеЗаполнено(Приказ.МакетДляПодписанияЭЦП) Тогда
							ПодписаниеНеДляРЭМД = Истина;
						КонецЕсли;
					ИначеЕсли ЗначениеЗаполнено(Приказ.МакетДляПодписанияЭЦП) Тогда
						ПодписаниеНеДляРЭМД = Истина;
			...
			Если ИспользуетсяРЭМД
				И МодульМедосмотрыСервер.ТребуетсяПодписаниеПечатнойФормыЗаключения(ЭтотОбъект["МедицинскийДокумент"])
			Тогда
				ПодписаниеНеДляРЭМД = Истина;
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьГруппуПриказовСправок()
	...
	Если МодульМедосмотрыСервер = Неопределено Тогда
		Возврат Ложь;
	Иначе
		Возврат МодульМедосмотрыСервер.ПолучитьГруппуПриказовСправок()
	КонецЕсли;
...
Функция ПустойТабДокШаблонаСправки(ШаблонОсмотраСправки)
	...
	МодульМедосмотрыСервер = ДопСерверныеФункции.ОбщийМодуль("МедосмотрыСервер");
	Возврат МодульМедосмотрыСервер.ПустойТабДокШаблонаСправки(ШаблонОсмотраСправки)
...
Функция ЕстьЗаполненныеМакетыПриказов(ФакторыИУсловияРаботы)
	...
	МодульМедосмотрыСервер = ДопСерверныеФункции.ОбщийМодуль("МедосмотрыСервер");
	Возврат МодульМедосмотрыСервер.ЕстьЗаполненныеМакетыПриказов(ФакторыИУсловияРаботы)
...
Процедура ОбновитьДанныеРЭМДПоМедицинскомуДокументу()
	...
	Если ИмяТипаДокумента = "Документ.Прием" Тогда
		...
		Запрос.Текст = 
			...
			|		ЕСТЬNULL(ВерсииШаблонов.ВидДокумента, ШаблоныПриемов.ВидДокумента) КАК ВидДокумента
			|	ИЗ
			...
			|				ВерсииШаблоновHTML.ВидДокумента КАК ВидДокумента
			|			ИЗ
			|				(ВЫБРАТЬ
			|					ШаблоныПриемов.ШаблонПриема КАК ШаблонПриема,
			...
			|		ВерсииШаблонов.ШаблонПриема КАК ШаблонЭМД,
			|		ВерсииШаблонов.ВидДокумента КАК ВидДокумента,
			...
			|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВерсииШаблонов КАК ВерсииШаблонов
			...
			|			ПО (ВерсииШаблонов.ШаблонПриема = ПриемШаблоныПриема.ШаблонПриема)
			|	ГДЕ
			|		ПриемШаблоныПриема.Ссылка = &Ссылка
			|		И ВерсииШаблонов.ВидДокумента <> ЗНАЧЕНИЕ(Справочник.РегистрируемыеЭлектронныеМедДокументы.ПустаяСсылка)
			|	
			|	СГРУППИРОВАТЬ ПО
			|		ПриемШаблоныПриема.УИД,
			|		ВерсииШаблонов.ВидДокумента,
...
Процедура УстановитьОтборСпециальностей()
	...
	Для Каждого СтрокаСпециализации Из ТекущийСотрудник.СпециализацииФРМР Цикл
		СпециализацииСотрудника.Добавить(СтрокаСпециализации.Специализация);
	КонецЦикла;
	
	ПараметрыВыбораПоля = Новый Массив;
	ПараметрыВыбораПоля.Добавить(Новый ПараметрВыбора("Отбор.Ссылка", СпециализацииСотрудника));
	...
	Элементы.ДанныеНаФормеСпециальностьМинздрава.СписокВыбора.ЗагрузитьЗначения(СпециализацииСотрудника);
...
Процедура ОбновитьДанныеПоМедицинскомуДокументу()
	...
	Пока Выборка.Следующий() Цикл
		...
		Тогда
			...
			ТекущийУИД_ЭМД = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПриказыМедосмотров.Приказ29н").УникальныйИдентификатор();
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьСертификатыМОПользователя(МедицинскийДокумент, СмешанноеПодписание)
	
	Филиал = РаботаСДокументамиСервер.ФилиалДокумента(МедицинскийДокумент);
	
	Полномочия = Неопределено;
	Если СмешанноеПодписание Тогда
		Полномочия = Перечисления.ПолномочияЭЦП.РЭМД;
	КонецЕсли;
	
	СертификатыМОПользователя = ЭЦП_УМЦ_Сервер.ПолучитьСертификатыМОПользователя(Филиал, Полномочия);
	...
	Возврат СертификатыМОПользователя;
	
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьСертификатыТекущегоПользователя()
	
	СертификатыПользователя = ЭЦП_УМЦ_Сервер.ПолучитьСертификатыПользователя();
	Если СертификатыПользователя.Количество() = 0 Тогда
		СертификатыПользователя.Добавить(Справочники.СертификатыКлючейЭлектроннойПодписиИШифрования.ПустаяСсылка());
	КонецЕсли;
	
	Возврат СертификатыПользователя;
...
Функция ПолучитьДДФайлов(МедицинскийДокумент, УИДыЭМД)
	...
	ДанныеФайловЭМД = доп_РаботаСФайлами.ПолучитьДанныеФайловЭМД(МедицинскийДокумент);
...
Функция СформироватьФайлыДляСохранения(МедицинскийДокумент, РасширениеДляФайловПодписи)
	...
	ДанныеСохраненныхФайловЭМД = доп_РаботаСФайлами.ПолучитьДанныеФайловЭМД(МедицинскийДокумент, Истина);
...
Процедура ЗаполнитьСписокПодписей()
	...
	доп_СертификатыЭЦП.ЗаполнитьСписокПодписей(ЭтотОбъект["МедицинскийДокумент"], УникальныйИдентификатор, Неопределено, ЭлектронныеПодписи, ПодписиФайловЭМД);
...
Процедура УстановитьДоступностьКомандСпискаПодписей()
	...
	Элементы.ТаблицаПодписейУдалитьВсе.Доступность		= ЕстьПодписи;
	...
	Элементы.ТаблицаПодписейКонтекстноеМенюУдалитьВсе.Доступность		= ЕстьПодписи;
	Элементы.ТаблицаПодписейКонтекстноеМенюСохранить.Доступность		= ЕстьПодписи И ЭтоПодпись;
	
КонецПроцедуры

&НаКлиенте
Процедура УдалитьПодписи(УдалитьВсе = Ложь)
	
	Если УдалитьВсе Тогда
		ТекстВопроса = НСтр("ru='Удалить все подписи?'");
	ИначеЕсли Элементы.ЭлектронныеПодписи.ВыделенныеСтроки.Количество() > 1 Тогда
		ТекстВопроса = НСтр("ru='Удалить выделенные подписи?'");
	Иначе
		ТекстВопроса = НСтр("ru='Удалить выделенную подпись?'");
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("УдалитьПодписиПродолжение", ЭтотОбъект, Новый Структура("УдалитьВсе", УдалитьВсе));
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

&НаСервере
Процедура УдалитьПодписиИОбновитьСписок(РеквизитПодписанИзменен, ИдентификаторФормыДляБлокировки, УдалитьВсе)
	...
	Если УдалитьВсе Тогда
		...
		Для Каждого СтрокаЭЦП Из ЭлектронныеПодписи Цикл
			ИдентификаторыУдаляемыхСтрок.Добавить(СтрокаЭЦП.ПолучитьИдентификатор());
		КонецЦикла;
	Иначе
		ИдентификаторыУдаляемыхСтрок = Элементы.ЭлектронныеПодписи.ВыделенныеСтроки;
	КонецЕсли;
	
	Для Каждого ИдентификаторСтроки Из ИдентификаторыУдаляемыхСтрок Цикл
		УИД_ЭЦП = ЭлектронныеПодписи.НайтиПоИдентификатору(ИдентификаторСтроки).УникальныйИдентификатор;
		...
		КонецЦикла;
	КонецЦикла;
	ОбъектМД.ДополнительныеСвойства.Вставить("ПроверкаКлючевыхПолейОтключена", Истина);
	ОбъектМД.Записать();
	
	доп_СертификатыЭЦП.УдалитьПодписиИОбновитьСписок(РеквизитПодписанИзменен, ИдентификаторыУдаляемыхСтрок,
	...
	Если ЭлектронныеПодписи.Количество() = 0 Тогда
		...
		Тогда
			доп_РаботаСФайлами.УдалитьФайлЭМД(ЭтотОбъект["МедицинскийДокумент"]);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьДоступныеДолжностиМЗ()
	...
	ПараметрыВыбораПоля.Добавить(Новый ПараметрВыбора("Отбор.ВидКлассификатора", ПредопределенноеЗначение("Перечисление.ВидыКлассификаторовМинЗдрава.ДолжностиРаботниковМедОрганизаций")));
...
Процедура ПроверитьСтрокиПодписей(СтрокиПодписей)
	...
	доп_СертификатыЭЦПКлиент.ПроверитьПодписиОбъектаВФормеРаботыСЭМД(?(ПодписаниеИзФормыВладельца, ЭтотОбъект.ВладелецФормы, Неопределено), ЭтотОбъект, СтрокиПодписей,
		Новый ОписаниеОповещения("ПроверитьПодписиЗавершение", доп_СертификатыЭЦПКлиент, ПараметрыОповещения));
	
КонецПроцедуры

&НаКлиенте
Процедура ВключаяПодписиМО(Команда)
	Элементы.ФормаВключаяПодписиМО.Пометка = Не Элементы.ФормаВключаяПодписиМО.Пометка;
	ЗаполнитьДанныеНаФорме();
КонецПроцедуры
