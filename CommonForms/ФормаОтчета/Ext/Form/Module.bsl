...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//////////////////////////////////////////////////////////////////////
	// ОМГ         
	Если Параметры.Свойство("омг_ОтветственныйРегистратор") Тогда
		ОтчетОбъект.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить("омг_ОтветственныйРегистратор",Параметры.омг_ОтветственныйРегистратор);
	КонецЕсли;
	// ОМГ
	//////////////////////////////////////////////////////////////////////
...
Процедура ПриОткрытии(Отказ)
	...
	// ОМГ
		...
		Если не ЭтотОбъект.Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("омг_ОтветственныйРегистратор") = Неопределено Тогда
		омг_ОтветственныйРегистратор = ЭтотОбъект.Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("омг_ОтветственныйРегистратор").Значение;	
		Если ЗначениеЗаполнено(омг_ОтветственныйРегистратор) Тогда
			
			Для каждого строкаОтбора из ЭтотОбъект.Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
				
				Если строкаОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Регистратор.Ответственный") Тогда
					строкаОтбора.ПравоеЗначение = омг_ОтветственныйРегистратор;
					Прервать;
				КонецЕсли;
				
			КонецЦикла;
			
			Для каждого строкаОтбора из ЭтотОбъект.Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
				
				Если строкаОтбора.ИдентификаторПользовательскойНастройки = "b4c154c1-5400-454b-aa1f-c9a35e3dbd99" Тогда
					строкаОтбора.ПравоеЗначение = омг_ОтветственныйРегистратор;
					Прервать;
				КонецЕсли;
		 ...
		 ЭтотОбъект.Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("омг_ОтветственныйРегистратор").Значение	= ПредопределенноеЗначение("Справочник.Пользователи.ПустаяСсылка");
...
&НаКлиенте
Процедура доп_ОткрытьФормуСпискаВариантовОтчета(Команда)
...
&НаКлиенте
Процедура Подключаемый_доп_ИзменитьУровеньГруппировки(Команда)
