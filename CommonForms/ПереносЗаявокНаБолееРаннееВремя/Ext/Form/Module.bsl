...
Процедура НачатьРазговор(Команда)
	...
	Если Команда.Имя = "НачатьРазговор" Тогда
		доп_БитфонКлиент.ОткрытьБитфонНачатьРазговор(Элементы.ЗаявкиДляПереноса.ТекущиеДанные.Клиент);
	Иначе
		доп_АТСКлиент.ОткрытьМониторНачатьРазговор(Элементы.ЗаявкиДляПереноса.ТекущиеДанные.Клиент);
	КонецЕсли;
