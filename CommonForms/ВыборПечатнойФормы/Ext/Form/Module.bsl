...
Процедура ПоУмолчаниюНажатие(Элемент)
	...
	ТекущиеДанныеСтр.Вставить("ПечатнаяФорма", ?(ЗначениеЗаполнено(ТекущиеДанные.СсылкаНаВнешнююОбработку), ТекущиеДанные.СсылкаНаВнешнююОбработку, ТекущиеДанные.Имя));
...
Процедура ПоУмолчаниюНажатиеСервер(ТекущиеДанныеСтр, СсылкаДок)
	...
	Иначе
		...
		// Определяем, назначена встроенная форма или нет
		УстановитьПривилегированныйРежим(Истина);
		ЭтоВстроеннаяФорма = Ложь;
		Попытка
			ПечатныеФормыОбъекта = ПараметрыСеанса.ПечатныеФормы.Получить(Новый(ТипЗнч(ДокументСсылка)));
			Для Каждого ОписаниеПечатнойФормы Из ПечатныеФормыОбъекта Цикл
				Если ТипЗнч(ОписаниеПечатнойФормы.ПечатнаяФорма) = Тип("Строка")
					И ОписаниеПечатнойФормы.ПечатнаяФорма = ТекущиеДанныеСтр.Имя 
				Тогда
					ЭтоВстроеннаяФорма = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		...
		Если ЭтоВстроеннаяФорма Тогда
			ХранилищеПользовательскихНастроекОтчетов.Сохранить(ИмяМетаданных,"ПечатнаяФорма", ТекущиеДанныеСтр.Имя);
		Иначе
			ХранилищеПользовательскихНастроекОтчетов.Сохранить(ИмяМетаданных,"ПечатнаяФорма", ТекущиеДанныеСтр.ПечатнаяФорма);
		КонецЕсли;
	КонецЕсли;
...
Процедура НастроитьДеревоМакетовПечатиСервер(Ссылка)
	Попытка
		СтруктураВнутреннихПечатныхФорм = ПечатьДокументовСервер.ПолучитьСтруктуруПечатныхФормСервер(Ссылка);
	Исключение
	...
	ПечатнаяФормаПоУмолчанию = ХранилищеПользовательскихНастроекОтчетов.Загрузить(ДокументСсылка.Метаданные().Имя, "ПечатнаяФорма");
	Если ПечатнаяФормаПоУмолчанию <> Неопределено Тогда
		Если ТипЗнч(ПечатнаяФормаПоУмолчанию) = Тип("Строка") Тогда
			СтрокаПоУмолчанию = Дерево.Строки.Найти(ПечатнаяФормаПоУмолчанию, "Имя");
			Если СтрокаПоУмолчанию = Неопределено Тогда
				СтрокаПоУмолчанию = Дерево.Строки.Найти(ПечатнаяФормаПоУмолчанию, "Текст");
			КонецЕсли;
		ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ПечатнаяФормаПоУмолчанию)) Тогда
			СтрокаПоУмолчанию = Дерево.Строки.Найти(ПечатнаяФормаПоУмолчанию, "СсылкаНаВнешнююОбработку");
		КонецЕсли;
	КонецЕсли;
