...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	///////////////////////////////////////////////////////////
	// ОМГ
	РискПадения = Перечисления.омг_РискиПадения.Низкий;
	// ОМГ 
	///////////////////////////////////////////////////////////
...
Процедура АдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		ЗначенияПолей = КонтактнаяИнформацияСервер.ЗначенияПолейСГородомИзКэшаФилиала(ПустаяСсылка);
...
Процедура ОповеститьОВыбореКлиента(Клиент)
	...
	Если ВладелецФормы <> Неопределено Тогда
		//DreAM++
		//обновим данные по клиенту из ЕИП в фоне
		ИнтеграцияЕИП.ОбновитьДанныеПоКлиентуИзЕИПВФоне(Клиент);
		//DreAM--
...
Функция СоздатьНовогоКлиентаСервер(ДанныеКлиента, СоздаватьМедкарту = Ложь)
	...
	КлиентСсылка = Справочники.Клиенты.ПолучитьСсылку(Новый УникальныйИдентификатор);
	Объект.УстановитьСсылкуНового(КлиентСсылка);
	...
	///////////////////////////////////////////////////////////
	// ОМГ   
	Объект.омг_РискПадения = 	ДанныеКлиента.РискПадения; 
	// ОМГ 
	///////////////////////////////////////////////////////////
	...
	//DreAM ++
	Если ДанныеКлиента.Свойство("ВидСкидки") И ЗначениеЗаполнено(ДанныеКлиента.ВидСкидки) Тогда
		Объект.ВидСкидки = ДанныеКлиента.ВидСкидки; 
		НовСтр = Объект.омг_КартыСкидок.Добавить();
		...
		Запрос.УстановитьПараметр("ВидСкидки", Объект.ВидСкидки);
		ВыборкаВидовКарт = Запрос.Выполнить().Выбрать();
		
		Если Не ВыборкаВидовКарт.Следующий() Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Не удалось создать клиента. Для вида скидки """ + ДанныеКлиента.ВидСкидки + """ не указано соответствие виду карты";
			Сообщение.Сообщить();
			Возврат Неопределено;
		КонецЕсли;
		
		НоваяКарта = Справочники.КартыСкидок.СоздатьЭлемент();
		НоваяКарта.ВидКарты = ВыборкаВидовКарт.ВидКарты;
		НоваяКарта.ВладелецКарты = КлиентСсылка;
		НоваяКарта.Наименование = ДанныеКлиента.НомерКартыСкидки;
		НоваяКарта.Записать();
		
		
		НовСтр.Карта = НоваяКарта.Ссылка;//ИнтеграцияЕИП.ПолучитьКартуСкидкиПоВидуСкидки(Объект.ВидСкидки);
		НовСтр.НомерКарты = ДанныеКлиента.НомерКартыСкидки;
	КонецЕсли;
	//DreAM --
	...
	/////////////////////////////////////////////////////////////////////////
	// ОМГ 
		Объект.Гражданство = ДанныеКлиента.ВидДокумента.омг_Страна; 	
	// ОМГ 
	/////////////////////////////////////////////////////////////////////////
	...
	//DreAM ++
	//запишем сумму накоплений по скидкам
	Если ДанныеКлиента.Свойство("СуммаНакоплений") И ЗначениеЗаполнено(ДанныеКлиента.СуммаНакоплений) Тогда
		// Старый вариант
		// ИнтеграцияЕИП.СделатьЗаписьПоИсторииСкидок(Объект.Ссылка, Объект.ВидСкидки, ИнтеграцияЕИП.ПолучитьКартуСкидкиПоВидуСкидки(Объект.ВидСкидки), ДанныеКлиента.СуммаНакоплений, ТекущаяДата()); 
		ИнтеграцияЕИП.СделатьЗаписьПоИсторииСкидок(Объект.Ссылка, Объект.ВидСкидки, НоваяКарта.Ссылка, ДанныеКлиента.СуммаНакоплений, ТекущаяДата()); 
	КонецЕсли;
	//DreAM --
...
Процедура ОбработкаПоискаПоФИО(Текст)
	...
	Если Не ПустаяСтрока(Текст) И СтрДлина(Текст) >= 1 Тогда
		...
		//DreAM ++
		//клиента в существующей базе не нашли, указали иин, значит можно поискать в ЕИП
		Если Результат.Количество() = 0 И СтрДлина(Текст) = 12 И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Текст) И ИнтеграцияЕИПКлиент.ИИНБИНСоответствуетТребованиям(Текст) Тогда
			СтруктураНовогоКлиента = ИнтеграцияЕИП.ПоискКартыПоИИН(Текст);
			/////////////////////////////////////////////////////////////
			// ОМГ
			ИНН = Текст;
			// ОМГ  
			/////////////////////////////////////////////////////////////
			//данные нашлись
			Если СтруктураНовогоКлиента.ОтветПустой = Ложь Тогда
				...
				ВидСкидки = СтруктураНовогоКлиента.ВидСкидки;
				СуммаНакоплений = СтруктураНовогоКлиента.Сумма;
				НомерКартыСкидки = СтруктураНовогоКлиента.НомерКарты;
			КонецЕсли;
		КонецЕсли;
		//DreAM --
...
Функция ВыполнитьПоиск(ПолеПоиска,Знач ЗначениеПоиска)
	...
	// Изменения Макеев 08-06-2026 (
	//////////////////////////////////////////////////////////////////////
	// Доп 
	ПараметрыСтрокиПоиска = Новый Структура("ЕстьЦифры, ЕстьБуквы, ИспользоватьАвтоПодборКлиентовПоИмениИФамилии", Ложь, Ложь, Истина);
	мсНайденныеКлиенты = РаботаСФормамиСервер.ПолучитьМассивКлиентовПоАвтоподбору(ЗначениеПоиска, УправлениеНастройками.ПолучитьПараметрУчетнойПолитики("ИспользоватьАвтоПодборКлиентовПоКоду"), ПараметрыСтрокиПоиска);

	Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЗначениеПоиска) Тогда
	...
	Запрос.УстановитьПараметр("ИНН", ЗначениеПоиска);
	...
	Если ВыборкаДетальныеЗаписи.Количество() <> 0 Тогда
		...
		мсНайденныеКлиенты.Очистить();
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл  
			Представление = ВыборкаДетальныеЗаписи.Ссылка.Наименование + ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Ссылка.ДатаРождения),  " (" + Формат(ВыборкаДетальныеЗаписи.Ссылка.ДатаРождения,"ДЛФ=D") + ")","");
			мсНайденныеКлиенты.Добавить(ВыборкаДетальныеЗаписи.Ссылка, Представление);
		КонецЦикла;
...
Функция ПроверитьЗаполнениеОбязательныхРеквизитовПередЗаписью(Отказ, Объект, ДанныеКлиента)
	...
	ТекстОшибки = РегистрыСведений.ОбязательныеРеквизитыОбъектов.ПроверитьЗаполненностьОбязательныхРеквизитов(ОбъектСтруктура, Объект.Ссылка.Пустая(), Отказ).ТекстОшибки;
...
Функция ПараметрыСозданияКлиента()
	...
	ДанныеКлиента.Вставить("ВидСкидки", ВидСкидки); //DreAM
	ДанныеКлиента.Вставить("СуммаНакоплений", СуммаНакоплений); //DreAM
	ДанныеКлиента.Вставить("НомерКартыСкидки", НомерКартыСкидки);
	
	///////////////////////////////////////////////////////////
	// ОМГ    
	ДанныеКлиента.Вставить("РискПадения", РискПадения);
	// ОМГ 
	///////////////////////////////////////////////////////////
