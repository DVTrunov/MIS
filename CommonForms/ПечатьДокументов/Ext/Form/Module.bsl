...
Процедура СоздатьРеквизитыИЭлементыФормыДляПечатныхФорм(КоллекцияПечатныхФорм)
	...
	Для Каждого РеквизитФормы Из НовыеРеквизитыФормы Цикл
		...
		ОписаниеПечатнойФормы.Экземпляров = ?(ОписаниеПечатнойФормы.Экземпляров = Неопределено, 1, ОписаниеПечатнойФормы.Экземпляров); //+ОЛИМП
...
Процедура НастроитьВидимостьЭлементовФормы(Знач ЕстьРазрешенныйВывод)
	...
	Элементы.КнопкаСохранить.Видимость = ЕстьДанныеДляПечати И ЕстьРазрешенныйВывод; //+ОЛИМП И ЕстьРазрешенноеРедактирование;
