...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.ДанныеСобытия) = Тип("Структура") Тогда
		...
		ДопСерверныеФункции.СвернутьТаблицуЗначений(ТабОбъектыКОбработке, "");
...
Процедура КомандаОК(Команда)
	...
	Если ВремяНач >= ВремяКон Тогда
		ПоказатьПредупреждение(,"Время начала события должно быть меньше времени его окончания!");
		Возврат
	КонецЕсли;
	...
	Тогда
		...
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(,"Некорректно указан период события!");	
	КонецЕсли;
