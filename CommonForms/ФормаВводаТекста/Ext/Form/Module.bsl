...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = РаботаСШаблонамиHTML.ПредставлениеПараметраШаблона(Параметр);
	...
	ЗаполнениеИзИсторииСвязанныхПараметров_ПриСоздании();
...
Процедура ЗаполнениеИзИсторииСвязанныхПараметров_ПриСоздании()
	
	ОткрытьСводныеДанные = ОбщегоНазначенияСервер.НастройкиИзХранилища("Прием", "ОткрытьСводныеДанные");
	ОткрытьСводныеДанные = ?(ОткрытьСводныеДанные = Неопределено, Ложь, ОткрытьСводныеДанные);
	Элементы.ФормаОткрытьСводныеДанные.Пометка = ОткрытьСводныеДанные;
	Элементы.ГруппаПраваяКолонка.Видимость = ОткрытьСводныеДанные;
	Если Элементы.ГруппаПраваяКолонка.Видимость Тогда
		ЗаполнитьЗначенияИсторииСвязанныхПараметров(Параметры.ПервичныйПрием);
		СводныеДанныеЗаполнены = Истина;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьЗначенияИсторииСвязанныхПараметров(ПервичныйПрием)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Источники.Приемник КАК Приемник
	               |ИЗ
	               |	РегистрСведений.ПараметрыШаблоновИсточникиВводаДругихПараметров КАК Источники
	               |ГДЕ
	               |	Источники.Приемник = &Параметр
	               |	И Источники.Источник.ИспользуетсяДляВводаДругихПараметров";
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Источники = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Источник");
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		               ...
		               |	ПриемыПоПервичному.Дата КАК Дата,
		               ...
		               |		И НЕ Прием.ПометкаУдаления) КАК ПриемыПоПервичному
		               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияПараметровHTML КАК ЗначенияПараметровHTML
		               |		ПО ПриемыПоПервичному.Ссылка = ЗначенияПараметровHTML.Документ
		               |			И (ЗначенияПараметровHTML.Параметр В (&Источники))
		               |
		               |УПОРЯДОЧИТЬ ПО
		               |	Дата
		               |ИТОГИ ПО
		...
		Пока Выборка.Следующий() Цикл
			...
			Пока ВыборкаДетальная.Следующий() Цикл
				...
				СтрокаВрача.Дата = ВыборкаДетальная.Дата;
		...
		Элементы.ДеревоПараметров.НачальноеОтображениеДерева = НачальноеОтображениеДерева.РаскрыватьВсеУровни;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьСводныеДанные(Команда)
	
	Если Не СводныеДанныеЗаполнены Тогда
		ЗаполнитьЗначенияИсторииСвязанныхПараметров(Параметры.ПервичныйПрием);
	КонецЕсли;
	
	Элементы.ФормаОткрытьСводныеДанные.Пометка = Не Элементы.ФормаОткрытьСводныеДанные.Пометка;
	ОбщегоНазначенияСервер.НастройкиВХранилище("Прием", "ОткрытьСводныеДанные", Элементы.ФормаОткрытьСводныеДанные.Пометка);
	Элементы.ГруппаПраваяКолонка.Видимость = Элементы.ФормаОткрытьСводныеДанные.Пометка;	
...
Процедура ДеревоПараметровВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.Уровень = 2 Тогда
		Текст = СокрЛП(Текст) + " " + СокрЛП(ТекущиеДанные.Значение);
	КонецЕсли;
