...
Процедура ОбновитьКоличествоДокументов(ЭлементВсего, ЭлементЗаСегодня, ТекстЗапросаДокументов)
	...
	Если Метаданные.Документы.Найти("ПрохождениеМедосмотра") <> Неопределено Тогда
		...
		Запрос.УстановитьПараметр("ВидЗаключения_230",					ИнтеграцияЕГИСЗОбщегоНазначенияПовтИсп.РегистрируемыеЭлектронныеМедДокументы(230));
		Запрос.УстановитьПараметр("ВидЗаключения_194",					ИнтеграцияЕГИСЗОбщегоНазначенияПовтИсп.РегистрируемыеЭлектронныеМедДокументы(194));
		Запрос.УстановитьПараметр("ВидЗаключения_103",					ИнтеграцияЕГИСЗОбщегоНазначенияПовтИсп.РегистрируемыеЭлектронныеМедДокументы(103));
	КонецЕсли;
