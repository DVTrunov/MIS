...
Процедура ЛокальнаяПроверкаИОтправкаЗапроса()
	...
	ПараметрыЗапросаКМ.КодЕдиницыИзмерения 			= ПроверяемыйКМ.КодЕдиницыИзмерения;
	ПараметрыЗапросаКМ.МераКоличестваПредметаРасчета =ПроверяемыйКМ.МераКоличестваПредметаРасчета; 
...
Процедура ЛокальнаяПроверкаСредствамиККТЗавершение(ДанныеОтвета, ПараметрыПроверки) Экспорт
	...
	Иначе
		ВызватьИсключение "Произошла ошибка на ККМ: " + ?(ЗначениеЗаполнено(ДанныеОтвета.ОписаниеОшибки), ДанныеОтвета.ОписаниеОшибки, "Неизвестная ошибка");
