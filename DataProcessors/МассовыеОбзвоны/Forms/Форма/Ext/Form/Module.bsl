...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заявки.Параметры.УстановитьЗначениеПараметра("ДатаСреза", Дата(1,1,1,0,0,0));
...
Процедура ПриОткрытии(Отказ)
	...
	// Данные переменной хранятся в пользовательских настройках, поэтому в ПриСозданииНаСервере значение
	// еще не инициализировано, поэтому устанавливаем после инициализации
	Если ВерхнийПределЗаявок > 0 Тогда
		АктуализироватьЗаявкиПоВерхнемуПределу();
	КонецЕсли;
...
&НаКлиенте
Процедура ИзменитьВерхнийПределЗаявок(Команда)
	Оповещение = Новый ОписаниеОповещения("ИзменитьВерхнийПределЗаявокПродолжить", ЭтотОбъект);
	ПоказатьВводЧисла(Оповещение, ВерхнийПределЗаявок, "Верхний предел заявок в днях", 3, 0);
КонецПроцедуры
...
Процедура НачатьЗвонок(ИмяКоманды)
	...
	Если ИмяКоманды = "ПозвонитьБитфон" Тогда
		доп_БитфонКлиент.ОткрытьБитфонНачатьРазговор(ВыбранныйКлиент);
	ИначеЕсли ИмяКоманды = "ПозвонитьСоСтационарного" Тогда
		доп_АТСКлиент.ОткрытьМониторНачатьРазговор(ВыбранныйКлиент);
	КонецЕсли;
	...
	Если ТипЗнч(ВыбранныйОбъект) = Тип("ДокументСсылка.Заявка") Тогда
		...
		ЗаблокированныеЗаявкиНаДеньСоответствие = ЗаблокироватьВсеЗаявкиКлиентаНаДень(ВыбранныйОбъект);
...
Процедура ПослеЗаписиКомментария(Результат)
	...
	ИначеЕсли ТипЗнч(ЗаблокированныйОбъект) = Тип("ДокументСсылка.доп_СписокОбзвона") Тогда
...
Процедура ИзменитьОбъект(Объект, ПризнакПодтвержденнойЗаявки = Неопределено, Комментарий = Неопределено, Клиент = Неопределено)
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументСсылка.доп_СписокОбзвона") Тогда
		...
		ОбщегоНазначенияБИТКлиентСервер.КонкатенацияСтрок(СтрокаКлиента.Комментарий, Комментарий, Символы.ПС, Истина);
		Попытка
...
&НаСервереБезКонтекста
Функция ЗаблокироватьВсеЗаявкиКлиентаНаДень(ВыбранныйОбъект)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ДокументЗаявка.ДатаНачала МЕЖДУ НАЧАЛОПЕРИОДА(&Дата, ДЕНЬ) И КОНЕЦПЕРИОДА(&Дата, ДЕНЬ)
		|	И ДокументЗаявка.Клиент = &Клиент
		|	И ДокументЗаявка.Ссылка <> &Ссылка";
...
&НаКлиенте
Процедура ИзменитьВерхнийПределЗаявокПродолжить(Результат, ДопПараметры) Экспорт
	
	Если Результат <> Неопределено Тогда
		ВерхнийПределЗаявок = Результат;
		АктуализироватьЗаявкиПоВерхнемуПределу();
	КонецЕсли;
...
&НаКлиенте
Процедура АктуализироватьЗаявкиПоВерхнемуПределу()
	АктуализироватьЗаявкиПоВерхнемуПределуСервер();
	ПодключитьОбработчикОжидания("АктуализироватьЗаявкиПоВерхнемуПределу", 86400/2, Ложь);
КонецПроцедуры

&НаСервере
Процедура АктуализироватьЗаявкиПоВерхнемуПределуСервер()
	
	Если ВерхнийПределЗаявок = 0 Тогда
		ДатаСреза = Дата(1,1,1,0,0,0);
	Иначе
		ДатаСреза = ТекущаяДата() + 86400 * ВерхнийПределЗаявок;
	КонецЕсли;
	
	Заявки.Параметры.УстановитьЗначениеПараметра("ДатаСреза", ДатаСреза);
