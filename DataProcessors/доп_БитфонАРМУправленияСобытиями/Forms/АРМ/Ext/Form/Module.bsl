...
Процедура ДобавитьСобытиеНаПроизвольнуюДату(Команда)
	...
	ОткрытьФорму("Обработка.доп_БитфонАРМУправленияСобытиями.Форма.ДобавлениеСобытияНаПроизвольнуюДату",, ЭтотОбъект, УникальныйИдентификатор,,, ОписаниеОповещенияОЗакрытии);
...
Процедура ЗаполнитьСобытия()
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	доп_СобытияБитФон.Дата <= КОНЕЦПЕРИОДА(&ДатаОкончания, ДЕНЬ)
		|	И доп_СобытияБитФон.Дата >= НАЧАЛОПЕРИОДА(&ДатаНачала, ДЕНЬ)
		|	И (&Пользователь = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
		|			ИЛИ доп_СобытияБитФон.Пользователь = &Пользователь)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Пользователь,
		|	Дата,
...
Процедура ЗаполнитьДатыЗвонков(тзПервыеСобытия, тзПоследниеСобытия, тзСобытия)
	...
	Запрос.Текст = 
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.доп_ИсторияЗвонков КАК доп_ИсторияЗвонков
		|		ПО (доп_ИсторияЗвонков.Пользователь = ПервыеСтроки.Пользователь)
		|			И (доп_ИсторияЗвонков.Дата МЕЖДУ НАЧАЛОПЕРИОДА(ПервыеСтроки.Дата, ДЕНЬ) И КОНЕЦПЕРИОДА(ПервыеСтроки.Дата, ДЕНЬ))
		|			И (доп_ИсторияЗвонков.Дата = ПервыеСтроки.ДатаНачалаЗвонка)
		|
		|СГРУППИРОВАТЬ ПО
		|	ПервыеСтроки.Пользователь,
		|	ПервыеСтроки.Дата,
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.доп_ИсторияЗвонков КАК доп_ИсторияЗвонков
		|		ПО (доп_ИсторияЗвонков.Пользователь = ПоследниеСтроки.Пользователь)
		|			И (доп_ИсторияЗвонков.Дата МЕЖДУ НАЧАЛОПЕРИОДА(ПоследниеСтроки.Дата, ДЕНЬ) И КОНЕЦПЕРИОДА(ПоследниеСтроки.Дата, ДЕНЬ))
		|			И (ДОБАВИТЬКДАТЕ(доп_ИсторияЗвонков.Дата, СЕКУНДА, доп_ИсторияЗвонков.ДлительностьЗвонка) = ПоследниеСтроки.ДатаКонцаЗвонка)
		|
		|СГРУППИРОВАТЬ ПО
		|	ПоследниеСтроки.Пользователь,
...
Процедура ДобавитьНовоеСобытиеВРегистр(Дата, Событие, Пользователь)
	...
	МЗ = РегистрыСведений.доп_СобытияБитФон.СоздатьМенеджерЗаписи();
	МЗ.Событие = Событие;
