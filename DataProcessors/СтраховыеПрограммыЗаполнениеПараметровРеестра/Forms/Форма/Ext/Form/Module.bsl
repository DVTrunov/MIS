...
&НаКлиенте
Процедура СоглашениеСтрахованияПриИзменении(Элемент)
	
	Использование = ЗначениеЗаполнено(СоглашениеСтрахования);
	Если Использование Тогда
		ЗначенияОтбора = ПолучитьЗначенияОтбора(СоглашениеСтрахования);
	КонецЕсли;
	
	// Изменяем отборы
	Для Каждого Отбор Из СписокПродаж.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
		Если ТипЗнч(Отбор) = Тип("ОтборКомпоновкиДанных") Тогда
			Для Каждого ЭлементОтбора Из Отбор.Элементы Цикл
				Если Лев(ЭлементОтбора.Представление, 22) = "СоглашениеСтрахования_" Тогда
					Если Использование Тогда
						ЭлементОтбора.ПравоеЗначение = ЗначенияОтбора.Получить(ЭлементОтбора.Представление);
					КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьЗначенияОтбора(Договор)
	
	Соответствие = Новый Соответствие; 
	
	Соответствие.Вставить("СоглашениеСтрахования_ДатаБольше", Договор.ДатаНачала);
	Соответствие.Вставить("СоглашениеСтрахования_ДатаМеньше", Договор.ДатаОкончания);
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	ВидыПолисовДоговоры.Договор = &Договор";
	
	Запрос.УстановитьПараметр("Договор", Договор);
	МассивВидовПолисов = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	
	Соответствие.Вставить("СоглашениеСтрахования_ВидПолиса", МассивВидовПолисов);
