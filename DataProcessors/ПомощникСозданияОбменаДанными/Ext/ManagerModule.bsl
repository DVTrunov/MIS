...
Процедура ПриПолученииДоступныхНастроекСинхронизацииДанных(ТаблицаНастройки)
	...
	УдалитьНеПоддерживаемыеВариантыНастроек(ТаблицаНастройки);
...
Процедура УдалитьНеПоддерживаемыеВариантыНастроек(ТаблицаНастройки)
	
	НеПоддерживаемыеНастройки = Новый Массив;
	
	Для Каждого СтрокаНастройки Из ТаблицаНастройки Цикл
		
		Если СтрокаНастройки.ИмяПланаОбмена = "ОбменУправлениеТорговлей22Бухгалтерия30"
			ИЛИ СтрокаНастройки.ИдентификаторНастройки = "ОбменЗУПБП" Тогда
			
			НеПоддерживаемыеНастройки.Добавить(СтрокаНастройки);
			
		КонецЕсли;
		
	КонецЦикла;
	
	Для Сч = 1 По НеПоддерживаемыеНастройки.Количество() Цикл
		ТаблицаНастройки.Удалить(НеПоддерживаемыеНастройки[Сч - 1]);
	КонецЦикла;
...
Процедура ПроверитьПодключениеККорреспонденту(Параметры, АдресРезультата) Экспорт
	...
	Если НастройкиПодключения.ВидТранспортаСообщенийОбмена = Перечисления.ВидыТранспортаСообщенийОбмена.COM Тогда
		...
		Если РезультатПроверки.ВерсииИнтерфейса.Найти("3.0.1.1") <> Неопределено
			...
			Если Не ПараметрыКорреспондента.ПланОбменаСуществует Тогда
				РезультатПроверки.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В корреспонденте не найден план обмена ""%1"".'") + Символы.ПС +
					НСтр("ru = 'Убедитесь, что'") + Символы.ПС +
					НСтр("ru = ' - выбран правильный вид приложения для настройки обмена;'") + Символы.ПС +
					НСтр("ru = ' - корректно указано расположение программы, к которой выполняется подключение.'"),
					НастройкиПодключения.ИмяПланаОбмена);
		...
		Иначе
			...
			РезультатПроверки.СообщениеОбОшибке = НСтр("ru = 'Корреспондент не поддерживает версию 3.0.1.1 интерфейса ""ОбменДанными""'") + Символы.ПС +
			НСтр("ru = 'Для настройки подключения обновите конфигурацию корреспондента, или начните настройку из него.'");
	...
	ИначеЕсли НастройкиПодключения.ВидТранспортаСообщенийОбмена = Перечисления.ВидыТранспортаСообщенийОбмена.WS Тогда
		...
		Если РезультатПроверки.ВерсииИнтерфейса.Найти("3.0.1.1") <> Неопределено 
			...
			Если Не ПараметрыКорреспондента.ПланОбменаСуществует Тогда
				РезультатПроверки.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					НСтр("ru = 'Убедитесь, что'") + Символы.ПС +
					НСтр("ru = ' - выбран правильный вид приложения для настройки обмена;'") + Символы.ПС +
					НСтр("ru = ' - указан правильный адрес приложения в Интернете.'"),
					НастройкиПодключения.ИмяПланаОбменаКорреспондента);
		...
		Иначе
			...
			РезультатПроверки.СообщениеОбОшибке = 
				НСтр("ru = 'Корреспондент не поддерживает версию интерфейса 3.0.1.1 или 3.0.2.1 ""ОбменДанными"".'") + Символы.ПС +
				      НСтр("ru = 'Для настройки подключения обновите конфигурацию корреспондента, или начните настройку из него.'");
...
Процедура УдалитьНастройкуСинхронизации(Параметры, АдресРезультата) Экспорт
	...
	Если НастройкиУдаления.УдалитьНастройкуВКорреспонденте Тогда
		...
		Если Не Результат.НастройкаУдаленаВКорреспонденте Тогда
			...
			(НСтр("ru = 'При удалении настройки синхронизации в программе ""%1"" возникла ошибка: %2.'"),Строка(НастройкиУдаления.УзелОбмена),
				Результат.СообщениеОбОшибкеВКорреспонденте) + Символы.ПС +
				(НСтр("ru = 'Попробуйте выполнить удаление позже, или снимите флажок ""Удалить настройку также в программе..."".'"));
...
Процедура СоздатьОбновитьУзлыПланаОбмена(НастройкиПодключения)
	...
	Если ОбменДаннымиПовтИсп.ЭтоПланОбменаРаспределеннойИнформационнойБазы(НастройкиПодключения.ИмяПланаОбмена)
		...
		Если ГлавныйУзел = Неопределено Тогда
			...
			ВызватьИсключение НСтр("ru = 'Главный узел для текущей информационной базы не определен.'") + Символы.ПС +
							НСтр("ru = 'Возможно, информационная база не является подчиненным узлом в РИБ.'");
		КонецЕсли;
