...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидимостьДоступность(ЭтотОбъект, ОбластьПоискаДублей);
...
Процедура ВидимостьДоступность(Форма, ОбластьПоискаДублей)
	...
	// +ОЛИМП
	Элементы.доп_ОбъединитьКлиентов.Видимость = ОбластьПоискаДублей = "Справочник.Клиенты"; 
	// -ОЛИМП
...
Функция ПарыЗаменДублей()
	...
	Для Каждого Родитель Из ДеревоДублей.Строки Цикл
		Если ЗначениеЗаполнено(Родитель.Пометка) Тогда // +ОЛИМП
...
&НаКлиенте
Процедура доп_ВыбратьВсеГруппыЗамены(Команда)
...
&НаКлиенте
Процедура доп_СнятьВсеГруппыЗамены(Команда)
...
&НаКлиенте
Процедура доп_ОбъединитьКлиентов(Команда)
	
	Элемент = Элементы.НайденныеДубли.ТекущиеДанные;

	Оригинал = Неопределено;
	Дубль = Неопределено;
			
	Если Элемент.Ссылка <> Неопределено Тогда
		Если Элемент.Основной Тогда
			Оригинал = Элемент.Ссылка;
		Иначе
			Дубль = Элемент.Ссылка;	
		КонецЕсли;
	...
	СтрокиДублей = Родитель.ПолучитьЭлементы();
	Для Каждого СтрокаДубля Из СтрокиДублей Цикл
		Если Дубль <> Неопределено И Оригинал <> Неопределено Тогда
			Прервать;
		Иначе
			Если СтрокаДубля.Основной Тогда
				Если Оригинал = Неопределено Тогда
					Оригинал = СтрокаДубля.Ссылка;	
				КонецЕсли;
			Иначе
				Если Дубль = Неопределено И СтрокаДубля.Пометка <> 0 Тогда
					Дубль = СтрокаДубля.Ссылка;	
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;	
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(Дубль) И ЗначениеЗаполнено(Оригинал) Тогда
		Дубль = СтрокаДубля.Ссылка;	
	КонецЕсли;		
	
	ОткрытьФорму("Обработка.ОбъединениеДублирующихсяКлиентов.Форма.Форма", Новый Структура("Оригинал, Дубль", Оригинал, Дубль));
