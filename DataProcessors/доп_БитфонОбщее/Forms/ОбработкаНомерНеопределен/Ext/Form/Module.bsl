...
Процедура СоздатьКонтрагента(Команда)
	//+Переопределенное
	//доп_ТелефонияКлиентПереопределяемый.СоздатьНовогоКонтрагента(Параметры.НеизвестныйНомер);
	доп_ТелефонияКлиентПереопределяемый.СоздатьНовогоКонтрагента(Параметры.НеизвестныйНомер, ЭтаФорма.Событие);
	//-Переопределенное
	Закрыть();
КонецПроцедуры
...
Процедура ДобавитьНомер(Команда)
	//+Переопределенное
	//доп_ТелефонияКлиентПереопределяемый.ВыбратьКонтрагентаЗадатьНомер(Параметры.НеизвестныйНомер);
	доп_ТелефонияКлиентПереопределяемый.ВыбратьКонтрагентаЗадатьНомер(Параметры.НеизвестныйНомер, ЭтаФорма.Событие);
	//-Переопределенное
	Закрыть();
КонецПроцедуры
