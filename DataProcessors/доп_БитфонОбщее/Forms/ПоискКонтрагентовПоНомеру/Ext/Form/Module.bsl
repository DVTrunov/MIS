...
Процедура НайтиКонтрагентовНаСервере()
	стрНомер = доп_ТелефонияКлиентСервер.СократитьНомерДляПоиска(доп_ТелефонияКлиентСервер.ОчиститьНомерТолькоЦифры(НомерТелефона));
	таблКонтраг = доп_ТелефонияСерверПереопределяемый.НайтиКонтрагентовПоНомеруВКонтактах(стрНомер, Ложь);
