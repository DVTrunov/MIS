...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	доп_ТелефонияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(НомераБитАТС, "Владелец", Параметры.СсылкаАТС);
...
Процедура ЗаполнитьСписокКонтактныхЛицКонтрагента(КонтрагентСсылка)
	...
	массКонтактныеЛицаКонтрагента = доп_ТелефонияСерверПереопределяемый.НайтиКонтактныеЛицаКонтрагента(КонтрагентСсылка);
...
Процедура ЗаполнитьСписокТелефоновКонтрагента(КонтрагентСсылка)
	...
	КонецЕсли;
	Телефоны.Очистить();
	массивНомераТелефонов = доп_ТелефонияСерверПереопределяемый.НайтиНомераКонтрагента(КонтрагентСсылка);
...
Процедура ЗаполнитьСписокТелефоновКонтактногоЛица(КонтактноеЛицоСсылка)
	...
	КонецЕсли;
	Телефоны.Очистить();
	массивНомераТелефонов = доп_ТелефонияСерверПереопределяемый.НайтиНомераКонтактногоЛица(КонтактноеЛицоСсылка);
	Для Каждого тел Из массивНомераТелефонов Цикл
		строкаТелефон = Телефоны.Добавить();
		строкаТелефон.Номер = тел;
	КонецЦикла;
