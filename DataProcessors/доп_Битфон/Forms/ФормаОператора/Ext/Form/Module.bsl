...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	Если Источник = "1С_Софтфон" Тогда	
		...
		ИначеЕсли	Событие = "Софтфон_Ошибка" Тогда        //---Оповещение об ошибке
			доп_ТелефонияКлиент.ВывестиСообщение("Ошибка Софтфон: " + Данные);
		КонецЕсли;
	КонецЕсли;	
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "БитФонОператор_СофтфонПодключен" Тогда
		...
		Если СофтФон <> Неопределено Тогда
			доп_БитфонКлиент.ПроверкаРазрешенногоАдреса(СофтФон, "БитФонОператор_ПроверкаАдресаПодключения");
		КонецЕсли;
	ИначеЕсли ИмяСобытия = "БитФонОператор_ПроверкаАдресаПодключения" Тогда
		Если Параметр.Результат = КодВозвратаДиалога.Да Тогда
			...
			доп_БитфонКлиент.ИнициализироватьСофтфонРегистрацияНаАТС(СофтФон);
			//+Переопределенное
...
Процедура ОбработчикСобытияСостояниеЗвонка(Данные)
	...
	Если Канал = текущийКанал Тогда
		...
		ИначеЕсли Состояние = "ЗвонокПрерван" Тогда 	//Обработчик завершения разговора
			...
			доп_ТелефонияСервер.ДобавитьВИсториюЗвонков(СвойНомер, Ложь, стрНомерДляИстории, Контрагент,
...
Процедура Подключение()
	...
	Если ЗначениеЗаполнено(СвойНомер) Тогда
		Заголовок = "ОЛИМП.Phone Оператор - " + СвойНомер;
	КонецЕсли;
	доп_БитфонКлиент.ПодключитьСофтфон("БитФонОператор_СофтфонПодключен");
...
Функция НачатьЗапись()
	...
	Если доп_БитфонСервер.ПолучитьФлагПолучатьЗаписиРазговоровСБИТАТС() Тогда
		стрНомерДляЗаписи = доп_ТелефонияКлиентСервер.СократитьНомерДляПоиска(Номер);
...
Процедура ПолучитьНомерОбзвонаНачатьРазговор()
	Если Не ИдетРазговор Тогда
		ОчиститьИнформацию();
		получен = доп_БитфонСервер.ПолучитьНомерДляОператора(Номер, Контрагент, КонтактноеЛицо);
		Если НЕ получен Тогда
			доп_ТелефонияКлиент.ВывестиСообщение("Не удалось получить номер из списка обзвона");
			Возврат;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Номер) Тогда
			доп_ТелефонияКлиент.ВывестиСообщение("Номер из списка обзвона не заполнен");
			Возврат;
		КонецЕсли;
		Номер = доп_БитфонКлиент.СформироватьНомерСПрефиксами(Номер);
...
Процедура ПодключитьВКМонитораБИТАТС()
	...
	//
	Если доп_БитфонСервер.ПолучитьФлагПолучатьЗаписиРазговоровСБИТАТС() Тогда
		доп_АТСКлиент.ПодключениеКомпонентыУправлениеАТС("БитФонОператор_ИнтегрМониторБИТАТС_ПодключениеВнешнейКомпоненты");
	КонецЕсли;
КонецПроцедуры
