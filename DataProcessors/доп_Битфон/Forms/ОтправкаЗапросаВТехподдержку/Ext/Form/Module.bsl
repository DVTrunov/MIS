...
Процедура ОтправитьЗапрос(Команда)
	...
	стрПрофиль = доп_ТелефонияКлиент.ПолучитьПапкуПриложенийПользователя(стрЗаголовокОшибки);
	Если НЕ ЗначениеЗаполнено(стрПрофиль) Тогда
		// вывод сообщения об ошибке уже есть в функции получения папки
		Возврат;
	КонецЕсли;
	стрЛогин = доп_БитфонСервер.ПолучитьПараметрыСоединения().Логин;
	Если НЕ ЗначениеЗаполнено(стрЛогин) Тогда
		доп_ТелефонияКлиент.ВывестиСообщение(стрЗаголовокОшибки + "Не задан логин подключения");
		Возврат;
	КонецЕсли;
	...
	доп_БитфонСервер.ОтправитьПочтовоеСообщениеВТехподдержку(стрТемаПисьма, стрТекстПисьма, стрФайлАрхива, данныеАрхива, сообщение);
	сообщение.Сообщить();
