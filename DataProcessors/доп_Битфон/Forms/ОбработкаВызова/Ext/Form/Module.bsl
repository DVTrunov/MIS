...
Процедура ОткрытьКарточкуКлиента(Команда)
	Клиент = доп_ТелефонияКлиентПереопределяемый.ОткрытьКарточкуКонтрагента(Абонент, НабранныйНомер);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	Если доп_фрмКарточкаКлиента <> Неопределено Тогда
		Попытка
			Если доп_фрмКарточкаКлиента.Открыта() Тогда
				доп_фрмКарточкаКлиента.Закрыть();
			КонецЕсли; 
		Исключение
		КонецПопытки;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "ОткрытаФормаНовогоКлиентаБИТФОН" Тогда
		доп_фрмКарточкаКлиента = Параметр;
	КонецЕсли;
КонецПроцедуры
