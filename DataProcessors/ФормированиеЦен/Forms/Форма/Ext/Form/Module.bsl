...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если (ТипЗнч(ВыбранноеЗначение) = Тип("Массив")) Тогда
		Для Каждого Номенклатура Из ВыбранноеЗначение Цикл
			...
			НоваяСтрока.ЕдиницаИзмерения = ЕдиницаИзмеренияНоменклатурыПоУмолчанию(НоваяСтрока.Номенклатура);
		КонецЦикла;
	КонецЕсли;
...
Процедура НоменклатураИлиХарактеристикаПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено
		И Не ЗначениеЗаполнено(ТекущиеДанные.ЕдиницаИзмерения)
		И ЗначениеЗаполнено(ТекущиеДанные.Номенклатура)
	Тогда
		ТекущиеДанные.ЕдиницаИзмерения = ЕдиницаИзмеренияНоменклатурыПоУмолчанию(ТекущиеДанные.Номенклатура);
		мТекущаяЕдиница = ТекущиеДанные.ЕдиницаИзмерения;
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ЕдиницаИзмеренияНоменклатурыПоУмолчанию(Номенклатура)
	
	Единицы = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "ЕдиницаХраненияОстатков, ЕдиницаТоваров");
	Если ЗначениеЗаполнено(Единицы.ЕдиницаТоваров) Тогда
		Единица = Единицы.ЕдиницаТоваров;
	Иначе
		Единица = Единицы.ЕдиницаХраненияОстатков;
	КонецЕсли;
...
Процедура КомандаИзменитьСтроку(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		ТекущийЭлемент = Элементы.ТоварыНоменклатура;
		Элементы.Товары.ИзменитьСтроку();
		ОткрытьФорму("Справочник.Номенклатура.ФормаВыбора", Новый Структура("ТекущаяСтрока", ТекущиеДанные.Номенклатура), Элементы.ТоварыНоменклатура);
	КонецЕсли;
...
Процедура КомандаДобавитьСтроки(Команда)
	...
	ОткрытьФорму("Справочник.Номенклатура.ФормаВыбора", Новый Структура("МножественныйВыбор", Истина), ЭтотОбъект);
