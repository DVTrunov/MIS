...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(ЗначенияПолей) Тогда
		...
		Если ЗначениеЗаполнено(Параметры.Объект) Тогда
			доп_КраткоеПредставление = ХранилищеПользовательскихНастроекОтчетов.Загрузить("ФормаЗаписиАдресаКраткоеПредставлениеАдреса", "КраткоеПредставление" + Параметры.Объект.Метаданные().Имя);
		Иначе
			доп_КраткоеПредставление = Истина;	
		КонецЕсли;
		//-ОЛИМП
	ИначеЕсли УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(ЗначенияПолей) Тогда
		...
		Если ДанныеАдреса.Свойство("КраткоеПредставление") Тогда
			доп_КраткоеПредставление = ДанныеАдреса.КраткоеПредставление;
		КонецЕсли;
		//-ОЛИМП
	Иначе
		...
		Если ТипЗнч(XDTOКонтактная.Состав.Состав) = Тип("ОбъектXDTO") Тогда
			доп_КраткоеПредставление = ОбщегоНазначенияКлиентСервер.СвойствоОбъекта(XDTOКонтактная.Состав.Состав, "КраткоеПредставление", Ложь);
		КонецЕсли;
		//-ОЛИМП
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	//+ОЛИМП
	Если Не ЗначениеЗаполнено(Улица) И доп_КраткоеПредставление Тогда
...
Процедура ПодтвердитьИЗакрыть(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Модифицированность Тогда // При немодифицированности работает как "отмена".
		...
		//+ОЛИМП
		Контекст.Вставить("доп_КраткоеПредставление", доп_КраткоеПредставление);
		//-ОЛИМП
...
#КонецЕсли
	...
	СохранитьНастройкиФормы(доп_КраткоеПредставление, Параметры.Объект);
	// -ОЛИМП
...
Функция РезультатВыбора(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	Если ТипЗнч(ДанныеВыбора) = Тип("Структура") Тогда
		...
		//+ОЛИМП
		ДанныеВыбора.Вставить("КраткоеПредставление", Контекст.доп_КраткоеПредставление);
		//-ОЛИМП
	...
	Результат.ДанныеВыбора.Вставить("ВВидеГиперссылки", Истина);//+ОЛИМП Не используется // Контекст.ВидКонтактнойИнформации.ВидРедактирования = "Диалог");
...
Процедура ОчиститьАдресКлиент()
	...
	//+ОЛИМП
	НаселенныйПунктДетально.Вставить("КраткоеПредставление", доп_КраткоеПредставление);
	//-ОЛИМП
...
Процедура ЗаполнитьПредопределенныеВариантыАдреса()
	...
	Если ПустаяСтрока(Помещение) И ПустаяСтрока(ТипПомещения) Тогда
		//+ОЛИМП Установка типа помещения по-умолчанию по типу владельца
...
Процедура ПослеВыбораУлицы(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		КонецЕсли;
	Иначе
		//+ОЛИМП убрали, т.к. бред какой-то.
...
#Область доп_ДополнительныеПроцедуры //+ОЛИМП


&НаКлиенте
Процедура доп_КраткоеПредставлениеПриИзменении(Элемент)
