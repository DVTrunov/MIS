...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОбщиеНастройки.Доступность = РольДоступна("доп_сл_Администратор") ИЛИ
	                                      РольДоступна("ПолныеПрава") ИЛИ
										  РольДоступна("доп_ПолныеПрава") ИЛИ
...
Процедура ЛокализироватьИнтерфейс();
	...
	ЛокализацияСоответствие = доп_сл_общий.ПолучитьДанныеЛокализации();
	...
	Для Каждого ЭлементЗн Из ЛокализацияСоответствие Цикл
		...
		Если Лев(ЭлементЗн.Ключ, 36) = "сл_КлиентЛицензирования_Интерфейс_01" ИЛИ
			...
			Исключение
				...
				Сообщить( доп_сл_общий.ПолучитьСтрокуСообщения("сл_КлиентЛицензирования_ЛокализацияСодержитОшибки", ПараметрыСообщения) );							
...
Процедура ВывестиСообщениеВероятноНеПравильныйПорт(ПолеИсточник);
	...
	ЛокализированноеСообщение = доп_сл_общий.ПолучитьСтрокуСообщения("сл_КлиентЛицензирования_НеПравильныПорт");
...
Процедура ОбновитьСостояние(Команда)
	...
	Объект.ОсновнаяНадпись = Объект.ОсновнаяНадпись = доп_сл_общий.ПолучитьСтрокуСообщения("сл_сервер_ПожалуйстаПодождите");
...
Процедура ОткрытьФормуСписка(Команда)
	...
	СовместимостьФорм_8_2_13 = доп_сл_сервер.ИспользоватьРежимСовместимостиФормСПлатформой_8_2_13();
	...
	Если СовместимостьФорм_8_2_13 Тогда
		...
		ИмяФормыОбработки = "Обработка.доп_сл_КлиентЛицензирования.Форма.СписокПользователей_упр_8_2_13";
