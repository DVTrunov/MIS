...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущийЯзыкПользователя = доп_сл_сервер_ассистент.ПолучитьЯзыкКлиентаЛицензирования();
	доп_сл_сервер.ОбновитьЗначенияРеквизитов(Объект);

	Элементы.ОбщиеНастройки.Доступность = (Метаданные.Роли.Найти("доп_сл_Администратор") <> Неопределено И РольДоступна("доп_сл_Администратор")) ИЛИ
	                                      (Метаданные.Роли.Найти("ПолныеПрава") <> Неопределено И РольДоступна("ПолныеПрава")) ИЛИ
										  (Метаданные.Роли.Найти("доп_ПолныеПрава") <> Неопределено И РольДоступна("доп_ПолныеПрава")) ИЛИ
...
Процедура ЛокализироватьИнтерфейсИзСтруктуры(ЛокализацияФормы)
	...
	Для Каждого ЭлементЗн Из ЛокализацияФормы Цикл
			...
			Исключение
				...
				Сообщить( доп_сл_общий.ПолучитьСтрокуСообщения("ЛокализацияСодержитОшибки", ПараметрыСообщения, ТекущийЯзыкПользователя) );							
...
Процедура ЛокализироватьИнтерфейс()
	...
	ЛокализироватьИнтерфейсИзСтруктуры(доп_сл_общий.ПолучитьДанныеЛокализации(ТекущийЯзыкПользователя).доп_сл_КлиентЛицензирования.Форма);
	ЛокализироватьИнтерфейсИзСтруктуры(доп_сл_общий.ПолучитьДанныеЛокализации(ТекущийЯзыкПользователя).доп_сл_КлиентЛицензирования.Форма_8_3_6);                                              
...
Процедура ОбновитьДанныеФормыСервер(СтруктураОбработки)
	...
	доп_сл_сервер.ОбновитьЗначенияРеквизитов(СтруктураОбработки);
...
Процедура ПередатьНастройкиНаСервер()
	...
	доп_сл_сервер.СохранитьНастройкиВОбъект(Объект);
...
Процедура ВывестиСообщениеВероятноНеПравильныйПорт(ПолеИсточник)
	...
	ЛокализированноеСообщение = доп_сл_общий.ПолучитьСтрокуСообщения("НеПравильныйПорт", Неопределено, ТекущийЯзыкПользователя);	
	доп_сл_общий.ВывестиСообщение(ЛокализированноеСообщение, ПолеИсточник);	
...
Процедура Подключаемый_ОбновитьСостояниеДействие()
	...
	доп_сл_клиент.ПередОбновлениемСостояния();
	доп_сл_сервер.ОтключитьсяОтСервера();
	доп_сл_сервер.ПолучитьЛицензиюДляТекущегоПродукта();	
	доп_сл_клиент.ПриОбновленииСостояния();	
...
Процедура ДекорацияИзменитьВерсиюПродуктаНажатие(Элемент)
	...
	доп_сл_клиент.ПриОткрытииФормыВыбораВерсииПродукта();
...
Процедура ОбновитьСостояние(Команда)
	...
	доп_сл_сервер.ОжиданиеПереподключения();
	
	Объект.ОсновнаяНадпись = доп_сл_общий.ПолучитьСтрокуСообщения("ПожалуйстаПодождите", Неопределено, ТекущийЯзыкПользователя);	
	Объект.СостояниеАктивации = доп_сл_общий.ПолучитьСтрокуСообщения("НеАктивирован", Неопределено, ТекущийЯзыкПользователя);
...
Процедура ОткрытьФормуСписка(Команда)
	...
	Если СовместимостьФорм_8_3_6 Тогда
		...
		ИмяФормыОбработки = "Обработка.доп_сл_КлиентЛицензирования.Форма.СписокПользователей_упр_8_3_6";
		
	Иначе
		
		ИмяФормыОбработки = "Обработка.доп_сл_КлиентЛицензирования.Форма.СписокПользователей_упр";
