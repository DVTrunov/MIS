...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АвтоПробитиеЧека = Параметры.Пробить;
	...
	омг_WebKassa.НастроитьПодключаемоеОборудование(ЭтаФорма);
	...
	Элементы.ПолучательИИН.Видимость = Истина;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПараметрыОперацииФискализацииЧека.Свойство("ТипРасчета") И ПараметрыОперацииФискализацииЧека.ТипРасчета = ПредопределенноеЗначение("Перечисление.ТипыРасчетаДенежнымиСредствами.ВозвратДенежныхСредств") Тогда 
		
		Элементы.ГруппаДанныеЧекаПродажи.Видимость = Истина;
		
		ЧекПродажи = ПараметрыОперацииФискализацииЧека.ЧекПродажи;
		ЗаполнитьДанныеПервичногоЧека();
		
	Иначе
		Элементы.ГруппаДанныеЧекаПродажи.Видимость = Ложь;
	КонецЕсли;

	ПодготовитьПараметрыОперацииФискализацииЧекаКПередаче();
	
		
	//Если АвтоПробитиеЧека Тогда
		ПробитьЧекАвто();		
	//КонецЕсли;	
...
Процедура ПодготовитьПараметрыОперацииФискализацииЧекаКПередаче()
	...
	//Параметры для Чека возврата  
	ПараметрыОперацииФискализацииЧека.ДанныеПервичногоЧека.НомерЧека					= НомерПервичногоЧека;
	ПараметрыОперацииФискализацииЧека.ДанныеПервичногоЧека.ДатаВремяЧека				= ДатаВремяПервичногоЧека;
	ПараметрыОперацииФискализацииЧека.ДанныеПервичногоЧека.РегистрационныйНомерКассы	= РНКПервичногоЧека;
	ПараметрыОперацииФискализацииЧека.ДанныеПервичногоЧека.ОбщаяСуммаЧека				= ОбщаяСуммаПервичногоЧека;
	ПараметрыОперацииФискализацииЧека.ДанныеПервичногоЧека.АвтономныйРежимРаботыКассы 	= ПризнакАвтономностиПервичногоЧека;

	/////////////////////////////////////////////
	// ОМГ
	Если не ЗначениеЗаполнено(ПараметрыОперацииФискализацииЧека.ДанныеПервичногоЧека.НомерЧека) и ЗначениеЗаполнено(ПараметрыОперацииФискализацииЧека.ЧекПродажи) Тогда 
		
		ВыборкаПервичногоЧека = омг_WebKassa.ВернутьДанныеПервичногоЧека(ПараметрыОперацииФискализацииЧека.ЧекПродажи);
		
		Если не ВыборкаПервичногоЧека = Неопределено Тогда
			
		ЗаполнитьЗначенияСвойств(ПараметрыОперацииФискализацииЧека.ДанныеПервичногоЧека,ВыборкаПервичногоЧека)
...
Процедура ПечатьЧека_Завершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////
		// ОМГ       
		ПараметрыФ = Новый Структура("Ключ", ЭтотОбъект.ДокументСсылка);
		
		Если ТипЗнч(ЭтотОбъект.ДокументСсылка) = Тип("ДокументСсылка.ОплатаПлатежнойКартой") Тогда
		Форма = ПолучитьФорму("Документ.ОплатаПлатежнойКартой.Форма.ФормаДокументаУпр", ПараметрыФ);	
		ИначеЕсли ТипЗнч(ЭтотОбъект.ДокументСсылка) = Тип("ДокументСсылка.ВозвратПоПлатежнойКарте") Тогда 
		Форма = ПолучитьФорму("Документ.ВозвратПоПлатежнойКарте.Форма.ФормаДокументаУпр", ПараметрыФ);  
		ИначеЕсли ТипЗнч(ЭтотОбъект.ДокументСсылка) = Тип("ДокументСсылка.ОказаниеУслуг") Тогда
		Форма = ПолучитьФорму("Документ.ОказаниеУслуг.Форма.ФормаДокумента", ПараметрыФ);
		ИначеЕсли ТипЗнч(ЭтотОбъект.ДокументСсылка) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
		Форма = ПолучитьФорму("Документ.РасходныйКассовыйОрдер.Форма.ФормаДокументаУпр", ПараметрыФ);
		ИначеЕсли ТипЗнч(ЭтотОбъект.ДокументСсылка) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда
		Форма = ПолучитьФорму("Документ.ПриходныйКассовыйОрдер.Форма.ФормаДокументаУпр", ПараметрыФ);
		КонецЕсли;
 
        
        Форма.НапечататьЧекКлиентЗавершение(РезультатВыполнения, ДополнительныеПараметры); 
		омг_WebKassa.ЗаписатьДанныеОперацииПоЧеку(ЭтотОбъект.ДокументСсылка,РезультатВыполнения.ВыходныеПараметры[4],РезультатВыполнения.ИдентификаторУстройства,РезультатВыполнения.ОписаниеОшибки,РезультатВыполнения.ВыходныеПараметры[5])
		// ОМГ
		////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////
	// ОМГ		
	Если РезультатВыполнения.ОписаниеОшибки = "Кассовая смена не открыта или истекла." Тогда
	
		
	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаОшибкаПечати",
      ЭтотОбъект);	
    ...
    ПоказатьВопрос(Оповещение,
        ...
        "Кассовая смена не открыта или истекла!" // (необ.) заголовок
    );    
	КонецЕсли;
	// ОМГ
	////////////////////////////////////////////////////////////////////////////////////////////////////
...
Процедура ОбработатьШтрихкодСервер(ШтрихКодКоличество, НедопустимыеСимволы = Ложь)
	...
	РаботаСМаркировкойДоступна = Ложь;	
	КодМаркировки = Ложь;
	КодМаркировкиBase64 = "";
	ШтрихкодEAN	  = "";
...
Процедура ПослеЗакрытияВопросаОшибкаПечати(Результат, Параметры) Экспорт
...
&НаКлиенте
Процедура ДатаВремяПервичногоЧекаПриИзменении(Элемент)
...
&НаСервере
Процедура ЗаполнитьДанныеПервичногоЧека()
	
	Если ЗначениеЗаполнено(ЧекПродажи) Тогда 
		
		СтруктураПервичногоДокумента = ПодключаемоеОборудованиеБКВызовСервера.ПолучитьФискальнуюОперациюРК(ЧекПродажи);
		
		Если Не СтруктураПервичногоДокумента = Неопределено Тогда 
			
			НомерПервичногоЧека 				= СтруктураПервичногоДокумента.НомерЧека;
			ДатаВремяПервичногоЧека 			= СтруктураПервичногоДокумента.ДатаВремяЧека;
			РНКПервичногоЧека 					= СтруктураПервичногоДокумента.РегистрационныйНомерКассы;
			ОбщаяСуммаПервичногоЧека 			= СтруктураПервичногоДокумента.ОбщаяСуммаЧека;
			ПризнакАвтономностиПервичногоЧека 	= СтруктураПервичногоДокумента.АвтономныйРежимРаботыКассы;
			
		Иначе 
			
			НомерПервичногоЧека 				= "";
			ДатаВремяПервичногоЧека 			= Дата("00010101");
			РНКПервичногоЧека 					= "";
			ОбщаяСуммаПервичногоЧека 			= 0;
			ПризнакАвтономностиПервичногоЧека 	= Ложь;
...
&НаКлиенте
Процедура НомерПервичногоЧекаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ОбщаяСуммаПервичногоЧекаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПризнакАвтономностиПервичногоЧекаПриИзменении(Элемент)
...
&НаКлиенте
Процедура РНКПервичногоЧекаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ЧекПродажиПриИзменении(Элемент)
