...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		РаботаСФормамиКлиентСервер.УстановитьПолеЭлементаФормы(Элементы, "СтраницаФакторыРиска", "Видимость", Ложь);
	КонецЕсли;
	
	// wkhtmltopdf
	ПараметрыЭлемента = Новый Структура("ПоложениеВКоманднойПанели", ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю);
	УниверсальныеМеханизмыСервер.НастроитьПараметрыWkhtmltopdfПриСозданииНаСервере(ЭтотОбъект, Элементы.КомандыФормы, ПараметрыЭлемента);
...
Процедура ПриОткрытии(Отказ)
	...
	НастроитьСостояниеФормы(); 
		...
		//Добавлено Карсаков
	 ЗаполнитьЛабораторныеИсследования();
...
Функция ПолучитьПриемыПоКлиенту(ЭтотКлиент)
	...
	Запрос.Текст = 
		...
		|				МИНИМУМ(ВерсииШаблоновHTML.Дата) КАК ДатаВерсии
		|			ИЗ
		|				ШаблоныПриемов КАК ШаблоныПриемов
		...
		Если ОписаниеРаздела.ТипРаздела = ПредопределенноеЗначение("Перечисление.ТипыРазделовМедицинскихКарт.ВыводШаблонов") Тогда
			...
			Для Каждого СтрокаДокумента Из СтрокиДокументов Цикл
				...
				Тогда
					...
					ТекстОсмотра = РаботаСШаблонамиHTMLКлиентСервер.ПолучитьТекстHTMLШапкаШаблонаПриема(СтрокаДокумента.ДатаПриема, СтрокаДокумента.Сотрудник, СтрокаДокумента.ШаблонПриема, Ложь)
...
Процедура ЗаполнитьМаршрутыПациента()
	МаршрутыПациента.Загрузить(
		МаршрутыПациентовСервер.МаршрутыПациентаТаблица(Клиент));
КонецПроцедуры
...
Процедура МаршрутыПациентаВыбор(Элемент, Строка, Поле, СтандартнаяОбработка)
	...
	ПостроитьГантВПолеОсмотр(ТекущиеДанные.ПервичныйПрием, ТекущиеДанные.СтандартЛечения);
КонецПроцедуры

&НаСервере
Процедура ПостроитьГантВПолеОсмотр(ПервичныйПрием, СтандартЛечения)
	Данные = МаршрутыПациентовСервер.ДанныеМаршрута(ПервичныйПрием, СтандартЛечения);
	ПолеОсмотр = Отчеты.МаршрутЛеченияПациента.ПостроитьHTML(Данные);
КонецПроцедуры
...
Функция СформироватьПолныйТекстПриемов(ТекстHTML, ТекстHTMLПриемов, ДанныеИзображений, ИдентификаторФормы)
	...
	РаботаСHTMLКлиентСервер.ПеренестиСтилиИзТелаВHeadДокумента(ДокументHTML, Истина);
	РаботаСШаблонамиHTMLКлиентСервер.УстановитьСтильСкрываемостиПриПечатиВЗаголовокДокумента(ДокументHTML);
...
Функция HTMLПредставлениеМедДокументов(ТаблицаДокументов, УникальныйИдентификатор)
	...
	Для Каждого СтрокаШапкаПриема Из ТаблицаДокументов Цикл
		...
		Если СтрокаПриема = Неопределено Тогда
			...
			ТекстШаблона = Строка(ДанныеПриемов.ПредставлениеИзШапкиДокументов.Получить(СтрокаШапкаПриема.Ссылка));
			ТекстШаблона = РаботаСDOMКлиентСервер.ПолучитьКодТелаHTMLИзКодаДокумента(ТекстШаблона);
			ТекстШапки = РаботаСШаблонамиHTMLКлиентСервер.ПолучитьТекстHTMLШапкаШаблонаПриема(СтрокаШапкаПриема.Дата, СтрокаШапкаПриема.Врач, "", Ложь);
			
			ТекстОсмотра = ТекстШапки + ТекстШаблона;
		Иначе
			...
			Для Каждого СтрокаШаблона Из СтрокаПриема.Строки Цикл
				...
				ТекстыОсмотров.Добавить(РаботаСШаблонамиHTMLКлиентСервер.ПолучитьТекстHTMLШапкаШаблонаПриема(СтрокаШаблона.Дата, СтрокаШаблона.Врач, СтрокаШаблона.ШаблонПриема, Ложь));
				ТекстыОсмотров.Добавить(РаботаСDOMКлиентСервер.ПолучитьКодТелаHTMLИзКодаДокумента(ТекстШаблона));
			КонецЦикла;
			
			ТекстОсмотра = СтрСоединить(ТекстыОсмотров, Символы.ПС);
		КонецЕсли;
...
Функция ПолучитьТаблицыРисунковПриемов(Приемы, УникальныйИдентификатор)
	...
	Пока ВыборкаПриемов.Следующий() Цикл
		...
		Пока Рисунки.Следующий() Цикл
			ДвоичныеДанныеФайла = доп_РаботаСФайлами.ПолучитьДвоичныеДанныеФайлаРедактируемогоЭлемента(Рисунки.ПутьКФайлу, Рисунки.Рисунок);
...
Функция ПолучитьТаблицыДобавленныхИзображенийПриемов(Приемы, УникальныйИдентификатор)
	...
	Пока ВыборкаПриемов.Следующий() Цикл
		...
		Пока ДобавленныеИзображения.Следующий() Цикл
			ДвоичныеДанныеФайла = доп_РаботаСФайлами.ПолучитьДвоичныеДанныеФайлаРедактируемогоЭлемента(ДобавленныеИзображения.ПутьКФайлу, ДобавленныеИзображения.Изображение);
			Если ДвоичныеДанныеФайла = Неопределено Тогда
				Продолжить;
			КонецЕсли;
...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	// Добавлено Макеев (
	Если ТекущаяСтраница = Элементы.СтраницаЛабораторныеИсследования Тогда
		ЛабораторныеИсследованияПриАктивизацииСтроки(Неопределено);
	КонецЕсли;
	// )
КонецПроцедуры
...
//Добавлено Карсаков
&НаСервере
Функция ПолучитьЛабораторныеИсследованияНаСервере(Клиент, МедицинскаяКарта)
    Запрос = Новый Запрос;
    Запрос.Текст = 
     "ВЫБРАТЬ
     ...
     |	Бланки.НаименованиеИсследованияЛИС КАК НаименованиеИсследованияЛИС,
     |	Бланки.Файл КАК Файл
     |ИЗ
     |	РегистрСведений.омг_БланкиРезультатовИсследованийЛИС КАК Бланки
     |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.омг_ЗаявкаНаЛабораторноеИсследованиеВЛИС КАК ДокЗаявка
     |		ПО Бланки.Заявка = ДокЗаявка.Ссылка
     |ГДЕ
     |	ДокЗаявка.Пациент = &Пациент";
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Строка.Вставить("НаименованиеИсследованияЛИС", Выборка.НаименованиеИсследованияЛИС);
        Строка.Вставить("Файл", Выборка.Файл);
...
&НаКлиенте
Процедура ЗаполнитьЛабораторныеИсследования()
    ЛабораторныеИсследования.Очистить();
    
    Если ЗначениеЗаполнено(Клиент) И ЗначениеЗаполнено(МедицинскаяКарта) Тогда
		Данные = ПолучитьЛабораторныеИсследованияНаСервере(Клиент, МедицинскаяКарта);
        Для Каждого СтрокаДанных Из Данные Цикл
            НоваяСтрока = ЛабораторныеИсследования.Добавить();
            НоваяСтрока.Дата = СтрокаДанных.Дата;
          
            НоваяСтрока.Исследование = СтрокаДанных.НаименованиеИсследованияЛИС;
            НоваяСтрока.Результат = СтрокаДанных.Файл;
            НоваяСтрока.Ссылка = СтрокаДанных.Ссылка;
        КонецЦикла;
    КонецЕсли;
        Элементы.ЛабораторныеИсследования.Обновить();
КонецПроцедуры

&НаКлиенте
Процедура ЛабораторныеИсследованияПриАктивизацииСтроки(Элемент)
	Если Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаЛабораторныеИсследования" Тогда
	ТекДанные = Элементы.ЛабораторныеИсследования.ТекущиеДанные;
	Если ТекДанные = Неопределено Тогда
		ПолеОсмотр = РаботаСHTMLКлиентСервер.ПолучитьТекстHTMLПустогоДокумента();
	Иначе	
	
	ПутьКФайлу = Элементы.ЛабораторныеИсследования.ТекущиеДанные.Результат;
...
&НаКлиенте
Процедура ОткрытьБланкЛабораторныхИсследований(Команда)
	Если Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаЛабораторныеИсследования" Тогда
		ТекДанные = Элементы.ЛабораторныеИсследования.ТекущиеДанные;
		Если ТекДанные = Неопределено Тогда
			ПолеОсмотр = РаботаСHTMLКлиентСервер.ПолучитьТекстHTMLПустогоДокумента();
...
&НаКлиенте
Процедура омг_ОткрытьОтбор(Команда)
	Элементы.ГруппаПериодСпец.Показать();
КонецПроцедуры

&НаКлиенте
Процедура омг_СкрытьОтбор(Команда)
	Элементы.ГруппаПериодСпец.Скрыть();
КонецПроцедуры
