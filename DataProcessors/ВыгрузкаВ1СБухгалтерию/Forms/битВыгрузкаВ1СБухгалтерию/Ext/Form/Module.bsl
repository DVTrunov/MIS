...
Процедура ПриОткрытии(Отказ)
	...
	Если доп_ДетализироватьКлиентов Тогда
		доп_ДетализироватьКлиентовПриИзменении(Элементы.доп_ДетализироватьКлиентов, Истина);
	КонецЕсли;
	
	// Если выбраны встроенные правила, загружаем их сразу при открытии.
	Если Объект.доп_ИсточникПравилОбмена <> 0 Тогда
...
Процедура ВыполнитьЗагрузкуПравилОбмена(АдресФайлаПравилВХранилище = "", ИмяФайлаДляРасширения = "")
	...
	//+ОЛИМП
	доп_ПроводитьДокументыПриЗагрузкеПриИзменении(Неопределено);
	доп_ДетализироватьКлиентовПриИзменении(Неопределено, Истина);
	//-ОЛИМП
...
Процедура ИзменитьРасположениеФайла()
	...
	//+ОЛИМП
	Если Не ЭтоКлиент Тогда
		Элементы.ИмяФайлаПравил.Видимость = ?(Объект.доп_ИсточникПравилОбмена = 0, Истина, Ложь); 	
	КонецЕсли;
	//-ОЛИМП
...
//+ОЛИМП
&НаКлиенте
Процедура доп_ПроводитьДокументыПриЗагрузкеПриИзменении(Элемент)
	...
	Для Каждого Строка Из НайденныеСтроки Цикл
		Строка.Значение = доп_ПроводитьДокументыПриЗагрузке;
	КонецЦикла; 
	
КонецПроцедуры

&НаКлиенте
Процедура доп_ФилиалыВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Объект.доп_ФилиалыВыгрузки.ДоступныеЗначения = ПолучитьСписокФилиалов(); 
	
КонецПроцедуры

&НаКлиенте
Процедура доп_ОрганизацииВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Объект.доп_ОрганизацииВыгрузки.ДоступныеЗначения = доп_ВсеОрганизации(); 
	
КонецПроцедуры

&НаКлиенте
Процедура доп_ДетализироватьКлиентовПриИзменении(Элемент, ДелайМолча = Ложь)
	...
	Для Каждого Строка Из НайденныеСтроки Цикл
		Строка.Значение = доп_ДетализироватьКлиентов;
	КонецЦикла;
...
&НаКлиенте
Процедура доп_ИсточникПравилОбменаПриИзменении(Элемент)
	
	ИзменитьРежимОбработки(ЭтоКлиент);
	
	Если Объект.доп_ИсточникПравилОбмена > 0 Тогда
		
		Если Объект.доп_ИсточникПравилОбмена = 2 Тогда 
...
&НаКлиенте
Процедура доп_ФилиалыВыгрузкиПриИзменении(Элемент)
	
	Если ЗначениеЗаполнено(Объект.доп_ФилиалыВыгрузки) Тогда
		Объект.доп_ОрганизацииВыгрузки.Очистить();
		Элементы.доп_ОрганизацииВыгрузки.Доступность = Ложь;
	Иначе  
		Элементы.доп_ОрганизацииВыгрузки.Доступность = Истина;
	КонецЕсли
