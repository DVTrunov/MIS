...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоКассоваяСсылка = ЗначениеЗаполнено(ЭквайринговыйТерминал.КассоваяСсылкаСБП);
	Если ЭтоКассоваяСсылка Тогда
		Результат = Новый Структура;
		Результат.Вставить("КассоваяСсылка", ЭквайринговыйТерминал.КассоваяСсылкаСБП);
		Результат.Вставить("ИдентификаторОплаты", ЭквайринговыйТерминал.ИдентификаторОплатыСБП);
		ДанныеКассовойСсылки = Результат; 
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗначениеЗаполнено(ТорговаяТочка) Тогда
		...
		//+ОЛИМП Тут не происходит оповещения о закрытии или сообщения пользователю в типовой БИП, сообщаем в явном виде
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Результат.ОписаниеОшибки);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Операция приема оплаты по СБП прервана");
		//-ОЛИМП
	Иначе
		Если ЭтоКассоваяСсылка Тогда
			ПодключитьОбработчикОжидания("АктивироватьКассовуюСсылку", 0.1, Истина);
		Иначе
...
Процедура НеОплачен(Команда)
	...
	// Для кассовых ссылок необходимо перед закрытием формы выполнить
	// деактивацию.
	Если ЭтоКассоваяСсылка Тогда
		РезультатДеактивации = ДеактивироватьКассовуюСсылкуНаСервере(
		ДанныеКассовойСсылки,
		ТорговаяТочка);
		Если ЗначениеЗаполнено(РезультатДеактивации.КодОшибки) Тогда
			ПоказатьОповещениеПользователя(
			НСтр("ru = 'Не удалось деактивировать ссылку'"),
			,
			РезультатДеактивации.СообщениеОбОшибке,
...
Процедура АктивироватьКассовуюСсылку()
	...
	ИдентификаторЗаданияПлатежнаяСсылка = РезультатВыполнения.ИдентификаторЗадания;
	ОповещениеОЗавершении = Новый ОписаниеОповещения(
		"АктивироватьКассовуюСсылкуЗавершение",
		ЭтотОбъект);
	
	Если РезультатВыполнения.Статус <> "Выполняется" Тогда
		АктивироватьКассовуюСсылкуЗавершение(РезультатВыполнения, Неопределено);
		Возврат;
	КонецЕсли;
...
Функция АктивироватьКассовуюСсылкуНаСервере(
	...
	ПараметрыПроцедуры.Вставить("ДанныеСсылки",  ДанныеСсылки);
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Активация кассовой ссылки.'");
	
	РезультатВыполнения = ДлительныеОперации.ВыполнитьВФоне(
		"ИнтеграцияСБП.АктивироватьКассовуюСсылку",
		ПараметрыПроцедуры,
		ПараметрыВыполнения);
...
&НаКлиенте
Процедура АктивироватьКассовуюСсылкуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если ЗначениеЗаполнено(РезультатОперации.КодОшибки) Тогда
			...
			УстановитьВидимостьДоступность(
				РезультатОперации.СообщениеОбОшибке);
		Иначе
			СформироватьДанныеQRКода(ДанныеКассовойСсылки.КассоваяСсылка);
			QRКод = ДанныеКассовойСсылки.КассоваяСсылка;
			ДоступнаОтложеннаяОперация = Истина;
			Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.ГруппаQRКод;
			УстановитьВидимостьДоступность();
			ПодключитьОбработчикОжидания("ОпределитьСтатусОплаты", 0.1, Истина);
...
Функция ДеактивироватьКассовуюСсылкуНаСервере(
	...
	Возврат ИнтеграцияСПлатежнымиСистемами.ДеактивироватьКассовуюСсылку(
		ДанныеСсылки,
		НастройкаПодключения);
