...
Процедура СсылкаПодробнееНажатие(Элемент)
	...
	Если ЭтоОшибкаУстановкиМонопольногоРежима(ИнформацияОбОшибкеФоновогоЗадания) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Невозможно выполнить удаление помеченных объектов, т.к. в приложении работают другие пользователи:'"));
		//-ОЛИМП
		Возврат;
	КонецЕсли;
    //+ОЛИМП
...
Процедура ЗаполнитьИнформациюОбОшибкеФоновогоЗадания(Форма, Результат)
	...
	//+ОЛИМП
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Результат, "ИнформацияОбОшибке") Тогда
		ИнфОбОшибке = ОбработкаОшибок.КраткоеПредставлениеОшибки(Результат.ИнформацияОбОшибке);
	Иначе
		ИнфОбОшибке = Результат.КраткоеПредставлениеОшибки;
	КонецЕсли;
	//-ОЛИМП	
	
	Форма.ИнформацияОбОшибкеФоновогоЗадания = ИнфОбОшибке;
	Форма.Элементы.ДекорацияПредставлениеЗавершенияНеУспешно.Заголовок = 
		?(ЭтоОшибкаУстановкиМонопольногоРежима(ИнфОбОшибке),
...
Функция ЭтоОшибкаУстановкиМонопольногоРежима(ИнформацияОбОшибке)
	...
	//+ОЛИМП
	ТекстОшибкиМонопольно2 = "База данных заблокирована";
	Возврат СтрНайти(ИнформацияОбОшибке, ТекстОшибкиМонопольно) <> 0 ИЛИ СтрНайти(ИнформацияОбОшибке, ТекстОшибкиМонопольно2) <> 0; 
	//-ОЛИМП
КонецФункции
