...
Процедура ПриОткрытии(Отказ)
	...
	Если Параметры.РежимПеревода Тогда
		...
		Если ПроверкаСтатусаЧерезSIP Тогда
			Элементы.ИндексКартинкиСтатуса.КартинкаЗначений = БиблиотекаКартинок.доп_Статусы;
		Иначе
			Элементы.ИндексКартинкиСтатуса.КартинкаЗначений = БиблиотекаКартинок.доп_СтатусыAMI;
		КонецЕсли;
		Элементы.Позвонить.Картинка = БиблиотекаКартинок.доп_ПеревестиЗвонок;
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если Источник = "1С_Софтфон" И Событие = "СостояниеПодписки" Тогда
		...
		СтрокаСтатуса = доп_БитфонКлиент.ПолучитьОписаниеСтатусаПоИндексуКартинки(ИндексКартинкиСтатуса);
...
Процедура ОбновитьСтатусНомера()
	...
	стрНомер = доп_ТелефонияКлиентСервер.ОчиститьНомерТолькоЦифры(Номер);
...
Процедура ОбновитьСтатусНомераAMI()
	...
	Попытка
		...
		СтрокаСтатуса = доп_АТСКлиент.ПолучитьОписаниеСтатуса(СтатусНомера);
