...
Процедура ПриОткрытии(Отказ)
	...
	доп_ТелефонияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ДеревоНомеров, "Владелец",
							ПредопределенноеЗначение("Справочник.доп_ТелефонныеСтанцииБитАТС.ПустаяСсылка"));
		
	ОбновитьДоступность();
		
	АвторизацияАдминистратор = доп_АТССервер.ПроверитьПраваАдминистрировать();
	
	Если АвторизацияАдминистратор Тогда
		Заголовок = "Панель управления звонками ОЛИМП.АТС";
	Иначе
		Заголовок = "Панель просмотра звонков ОЛИМП.АТС";
	КонецЕсли;
...
Процедура ЗагрузкаТранковВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	формаИнфТранка = ПолучитьФорму("Обработка.доп_БитАТС.Форма.ИнформацияТранк");
	...
	доп_ТелефонияКлиентПереопределяемый.ОткрытьФормуСБлокировкойВладельца(формаИнфТранка);
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Иначе
		...
		Если бОбновлениеОчередей ИЛИ бОбновлениеНомеров Тогда
			...
			соотвИнфо = доп_ТелефонияКлиентСервер.ЗаполнитьСоответствиеИзXML(Данные);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "БитфонПанельБИТАТС_ПеревестиОжидающийЗвонок" Тогда
		...
		Если ЗначениеЗаполнено(Параметр.Результат) Тогда
			...
			доп_АТСКлиент.ПеревестиОжидающийЗвонокНаНомер(ЭтаФорма, "Ожидающие", НомерСвязанногоТелефона, номерПеревода,
...
Процедура ЗавершитьРазговор(Команда)
	...
	Если текущ = Неопределено Тогда
		доп_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("Не выбран номер");
		Возврат;
	КонецЕсли;
	...
	Если НЕ ((индСтатусаНомера = 2) ИЛИ (индСтатусаНомера = 5) ИЛИ (индСтатусаНомера = 6)) Тогда
		доп_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("На номере нет активного разговора");
		Возврат;
	КонецЕсли;
	...
	Если НЕ УправлениеРазрешено Тогда
		доп_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("Недостаточно прав для завершения разговора");
		Возврат;
	КонецЕсли;
...
Процедура НомераИнформация(Команда)
	формаЛегенда = ПолучитьФорму("Обработка.доп_БитАТС.Форма.ЛегендаСтатусов", , ЭтаФорма);
	доп_ТелефонияКлиентПереопределяемый.ОткрытьФормуСБлокировкойВладельца(формаЛегенда);
КонецПроцедуры
...
Процедура ПолучитьСвойстваКанала(Команда)
	...
	Если ТекущаяСтрокаНомеров <> Неопределено Тогда
		...
		//
		доп_ТелефонияКлиент.ВывестиСообщение("Имя канала: " + ИмяКанала, ЭтаФорма);
		доп_ТелефонияКлиент.ВывестиСообщение("Имя связанного канала: " + ИмяСвязанногоКанала, ЭтаФорма);
		//
		Попытка
			Если ИспользоватьВебСервис Тогда
				ХэшЗаписи = доп_АТССервер.ВебСервисПолучитьХэшЗаписиРазговора(ТекущаяАТС, ВебСервисИдКлиента, ИмяКанала, флагВходящий, ИмяСвязанногоКанала);
			Иначе
				ХэшЗаписи = обКонтроллерАТС.GetRecordHash(ИмяКанала, флагВходящий, ИмяСвязанногоКанала);
			КонецЕсли;
			доп_ТелефонияКлиент.ВывестиСообщение("Хэш записи: " + ХэшЗаписи, ЭтаФорма);
		Исключение
			доп_ТелефонияКлиент.ВывестиСообщение("Ошибка получения хэша записи - " + ОписаниеОшибки(), ЭтаФорма);
		КонецПопытки;
		//
		Попытка
			Если ИспользоватьВебСервис Тогда
				ТранзитИд = доп_АТССервер.ВебСервисПолучитьТранзитИд(ТекущаяАТС, ВебСервисИдКлиента, ИмяКанала);
			Иначе
				ТранзитИд = обКонтроллерАТС.GetTransitID(ИмяКанала);
			КонецЕсли;
			доп_ТелефонияКлиент.ВывестиСообщение("TransitID: " + ТранзитИд, ЭтаФорма);
		Исключение
			доп_ТелефонияКлиент.ВывестиСообщение("Ошибка получения TransitID - " + ОписаниеОшибки(), ЭтаФорма);
		КонецПопытки;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПолучитьПеременнуюКанала(Команда)
	стрИмяПеременной = "";
	доп_ТелефонияКлиентПереопределяемый.ПоказВводСтроки(стрИмяПеременной, "Имя переменной канала", "БитфонПанельБИТАТС_ПолучитьПеременнуюКанала");
КонецПроцедуры
...
Процедура ПолучитьПеременнуюКаналаЗавершение(стрИмяПеременнойКанала)
	...
	Если ТекущаяСтрокаНомеров <> Неопределено Тогда
		...
		Попытка
			Если ИспользоватьВебСервис Тогда
				стрЗначПеременнойКанала = доп_АТССервер.ВебСервисПолучитьПеременнуюКанала(ТекущаяАТС, ВебСервисИдКлиента, ИмяКанала, стрИмяПеременнойКанала);
			Иначе
				стрЗначПеременнойКанала = обКонтроллерАТС.GetChannelVariable(ИмяКанала, стрИмяПеременнойКанала);
			КонецЕсли;
			доп_ТелефонияКлиент.ВывестиСообщение("Значение переменной " + стрИмяПеременнойКанала + ": " + стрЗначПеременнойКанала, ЭтаФорма);
		Исключение
			доп_ТелефонияКлиент.ВывестиСообщение("Ошибка получения переменной канала - " + ОписаниеОшибки(), ЭтаФорма);
		КонецПопытки;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПолучитьСвойстваНомера(Команда)
	...
	Если ТекущаяСтрокаНомеров <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СипЛогин) Тогда
			Попытка
				Если ИспользоватьВебСервис Тогда
					стрПирИнфо = доп_АТССервер.ВебСервисПолучитьСвойстваНомера(ТекущаяАТС, ВебСервисИдКлиента, СипЛогин);
				Иначе
					стрПирИнфо = обКонтроллерАТС.GetSIPPeerInfo(СипЛогин);
				КонецЕсли;
				доп_ТелефонияКлиент.ВывестиСообщение("Информация номера " + НомерТекущ + " (SIP логин " + СипЛогин + "): " +
					Символы.ВК + Символы.ПС + стрПирИнфо, ЭтаФорма);
			Исключение
				доп_ТелефонияКлиент.ВывестиСообщение( ОписаниеОшибки(), ЭтаФорма );
			КонецПопытки;
		Иначе
			доп_ТелефонияКлиент.ВывестиСообщение("Не найден SIP логин номера", ЭтаФорма);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПозвонитьСоСвязанногоТелефона(Команда)
	формаВводаНомера = доп_ТелефонияКлиент.ПолучитьФормуВводаНомера(ЭтаФорма, Ложь, Неопределено, Ложь, "");
	доп_ТелефонияКлиентПереопределяемый.ОткрытьФормуСБлокировкойВладельца(формаВводаНомера);
КонецПроцедуры

&НаКлиенте
Процедура Настройки(Команда)
	доп_АТСКлиент.ОткрытьФормуНастроек(Истина);
КонецПроцедуры

&НаКлиенте
Процедура ПеревестиНаВыбранныйНомер(Команда)
	доп_АТСКлиент.ПеревестиЗвонокВыборНомера(ЭтаФорма, "Ожидающие", обКонтроллерАТС, НомерСвязанногоТелефона,
					"БитфонПанельБИТАТС_ПеревестиОжидающийЗвонок");
КонецПроцедуры

&НаКлиенте
Процедура ПеревестиНаСвойНомер(Команда)
	доп_АТСКлиент.ПеревестиОжидающийЗвонокНаСвойНомер(ЭтаФорма, "Ожидающие", НомерСвязанногоТелефона,
		обКонтроллерАТС, ИспользоватьВебСервис, ВебСервисИдКлиента);
КонецПроцедуры
...
Функция ПроверитьДоступностьПодключенияКРазговоруСервер(ГруппаДоступаСсылка)
	...
	Если АдминистраторАТС Тогда
		Возврат Истина;
	КонецЕсли;
	ТекущПольз = доп_ТелефонияСервер.ПолучитьТекущегоПользователя();
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	доп_ГруппыДоступаБитАТСПользователи.Пользователь.Ссылка
	               |ИЗ
	               |	Справочник.доп_ГруппыДоступаБитАТС.Пользователи КАК доп_ГруппыДоступаБитАТСПользователи
	               |ГДЕ
	               |	доп_ГруппыДоступаБитАТСПользователи.Ссылка = &ГруппаДоступа
	               |	И доп_ГруппыДоступаБитАТСПользователи.Пользователь.Ссылка = &Пользователь";
...
Процедура ПодключитьВК()
	...
	КонецЕсли;
	доп_АТСКлиент.ПодключениеКомпонентыУправлениеАТС("БитфонПанельБИТАТС_ПодключениеВнешнейКомпоненты");
КонецПроцедуры
...
Процедура ПодключитьРазрешенныйАдресЗавершение()
	...
	Попытка
		...
		Если ИспользоватьВебСервис Тогда
			ВебСервисИдКлиента = доп_АТССервер.ВебСервисОткрытьСоединение(ТекущаяАТС, Хост.Адрес, Хост.Пользователь, Хост.Пароль, СписокНомеровХоста, Ложь);
			Подключен = Истина;
		Иначе
			доп_АТСКлиент.УстановитьПараметрыПроверкиЛицензии(обКонтроллерАТС, Ложь);
		...
		доп_ТелефонияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ДеревоНомеров, "Владелец", ТекущаяАТС);
		
		//
		ОбновитьДоступность();
		
		Элементы.ДеревоНомеров.ТекущаяСтрока = ПредопределенноеЗначение("Справочник.доп_НомераБитАТС.ПустаяСсылка");
		ОбработкаВыбораГруппыДереваНомеров();
...
Процедура ЗаполнитьТаблицыОчередейИОжидающих(соотвИнфо)
	...
	Для Каждого элСпискаОчередь Из массОчередей Цикл
		...
		Для Каждого домОжидающийОчереди ИЗ списокОжидающих Цикл
			...
			доп_АТСКлиент.СократитьНомерНайтиКонтрагента(кэшКонтактов, СоответствиеНомеровХоста, стрНомер, КонтрагентСсылка, КонтактноеЛицоСсылка);
			...
			СтрокаТаблОжидающих.ВремяОжидания	= доп_ТелефонияКлиентСервер.ФорматироватьДлительностьЗвонкаСЛидНулями(числВремяОжидания);
			//
...
Процедура ЗаполнитьТаблицыНомеровЗвонковТранков(соотвХмл)
	...
	Для Каждого Экстеншен Из массивЭкстеншенов Цикл
		...
		Для йАккаунт=0 По (колвоАккаунтовНомера-1) Цикл
			...
			//
			ИндексКартинкиСтатуса = доп_АТСКлиент.ПолучитьИндексКартинкиСтатуса(СтатусНомера);
			//
			...
			Если колвоЗвонковАккаунта > 0 Тогда
				Для йЗвонокАккаунта=0 По (колвоЗвонковАккаунта-1) Цикл
					...
					Иначе
						СоединениеНомера = доп_АТСКлиент.СократитьНомер(СоединениеНомера, Ложь, ВнешнийВызов);
					КонецЕсли;
					//
					...
					//	НабранныйНомер = доп_ТелефонияСервер.ПолучитьНаименованиеНабранногоНомера(НабранныйНомер);
					//КонецЕсли;
					//
					ДобавитьСтатусНомераВМассив(ИмяЭкст, НомерЭкст, стрСтатусЭкстенш, ГруппаДоступаЭкст,
	...
	Для й=1 По Номера.Количество() Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблНомеров.Соединен) Тогда
			...
			КонтрагентСсылка = Неопределено;
			КонтактноеЛицоСсылка = Неопределено;
			доп_АТСКлиент.НайтиКонтрагентаПоСокращНомеру(кэшКонтактов, СоответствиеНомеровХоста, СтрокаТаблНомеров.Соединен,
	...
	Для й = 0 По (КолвоЗвонков - 1) Цикл
		...
		доп_АТСКлиент.СократитьНомерНайтиКонтрагента(кэшКонтактов, СоответствиеНомеровХоста, НомерЗвОт, КонтрагентОтСсылка, КонтактноеЛицоОтСсылка);
		//
		НомерЗвКому = обАктивныйЗвонок.Получить("To");
		КонтрагентКомуСсылка = Неопределено;
		КонтактноеЛицоКомуСсылка = Неопределено;
		доп_АТСКлиент.СократитьНомерНайтиКонтрагента(кэшКонтактов, СоответствиеНомеровХоста, НомерЗвКому, КонтрагентКомуСсылка, КонтактноеЛицоКомуСсылка);
		//
	...
	Попытка
		...
		Если ИспользоватьВебСервис Тогда
			...
			Если ИмяТекСтраницы = "ГруппаНомера" ИЛИ ИмяТекСтраницы = "ГруппаЗвонки" ИЛИ ИмяТекСтраницы = "ГруппаТранки" Тогда
				соотвИнфо = доп_АТССервер.ВебСервисОбновитьНомера(ТекущаяАТС, ВебСервисИдКлиента);
				ЗаполнитьТаблицыНомеровЗвонковТранков(соотвИнфо);
			...
			ИначеЕсли ИмяТекСтраницы = "ГруппаОчереди" ИЛИ ИмяТекСтраницы = "ГруппаОжидающие" ИЛИ ИмяТекСтраницы = "ГруппаКлиентскиеГруппы" Тогда
				соотвИнфо = доп_АТССервер.ВебСервисОбновитьОчереди(ТекущаяАТС, ВебСервисИдКлиента);
				ЗаполнитьТаблицыОчередейИОжидающих(соотвИнфо);
...
Процедура ОткрытьДетальнуюИнформациюОчереди(ОчередьИд, ОчередьИмя, ОчередьНомер)
	формаИнфОчереди = ПолучитьФорму("Обработка.доп_БитАТС.Форма.ИнформацияОчередь");
	...
	доп_ТелефонияКлиентПереопределяемый.ОткрытьФормуСБлокировкойВладельца(формаИнфОчереди);
КонецПроцедуры
...
Функция ПроверкаСтатусаСвободен()
	...
	Иначе
		...
		Если НЕ Свободен Тогда
			доп_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("Звонок возможен только на свободный номер");
		КонецЕсли;
	КонецЕсли;
	Возврат Свободен;
КонецФункции
...
Функция ПроверкаСтатусаРазговаривает()
	...
	Иначе
		...
		Если НЕ Разговаривает Тогда
			доп_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("На выбранном номере нет идущего разговора");
		КонецЕсли;
	КонецЕсли;
...
Процедура НачатьЗвонок(ВызываемыйНомер)
	...
	доп_АТСКлиент.НачатьЗвонок(Подключен, НомерСвязанногоТелефона, ВызываемыйНомер, обКонтроллерАТС,
...
Функция ПолучитьПараметрыТекущегоРазговора(SIPЛогинТекущ, НомерТекущ, ГруппаДоступаТекущ)
	...
	ИначеЕсли ИмяТекСтраницы = "ГруппаЗвонки" Тогда
		Если Элементы.Звонки.ТекущиеДанные = Неопределено Тогда
			доп_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("Не выбран звонок");
			получены = Ложь;
		Иначе
...
Процедура ПодключитьсяКРазговору(РежимПодключения)
	...
	Если НЕ ЗначениеЗаполнено(НомерСвязанногоТелефона) Тогда
		доп_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("Не задан номер связанного телефона", , "Ошибка");
		Возврат;
	КонецЕсли;
	...
	Если НЕ параметрыТекРазгПолучены Тогда
		доп_ТелефонияКлиент.ВывестиСообщение("Не удалось получить параметры текущего разговора", ЭтаФорма);
		Возврат;
	КонецЕсли;
	...
	Если НЕ УправлениеРазрешено Тогда
		доп_ТелефонияКлиентПереопределяемый.ПоказПредупреждение("Недостаточно прав для подключения к разговору");
		Возврат;
	КонецЕсли;
	...
	Попытка
		Если ИспользоватьВебСервис Тогда
			доп_АТССервер.ВебСервисПодключитьсяКРазговору(ТекущаяАТС, ВебСервисИдКлиента, НомерСвязанногоТелефона, СлушающийНомер,
...
Процедура ОбработкаВыбораГруппыДереваНомеров()
	Если Хост <> Неопределено Тогда 
		Хост.Номера = доп_АТССервер.ПолучитьНомераАТС(ТекущаяАТС, Элементы.ДеревоНомеров.ТекущаяСтрока);
		ОбновитьТаблицы();
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработкаОшибкиВебСервиса(стрОписаниеИсключения)
	доп_АТСКлиент.ОбработкаОшибки(ТекущаяАТС, "ВебСервисУправлениеАТС", "ВебСервисУправлениеАТС_Ошибка", стрОписаниеИсключения, ЭтаФорма);
	ПроверкаВосстановленияПодключения();
КонецПроцедуры
