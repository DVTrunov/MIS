	...
	Попытка
		...
		Если обТранк <> Неопределено Тогда
			...
			Для й = 0 По (КолвоЗвонковТранка-1) Цикл
				...
				строкаЗвонок.Разговор		= доп_АТСКлиент.ПолучитьИмяПоВнутреннемуНомеру(СоответствиеНомеровХоста, ЗвонокНаТранке);
				строкаЗвонок.Направление	= ? (НаправлениеВходящНаТранке,  "<-" , "->");
				строкаЗвонок.Длительность	= ДлительностьНаТранке;
				доп_АТСКлиент.СократитьНомерНайтиКонтрагента(кэшКонтактов, СоответствиеНомеровХоста, ВнешнийАбонентНаТранке, КонтрагентСсылка, КонтактноеЛицоСсылка);
