...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "БитфонОчередьБИТАТС_ПеревестиОжидающийЗвонок" Тогда
		Если ЗначениеЗаполнено(Параметр.Результат) Тогда
			...
			доп_АТСКлиент.ПеревестиОжидающийЗвонокНаНомер(ЭтаФорма, "Ожидающие", НомерСвязанногоТелефона, номерПеревода, обКонтроллерАТС, ИспользоватьВебСервис);
		КонецЕсли;
	КонецЕсли;
	//
...
Процедура ПеревестиНаСвойНомер(Команда)
	доп_АТСКлиент.ПеревестиОжидающийЗвонокНаСвойНомер(ЭтаФорма, "Ожидающие", НомерСвязанногоТелефона, обКонтроллерАТС, ИспользоватьВебСервис, ВебСервисИдКлиента);
КонецПроцедуры
...
Процедура ЗагрузитьТаблицы(соотвИнфо)
	...
	Для Каждого домЧленОчереди ИЗ списокЧленов Цикл
		...
		строкаТабл.Наименование		= доп_АТСКлиент.ПолучитьИмяПоВнутреннемуНомеру(СоответствиеНомеровХоста, ИмяЧлОчереди);
	...
	Для Каждого домОжидающийОчереди ИЗ списокОжидающих Цикл
		...
		строкаТабл.ВремяОжидания	= доп_ТелефонияКлиентСервер.ФорматироватьДлительностьЗвонкаСЛидНулями(числВремяОжидания);
		//
		КонтрагентСсылка = Неопределено;
		КонтактноеЛицоСсылка = Неопределено;
		доп_АТСКлиент.СократитьНомерНайтиКонтрагента(кэшКонтактов, СоответствиеНомеровХоста, ОжидающийНомер, КонтрагентСсылка, КонтактноеЛицоСсылка);
