...
Функция Печать(Объект, ПредполагаемыеУслуги, СуммаСметы, ВыводитьДействияМедосмотра = Ложь, ПоказыватьУслугиСНулевойЦеной = Истина) Экспорт
	...
	Для Каждого Шапка Из Объект.Смета Цикл
		...
		Область.Параметры.Фактор = СформироватьСтрокуФредныхФакторов(Объект, Шапка.ВредныйФактор);
...
Функция СформироватьСтрокуФредныхФакторов(Объект, СтрокаВредностей)
	...
	Для Каждого Фактор Из МассивФакторов Цикл
		...
		Если СтрокиФакторов.Количество() > 0 Тогда
			ВредныйФакторСсылка = СтрокиФакторов[0].ВредныйФактор;
			ВредныйФакторНазвание = СокрЛП(ОбщегоНазначенияБИТКлиентСервер.ЗаполненноеЗначение(ВредныйФакторСсылка.НаименованиеПолное, ВредныйФакторСсылка.Наименование));
			ВредныйФакторНазвание = СтрЗаменить(ВредныйФакторНазвание, Символы.ВК, "");
		КонецЕсли;
		СтрокаИтог = СтрокаИтог + Фактор + " " + ВредныйФакторНазвание + "," + Символы.ПС;
	КонецЦикла;
