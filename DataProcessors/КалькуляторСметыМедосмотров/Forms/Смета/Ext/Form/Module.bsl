...
Процедура ЗаполнитьСметуНаСервере()
	...
	ЗаполнитьТаблицуСсылокВредныхФакторов(СписокКлиентов);
...
Функция ПолучитьФорматФайлаПоУмолчанию(СтруктураШапки)
	...
	Для Каждого Эл Из ВидМедосмотра.Приказы Цикл
		Если ТипЗнч(Эл.Приказ) = Тип("СправочникСсылка.ПереченьВредныхФакторовИРабот") И Не Эл.ИспользоватьБезВФ Тогда
			Продолжить;
		КонецЕсли;
...
Функция НайтиВредныйФактор(Фактор)
	...
	ПриказСтрока = Лев(Фактор, СтрНайти(Фактор, "_") - 1);
	НомерПП = Прав(Фактор, СтрДлина(Фактор) - СтрНайти(Фактор, "_"));
	СтруктураОтбора = Новый Структура("ПриказСтрока, НомерПП", ПриказСтрока, НомерПП);
	
	СтрокиФакторов = Объект.ТаблицаСсылокВредныхФакторов.НайтиСтроки(СтруктураОтбора);
	Если СтрокиФакторов.Количество() > 0 Тогда
		Возврат СтрокиФакторов[0].ВредныйФактор;
	КонецЕсли;
...
&НаСервере
Функция ЗаполнитьТаблицуСсылокВредныхФакторов(СписокКлиентов)
	
	ТЗ = Новый ТаблицаЗначений;
	ТЗ.Колонки.Добавить("НомерПП", Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(20)));
	ТЗ.Колонки.Добавить("Приказ", Новый ОписаниеТипов("СправочникСсылка.ПриказыМедосмотров"));
	ТЗ.Колонки.Добавить("ПриказСтрока", Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(150)));
	...
	Для Каждого Строка Из СписокКлиентов Цикл
		...
		Для Каждого Фактор Из МассивВредностей Цикл
			...
			НоваяСтрока.ПриказСтрока = Лев(Фактор, СтрНайти(Фактор, "_") - 1);
			НоваяСтрока.НомерПП = Прав(Фактор, СтрДлина(Фактор) - СтрНайти(Фактор, "_"));
			
			Если Не ЗначениеЗаполнено(НоваяСтрока.ПриказСтрока) Тогда
				НоваяСтрока.Приказ = Справочники.ПриказыМедосмотров.Приказ29н;
			Иначе
				НоваяСтрока.Приказ = Справочники.ПриказыМедосмотров.НайтиПоКоду(НоваяСтрока.ПриказСтрока);
			КонецЕсли;
    ...
    Запрос.Текст = "ВЫБРАТЬ
                   |	ТЗ.НомерПП КАК НомерПП,
                   |	ТЗ.Приказ КАК Приказ,
				   |	ТЗ.ПриказСтрока КАК ПриказСтрока
                   |ПОМЕСТИТЬ ТЗ
                   |ИЗ
                   |	&ТЗ КАК ТЗ";
	...
	Запрос.Текст = "ВЫБРАТЬ Различные
	               |	ПереченьВредныхФакторовИРабот.Ссылка КАК ВредныйФактор,
				   |  	ТЗ.НомерПП КАК НомерПП,
				   |    ТЗ.Приказ КАК Приказ,
				   |    ТЗ.ПриказСтрока КАК ПриказСтрока
	               |ИЗ
				   ...
				   |		ПО ТЗ.НомерПП = ПереченьВредныхФакторовИРабот.НомерПП И ТЗ.Приказ = ПереченьВредныхФакторовИРабот.Приказ";
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(Объект.ТаблицаСсылокВредныхФакторов.Добавить(), Выборка);	
	КонецЦикла;
