...
Процедура Обработать(Команда)
	...
	ИсследованияКлиента = Новый Массив;
	Для Каждого ДанныеИсследования Из ДанныеВыбранныхСтрок Цикл
		Если ДанныеИсследования.Клиент = КлиентИсследования Тогда
			...
			ИсследованияКлиента.Добавить(ДанныеИсследования.Номенклатура);
		КонецЕсли;
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(ФормаПриема.Объект.Ссылка) Тогда
		МедосмотрыСервер.УстановитьОтметкиДляДействийПМОИзЛаборатоныхИследований(КлиентИсследования, ИсследованияКлиента, ФормаПриема.Объект.Ссылка);
	КонецЕсли;
