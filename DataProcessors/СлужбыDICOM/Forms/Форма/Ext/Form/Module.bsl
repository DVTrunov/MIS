...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Отказ = Не (Константы.ИспользоватьDICOM.Получить()); // Запрещено начинать использовать блок, только продолжать (снят с продаж).
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	...
	стрСервис.Автозапуск = Константы.DCMTK_STORESCP_АвтоматическийЗапуск.Получить();
	...
	стрСервис.Статус = СтатусСтрокой(СтатусСлужбы);
	стрСервис.Автозапуск = Константы.DCMTK_MWLSCP_АвтоматическийЗапуск.Получить();
	...
	ЭтаФорма.АвтозапускНаКлиенте = НастройкиDICOM.DCMTK_STORESCP_АвтоматическийЗапуск;
...
Функция ПолучитьСвойства(строкаТЗ)
	...
	Попытка
		...
		Иначе
			AETitle = Константы.DCMTK_STORESCP_ИспользуемыйAETitle.Получить();
		КонецЕсли;
...
Процедура ЗаписатьНайстройки()
	...
	НаборЗаписей[0].DCMTK_STORESCP_АвтоматическийЗапуск	= Найденные[0].Автозапуск;
...
Процедура СервисыАвтозапускПриИзменении(Элемент)
	...
	ИначеЕсли ЭтаФорма.Элементы.Сервисы.ТекущиеДанные.Идентификатор = "STORAGESCP" Тогда
		ЗаписатьКонстантуАвтозапуск(
				"DCMTK_STORESCP_АвтоматическийЗапуск",
				ЭтаФорма.Элементы.Сервисы.ТекущиеДанные.Автозапуск
			);
	ИначеЕсли ЭтаФорма.Элементы.Сервисы.ТекущиеДанные.Идентификатор = "MWLSCP" Тогда
		ЗаписатьКонстантуАвтозапуск(
				"DCMTK_MWLSCP_АвтоматическийЗапуск",
				ЭтаФорма.Элементы.Сервисы.ТекущиеДанные.Автозапуск
			);
	КонецЕсли;
КонецПроцедуры
