...
Процедура ПриОткрытии(Отказ)
	...
	ОбновитьДоступность();
	Элементы.ВерсияОЛИМП_Phone.Заголовок = "Версия ОЛИМП.Phone " + доп_ТелефонияСервер.ПолучитьВерсиюПодсистемы();
	...
	списокУстройств = доп_ТелефонияКлиентСервер.СтрРазбить(Параметры.СписокУстройствВывода, ";");
...
Процедура ДиректорияЗаписанныхФайловНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	доп_ТелефонияКлиентПереопределяемый.ПоказВыборФайла(длг, "БитФонНастройки_ВыборДиректорииЗаписиЗавершение", стрКаталогНач);
КонецПроцедуры

&НаКлиенте
Процедура ПрофильНастроекПриИзменении(Элемент)
	
	доп_БитфонКлиент.ОбработчикИзмененияПрофиляНастроек(Запись, Истина);
...
Процедура ФайлВходящегоЗвонкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	доп_ТелефонияКлиентПереопределяемый.ПоказВыборФайла(длг, "БитФонНастройки_ВыборФайлаВходящегоЗвонкаЗавершение", стрФайлНач);
КонецПроцедуры
...
Процедура ОткрытьПапкуСЛогФайлом(Команда)
	доп_ТелефонияКлиентПереопределяемый.ЗапускПрограммы("%APPDATA%\BIT");
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьЗапросВТехподдержку(Команда)
	доп_БитфонКлиент.ОтправитьЗапросВТехподдержкуСАрхивомЛога();
КонецПроцедуры
...
Процедура ОбновитьДоступность()
	...
	//
	флагПрофильНастроекБИТАТС = (Запись.ПрофильНастроек = ПредопределенноеЗначение("Перечисление.доп_ПрофилиНастроекБИТPhone.доп_АТС")) ИЛИ
								(Запись.ПрофильНастроек = ПредопределенноеЗначение("Перечисление.доп_ПрофилиНастроекБИТPhone.Произвольные"));
	...
	//
	Элементы.КомандаПереадресации.Доступность = Запись.ТипПереводаЗвонка = ПредопределенноеЗначение("Перечисление.доп_ТипПереводаЗвонка.Условный");
	//
	Элементы.ПроверкаСтатусаНомераПередПереводом.Доступность = ((Запись.ТипПереводаЗвонка = ПредопределенноеЗначение("Перечисление.доп_ТипПереводаЗвонка.Условный"))
															ИЛИ (Запись.ТипПереводаЗвонка = ПредопределенноеЗначение("Перечисление.доп_ТипПереводаЗвонка.Безусловный")));
	//
...
Процедура ОбновитьСписокКодеков()
	...
	массКодеков = доп_БитфонКлиент.РазобратьСписокКодеков(Запись.СписокКодеков);
