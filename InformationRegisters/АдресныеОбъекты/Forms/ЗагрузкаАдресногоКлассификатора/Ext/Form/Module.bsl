...
Процедура ЗагрузитьКлассификаторССайтаАутентификация(Знач КодыРегионов)
	...
	НастройкиОжидания.ВыводитьПрогрессВыполнения = Истина; //+ОЛИМП.
...
Процедура ВывестиСообщениеОбОшибке(Результат)
    ...
    //+ОЛИМП Вернем старый алгоритм оповещения о ошибке, т.к. БСП методы не совместимы!
...
Функция ЗапуститьФоновуюЗагрузкуССайтаНаСервере(КодыРегионов, ЗагружатьИсторию, УникальныйИдентификатор, ВременныйКаталог)
	...
	//+ОЛИМП не имеется механизма выполнения процедур в несколько потоков
	//БЫЛО:
	...
	//Стало:
	ФоновоеЗадание = ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения, "АдресныйКлассификаторСлужебный.ФоноваяЗагрузкаРегионаАдресногоКлассификатораССайта",
		ПараметрыМетода);
	//-ОЛИМП
...
Функция ЗапуститьФоновуюЗагрузкуНаСервере(Знач КодыРегионов, Знач ОписаниеФайловЗагрузки, 
	...
	//	ПараметрыВыполнения, ПараметрыМетода);
	//Стало:
	ФоновоеЗадание = ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения, "АдресныйКлассификаторСлужебный.ФоноваяЗагрузитьДанныеКлассификатораАдресовИзФайла",
		ПараметрыМетода);
	//-ОЛИМП
...
Процедура СсылкаНаФайлыФИАСНажатие(Элемент)
	ЗапуститьПриложение("https://its.1c.ru/download/gar");
КонецПроцедуры
