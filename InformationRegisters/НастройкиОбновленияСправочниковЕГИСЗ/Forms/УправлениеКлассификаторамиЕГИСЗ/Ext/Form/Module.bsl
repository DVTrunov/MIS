...
Процедура ОбновитьКлассификатор(Команда)
	...
	ИдентификаторыОбновляемыхСправочников = Новый Массив;
	...
	Для Каждого ИдентификаторСтроки Из Элементы.СписокКлассификаторов.ВыделенныеСтроки Цикл
		...
		Иначе
			...
			ИдентификаторыОбновляемыхСправочников.Добавить(ВыделеннаяСтрока.OID);
		КонецЕсли;
	КонецЦикла;
	
	Если ИдентификаторыОбновляемыхСправочников.Количество() > 0 Тогда
		Если ИдентификаторыОбновляемыхСправочников.Найти("1.2.643.5.1.13.13.99.2.594") <> Неопределено
			И ИдентификаторыОбновляемыхСправочников.Найти("1.2.643.5.1.13.13.99.2.605") = Неопределено
		Тогда
			ИдентификаторыОбновляемыхСправочников.Добавить("1.2.643.5.1.13.13.99.2.605")
		КонецЕсли;
		
		Для Каждого ИдентификаторСправочника Из ИдентификаторыОбновляемыхСправочников Цикл
			ИнтеграцияЕГИСЗВызовСервера.ОбновитьСправочник(ИдентификаторСправочника);
		КонецЦикла;
...
&НаКлиенте
Процедура ОчиститьВерсиюУВсех(Команда)
	
	Для Каждого ТекущаяСтрока Из ЭтотОбъект.СписокКлассификаторов Цикл
		ОчиститьВерсиюСтроки(ТекущаяСтрока);
	КонецЦикла;
...
Процедура ПроверитьДоступностьНаСервере()
	...
	ЗагрузкаКлассификаторовНСИЕГИСЗ.ПроверитьДоступностьНСИ();
