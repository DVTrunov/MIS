...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИдентификацияПриложения = РегистрыСведений.СеансовыеДанныеGoogle.ИдентификацияПриложения(Параметры.ИДПриложения);
...
Процедура ПриОткрытии(Отказ)
	...
	Браузер = АдресЗапросаНаПодтверждениеДоступа(Истина);
...
Функция ПараметрАвторизации(ИдентификаторАвторизации, БраузерВОтдельномОкне)
	...
	Формат(0, "ЧГ="),    // ЗначениеРазделителяСеанса = 0 (Разделение не включено)
...
Процедура ОбработатьСобытиеБраузерДокументСформирован()
	...
	Если Элементы.Браузер.Документ.URL = "about:blank" Тогда
		textcontent = Элементы.Браузер.Документ.body.textcontent;
		Если СтрНайти(textcontent, "Couldn't connect to server: http://localhost") Тогда
			Если СтрНайти(textcontent, "error") Тогда
				СтруктураЗаголовка.Статус = СтатусОтказ();
			Иначе
				ТокенЗапроса = Сред(textcontent, СтрНайти(textcontent, "http://localhost"), СтрДлина(textcontent));
				ОбработатьПолучениеТокенаЗапроса();
				Возврат;
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли СтрНайти(Элементы.Браузер.Документ.URL, "http://localhost") И СтрНайти(Элементы.Браузер.Документ.URL, "redirect_uri=http://localhost") = 0 Тогда
		ТокенЗапроса = Элементы.Браузер.Документ.URL;
		ОбработатьПолучениеТокенаЗапроса();
		Возврат;
	КонецЕсли;
...
Функция ОбменятьТокенЗапросаНаСеансовыеДанные(ТокенЗапроса, ИдентификацияПриложения, ОбластьДанных)
	...
	Результат.Вставить("ИДGoogleПриложения", Параметры.ИДПриложения);
...
Функция СтруктураЗаголовка(Знач ЗаголовокВебСтраницы)
	...
	Результат.Параметры = СтроковыеФункцииКлиентСервер.ПолучитьПараметрыИзСтроки(КомпонентыЗаголовка[1], "&");
	Возврат Результат;
