...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОсновнойМакетОформленияСКД.ПодсказкаВвода = УниверсальныеМеханизмыСервер.ПредставлениеОсновногоМакетаОформленияОтчетовНаСКД();
	
	ЗаполнитьСписокВыбораПоляПоискаЭлемента();
	
	//+Обмен с Google 
	ИспользоватьОбменСКалендаремGoogle = Константы.ИспользоватьОбменСКалендаремGoogle.Получить();
	ИмпортИзGoogleЛистовОжидания = Константы.ИмпортИзGoogleЛистовОжидания.Получить();
	РаботаСФормамиКлиентСервер.УстановитьДоступностьЭлементаФормы(Элементы,"ИмпортИзGoogleЛистовОжидания",ИспользоватьОбменСКалендаремGoogle);
	Элементы.ДекорацияПриложенияGoogle.Доступность = ИспользоватьОбменСКалендаремGoogle;
	//-Обмен с Google
	
	#Область Стоматология
	Если ПроцедурыСпециализацииПоставки.ЕстьПодсистемаСтоматологии() Тогда
		ПроцедурыСпециализацииПоставки.ДобавитьКомандуСтоматологииВФормуУчетнойПолитики(ЭтаФорма);
	КонецЕсли;
	#КонецОбласти
	// Добавлено Макеев (
	// Очистка поля при открытии, если оно было когда то заполнено. РИБ не используется
	// Чтобы не мешало в дальнейшем в других местах.
	Если Не Филиал.Пустая() Тогда
		Филиал = "";
	КонецЕсли;
	// Так же программное скрытие настроек обмена РИБ.
	// Программно, потому чтобы потом не потерять изменения при командном взаимодействии.
	Элементы.РГОбменСДРБазами.Видимость = Ложь;  
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("ПериодичностьПланированияКалендаря", Тип("ПолеФормы"),Элементы.ГруппаКванты);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Запись.омг_ПериодичностьПланированияКалендаря"; 
	Элементы.Переместить(НовыйЭлемент,Элементы.ГруппаКванты,Элементы.ОтображатьКолонкуЖЗДлительностиИнтервала);
	// Запрет на уровне записей
	Элементы.ГруппаРазграничениеДоступаОсн.Видимость = Ложь;
	ПолеЗапретНаУровнеЗаписей = Элементы.Добавить("омг_ЗапретНаУровнеЗаписей", Тип("ПолеФормы"), Элементы.ГруппаРазграничениеДоступа);
	ПолеЗапретНаУровнеЗаписей.Вид = ВидПоляФормы.ПолеФлажка;
	ПолеЗапретНаУровнеЗаписей.ПутьКДанным = "Запись.омг_ЗапретНаУровнеЗаписей";
	омг_ИспользоватьПолнотекстовыйПоиска = Константы.омг_ИспользоватьПолнотекстовыйПоиск.Получить();
	// )
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//+Обмен с Google
	Константы.ИспользоватьОбменСКалендаремGoogle.Установить(ИспользоватьОбменСКалендаремGoogle);
	Константы.ИмпортИзGoogleЛистовОжидания.Установить(ИмпортИзGoogleЛистовОжидания);
	//-Обмен с Google
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// Добавлено Макеев (
	Константы.омг_ИспользоватьПолнотекстовыйПоиск.Установить(омг_ИспользоватьПолнотекстовыйПоиска);
	// )
КонецПроцедуры
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда        
		Если СтратегияСписанияПартийТоваровПоДатам = ПредопределенноеЗначение("Перечисление.СтратегияСписанияПартийТоваровПоДатам.Средняя") Тогда
			...
			Ответ = Вопрос(НСтр("ru='Если ранее стратегия списания была не по средней стоимости, текущие остатки на складах еще учитываются в разрезе отдельных партий приходования.
							...
							|В случае, если старые партии не будут очищены, они будут списаны в первую очередь, а по новым поступлениям учет будет вестись без детализации по партиям (по средней).'"),ВариантыОтветов);
			Если Ответ = КодВозвратаДиалога.Отмена Тогда
...
&НаКлиенте
Процедура ККМНастройкаРазрешительногоРежима(Команда)
	ОткрытьФорму("РегистрСведений.НастройкиРазрешительногоРежима.Форма.ФормаСписка");
КонецПроцедуры
...
Процедура ОбновитьСостояниеСертификатов(РезультатОперации)
	...
	Если РезультатОперации.Результат И МенеджерОборудованияВызовСервера.ИспользуетсяСертификатыНУЦМинцифры() Тогда
		Текст = "";
		ДоверенныеСертификаты = ПолучитьДоверенныеСертификаты();
...
Функция ПолучитьДоверенныеСертификаты() 
	...
	Если МенеджерОборудованияВызовСервера.ИспользуетсяСертификатыНУЦМинцифры() Тогда
		Если ОбщегоНазначенияБПО.ИспользуетсяБСП() И Не ОбщегоНазначения.ЭтоВебКлиент() Тогда
...
&НаКлиенте
Процедура ИспользоватьОбменСКалендаремGoogleПриИзменении(Элемент)
	
	РаботаСФормамиКлиентСервер.УстановитьДоступностьЭлементаФормы(Элементы,"ИмпортИзGoogleЛистовОжидания",ИспользоватьОбменСКалендаремGoogle);
	Элементы.ДекорацияПриложенияGoogle.Доступность = ИспользоватьОбменСКалендаремGoogle;
	Если НЕ ИспользоватьОбменСКалендаремGoogle Тогда
		ИмпортИзGoogleЛистовОжидания = Ложь;
	КонецЕсли;	
		
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияПриложенияGoogleНажатие(Элемент)
	Если GoogleЕстьПриложения() Тогда
		ОткрытьФорму("РегистрСведений.ПриложенияGoogle.ФормаСписка");
	Иначе
		ОткрытьФорму("РегистрСведений.ПриложенияGoogle.ФормаЗаписи");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияНастройкиФормированияПредставленияКлиентаНажатие(Элемент)
	
	ОткрытьФорму("ОбщаяФорма.НастройкиФормированияПредставленияКлиентаGoogle",, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция GoogleЕстьПриложения()
	
	Выб = РегистрыСведений.ПриложенияGoogle.Выбрать();
