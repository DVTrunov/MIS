...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Выб = Справочники.ПричиныОтменыЗаявок.Выбрать();
	Пока Выб.Следующий() Цикл
		Если Не Выб.ПометкаУдаления
			И Не Выб.ЭтоГруппа
		Тогда
			СписокВыбора.Добавить(Выб.Ссылка, Выб.Наименование);
		КонецЕсли;
	КонецЦикла;
...
Процедура ОК(Команда)
	...
	Если Параметры.ДляВводаПричины Тогда
		...
		Иначе
			...
			Оповестить("ЗаявкаИзменение", Новый Структура("Дата", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.Заявка,"ДатаНачала")), ЭтаФорма); 
			Закрыть(СтруктураЗаписи);

		КонецЕсли;
	Иначе
		Оповестить("ЗаявкаИзменение", Новый Структура("Дата", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.Заявка,"Дата")), ЭтаФорма); 
		ЭтаФорма.Записать();
	КонецЕсли;
...
&НаКлиенте
Процедура ПричинаОтменыЗаявкиАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	
	Если Ожидание = 0 И Не ЗначениеЗаполнено(Текст) Тогда
		СтандартнаяОбработка = Ложь;
		ДанныеВыбора = ОбщегоНазначенияКлиентСервер.СкопироватьСписокЗначений(СписокВыбора);
	КонецЕсли;
