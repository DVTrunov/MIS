...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("ВебКассаПользователь",Тип("ПолеФормы"),ЭтаФорма); 
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.Омг_ВебКассаПользователь"; 
	НовыйЭлемент.Заголовок = "Пользователь (Web kassa)";
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("ВебКассаПароль",Тип("ПолеФормы"),ЭтаФорма);
	НовыйЭлемент.ПутьКДанным = "Объект.Омг_ВебКассаПароль";
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.Заголовок = "Пароль (Web kassa)";
...
Процедура СоздатьМЧД(Команда)
	...
	ОткрытьФорму("Справочник.МашиночитаемыеДоверенности.Форма.ФормаЭлемента", ПараметрыФормы,ЭтотОбъект,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаСервереБезКонтекста
Процедура МЧДПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	
	Для Каждого СтрокаДинСписка Из Строки Цикл
		Полномочия = Новый Массив;
		Для Каждого СтрокаТЧ Из СтрокаДинСписка.Ключ.Полномочия Цикл
			Полномочия.Добавить(Строка(СтрокаТЧ.Полномочие));
		КонецЦикла;
		СтрокаДинСписка.Значение.Данные.ПолномочияСтрокой = СтрСоединить(Полномочия, "; ");
	КонецЦикла;
