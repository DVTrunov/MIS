...
Процедура ПередПоискомЭлементаКлассификатораВБазе(СтрокаКлассификатора) Экспорт
	...
	Тогда
		...
		Если ПозицияОткрывающей <> 0 Тогда
			КодДиагнозаИзНаименования = Сред(СтрокаКлассификатора.Наименование, ПозицияОткрывающей + 1, СтрДлина(СтрокаКлассификатора.Наименование) - ПозицияОткрывающей - 1);
			КодДиагнозаЛатиницей = СокрЛП(СтрокаКодаЛатиницей(КодДиагнозаИзНаименования));
			
			Если ЗначениеЗаполнено(КодДиагнозаЛатиницей)
				И СтрНайти(КодДиагнозаЛатиницей, "-") = 0
			Тогда
				КодДиагнозаЛатиницей = СтрШаблон("%1-%1", КодДиагнозаЛатиницей);
			ИначеЕсли КодДиагнозаЛатиницей = "J10-J18" Тогда
				КодДиагнозаЛатиницей = "J09-J18";
			КонецЕсли;
			
			СтрокаКлассификатора.Код = КодДиагнозаЛатиницей;
			СтрокаКлассификатора.КодДиагноза = КодДиагнозаЛатиницей;
		КонецЕсли;
	КонецЕсли;
