...
// ОМГ 
	...
	Если не РольДоступна("ПолныеПрава") Тогда	
		...
		Элементы.ВладелецСертификата.ТолькоПросмотр = Истина;
	КонецЕсли;  
...
Процедура БыстрыйПоискНажатие(Элемент)
	ОткрытьФорму("ОбщаяФорма.ФормаПоискаКлиентаУпр",,Элементы.ВладелецСертификата);
КонецПроцедуры
...
Процедура ОбновитьСписок()
	...
	Если КартыТолькоВладельца Тогда
		Если ПоказыватьКартыРодственников И ЗначениеЗаполнено(ВладелецКарты) Тогда
			...
			СписокКлиентов.Добавить(ПредопределенноеЗначение("Справочник.Клиенты.ПустаяСсылка"));
			РаботаСФормамиКлиент.УстановитьОтборСписка("ВладелецКарты", СписокКлиентов, Список);  
		Иначе 
			ВладелецКартыСписок = Новый СписокЗначений;
			ВладелецКартыСписок.Добавить(ВладелецКарты);
			ВладелецКартыСписок.Добавить(ПредопределенноеЗначение("Справочник.Клиенты.ПустаяСсылка"));
			РаботаСФормамиКлиент.УстановитьОтборСписка("ВладелецКарты", ВладелецКартыСписок, Список);
		КонецЕсли;
	Иначе
...
&НаКлиенте
Процедура ВладелецСертификатаОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
