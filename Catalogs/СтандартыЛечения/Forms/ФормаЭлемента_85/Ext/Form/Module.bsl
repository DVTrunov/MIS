...
Процедура ДобавитьСтрокуНазначения(ВидНазначения, Назначение = Неопределено)
	...
	НоваяСтрока.ЭтапМаршрутаПациента = ЭтапЗаполнения;
...
Процедура НазначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Тогда
		...
		НоваяРабота = РаботаСФормамиКлиент.ВыбратьЭлементНоменклатурыПоВиду(Истина, Элементы.Назначения.ТекущиеДанные.Назначение, Элементы.Назначение, Ложь, Оповещение);
	Иначе
...
Процедура ПутьКФайлуДокументаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Фильтр="Текстовые документы (*.dot, *.doc, *.docx, *.rtf, *.odt, *.pdf, *.txt)|*.dot; *.doc; *.docx; *.rtf; *.odt; *.pdf; *.txt";
...
Процедура ВыгрузитьНазначения(ТекущийОбъект)
	...
	Для Каждого Назначение Из Назначения.ПолучитьЭлементы() Цикл 
		...
		НоваяЗапись.ЭтапМаршрутаПациента = Назначение.ЭтапМаршрутаПациента;
...
Процедура УстановитьДоступностьВидимость()
	...
	Элементы.ЭтапМаршрутаПациента.Видимость       = Не Объект.ВидСтандарта;
	Элементы.ЭтапЗаполнения.Видимость             = Не Объект.ВидСтандарта;
	Элементы.УстановитьЭтапВыделенным.Видимость   = Не Объект.ВидСтандарта;
	Элементы.НазначенияКолонкиМедикамента.Видимость = Не Объект.ВидСтандарта;
	
	#Область Стоматология // Внутренние стандарты для стоматологии
	Элементы.НазначенияКолонкиМедикамента.Видимость	 = Не Объект.Стоматологический И Элементы.НазначенияКолонкиМедикамента.Видимость;
	Элементы.КнопкаДобавитьМедикаменты.Видимость	 = Не Объект.Стоматологический И Элементы.КнопкаДобавитьМедикаменты.Видимость;
	Элементы.Стоматологический.Видимость			 = Не Объект.ВидСтандарта;
	#КонецОбласти
...
&НаКлиенте
Процедура СтоматологическийПриИзменении(Элемент)
	
	Если Объект.Стоматологический Тогда
		СтрокиНазначений = Назначения.ПолучитьЭлементы();
		мОбрИнд = СтрокиНазначений.Количество()-1;
		Для Сч = 0 По мОбрИнд Цикл
			
			Назначение = СтрокиНазначений[мОбрИнд-Сч];
			Если Назначение.ВидНазначения <> ПредопределенноеЗначение("Перечисление.ВидыНазначений.Обследование")
				И Назначение.ВидНазначения <> ПредопределенноеЗначение("Перечисление.ВидыНазначений.Процедура")
			Тогда
				СтрокиНазначений.Удалить(мОбрИнд-Сч);
			КонецЕсли;
...
&НаКлиенте
Процедура УстановитьЭтапВыделенным(Команда)
		
	Если Не ЗначениеЗаполнено(ЭтапЗаполнения) Тогда
		ПоказатьПредупреждение(, "Выберите этап маршрута в поле ""Этап для заполнения"".");
		Возврат;
	КонецЕсли;
	
	ВыделенныеСтроки = Элементы.Назначения.ВыделенныеСтроки;
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, "Выделите строки назначений, которым нужно установить этап.");
		Возврат;
	КонецЕсли;
	
	Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		СтрокаНазначения = Назначения.НайтиПоИдентификатору(ИдентификаторСтроки);
		Если СтрокаНазначения <> Неопределено Тогда
			СтрокаНазначения.ЭтапМаршрутаПациента = ЭтапЗаполнения;
		КонецЕсли;
	КонецЦикла;
