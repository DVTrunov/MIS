...
Процедура ПриОткрытии(Отказ)
	...
	омг_УстановитьВидимостьЛабораторныыеУслуги();
	// )     
	
	//трунов
	Список = Объект.омг_СписокТегов;
	Для каждого Элемент из Список Цикл
		Этаформа.омг_ТегиЗначения.Добавить(Элемент.омг_ТегНоменклатуры);
	КонецЦикла;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("омг_ТегиУжеЗаписаны") Тогда
		Возврат;
	Иначе
		Список = ЭтаФорма.омг_ТегиЗначения;
		Объект.омг_СписокТегов.Очистить();
		Объект.омг_Теги = "";
		Для Каждого Элемент Из Список Цикл     
			НоваяСтрока = Объект.омг_СписокТегов.Добавить();
			НоваяСтрока.омг_ТегНоменклатуры = Элемент.Значение;
			Объект.омг_Теги = Объект.омг_Теги + Строка(Элемент.Значение) + ", "; 
		КонецЦикла;
		
		ПараметрыЗаписи.Вставить("омг_ТегиУжеЗаписаны", Истина);
...
Процедура ВидНоменклатурыПриИзменении(Элемент)
	...
	Элементы.ГруппаМаркировка.Видимость				= Не (ЭтоУслуга Или ЭтоНабор);
...
&НаКлиенте
Процедура омг_УстановитьВидимостьЛабораторныыеУслуги()
	
	//Элементы.омг_ГруппаЛабораторныеУслуги.Видимость = Объект.Анализ;
...
Процедура ВидНоменклатурыПриИзмененииМедицина(ЭтоУслуга, ЭтоНабор)
	...
	РаботаСФормамиКлиентСервер.УстановитьВидимостьЭлементаФормы(Элементы, "ГруппаНоменклатураМедицинскихУслуг", ЭтоУслуга);
	РаботаСФормамиКлиентСервер.УстановитьВидимостьЭлементаФормы(Элементы, "ГруппаИнструментальноеИсследование", ЭтоУслуга);
...
Процедура омг_ДопЭлементы()            
	...
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("НеПрименятьСкидки",Тип("ПолеФормы"),Элементы.ГруппаМедицинскиеФлаги); 
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_НеПрименятьСкидку";
	НовыйЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	
	НоваяКоманда = Команды.Добавить("омг_ПоказатьПричинуАрхивирования");
	НоваяКоманда.Заголовок = "Показать причину архивирования";
	НоваяКоманда.Отображение = ОтображениеКнопки.Картинка;
	НоваяКоманда.Картинка = БиблиотекаКартинок.Информация;
	НоваяКоманда.Действие = "омг_ПоказатьПричинуАрхивирования";
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_ПоказатьПричинуАрхивирования",Тип("КнопкаФормы"),Элементы.ГруппаШапкаСтрока3); 
	НовыйЭлемент.Вид = ВидКнопкиФормы.ОбычнаяКнопка;
	НовыйЭлемент.ИмяКоманды = "омг_ПоказатьПричинуАрхивирования";
	
	Элементы.Переместить(НовыйЭлемент, Элементы.ГруппаШапкаСтрока3, Элементы.омг_ДатаПоследнегоОбновленияЛИС);
	
	Элементы.Архив.УстановитьДействие("ПриИзменении", "омг_АрхивПриИзменении");
	Элементы.Архив.Заголовок = "Заблокирована";
	
	Элементы.омг_ПоказатьПричинуАрхивирования.Видимость = Объект.Архив;
	
КонецПроцедуры	

&НаКлиенте
Процедура омг_АрхивПриИзменении(Элемент)
...
&НаКлиенте
Процедура Обработчик_ПослеВводаПричиныАрхивирования(Результат, ДопПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Объект.омг_ПричинаАрхивирования = Результат.Текст;
...
&НаКлиенте
Процедура омг_ПоказатьПричинуАрхивирования(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("Обработчик_ПослеВводаПричиныАрхивирования", ЭтаФорма);
	
	ОткрытьФорму("ОбщаяФорма.ВводМногострочногоТекстаТабличнойЧасти", Новый Структура("Текст", Объект.омг_ПричинаАрхивирования), 
		ЭтаФорма, УникальныйИдентификатор,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура КомплектующиеНоменклатурыКомплектующаяНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	МассивВН = Новый массив;
	МассивВН.Добавить(ПредопределенноеЗначение("Перечисление.ВидыНоменклатуры.Услуга"));
	МассивВН.Добавить(ПредопределенноеЗначение("Перечисление.ВидыНоменклатуры.Набор"));
	ПараметрыОтбора = Новый Структура;
	Параметр1 = Новый Структура;
	Параметр1.Вставить("ВидНоменклатуры",МассивВН);
	ПараметрыОтбора.Вставить("Отбор",Параметр1);
	ОткрытьФорму("Справочник.Номенклатура.Форма.ФормаВыбораУпр",ПараметрыОтбора,Элемент);
