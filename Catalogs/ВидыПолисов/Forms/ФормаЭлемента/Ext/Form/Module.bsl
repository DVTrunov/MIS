...
Процедура ЗаполнитьПоГруппе(Группа, ДополнительныеПараметры) Экспорт
	...
	Если Группа <> Неопределено Тогда
		ДополнитьНоменклатуруМассивом(НоменклатураЗаполненияПоГруппе(Группа, Объект.УчетПлатныхМатериаловОбособленноОтУслуг));
	КонецЕсли;
...
Процедура ПодборРаботы(Команда)
	...
	РаботаСФормамиКлиент.КнопкаПодборПриНажатии(ЭтаФорма, "ОсновныеУслуги", "Услуга", "Материал");
...
Процедура ДополнитьПоПрейскуранту(Команда)
	...
	ДополнитьНоменклатуруМассивом(НоменклатураЗаполненияИзПрейскуранта(Объект.Прейскурант, Объект.УчетПлатныхМатериаловОбособленноОтУслуг));
...
Процедура ДополнитьИзУстановкиЦен_ПослеВыбора(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		ДополнитьНоменклатуруМассивом(НоменклатураЗаполненияИзДокументаУстановкиЦен(Результат, Объект.УчетПлатныхМатериаловОбособленноОтУслуг));		
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция НоменклатураЗаполненияПоГруппе(Группа, УчетПлатныхМатериаловОбособленноОтУслуг)
	...
	|			ИЛИ &ВидНоменклатуры = НЕОПРЕДЕЛЕНО)
	|	И НЕ ТабНом.Архив
	|	И НЕ ТабНом.ПометкаУдаления"
	;
	
	Запрос.УстановитьПараметр("Группа",Группа);
	Запрос.УстановитьПараметр("ВидНоменклатуры",?(УчетПлатныхМатериаловОбособленноОтУслуг,Неопределено,ПредопределенноеЗначение("Перечисление.ВидыНоменклатуры.Услуга")));
...
&НаСервереБезКонтекста
Функция НоменклатураЗаполненияИзПрейскуранта(Прейскурант, УчетПлатныхМатериаловОбособленноОтУслуг)
...
&НаСервереБезКонтекста
Функция НоменклатураЗаполненияИзДокументаУстановкиЦен(УстановкаЦен, УчетПлатныхМатериаловОбособленноОтУслуг)
	...
	Для Каждого СтрокаНоменклатуры Из УстановкаЦен.Номенклатура Цикл
		Если СтрокаНоменклатуры.Номенклатура.ВидНоменклатуры = Перечисления.ВидыНоменклатуры.Услуга
			Или УчетПлатныхМатериаловОбособленноОтУслуг 
