...
//
Процедура ПриНачальномЗаполненииЭлементов(ЭтоОбновление = Ложь) Экспорт //+ОЛИМП для обновления из макета
	...
	Попытка
		...
		Запрос.Текст = 
			...
			Если Не ЭтоОбновление Тогда
				Запрос.Текст = Запрос.Текст + Символы.ПС + "И (ВЫРАЗИТЬ(ШаблоныЭтикетокИЦенниковБПО.XMLОписаниеМакета КАК СТРОКА(1))) = """"";
			КонецЕсли;
			//-ОЛИМП
...
Процедура ОпределитьПоляШаблона(Источник, Объект)
	...
	Если ОборудованиеПринтерыЭтикеток.ПолучитьОписаниеМакета(Источник, СообщениеОбОшибке, СодержимоеXMLФайла) Тогда
