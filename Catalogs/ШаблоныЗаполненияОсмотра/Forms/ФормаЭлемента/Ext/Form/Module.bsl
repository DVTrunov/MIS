...
Если НЕ РольДоступна("ПолныеПрава") Тогда
		Элементы.омг_ВрачВладелец.Доступность=Ложь;
	КонецЕсли;	
КонецПроцедуры
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ РольДоступна("ПолныеПрава") Тогда
        ...
        Если Пользователь <> Неопределено И Пользователь.Сотрудник <> Неопределено Тогда
            ТекущийОбъект.омг_ВрачВладелец = Пользователь.Сотрудник;
        Иначе
            Сообщить("Невозможно определить врача по текущему пользователю");
        КонецЕсли;
    КонецЕсли;	
