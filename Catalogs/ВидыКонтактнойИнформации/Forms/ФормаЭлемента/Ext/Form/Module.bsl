...
&НаКлиенте
Процедура ПривестиТелефоныКМаске(Команда)
	
	ПараметрыМетода = Новый Структура;
	ПараметрыМетода.Вставить("ВидКИ", Объект.Ссылка);
	ПараметрыМетода.Вставить("Маска", Объект.МаскаВводаТелефонногоНомера);
	
	Задание = ПолучитьФоновоеЗадание_ПривестиНомераТелефоновПоВидуКМаскеВФоне(ПараметрыМетода, ЭтотОбъект.УникальныйИдентификатор);
		
	НастройкиОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
	НастройкиОжидания.ВыводитьОкноОжидания = Истина;
	НастройкиОжидания.ТекстСообщения = НСтр("ru = 'Выполняется приведение телефонов к маске.'");
	
	Обработчик = Новый ОписаниеОповещения("ПривестиТелефоныКМаскеЗавершение", ЭтотОбъект, Параметры);
...
Процедура НастроитьВидимостьДоступность()
	...
	Элементы.ПривестиТелефоныКМаске.Доступность = Объект.ИспользоватьМаскуВвода И Не Объект.Ссылка.Пустая(); // Для новых элементов не имеет смысла.
...
Функция ПолучитьФоновоеЗадание_ПривестиНомераТелефоновПоВидуКМаскеВФоне(ПараметрыМетода, УникальныйИдентификатор)

	ИмяМетода = "КонтактнаяИнформацияСервер.ПривестиНомераТелефоновПоВидуКМаскеВФоне";
	
	НастройкиЗапуска = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Контактная информация: приведение номеров телефонов к маске'");
...
&НаКлиенте
Процедура ПривестиТелефоныКМаскеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат.Статус = "Выполнено" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Операция завершена!");
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		ПоказатьПредупреждение(Неопределено, Результат.КраткоеПредставлениеОшибки);
	КонецЕсли;
