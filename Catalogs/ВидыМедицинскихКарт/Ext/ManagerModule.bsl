	...
	Пока Выборка.Следующий() Цикл
		Если Объект.ЗаполняемыеПараметры.НайтиСтроки(Новый Структура("ИмяПараметра", Выборка.Ссылка)).Количество() = 0 Тогда
			...
			Иначе
				...
				Если ТипЗначенияПараметра.Типы().Количество() = 1 И ЗначениеЗаполнено(Стр.ИмяПараметра.ПараметрШаблонаОсмотра) Тогда
					Стр.ЗначениеПоУмолчанию = ОТ.ПривестиЗначение(Стр.ЗначениеПоУмолчанию);	
				Иначе
