...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Добавлено Макеев (
	омг_УстановитьВидимостьЭлементов();
	омг_СозданиеСтраницыИсследованияMedreview();
	// )    
	//////////////////////////////////////////////
	// ОМГ
	омг_ДопЭлементы(); 
	Если Не Параметры.Ключ.Пустая() Тогда
		СуммаНачислений = ИнтеграцияЕИП.ПолучитьНакопленияИзРегистра(Объект.Ссылка);
		Элементы.омг_СуммаНачислений.Заголовок = "Сумма накоплений : " + СуммаНачислений;
	Иначе	
		Объект.омг_РискПадения = Перечисления.омг_РискиПадения.Низкий;
	КонецЕсли;
	ЗаполнитьМаршрутыПациента();
	// ОМГ
    //////////////////////////////////////////////
...
#Область омг_ДополнительныеПроцедурыИФункции

&НаСервере
Процедура ЗаполнитьМаршрутыПациента()
	МаршрутыПациента.Загрузить(
		МаршрутыПациентовСервер.МаршрутыПациентаТаблица(Объект.Ссылка));
КонецПроцедуры

&НаКлиенте
Процедура МаршрутыПациентаВыбор(Элемент, Строка, Поле, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ТекущиеДанные = Элементы.МаршрутыПациента.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда Возврат; КонецЕсли;
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ПервичныйПрием", ТекущиеДанные.ПервичныйПрием);
	ПараметрыФормы.Вставить("СтандартЛечения", ТекущиеДанные.СтандартЛечения);
	ОткрытьФорму("Отчет.МаршрутЛеченияПациента.Форма.ФормаОтчета_85", ПараметрыФормы, ЭтотОбъект);
КонецПроцедуры

&НаСервере
Процедура омг_ДопЭлементы()
	
	ГруппаСоц = ЭтаФорма.Элементы.Добавить("ГруппаСоц", Тип("ГруппаФормы"),ЭтаФорма.Элементы.ПанельФормы);
	ГруппаСоц.Вид = ВидГруппыФормы.Страница;
	ГруппаСоц.ОтображатьЗаголовок = Истина; 
	ГруппаСоц.Заголовок = "Соц.льготы"; 
	ГруппаСоц.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;

	
	ГруппаИнвалидность = ЭтаФорма.Элементы.Добавить("ГруппаИнвалидность", Тип("ГруппаФормы"),ГруппаСоц);
	ГруппаИнвалидность.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаИнвалидность.ОтображатьЗаголовок = Истина; 
	ГруппаИнвалидность.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	ГруппаИнвалидность.Заголовок = "Сведения об инвалидности";
    ГруппаИнвалидность.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение;
	
	ГруппаПенсия = ЭтаФорма.Элементы.Добавить("ГруппаПенсия", Тип("ГруппаФормы"),ГруппаСоц);
	ГруппаПенсия.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаПенсия.ОтображатьЗаголовок = Истина; 
	ГруппаПенсия.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	ГруппаПенсия.Заголовок = "Сведения о пенсионном обеспечении";    
	ГруппаПенсия.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение;
	
	ГруппаВОВ = ЭтаФорма.Элементы.Добавить("ГруппаВОВ", Тип("ГруппаФормы"),ГруппаСоц);
	ГруппаВОВ.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаВОВ.ОтображатьЗаголовок = Истина; 
	ГруппаВОВ.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда; 
	ГруппаВОВ.Заголовок = "Сведения об участии в ВОВ";   
	ГруппаВОВ.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение;  
	
	ГруппаСП = ЭтаФорма.Элементы.Добавить("ГруппаСП", Тип("ГруппаФормы"),ГруппаСоц);
	ГруппаСП.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаСП.ОтображатьЗаголовок = Истина; 
	ГруппаСП.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда; 
	ГруппаСП.Заголовок = "Страховые программы";   
	ГруппаСП.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение; 
	
	ГруппаПрочее = ЭтаФорма.Элементы.Добавить("ГруппаПрочее", Тип("ГруппаФормы"),ГруппаСоц);
	ГруппаПрочее.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаПрочее.ОтображатьЗаголовок = Истина; 
	ГруппаПрочее.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда; 
	ГруппаПрочее.Заголовок = "Прочее";   
	ГруппаПрочее.Отображение = ОтображениеОбычнойГруппы.ОбычноеВыделение; 
	
	ГруппаРиск = ЭтаФорма.Элементы.Добавить("ГруппаРиск", Тип("ГруппаФормы"),Элементы.ГруппаРискПадения);
	ГруппаРиск.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаРиск.ОтображатьЗаголовок = Ложь; 
	ГруппаРиск.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;   
	ГруппаРиск.Отображение = ОтображениеОбычнойГруппы.Нет;
	//Элементы.Переместить(ГруппаРиск,Элементы.ГруппаШапкаРеквизиты,Элементы.ДатаПол);
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_РискПадения", Тип("ПолеФормы"),ГруппаРиск);
	...
	НовыйЭлемент.ПутьКДанным = "Объект.омг_РискПадения"; 
	НовыйЭлемент.СписокВыбора.Добавить(Перечисления.омг_РискиПадения.Низкий);
	НовыйЭлемент.СписокВыбора.Добавить(Перечисления.омг_РискиПадения.Высокий);
	НовыйЭлемент.УстановитьДействие("ПриИзменении","РискиПриИзменении");
	//Элементы.Переместить(Элементы.Родитель,ГруппаРиск);
	Элементы.Переместить(НовыйЭлемент,ГруппаРиск); 
	
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_Инвалидность", Тип("ПолеФормы"),ГруппаИнвалидность);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_Инвалидность";   
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "омг_ПриИзмененииСоц");
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_КатегорияИнвалидности", Тип("ПолеФормы"),ГруппаИнвалидность);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_КатегорияИнвалидности";   
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_ИнвалидностьУдостоверение", Тип("ПолеФормы"),ГруппаИнвалидность);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_ИнвалидностьУдостоверение";
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_Пенсионер", Тип("ПолеФормы"),ГруппаПенсия);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_Пенсионер";  
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "омг_ПриИзмененииСоц");
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_ПенсионерУдостоверение", Тип("ПолеФормы"),ГруппаПенсия);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_ПенсионерУдостоверение";   
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_ВетеранВОВ", Тип("ПолеФормы"),ГруппаВОВ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_ВетеранВОВ"; 
    НовыйЭлемент.УстановитьДействие("ПриИзменении", "омг_ПриИзмененииСоц");
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_ВОВУдостоверение", Тип("ПолеФормы"),ГруппаВОВ);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_ВОВУдостоверение";
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_МногодетнаяМать", Тип("ПолеФормы"),ГруппаПрочее);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_МногодетнаяМать";
	
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_ЗеленыйСП", Тип("ПолеФормы"),ГруппаСП);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_ЗеленыйСП";  
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_ЖелтыйСП", Тип("ПолеФормы"),ГруппаСП);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_ЖелтыйСП";
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_КрасныйСП", Тип("ПолеФормы"),ГруппаСП);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_КрасныйСП";
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_НомерКартыСкидки", Тип("ПолеФормы"),Элементы.омг_КартыСкидок);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_КартыСкидок.НомерКарты";
	
	ДобавляемыеРеквизиты = Новый Массив;
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("омг_СуммаНачисленийПоКарте", 
	ОбщегоНазначения.ОписаниеТипаЧисло(15, 2),, "Сумма начислений")); 
			
	ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	
	НовыйЭлемент = Элементы.Добавить("омг_СуммаНачислений", Тип("ДекорацияФормы"), Элементы.ГруппаКартыСкидок);
	НовыйЭлемент.Заголовок = "Всего начислений по карте: 0";
	...
	Запрос.Текст = 
		...
		|	И омг_ПостоянныеКлиенты.Период <= &ТекДата
		|	И омг_ПостоянныеКлиенты.ДоДаты >= &ТекДата";
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		НовыйЭлемент = Элементы.Добавить("омг_ПостоянныйКлиент", Тип("ДекорацияФормы"), Элементы.ИсточникНапоминать);
		НовыйЭлемент.Заголовок = "Это постоянный клиент";
		НовыйЭлемент.Вид = ВидДекорацииФормы.Надпись; 
		НовыйЭлемент.ЦветТекста = WebЦвета.ТемноЗеленый;  
...
&НаКлиенте
Процедура омг_ПриИзмененииСоц() 
	
	Элементы.омг_КатегорияИнвалидности.Доступность = Объект.омг_Инвалидность = Истина;
	Элементы.омг_ИнвалидностьУдостоверение.Доступность = Объект.омг_Инвалидность = Истина;
	Элементы.омг_ПенсионерУдостоверение.Доступность = Объект.омг_Пенсионер = Истина;
	Элементы.омг_ВОВУдостоверение.Доступность = Объект.омг_ВетеранВОВ = Истина;
		
КонецПроцедуры	

&НаСервере
Процедура омг_СозданиеСтраницыИсследованияMedreview()
	
	ЭлСтраницаИсследованияMedreview = Элементы.Добавить("Группаомг_ИсследованияMedreview", Тип("ГруппаФормы"), Элементы.ПанельФормы);
	ЭлСтраницаИсследованияMedreview.Вид = ВидГруппыФормы.Страница;
	ЭлСтраницаИсследованияMedreview.Заголовок = "Исследования";
	
	Элементы.Переместить(ЭлСтраницаИсследованияMedreview, Элементы.ПанельФормы, Элементы.ПолисыГруппа);
	
	ДобавляемыеРеквизиты = Новый Массив;
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("омг_ИсследованияMedreview", НовыЙ ОписаниеТипов("ДинамическийСписок")));
	
	ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	
	ЭтаФорма.омг_ИсследованияMedreview.ПроизвольныйЗапрос = Истина;
	ЭтаФорма.омг_ИсследованияMedreview.ТекстЗапроса = "ВЫБРАТЬ
	                                                  |	омг_РезультатИсследованияMedreview.Ссылка КАК Ссылка,
	                                                  |	омг_РезультатИсследованияMedreview.Номер КАК Номер,
	                                                  |	омг_РезультатИсследованияMedreview.Дата КАК Дата,
	                                                  |	омг_РезультатИсследованияMedreview.Услуга КАК Услуга,
	                                                  |	омг_РезультатИсследованияMedreview.Пациент КАК Пациент,
	                                                  |	омг_РезультатИсследованияMedreview.Врач КАК Врач,
	                                                  |	омг_РезультатИсследованияMedreview.Филиал КАК Филиал
	                                                  |ИЗ
	                                                  |	Документ.омг_РезультатИсследованияMedreview КАК омг_РезультатИсследованияMedreview
	                                                  |ГДЕ
	                                                  |	омг_РезультатИсследованияMedreview.Пациент = &Пациент";
	
	ЭтаФорма.омг_ИсследованияMedreview.Параметры.УстановитьЗначениеПараметра("Пациент", Объект.Ссылка);
	ДобавляемыеРеквизиты.Очистить();
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("омг_ИсследованияMedreviewСсылка", 
		Новый ОписаниеТипов("ДокументСсылка.омг_РезультатИсследованияMedreview")));
	
	ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	
	Эломг_ИсследованияMedreview = Элементы.Добавить("омг_ИсследованияMedreview", Тип("ТаблицаФормы"), ЭлСтраницаИсследованияMedreview);
	Эломг_ИсследованияMedreview.ПутьКДанным  = "омг_ИсследованияMedreview";
	Эломг_ИсследованияMedreview.УстановитьДействие("Выбор", "омг_ИсследованияMedreviewВыбор");
	
	омг_ДобавитьЭлементИсследованияMedreview("Номер");
	омг_ДобавитьЭлементИсследованияMedreview("Дата");
	омг_ДобавитьЭлементИсследованияMedreview("Услуга");
	омг_ДобавитьЭлементИсследованияMedreview("Пациент");
	омг_ДобавитьЭлементИсследованияMedreview("Врач");
	омг_ДобавитьЭлементИсследованияMedreview("Филиал");
	
	ЭтаФорма.омг_ИсследованияMedreview.УстановитьОбязательноеИспользование("Ссылка", Истина);

КонецПроцедуры

&НаСервере
Процедура омг_УстановитьВидимостьЭлементов()

	Элементы.Тестовый.Видимость = Ложь;
	Элементы.ИНН.Заголовок = "ИИН/БИН";
	
КонецПроцедуры

&НаСервере
Процедура омг_ДобавитьЭлементИсследованияMedreview(ИмяЭлемента)
	
	Эломг_ИсследованияMedreviewСсылка = Элементы.Добавить("омг_ИсследованияMedreview" + ИмяЭлемента, Тип("ПолеФормы"), Элементы.омг_ИсследованияMedreview);
	Эломг_ИсследованияMedreviewСсылка.ПутьКДанным = "омг_ИсследованияMedreview." + ИмяЭлемента;

КонецПроцедуры

&НаКлиенте
Процедура омг_ИсследованияMedreviewВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ПоказатьЗначение(, Элементы.омг_ИсследованияMedreview.ТекущиеДанные.Ссылка);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	КонецЕсли;
	  	Объект.омг_РасшифровкаПротокола.Очистить();
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ОписаниеОшибки = ПроверитьЗаполнениеОбязательныхРеквизитовПередЗаписью(Отказ);
	Если Отказ Тогда
		Для Каждого СтруктураОшибкиПоля Из ОписаниеОшибки.СтруктураОшибки Цикл
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтруктураОшибкиПоля.ТекстОшибки,,,"Объект." + СтруктураОшибкиПоля.ПутьКДанным);
		КонецЦикла;
	ИначеЕсли ЗначениеЗаполнено(ОписаниеОшибки.ТекстОшибки) Тогда
		ПараметрыЗаписи.Вставить("ПредупреждениеПослеЗаписи", ОписаниеОшибки.ТекстОшибки);
	КонецЕсли;
...
Асинх Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ПредупреждениеПослеЗаписи") Тогда
		ПоказатьПредупреждение(, ПараметрыЗаписи.ПредупреждениеПослеЗаписи);
	КонецЕсли;
...
Процедура НастроитьТабличныеЧасти(ПриСозданииФормы = Ложь)
	...
	///////////////////////////////////////////////////////////////////////////////////////
	// ОМГ
	Элементы.ПанельФормы.ТекущаяСтраница = Элементы.ПанельФормы.ПодчиненныеЭлементы.омг_ИсторияПриемов;
	// ОМГ
    ///////////////////////////////////////////////////////////////////////////////////////
	...
	///////////////////////////////////////////////////////////////////////////////////////
	// ОМГ
	омг_ИсторияПриемов.Параметры.УстановитьЗначениеПараметра("Владелец",Объект.Ссылка);
	омг_ЛабораторныеИсследования.Параметры.УстановитьЗначениеПараметра("Владелец",Объект.Ссылка);
	// ОМГ
    ///////////////////////////////////////////////////////////////////////////////////////
...
Функция ПроверитьЗаполнениеОбязательныхРеквизитовПередЗаписью(Отказ)
	...
	ОписаниеОшибки = РегистрыСведений.ОбязательныеРеквизитыОбъектов.ПроверитьЗаполненностьОбязательныхРеквизитов(ОбъектСтруктура, Объект.Ссылка.Пустая(), Отказ);
...
КонецПроцедуры
 //Добавлено Карсаков
&НаКлиенте
Процедура омг_ОткрытьПротокол(Команда)  Экспорт
     // 1. Получаем выбранную строку
	Объект.омг_РасшифровкаПротокола.Очистить();
    ВыбраннаяСтрока = Элементы.омг_ИсторическиеСведения1.ТекущиеДанные;
    Если ВыбраннаяСтрока = Неопределено Тогда
        Сообщить("Выберите строку с протоколом!");
        Возврат;
    КонецЕсли;
    
    // 2. Получаем полный путь из таблицы
    ПолныйПуть = ВыбраннаяСтрока.омг_Протокол;
    ПодключениеКФТП(ПолныйПуть);
    // 3. Разделяем путь на директорию и файл
	//РазделенныеЧасти = РазделитьПутьНаFTP(ПолныйПуть);
	//Директория = РазделенныеЧасти.Директория;
	//ИмяФайла = РазделенныеЧасти.ИмяФайла; 
	// // 4. Подключаемся к FTP 
	// 
	// // Указываем параметры подключения к FTP
	//FTPСервер = "178.89.186.68";
	//FTPПорт = "992";
	//FTPПорт = Число(FTPПорт);
	//Логин = "us_access";
	//Пароль = ".1gy$7WlkY1s";
	//	ssl4 = Новый ЗащищенноеСоединениеOpenSSL( неопределено, неопределено );
	//Попытка
	//       FTP = Новый FTPСоединение(FTPСервер,FTPПорт ,Логин,Пароль, ,Истина, ,ssl4);        
	//    // 5. Переходим в нужную директорию
	//    Если Не ПустаяСтрока(Директория) Тогда
	//        FTP.УстановитьТекущийКаталог(Директория);
	//    КонецЕсли;
	//	
	//	НайденныеФайлы = FTP.НайтиФайлы(ПолныйПуть);
	//	Если  НайденныеФайлы.Количество() <> 0 Тогда
	//		  FTPФайл = НайденныеФайлы[0]; 
	//	КонецЕсли;	  
	//	
	//	  // 6. Скачивание файла
	//    ВременныйФайл = ПолучитьИмяВременногоФайла("xml");
	//      
	//	FTP.Получить(FTPФайл.Имя, ВременныйФайл);
	//   	        
	//Исключение
	//    Сообщить("Ошибка FTP: " + ОписаниеОшибки());
	//    Возврат;
	//КонецПопытки;
	//
	//Попытка
	// // 7. Форматируем и показываем XML
	//   		//ПоказатьФорматированныйXML(ФорматированныйXML, ИмяФайла);
	//ФорматироватьXMLНаСервере(ВременныйФайл);
	// 
	//Исключение
...
Процедура  ПодключениеКФТП(ПолныйПуть) Экспорт
 РазделенныеЧасти = РазделитьПутьНаFTP(ПолныйПуть);
    Директория = РазделенныеЧасти.Директория;
    ИмяФайла = РазделенныеЧасти.ИмяФайла; 
	 // 4. Подключаемся к FTP 
...
КонецПроцедуры
&НаСервере
Функция ФорматироватьXMLНаСервере(ВременныйФайл) Экспорт
    ...
    Пока ЧтениеXML.Прочитать() Цикл
        Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
            ИмяЭлемента = ЧтениеXML.Имя;
			 ...
			 // Обрабатываем атрибуты до <f
            Если Лев(ИмяЭлемента, 2) = "ps" Тогда
                // Если элемент имеет атрибуты "Name", "Dept", "Created", сохраняем их
                ИмяАтрибутаName = ЧтениеXML.ЗначениеАтрибута("Name");
                ИмяАтрибутаDept = ЧтениеXML.ЗначениеАтрибута("Dept");
                ИмяАтрибутаCreated = ЧтениеXML.ЗначениеАтрибута("Created");
                
                Если ИмяАтрибутаName <> Неопределено Тогда
                    Объект.омг_НазваниеПротокола = ИмяАтрибутаName;
                КонецЕсли;
                
                Если ИмяАтрибутаDept <> Неопределено Тогда
                    Объект.омг_Клиника = ИмяАтрибутаDept;
                КонецЕсли;
                
                Если ИмяАтрибутаCreated <> Неопределено Тогда
                    Объект.омг_ДатаСозданияПротокола = ИмяАтрибутаCreated;
                КонецЕсли;
            КонецЕсли;
			
            // Если элемент начинается с "f", то обрабатываем его
            Если Лев(ИмяЭлемента, 1) = "f" Тогда
                // Проверяем наличие атрибутов Name и V
                ИмяАтрибутаName = ЧтениеXML.ЗначениеАтрибута("Name");
                ИмяАтрибутаV = ЧтениеXML.ЗначениеАтрибута("V");

                Если ИмяАтрибутаName <> Неопределено И ИмяАтрибутаV <> Неопределено Тогда
                    // Создаем новую строку в табличной части
                    НоваяСтрока = Объект.омг_РасшифровкаПротокола.Добавить();
                    НоваяСтрока.ПунктПротокола = ИмяАтрибутаName; // Записываем Name
                    НоваяСтрока.Значение = ИмяАтрибутаV; // Записываем V
                КонецЕсли;
            КонецЕсли;
        КонецЕсли;
    КонецЦикла;
...
&НаСервере
Функция РазделитьПутьНаFTP(ПолныйПуть)
    
    Результат = Новый Структура("Директория, ИмяФайла");
    
    // Ищем последний слеш
    ПозицияСлеша = СтрНайти(ПолныйПуть, "/", НаправлениеПоиска.СКонца);
    
    Если ПозицияСлеша > 0 Тогда
        Результат.Директория = Лев(ПолныйПуть, ПозицияСлеша);
        Результат.ИмяФайла = Сред(ПолныйПуть, ПозицияСлеша + 1);
    Иначе
        // Если слешей нет - считаем что это имя файла в корне
        Результат.Директория = "";
        Результат.ИмяФайла = ПолныйПуть;
    КонецЕсли;
    
    Возврат Результат;
    
КонецФункции
#Область доп_Phone // ОЛИМП:Phone
	...
	Если ТДСписокСобытий <> Неопределено Тогда
		...
		Иначе
			Если ЗначениеЗаполнено(ТДСписокСобытий.ЗаписьРазговора) Тогда
				доп_БитфонКлиент.ВоспроизвестиЗаписьРазговора(ТДСписокСобытий.ЗаписьРазговора);	
			Иначе
...
// ОМГ
&НаКлиенте
Процедура РискиПриИзменении(Элемент)

	//Если Объект.омг_РискПадения = ПредопределенноеЗначение("Перечисление.омг_РискиПадения.Низкий") Тогда
	//	Элементы.омг_РискПадения.ЦветФона = Webцвета.СветлоЗеленый;
	//иначе
	//	Элементы.омг_РискПадения.ЦветФона = Webцвета.ЛососьСветлый;
	//КонецЕсли;	
	
КонецПроцедуры	

&НаКлиенте
Процедура омг_ОткрытьДополнительнуюИнформацию(Команда)
		Элементы.ГруппаДопИнформации.Показать();		
		
КонецПроцедуры

&НаКлиенте
Процедура омг_СкрытьДополнительнуюИнформацию(Команда)
	Элементы.ГруппаДопИнформации.Скрыть();
КонецПроцедуры
...
Процедура ОткрытьМаршрутыЛечения(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(, "Сначала запишите карту клиента.");
		Возврат;
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура("ОтборПациент", Объект.Ссылка);
	ОткрытьФорму("Отчет.МаршрутЛеченияПациента.Форма.ФормаОтчета_85", ПараметрыФормы, ЭтотОбъект);
КонецПроцедуры
