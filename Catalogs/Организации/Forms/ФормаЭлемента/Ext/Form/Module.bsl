...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.МедицинскаяОрганизация.Видимость = Ложь;
...
&НаКлиенте
Процедура омг_БанковскиеСчетаОсновнойСчетПриИзменении(Элемент)
	
	Если Элементы.омг_БанковскиеСчета.ТекущиеДанные.ОсновнойСчет Тогда
		
		НомерСтрокиТек = Элементы.омг_БанковскиеСчета.ТекущиеДанные.НомерСтроки;	
		Для каждого стр из Объект.омг_БанковскиеСчета Цикл
			Если не стр.НомерСтроки = НомерСтрокиТек и стр.ОсновнойСчет Тогда	
				стр.ОсновнойСчет = Ложь
			КонецЕсли;
		КонецЦикла;
...
&НаКлиенте
Процедура омг_БанковскиеСчетаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
