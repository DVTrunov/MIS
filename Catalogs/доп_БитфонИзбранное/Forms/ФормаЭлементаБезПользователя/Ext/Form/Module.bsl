...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если НЕ ЗначениеЗаполнено(Объект.Пользователь) Тогда
		Объект.Пользователь = доп_ТелефонияСервер.ПолучитьТекущегоПользователя();
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Стр = доп_ТелефонияКлиентСервер.ОчиститьНомерТолькоЦифры(Объект.Номер);
	Объект.Номер = Стр;
	Если Модифицированность ИЛИ Объект.Ссылка = ПредопределенноеЗначение("Справочник.доп_БитфонИзбранное.ПустаяСсылка") Тогда
