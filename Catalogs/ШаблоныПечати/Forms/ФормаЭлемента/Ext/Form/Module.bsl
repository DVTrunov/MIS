...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого СтрокаПараметра Из Объект.ПараметрыПечати Цикл
		Если СтрокаПараметра.СтандартныйАлгоритм
			И Не ЗначениеЗаполнено(СтрокаПараметра.Значение)	
		Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не выбран стандартный алгоритм для параметра " + СтрокаПараметра.ИмяПараметра);
			Отказ = Истина;
		КонецЕсли;
	КонецЦикла; 
...
&НаКлиенте
Процедура ПараметрыПечатиСтандартныйАлгоритмПриИзменении(Элемент)
	
	ТекДанные = Элементы.ПараметрыПечати.ТекущиеДанные;
	Если ТекДанные <> Неопределено Тогда
		Если ТекДанные.СтандартныйАлгоритм Тогда
			ИмяСтандартногоАлгоритма = ОбщегоНазначенияСервер.ФункцияМенеджера("Справочники.ШаблоныПечати", "ПолучитьПараметрПоСтандартномуАлгоритму", ТекДанные.Значение);
			Если ЗначениеЗаполнено(ИмяСтандартногоАлгоритма) Тогда
				ТекДанные.Значение = РаботаСШаблонамиПечатиКлиентСервер.ПрефиксСтандартныхАлгоритмов() + ИмяСтандартногоАлгоритма;
			Иначе
				ШаблоныРеквизитовВыбор(Элементы.ПараметрыПечати,ТекДанные.ПолучитьИдентификатор(),Элементы.ШаблоныРеквизитовЗначение, Ложь, Истина);	
			КонецЕсли;
		Иначе
			ТекДанные.Значение = ПолучитьСтандартныйАлгоритмПараметра(ТекДанные.Значение);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьПараметрПослеВыбора(ЗначениеВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеВыбора <> Неопределено Тогда
		...
		СтрокаПараметра.СтандартныйАлгоритм = ЗначениеВыбора.СтандартныйАлгоритм;
		Если СтрокаПараметра.СтандартныйАлгоритм Тогда
			СтрокаПараметра.Значение = РаботаСШаблонамиПечатиКлиентСервер.ПрефиксСтандартныхАлгоритмов() + ИмяПараметра;
		Иначе
...
&НаКлиенте
Процедура ШаблоныРеквизитовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка, АвтоВыбор = Ложь)
	
	Если Поле = Элементы.ШаблоныРеквизитовЗначение Тогда
		СтандартнаяОбработка = Ложь;
		ТекДанные = Элементы.ПараметрыПечати.ТекущиеДанные;
		Если ТекДанные.СтандартныйАлгоритм Тогда
			Оповещение = Новый ОписаниеОповещения("ПослеВыбораСтандартногоАлгоритма", ЭтотОбъект, Новый Структура("ВыбраннаяСтрока, АвтоВыбор", Элементы.ПараметрыПечати.ТекущиеДанные.НомерСтроки, АвтоВыбор));
			ОткрытьФорму("Справочник.ШаблоныПечати.Форма.ВыборВстроенногоАлгоритмаПараметра",,ЭтотОбъект,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);			
		Иначе			
...
&НаКлиенте
Процедура ПослеВыбораСтандартногоАлгоритма(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено Тогда 
		Если Результат.СтандартныйАлгоритм Тогда
			Объект.ПараметрыПечати[ДополнительныеПараметры.ВыбраннаяСтрока - 1].Значение = РаботаСШаблонамиПечатиКлиентСервер.ПрефиксСтандартныхАлгоритмов() + Результат.ИмяПараметра;
		Иначе
			Объект.ПараметрыПечати[ДополнительныеПараметры.ВыбраннаяСтрока - 1].СтандартныйАлгоритм = Ложь;
			Объект.ПараметрыПечати[ДополнительныеПараметры.ВыбраннаяСтрока - 1].Значение = Результат.Алгоритм;
		КонецЕсли;
	ИначеЕсли ДополнительныеПараметры.АвтоВыбор Тогда
		Объект.ПараметрыПечати[ДополнительныеПараметры.ВыбраннаяСтрока - 1].СтандартныйАлгоритм = Ложь;	
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьСтандартныйАлгоритмПараметра(ПараметрПечати)
	
	Префикс = РаботаСШаблонамиПечатиКлиентСервер.ПрефиксСтандартныхАлгоритмов();
	ДлинаПрефикса = СтрДлина(Префикс);
	ИмяПараметра = Прав(ПараметрПечати, СтрДлина(ПараметрПечати) - ДлинаПрефикса);
	
	Возврат Справочники.ШаблоныПечати.ПолучитьСтандартныйАлгоритмПараметра(ИмяПараметра);
