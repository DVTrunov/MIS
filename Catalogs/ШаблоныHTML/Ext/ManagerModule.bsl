	...
	Для Каждого ШаблонКВыгрузке Из ШаблоныКВыгрузке Цикл
		...
		Для Каждого ШаблонОсмотра Из ШаблонИПодчиненные Цикл
			...
			Если Не ШаблонОсмотра.ЭтоГруппа Тогда
				...
				Для Каждого СтрокаКонстанты Из ШаблонОсмотра.КонстантыCDAДокумента Цикл
					Если ЗначениеЗаполнено(СтрокаКонстанты.ВычисляемоеЗначение)
						И ВычисляемыеЗначения.Найти(СтрокаКонстанты.ВычисляемоеЗначение) = Неопределено
					Тогда
						ВычисляемыеЗначения.Добавить(СтрокаКонстанты.ВычисляемоеЗначение);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Функция ПолучитьСтруктуруПарамтеровДляПроверитьИЗаменитьРеквизитыНаЭталонные()
	...
	ПропускаемыеТипы.Добавить(Тип("Дата"));
...
Процедура ПроверитьИЗаменитьРеквизитыНаЭталонные(ТекЭлементDOM, РезультатПоиска = Неопределено, ЭталоноеЗначение = Неопределено, СтруктураПараметров, МассивДобавленных)
	...
	Если РезультатПоиска <> Неопределено
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РезультатПоиска, "Тип")
		И ЭтоПримитивныйНеСоставнойТип(РезультатПоиска.Тип)
		И ТекЭлементDOM.ТекстовоеСодержимое = ""
	Тогда
		ТекЭлементDOM.ТекстовоеСодержимое = XMLСтрока(РезультатПоиска.Тип.ПривестиЗначение());
	КонецЕсли;
...
Функция ЭтоПримитивныйНеСоставнойТип(ОписаниеТипа)
	
	Типы = ОписаниеТипа.Типы();
	Возврат Типы.Количество() = 1
		И (Типы.Найти(Тип("Булево")) <> Неопределено
			Или Типы.Найти(Тип("Строка")) <> Неопределено
			Или Типы.Найти(Тип("Число")) <> Неопределено
			Или Типы.Найти(Тип("Дата")) <> Неопределено);
...
Функция ПолучитьЭталонныйXMLШаблона(Путь, ПолнаяЗагрузка = Истина) Экспорт
	...
	Если ПолнаяЗагрузка Тогда
		...
		ДобавитьСтрокуЭталона(ТаблицаЭталонов, "ЭлементКлассификатора",		Справочники.ВидыМедицинскихДокументов);
	КонецЕсли;
...
Функция ЭлементСТакимКодомСуществует(Данные)
	...
	Попытка
		ТребуетсяПроверкаУникальностиКода = МенеджерОбъекта.ТребуетсяПроверкаУникальностиКодаПриИмпортеШаблонаHTML();
	Исключение
		ТребуетсяПроверкаУникальностиКода = Истина;
	КонецПопытки;
	
	// Собственно проверка существования с этим кодом.
	Если МенеджерОбъекта <> Неопределено
		И ТребуетсяПроверкаУникальностиКода
...
Функция ТаблицаОбъектовКлассификатора(ВыгружаемыеШаблоны, КлассификаторыЕГИСЗ)
	...
	ТипыЗначенияПоиска = Новый Массив;
	ТипыЗначенияПоиска.Добавить(Тип("Число"));
	ТипыЗначенияПоиска.Добавить(Тип("Строка"));
	...
	Таблица.Колонки.Добавить("ЗначениеПоиска",		Новый ОписаниеТипов(ТипыЗначенияПоиска));
	Таблица.Колонки.Добавить("ПолеПоиска",			Новый ОписаниеТипов("Строка"));
	...
	Для Каждого ШаблонОсмотра Из ВыгружаемыеШаблоны Цикл
		...
		Если ЗначениеЗаполнено(ШаблонОсмотра.ВидДокумента) Тогда
			...
			НоваяСтрока.ЗначениеПоиска = ШаблонОсмотра.ВидДокумента.УИДЕГИСЗ;
			НоваяСтрока.ПолеПоиска = "УИДЕГИСЗ";
			...
			НоваяСтрока = Таблица.Добавить();
			НоваяСтрока.ЗначениеПоиска = ШаблонОсмотра.ВидДокумента.ВидДокумента.УИДЕГИСЗ;
			НоваяСтрока.ПолеПоиска = "УИДЕГИСЗ";
...
Функция ПолучитьТаблицыПараметровШаблона(ТекстHTML)
	...
	Для Каждого ЭлементТаблицы Из ЭлементыТаблиц Цикл
		ТаблицаШаблона = РаботаСШаблонамиHTML.ТаблицаШаблоновПоСтрокеУИД(ЭлементТаблицы.ПолучитьАтрибут("data-bit-dyntable-id"));
		Если ЗначениеЗаполнено(ТаблицаШаблона)
			И ТаблицыШаблона.Найти(ТаблицаШаблона) = Неопределено
		Тогда
			ТаблицыШаблона.Добавить(ТаблицаШаблона);
		КонецЕсли;
	КонецЦикла;
	
	ЭлементыКонтейнеровИнтерфейсовСТаблицами = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML,,, "data-bit-interface-dyntables");
	Для Каждого ЭлементКонтейнераИнтерфейса Из ЭлементыКонтейнеровИнтерфейсовСТаблицами Цикл
		ЗначениеАтрибутаТаблиц = ЭлементКонтейнераИнтерфейса.ПолучитьАтрибут("data-bit-interface-dyntables");
		Для Каждого НастройкаТаблицы Из СтрРазделить(ЗначениеАтрибутаТаблиц, ";", Ложь) Цикл
			ЭлементыОписанияТаблицы = СтрРазделить(НастройкаТаблицы, ":", Ложь);
			Если ЭлементыОписанияТаблицы.Количество() < 2 Тогда
				Продолжить;
			КонецЕсли;
			ТаблицаШаблона = РаботаСШаблонамиHTML.ТаблицаШаблоновПоСтрокеУИД(ЭлементыОписанияТаблицы[1]);
			Если ЗначениеЗаполнено(ТаблицаШаблона)
				И ТаблицыШаблона.Найти(ТаблицаШаблона) = Неопределено
			Тогда
				ТаблицыШаблона.Добавить(ТаблицаШаблона);
...
Процедура ОбработатьТаблицуСсылокНаКлассификаторыПриИмпорте(ТаблицаКлассификаторов)
	...
	Для Каждого СтрокаОбъекта Из ТаблицаКлассификаторов Цикл
		...
		Если ТаблицаКлассификаторов.Колонки.Найти("УИДЕГИСЗ") = Неопределено Тогда
			СуществующаяСсылка = НайтиЭлементКлассификатора(СтрокаОбъекта.Справочник, СтрокаОбъекта.ВидКлассификатора, СтрокаОбъекта.ПолеПоиска, СтрокаОбъекта.ЗначениеПоиска);
		Иначе
			СуществующаяСсылка = НайтиЭлементКлассификатораПоУИДЕГИСЗ(СтрокаОбъекта.Справочник, СтрокаОбъекта.ВидКлассификатора, СтрокаОбъекта.УИДЕГИСЗ);
		КонецЕсли;
...
Процедура ОбработатьКлассификаторыВШаблонеПриИмпорте(Данные, ТаблицаКлассификаторов, Отказ)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Данные, "УИДЕГИСЗ") Тогда
...
Процедура ДобавитьСтрокуКлассификатора(ЗначениеКлассификатора, Таблица, КлассификаторыЕГИСЗ)
	...
	Тогда
		...
		МенеджерСправочника = ОбщегоНазначения.МенеджерОбъектаПоСсылке(ЗначениеКлассификатора);
		
		Если ТипЗнч(ЗначениеКлассификатора) = Тип("СправочникСсылка.КлассификаторыМинЗдрава") Тогда
			ПорядокПоиска = МенеджерСправочника.ПолучитьПорядокПоискаСуществующихОбъектов(ЗначениеКлассификатора.ВидКлассификатора);
		Иначе
			Попытка
				ПорядокПоиска = МенеджерСправочника.ПолучитьПорядокПоискаСуществующихОбъектов();
			Исключение
				ПорядокПоиска = МенеджерСправочника.ПолучитьПорядокПоискаСуществующихОбъектов(ЗначениеКлассификатора.OIDСправочникаИсточника);
			КонецПопытки;
		КонецЕсли;
		
		КлючПоиска = Неопределено;
		Для Каждого ВариантПоиска Из ПорядокПоиска Цикл
			Для Каждого КлючЗначение Из ВариантПоиска Цикл
				КлючПоиска = КлючЗначение.Ключ;
				Прервать;
			КонецЦикла;
		...
		Если СтрНачинаетсяС(КлючПоиска, "ТЧ_Атрибуты_") Тогда
			ИмяАтрибутаПоиска = СтрЗаменить(КлючПоиска, "ТЧ_Атрибуты_", "");
			СтрокаАтрибута = ЗначениеКлассификатора.Атрибуты.Найти(ИмяАтрибутаПоиска, "Ключ");
			Если СтрокаАтрибута = Неопределено Тогда
				ВызватьИсключение(СтрШаблон(НСтр("ru='Отсутствует ключевой реквизит элемента классификатора, необходимо обновить классификаторы минздрава'"), Строка(ЗначениеКлассификатора)));
			КонецЕсли;
			НоваяСтрока.ЗначениеПоиска = ИмяАтрибутаПоиска;
		Иначе
			НоваяСтрока.ЗначениеПоиска = ЗначениеКлассификатора[КлючПоиска];
		КонецЕсли;
		
		НоваяСтрока.ПолеПоиска = КлючПоиска;
		...
		Если ЗначениеКлассификатора.Метаданные().Иерархический
			И ЗначениеЗаполнено(ЗначениеКлассификатора.Родитель)
			И КлассификаторыЕГИСЗ.Найти(ЗначениеКлассификатора.Родитель) = Неопределено
		Тогда
			ДобавитьСтрокуКлассификатора(ЗначениеКлассификатора.Родитель, Таблица, КлассификаторыЕГИСЗ);
		КонецЕсли;
	КонецЕсли;
...
Функция НайтиЭлементКлассификатораПоУИДЕГИСЗ(ИмяСправочника, ВидКлассификатора, УИДЕГИСЗ)
...
Функция НайтиЭлементКлассификатора(ИмяСправочника, ВидКлассификатора, ПолеПоиска, ЗначениеПоиска)
	
	НайденныйЭлемент = Неопределено;
	
	Если ЗначениеЗаполнено(ВидКлассификатора) Тогда
		ВидКлассификатораДляПоиска = ВидКлассификатора;
	Иначе
		ВидКлассификатораДляПоиска = Неопределено;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ВидКлассификатораДляПоиска)
		И СтрНачинаетсяС(ПолеПоиска, "ТЧ_Атрибуты_")
	Тогда
		ИмяАтрибутаПоиска = СтрЗаменить(ПолеПоиска, "ТЧ_Атрибуты_", "");
		НайденныйЭлемент = ИнтеграцияЕГИСЗСервер.ПолучитьЭлементКлассификатораПоЗначениюАтрибута(ИмяАтрибутаПоиска, ЗначениеПоиска, ВидКлассификатораДляПоиска,, Истина);
	ИначеЕсли ПолеПоиска = "УИДЕГИСЗ" Тогда
		НайденныйЭлемент = ИнтеграцияЕГИСЗОбщегоНазначенияПовтИсп.ЭлементКлассификатораПоУИДЕГИСЗ(ЗначениеПоиска, ВидКлассификатораДляПоиска, ИмяСправочника);
	Иначе
		НайденныйЭлемент = ИнтеграцияЕГИСЗОбщегоНазначенияПовтИсп.ЭлементКлассификатораПоЗначению(ЗначениеПоиска, ВидКлассификатораДляПоиска, ИмяСправочника, СтрШаблон("КлассификаторыМинЗдрава.%1 = &ЗначениеДляПоиска", ПолеПоиска));
	КонецЕсли;
