...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Добавлено Макеев (
	КмЗаполнитьПоЗначения = Команды.Добавить("омг_ЗаполнитьПоЗначениям");
	КмЗаполнитьПоЗначения.Действие = "омг_ЗаполнитьПоЗначениям";
	КмЗаполнитьПоЗначения.Заголовок = "Заполнить по значениям";
	
	ЭлЗаполнитьПоЗначения = Элементы.Добавить("омг_ЗаполнитьПоЗначениям", Тип("КнопкаФормы"), Элементы.СписокВыбораКоманднаяПанель);
	ЭлЗаполнитьПоЗначения.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
	ЭлЗаполнитьПоЗначения.ИмяКоманды = "омг_ЗаполнитьПоЗначениям";
	// )
КонецПроцедуры
...
Функция омг_ПолучитьСписокВыбора(Параметр)
	
	СписокВыбора = Новый СписокЗначений;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ПараметрыHTMLЗначенияВыбора.Значение КАК Значение,
	               |	ПараметрыHTMLЗначенияВыбора.Представление КАК Представление
	               |ИЗ
	               |	ПланВидовХарактеристик.ПараметрыHTML.ЗначенияВыбора КАК ПараметрыHTMLЗначенияВыбора
	               |ГДЕ
	               |	ПараметрыHTMLЗначенияВыбора.Ссылка = &Ссылка";
	Запрос.УстановитьПараметр("Ссылка", Параметр);
	РезультатЗапроса = Запрос.Выполнить();
	Если Не РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать();
		Пока Выборка.Следующий() Цикл
			СтрокаСписка = СписокВыбора.Добавить();
			СтрокаСписка.Значение = Выборка.Значение;
			СтрокаСписка.Представление = Выборка.Представление;
		КонецЦикла;
	КонецЕсли; 
...
&НаКлиенте
Процедура омг_ЗаполнитьПоЗначениям()
	
	СписокВыбора.Очистить();
	
	СписокЗначенийВыбора = омг_ПолучитьСписокВыбора(Параметр);
	Для Каждого СтрокаСписка Из СписокЗначенийВыбора Цикл
		ЗаполнитьЗначенияСвойств(СписокВыбора.Добавить(), СтрокаСписка);
	КонецЦикла;
