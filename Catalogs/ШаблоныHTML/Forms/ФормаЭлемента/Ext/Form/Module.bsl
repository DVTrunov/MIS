...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.ВидШаблона <> ПредопределенноеЗначение("Перечисление.ВидыШаблонов.HTMLШаблон")
	...
	ПроверитьЗаполненностьНастроекИнтерфейсов(Объект.ТекстHTML);
	...
	Если ИмяСобытия = "СобытиеЗакрытияФормыВыбораПараметраHTML" Тогда
		...
		Если ИмяКоманды = "КонтекстноеМенюИзменение" Тогда
			...
			Тогда
				...
				ЭлементПараметраHTML = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML, ДанныеЭлементаHTMLДляКонтекстногоМеню.id,,, Истина);
				Если ЭлементПараметраHTML <> Неопределено Тогда
					...
					ЭлементПараметраHTML.id = УИДНовогоПараметра;
				КонецЕсли;
...
Процедура ОбработкаПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	БлижайшийЭлементКнопки = РаботаСHTMLКлиентСервер.ПолучитьБлижайшийУзел(ДанныеСобытия.Element, "button");
	Если БлижайшийЭлементКнопки <> Неопределено Тогда
		ОбработатьНажатиеКнопки(БлижайшийЭлементКнопки);
	КонецЕсли;
...
Асинх Процедура ОбработатьНажатиеКнопки(ЭлементHTMLКнопка)
	...
	ИдентификаторИнтерфейсаЗаполненияПараметров = ЭлементHTMLКнопка.getAttribute("data-bit-interface-id");
	
	Если ЗначениеЗаполнено(ИдентификаторИнтерфейсаЗаполненияПараметров) Тогда
		РаботаСИнтерфейсамиЗаполненияПараметровКлиент.ОбработатьНажатиеКнопкиОткрытияИнтерфейсаЗаполненияПараметров(ЭлементHTMLКнопка, ИдентификаторИнтерфейсаЗаполненияПараметров);
		Возврат;
	КонецЕсли;
...
Функция ПроверитьИзменениеРеквизитовШаблона()
	...
	Если ПредШаблон.ТекстHTML <> РаботаСШаблонамиHTMLКлиентСервер.НормализоватьОтступы(Объект.ТекстHTML)
		...
		Или ТаблицаИзменилась("КонстантыCDAДокумента", "localid, Значение, ГруппирующееПоле, КлассификацияПоля, Вычисляется, ВычисляемоеЗначение")
...
Функция СформироватьТекстСообщенияСТребованиемПоВерсионированию(ДокументHTML)
	...
	ИначеЕсли РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML,,, "data-bit-interface-id").Количество() > 0 Тогда
		Возврат НСтр("ru='В шаблоне используются интерфейсы заполнения параметров. Необходимо создать новую версию шаблона.'");
	КонецЕсли;
	...
	ИначеЕсли РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ИсходныйДокументHTML,,, "data-bit-interface-id").Количество() > 0 Тогда
		Возврат НСтр("ru='В исходном тексте шаблона использовались интерфейсы заполнения параметров. Необходимо создать новую версию шаблона.'");
	КонецЕсли;
...
Процедура ОбработкаПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Элементы.РежимСкрытияТега.Доступность		= Ложь;
...
Процедура СписокПараметровПриАктивизацииСтроки(Элемент)
	...
	Если ЗначениеЗаполнено(ТекПараметрВид) Тогда
		...
		КонецЦикла;
		ТекПараметрЗаголовок = РаботаСШаблонамиHTML.ПредставлениеПараметраШаблона(ТекДанные.Ссылка) + ":";
	Иначе
...
Процедура ПослеВводаКомментария(ТекстКомментария, Параметры) Экспорт
	...
	ШаблонТекстаЭлемента = 
		"<div data-bit-hidden='true' style='font-size:small;color:#888888;font-style:italic;text-align:left'>
		|	&lt;%1&gt;
		|</div>";
...
Процедура ЗаполнитьКешПараметров()
	...
	Для Каждого СтрокаПараметр Из Объект.СоответствияПараметрам Цикл
		...
		НоваяСтрока.НаименованиеДляВрачаВОбъединяющемПоле = СтрокаПараметр.НаименованиеДляВрачаВОбъединяющемПоле;
...
Процедура КешДобавляемыеПараметрыПриАктивизацииСтроки(Элемент)
	...
	ЭлементПоУИД = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML, ТекДанные.СтрокаУидПараметра,,, Истина);
	...
	ВыделитьЭлементHTML(ДокументHTML, ЭлементПоУИД);
	РежимВставки = 1;
...
Процедура ПолеHTMLДокументаПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если ТекущийЭлементШаблонаHTMLИмеющийTag.hasAttribute("data-bit-hidden") Тогда
		РежимСкрытияТега = 1;
	ИначеЕсли ТекущийЭлементШаблонаHTMLИмеющийTag.hasAttribute("data-bit-always-hidden") Тогда
		РежимСкрытияТега = 2;
	ИначеЕсли ТекущийЭлементШаблонаHTMLИмеющийTag.hasAttribute("data-bit-edity-hidden") Тогда
		РежимСкрытияТега = 3;
	КонецЕсли;
	...
	Если ТегиПоддерживающиеСкрываемость.Найти(ИмяТега) <> Неопределено Тогда
		Элементы.РежимСкрытияТега.Доступность		= Истина;
	...
	Если ЭлементHTML <> Неопределено Тогда
		...
		Пока Не ОбъектМожноМенять И ИмяТега <> "body" Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(ИмяТега)
				И ПредставлениеИменТегов.Получить(ИмяТега) <> Неопределено
			Тогда
				ВыбранныйЭлементHTML = ПредставлениеИменТегов.Получить(ИмяТега);
			Иначе
			...
			ИначеЕсли ИмяТега	= "img" Тогда
				...
				Если ЭлементHTML.hasAttribute("data-bit-selected-img-id") Тогда
					ЗаполнитьНастройкиДобавленияИзображения(ЭлементHTML)
				КонецЕсли;
...
Процедура ОбновитьПараметрыTDЭлемента()
	...
	ТекущийЭлементШаблонаHTML = ПолучитьТекущийЭлементШаблонаHTML();
	Если ТекущийЭлементШаблонаHTML = Неопределено Тогда
		СообщитьНетТекущегоТега();
		Возврат;
	КонецЕсли;
...
Процедура СкрыватьТегПриИзменении(Элемент)
	...
	ТекущийЭлементШаблонаHTMLИмеющийTag.removeAttribute("data-bit-always-hidden");
	ТекущийЭлементШаблонаHTMLИмеющийTag.removeAttribute("data-bit-edity-hidden");
	
	Если РежимСкрытияТега = 1 Тогда
		ТекущийЭлементШаблонаHTMLИмеющийTag.setAttribute("data-bit-hidden","true");
	ИначеЕсли РежимСкрытияТега = 2 Тогда
		ТекущийЭлементШаблонаHTMLИмеющийTag.setAttribute("data-bit-always-hidden","true");
	ИначеЕсли РежимСкрытияТега = 3 Тогда
		ТекущийЭлементШаблонаHTMLИмеющийTag.setAttribute("data-bit-edity-hidden","true");
	КонецЕсли;
	ОбновитьДеревоТегов();
	ТекущийЭлементШаблонаHTMLИмеющийTag.click();
...
Процедура ЗаполнитьДеревоТегов(Документ, УровеньДерева, НомерУзла)
	...
	Если РаботаСHTMLКлиентСервер.ЕстьАтрибутВТекстеТега("data-bit-hidden", НужныеСвойстваУзла.outerHTML, НужныеСвойстваУзла.innerHTML) Тогда
		СтрокаДереваТегов.СкрыватьТег = Истина;
		СтрокаДереваТегов.РежимСкрытияТега = 1;
	ИначеЕсли РаботаСHTMLКлиентСервер.ЕстьАтрибутВТекстеТега("data-bit-always-hidden", НужныеСвойстваУзла.outerHTML, НужныеСвойстваУзла.innerHTML) Тогда
		СтрокаДереваТегов.СкрыватьТег = Истина;
		СтрокаДереваТегов.РежимСкрытияТега = 2;
	ИначеЕсли РаботаСHTMLКлиентСервер.ЕстьАтрибутВТекстеТега("data-bit-edity-hidden", НужныеСвойстваУзла.outerHTML, НужныеСвойстваУзла.innerHTML) Тогда
		СтрокаДереваТегов.СкрыватьТег = Истина;
		СтрокаДереваТегов.РежимСкрытияТега = 3;
	Иначе
		СтрокаДереваТегов.СкрыватьТег = Ложь;
		СтрокаДереваТегов.РежимСкрытияТега = 0;
	КонецЕсли;
...
Процедура ДеревоТеговПриАктивизацииСтроки(Элемент)
	...
	Иначе
		...
		Если СтрокаДерева = Неопределено Тогда
			Возврат;
		КонецЕсли;
		РежимСкрытияТега = СтрокаДерева.РежимСкрытияТега;
...
Процедура ПриОткрытииЗаполнитьКонстантыФормы()
	...
	ПредставлениеИменТегов = Новый Соответствие;
	...
	Для Каждого ПараметрШаблона Из ПараметрыШаблонаПоПорядку Цикл
		...
		Иначе
			...
			Если СтрокиТаблицыПараметра.Количество() > 0 Тогда
				...
				ЗаполнитьЗначенияСвойств(СтрокаПараметра, СтрокиТаблицыПараметра[0], "localid, КлассификацияПоля, ГруппирующееПоле, ОбъединяющееПоле, ВидКлассификатора, НаименованиеДляВрачаВОбъединяющемПоле");
				Модифицированность = Истина;
			КонецЕсли;
			...
			//Если ЗначениеЗаполнено(СтрокаПараметра.localid) Тогда
				...
				Если СтрокиLocalid.Количество() > 0 Тогда
					...
					Иначе
						ОчиститьСтрокуПараметра = Не ОбщегоНазначенияБИТКлиентСервер.ТипыДанныхСовпадают(СтрокаПараметра.ТипЗначения.Типы(), СтрокиLocalid[0].ТипЗначения.Типы());
					КонецЕсли;
				КонецЕсли;
...
Процедура КонтекстноеМенюИзменитьПолеВыбора(Команда)
	...
	КонтекстноеМенюПараметр = КонтекстноеМенюПолучитьТекущийПараметр();
	Если КонтекстноеМенюПараметр <> Неопределено Тогда
		ЭлементПараметраHTML = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(Элементы.ПолеHTMLДокумента.Документ, ДанныеЭлементаHTMLДляКонтекстногоМеню.id,,, Истина);
		МассивЭлементовВыбора = Новый Массив;
		НомерСтрокиПоУмолчанию = Неопределено;
		Для Каждого ЭлементВыбора Из ЭлементПараметраHTML.querySelectorAll("option") Цикл
		...
		Если СписокВыбора <> Неопределено Тогда
			...
			ЭлементПараметраHTML.innerHTML = ТекстHTMLЭлементовВыбора;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьДоступностьКнопокКонтекстногоМенюПоляHTML()
	...
	Элементы.ПолеHTMLДокументаКонтекстноеМенюНастройкиПараметровИнтерфейса.Видимость = Ложь;
	...
	Тогда
		...
		Тогда
			Элементы.ПолеHTMLДокументаКонтекстноеМенюНастройкиПараметровИнтерфейса.Видимость = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		Иначе
			...
			Если Объект.Ссылка.Пустая() Тогда
				Записать();
			КонецЕсли;
			ОбработкаОбъект.ШаблонHTML = Объект.Ссылка;
			// )
			ПараметрыШаблона = ОбработкаОбъект.ПолучитьСписокПараметров();
		КонецЕсли;
	Исключение
...
Процедура КонтекстноеМенюНастройкиПараметровИнтерфейса(Команда)
	...
	ПараметрыОткрытия.Вставить("ТекущийТекстHTML",			Элементы.ПолеHTMLДокумента.Документ.body.outerHTML);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеНастроекПараметровИнтерфейса", ЭтотОбъект, Новый Структура("Идентификатор", ПараметрыОткрытия.Идентификатор));
	ОткрытьФорму("Обработка.ИнтерфейсыЗаполненияПараметровШаблоновПриема.Форма.НастройкиЭлементаHTML", ПараметрыОткрытия, ЭтотОбъект,,,, ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеНастроекПараметровИнтерфейса(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПоляТаблицыИнтерфейса = РаботаСИнтерфейсамиЗаполненияПараметровКлиентСервер.ПоляТаблицыИнтерфейса(ДополнительныеПараметры.Идентификатор);
	
	УзлыОписанияИнтерфейса = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(Элементы.ПолеHTMLДокумента.Документ, ДополнительныеПараметры.Идентификатор,, "data-bit-interface-id");
	
	Для Каждого УзелОписанияИнтерфейса Из УзлыОписанияИнтерфейса Цикл
		УзелОписанияИнтерфейса.innerHTML = "";
		УзелОписанияИнтерфейса.setAttribute("contenteditable", Ложь);
		
		УзелЗаголовка = УзелОписанияИнтерфейса.ownerDocument.createElement("span");
		УзелЗаголовка.setAttribute("data-bit-interface-title", "true");
		УзелЗаголовка.appendChild(УзелОписанияИнтерфейса.ownerDocument.createTextNode(Результат.ЗаголовокКнопкиИнтерфейса));
		УзелОписанияИнтерфейса.appendChild(УзелЗаголовка);
		
		УзлыПараметров = Новый Массив;
		Для Каждого СтруктураПараметраПоля Из ПоляТаблицыИнтерфейса.Поля Цикл
			
			ИдентификаторПараметра = Результат.ПараметрыПолейИнтерфейса.Получить(СтруктураПараметраПоля.Имя);
			
			УзелПараметра = УзелОписанияИнтерфейса.ownerDocument.createElement("a");
			Если ЗначениеЗаполнено(ИдентификаторПараметра) Тогда
				УзелПараметра.id = ИдентификаторПараметра;
			КонецЕсли;
			
			УзелПараметра.setAttribute("data-bit-interface-parameter-name", СтруктураПараметраПоля.Имя);
			УзлыПараметров.Добавить(УзелПараметра);
		КонецЦикла;
		
		УзелКонтейнера = УзелОписанияИнтерфейса.ownerDocument.createElement("div");
		УзелКонтейнера.setAttribute("data-bit-interface-parameters", "true");
		УзелКонтейнера.setAttribute("style", "display:none");
		
		ЧастиАтрибутаТаблицПараметров = Новый Массив;
		
		Для Каждого СтруктураТаблицы Из ПоляТаблицыИнтерфейса.Таблицы Цикл
			ИдентификаторТаблицы = Результат.ДанныеТаблицПараметров.Получить(СтруктураТаблицы.Имя);
			Если ЗначениеЗаполнено(ИдентификаторТаблицы) Тогда
				ЧастиАтрибутаТаблицПараметров.Добавить(СтрШаблон("%1:%2", СтруктураТаблицы.Имя, ИдентификаторТаблицы));
			Иначе
				ЧастиАтрибутаТаблицПараметров.Добавить(СтруктураТаблицы.Имя);
			КонецЕсли;
		КонецЦикла;
		УзелКонтейнера.setAttribute("data-bit-interface-dyntables", СтрСоединить(ЧастиАтрибутаТаблицПараметров, ";"));
		
		УзелОписанияИнтерфейса.appendChild(УзелКонтейнера);
		Для Каждого УзелПараметра Из УзлыПараметров Цикл
			УзелКонтейнера.appendChild(УзелПараметра);
		КонецЦикла;
	КонецЦикла;
	
	ЗаполнитьКешПараметровПоШаблону(Элементы.ПолеHTMLДокумента.Документ);
...
Процедура ЗаполнитьПеременныеИнтерфейсовЗаполненияПараметров(ДанныеСобытия)
	...
	ИдентификаторИнтерфейса = ЭлементИнтерфейсаЗаполненияПараметров.getAttribute("data-bit-interface-id");
	...
	ДанныеТекущегоИнтерфейсаЗаполненияПараметров.Идентификатор = ИдентификаторИнтерфейса;
	ДанныеТекущегоИнтерфейсаЗаполненияПараметров.ЗаголовокКнопки = РаботаСИнтерфейсамиЗаполненияПараметровКлиентСервер.ЗаголовокИнтерфейса(ЭлементИнтерфейсаЗаполненияПараметров, ИдентификаторИнтерфейса);
...
&НаСервереБезКонтекста
Процедура ПроверитьЗаполненностьНастроекИнтерфейсов(ТекстHTML)
	
	Если СтрНайти(ТекстHTML, "data-bit-interface-id") = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДокументHTML = РаботаСDOMКлиентСервер.СоздатьДокументDOM(ТекстHTML);
	Для Каждого ЭлементИнтерфейса Из РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML,,, "data-bit-interface-id") Цикл
		ИдентификаторИнтерфейса = ЭлементИнтерфейса.ПолучитьАтрибут("data-bit-interface-id");
		
		Если РаботаСИнтерфейсамиЗаполненияПараметровКлиентСервер.ИнтерфейсЗаполнения(ИдентификаторИнтерфейса) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ТекстыОшибокПоИнтерфейсу = Новый Массив;
		
		// Те настройки, которые указаны в тексте HTML
		СтруктурыТаблицПараметров = РаботаСИнтерфейсамиЗаполненияПараметровКлиентСервер.ПолучитьСтруктурыТаблицПараметровЭлементаИнтерфейса(ЭлементИнтерфейса, ТекстыОшибокПоИнтерфейсу);
		
		// Те настройки, которые должны быть в интерфейсе в текущем релизе
		...
		ДанныеТаблиц = Новый Соответствие;
		
		Если СтруктурыТаблицПараметров <> Неопределено Тогда
			Для Каждого СтруктураПоля Из СтруктурыТаблицПараметров.ПараметрыПолей Цикл
				ДанныеПолей.Вставить(СтруктураПоля.ИмяПоля, Новый Структура("Идентификатор", СтруктураПоля.Идентификатор));
			КонецЦикла;
			
			Для Каждого СтруктураТаблицы Из СтруктурыТаблицПараметров.ТаблицыПараметровИнтерфейса Цикл
				ДанныеТаблиц.Вставить(СтруктураТаблицы.ИмяТаблицы, Новый Структура("Идентификатор", СтруктураТаблицы.Идентификатор));
			КонецЦикла;
		КонецЕсли;
		
		// Необходимо убедиться, что набор полей/таблиц интерфейса совпадает с тем, который предполагается текущим релизом.
		// Например, если в очередном релизе в интерфейсе добавилась новая таблица - настройки в тексте узла интерфейса не содержат
		// информации о ней (т.к. на момент формирования текста этого узла этой таблицы не существовало). Нужно сообщить об этом пользователю.
		Для Каждого СтруктураПоля Из ПоляТаблицыИнтерфейса.Поля Цикл
			ДанныеПоля = ДанныеПолей.Получить(СтруктураПоля.Имя);
			Если ДанныеПоля <> Неопределено
				И ЗначениеЗаполнено(ДанныеПоля.Идентификатор)
				И Не ОбщегоНазначения.СсылкаСуществует(ПланыВидовХарактеристик.ПараметрыHTML.ПолучитьСсылку(ДанныеПоля.Идентификатор))
			Тогда
				ТекстыОшибокПоИнтерфейсу.Добавить(СтрШаблон(НСтр("ru='Не найден параметр ""%1""'"), Строка(ДанныеПоля.Идентификатор)));
			КонецЕсли;
		КонецЦикла;
		Для Каждого СтруктураТаблицы Из ПоляТаблицыИнтерфейса.Таблицы Цикл
			ДанныеТаблицы = ДанныеТаблиц.Получить(СтруктураТаблицы.Имя);
			Если ДанныеТаблицы <> Неопределено
				И ЗначениеЗаполнено(ДанныеТаблицы.Идентификатор)
				И Не ОбщегоНазначения.СсылкаСуществует(Справочники.ТаблицыШаблоновПриема.ПолучитьСсылку(ДанныеТаблицы.Идентификатор))
			Тогда
				ТекстыОшибокПоИнтерфейсу.Добавить(СтрШаблон(НСтр("ru='Не найдена таблица ""%1""'"), Строка(ДанныеТаблицы.Идентификатор)));
			КонецЕсли;
		КонецЦикла;
		
		Если ТекстыОшибокПоИнтерфейсу.Количество() > 0 Тогда
			ЗаголовокИнтерфейса = РаботаСИнтерфейсамиЗаполненияПараметровКлиентСервер.ЗаголовокИнтерфейса(ЭлементИнтерфейса, ИдентификаторИнтерфейса);
			ТекстыОшибокПоИнтерфейсу.Вставить(0, СтрШаблон(НСтр("ru='В настройке интерфейса ""%1"" допущены ошибки:'"), ЗаголовокИнтерфейса));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрСоединить(ТекстыОшибокПоИнтерфейсу, Символы.ПС + Символы.Таб));
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура СообщитьНетТекущегоТега()
	
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не удалось определить текущую позицию в шаблоне. Текущий элемент не найден.'"));	
...
Процедура ВидДокументаПриИзменении(Элемент)
	...
	// Сообщаем о том, что регистрация данного вида документа в РЭМД скоро прекратится
	ПроверитьНаличиеДатыОкончанияРегистрацииВидаЭМД(Объект.ВидДокумента, ИспользуетсяИнтеграцияРЭМД);
...
Процедура ПараметрПриИзменении(Элемент)
	...
	ТекДанные = ИнтеграцияЕГИСЗ_СЭМДКлиент.ТаблицаРодительЭлемента(Элемент).ТекущиеДанные;
...
Процедура ТаблицаКонстантЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИнтеграцияЕГИСЗ_СЭМДКлиент.ТаблицаКонстантЗначениеНачалоВыбора(Элемент, ТаблицаLocalidМакета, Объект.КонстантыCDAДокумента, ИнтеграцияЕГИСЗ_СЭМДКлиент.ТаблицаРодительЭлемента(Элемент).ТекущиеДанные);
...
Процедура ТаблицаКонстантЗначениеАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	ИнтеграцияЕГИСЗ_СЭМДКлиент.ТаблицаКонстантЗначениеАвтоПодбор(Элемент, ТаблицаLocalidМакета, Объект.КонстантыCDAДокумента, ИнтеграцияЕГИСЗ_СЭМДКлиент.ТаблицаРодительЭлемента(Элемент).ТекущиеДанные);
...
Процедура ЗаполнитьТипыЗначенийПараметров()
	...
	Если Объект.ВидШаблона = Перечисления.ВидыШаблонов.Обработка Тогда
		...
		КоллекцииПараметров.Добавить(Объект.СоответствияПараметрам);
		КоллекцииПараметров.Добавить(Объект.СоответствияТаблицамШаблонов);
	...
	Иначе
		КоллекцииПараметров = Новый Массив;
		КоллекцииПараметров.Добавить(ТаблицаПараметров);
		КоллекцииПараметров.Добавить(Объект.СоответствияПараметрам);
...
Функция ПолучитьСтруктуруПараметровПоСекциям()
	...
	Для Каждого СтрокаПараметра Из ТаблицаПараметров Цикл
		...
		ЭлементПоУИД = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(Элементы.ПолеHTMLДокумента.Документ, СтрокаПараметра.СтрокаУидПараметра,,, Истина);
		Если ЭлементПоУИД = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПроверитьНаличиеДатыОкончанияРегистрацииВидаЭМД(ВидДокумента, ИспользуетсяИнтеграцияРЭМД)
	
	ТекстПредупреждения = "";
	ЗаполнитьТекстПредупрежденияДатыОкончанияРегистрацииВидаЭМД(ВидДокумента, ТекстПредупреждения, ИспользуетсяИнтеграцияРЭМД);
...
&НаСервереБезКонтекста
Процедура ЗаполнитьТекстПредупрежденияДатыОкончанияРегистрацииВидаЭМД(ВидДокумента, ТекстПредупреждения, ИспользуетсяИнтеграцияРЭМД)
	
	Если ИспользуетсяИнтеграцияРЭМД
		И ЗначениеЗаполнено(ВидДокумента)
	Тогда
		СтрокаДатаОкончанияРегистрации = ВидДокумента.Атрибуты.Найти("END_DATE", "Ключ");
		Если СтрокаДатаОкончанияРегистрации <> Неопределено
			И ЗначениеЗаполнено(СтрокаДатаОкончанияРегистрации.Значение)
		Тогда
			ТекстПредупреждения = СтрШаблон(НСтр("ru='Документы с видом ""%1"" с %2 не принимаются РЭМД в связи с окончанием срока регистрации.'"), ВидДокумента.НаименованиеПолное, СтрокаДатаОкончанияРегистрации.Значение);
		КонецЕсли;
	КонецЕсли;
