...
//  Массив.
//
Функция ПолучитьИдентификаторыДоступныеРЭМД(ТабличныйШаблонСправки = Ложь, МожноPDF = Истина, ВключатьЗаполняемыеПрограммно = Ложь) Экспорт
	...
	Если Не ТабличныйШаблонСправки Тогда
		...
		СуществующиеCDAРЭМД.Добавить(158);
	...
	СуществующиеCDAРЭМД.Добавить(141);
	...
	СуществующиеCDAРЭМД.Добавить(196);
	// Медицинское заключение (CDA) Редакция 1
	СуществующиеCDAРЭМД.Добавить(204);
	
	// Для отображения всех поддерживаемых из реализованных на текущий момент
	Если ВключатьЗаполняемыеПрограммно Тогда
		...
		СуществующиеCDAРЭМД.Добавить(230);
	КонецЕсли;
	
	// Не поддерживаются
	Если Ложь Тогда
		//Эпикриз по законченному случаю амбулаторный (CDA) Редакция 3
		СуществующиеCDAРЭМД.Добавить(42);
		...
		СуществующиеCDAРЭМД.Добавить(99);
		// Эпикриз в стационаре выписной (CDA) Редакция 5
		СуществующиеCDAРЭМД.Добавить(147);
