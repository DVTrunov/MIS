...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоСтандартныйАлгоритм = Справочники.ВычисляемыеЗначенияПараметров.ЭтоКлючСтандартногоАлгоритма(Объект.АлгоритмВычисления);
	
	Если ЭтоСтандартныйАлгоритм Тогда
		Алгоритмы = Справочники.ВычисляемыеЗначенияПараметров.ПолучитьСтандартныеАлгоритмыВычисляемыхЗначений(Истина);
		СтруктураАлгоритма = Алгоритмы.Получить(Объект.АлгоритмВычисления);
		Алгоритм = СтруктураАлгоритма.Алгоритм;
		Комментарий = СтруктураАлгоритма.Комментарий;
		Элементы.НадписьОписание.Заголовок = СтруктураАлгоритма.Описание;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	НастроитьЭлементыФормыПоСтандартномуАлгоритму();
...
&НаКлиенте
Процедура СтандартныйАлгоритмНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыФормы = Новый Структура("Получатель", Объект.ПолучательВычисляемогоЗначения);
	Оповещение = Новый ОписаниеОповещения("ВыборСтандартногоАлгоритма_Завершение", ЭтотОбъект);
	ОткрытьФорму("Справочник.ВычисляемыеЗначенияПараметров.Форма.ВыборСтандартногоАлгоритма",ПараметрыФормы,ЭтотОбъект,,,,Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
КонецПроцедуры

&НаКлиенте
Процедура СтандартныйАлгоритмОчистка(Элемент, СтандартнаяОбработка)

	СтандартнаяОбработка = Ложь;
	
	Объект.АлгоритмВычисления = "";
	Алгоритм = "";
	Комментарий = "";
...
&НаКлиенте
Асинх Процедура ЭтоСтандартныйАлгоритмПриИзменении(Элемент)
	
	Модифицированность = Истина;
	
	Если ЭтоСтандартныйАлгоритм Тогда
		Если ЗначениеЗаполнено(Объект.АлгоритмВычисления) Тогда
			Ответ = Ждать ВопросАсинх("При выборе стандартного алгоритма, текущий алгоритм будет затерт. Продолжить?", РежимДиалогаВопрос.ДаНет); 
			Если Ответ = КодВозвратаДиалога.Да Тогда 
				Объект.АлгоритмВычисления = "";
				Объект.СвязанныеРеквизиты.Очистить();
			Иначе
				ЭтоСтандартныйАлгоритм = Ложь;
			КонецЕсли;	
		КонецЕсли;
	Иначе
		Объект.АлгоритмВычисления = Алгоритм;
		Алгоритм = "";
		Комментарий = "";
...
&НаКлиенте
Процедура ВыборСтандартногоАлгоритма_Завершение(Результат, ДополнительныеПараметры) Экспорт

	Если Результат <> Неопределено Тогда 
		
		Модифицированность = Истина;
		
		Объект.АлгоритмВычисления = РаботаСШаблонамиПечатиКлиентСервер.ПрефиксСтандартныхАлгоритмов() + Результат.ИмяАлгоритма;
		Алгоритм = Результат.Алгоритм;
		Комментарий = Результат.Комментарий;
		Элементы.НадписьОписание.Заголовок = Результат.Представление;
		
		МассивРеквизитов = СтрРазделить(Результат.СвязанныеПоля, ",", Ложь);
		
		Объект.СвязанныеРеквизиты.Очистить();	
		Для Каждого Реквизит Из МассивРеквизитов Цикл
			СтрокаРеквизита = Объект.СвязанныеРеквизиты.Добавить();	
			СтрокаРеквизита.Реквизит = СокрЛП(Реквизит);
		КонецЦикла;
...
&НаКлиенте
Процедура НастроитьЭлементыФормыПоСтандартномуАлгоритму()
	
	Если ЭтоСтандартныйАлгоритм Тогда
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаСтандартныйАлгоритм;	
	Иначе
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаПользовательскийАлгоритм;	
	КонецЕсли;
	
	Элементы.СтандартныйАлгоритм.Видимость = ЭтоСтандартныйАлгоритм;
	Элементы.НадписьОписание.Видимость = ЭтоСтандартныйАлгоритм;
	Элементы.СписокРеквизитов.Доступность = Не ЭтоСтандартныйАлгоритм;
