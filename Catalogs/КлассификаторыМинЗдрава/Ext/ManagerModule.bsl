...
Функция ПолучитьАльтернативныеOID(ВидКлассификатора) Экспорт
	...
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ОсновныеКатегорииЖизнедеятельностиЧеловека Тогда
		СписокOID.Добавить("1.2.643.5.1.13.2.1.1.704");
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.СтепеньВыраженностиОграниченийКатегорийЖизнедеятельностиЧеловека Тогда
		СписокOID.Добавить("1.2.643.5.1.13.2.1.1.710");
	Иначе
...
//  Соответствие.
//
Функция ПолучитьСопоставленийРеквизитовИXMLСправочникаЕГИСЗ(ВидКлассификатора, Версия = Неопределено) Экспорт
	...
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ЛьготныеКатегорииНаселения Тогда
		...
		Сопоставление.Вставить("ID", "УИДЕГИСЗ");
		Если Версия = "1.2" Тогда
			//3) NAME_SHOT, Краткое название категории льготы, Строковое, обязательное поле, текстовый формат, Обязательное;
	...
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.СтатусВыполненияМедицинскойУслуги Тогда
		...
		Сопоставление.Вставить("NAME", "Наименование");
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ВидыПредоставляемыхЛьгот Тогда
		//1) ID, Код, Целочисленное, Обязательное;
		Сопоставление.Вставить("ID", "УИДЕГИСЗ");
		//2) ID_LGOTA, Код льготы, Строковое, код льготы, соответствует коду справочника «Льготные категории граждан» (OID 1.2.643.5.1.13.13.99.2.541);
		Сопоставление.Вставить("ID_LGOTA", "Наименование");
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ПереченьПредоставляемыхЛьгот Тогда
		//1) ID, Код, Целочисленное, Обязательное;
		Сопоставление.Вставить("ID", "УИДЕГИСЗ");
		//2) LGOTA, Наименование, Строковое;
		Сопоставление.Вставить("LGOTA", "Наименование");
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ИнтерпретацияРезультатовОценкиПоКлиническимШкаламИОпросникам Тогда
		//1) ID, Уникальный идентификатор, Целочисленное, Обязательное;
		Сопоставление.Вставить("ID", "УИДЕГИСЗ");
		//2) PARENT_ID, Код родительской записи, Целочисленное;
		Сопоставление.Вставить("PARENT_ID", "УИДЕГИСЗРодителя");
...
//  Соответствие.
//
Функция ПолучитьДополнительныеАтрибуты(ВидКлассификатора, Версия = Неопределено) Экспорт
	...
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ИммунобиологическиеЛекарственныеПрепараты Тогда
		...
		//7) DISEASEGROUP,Заболевание/ группа заболеваний МИБП,Целочисленное,соответствует уникальным идентификаторам записей справочника "Группа заболеваний, для диагностики, профилактики и лечения которых используются иммунобиологические препараты" (OID: 1.2.643.5.1.13.13.99.2.1111);
		...
		//13) PREVENTIVEGROUP,Группа МИБП для сертификата профилактических прививок,Целочисленное,соответствует уникальному идентификатору записи справочника "Группа иммунобиологических препаратов для сертификата профилактических прививок" (OID: 1.2.643.5.1.13.13.99.2.848);
		ДополнительныеАтрибуты.Вставить("PREVENTIVEGROUP", "PREVENTIVEGROUP");
		//14) DRUGFORMGR, Вид лекарственной формы МИБП , Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("DRUGFORMGR", "DRUGFORMGR");
		//15) DRUGFORM, Лекарственная форма МИБП , Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("DRUGFORM", "DRUGFORM");
		//16) DRUGFORMSH, Краткое название лекарственной формы МИБП , Строковое, Соответствует базовой лекарственной форме из справочника "Базовые лекарственные формы и состояния вещества" (OID: 1.2.643.5.1.13.13.11.1464);
		ДополнительныеАтрибуты.Вставить("DRUGFORMSH", "DRUGFORMSH");
		//17) DRUGPACK, Упаковка 1 уровня (первичная упаковка) МИБП , Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("DRUGPACK", "DRUGPACK");
		//18) DRUGPACKGR, Вид упаковки 1 уровня МИБП , Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("DRUGPACKGR", "DRUGPACKGR");
		//19) DOZA, Дозировка МИБП , Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("DOZA", "DOZA");
		//20) NUM_DOSA, Количество доз, Строковое, Количество доз в первичной упаковке;
		ДополнительныеАтрибуты.Вставить("NUM_DOSA", "NUM_DOSA");
		//21) MASS, Масса/объем МИБП, Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("MASS", "MASS");
		//22) SCTID, Код SNOMED CT, Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("SCTID", "SCTID");
		//23) ICD10, Код МКБ10 , Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("ICD10", "ICD10");
		//24) NAMESH, Общепринятое сокращение названия МИБП, Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("NAMESH", "NAMESH");
		//25) PRODUCER, Производитель МИБП , Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("PRODUCER", "PRODUCER");
		//26) PRODUCER_ENG, Производитель МИБП на английском языке , Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("PRODUCER_ENG", "PRODUCER_ENG");
		//27) COUNTRY, Страна-производитель МИБП , Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("COUNTRY", "COUNTRY");
		//28) BARCODE, Штрих-код МИБП , Строковое, текстовое поле;
		ДополнительныеАтрибуты.Вставить("BARCODE", "BARCODE");
		//29) ACTUAL, Признак актуальности, Логическое, (true – запись актуальна, false – запись неактуальна);
		ДополнительныеАтрибуты.Вставить("ACTUAL", "ACTUAL");
		//30) KLP, Код КЛП, Строковое;
		ДополнительныеАтрибуты.Вставить("KLP", "KLP");
		//31) SMNN, Код СМНН, Строковое;
		ДополнительныеАтрибуты.Вставить("SMNN", "SMNN");
		//32) KTRU, Код КТРУ, Строковое;
	...
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ЛьготныеКатегорииНаселения Тогда
		...
		Если Версия = "1.2" Тогда
			//3) NAME_SHOT, Краткое название категории льготы, Строковое, обязательное поле, текстовый формат, Обязательное;
			ДополнительныеАтрибуты.Вставить("NAME_SHOT", "NAME_SHOT");
		Иначе
			//4) NAME_SHORT, Краткое название категории льготы, Строковое, обязательное поле, текстовый формат, Обязательное;
	...
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ВидыПредоставляемыхЛьгот Тогда
		//2) ID_LGOTA, Код льготы, Строковое, код льготы, соответствует коду справочника «Льготные категории граждан» (OID 1.2.643.5.1.13.13.99.2.541);
		ДополнительныеАтрибуты.Вставить("ID_LGOTA", "ID_LGOTA");
		//3) ID_LGOTA_LIST, Код перечня льготы, Целочисленное, код перечня льготы, соответствует коду справочника «Перечень предоставляемых льгот» (OID 1.2.643.5.1.13.13.99.2.594);
		ДополнительныеАтрибуты.Вставить("ID_LGOTA_LIST", "ID_LGOTA_LIST");
		//4) PERCENT, Размер льготы, Целочисленное;
		ДополнительныеАтрибуты.Вставить("PERCENT", "PERCENT");
		//5) PLANNED_NEED, Плановая потребность, Логическое;
		ДополнительныеАтрибуты.Вставить("PLANNED_NEED", "PLANNED_NEED");
		//6) PROGNOSIS_NEED, Прогнозируемая потребность, Логическое;
		ДополнительныеАтрибуты.Вставить("PROGNOSIS_NEED", "PROGNOSIS_NEED");
		//7) DATA_BEGIN, Дата начала действия, Дата, дата введения предоставления вида льготы;
		ДополнительныеАтрибуты.Вставить("DATA_BEGIN", "DATA_BEGIN");
		//8) NPA_BEGIN, НПА ввода вида льготы, Целочисленное, код нормативно-правового акта, вводившего данную льготу, соответствует коду НПА справочника «Нормативно-правовые акты справочников ФР НСИ» (OID 1.2.643.5.1.13.13.99.2.533);
		ДополнительныеАтрибуты.Вставить("NPA_BEGIN", "NPA_BEGIN");
		//9) DATA_END, Дата окончания действия, Дата, дата отмены предоставления вида льготы;
		ДополнительныеАтрибуты.Вставить("DATA_END", "DATA_END");
		//10) NPA_END, НПА отмены вида льготы, Целочисленное, код нормативно-правового акта, отменившего данную льготу, соответствует коду НПА справочника «Нормативно-правовые акты справочников ФР НСИ» (OID 1.2.643.5.1.13.13.99.2.533);
		ДополнительныеАтрибуты.Вставить("NPA_END", "NPA_END");
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ГруппаЗаболеванийМИБП Тогда
		//3) SYNONYMS, Синонимы, Строковое;
		ДополнительныеАтрибуты.Вставить("SYNONYMS", "SYNONYMS");
		//4) NAME_EPMB, Наименование в ЭЛМК, Строковое;
		ДополнительныеАтрибуты.Вставить("NAME_EPMB", "NAME_EPMB");
		//5) EPMB, Использование в ЭЛМК, Логическое;
		ДополнительныеАтрибуты.Вставить("EPMB", "EPMB");
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ЦелиПроведенияВК Тогда
		//3) CONSULTATION, Консилиум, Логическое, Признак указывает на тот факт, что данная цель проводится в рамках консилиума;
		ДополнительныеАтрибуты.Вставить("CONSULTATION", "CONSULTATION");
		//4) COMMISSION, Комиссия, Логическое, Признак указывает на тот факт, что данная цель проводится в рамках комиссии;
		ДополнительныеАтрибуты.Вставить("COMMISSION", "COMMISSION");
		//5) NPA, НПА, Строковое, Нормативно-правовой акт, уточняющий отношение к консилиуму или комиссии;
		ДополнительныеАтрибуты.Вставить("NPA", "NPA");
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ОсновныеКатегорииЖизнедеятельностиЧеловека Тогда
		//3) COMMENT, Примечание, Строковое, необязательное поле. Возможные значения – текст;
		ДополнительныеАтрибуты.Вставить("COMMENT", "COMMENT");
		//4) SCTID, Код SNOMED CT, Строковое, соответствующий код номенклатуры, необязательное поле, содержит не более 15 символов;
		ДополнительныеАтрибуты.Вставить("SCTID", "SCTID");
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ИнтерпретацияРезультатовОценкиПоКлиническимШкаламИОпросникам Тогда
		//4) Description, Описание, Строковое;
		ДополнительныеАтрибуты.Вставить("Description", "Description");
		//5) POINT, Итоговая оценка по шкале, Строковое;
		ДополнительныеАтрибуты.Вставить("POINT", "POINT");
		//6) GROUPING_SCALE, Группировка по шкалам, Целочисленное;
		ДополнительныеАтрибуты.Вставить("GROUPING_SCALE", "GROUPING_SCALE");
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.КомпонентыКрови Тогда
		//3) SORT, Порядок сортировки, Целочисленное;
		ДополнительныеАтрибуты.Вставить("SORT", "SORT");
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ПоказанияКГемотрансфузии Тогда
		//3) SORT, Порядок сортировки, Целочисленное;
		ДополнительныеАтрибуты.Вставить("SORT", "SORT");
	ИначеЕсли ВидКлассификатора = Перечисления.ВидыКлассификаторовМинЗдрава.ПереченьКлиническихШкалИОпросников Тогда
		//3) OID_PARAMETER, OID справочника с параметрами шкалы, Строковое;
		ДополнительныеАтрибуты.Вставить("OID_PARAMETER", "OID_PARAMETER");
		//4) ID_PARAMETER, Группировочный код шкалы в параметрах, Целочисленное, Соответствует группировке по шкалам справочника «Параметры клинических шкал и опросников» (OID: 1.2.643.5.1.13.13.11.1515);
		ДополнительныеАтрибуты.Вставить("ID_PARAMETER", "ID_PARAMETER");
		//5) OID_INTERPRETATION, OID справочника с интерпретацией шкалы, Строковое;
		ДополнительныеАтрибуты.Вставить("OID_INTERPRETATION", "OID_INTERPRETATION");
		//6) ID_INTERPRETATION, Группировочный код шкалы в интерпретациях, Целочисленное, Соответствует группировке по шкалам справочника «Интерпретация результатов оценки по клиническим шкалам и опросникам» (OID: 1.2.643.5.1.13.13.11.1516);
		ДополнительныеАтрибуты.Вставить("ID_INTERPRETATION", "ID_INTERPRETATION");
		//7) POINT_SCALE, Признак балльной шкалы, Логическое, Истинно (true) – для данной шкалы/опросника производится суммация баллов для дальнейшей интерпретации результата;
		ДополнительныеАтрибуты.Вставить("POINT_SCALE", "POINT_SCALE");
		//8) VALID, Валидация, Строковое, Признак валидации шкалы или опросника;
		ДополнительныеАтрибуты.Вставить("VALID", "VALID");
		//9) VALID_AUTHOR, Название и авторы, Строковое, Название и авторы оригинальной публикации;
		ДополнительныеАтрибуты.Вставить("VALID_AUTHOR", "VALID_AUTHOR");
		//10) VALID_DOI, Идентификатор публикации, Строковое, Цифровой уникальный идентификатор оригинальной публикации;
		ДополнительныеАтрибуты.Вставить("VALID_DOI", "VALID_DOI");
		//11) ORG, Организация, Строковое, Организация, ответственная за перевод и/или валидацию клинической шкалы или опросника;
		ДополнительныеАтрибуты.Вставить("ORG", "ORG");
	Иначе
		ИмяЗначения = ОбщегоНазначения.ИмяЗначенияПеречисления(ВидКлассификатора);
		Если ИмяЗначения = "ПричиныОтказовВГоспитализации" Тогда
