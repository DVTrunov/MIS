...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Добавлено Макеев (
	ЭлИКЛ = Элементы.Добавить("омг_ИдентификационныйКодЛичности", Тип("ПолеФормы"), Элементы.ГруппаРеквизитыВШапки);
	ЭлИКЛ.Вид = ВидПоляФормы.ПолеВвода;
	ЭлИКЛ.ПутьКДанным = "Объект.омг_ИдентификационныйКодЛичности";
	ЭлИКЛ.Заголовок = "ИИН";
	
	омг_idoctorНеВыгружать = Элементы.Добавить("омг_idoctorНеВыгружать", Тип("ПолеФормы"), Элементы.ГруппаРеквизитыВШапки);
	омг_idoctorНеВыгружать.Вид = ВидПоляФормы.ПолеФлажка;
	омг_idoctorНеВыгружать.ПутьКДанным = "Объект.омг_idoctorНеВыгружать"; 
	
	Если рольДоступна("ПолныеПрава") Тогда 
	омг_idoctorId = Элементы.Добавить("омг_idoctorIdЭлемент", Тип("ПолеФормы"), Элементы.ГруппаРеквизитыВШапки);
	омг_idoctorId.Вид = ВидПоляФормы.ПолеВвода;
	омг_idoctorId.ПутьКДанным = "Объект.омг_idoctorId";
	омг_idoctorId.Заголовок = "омг_idoctorId";
    КонецЕсли;
	
	Элементы.РежимВвода.Видимость = Ложь;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Элементы.РаботыПодбор.Доступность Тогда
...
Процедура НастроитьТабличныеЧасти(ПриСозданииФормы = Ложь)
	...
	Если ПриСозданииФормы Тогда
		...
		Если ПравоДоступа("Изменение", Метаданные.РегистрыСведений.ОсновныеУслугиСотрудников) Тогда
			НастройкаПодбораПриСоздании();
		Иначе
			Элементы.РаботыПодбор.Доступность = Ложь;
			Элементы.ОсновныеУслугиСкопироватьУслуги.Доступность = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	ИзменитьТекстЗапросаДолжностейМЗ();
	
	ДолжностиМинздрава.Параметры.УстановитьЗначениеПараметра("Сотрудник", Объект.Ссылка);
	...
	РаботаСФормамиСервер.УстановитьОтборСписка("Сотрудник", Объект.Ссылка, ДолжностиМинздрава);
	
КонецПроцедуры

&НаСервере
Процедура ИзменитьТекстЗапросаДолжностейМЗ()
	
	СвязиДолжностей = СвязиДолжностей();
	
	Если СвязиДолжностей.Совпадающие.Количество() = 0
		И СвязиДолжностей.Различающиеся.Количество() = 0
	Тогда
		Возврат;
	КонецЕсли;
	
	ДобавленныеПараметры = Новый Массив;
	СтрокиЗапросов = Новый Массив;
	
	ИменаПолейЗаданы = Ложь;
	
	Для Каждого СтрокаСвязи Из СвязиДолжностей.Различающиеся Цикл
		Если Не ИменаПолейЗаданы Тогда
			ШаблонВложенногоЗапроса = "ВЫБРАТЬ %1 КАК ИдентификаторДолжностиМЗ, %2 КАК ИдентификаторДолжностиФРМР";
			ИменаПолейЗаданы = Истина;
		Иначе
			ШаблонВложенногоЗапроса = "ВЫБРАТЬ %1, %2";
		КонецЕсли;
		СтрокиЗапросов.Добавить(СтрШаблон(ШаблонВложенногоЗапроса, СтрокаСвязи.ИдентификаторДолжностиМЗ, СтрокаСвязи.ИдентификаторДолжностиФРМР));
	КонецЦикла;
	
	ШаблонЗапросаДолжностейФРМРПоМЗ = 
		"ВЫБРАТЬ
		|	СвязиИдентификторовДолжностей.ИдентификаторДолжностиМЗ,
		|	СвязиИдентификторовДолжностей.ИдентификаторДолжностиФРМР
		|ПОМЕСТИТЬ СвязиИдентификторовДолжностей
		|ИЗ (%1) КАК СвязиИдентификторовДолжностей;";
	
	ЗапросДолжностейФРМРПоМЗ = СтрШаблон(ШаблонЗапросаДолжностейФРМРПоМЗ, СтрСоединить(СтрокиЗапросов, Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС));
	
	ОсновнойЗапрос = 
		"ВЫБРАТЬ
		|	КлассификаторДолжностейФРМР.Ссылка КАК ДолжностьФРМР,
		|	СвязиИдентификторовДолжностей.ИдентификаторДолжностиМЗ КАК ИдентификаторДолжностиМЗ
		|ПОМЕСТИТЬ ДолжностиФРМРПоИдентификаторамДолжностейМЗ
		|ИЗ
		|	СвязиИдентификторовДолжностей КАК СвязиИдентификторовДолжностей
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлассификаторыМинЗдрава КАК КлассификаторДолжностейФРМР
		|		ПО (КлассификаторДолжностейФРМР.ВидКлассификатора = ЗНАЧЕНИЕ(Перечисление.ВидыКлассификаторовМинЗдрава.ФРМРДолжностиМедицинскогоПерсонала))
		|			И (НЕ КлассификаторДолжностейФРМР.ПометкаУдаления)
		|			И (КлассификаторДолжностейФРМР.УИДЕГИСЗ = СвязиИдентификторовДолжностей.ИдентификаторДолжностиФРМР)
		|			И (НЕ КлассификаторДолжностейФРМР.Архив)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	КлассификаторДолжностейФРМР.Ссылка,
		|	КлассификаторДолжностейФРМР.УИДЕГИСЗ
		|ИЗ
		|	Справочник.КлассификаторыМинЗдрава КАК КлассификаторДолжностейФРМР
		|ГДЕ
		|	КлассификаторДолжностейФРМР.ВидКлассификатора = ЗНАЧЕНИЕ(Перечисление.ВидыКлассификаторовМинЗдрава.ФРМРДолжностиМедицинскогоПерсонала)
		|	И КлассификаторДолжностейФРМР.УИДЕГИСЗ В(&СовпадающиеИдентификаторы)
		|	И НЕ КлассификаторДолжностейФРМР.ПометкаУдаления
		|	И (НЕ КлассификаторДолжностейФРМР.Архив)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РегистрСведенийДолжностиМинздрава.Сотрудник КАК Сотрудник,
		|	РегистрСведенийДолжностиМинздрава.ДолжностьМинздрава КАК ДолжностьМинздрава,
		|	ВЫБОР
		|		КОГДА РегистрСведенийДолжностиМинздрава.ДолжностьФРМР = ЗНАЧЕНИЕ(Справочник.КлассификаторыМинЗдрава.ПустаяСсылка)
		|			ТОГДА ДолжностиФРМРПоИдентификаторамДолжностейМЗ.ДолжностьФРМР
		|		ИНАЧЕ РегистрСведенийДолжностиМинздрава.ДолжностьФРМР
		|	КОНЕЦ КАК ДолжностьФРМР
		|ИЗ
		|	РегистрСведений.ДолжностиМинздрава КАК РегистрСведенийДолжностиМинздрава
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлассификаторыМинЗдрава КАК КлассификаторДолжностейМЗ
		|		ПО (КлассификаторДолжностейМЗ.Ссылка = РегистрСведенийДолжностиМинздрава.ДолжностьМинздрава)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ДолжностиФРМРПоИдентификаторамДолжностейМЗ КАК ДолжностиФРМРПоИдентификаторамДолжностейМЗ
		|		ПО (ДолжностиФРМРПоИдентификаторамДолжностейМЗ.ИдентификаторДолжностиМЗ = КлассификаторДолжностейМЗ.УИДЕГИСЗ)
		|ГДЕ
		|	РегистрСведенийДолжностиМинздрава.Сотрудник = &Сотрудник";
	
	ДолжностиМинздрава.ТекстЗапроса = ЗапросДолжностейФРМРПоМЗ + Символы.ПС + ОсновнойЗапрос;
	
	ДолжностиМинздрава.Параметры.УстановитьЗначениеПараметра("СовпадающиеИдентификаторы", СвязиДолжностей.Совпадающие);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция СвязиДолжностей()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	МИНИМУМ(АтрибутыКлассификатора.Ссылка.УИДЕГИСЗ) КАК ИдентификаторДолжностиФРМР,
		|	ВЫРАЗИТЬ(АтрибутыКлассификатора.Значение КАК СТРОКА(10)) КАК ИдентификаторДолжностиМЗ
		|ИЗ
		|	Справочник.КлассификаторыМинЗдрава.Атрибуты КАК АтрибутыКлассификатора
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлассификаторыМинЗдрава КАК КлассификаторДолжностейФРМР
		|		ПО (КлассификаторДолжностейФРМР.ВидКлассификатора = ЗНАЧЕНИЕ(Перечисление.ВидыКлассификаторовМинЗдрава.ФРМРДолжностиМедицинскогоПерсонала))
		|			И (КлассификаторДолжностейФРМР.Ссылка = АтрибутыКлассификатора.Ссылка)
		|			И (АтрибутыКлассификатора.Ключ = ""federal_code"")
		|			И (НЕ КлассификаторДолжностейФРМР.ПометкаУдаления)
		|
		|СГРУППИРОВАТЬ ПО
		|	ВЫРАЗИТЬ(АтрибутыКлассификатора.Значение КАК СТРОКА(10))";
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Совпадающие = Новый Массив;
	Различающиеся = Новый Массив;
	
	Пока Выборка.Следующий() Цикл
		Если Выборка.ИдентификаторДолжностиФРМР = Число(Выборка.ИдентификаторДолжностиМЗ) Тогда
			// Оптимизация. Тольку у ~трети элементов идентификаторы отличаются
			Совпадающие.Добавить(Выборка.ИдентификаторДолжностиФРМР);
		Иначе
			СтруктураСвязи = Новый Структура("ИдентификаторДолжностиФРМР, ИдентификаторДолжностиМЗ");
			ЗаполнитьЗначенияСвойств(СтруктураСвязи, Выборка);
			Различающиеся.Добавить(СтруктураСвязи);
		КонецЕсли;
	КонецЦикла;
	
	Возврат Новый Структура("Совпадающие, Различающиеся", Совпадающие, Различающиеся);
...
Процедура ОбновитьДополнительнуюДолжностьФРМР()
	...
	ОбновитьОсновныеДолжности(Объект.Должность, ОсновнаяДолжностьМинздрава, ОсновнаяДолжностьФРМР);
...
&НаСервереБезКонтекста
Процедура ОбновитьОсновныеДолжности(ОсновнаяДолжность, ОсновнаяДолжностьМинздрава, ОсновнаяДолжностьФРМР);
	
	ОсновнаяДолжностьМинздрава = ОсновнаяДолжность.ДолжностьМинздрава;
	
	Если ЗначениеЗаполнено(ОсновнаяДолжность.ДолжностьФРМР) Тогда
		ОсновнаяДолжностьФРМР = ОсновнаяДолжность.ДолжностьФРМР;
	Иначе
		ОсновнаяДолжностьФРМР = ИнтеграцияЕГИСЗВызовСервера.ПолучитьДолжностьФРМРПоДолжностиМЗ(ОсновнаяДолжностьМинздрава);
	КонецЕсли;
