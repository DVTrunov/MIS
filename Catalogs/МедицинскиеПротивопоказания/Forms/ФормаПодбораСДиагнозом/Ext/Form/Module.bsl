...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ТолькоПросмотр <> Неопределено И Параметры.ТолькоПросмотр Тогда
		Элементы.ДеревоПротивопоказанийВыбрать.Заголовок = "Закрыть";
		ТолькоПросмотр = Истина;
		ЭтаФорма.Заголовок = НСтр("ru='Противопоказания'"); 
...
Функция ДобавитьСтрокуПротивопоказания(СтрокаВредность, ВыбПротивопоказания)
	...
	СтрокаПротивопоказания.Представление = СтрЗаменить(СтрокаПротивопоказания.Представление, Символы.ПС, ". ");
