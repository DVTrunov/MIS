...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		// Тип по умолчанию нового
		Если Объект.ТипЗначения.Типы().Количество() = 1 
			И Объект.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектов"))
			И Не ЗначениеЗаполнено(Параметры.ЗначениеКопирования) // Не копирование.
		Тогда
			Объект.ТипЗначения = Новый ОписаниеТипов("Булево");
			Элементы.ЗначенияСвойства.Доступность = Ложь;
		КонецЕсли;
	Иначе
