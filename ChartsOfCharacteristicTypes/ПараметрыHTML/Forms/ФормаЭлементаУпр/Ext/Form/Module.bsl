...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьОтборИсточникаСвязейПараметров();
	...
	Элементы.НаименованиеДляВрача.ПодсказкаВвода = Объект.Наименование;
	Элементы.ГруппаПолучателиЗначений.Видимость = Объект.ИспользуетсяДляВводаДругихПараметров;
	
	УстановитьЗначениеПереключателяНазначенияИзПеречня();
	
	УстановитьДоступностьПоВстроеннымОбработкам();
	
	Для Каждого РеквизитКолонка Из ПолучитьРеквизиты("ДеревоЗначенийВыбора") Цикл
		Если РеквизитКолонка.Имя = "Значение" Тогда
			ТипыЗначенийВыбора.ЗагрузитьЗначения(РеквизитКолонка.ТипЗначения.Типы());
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ПараметрПолучателиЗначенийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
КонецПроцедуры

&НаКлиенте
Процедура ИспользуетсяДляВводаДругихПараметровПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПараметрПолучателиЗначенийПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	
	Если Не Копирование Тогда
		Отказ = Истина;
	
		ОповещениеДействияПослеЗаписи = Новый ОписаниеОповещения("ПараметрПолучателиЗначенийПередНачаломДобавления_ПослеЗаписи", ЭтотОбъект);
		РаботаСДиалогамиКлиент.ЗаписатьНовыйОбъектВФорме(ЭтотОбъект, ОповещениеДействияПослеЗаписи);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПараметрПолучателиЗначенийПередНачаломДобавления_ПослеЗаписи(Результат, ДополнительныеПараметры) Экспорт
	
	Оповещение = Новый ОписаниеОповещения("ПараметрПолучателиЗначенийПередНачаломДобавления_ПослеВыбораПараметра", ЭтотОбъект);
	ОткрытьФорму("ПланВидовХарактеристик.ПараметрыHTML.ФормаВыбора",,,,,, Оповещение);

КонецПроцедуры

&НаКлиенте
Процедура ПараметрПолучателиЗначенийПередНачаломДобавления_ПослеВыбораПараметра(ВыбранныйПараметр, ДополнительныеПараметры) Экспорт
	
	Если ЗначениеЗаполнено(ВыбранныйПараметр) Тогда
		ПараметрПолучателиЗначенийПередНачаломДобавления_ДобавитьЗапись(Объект.Ссылка, ВыбранныйПараметр);
		Элементы.ПараметрПолучателиЗначений.Обновить();
	КонецЕсли;

КонецПроцедуры

&НаСервереБезКонтекста
Процедура ПараметрПолучателиЗначенийПередНачаломДобавления_ДобавитьЗапись(Источник, Приемник)
	
	Запись = РегистрыСведений.ПараметрыШаблоновИсточникиВводаДругихПараметров.СоздатьМенеджерЗаписи();
	Запись.Источник = Источник;
	Запись.Приемник = Приемник;
...
&НаСервере
Процедура УстановитьОтборИсточникаСвязейПараметров()
	
	РаботаСФормамиСервер.УстановитьОтборСписка("Источник", Объект.Ссылка, ПараметрПолучателиЗначений);
...
Процедура ТипЗначенияПриИзменении(Элемент)
	...
	Если НедопустимыйСоставнойТип() Тогда
		...
		СтрокиПредупреждения.Добавить(НСтр("ru='Допустимые варианты составных типов:'"));
		
		НомерВарианта = 1;
		Для Каждого Вариант Из ВариантыДопустимыхСоставныхТипов() Цикл
			ПредставленияТипов = Новый Массив;
			Для Каждого Тип Из Вариант Цикл
				ПредставленияТипов.Добавить(СтрШаблон("""%1""", Тип));
			КонецЦикла;
			СтрокиПредупреждения.Добавить(СтрШаблон("%1) %2", НомерВарианта, СтрСоединить(ПредставленияТипов, ", ")));
			НомерВарианта = НомерВарианта + 1;
		КонецЦикла;
		
		ПоказатьПредупреждение(, СтрСоединить(СтрокиПредупреждения, символы.ПС));
...
Функция НедопустимыйСоставнойТип()
	...
	Для Каждого Вариант Из ВариантыДопустимыхСоставныхТипов() Цикл
		ДопустимыйВариант = Истина;
		Для Каждого Тип Из Объект.ТипЗначения.Типы() Цикл
			Если Вариант.Найти(Тип) = Неопределено Тогда
				ДопустимыйВариант = Ложь;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если ДопустимыйВариант Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Функция ВариантыДопустимыхСоставныхТипов()
	
	Варианты = Новый Массив;
	Варианты.Добавить(ОбщегоНазначенияБИТКлиентСервер.НовыйМассив(Тип("ДокументСсылка.ДоговорСКлиентом"), Тип("СправочникСсылка.СоглашенияСтрахования"), Тип("СправочникСсылка.СтраховыеПолисы")));
	Варианты.Добавить(ОбщегоНазначенияБИТКлиентСервер.НовыйМассив(Тип("Строка"), Тип("СправочникСсылка.НоменклатураМедицинскихУслуг")));
...
Процедура УстановитьСвойстваЗначенияВыбора()
	...
	КлассификаторЕГИСЗ = Объект.ТипЗначения.СодержитТип(Тип("СправочникСсылка.КлассификаторыМинЗдрава"))
						...
						Или Объект.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ВидыМедицинскихДокументов"));
...
Процедура НастройкаРеквизитовФормыПриСоздании()
	...
	ТипЗначенияВыбора = Новый ОписаниеТипов(Метаданные.ПланыВидовХарактеристик.ПараметрыHTML.ТабличныеЧасти.ЗначенияВыбора.Реквизиты.Значение.Тип);
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Значение", ТипЗначенияВыбора, "ДеревоЗначенийВыбора", "Значение"));
	ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	
	ЭлементЗначения = Элементы.Добавить("ДеревоЗначенийЗначение", Тип("ПолеФормы"), Элементы.ДеревоЗначенийВыбора);
	Элементы.Переместить(ЭлементЗначения, Элементы.ДеревоЗначенийВыбора, Элементы.ДеревоЗначенийПредставление);
	ЭлементЗначения.Вид = ВидПоляФормы.ПолеВвода;
	ЭлементЗначения.ПутьКДанным = "ДеревоЗначенийВыбора.Значение";
	ЭлементЗначения.Формат = "ЧДЦ=2";
	ЭлементЗначения.КнопкаВыбора = Истина;
	ЭлементЗначения.ВыборГруппИЭлементов = ГруппыИЭлементы.ГруппыИЭлементы;
	ЭлементЗначения.ВыбиратьТип = Ложь;
	СвязиВыбора = Новый Массив;
	СвязиВыбора.Добавить(Новый СвязьПараметраВыбора("Отбор.ВидКлассификатора", "Объект.ВидКлассификатора", РежимИзмененияСвязанногоЗначения.Очищать));
	ЭлементЗначения.СвязиПараметровВыбора = Новый ФиксированныйМассив(СвязиВыбора);
	ЭлементЗначения.СвязьПоТипу = Новый СвязьПоТипу("Объект.ТипЗначения", 0);
	ЭлементЗначения.УстановитьДействие("НачалоВыбора", "ДеревоЗначенийЗначениеНачалоВыбора");
...
Процедура НастроитьВыборЗначенийПоСсылкеПоТипуПараметра()
	...
	ПодходитТип = ОбщегоНазначенияБИТКлиентСервер.ТипыДанныхСовпадают(Объект.ТипЗначения.Типы(), ТипыЗначенийВыбора.ВыгрузитьЗначения());
	...
	КонецЕсли;
	Элементы.ГруппаДеревоЗначенийВыбора.Видимость = Объект.ВыборПоСсылкеИзОграниченногоСписка;
	Элементы.СгенерироватьЗначения.Видимость = Объект.ТипЗначения.СодержитТип(Тип("Число"));
