...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураМакетов.Вставить("ТоварныйЧек", НСтр("ru='Товарный чек'")); 
		СтруктураМакетов.Вставить("АктВозврата", НСтр("ru='Акт о возврате'"));
...
Функция Печать(СсылкаНаОбъект, ИмяМакета) Экспорт
    ...
    ТабДокумент = ПечатьАктаВозврата(СсылкаНаОбъект);
...
Функция ПечатьАктаВозврата(СсылкаНаОбъект)

    ТабДокумент = Новый ТабличныйДокумент;
    ТабДокумент.АвтоМасштаб = Истина; 
	ТекущаяДата = ТекущаяДата(); 
    ДатаПечати = Формат(ТекущаяДата, "ДФ=dd.MM.yyyy HH:mm:ss");  
	// Преобразуем номер в строку и удаляем ведущие нули
    // Обработка номера
    НомерСтрока = Строка(СсылкаНаОбъект.Номер); // Преобразуем в строку
    НомерБезНулей = "";

// Удаляем ведущие нули
    Для Сч = 1 По СтрДлина(НомерСтрока) Цикл
    ТекСимвол = Сред(НомерСтрока, Сч, 1);
    Если ТекСимвол <> "0" Тогда
        НомерБезНулей = Сред(НомерСтрока, Сч);
        Прервать;
    КонецЕсли;
КонецЦикла;

// Если все символы были нулями
Если ПустаяСтрока(НомерБезНулей) Тогда
    НомерБезНулей = "0";
КонецЕсли;	// Получение директора
    ОрганизацияДокумента = СсылкаНаОбъект.Организация;
    Запрос = Новый Запрос;
    Запрос.Текст = 
	//"ВЫБРАТЬ
	//|   ОтветственныеЛица.ФизическоеЛицо
	//|ИЗ
	//|   РегистрСведений.ОтветственныеЛица.СрезПоследних(&ДатаДокумента, ) КАК ОтветственныеЛица
	//|ГДЕ
	//|   ОтветственныеЛица.СтруктурнаяЕдиница = &Филиал
	//|   И ОтветственныеЛица.ОтветственноеЛицо = &Должность";    
	 "ВЫБРАТЬ
    |   ОтветственныеЛица.ФизическоеЛицо
    |ИЗ
    ...
    Запрос.УстановитьПараметр("Филиал", СсылкаНаОбъект.Филиал);
    Запрос.УстановитьПараметр("Должность", Перечисления.ДолжностныеЛицаОрганизации.Директор);
	//Запрос.УстановитьПараметр("ДатаДокумента", СсылкаНаОбъект.Дата);
    
    РезультатЗапроса = Запрос.Выполнить().Выбрать();
    Если РезультатЗапроса.Следующий() Тогда
        Директор = РезультатЗапроса.ФизическоеЛицо;
    Иначе
        Директор = Неопределено;
    КонецЕсли;

    Макет = Документы.ОказаниеУслуг.ПолучитьМакет("АктВозврата");

    // Пример: Заполнение шапки
    Область = Макет.ПолучитьОбласть("Шапка");
    Область.Параметры.Организация = СсылкаНаОбъект.Организация;
    Область.Параметры.БИН = СсылкаНаОбъект.Организация.ИНН;
    // (Добавь остальные параметры: Клиент, Причина возврата, Сумма и т.д.)
    ТабДокумент.Вывести(Область);
	
	Область = Макет.ПолучитьОбласть("Заголовок");
	Область.Параметры.Номер = НомерБезНулей;
	Область.Параметры.Дата = СсылкаНаОбъект.Дата;
	Область.Параметры.Клиент = СсылкаНаОбъект.Клиент;
	Область.Параметры.НомерККМ = СсылкаНаОбъект.Касса.ЗаводскойНомерККМ;
	Область.Параметры.НомерРегКарты = СсылкаНаОбъект.Касса.РегистрационнаяКарточка;
	Область.Параметры.Ответственный = СсылкаНаОбъект.Ответственный;
	ТабДокумент.Вывести(Область);  
	
	Область = Макет.ПолучитьОбласть("Таблица");
	Если Директор <> Неопределено Тогда
	Область.Параметры.Директор = Директор; 
Иначе
	Область.Параметры.Директор = "Директор не найден на дату документа!";
КонецЕсли; 
    Область.Параметры.НомерЧека = СсылкаНаОбъект.НомерЧекаККМ;
	Область.Параметры.Касса = СсылкаНаОбъект.Касса;
	Область.Параметры.Ответственный = СсылкаНаОбъект.Ответственный;
	Область.Параметры.Сумма = СсылкаНаОбъект.СуммаДокумента;
    ...
    Область.Параметры.Ответственный = СсылкаНаОбъект.Ответственный;
    Область.Параметры.Клиент = СсылкаНаОбъект.Клиент;
	Область.Параметры.ДатаПечати = ДатаПечати;
	ТабДокумент.Вывести(Область);
    // Таблица чеков (если реализовано в табличной части — добавь вывод строк через цикл)

	//// Подписи и завершение
	//Область = Макет.ПолучитьОбласть("Подписи");
	//ТабДокумент.Вывести(Область);
...
Процедура ПерезаполнитьСкидкиДокумента(Объект, Знач РасчитатьСкидки = Истина) Экспорт
	...
	Если не ЗначениеЗаполнено(Объект.омг_СтраховаяПрограмма) Тогда
		СтрокаТЧ.Сумма = Окр(СтрокаТЧ.Количество * СтрокаТЧ.Цена,0,РежимОкругления.Окр15как10)
	иначе
	...
	////////////////////////////////////////////////////////////////////////////////////////
	// ОМГ	
	Если ЗначениеЗаполнено(ДокументОбъект.омг_КодСкидки) Тогда
		ВидСкидки = омг_КодСкидкиОкончаниеВводаТекстаНаСервере(ДокументОбъект.омг_КодСкидки);   
		
		Если ЗначениеЗаполнено(ВидСкидки) Тогда  
		ДопСкидка = ТаблицаСкидок.Добавить();
		ЗаполнитьЗначенияСвойств(ДопСкидка,ВидСкидки);
		ДопСкидка.ВидСкидки = ВидСкидки; 
		КонецЕсли;
	...
	// На наборы тоже есть скидки
	// Добавлено Макеев (
	//УдаляемыеСтроки = Новый Массив;
	//Для Индекс = -(ТаблицаПродаж.Количество() - 1) По 0 Цикл
	//	СтрокаТаблицы = ТаблицаПродаж[-Индекс];
	//	Если ЗначениеЗаполнено(СтрокаТаблицы.омг_Набор) Тогда 
	...
	////////////////////////////////////////////////////////////////////////////////////////
	// ОМГ
	Если ЗначениеЗаполнено(ДокументОбъект.омг_СтраховаяПрограмма) Тогда
		СкидкиДоЗаполнения.Очистить();
	...
	МассивНЕПРименяемыхСкидок = Вернутьомг_КатегорияСкидкиЛИС(ДокументОбъект); 
	...
	Для Каждого СтрокаСкидки Из ДокументОбъект.Скидки Цикл                                
		...
		ИндексНЕПРименяемыхСкидок = МассивНЕПРименяемыхСкидок.Найти(СтрокаСкидки.Скидка);
		
		Если ИндексНЕПРименяемыхСкидок <> Неопределено Тогда
			СтрокаСкидки.ПрименятьСкидку = Ложь;
		КонецЕсли;
...
//  Объект			 - ДокументОбъект.ОказаниеУслуг	 - заполняемый документ.
//
Процедура РассчитатьСкидкиДокумента(Объект, ОбновитьОплату = Ложь) Экспорт
	...
	ТаблицаРассчитанныхСкидок.Колонки.Добавить("омг_ИсходнаяСуммаСкидки", Новый ОписаниеТипов("Число"));
	
	НулевыеСкидки = Новый Массив; // Строки рассчитанных скидок, которые можно будет удалить из итогового результата расчета.
	
	омг_СкидкиПоДокументу = Новый Соответствие;
		...
		Если СкидкаДокумента.ПрименятьСкидку Тогда
			...
			Тогда
				...
				Если СтрокаПродаж <> Неопределено Тогда
					...
					Если СкидкаДокумента.Скидка.ФормаСкидки = Перечисления.ФормыСкидки.Сумма
						И Не СкидкаДокумента.Скидка.СуммоваяСкидкаНаКаждуюЕдиницу Тогда
						омг_РасходуемаяСуммаСкидки = омг_СкидкиПоДокументу.Получить(СкидкаДокумента.Скидка);
						Если омг_РасходуемаяСуммаСкидки = Неопределено Тогда
							омг_СкидкиПоДокументу.Вставить(СкидкаДокумента.Скидка, СтрокаРасчетаСкидки.омг_ИсходнаяСуммаСкидки);
							омг_РасходуемаяСуммаСкидки = СтрокаРасчетаСкидки.омг_ИсходнаяСуммаСкидки;
						КонецЕсли;
						СтрокаРасчетаСкидки.СуммаСкидки = Мин(омг_РасходуемаяСуммаСкидки, СтрокаПродаж.Сумма);
						омг_РасходуемаяСуммаСкидки = омг_РасходуемаяСуммаСкидки - Мин(омг_РасходуемаяСуммаСкидки, СтрокаПродаж.Сумма);
						
						омг_СкидкиПоДокументу.Вставить(СкидкаДокумента.Скидка, омг_РасходуемаяСуммаСкидки);
					КонецЕсли;
	...
	///////////////////////////////////////////////////
	// ОМГ       
	НоменклатураПроверки = СтрокаПродаж.Номенклатура;
	Если ЗначениеЗаполнено(СтрокаПродаж.омг_Набор) Тогда 
		 НоменклатураПроверки = СтрокаПродаж.омг_Набор 
	КонецЕсли;
	...
	///////////////////////////////////////////////////
		...
		////////////////////////////////////////////////////////////
		// ОМГ
		омг_МаксимальныйРазмерСкидки = 0;
		// ОМГ
		////////////////////////////////////////////////////////////
		...
		Для Каждого СтрокаРасчетаСкидки Из мсСтр Цикл
			...
			////////////////////////////////////////////////////////////
			// ОМГ
			омг_МаксимальныйРазмерСкидки = Макс(омг_МаксимальныйРазмерСкидки,СтрокаРасчетаСкидки.Скидка.омг_МаксимальныйРазмерСкидки);
			// ОМГ
			////////////////////////////////////////////////////////////
		...
		////////////////////////////////////////////////////////////
		// ОМГ 
		омг_МаксСтрокаРасчетаСкидки = 0;
		Если омг_МаксимальныйРазмерСкидки > 0 Тогда
			
			омг_МаксСтрокаРасчетаСкидки = ОКР(СтрокаПродаж.Сумма  * омг_МаксимальныйРазмерСкидки/100,2);
			
		КонецЕсли;
		
		Если омг_МаксСтрокаРасчетаСкидки > 0 Тогда
			Если ЕстьСуммируемыеСкидки Тогда
				СуммаСкидки = МИН(СуммаСкидкиСуммируемыхСкидок, омг_МаксСтрокаРасчетаСкидки);
			Иначе
				СуммаСкидки = омг_МаксСтрокаРасчетаСкидки;
			КонецЕсли;
			
		ИначеЕсли МаксСтрокаРасчетаНесуммируемойСкидки <> Неопределено Тогда
			Если ЕстьСуммируемыеСкидки Тогда
			...
			Если БонусовКлиента > 0 Тогда
				...
				Если мсСтрокиСкидки.Количество() <> 0 Тогда
					...
					Если БонуснаяСкидка.ФормаСкидки = Перечисления.ФормыСкидки.Процент Тогда
						МаксимальныйБонус = Цел(РазмерПределаЗачета * СтрокаПродаж.Сумма)/100; // С округглением до копеек.
	...
	Для Каждого СтрокаПродаж Из ТаблицаПродаж Цикл
		...
		// ОМГ
		Если не ЗначениеЗаполнено(Объект.омг_СтраховаяПрограмма) Тогда
			СтрокаДокумента.Сумма = ОКР(СтрокаДокумента.Сумма,0,РежимОкругления.Окр15как10); 
		КонецЕсли;	
		// ОМГ 
		//////////////////////////////////////////////////
	КонецЦикла;
	
	ОкруглитьБонусыДокумента(ДокументОбъект, ОбновитьОплату);
...
Процедура ОкруглитьБонусыДокумента(ДокументОбъект, ОбновитьОплату = Ложь)
	
	ВсегоБонусов = 0;
	Для Каждого СтрокаСкидки Из ДокументОбъект.Скидки.НайтиСтроки(Новый Структура("ПрименятьСкидку, РучноеРедактирование", Истина, Ложь)) Цикл
		//Если СтрокаСкидки.ПрименятьСкидку И Не СтрокаСкидки.РучноеРедактирование Тогда
			ВсегоБонусов = ВсегоБонусов + СтрокаСкидки.КоличествоБонусов;	
		//КонецЕсли;
	КонецЦикла;
	
	ПорядокОкругления = УправлениеНастройками.ПолучитьПараметрУчетнойПолитики("ПорядокОкругленияПослеСкидки");
	РаботаСДокументамиКлиентСервер.РасчитатьСуммуДокументаОказанияУслуг(ДокументОбъект, ОбновитьОплату);
	ЦелеваяСуммаДокумента = Ценообразование.ОкруглитьЦену(ДокументОбъект.СуммаДокумента, ПорядокОкругления, Истина);
	РазностьСумм = ЦелеваяСуммаДокумента - ДокументОбъект.СуммаДокумента;
	Если ВсегоБонусов >= РазностьСумм И РазностьСумм > 0 Тогда
		Для Каждого СтрокаСкидки Из ДокументОбъект.Скидки Цикл
			Если СтрокаСкидки.ПрименятьСкидку
				И Не СтрокаСкидки.РучноеРедактирование
				И СтрокаСкидки.Скидка.ТипСкидки = Перечисления.ТипыСкидок.Бонусы 
				И СтрокаСкидки.КоличествоБонусов > 0 
			Тогда
			
				ВычетБонусов = Мин(СтрокаСкидки.КоличествоБонусов, РазностьСумм);
				СтрокаСкидки.КоличествоБонусов = СтрокаСкидки.КоличествоБонусов - ВычетБонусов;
				
				СтрокиПродаж = ДокументОбъект[СтрокаСкидки.ТабЧасть].НайтиСтроки(Новый Структура("КлючСтроки", СтрокаСкидки.КлючСтроки));
				Если СтрокиПродаж.Количество() > 0 Тогда
					СтрокаПродаж = СтрокиПродаж[0];	
				КонецЕсли;
				СтрокаПродаж.Сумма = СтрокаПродаж.Сумма + ВычетБонусов;
				
				РазностьСумм = РазностьСумм - ВычетБонусов;
...
Функция ПолучитьТаблицуПродажДокумента(Документ, ДляРасчетаСкидок = Истина) Экспорт
	...
	// Добавлено Макеев (
	ТаблСтроки.Колонки.Добавить("омг_Набор");
	// )
...
Процедура ПерезаполнитьЧекиДокумента(Объект) Экспорт
	...
	Для Каждого СтрокаЧека Из ДокументОбъект.Чеки Цикл
		...
		СтрокаЧека.СтавкаНДСПоУмолчаниюФискализации = УчетНДСКлиентСервер.ПолучитьОсновнуюСтавкуНДСПоСНО(СтрокаЧека.СистемаНалогоОбложения, НастройкаПечатиЧеков, ТекущаяДатаСеанса(), Объект.Филиал);
	КонецЦикла;
	...
	Для Каждого СтрокаЧека Из ДокументОбъект.Чеки Цикл
		Если ЕстьУчетПоПатенту И СтрокаЧека.СистемаНалогоОбложения = Перечисления.СистемыНалогообложения.ПатентнаяСистемаНалогообложения Тогда 
...
Функция ПолучитьШаблонЧека(Объект, СтрокаЧекаИндекс, ПараметрыЧека) Экспорт
	...
	ОбщиеПараметры.Вставить("Организация"			  , РаботаСДокументамиКлиентСервер.ОрганизацияОбъекта(Объект.Филиал)); // Ссылка,Обязательно. Организация по которой пробивается чек.
	...
	ОсновнаяСтавкаНДС = УчетНДСКлиентСервер.ПолучитьОсновнуюСтавкуНДСПоСНО(СтрокаТЧЧеки.СистемаНалогообложения, НастройкаПечатиЧеков, ТекущаяДата(), Объект.Филиал);
...
Функция СформироватьТаблицуТоваров(КлючСтроки = Неопределено, ОсновнаяСтавкаНДС = Неопределено, СписаниеСДепозитаУказыватьВОплатах = Истина, СуммаВзносаНаДепозит = 0, Объект, НастройкаПечатиЧеков)
	...
	Для Каждого СтрокаТЗ Из ТЗ Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТЗ.Штрихкод) Тогда
			...
			// Нет // Убрано, пока в ходу еще есть ФФД 1.1, нужно чтобы были Товары.
			Если ДанныеШтрихкода.Разобран
				...
				СтрокаТЗ.ПризнакПредметаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.ТоварМаркируемыйСИИмеющийКМ");		
			КонецЕсли;
		КонецЕсли;
...
Функция ТекстЗапросаРеквизитыПечатиЧека(НомераТаблиц)
	...
	|	ОказаниеУслуг.Дата КАК Дата,
	|	ОказаниеУслуг.Номер КАК Номер,
	|	ОказаниеУслуг.Филиал.Организация КАК Организация,
	|	ОказаниеУслуг.ДокументОснование КАК Основание,
	|	"""" КАК ВидОперации,
	|	ОказаниеУслуг.Контрагент КАК Контрагент,
	|	ОказаниеУслуг.ПринятоОт КАК ПринятоОт,
	|	ОказаниеУслуг.СуммаДокумента КАК СуммаДокумента,
	|	"""" КАК ВалютаДокумента,
	|	ОказаниеУслуг.ДокументОснование КАК ДокументРасшифровкиПозиций,
	|	ОказаниеУслуг.Клиент.ИНН КАК ПолучательИИН
	|ИЗ
	|	Документ.ОказаниеУслуг КАК ОказаниеУслуг
	|ГДЕ
	|	ОказаниеУслуг.Ссылка = &ОказаниеУслуг
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОказаниеУслугРаботы.Номенклатура.НаименованиеПолное КАК Ссылка,
	|	"""" КАК ДоговорКонтрагента,
	|	ОказаниеУслугРаботы.Ссылка.ОМГ_СтавкаНДС КАК СтавкаНДС,
	|	ОказаниеУслугРаботы.Омг_СуммаНДС КАК СуммаНДС,
	|	ОказаниеУслугРаботы.Сумма КАК СуммаПлатежа,
	|	ОказаниеУслугРаботы.Сумма КАК СуммаВзаиморасчетов,
...
Функция ПодготовитьПараметрыПечатиЧека(ДокументСсылка)
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ОказаниеУслуг", ДокументСсылка);
...
//////////////////////////////////////////////////////
// ОМГ
Функция омг_КодСкидкиОкончаниеВводаТекстаНаСервере(КодСкидки)
	
	Скидка = Справочники.ВидыСкидок.ПустаяСсылка();
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	ВидыСкидок.омг_КодСкидки = &КодСкидки";
	
	Запрос.УстановитьПараметр("КодСкидки", КодСкидки);
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		Скидка = ВыборкаДетальныеЗаписи.Ссылка 
	КонецЕсли;
...
Функция омг_ПечатьЧека(СсылкаНаОбъект,ДанныеЧека,ТабДок) Экспорт
	
	ТабДок.АвтоМасштаб = Истина;
	Макет = Документы.ОказаниеУслуг.ПолучитьМакет("омг_Чек");
	
	ОбластьМакета = Макет.ПолучитьОбласть("ЧекОбласть");
	ОбластьМакета.Параметры.ЧекТекст = ДанныеЧека[0];
...
Процедура омг_ПересчитатьОплатыНаСервере(Объект, ДанныеФормы) Экспорт
	
	СтрПоиска = Новый Структура;
	СтрПоиска.Вставить("ВидОплаты",Перечисления.ВидыОплаты.Безналичные);
	МасОплат = Объект.Оплата.НайтиСтроки(СтрПоиска);
	
	Если ДанныеФормы.ОплаченоБезналичным > 0  Тогда
		
		Если МасОплат.Количество() > 0 Тогда
			стрОплаты = МасОплат[0]	
		иначе
			стрОплаты = Объект.Оплата.Добавить(); 
			стрОплаты.ВидОплаты = Перечисления.ВидыОплаты.Безналичные;
			стрОплаты.ЭквайринговыйТерминал = ДанныеФормы.ЭквайринговыйТерминалНаФорме;
		КонецЕсли;	
		стрОплаты.Сумма = ДанныеФормы.ОплаченоБезналичным;
	иначе
		Для каждого строка из МасОплат Цикл	
			Объект.Оплата.Удалить(строка)	
		КонецЦикла;
	КонецЕсли;	
	
	СтрПоиска = Новый Структура;
	СтрПоиска.Вставить("ВидОплаты",Перечисления.ВидыОплаты.Наличными);
	МасОплат = Объект.Оплата.НайтиСтроки(СтрПоиска);
	...
	Если Объект.ПолученоНаличными > 0  Тогда
		...
		иначе
			стрОплаты = Объект.Оплата.Добавить(); 
			стрОплаты.ВидОплаты = Перечисления.ВидыОплаты.Наличными;
		КонецЕсли;	
		стрОплаты.Сумма = Объект.ПолученоНаличными;	  
	иначе
		Для каждого строка из МасОплат Цикл	
			Объект.Оплата.Удалить(строка)	
		КонецЦикла;
	...
	Для Каждого строка из Объект.Работы  Цикл	
			...
			строка.Омг_СуммаНДС = строка.Сумма*Объект.ОМГ_СтавкаНДС/(100+Объект.ОМГ_СтавкаНДС)
...
Процедура омг_РаботаСНДС(Объект) Экспорт
	
	ПроцНДС = 0;            
	
	СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
	
	Если Объект.ВидОперации =  Перечисления.ВидыОперацийОказанияУслуг.ВозвратКлиенту и ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		
		ПроцНДС = омг_МедицинскаяДеятельность.омг_ВернутьСтавкуНДСДокументаОснования(Объект.ДокументОснование) 
		
	иначе
		
		СтавкаНДС = УчетНДСВызовСервера.ПолучитьСтавкуНДСДляУСН(Объект.Филиал.Организация,Объект.Дата, Объект.омг_СтраховаяПрограмма);
		...
		ИначеЕсли СтавкаНДС = Перечисления.СтавкиНДС.НДС16  Тогда
	...
	Объект.ОМГ_СтавкаНДС = ПроцНДС;
	
	Для каждого строка из Объект.Работы Цикл
		
		строка.Омг_СуммаНДС = строка.Сумма*ПроцНДС/(100+ПроцНДС); 
		строка.омг_СтавкаНДС             = ПроцНДС;
	КонецЦикла;	
	
КонецПроцедуры	

// Определяет скидки, конфликтующие по категории ЛИС (омг_КатегорияСкидкиЛИС),
// оставляет в документе только скидку с наибольшей рассчитанной суммой в
// каждой категории, у остальных снимает ПрименятьСкидку.
//
// Параметры:
//  ДокументОбъект - ДокументОбъект.ОказаниеУслуг - документ, ТЧ "Скидки" которого проверяется.
//
// Возвращаемое значение:
//  Массив - ссылки Справочник.ВидыСкидок, исключенные из применения по конфликту категории.
//
Функция Вернутьомг_КатегорияСкидкиЛИС(ДокументОбъект) Экспорт
	
	ТабСкидок = ДокументОбъект.Скидки.Выгрузить();
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТабСкидок", ТабСкидок);
	Запрос.Текст = ОмгТекстЗапросаСкидокПоКатегориямЛИС();
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	// Группировка применяемых скидок по категории ЛИС.
	СкидкиПоКатегориям = Новый Соответствие;
	Пока Выборка.Следующий() Цикл
		Если Не ЗначениеЗаполнено(Выборка.КатегорияЛИС) Тогда
			Продолжить;
		КонецЕсли;
		СпособСкидок = СкидкиПоКатегориям.Получить(Выборка.КатегорияЛИС);
		Если СпособСкидок = Неопределено Тогда
			СпособСкидок = Новый Массив;
			СкидкиПоКатегориям.Вставить(Выборка.КатегорияЛИС, СпособСкидок);
		КонецЕсли;
		СпособСкидок.Добавить(Выборка.Скидка);
	КонецЦикла;
	
	МассивНЕПрименимых = Новый Массив;
	
	// Только категории, где применяется более одной скидки, требуют сравнения.
	Если СкидкиПоКатегориям.Количество() > 0 Тогда
		
		СтруктураШапкиДокумента = ОбщегоНазначенияСервер.СформироватьСтруктуруШапкиДокумента(ДокументОбъект);
		ТаблицаПродаж = ПолучитьТаблицуПродажДокумента(ДокументОбъект);
		
		Для Каждого ЭлементКатегории Из СкидкиПоКатегориям Цикл
			
			СпособСкидок = ЭлементКатегории.Значение;
			Если СпособСкидок.Количество() < 2 Тогда
				Продолжить;
			КонецЕсли;
			
			НаибольшаяСкидка		= Неопределено;
			НаибольшаяСуммаСкидки	= -1;
			
			Для Каждого КандидатСкидка Из СпособСкидок Цикл
				СуммаКандидата = ОмгРассчитатьСуммуСкидкиДляСравнения(КандидатСкидка, СтруктураШапкиДокумента, ТаблицаПродаж, ДокументОбъект.Дата);
				Если СуммаКандидата > НаибольшаяСуммаСкидки Тогда
					НаибольшаяСуммаСкидки	= СуммаКандидата;
					НаибольшаяСкидка		= КандидатСкидка;
				КонецЕсли;
			КонецЦикла;
			
			Для Каждого КандидатСкидка Из СпособСкидок Цикл
				Если КандидатСкидка <> НаибольшаяСкидка Тогда
					МассивНЕПрименимых.Добавить(КандидатСкидка);
				КонецЕсли;
			КонецЦикла;
	...
	// Само снятие ПрименятьСкидку у проигравших скидок делает вызывающий код
	// (ПерезаполнитьСкидкиДокумента) по возвращаемому массиву - здесь только
	// вычисление конфликтов и уведомление пользователя.
	Если МассивНЕПрименимых.Количество() > 0 Тогда
		ОмгСообщитьОбИсключенныхСкидкахЛИС(МассивНЕПрименимых);
	КонецЕсли;
	
	Возврат МассивНЕПрименимых;
	
КонецФункции                

// Рассчитывает суммарный размер указанной скидки по всем строкам продаж
// документа, к которым она применима. Используется только для сравнения
// величины скидок при конфликте категорий ЛИС - не заменяет основной расчет.
Функция ОмгРассчитатьСуммуСкидкиДляСравнения(Скидка, СтруктураШапкиДокумента, ТаблицаПродаж, ДатаДокумента)
	
	Если Скидка.ФормаСкидки = Перечисления.ФормыСкидки.НатуральнаяСкидка Тогда
		Возврат 0; // Натуральная скидка не сравнивается по сумме - остается низшим приоритетом.
	КонецЕсли;
	
	СуммаСкидкиИтого = 0;
	
	ТаблицаПродажПрименение = РаботаСоСкидками.ПолучитьПродажиСоответствующиеОтбору(ТаблицаПродаж, Скидка, , ДатаДокумента);
	
	ТаблицаРасчета = ТаблицаПродаж.СкопироватьКолонки();
	ТаблицаРасчета.Колонки.Добавить("Скидка", Новый ОписаниеТипов("СправочникСсылка.ВидыСкидок"));
	ТаблицаРасчета.Колонки.Добавить("омг_ИсходнаяСуммаСкидки", Новый ОписаниеТипов("Число"));
	
	Для Каждого ПрименяемаяСтрока Из ТаблицаПродажПрименение Цикл
		СтрокаРасчета = ТаблицаРасчета.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаРасчета, ПрименяемаяСтрока);
		СтрокаРасчета.Скидка = Скидка;
		РаботаСоСкидками.РассчитатьСуммуПроцентСкидкиСтроки(Скидка, СтрокаРасчета, СтруктураШапкиДокумента);
		СуммаСкидкиИтого = СуммаСкидкиИтого + СтрокаРасчета.СуммаСкидки;
	КонецЦикла;
	
	Возврат СуммаСкидкиИтого;
	
КонецФункции

// Выводит пользователю сообщение об исключенных из-за конфликта категории ЛИС скидках.
Процедура ОмгСообщитьОбИсключенныхСкидкахЛИС(МассивНЕПрименимых)
	
	СтрокаСкидок = "";
	Для Каждого ИсключеннаяСкидка Из МассивНЕПрименимых Цикл
		СтрокаСкидок = СтрокаСкидок + ?(ПустаяСтрока(СтрокаСкидок), "", ", ") + Строка(ИсключеннаяСкидка);
	КонецЦикла;
	
	ТекстСообщения = НСтр("ru='Обнаружено несколько скидок одной категории ЛИС. Применена скидка с наибольшим размером, исключены: %1.'");
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, СтрокаСкидок);
	
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	
КонецПроцедуры         

Функция ОмгТекстЗапросаСкидокПоКатегориямЛИС()    

	ТекстЗапроса =  "ВЫБРАТЬ
	   |	ТабСкидок.Скидка КАК Скидка,
	   |	ТабСкидок.ПрименятьСкидку КАК ПрименятьСкидку
	   |ПОМЕСТИТЬ ВтСкидкиИсходные
	   |ИЗ
	   |	&ТабСкидок КАК ТабСкидок
	   |;
	   |
	   |////////////////////////////////////////////////////////////////////////////////
	   |ВЫБРАТЬ
	   |	ВтСкидкиИсходные.Скидка КАК Скидка
	   |ПОМЕСТИТЬ ВтСкидкиОбр
	   |ИЗ
	   |	ВтСкидкиИсходные КАК ВтСкидкиИсходные
	   |ГДЕ
	   |	ВтСкидкиИсходные.ПрименятьСкидку
	   |
	   |СГРУППИРОВАТЬ ПО
	   |	ВтСкидкиИсходные.Скидка
	   |;
	   |
	   |////////////////////////////////////////////////////////////////////////////////
	   |ВЫБРАТЬ
	   |	ВтСкидкиОбр.Скидка КАК Скидка,
	   |	ВидыСкидок.омг_КатегорияСкидкиЛИС КАК КатегорияЛИС
	   |ИЗ
	   |	ВтСкидкиОбр КАК ВтСкидкиОбр
	   |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыСкидок КАК ВидыСкидок
	   |		ПО ВтСкидкиОбр.Скидка = ВидыСкидок.Ссылка
	   |
	   |УПОРЯДОЧИТЬ ПО
	   |	КатегорияЛИС";
