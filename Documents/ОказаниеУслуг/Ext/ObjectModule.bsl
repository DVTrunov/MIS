	...
	Если не омг_МедицинскаяДеятельность.омг_ВыработкаВПриеме(Дата) Тогда
		Если ЭтоПродажа Или УменьшатьВыработкуПриВозврате Тогда 
			...
			Если УчитыватьВыработуВЗП Тогда
				...
				СписокУчитываемойНомеклатуры = омг_РасчетЗаработнойПлатыСервер.ПроверкаНаИсключениеИзВыработки(ТаблицаПоРаботам.ВыгрузитьКолонку("Номенклатура"),Сотрудник,Филиал);
			...
			Для Каждого СтрокаТаблицы Из ТаблицаПоРаботам Цикл
				Если СтрокаТаблицы.Сертификат.Пустая() Или СтрокаТаблицы.СертификатВключатьВыполнениеРаботВВыработку Тогда
					...
					Если омг_МедицинскаяДеятельность.ПроверкаНаИсключениеИзВыработки(СтрокаТаблицы.Номенклатура) Тогда  
						Продолжить
					КонецЕсли;	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	КонецЕсли;	
		...
		иначе 
				...
				|	омг_НоменклатураОказанийДляПриемов.Количество КАК Количество
				|ИЗ
				...
				Для каждого строкаТабДляДвижения из ТабДляДвижения Цикл
					...
					Пока Выборкаомг_НоменклатураОказанийДляПриемов.НайтиСледующий(СтрПоиска) и КоличествоСписатьомг_НоменклатураОказанийДляПриемов > 0 Цикл
						...
						ЗаполнитьЗначенияСвойств(стрДвиженияПриемов,Выборкаомг_НоменклатураОказанийДляПриемов); 
						стрДвиженияПриемов.омг_ОказаниеУслуг = Ссылка;
			...
			Если мУчетнаяПолитика.УменьшатьВыработкуПриВозврате Тогда
	...
	Если Не Отказ Тогда
		...
		Если омг_СуммаПредоплатыСписать > 0 Тогда
