...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ИдентификаторКорзиныНСПК") Тогда
		СтараяКорзина = Параметры.ИдентификаторКорзиныНСПК;
	КонецЕсли;
	
	Если ЭтоВозврат Тогда
		Элементы.Подтвердить.Видимость = Ложь;
		Элементы.Проверить.КнопкаПоУмолчанию = Истина;
		Элементы.УдалитьДанныеНСПК.Доступность = Ложь;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоВозврат Тогда
		...
		ПараметрыОперации.ИдентификаторКорзины = СтараяКорзина;
...
Процедура ПолучениеДанныхКартыЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		ХешНомерКарты = ?(РезультатВыполнения.Свойство("ХешНомерКарты"), СокрЛП(РезультатВыполнения.ХешНомерКарты), "");
		НомерКарты = ?(РезультатВыполнения.Свойство("НомерКарты"), СокрЛП(РезультатВыполнения.НомерКарты), "");
		СсылкаНаПлатежныйСчет = ?(РезультатВыполнения.Свойство("СсылкаНаПлатежныйСчет"), СокрЛП(РезультатВыполнения.СсылкаНаПлатежныйСчет), "");
		...
		Если ЭтоВозврат Тогда
			ЭлектронныеСертификатыНСПККлиент.НачатьПредварительноеОдобрениеВозврата(Оповещение, ПараметрыОперации);	
		Иначе
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Для Каждого ТоварнаяПозиция Из РезультатВыполнения.ТоварныеПозиции Цикл
			...
			Если ТоварнаяПозиция.Свойство("Сертификаты") И ТоварнаяПозиция.Сертификаты.Количество() > 0 Тогда
				...
				СтрокаТовара.СуммаПоЭС = ?(КоличествоПоСертификату = 0, 0, Окр(СуммаПоСертификату/КоличествоПоСертификату * СтрокаТовара.КоличествоПоЭС, 2)); 
			КонецЕсли;
		КонецЦикла;
		
		Если СуммаЭС = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Получена пустая корзина!");	
		КонецЕсли;
		
		Если ЭтоВозврат Тогда
			Если ЗначениеЗаполнено(ИдентификаторКорзиныНСПК) Тогда
				Подтвердить(Неопределено);				// Автоматически переносим корзину для возврата	
			КонецЕсли;
		Иначе
...
Процедура Подтвердить(Команда)
	...
	Если ЭтоВозврат Тогда
		ПараметрыВозврата.Вставить("ПечатьЧека", Истина);
	КонецЕсли;
