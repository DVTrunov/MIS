...
////    Код 8.3. Сейчас не используется	
...
Процедура ПриНажатииКнопкиПодбор(ИмяТабличнойЧасти)
	...
	Если Объект.СуммаДокумента <> ОплаченоБезналичным + Объект.ПолученоНаличными Тогда
		Разница = Объект.СуммаДокумента - ОплаченоБезналичным;
		
		Если значениеЗаполнено(ЭквайринговыйТерминалНаФорме) Тогда
			ОплаченоБезналичным = Объект.СуммаДокумента;	
	...
	омг_ИзменитьВидимостьВводаКоличестваЦены(Элементы, ЭтаФорма);
	ЭтаФорма.РеквизитСписокПодбор.Параметры.УстановитьЗначениеПараметра("омг_Филиал", Объект.Филиал); 
	Элементы.ГруппаПрейскурант.Видимость = не ПодборПометка; 
...
Процедура РаботыПередУдалением(Элемент, Отказ)
	...
	Если омг_МедицинскаяДеятельность.омг_РольДоступна("омг_ЗапретУдаленияУслугВОказанииУслуг") и Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОказанияУслуг.ПродажаКлиенту") и ЗначениеЗаполнено(Объект.Ссылка) Тогда  
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	Отказ = омг_ДопПраваРегистраторПроверка();
...
&НаКлиенте
// Изменено Макеев (
Процедура РаботыРаботаПриИзмененииТекСтрока(Элемент, ТекущаяСтрока = Неопределено, омг_Набор = Неопределено,омг_НаборУровень= Неопределено)
// )
	...
	Если РаботаСФормамиСервер.ЭтоУслуга(Ном) Тогда
		...
		ДопПараметры.Вставить("омг_Набор", омг_Набор);
		ДопПараметры.Вставить("омг_НаборУровень", омг_НаборУровень);

		ОбработкаРеквизитаУпр("Работы.Номенклатура",СтрокаТабличнойЧасти,ЭтаФорма, ДопПараметры);
		// )
...
Процедура КлиентПриИзменении(Элемент, ПриОткрытии = Ложь)
	...
	//DreAM++
	КартаСкидкиКлиента = ИнтеграцияЕИП.ПолучитьКартуСкидкиКлиента(Объект.Клиент);
	Если ЗначениеЗаполнено(КартаСкидкиКлиента) и не ЗначениеЗаполнено(Объект.омг_СтраховаяПрограмма) Тогда
		Объект.КартаКлиента = КартаСкидкиКлиента;
	КонецЕсли;
	//DreAM--
...
Процедура РасчитатьСуммуДокумента()
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Если омг_Предоплата.омг_ИспользуетсяПредоплата(Организация) Тогда
			омг_ОбновитьПредоплату();	
			омг_СуммаПредоплатыСписать = 0; 
			Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОказанияУслуг.ПродажаКлиенту") Тогда 
				
				РаботаСДокументамиКлиентСервер.РасчитатьСуммуДокументаОказанияУслуг(Объект,,омг_ПредоплатаФорма,омг_СуммаПредоплатыСписать);   
				
				Если не Объект.омг_СуммаПредоплатыСписать = омг_СуммаПредоплатыСписать Тогда
					
					Объект.омг_СуммаПредоплатыСписать = омг_СуммаПредоплатыСписать
					
				КонецЕСли;
			иначе
				РаботаСДокументамиКлиентСервер.РасчитатьСуммуДокументаОказанияУслуг(Объект,,Объект.омг_СуммаПредоплатыСписать,Объект.омг_СуммаПредоплатыСписать);   
				
				Если не Объект.омг_СуммаПредоплатыСписать = омг_СуммаПредоплатыСписать Тогда
					
					омг_СуммаПредоплатыСписать  = Объект.омг_СуммаПредоплатыСписать 
...
Процедура ПроверитьПрейскурантФилиала(ДополнительныеПараметры)
	...
	Тогда
		...
		Иначе
			...
			РаботаСФормамиКлиент.УстановитьПараметрПрейскурантПодбора(ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОплаченоБезналичнымПриИзменении(Элемент)
	...
	////////////////////////////////////////////////////////////////////
	// ОМГ
	Если ЗначениеЗаполнено(Объект.Касса) и ЗначениеЗаполнено(ЭквайринговыйТерминалНаФорме) 
		и  Объект.ПолученоНаличными  >= Объект.СуммаДокумента и ОплаченоБезналичным <= Объект.СуммаДокумента Тогда
		Объект.ПолученоНаличными = Объект.СуммаДокумента - ОплаченоБезналичным 
	КонецЕсли;	
	...
	Если Объект.СуммаДокумента <> ОплаченоБезналичным + Объект.ПолученоНаличными Тогда
		...
		Если Разница < 0  И значениеЗаполнено(ЭквайринговыйТерминалНаФорме) Тогда
			ОплаченоБезналичным = Объект.СуммаДокумента;	
			Объект.ПолученоНаличными = 0;
		иначе
...
Процедура ЭкваринговыйТерминалНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//////////////////////////////////////////////////////////////
	// ОМГ
	    И не омг_МедицинскаяДеятельность.омг_РольДоступна("ПолныеПрава")
	// ОМГ   
	//////////////////////////////////////////////////////////////
	Тогда
...
Процедура Оплата_СуммаОплатыЧерезНСПК_ПриИзменении(Сумма)
	...
	РаботаСФормамиКлиент.ОплатаПриОкончанииРедактирования(ЭтаФорма,Неопределено, Неопределено, Неопределено);
	ОбнулениеОплатыНаличными();
	Если СуммаОплаты > Объект.СуммаДокумента И СдачаИлиДепозит = 1 Тогда
		ОплаченоБезналичнымОткрытие(Неопределено, Ложь);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Изменения Макеев 02-04-2026 (
	////////////////////////////////////////////////////////////
	// ОМГ   
	Если Объект.Ссылка.Пустая() И РольДоступна("омг_СозданиеОказанияУслуг") или 
		Объект.Ссылка.Пустая() И РольДоступна("омг_РегистраторПЗБ") или
		Объект.Ссылка.Пустая() И РольДоступна("омг_СозданиеОказанияУслуг")
		Тогда  
	иначе	
	...
	Если РольДоступна("омг_ЗапретРедктированияСкидокВОказанииУслуг") Тогда 
		Элементы.КартаКлиента.ТолькоПросмотр = Истина;
	КонецЕсли;  
	
	Если не РольДоступна("ДополнительныеПраваРедактированиеФискализированныхДокументовККМ") 
		и Объект.СостояниеЧековККМ = Перечисления.СостоянияЧековККМ.ЧекиПробиты и не Объект.Ссылка.Пустая() Тогда 
		омг_ЗаблокироватьОтИзменний()
	КонецЕсли;  
			
	Если Объект.Ссылка.Пустая() И не ЗначениеЗаполнено(Объект.Сотрудник) Тогда
    	Объект.Сотрудник = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ПараметрыСеанса.ТекущийПользователь,"омг_ОсновнойВрачВОказанииУслуг") 
	КонецЕсли; 
	...
	// ОМГ
	Если Объект.Ссылка.Пустая() Тогда
	КасаОМГ = омг_WebKassa.ВернутьКассуПоРабочемуМесту(); 
	
	Если ЗначениеЗаполнено(КасаОМГ)  Тогда
		  Объект.Касса = КасаОМГ
	  КонецЕсли;
	...
	////////////////////////////////////////////////////////////
	// ОМГ
	Если Объект.СостояниеЧековККМ = Перечисления.СостоянияЧековККМ.ЧекиПробиты Тогда
	...
	Если ЗначениеЗаполнено(Объект.омг_СтраховаяПрограмма) = Перечисления.СостоянияЧековККМ.ЧекиПробиты Тогда
	...
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("НомерЧекаЭТВирт",Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(500))));
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("омг_НомерТранзакцииТерминалаВирт",Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(500))));
	ИзменитьРеквизиты(ДобавляемыеРеквизиты); 
	ОбновитьВыбранныеПозицииДляПодбора();
	// )
	...
	Если РольДоступна("омг_Регистратор") и не РольДоступна("ПолныеПрава") Тогда 
		Элементы.РаботыЦена.Доступность = Ложь;
		Элементы.РаботыСумма.Доступность = Ложь;
	КонецЕсли;
	
	Если РольДоступна("омг_ДопПраваРегистратор") и Объект.СостояниеЧековККМ = Перечисления.СостоянияЧековККМ.ЧекиПробиты и не Объект.Ссылка.Пустая() Тогда 
		Если Не Элементы.Найти("КнопкаФилиал") = Неопределено Тогда
			Элементы.КнопкаФилиал.Доступность = Истина;
		КонецЕсли;
	КонецЕсли;
	
	Если РольДоступна("омг_УбратьРучныеСкидки") Тогда 
		Элементы.РучноеРедактированиеСкидок.Доступность = Ложь;
		Элементы.РаботыПроцентСкидки.Доступность = Ложь;
		Элементы.РаботыСуммаСкидки.Доступность = Ложь;
	КонецЕсли;

	
	//Если ЗначениеЗаполнено(Объект.омг_Причина) Тогда
	//НовЭлемент = Элементы.Добавить("ПричинаИзменения",Тип("ПолеФормы"),Элементы.ГруппаДокумента);
	НовЭлемент = Элементы.Добавить("ПричинаИзменения",Тип("ПолеФормы"),Элементы.ГруппаУказанияПричиныОтмены); 
	НовЭлемент.ПутьКДанным = "Объект.омг_Причина";
	НовЭлемент.Вид = ВидПоляФормы.ПолеВвода; 
	НовЭлемент.Заголовок = "Причина пометки на удаление/ снятия с проведения "; 
	НовЭлемент.АвтоМаксимальнаяШирина = Ложь; 
	НовЭлемент.МаксимальнаяШирина = 0;
	НовЭлемент.РастягиватьПоГоризонтали = Истина; 
	
	//Элементы.Переместить(НовЭлемент,Элементы.ГруппаДокумента,Элементы.ГруппаИнтеграция);
	//КонецЕсли; 
	Если Объект.Ссылка.Пустая() Тогда	
		омг_РаботаСНДС()
	КонецЕсли;    
	
	Если не Объект.ВидОперации = Перечисления.ВидыОперацийОказанияУслуг.ПродажаКлиенту Тогда
	Элементы.СуммаДокумента.Заголовок = "Итого к возврату"
	КонецЕсли; 
	
	ВидимостьПредоплаты();
	ЗаполнитьИнструкциюНаСервере();  
	
	Если Объект.ВидОперации = Перечисления.ВидыОперацийОказанияУслуг.ВозвратКлиенту Тогда

		Элементы.Декорация9.Заголовок = "Итого к возврату";
		Элементы.Декорация10.Заголовок = "Возвращено всего"
...
Процедура ПриОткрытии() 
	...
	// Добавлено Макеев (
	Если Не Объект.омг_СтраховаяПрограмма.Пустая() И Параметры.Ключ.Пустая() Тогда
		Объект.СуммаДокумента = 0;
		ОплаченоБезналичным = 0;
		ОплаченоБезналичнымПриИзменении(Неопределено);
		Объект.ПолученоНаличными = 0;
		ПолученоНаличнымиПриИзменении(Неопределено);    
		Элементы.КартаКлиента.ТолькоПросмотр = Истина;
	КонецЕсли;
	омг_ИзменитьВидимостьВводаКоличестваЦены(Элементы, ЭтаФорма);  
	омг_ЗаблокироватьОтИзменний();
	...
	Если ЗначениеЗаполнено(Объект.омг_СтраховаяПрограмма) Тогда
		Элементы.ГруппаСтраховойПрограммы.Показать();
	КонецЕсли;
	ОбновитьИнформациюОКорзине();
	ОбновитьИнформациюОСкидках();
	//)
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура омг_ИзменитьВидимостьВводаКоличестваЦены(Элементы, ЭтаФорма)
	
	Если омг_ЕстьДоступНаВводКоличестваИЦены() Тогда
		Элементы.ПолеЗапрашиватьКоличество.Видимость = Истина;
		Элементы.ПолеЗапрашиватьЦену.Видимость = Истина;
		Элементы.ПолеЗапрашиватьХарактеристику.Видимость = Истина;
	Иначе
		ЭтаФорма.ЗапрашиватьКоличество = Ложь;
		ЭтаФорма.ЗапрашиватьЦену = Ложь;
		ЭтаФорма.ЗапрашиватьХарактеристику = Ложь;
		Элементы.ПолеЗапрашиватьКоличество.Видимость = Ложь;
		Элементы.ПолеЗапрашиватьЦену.Видимость = Ложь;
		Элементы.ПолеЗапрашиватьХарактеристику.Видимость = Ложь;
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция омг_ЕстьДоступНаВводКоличестваИЦены()
	
	Возврат РольДоступна("омг_ПравоВводитьКоличествоИЦенуПриПодбореВОказанииУслуг");
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//////////////////////////////////////////////////////////////////////////////
	//// ОМГ +  
	//Если ЗначениеЗаполнено(Объект.Ссылка) и омг_МедицинскаяДеятельность.омг_РольДоступна("омг_Регистратор") Тогда	
	//	Если не омг_МедицинскаяДеятельность.омг_ПроверкаПометкиНаУдаление(Объект.Ссылка) и Объект.ПометкаУдаления
	//		Или ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда	
	//		Если Объект.НомерЧекаККМ = 0 Тогда
	//			Стр = "";
	//			Если ВвестиСтроку(
	//				Стр,
	//				"Введите причину изменения документа!",
	//				0, // (необ.) длина
	//				Истина // (необ.) многострочность
	//				)
	//				Тогда 
	//				Объект.омг_Причина = Стр; 
	//			иначе 
	//				Отказ = Истина;
	//				Сообщение = Новый СообщениеПользователю;
	//				Сообщение.Текст = "Неободимо ввести причину изменения документа!";
	//				Сообщение.Сообщить();
	//				Возврат;	
	//			КонецЕсли;
	//		КонецЕсли;		
	...
	РаботаСДокументамиКлиентСервер.РассчитатьОплатуСдачуПоПолученнымНаличным(Объект);
	Если не ЗначениеЗаполнено(Объект.Ссылка) и НачалоДня(Объект.Дата) = НачалоДня(ТекущаяДата()) Тогда
...
&НаКлиенте
Процедура ПослеВводаПричиныОтмены(ВведеннаяСтрока, ПараметрыЗаписи) Экспорт
	...
	Если СтрДлина(ВведеннаяСтрока) < 10 Тогда
		Сообщить("Причина слишком короткая. Изменение документа прервано.", СтатусСообщения.Важное);
		Возврат;
	КонецЕсли;
	
	// Записываем введенный текст в реквизит объекта
	Объект.омг_Причина = ВведеннаяСтрока;
	
	// Запускаем процесс записи заново. 
	// Теперь СтрДлина >= 10, блок проверок пропустит нас дальше.
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	////////////////////////////////////////////////////////////////////
	// ОМГ    
	Если Объект.ВидОперации = Перечисления.ВидыОперацийОказанияУслуг.ВозвратКлиенту и РольДоступна("омг_Регистратор") Тогда
		
		РаботыИКоличество = Объект.Работы.Выгрузить(,"Номенклатура, Количество");
		РаботыИКоличество.Свернуть("Номенклатура", "Количество");
		Отказ = не омг_МедицинскаяДеятельность.омг_ПроверкаВозвратов(Объект.ДокументОснование, Объект.Сотрудник,Объект.Клиент,Объект.Касса,ЭквайринговыйТерминалНаФорме,РаботыИКоличество);		
...
Процедура ОбработкаВыбора(ЗначениеВыбора, ИсточникВыбора, НовСтр = Неопределено)
	...
	Если ТипЗнч(ЗначениеВыбора) = Тип("Структура") Тогда
		...
		Если Действие = "ПодборРаботы" Тогда
			...
			ИначеЕсли ДопСерверныеФункции.ПолучитьРеквизит(ЗначениеВыбора.Номенклатура, "ВидНоменклатуры") = ПредопределенноеЗначение("Перечисление.ВидыНоменклатуры.Набор") Тогда
			////////////////////////////////////////////////////////
			// ОМГ
				...
				МассивНаборов.Добавить(ЗначениеВыбора.Номенклатура);
				омг_ПолучитьВсеНаборы(СоставНабора,МассивНаборов,МассивКомплектующих);
				Для каждого СоставНабора из МассивКомплектующих Цикл
					ОбработкаВнесенияНоменклатуры(СоставНабора.Комплектующая, СоставНабора.ХарактеристикаКомплектующей,Неопределено,
						СоставНабора.Количество * ЗначениеВыбора.Количество,,,,,,ДобавленныеСтроки,, ,ЗначениеВыбора.Номенклатура,СоставНабора.омг_Набор, СоставНабора.ЭтоНаборЛИС);
				...
				//									СоставНабора[Инд].Количество * ЗначениеВыбора.Количество,,,,,,ДобавленныеСтроки,, ,ЗначениеВыбора.Номенклатура);
				//	// )
				//КонецЦикла; 
		...
		Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.ГруппаРаботы;
		ВсеДобавленныеСтроки = Новый Массив; 
		
		Для Каждого ДанныеУслуги Из ЗначениеВыбора Цикл
			
			ЦенаУслуги = 0;
			ДанныеУслуги.Свойство("Цена", ЦенаУслуги);
			
			Характеристика = Неопределено;
			ДанныеУслуги.Свойство("ХарактеристикаНоменклатуры", Характеристика);
			
			Единица = Неопределено;
			ДанныеУслуги.Свойство("ЕдиницаИзмерения", Единица);
			
			ВидНом = ДопСерверныеФункции.ПолучитьРеквизит(ДанныеУслуги.Номенклатура, "ВидНоменклатуры");
			
			// Создаем локальный массив, чтобы поймать ИД только что добавленной строки
			СтрокиОднойУслуги = Новый Массив; 
			
			Если ВидНом = ПредопределенноеЗначение("Перечисление.ВидыНоменклатуры.Услуга") Тогда
				ОбработкаВнесенияНоменклатуры(ДанныеУслуги.Номенклатура, Характеристика, Единица,
											  ДанныеУслуги.Количество, ЦенаУслуги,,,,Истина,СтрокиОднойУслуги);
											  
			ИначеЕсли ВидНом = ПредопределенноеЗначение("Перечисление.ВидыНоменклатуры.Набор") Тогда
				СоставНабора = ОбщегоНазначенияСервер.ПолучитьСоставНабора(ДанныеУслуги.Номенклатура); 
				МассивНаборов = Новый Массив;
				МассивКомплектующих = Новый Массив; 
				МассивНаборов.Добавить(ДанныеУслуги.Номенклатура);
				омг_ПолучитьВсеНаборы(СоставНабора, МассивНаборов, МассивКомплектующих);
				
				Для каждого СоставНабора Из МассивКомплектующих Цикл
					ОбработкаВнесенияНоменклатуры(СоставНабора.Комплектующая, СоставНабора.ХарактеристикаКомплектующей, Неопределено,
						СоставНабора.Количество * ДанныеУслуги.Количество,,,,,,СтрокиОднойУслуги,, ,ДанныеУслуги.Номенклатура, СоставНабора.омг_Набор, СоставНабора.ЭтоНаборЛИС);
				КонецЦикла;
			КонецЕсли;
			
			Для Каждого СтрокаТаблицы Из СтрокиОднойУслуги Цикл
				
				ЗначениеНабора = Неопределено;
				Если ДанныеУслуги.Свойство("омг_Набор", ЗначениеНабора) И ЗначениеЗаполнено(ЗначениеНабора) Тогда
					СтрокаТаблицы.омг_Набор = ЗначениеНабора;
				КонецЕсли;
				
				ЗначениеНабораЛИС = Неопределено;
				Если ДанныеУслуги.Свойство("омг_НаборЛИС", ЗначениеНабораЛИС) И ЗначениеЗаполнено(ЗначениеНабораЛИС) Тогда
					СтрокаТаблицы.омг_НаборЛИС = ЗначениеНабораЛИС;
				КонецЕсли;
				
				ВсеДобавленныеСтроки.Добавить(СтрокаТаблицы);
				
			КонецЦикла;			
		КонецЦикла;
		
		ОбновитьФормыУчастияПослеИзмененияРабот(ВсеДобавленныеСтроки, Истина);
		ОбновитьВыбранныеПозицииДляПодбора();
...
Процедура ПолученоНаличнымиПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Касса) и ЗначениеЗаполнено(ЭквайринговыйТерминалНаФорме) 
		и ОплаченоБезналичным >= Объект.СуммаДокумента и Объект.ПолученоНаличными <= Объект.СуммаДокумента Тогда
		ОплаченоБезналичным = Объект.СуммаДокумента - Объект.ПолученоНаличными 
	КонецЕсли;	
	...
	Если ЗначениеЗаполнено(Объект.Касса) и не ЗначениеЗаполнено(ЭквайринговыйТерминалНаФорме) 
		и Объект.ПолученоНаличными >= Объект.СуммаДокумента  Тогда
		Объект.ПолученоНаличными  = Объект.СуммаДокумента 
	КонецЕсли;  
	
	Если ЗначениеЗаполнено(Объект.Касса) и  ЗначениеЗаполнено(ЭквайринговыйТерминалНаФорме) 
		и Объект.ПолученоНаличными + ОплаченоБезналичным > Объект.СуммаДокумента  Тогда
		Объект.ПолученоНаличными  =  ?((Объект.СуммаДокумента - ОплаченоБезналичным)> 0,Объект.СуммаДокумента - ОплаченоБезналичным,0); 
	КонецЕсли;
...
Процедура ОбработкаВнесенияНоменклатуры(Номенклатура, ХарактеристикаНоменклатуры, Единица, Количество = 1, 
										...
										, омг_Набор = Неопределено,омг_НаборУровень = Неопределено, омг_ЭтоНаборЛИС = Ложь)
	// Изменения Макеев 04-12-2025 (
	...
	ИначеЕсли ЭтоУслуга Тогда
		...
		КонецЕсли;
		// Добавлено Макеев (
		Если ЗначениеЗаполнено(омг_НаборУровень) Тогда
			СтруктураПоиска.Вставить("омг_Набор", омг_НаборУровень);
		ИначеЕсли ЗначениеЗаполнено(омг_Набор) Тогда
			СтруктураПоиска.Вставить("омг_Набор", омг_Набор);
		КонецЕсли; 
		//// Ищем по верхнему уровню
		//Если ЗначениеЗаполнено(омг_Набор) Тогда
		//	СтруктураПоиска.Вставить("омг_Набор", омг_Набор);
		//КонецЕсли;
		// )
		...
		Если мсСтрокиРаботы.Количество()=0 Тогда
			...
			СтрокаРаботы.КлючСтроки	= ОбщегоНазначенияКлиентСервер.ПолучитьНовыйКлючСтроки(Объект.Работы);
			// Изменено Макеев (
			//РаботыРаботаПриИзмененииТекСтрока(Неопределено, СтрокаРаботы.НомерСтроки-1, омг_НаборУровень);
			РаботыРаботаПриИзмененииТекСтрока(Неопределено, СтрокаРаботы.НомерСтроки-1,омг_Набор, омг_НаборУровень);
			// )
			// Добавлено Макеев (  
			СтрокаРаботы.омг_НаправившийВрач = ЭтаФорма.омг_ТекущийВрач;
			СтрокаРаботы.омг_Набор = омг_Набор;
			СтрокаРаботы.омг_НаборЛИС = ?(омг_ЭтоНаборЛИС, омг_НаборУровень, Неопределено);
			// )
			...
			////////////////////////////////////////////////////  
			// ОМГ   
			Если не ЗначениеЗаполнено(Объект.омг_СтраховаяПрограмма) Тогда
					СтрокаРаботы.Сумма = Окр(Количество*СтрокаРаботы.Цена,0,РежимОкругления.Окр15как10); 
				иначе
	...
	Иначе // Товар	
		...
		Если СтраницаДобавления = ПанельТЧ.ПодчиненныеЭлементы.ГруппаМатериалы Тогда
			...
			Тогда
				...
				ЗаполнитьЗначенияСвойств(СтрокаТовар, ПолучитьЕдиницуИКоэф(Номенклатура, Единица)); // Заполнить ЕдиницаИзмерения, Коэффициент.
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Штрихкод) Тогда
				...
				Тогда
					ЭтоШтрихкодМаркировки = Истина;
				КонецЕсли;
			КонецЕсли;
			
			Если Не ЭтоШтрихкодМаркировки Тогда 
				СтруктураПоиска.Вставить("Штрихкод", ""); // Не ищем строки с непустым ШК маркировки в табличной части.
			КонецЕсли;
			мсСтрокиТовары = Объект.Товары.НайтиСтроки(СтруктураПоиска);
			
			Если ЭтоШтрихкодМаркировки Тогда
				Для Каждого СтрокаТовар Из мсСтрокиТовары Цикл // Анализ итога поиска без отбора по штрикходу.
					...
					КонецЕсли;
				КонецЦикла;
				мсСтрокиТовары.Очистить(); // Очищаем результат поиска, чтобы добавилась новая строка товаров.
			КонецЕсли;
...
Процедура Подключаемый_ВыборПодбор(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.ГруппаРаботы Тогда
		...
		// Добавлено Макеев (
		Если Элемент.ТекущиеДанные.Приостановлено И Не СтруктураПараметров.Свойство("ПропускВопросаОПриостановлении") Тогда
			
			ТекстВопроса = "Исследование было приостановлено лабораторией. Продолжить добавление в документ?";
			ПараметрыОповещения = Новый Структура("ВыбранноеЗначение, Элемент", ВыбранноеЗначение, Элемент);
			ОписаниеОповещения = Новый ОписаниеОповещения("Обработчик_ПослеОтветаНаВопросОДобавленииПриостановленного", ЭтаФорма, ПараметрыОповещения);
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 60, КодВозвратаДиалога.Да, "", КодВозвратаДиалога.Нет);
			
			Возврат;
		Иначе
			СтруктураПараметров.Удалить("ПропускВопросаОПриостановлении");
		КонецЕсли;
		// )
		//// Добавлено Макеев (
		//Если Не ЗначениеЗаполнено(Элемент.ТекущиеДанные.Цена) Тогда
		//	Возврат;
		//КонецЕсли;
		//// )  
		///////////////////////////////////////////////////////////////////////////
		// ОМГ
		Если ЗначениеЗаполнено(Элемент.ТекущиеДанные.Цена) и ЭтаФорма.омг_ТекущийВрач =	Неопределено Тогда
			...
			Возврат;
		иначеЕсли ЗначениеЗаполнено(Элемент.ТекущиеДанные.Цена) и не ЭтаФорма.омг_ТекущийВрач =	Неопределено и не ВыбранноеЗначение = Неопределено Тогда
	...
	// Добавлено Макеев (
	Результат.Цена = Элемент.ТекущиеДанные.Цена;
	// )
...
Процедура Обработчик_ПослеОтветаНаВопросОДобавленииПриостановленного(Ответ, ДопПараметры) Экспорт
	...
	СтруктураПараметров.Вставить("ПропускВопросаОПриостановлении");
	
	Подключаемый_ВыборПодбор(ДопПараметры.Элемент, ДопПараметры.ВыбранноеЗначение, Ложь);
...
Процедура омг_ПослеВыбораНаправившегоВрача(Врач, Номенклатура) Экспорт
	...
	Если не Врач = Неопределено Тогда
		
		ЭтаФорма.омг_ТекущийВрач = Врач;  
		
	КонецЕсли;
	Результат = РаботаСФормамиКлиент.ВыборПодборОбработка(Номенклатура, ЭтаФорма.ЗапрашиватьКоличество, ЭтаФорма.ЗапрашиватьЦену, ЭтаФорма.ЗапрашиватьХарактеристику, ЭтаФорма.ЗапрашиватьСерию, "ПодборРаботы", ЭтаФорма); 
	
	// Добавлено Макеев (
	Если Не Элементы.СписокПодбор.ТекущиеДанные = Неопределено Тогда
		Результат.Цена = Элементы.СписокПодбор.ТекущиеДанные.Цена;
	КонецЕсли;
	// )
	Если Результат <> Неопределено Тогда
		ОбработкаВыбора(Результат, Неопределено);
	...
	// Добавлено Макеев (
	Если Не Объект.омг_СтраховаяПрограмма.Пустая() Тогда
		Объект.СуммаДокумента = 0;
		ОплаченоБезналичным = 0;
		ОплаченоБезналичнымПриИзменении(Неопределено);
		Объект.ПолученоНаличными = 0;
...
&НаКлиенте
Функция Чек_ЕстьОшибкиНСПК()
	
	Если СтруктураПараметров.Свойство("НСПК_ЭтоВозвратПродажиПоЭсНСПК") И Не ЗначениеЗаполнено(Объект.ИдентификаторКорзиныНСПК) Тогда
		НСПК_ОдобритьКорзину(Неопределено, Истина);
		Возврат Истина; // В случае успешного подтверждения корзины возврата будет вызвана печать чека без этой проверки.
	КонецЕсли;
	
	СтрокаОплатыНСПК = НСПК_СтрокаОплаты();
	ЕстьОплатыНСПК = СтрокаОплатыНСПК <>  Неопределено;
	Если ЗначениеЗаполнено(Объект.ИдентификаторКорзиныНСПК) И Не ЕстьОплатыНСПК Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Отсутствует оплата электронным сертификатом НСПК сертификатом! В документе заполнен идентификатор корзины НСПК, но не строки оплаты по НСПК'"));
		Возврат Истина;
		
	ИначеЕсли Не ЗначениеЗаполнено(Объект.ИдентификаторКорзиныНСПК) И ЕстьОплатыНСПК Тогда
...
Асинх Процедура Чек(Команда, ПроверятьНСПК = Истина)
	...
	Если не ПроверитьЗаполнениеБезнала() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Заполните Способ безналичной оплаты!";
		Сообщение.Поле = "ЭквайринговыйТерминалНаФорме";	
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;  
	
	Если не ПроверитьЗаполнениеНала() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Заполните Кассу!";
	...
	Если Объект.СуммаДокумента <> ОплаченоБезналичным + Объект.ПолученоНаличными Тогда
			ПоказатьПредупреждение(,НСтр("ru='Сумма оплат не совпадает с общей суммой документа!'"));
			Возврат;
	КонецЕсли;	
	
	//////////////////////////////////////////////////////////////////////////////////////
	// ОМГ
    Если не ЗначениеЗаполнено(Объект.Сотрудник) или не ЗначениеЗаполнено(Объект.Клиент) Тогда
			ПоказатьПредупреждение(,НСтр("ru='Заполните врача и клиента!'"));
			Возврат;
	КонецЕсли;
		
	Если не ЗначениеЗаполнено(Объект.Филиал) Тогда
			ПоказатьПредупреждение(,НСтр("ru='Заполните Клинику!'"));
			Возврат;
	КонецЕсли;
	// ОМГ 
	//////////////////////////////////////////////////////////////////////////////////////
	...
	// Проверки НСПК
	Если ПроверятьНСПК Тогда
		Если Чек_ЕстьОшибкиНСПК() Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не (Ждать Чек_ПроверитьНеЗаполненыШКМаркируемыхТоваров()) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьИндикациюЧекиПробиты() Экспорт
	...
	Если не омг_МедицинскаяДеятельность.омг_РольДоступна("ПолныеПрава") Тогда	
...
Процедура ДеревоСкидкиПрименятьСкидкуПриИзменении(Элемент)
	...
	РасчитатьСкидки(мБылаРучнаяПравкаОплаты,Истина);
...
Процедура УстановитьВидимость_ПриСозданииНаСервере()
	...
	////////////////////////////////////////////////////////////////////////
	// ОМГ
	ПоляОбъекта.Вставить("НомерЧекаККМ",Объект.НомерЧекаККМ);
	ПоляОбъекта.Вставить("ССылкаСуществует",ЗначениеЗаполнено(Объект.Ссылка));
...
Процедура УстановитьВидимостьПоТипуОперации_БезКонтекста(Элементы, Объект, мУчетнаяПолитика)
	...
	// Добавлено Макеев ( 
	Если Объект.НомерЧекаККМ > 0 и Объект.ССылкаСуществует Тогда
		 Элементы.ФормаЧек.Заголовок = "ПЕЧАТЬ КОПИИ ЧЕКА";
	Иначе	
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОказанияУслуг.ВозвратКлиенту") Тогда
			Элементы.ФормаЧек.Заголовок = "ОФОРМИТЬ ВОЗВРАТ";
		Иначе 
			Элементы.ФормаЧек.Заголовок = "ПРИНЯТЬ ОПЛАТУ";
		КонецЕсли;  
	КонецЕсли;
	// )
КонецПроцедуры
...
Процедура СоздатьЗаявку(Команда)
	...
	////////////////////////////////////////////////////////////////
	// ОМГ   
	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаСозданиеЗаявки",
      ЭтотОбъект);	
 
    ПоказатьВопрос(Оповещение,
        "Добавить работы в заявку?",
        РежимДиалогаВопрос.ДаНет,
...
&НаСервере
Процедура РасчитатьСкидки(мБылаРучнаяПравкаОплаты, ОбновитьОплату = Ложь)
	...
	Документы.ОказаниеУслуг.РассчитатьСкидкиДокумента(Объект, ОбновитьОплату);
	...
	Если ОбновитьОплату Тогда  
		...
		Для каждого строкаБезНал из НайдСтрокиБезнала Цикл
			СуммаБезНал = СуммаБезНал + строкаБезНал.Сумма
		КонецЦикла;	
		ОплаченоБезналичным = СуммаБезНал;
		...
		Для каждого строкаНал из НайдСтрокиНала Цикл
			СуммаНал = СуммаНал + строкаНал.Сумма
		КонецЦикла;	
		Объект.ПолученоНаличными = СуммаНал;
...
Процедура КартаКлиентаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.КартаКлиента) Тогда
		...
		РеквизитыКарты = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Объект.КартаКлиента, "СрокДействия, ВладелецКарты, омг_ТребуетОснования");
			
		Если РеквизитыКарты.омг_ТребуетОснования Тогда  
			
			ЗапроситьОснованиеСкидки() 
...
Процедура ТабличноеПолеОплатаЭквайринговыйТерминалНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если 	ТекущиеДанные = Неопределено
		Или ТекущиеДанные.ВидОплаты <> ПредопределенноеЗначение("Перечисление.ВидыОплаты.Безналичные")
	Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыВыбораЗначения <> Неопределено Тогда
		Оповещение = Новый ОписаниеОповещения("ВыборЭТОплаты_ВыборПолеОплатыЭТПродолжение", ЭтотОбъект, ТекущиеДанные);
		ОткрытьФорму("Справочник.ЭквайринговыеТерминалы.ФормаВыбора", ПараметрыВыбораЗначения,,,,,Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВыборЭТОплаты_ВыборПолеОплатыЭТПродолжение(Результат, ТекущиеДанные) Экспорт
...
Функция ПолучитьРаботыЗаявок(Заявки)
	...
	|	И НЕ ЗаявкаРаботы.Номенклатура.ТолькоДляПланирования
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	// Реквизиты кассового чека (отраслевой, операционный персональные данные)
	ПоместитьРеквизитыККТИзХЗвРеквизитыФормы(ТекущийОбъект);
...
&НаСервере
Процедура ПоместитьРеквизитыККТИзХЗвРеквизитыФормы(ОУОбъект)
...
Асинх Процедура ВыполнитьРедактированиеРеквизитаККТ(НазваниеРеквизитаККТ)
	...
	Если НазваниеРеквизитаККТ = "ПерсональныеДанныеПокупателя" Тогда
		...
		ПерсональныеДанные.Вставить("ЭтоФизЛицо", Не СтрДлина(Объект.ПринятоОтИНН) = 10);
...
&НаКлиенте
Асинх Функция Чек_ПроверитьНеЗаполненыШКМаркируемыхТоваров()
	
	Отказ = Ложь;
	
	РезультатПроверки = ПроверитьНеЗаполненыШКМаркируемыхТоваров();	
	
	Если РезультатПроверки.КонтролироватьЗаполнениеШКМаркированногоТовараПриПродаже > 0 
		И Не ПустаяСтрока(РезультатПроверки.ТекстОшибки)
	Тогда
		// Есть ошибка заполнения, и требуется реакция (согласно настройке)
		Если РезультатПроверки.КонтролироватьЗаполнениеШКМаркированногоТовараПриПродаже = 1 Тогда
			Отказ = Истина;
			ПоказатьПредупреждение(, РезультатПроверки.ТекстОшибки);
		Иначе
			ТекстВопроса = РезультатПроверки.ТекстОшибки + Символы.ПС + "Продолжить пробитие чека?";
			Ответ = Ждать ВопросАсинх(ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, 30, КодВозвратаДиалога.Отмена,,КодВозвратаДиалога.Отмена);
			Если Ответ <> КодВозвратаДиалога.ОК Тогда
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Функция ПроверитьНеЗаполненыШКМаркируемыхТоваров()
	
	ВозвращаемыеДанные = Новый Структура;
	ВозвращаемыеДанные.Вставить("ТекстОшибки", "");
	
	// Приобразование к число нужно для совместимости, ранее был булево!
	КонтролироватьЗаполнениеШКМаркированногоТовараПриПродаже = Число(мУчетнаяПолитика.КонтролироватьЗаполнениеШКМаркированногоТовараПриПродаже);
	
	ВозвращаемыеДанные.Вставить("КонтролироватьЗаполнениеШКМаркированногоТовараПриПродаже", КонтролироватьЗаполнениеШКМаркированногоТовараПриПродаже);
	
	// Проверка маркировки товаров
	Если КонтролироватьЗаполнениеШКМаркированногоТовараПриПродаже > 0 Тогда
		
		НоменклатураБезШК = Новый Массив;
		
		Для Каждого СтрокаТовары Из Объект.Товары Цикл
			Если ПустаяСтрока(СтрокаТовары.Штрихкод) Тогда
				НоменклатураБезШК.Добавить(СтрокаТовары.Номенклатура);
			КонецЕсли;
		КонецЦикла;
		
		Если НоменклатураБезШК.Количество() = 0 Тогда
			Возврат ВозвращаемыеДанные;
		КонецЕсли;
		
		МаркируемаяНоменклатура = МаркируемаяНоменклатураВПеречне(НоменклатураБезШК);
		
		Если МаркируемаяНоменклатура.Количество() = 0 Тогда
			Возврат ВозвращаемыеДанные;
		КонецЕсли;
			
		Для Каждого СтрокаТовара Из Объект.Товары Цикл
			Если ПустаяСтрока(СтрокаТовары.Штрихкод)
				И МаркируемаяНоменклатура.Найти(СтрокаТовары.Номенклатура) <> Неопределено
				И (Не ЗначениеЗаполнено(СтрокаТовара.СерияНоменклатуры)
					Или ДопСерверныеФункции.ПолучитьРеквизит(СтрокаТовара.СерияНоменклатуры, "СерияПромаркированаДляЦелейМДЛП")) // Игнорируем серии безт отметки маркируемости.
			Тогда
				Ошибка = "маркируемый товар с незаполненным штрихкодом!";
				ШаблонОшибки = НСтр("ru='В строке номер ""%1"" табличной части ""Товары"":  %2'");
				ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(ВозвращаемыеДанные.ТекстОшибки, СтрШаблон(ШаблонОшибки, СтрокаТовара.НомерСтроки, Ошибка), Символы.ПС);
			КонецЕсли;
		КонецЦикла;
...
&НаСервереБезКонтекста
Функция МаркируемаяНоменклатураВПеречне(НоменклатураМассив)
	...
	Запрос.Текст = 
		...
		|	И НЕ Номенклатура.ВидМаркированнойПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыМаркированнойПродукции.ПустаяСсылка)";
	
	МаркируемаяНоменклатура = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	Возврат МаркируемаяНоменклатура;
...
Процедура ПриЗаполненииИзмененииШКТовары()
	...
	Тогда
		ДробнаяЧасть = РаботаСДокументамиСервер.ПолучитьДробнуюЧасть(ТекущиеДанные.Номенклатура, ТекущиеДанные.Количество, ТекущиеДанные.ЕдиницаИзмерения);
...
Функция ПроверкаСтрокТоваровМДЛП(Знач Товары)
	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТовары.Штрихкод) Тогда
			Если СтрокаТовары.Количество > 1 И СтрокаТовары.КоличествоЗнаменатель = 0 Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТовары.Штрихкод) И ЗначениеЗаполнено(СтрокаТовары.КоличествоЗнаменатель) Тогда
			...
			ИначеЕсли СтрокаТовары.Коэффициент > СтрокаТовары.КоличествоЗнаменатель Тогда
				Ошибки.Добавить("В единице измерения маркированного товара больше вместимость, чем в потребительской упаковке" + Символы.ПС
															  + "Товары. Стр. " + СтрокаТовары.НомерСтроки);
			ИначеЕсли СтрокаТовары.Количество <> Окр(СтрокаТовары.Количество) Тогда
				Ошибки.Добавить("Количество к реализации долями должно быть целым числом" + Символы.ПС
															  + "Товары. Стр. " + СтрокаТовары.НомерСтроки);
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура НСПК_ОдобритьКорзину(Команда, ЭтоПечатьЧека = Ложь)
	
	Если Не ЭтоПечатьЧека И Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОказанияУслуг.ВозвратКлиенту") Тогда
		ПоказатьПредупреждение(, "Одобрение корзины ЭС НСПК происходит при пробитии чека.");
		Возврат;
	КонецЕсли;
...
Процедура ПредварительнаяПроверкаНСПКЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если Результат.Свойство("ИдентификаторКорзиныНСПК") Тогда
			...
			Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОказанияУслуг.ВозвратКлиенту") Тогда
				Элементы.НСПК_ОдобрениеКорзины.Доступность = Ложь;
			КонецЕсли;
		...
		Если Результат.Свойство("ПечатьЧека") Тогда
			ЭтотОбъект.Записать();
			Чек(Неопределено, Ложь);
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьИнформациюОСоставеКорзины(ИдентификаторКорзины)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СодержимоеКорзиныНСПК.ИдентификаторСтроки КАК ИдентификаторСтроки
	               |ИЗ
...
&НаКлиенте
Процедура НапечататьЧекКоманда(Команда)
	НапечататьЧек();
КонецПроцедуры
...
Процедура НапечататьЧекКлиентЗавершение(РезультатВыполнения, ПараметрыОперации) Экспорт
	...
	ИначеЕсли РезультатВыполнения.Свойство("Результат") И РезультатВыполнения.Результат Тогда
		...
		Объект.омг_ДатаОплаты = Дата(РезультатВыполнения.ВыходныеПараметры[4].ДатаЧека);  
		//Если Объект.Дата >= НачалоДня(Дата("20251126")) Тогда
		Если омг_WebKassa.ПроверкаДатыПереходаНаРегистрФискальныеОперацииРК(Объект.Дата) Тогда
			Если не омг_WebKassa.ВернутьДанныеПервичногоЧека(Объект.Ссылка,Истина) = Неопределено Тогда
...
Процедура ФискальнаяОперацияОбновитьСтатус()
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийОказанияУслуг.ПродажаКлиенту Тогда
...
Процедура Омг_ПослеЗакрытияВопроса(Результат, Параметры) Экспорт
	...
	Если Параметры = "Смешанный" или Параметры = "Безналичными" Тогда	
		// Добавлено Макеев 			
			ДанныеТерминалаИнтеграция = омг_WebKassa.омг_ПолучитьIPАдресТерминала(ЭквайринговыйТерминалНаФорме);
			
			Если ОплаченоБезналичным > 0 И Не ДанныеТерминалаИнтеграция.АдресТерминала = "" И ДанныеТерминалаИнтеграция.ИнтеграцияОплатаНаТерминал Тогда
				ПараметрыФормы = Новый Структура;
				ПараметрыФормы.Вставить("ДанныеТерминалаИнтеграция", ДанныеТерминалаИнтеграция); 
				ПараметрыФормы.Вставить("Сумма",              ОплаченоБезналичным);
				ЭтоПродажа = Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОказанияУслуг.ПродажаКлиенту");
				ПараметрыФормы.Вставить("ВидОперации", ?(ЭтоПродажа, "Продажа", "Возврат"));
...
Функция Омг_ПроверкаСмешаннойОплаты() 
	...
	Возврат	?(ЭтотОбъект.ОплаченоБезналичным > 0 и Объект.ПолученоНаличными > 0,Истина,Ложь);
...
Процедура омг_СтраховаяПрограммаПриИзменении(Элемент)
	...
	Если Не Объект.омг_СтраховаяПрограмма.Пустая() Тогда
		Объект.СуммаДокумента = 0;  
		Объект.КартаКлиента = ПредопределенноеЗначение("Справочник.КартыСкидок.ПустаяСсылка");
		Элементы.КартаКлиента.ТолькоПросмотр = Истина;
		ОплаченоБезналичным = 0;
		Объект.ПолученоНаличными = 0;
	иначе
		Элементы.КартаКлиента.ТолькоПросмотр = Ложь; 
	КонецЕсли;
	...
	//////////////////////////////////////////////////
	// ОМГ
	Если Объект.омг_СтраховаяПрограмма.Пустая() и ОплаченоБезналичным = 0 и Объект.ПолученоНаличными = 0 Тогда
		Если  ЗначениеЗаполнено(ЭквайринговыйТерминалНаФорме) Тогда
			ОплаченоБезналичным = Объект.СуммаДокумента
...
Процедура омг_ПересчитатьОплаты()
	...
	Если не ЗначениеЗаполнено(ЭквайринговыйТерминалНаФорме) Тогда  
		     ОплаченоБезналичным = 0; 
			 //Сообщить("Заполните способ безналичной оплаты!")
	КонецЕсли;	
    	
	омг_ПересчитатьОплатыНаСервере(); 
...
&НаКлиенте
Процедура Омг_ЭмуляцияРаботыТерминалаВирт(Команда)
    ...
    ПоказатьВопрос(Оповещение,
        "Отправить чек в систему Web kassa?",
        РежимДиалогаВопрос.ДаНетОтмена,
        0, // таймаут в секундах
        КодВозвратаДиалога.Да, // (необ.) кнопка по умолчанию
        "Пробить чек" // (необ.) заголовок
    );   
...
Процедура ДопРеквизиты()
	...
	Если РольДоступна("омг_ПравоПробитияЧека") Тогда   	
	НоваяКоманда = ЭтаФорма.Команды.Добавить("Омг_ЭмуляцияРаботыТерминалаВиртКоманда");
	НоваяКоманда.Заголовок = "Омг_ЭмуляцияРаботыТерминалаВирт ";
	НоваяКоманда.Действие = "Омг_ЭмуляцияРаботыТерминалаВирт"; 		
		
	//НовыйЭлемент = ЭтаФорма.Элементы.Добавить("НомерЧекаВирт", Тип("ПолеФормы"),Элементы.ГруппаОбщая);
	//НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	//НовыйЭлемент.ПутьКДанным = "НомерЧекаЭТВирт";
	//
	//НовыйЭлемент = ЭтаФорма.Элементы.Добавить("НомерТранзакцииТерминалаВирт", Тип("ПолеФормы"),Элементы.ГруппаОбщая);
	//НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	//НовыйЭлемент.ПутьКДанным = "омг_НомерТранзакцииТерминалаВирт";
		
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("ЭмуляцияТерминалаВебкасса", Тип("КнопкаФормы"),Элементы.ГруппаОбщая);
	НовыйЭлемент.Заголовок = "Пробить чек в Web kassa";
	НовыйЭлемент.ИмяКоманды = "Омг_ЭмуляцияРаботыТерминалаВиртКоманда";
	//Изменено Карсаков (
	НовыйЭлемент.РастягиватьПоГоризонтали = Истина;
	//Элементы.Переместить(НовыйЭлемент,Элементы.ГруппаПрейскурант,Элементы.НадписьОстатокСертификата);
	Элементы.Переместить(НовыйЭлемент,Элементы.ГруппаДопКомандЧека, );
	// ) Конец изменений
	КонецЕсли;
 
	Если Объект.НомерЧекаККМ <> 0 Тогда 
	Элементы.ОткрытьЧек.Видимость = Истина
	иначе
	Элементы.ОткрытьЧек.Видимость = Ложь
	КонецЕсли;

	ЭлементУФ = УсловноеОформление.Элементы.Добавить();
	ПоляУФ = ЭлементУФ.Поля.Элементы.Добавить();
	ПоляУФ.Поле = Новый ПолеКомпоновкиДанных("СписокПодбор");
	ШрифтОформления = ЭлементУФ.Оформление.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Шрифт")).Значение;
	
	ЭлементУФ.Оформление.УстановитьЗначениеПараметра("Шрифт", Новый Шрифт(ШрифтОформления, ,, Истина));
	ЭлементОтбора = ЭлементУФ.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("РеквизитСписокПодбор.ПозицияВыбрана");
	ЭлементОтбора.ПравоеЗначение = Истина;
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_ДатаОплаты", Тип("ПолеФормы"),Элементы.ГруппаНомерДата);
	НовыйЭлемент.Заголовок = "Дата оплаты";
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.омг_ДатаОплаты";
	НовыйЭлемент.ТолькоПросмотр = Истина;

КонецПроцедуры

&НаКлиенте
Процедура КлиентОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
    //DreAM++
	КартаСкидкиКлиента = ИнтеграцияЕИП.ПолучитьКартуСкидкиКлиента(ВыбранноеЗначение);
...
Функция омг_ДополнитьЗаявками();
	...
	//|	И Заявка.Ссылка <> &ДокументОснование
	|	И Заявка.ПометкаУдаления = ЛОЖЬ
	//|	И Заявка.ДатаНачала МЕЖДУ &ПериодНачала И &ПериодОкончания
	|
	|СГРУППИРОВАТЬ ПО
	|	Заявка.Ссылка";
	...
	Запрос.УстановитьПараметр("ПериодНачала", НачалоДня(Объект.ДатаНачала));
	Запрос.УстановитьПараметр("ПериодОкончания", КонецДня(Объект.ДатаНачала));
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Заявки.Добавить(ВыборкаДетальныеЗаписи.Ссылка)
	КонецЦикла; 
	
	Если не Объект.ДокументОснование = Неопределено Тогда
		Заявки.Добавить(Объект.ДокументОснование)	
	КонецЕсли;	   
...
Процедура ПослеЗакрытияВопроса(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Возврат;
        КонецЕсли;
		Объект.НомерЧекаЭТ         					= ""  + ТекущаяДата();
		Объект.омг_НомерТранзакцииТерминала         =  Объект.НомерЧекаЭТ;
		Объект.омг_ДатаОплаты         					= ТекущаяДата(); 
		//Если Объект.Дата >= НачалоДня(Дата("20251126")) Тогда
		Если омг_WebKassa.ПроверкаДатыПереходаНаРегистрФискальныеОперацииРК(Объект.Дата) Тогда
...
КонецПроцедуры
 ...
 &НаСервере  
Процедура омг_ПересчитатьОплатыНаСервере()
	
	Документы.ОказаниеУслуг.омг_ПересчитатьОплатыНаСервере(Объект, ЭтаФорма);
			
КонецПроцедуры	

&НаКлиенте
Процедура омг_КодСкидкиОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	
	Объект.омг_КодСкидки = Текст; 
	ПересчитатьСкидкуНаКлиенте();
...
Процедура ОткрытьЧек(Команда)
	Если Объект.НомерЧекаККМ <> 0 Тогда 
		ТекстСообщения = "";
		мДанныеXML = РаботаСДокументамиСервер.ПолучитьДанныеXMLИсторииПечатиПоДокументу(Объект.Ссылка);
		...
		иначе
			...
			Печать(ТабДок,Объект.Ссылка,мДанныеXML);
...
&НаСервере
Процедура Печать(ТабДок, ПараметрКоманды,мДанныеXML)
	Документы.ОказаниеУслуг.омг_ПечатьЧека(ПараметрКоманды, мДанныеXML, ТабДок);
КонецПроцедуры


 &НаСервере
Процедура омг_ВидимостьКопииЧека()
...
&НаКлиенте
Процедура ПослеЗакрытияВопросаСозданиеЗаявки(Результат, Параметры) Экспорт
 
    Если Результат = КодВозвратаДиалога.Да Тогда
       	ЗначенияЗаполнения = Новый Структура("Клиент, Сотрудник, Работы", Объект.Клиент, Объект.Сотрудник, Объект.Работы);
    иначе
...
Процедура ПолученоНаличнымиИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
	Омг_ПроверкаСпособовОплат(Истина)	
КонецПроцедуры  

&НаКлиенте
Процедура Омг_ПроверкаСпособовОплат(ЭтоНал = Ложь)

	Если ЭтоНал и не ЗначениеЗаполнено(Объект.Касса) Тогда
		
	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаСпособОплаты",
      ЭтотОбъект,ЭтоНал);	
    ...
    ПоказатьВопрос(Оповещение,
        ...
        "Заполните кассу!" // (необ.) заголовок
    );   
	
	КонецЕсли;

    Если не ЭтоНал и не ЗначениеЗаполнено(ЭквайринговыйТерминалНаФорме) Тогда
    ...
    ПоказатьВопрос(Оповещение,
        ...
        0, // таймаут в секундах
        КодВозвратаДиалога.ОК, // (необ.) кнопка по умолчанию
        "Заполните способ безналичной оплаты!" // (необ.) заголовок
    );   
...
&НаКлиенте
Процедура ПослеЗакрытияВопросаСпособОплаты(Результат, Параметры) Экспорт
	...
	КонецЕсли; 
 	...
 	ОповеститьПользователя(Параметры);	
		
КонецПроцедуры


&НаКлиенте
Процедура ОплаченоБезналичнымИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
	
	Омг_ПроверкаСпособовОплат(Ложь)

КонецПроцедуры

 &НаКлиенте
 Процедура ОповеститьПользователя(Параметры) 
	 
	
	Если Параметры Тогда

	ОбщегоНазначенияКлиент.СообщитьПользователю("Заполните кассу!",,"Касса","Объект.Касса");
		
	иначе
			
	ОбщегоНазначенияКлиент.СообщитьПользователю("Заполните Способ безналичной оплаты!",,"ЭквайринговыйТерминалНаФорме","ЭквайринговыйТерминалНаФорме");
...
&НаСервере
Процедура ОбновитьВыбранныеПозицииДляПодбора()
	
	ВыбранныеПозиции = Новый Массив;
	Для Каждого СтрокаТаблицы Из Объект.Работы Цикл
		ВыбранныеПозиции.Добавить(СтрокаТаблицы.Номенклатура);
	КонецЦикла;
	ЭтаФорма.РеквизитСписокПодбор.Параметры.УстановитьЗначениеПараметра("ВыбранныеПозиции", ВыбранныеПозиции);
	
КонецПроцедуры

&НаКлиенте
Функция ПроверитьЗаполнениеБезнала()
	
	ВозврЗначение = Истина;
	Если ОплаченоБезналичным > 0 и Не значениеЗаполнено(ЭквайринговыйТерминалНаФорме) Тогда
		ВозврЗначение = Ложь		
	КонецЕсли;   
...
&НаКлиенте
Функция ПроверитьЗаполнениеНала()
	
	ВозврЗначение = Истина;
	Если Объект.ПолученоНаличными > 0 и Не значениеЗаполнено(Объект.Касса) Тогда
		ВозврЗначение = Ложь		
	КонецЕсли;   
	Возврат ВозврЗначение; 
...
&НаСервере
Функция омг_ЗаблокироватьОтИзменний()

	Если не РольДоступна("ДополнительныеПраваРедактированиеФискализированныхДокументовККМ") Тогда
		Блок = ?(Объект.НомерЧекаККМ = 0,Ложь,Истина);
		Элементы.Страницы.ТолькоПросмотр = Блок;
		Элементы.ГруппаПрейскурантВертикальныйФорматСтраницы.ТолькоПросмотр = Блок;
		Элементы.ГруппаШапка.ТолькоПросмотр = Блок;
		Элементы.ПересчитатьСкидки.Видимость = НЕ Блок;
		Элементы.Работы.КоманднаяПанель.Видимость = НЕ Блок;
		Если не Элементы.Найти("ЭмуляцияТерминалаВебкасса") = Неопределено Тогда
			Элементы.ЭмуляцияТерминалаВебкасса.Доступность = НЕ Блок;
		КонецЕсли;
	КонецЕсли;       
		...
		Для каждого страницаЭлемент из Элементы.Страницы.ПодчиненныеЭлементы Цикл 
			Если не страницаЭлемент.Имя = "ГруппаРаботы" Тогда	
				страницаЭлемент.ТолькоПросмотр = Истина;	
			КонецЕсли;	
		КонецЦикла;	 
		
		Для каждого страницаЭлемент из Элементы.Работы.ПодчиненныеЭлементы Цикл 
			Если не страницаЭлемент.Имя = "РаботыСотрудник" и не страницаЭлемент.Имя = "Работыомг_НаправившийВрач" Тогда	
				страницаЭлемент.ТолькоПросмотр = Истина;	
			КонецЕсли;	
		КонецЦикла;  
		Элементы.ГруппаШапка.ТолькоПросмотр = Ложь;
		Элементы.ГруппаПересчитатьСкидку.ТолькоПросмотр = Истина;
		Элементы.ГруппаРеквизитыШапки.ТолькоПросмотр = Ложь;
		//Изменено Трунов
		//Для каждого страницаЭлемент из Элементы.Группа1.ПодчиненныеЭлементы Цикл 
		Для каждого страницаЭлемент из Элементы.ГруппаПересчитатьСкидку.ПодчиненныеЭлементы Цикл
			
		//
		// Изменено Карсаков (
			Если не страницаЭлемент.Имя = "Сотрудник" Тогда
			   Если ТипЗнч(страницаЭлемент) = Тип("ПолеФормы") Или ТипЗнч(страницаЭлемент) = Тип("ТаблицаФормы") Тогда
   					 страницаЭлемент.ТолькоПросмотр = Истина;
			   ИначеЕсли ТипЗнч(страницаЭлемент) = Тип("КнопкаФормы") Тогда
   					 страницаЭлемент.Доступность = Ложь; 
			   КонецЕсли;
...
КонецФункции	
  ...
  &НаКлиенте
Процедура ЗапроситьОснованиеСкидки()
    ...
    ПоказатьВводСтроки(
        ...
        "Введите основание для предоставления скидки",
        0, // (необ.) длина
        Истина // (необ.) многострочность
    );
...
Процедура ПослеВводаСтроки(Результат, Параметры) Экспорт
	...
	ИначеЕсли СтрДлина(Результат) < 3 Тогда    
	    Объект.КартаКлиента = Неопределено;
	   	Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Введите не меньше 3-х символов!";	
		Сообщение.Сообщить();
	иначе
		Объект.омг_ОснованиеСкидки = Результат
    КонецЕсли;
 
КонецПроцедуры

&НаСервере
Функция омг_ДопПраваРегистраторПроверка()
	
	ВозЗначение = Ложь;
	Если не РольДоступна("ПолныеПрава") Тогда
		Если РольДоступна("омг_ДопПраваРегистратор") и Объект.СостояниеЧековККМ = ПредопределенноеЗначение("Перечисление.СостоянияЧековККМ.ЧекиПробиты")и не Объект.Ссылка.Пустая() Тогда 
...
&НаСервере
Процедура омг_РаботаСНДС()
	
	Документы.ОказаниеУслуг.омг_РаботаСНДС(Объект);	
...
Процедура КлиентНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка) 
	СтандартнаяОбработка = Ложь;
	Элементы.ГруппаОКлиенте.Показать();
КонецПроцедуры

&НаКлиенте
Процедура омг_ПринятьВЛабораторию(Команда)
	Если Объект.Ссылка.Пустая() Тогда
...
&НаКлиенте
Процедура ОбновитьИнформациюОСкидках()
	
	Если Объект.Скидки.Количество() > 0 Тогда
		ИнформацияОСкидках = "Есть примененные скидки";
	Иначе
		ИнформацияОСкидках = ""; 
	КонецЕсли;
...
&НаКлиенте
Процедура омг_ЗакрытьКорзину(Команда)
...
&НаКлиенте
Процедура ДеревоСкидкиПриИзменении(Элемент)
	ОбновитьИнформациюОСкидках();
КонецПроцедуры

&НаКлиенте
Процедура ДеревоСкидкиПослеУдаления(Элемент)
	ОбновитьИнформациюОСкидках();
КонецПроцедуры
...
КонецПроцедуры
 ...
 Процедура ВидимостьПредоплаты()
	 ...
	 Если ЗначениеЗаполнено(Организация) Тогда
		...
		Если омг_Предоплата.омг_ИспользуетсяПредоплата(Организация) Тогда 
			...
			Элементы.омг_Предоплата.Видимость = Истина;
			Элементы.омг_СуммаПредоплатыСписать.Видимость = Истина;
			
		иначе
			
			Элементы.омг_Предоплата.Видимость = Ложь;
			Элементы.омг_СуммаПредоплатыСписать.Видимость = Ложь;
 ...
 КонецПроцедуры
&НаСервере
Процедура омг_ОбновитьПредоплату() 
	
	омг_ПредоплатаФорма = 0; 
	Объект.омг_Предоплаты.Очистить(); 
	Организация  = омг_ДополнительныеАлгоритмы.омг_ВернутьОрганизациюПоФилиалу(Объект.Филиал);
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Если омг_Предоплата.омг_ИспользуетсяПредоплата(Организация) Тогда 
			...
			Если Объект.ВидОперации = Перечисления.ВидыОперацийОказанияУслуг.ПродажаКлиенту Тогда
				Если ЗначениеЗаполнено(Объект.Клиент) Тогда
					Объект.омг_Предоплаты.Загрузить(омг_Предоплата.ВернутьСуммуПредоплаты(Объект.Клиент,Объект.Ссылка,Объект.Дата))
				КонецЕсли;   	
				омг_ПредоплатаФорма = Объект.омг_Предоплаты.Итог("СуммаОстаток");
			КонецЕсли; 
		КонецЕсли;
	КонецЕсли;
