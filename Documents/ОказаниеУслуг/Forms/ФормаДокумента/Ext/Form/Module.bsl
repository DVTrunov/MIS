...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийОказанияУслуг.ВозвратКлиенту Тогда
		...
		Отказ = не омг_МедицинскаяДеятельность.омг_ПроверкаВозвратов(Объект.ДокументОснование, Объект.Сотрудник,Объект.Клиент,Объект.Касса,ЭквайринговыйТерминалНаФорме,РаботыИКоличество,Объект.Ссылка);		
...
Асинх Процедура Чек(Команда, ПроверятьНСПК = Истина)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОказанияУслуг.ВозвратКлиенту")
		И Объект.СуммаДокумента < ОплаченоБезналичным + Объект.ПолученоНаличными
	Тогда
		
		СуммаОплатФакт = ОплаченоБезналичным + Объект.ПолученоНаличными;
		
		Если СуммаОплатФакт > 0 Тогда
			
			КоэфПересчета = Объект.СуммаДокумента / СуммаОплатФакт;
			
			НовоеБезнал    = Окр(ОплаченоБезналичным * КоэфПересчета, 2);
			НовоеНаличными = Объект.СуммаДокумента - НовоеБезнал;
			
			ОплаченоБезналичным      = НовоеБезнал;
			Объект.ПолученоНаличными = НовоеНаличными;
	...
	Если не ПроверитьКассуПоРабочемуМесту(МенеджерОборудованияКлиент.ПодключаемоеОборудование().РабочееМесто) Тогда   
		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Поле = "Объект.Касса";
		Сообщение.Текст = "Не верно указана касса!";	
		Сообщение.Сообщить();
		Возврат;
...
Процедура ЗаявкиДокументаСсылкаНажатие(Элемент)
	...
	ВыбранныеЗаявки = ПолучитьЗаявкиДокумента();		
	ПараметрыФормы.Вставить("ВыбранныеЗаявкиДокументаПродажи", ВыбранныеЗаявки);
...
Процедура ОбновитьТекстГиперссылкиЗаявок()
	...
	Если Элементы.ЗаявкиДокументаСсылка.Видимость Тогда
		КоличествоДопЗаявок = Объект.Заявки.Количество()-1;
...
Процедура ОбработкаВыбораЗаявок(ВыбранныеЗаявки, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.Заявка")
		...
		Объект.Заявки.Добавить().Заявка = Объект.ДокументОснование;
	КонецЕсли;
...
Процедура РаботыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Отказ = омг_ДопПраваРегистраторПроверка() или ПроверитьНаВозврат();
...
Функция омг_ЗаблокироватьОтИзменний()
	...
	Если ПроверитьНаВозврат() Тогда
		...
		Для каждого страницаЭлемент из Элементы.ПрескурантТД.ПодчиненныеЭлементы Цикл 
			Если не страницаЭлемент.Имя = "ГруппаПолученоОплаты"  Тогда	
				страницаЭлемент.Видимость = Ложь;	
			КонецЕсли;	
		КонецЦикла;  		
		Элементы.ГруппаДолг.ТолькоПросмотр = Истина;
		Элементы.ГруппаОплатаКомплексногоРасчета.ТолькоПросмотр = Истина; 
		Элементы.Группа6.ТолькоПросмотр = Истина;
		Элементы.ГруппаВремя.ТолькоПросмотр = Истина;
		Элементы.РаботыНоменклатура.ТолькоПросмотр = Истина;
		Элементы.Работыомг_Набор.ТолькоПросмотр = Истина;
		Элементы.Работыомг_НаборЛИС.ТолькоПросмотр = Истина;
		Элементы.Работыомг_НаправившийВрач.ТолькоПросмотр = Истина;
		Элементы.РаботыХарактеристикаНоменклатуры.ТолькоПросмотр = Истина;
		Элементы.РаботыЦена.ТолькоПросмотр = Истина;
		Элементы.РаботыГруппаСкидки.ТолькоПросмотр = Истина;
		Элементы.РаботыСумма.ТолькоПросмотр = Истина;
		Элементы.омг_СтавкаНДС.ТолькоПросмотр = Истина;
		Элементы.РаботыОмг_СуммаНДС.ТолькоПросмотр = Истина;
		Элементы.РаботыСотрудник.ТолькоПросмотр = Истина;
	КонецЕсли;                                      
			...
			Если Объект.ВидОперации = Перечисления.ВидыОперацийОказанияУслуг.ПродажаКлиенту Тогда
				Если ЗначениеЗаполнено(Объект.Клиент) Тогда 
					...
					Если ЗначениеЗаполнено(Объект.ДокументОснование) и ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.Заявка") Тогда
						...
						ДатаПереходаНаУчетПоЗаявкам = омг_Предоплата.ВернутьДатуПереходаУчетаПоЗаявкам(омг_ДополнительныеАлгоритмы.омг_ВернутьОрганизациюПоФилиалу(Объект.Филиал));
						
						Если ЗначениеЗаполнено(ДатаПереходаНаУчетПоЗаявкам) и Объект.Дата >= ДатаПереходаНаУчетПоЗаявкам Тогда
							Заявка = Объект.ДокументОснование;
						КонецЕсли;
						
						Объект.омг_Предоплаты.Загрузить(омг_Предоплата.ВернутьСуммуПредоплаты(Объект.Клиент,Объект.Ссылка,Объект.Дата,Заявка));
...
КонецПроцедуры
	 ...
	 Если ЗначениеЗаполнено(Организация) Тогда
		...
		Если омг_Предоплата.омг_ИспользуетсяПредоплата(Организация) Тогда 
			...
			Если РольДоступна("ПолныеПрава") Тогда
				...
				Элементы.ВернутьПредоплату.Видимость = Истина;
			иначе
				Элементы.ИспользоватьУчетаПоЗаявкам.Видимость = Ложь;
				Элементы.ВернутьПредоплату.Видимость = Ложь;
			КонецЕсли;
 ...
 &НаСервере
 Функция ПроверитьКассуПоРабочемуМесту(РабочееМесто) 
	 
	 Если не мУчетнаяПолитика.Омг_ИспользоватьПроверкуСоответствияКасс Тогда
		 Возврат Истина	 
	 иначе	 
		 ВозврЗначение = Ложь; 
		 Запрос = Новый Запрос;
		 ...
		 |
		 |СГРУППИРОВАТЬ ПО
		 |	ПодключаемоеОборудование.омг_Касса";
		 ...
		 СтрПоиска = Новый Структура("омг_Касса",Объект.Касса);
		 
		 Если ВыборкаДетальныеЗаписи.НайтиСтроки(СтрПоиска).Количество() > 0 Тогда
			 ВозврЗначение = Истина
		 КонецЕсли;  
	 КонецЕсли;
	 Возврат ВозврЗначение; 
...
КонецФункции
 ...
 &НаСервере
 Функция ПроверитьНаВозврат() 
	 
	 Если Объект.ВидОперации = Перечисления.ВидыОперацийОказанияУслуг.ВозвратКлиенту И не РольДоступна("ПолныеПрава") Тогда
