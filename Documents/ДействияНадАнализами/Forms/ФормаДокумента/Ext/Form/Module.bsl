...
Процедура РаспечататьЭтикетки(Команда)
	...
	Если Команда.Имя = "РаспечататьЭтикеткиВыбранныхКонтейнеров" Тогда
		ВыделенныеСтроки = Элементы.ДеревоРаспределенияБиоматериалов.ВыделенныеСтроки;
		СписокВсехКодов = Новый СписокЗначений;
		КодыПроб = Новый СписокЗначений;
		Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
			СтрокаДерева = Элементы.ДеревоРаспределенияБиоматериалов.ДанныеСтроки(ВыделеннаяСтрока);
			Если ЗначениеЗаполнено(СтрокаДерева.КодПробы) Тогда 
				КодыПроб.Добавить(СтрокаДерева.ПолучитьИдентификатор(), ПолучитьПредставлениеСтрокиДляВыбора(СтрокаДерева));
			КонецЕсли;
		КонецЦикла;          
		Для Каждого СтрокаЛаборатории Из ДеревоРаспределенияБиоматериалов.ПолучитьЭлементы() Цикл
			Для Каждого Строка Из СтрокаЛаборатории.ПолучитьЭлементы() Цикл
				Если ЗначениеЗаполнено(Строка.КодПробы) Тогда
					СписокВсехКодов.Добавить(Строка.ПолучитьИдентификатор(), ПолучитьПредставлениеСтрокиДляВыбора(Строка));
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
		Если СписокВсехКодов.Количество() = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю("Не имеется ни одного заполненного кода пробы");
		Иначе
			ПараметрыФормы = Новый Структура("СписокЗначений, ВыбранныеЭлементыСписка, ВыбиратьВсеЕслиВыбранныхНет, Заголовок",
												СписокВсехКодов, КодыПроб, Ложь, НСтр("ru='Выберите контейнеры'"));
			ОповещениеПродолжения = Новый ОписаниеОповещения("РаспечататьЭтикеткиПродолжить", ЭтотОбъект);
			ОткрытьФорму("ОбщаяФорма.ФормаВыбораЭлементовСпискаЗначений", ПараметрыФормы, ЭтотОбъект,,,,ОповещениеПродолжения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Функция ПолучитьПредставлениеСтрокиДляВыбора(СтрокаДерева)
	
	Возврат СтрШаблон("%1 %2 (%3)",
					СтрокаДерева.КодПробы,
					СтрокаДерева.КонтейнерПредставление,
					СтрокаДерева.БиоматериалПредставление);
	
КонецФункции

&НаКлиенте
Процедура РаспечататьЭтикеткиПродолжить(Результат, Контекст) Экспорт
	
	Если Результат <> Неопределено Тогда
		
		КодыПроб = Новый Массив;
		Для Каждого Эл Из Результат Цикл
			ДанныеСтроки = ДеревоРаспределенияБиоматериалов.НайтиПоИдентификатору(Эл.Значение);
			Если ДанныеСтроки <> Неопределено Тогда
				КодыПроб.Добавить(ДанныеСтроки.КодПробы);
			КонецЕсли;
		КонецЦикла;
		
		СтрокиКонтейнеров = Объект.Контейнеры;
		Если КодыПроб.Количество() > 0 Тогда
			ЛабораторияКлиент.НапечататьЭтикеткиПоДокументу(Объект.Ссылка, СтрокиКонтейнеров, Клиент, КодыПроб);
		КонецЕсли;
