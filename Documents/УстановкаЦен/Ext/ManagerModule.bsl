...
// Добавлено Макеев (
Функция омг_СформироватьЗапросПоНаборам(ДокументОбъект, Страховые = Ложь) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	УстановкаЦенНоменклатура.омг_СоглашенияСтрахования КАК омг_СоглашенияСтрахования
	               |ПОМЕСТИТЬ ВТ_Наборы
	               |ИЗ
	               |	Документ.УстановкаЦен.Номенклатура КАК УстановкаЦенНоменклатура
	               |ГДЕ
	               |	УстановкаЦенНоменклатура.Ссылка = &Ссылка
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	УстановкаЦеномг_НоменклатураНаборов.Набор КАК Набор,
	               |	УстановкаЦеномг_НоменклатураНаборов.Номенклатура КАК Номенклатура,
	               |	УстановкаЦеномг_НоменклатураНаборов.ЦенаВНаборе КАК ЦенаВНаборе,
	               |	ЗНАЧЕНИЕ(Справочник.ХарактеристикиНОменклатуры.ПустаяСсылка) КАК ХарактеристикаНоменклатуры,
	               |	ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмерения.Пустаяссылка) КАК ЕдиницаИзмерения,
	               |	НЕОПРЕДЕЛЕНО КАК Прейскурант,
	               |	УстановкаЦеномг_НоменклатураНаборов.НомерСтроки КАК НомерСтроки
	               |ПОМЕСТИТЬ ВТ_Комплектующие
	               |ИЗ
	               |	Документ.УстановкаЦен.омг_НоменклатураНаборов КАК УстановкаЦеномг_НоменклатураНаборов
	               |ГДЕ
	               |	УстановкаЦеномг_НоменклатураНаборов.Ссылка = &Ссылка
	               |	И УстановкаЦеномг_НоменклатураНаборов.ЦенаВНаборе > 0
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_Наборы.Номенклатура КАК Номенклатура,
	               |	ВТ_Наборы.Цена КАК Цена,
	               |	ВТ_Наборы.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	               |	0 КАК НомерСтроки,
	               |	NULL КАК Прейскурант,
	               |	ВТ_Наборы.ХарактеристикаНоменклатуры КАК ХарактеристикаНоменклатуры,
	               |	ВТ_Филиалы.Филиал КАК омг_Филиал,
	               |	ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка) КАК омг_Набор,
	               |	ВТ_Наборы.омг_СоглашенияСтрахования КАК омг_СоглашенияСтрахования
	               |ИЗ
	               ...
	               |	ВТ_Комплектующие.ЦенаВНаборе,
	               |	ВТ_Комплектующие.ЕдиницаИзмерения,
	               |	0,
	               |	&Прейскурант,
	               |	ВТ_Комплектующие.ХарактеристикаНоменклатуры,
	               |	ВТ_Филиалы.Филиал,
	               |	ВТ_Комплектующие.Набор,
	               |	ВТ_Наборы.омг_СоглашенияСтрахования
	               |ИЗ
	               |	ВТ_Комплектующие КАК ВТ_Комплектующие
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Наборы КАК ВТ_Наборы
	               |		ПО ВТ_Комплектующие.Набор = ВТ_Наборы.Номенклатура,
	               |	ВТ_Филиалы КАК ВТ_Филиалы";
	Запрос.УстановитьПараметр("Ссылка", ДокументОбъект.Ссылка);
	Если Страховые Тогда
		ОсновнойПрейскурант = Справочники.СоглашенияСтрахования.ПустаяСсылка()
	Иначе
