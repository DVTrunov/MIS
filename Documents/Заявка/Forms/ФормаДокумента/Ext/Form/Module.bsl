...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если омг_Предоплата.омг_ИспользуетсяПредоплата(Организация) Тогда
			Объект.Состояние = Справочники.ВидыСостоянийЗаявок.Забронирована;
		Иначе
	...
	омг_ПроверитьЗаполнениеФилиала();
	
	Если не Объект.Ссылка.Пустая() и не РольДоступна("ПолныеПрава") Тогда 
		
		ТолькоПросмотрФормы = омг_ДополнительныеАлгоритмы.ПроверитьДокументыНаОсновании(Объект.Ссылка);
		Элементы.ФормаОтменитьЗаявку.Доступность = не ТолькоПросмотрФормы;   
		Элементы.Состояние.Доступность = не ТолькоПросмотрФормы;   
...
Процедура ПриОткрытии(Отказ)
	...
	//Если ЭтаФорма.омг_СтатусПредоплаты = "Возврат в обработке" Тогда
	//	Элементы.омг_ВзятьПредоплату.Видимость = Ложь;
	//	Элементы.омг_Ссылка.Видимость = Ложь;
	//	Элементы.омг_ОбновитьПредоплату.Видимость = Истина;
	//КонецЕсли;
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если ЗначениеЗаполнено(Организация) Тогда
			...
			Если не ВернутьСтатусОплаты() и Не ЗначениеЗаполнено(Объект.ОМГ_ПричинаОтсутствияПредоплаты) и омг_Предоплата.омг_ИспользуетсяПредоплата(Организация) Тогда
				...
				"Возьмите оплату бронирования или выберите причину отсутствия оплаты бронирования!" // (необ.) заголовок
				);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// ОМГ
	//////////////////////////////////////////////////////////////////////////////
	// Изменения Макеев 18-06-2026 (
	Если ТекущийОбъект.Состояние = Справочники.ВидыСостоянийЗаявок.Забронирована
		И Не ТекущийОбъект.ОМГ_ПричинаОтсутствияПредоплаты.Пустая() Тогда
		ТекущийОбъект.Состояние = Справочники.ВидыСостоянийЗаявок.Запланирована;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ТипЗнч(Источник) = Тип("ФормаКлиентскогоПриложения")
		И СтрНайти(Источник.ИмяФормы, "ПричиныОтменыЗаявок") > 0
		И ИмяСобытия = "ЗаявкаИзменение" Тогда
		Объект.Состояние = ПредопределенноеЗначение("Справочник.ВидыСостоянийЗаявок.Отменена");
	КонецЕсли;
...
Процедура ОтменитьЗаявкуКлиент(Отказ = Ложь, СостояниеОтмены = Неопределено)
	...
	Если мУчетнаяПолитика.ЗапрашиватьПричинуЗакрытияЗаявок Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗаявкаПослеСменыСостоянияНаОтмену",
			омг_ОбработчикиОповещений, Новый Структура("Заявка, ФормаЗаявки", Объект.Ссылка, ЭтаФорма));
			
		КалендарьПланированияКлиент.ОткрытьДиалогПричиныОтменыЗаявки(Объект.Ссылка, СостояниеОтмены, ОписаниеОповещения);
	Иначе
...
Процедура омг_ПослеВыбораФилиала(Результат, ДопПараметры) Экспорт
	...
	Организация  = омг_ДополнительныеАлгоритмы.омг_ВернутьОрганизациюПоФилиалу(Объект.Филиал);		
	Если омг_Предоплата.омг_ИспользуетсяПредоплата(Организация) И Объект.Ссылка.Пустая() Тогда
		Объект.Состояние = ПредопределенноеЗначение("Справочник.ВидыСостоянийЗаявок.Забронирована");
	Иначе
		Объект.Состояние = ПредопределенноеЗначение("Справочник.ВидыСостоянийЗаявок.Запланирована");
	КонецЕсли;
...
Процедура ПослеЗакрытияВопроса(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда   
		...
		МассивПриемов = омг_ПроверитьОткрытыеПриемы();
		ТекстЗаблокированных = ""; 
		
		Для каждого прием из МассивПриемов Цикл
			
			СписокПросматривающих = омг_ДополнительныеАлгоритмы.ПолучитьСписокПросматривающих(прием); 
			
			Если не СписокПросматривающих = "" Тогда
            	ТекстЗаблокированных = ТекстЗаблокированных + прием + " открыт у: "+СписокПросматривающих+Символы.ПС ;
			КонецЕсли;
		
		КонецЦикла;	
		
		Если ТекстЗаблокированных = "" Тогда
			
			омг_ПерезаполнитьУслугиПерсоналПриемыНаСервере(Параметры)
			
		иначе 
			
			Сообщить( ТекстЗаблокированных + "С данными приемами работают пользователи, повторите попытку позже!")	
...
Процедура омг_ВзятьПредоплату(Команда) 
	...
	Если не значениеЗаполнено(Объект.Клиент) или не значениеЗаполнено(Объект.Сотрудник)  Тогда
		Сообщить("Заполните Клиента и Врача!",СтатусСообщения.ОченьВажное);
		Возврат;
	КонецЕсли;	
	...
	Если омг_СтатусПредоплаты = "Оплата принята" Тогда
		...
		ПоказатьВопрос(Оповещение, "Вернуть оплату?",РежимДиалогаВопрос.ДаНет,,КодВозвратаДиалога.Да); 
		
	иначе
		
		Кнопки = Новый СписокЗначений; 
		Кнопки.Добавить("БанковскаяКарта", "Банковская карта");     
		Если не ПроверитьРоль("омг_Оператор") Тогда
			Кнопки.Добавить("Наличные", "Наличные")
		КонецЕсли;
...
Процедура ОбновитьПредоплату()   
	...
	Элементы.омг_Ссылка.Видимость = Ложь;
	Элементы.омг_ВзятьПредоплату.Видимость = Ложь; 
	Элементы.ОМГ_ПричинаОтсутствияПредоплаты.Доступность  = Ложь;
	Организация  = омг_ДополнительныеАлгоритмы.омг_ВернутьОрганизациюПоФилиалу(Объект.Филиал);
	Элементы.омг_ОбновитьПредоплату.Видимость = Ложь;   
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Если омг_Предоплата.омг_ИспользуетсяПредоплата(Организация) Тогда 
			...
			Элементы.ОМГ_ПричинаОтсутствияПредоплаты.Доступность = Истина;
			...
			Если не Объект.Ссылка.Пустая() Тогда
				...
				Если ЗначениеЗаполнено(СтатусОплаты) Тогда
					...
					Если СтатусОплаты.Статус = ПредопределенноеЗначение("Перечисление.омг_СтатусыПредоплаты.ОплатаПринята") Тогда
						Элементы.омг_ВзятьПредоплату.Видимость = Истина;
						Элементы.омг_ВзятьПредоплату.Заголовок = "Вернуть оплату";
						Элементы.ОМГ_ПричинаОтсутствияПредоплаты.Доступность = Ложь;
					иначеЕсли СтатусОплаты.Статус = ПредопределенноеЗначение("Перечисление.омг_СтатусыПредоплаты.ВозвратВОбработке") Тогда
						Элементы.омг_ВзятьПредоплату.Видимость = Ложь;
						Элементы.омг_ВзятьПредоплату.Заголовок = "Вернутьоплату";
						//Элементы.омг_Ссылка.Видимость = Ложь;
						Элементы.омг_ОбновитьПредоплату.Видимость = Истина;
						Элементы.ОМГ_ПричинаОтсутствияПредоплаты.Доступность = Ложь;
					иначе
						Элементы.омг_ВзятьПредоплату.Видимость = Ложь;
						Элементы.омг_ВзятьПредоплату.Заголовок = "Взять оплату";
						Элементы.омг_ОбновитьПредоплату.Видимость = Истина; 
						Элементы.ОМГ_ПричинаОтсутствияПредоплаты.Доступность = Истина;
					КонецЕсли;	
				КонецЕсли;	
				
				Если ЗначениеЗаполнено(омг_Ссылка) Тогда
					Элементы.ОМГ_ПричинаОтсутствияПредоплаты.Доступность = Ложь;
				КонецЕсли;  
...
&НаКлиенте
Процедура омг_ОбновитьПредоплату(Команда)
	омг_Предоплата.омг_ОбновитьСтатусПредоплатПоЗапросу(Объект.Ссылка);
	ОбновитьПредоплату(); 
КонецПроцедуры

&НаСервере
Функция омг_ПроверитьОткрытыеПриемы() 
	
	Табсотрудников = Объект.Работы.Выгрузить(,"Сотрудник"); 
	Табсотрудников.Свернуть("Сотрудник");
	
	ТабВозврата = Новый ТаблицаЗначений; 
	ТабВозврата.Колонки.Добавить("Ссылка",Новый ОписаниеТипов("ДокументСсылка.Прием"));
	...
	Для каждого стрТабсотрудников из Табсотрудников Цикл 
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл    
		ЗаполнитьЗначенияСвойств(ТабВозврата.Добавить(),ВыборкаДетальныеЗаписи)	
		КонецЦикла;
		
	КонецЦикла; 
	
	ТабВозврата.Свернуть("Ссылка",);  
	
	Возврат ТабВозврата.ВыгрузитьКолонку("Ссылка");
	
КонецФункции

&НаСервере
Функция омг_ПроверитьЗаполнениеФилиала() 
