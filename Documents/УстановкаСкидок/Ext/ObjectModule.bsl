...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// Добавлено Макеев (
	Движения.РазмерыСкидок.Записывать = Истина;
	Если ТипЗнч(Получатель) = Тип("СправочникСсылка.ВидыКартСкидок") Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               |	МАКСИМУМ(ВидыСкидокРазмерыСкидки.Размер) КАК Размер,
		               |	ВидыСкидокРазмерыСкидки.Ссылка КАК Ссылка
		               |ИЗ
		               |	Справочник.ВидыСкидок.РазмерыСкидки КАК ВидыСкидокРазмерыСкидки
		               |ГДЕ
		               |	ВидыСкидокРазмерыСкидки.Ссылка = &ВидСкидки
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ВидыСкидокРазмерыСкидки.Ссылка";
		Запрос.УстановитьПараметр("ВидСкидки", ВидСкидки);
		РезультатЗапроса = Запрос.Выполнить();
		Если Не РезультатЗапроса.Пустой() Тогда
			Выборка = РезультатЗапроса.Выбрать();
			Пока Выборка.Следующий() Цикл
				Запись = Движения.РазмерыСкидок.Добавить();
				Запись.Период = Дата;
				Запись.Фильтр = Получатель;
				Запись.ПроцентСкидки = Выборка.Размер;
				Если ЗначениеЗаполнено(ДатаОкончанияСкидки) Тогда
					Запись.КонецИнтервала = ДатаОкончанияСкидки;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
