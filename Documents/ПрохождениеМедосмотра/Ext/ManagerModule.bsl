...
Функция ПолучитьСписокДинамическихПечатныхФорм() Экспорт
	...
	мПечатныхФорм.Добавить(Новый Структура("Имя, Синоним", "Заключение",			 НСтр("ru='Заключение медицинского осмотра (29н)'")));
	...
	мПечатныхФорм.Добавить(Новый Структура("Имя, Синоним", "Заключение342н",		 НСтр("ru='Заключение психиатрического освидетельствования работника (342н)'")));
...
//  ТабличныйДокумент.
//
Функция Печать(СсылкаНаОбъект, ИмяМакета, СохранениеЭМД = Ложь, ПриказВредныйФактор = Неопределено) Экспорт
	...
	ИначеЕсли ИмяМакета = "Заключение" Тогда
		ТабДокумент = ПечатьЗаключение(СсылкаНаОбъект, СохранениеЭМД, ПриказВредныйФактор);
	ИначеЕсли ИмяМакета = "Заключение342н" Тогда
...
Функция ПечатьЗаключение(СсылкаНаОбъект, СохранениеЭМД, ПриказВредныйФактор = Неопределено)
	...
	Если ПриказВредныйФактор <> Неопределено Тогда
		...
		Если ДопСерверныеФункции.ИмяСсылочногоТипа(ПриказВредныйФактор) = "Справочник.ПереченьВредныхФакторовИРабот" Тогда
			ВредныеФакторы.Добавить(ПриказВредныйФактор);
		ИначеЕсли ДопСерверныеФункции.ИмяСсылочногоТипа(ПриказВредныйФактор) = "Справочник.ПриказыМедосмотров" Тогда
			Для Каждого СтрокаФактора Из СсылкаНаОбъект.ФакторыИУсловияРаботы Цикл
				Если СтрокаФактора.Фактор.Приказ = ПриказВредныйФактор Тогда
					ВредныеФакторы.Добавить(СтрокаФактора.Фактор);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	
	ОбщаяОбласть.Параметры.ВредныеПроизводственныеФакторы = СформироватьТекстВредныеФакторыМедосмотра(СсылкаНаОбъект, Справочники.ПриказыМедосмотров.Приказ29н, ВредныеФакторы);
	
	ОбщаяОбласть.Параметры.РезультатОбследования = СформироватьТекстРезультатаОбследования(СсылкаНаОбъект, ВредныеФакторы);
...
Функция СформироватьТекстВредныеФакторыМедосмотра(ПрохождениеМедосмотра, Приказ = Неопределено, ВредныеФакторы = Неопределено)
	...
	Запрос.Текст = 
		...
		|	И &УсловиВредныхФакторов";
	
	Запрос.УстановитьПараметр("Приказ", Приказ);
	Запрос.УстановитьПараметр("ПрохождениеМедосмотра", ПрохождениеМедосмотра);
	Если ВредныеФакторы = Неопределено Тогда
		Запрос.УстановитьПараметр("УсловиВредныхФакторов", Истина);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиВредныхФакторов", "ПрохождениеМедосмотраФакторыИУсловияРаботы.Фактор В (&ВредныеФакторы)");
	...
	ОписанияВредныхФакторов = Новый Массив;
	Пока Выборка.Следующий() Цикл
		ОписанияВредныхФакторов.Добавить(?(ПустаяСтрока(Выборка.НомерПП), "", Выборка.НомерПП + " ") + Выборка.НаименованиеПолное);
	КонецЦикла;
	
	Возврат СтрСоединить(ОписанияВредныхФакторов, ";" + Символы.ПС);
	
КонецФункции

Функция СформироватьТекстЗаключенияПоИмеющимсяПротивопоказаниям(СсылкаНаОбъект, КритичныеПротивопоказания, ВредныеФакторыПротивопоказаний, ВредныеФакторы = Неопределено)
	
	ТекстИмеетПротивопоказания = "";
	ТекстНеИмеетПротивопоказания = "";
	
	Отбор = Новый Структура("Фактор");
	Для Каждого СтрокаВредныйФактор Из СсылкаНаОбъект.ФакторыИУсловияРаботы Цикл
		
		Если ВредныеФакторы <> Неопределено
			И ВредныеФакторы.Найти(СтрокаВредныйФактор.Фактор) = Неопределено
		Тогда
			Продолжить;
		КонецЕсли;
		
		Отбор.Фактор = СтрокаВредныйФактор.Фактор;
		мсСтр = СсылкаНаОбъект.ЗаключенияМедосмотровПоФакторам.НайтиСтроки(Отбор);
		
		// Удаление строк с не заполненным заключением
		мОбрИнд = мсСтр.Количество()-1;
		Для Сч = 0 По мОбрИнд Цикл
			СтрокаТЗ = мсСтр[мОбрИнд-Сч];
			Если Не ЗначениеЗаполнено(СтрокаТЗ.Заключение) Тогда // Заключение по классификатору ФР НСИ не указано и не учитывается.
				мсСтр.Удалить(мОбрИнд-Сч);
			КонецЕсли;
		КонецЦикла;
		
		// Определение наличия противопоказаний.
		Если мсСтр.Количество() = 0 Тогда
			// Если таблица пустая, значит РЭМД не используется, проверяем по наличию критичных противопоказаний по фактору
			ОтборПоВФПротивопоказаний = Новый Структура("ВредныйФактор");
			ОтборПоВФПротивопоказаний.ВредныйФактор = СтрокаВредныйФактор.Фактор;
			СтрокиКритичныхПротивопоказаний = СсылкаНаОбъект.ВредныеФакторыПротивопоказаний.НайтиСтроки(ОтборПоВФПротивопоказаний);
			Если СтрокиКритичныхПротивопоказаний.Количество() = 0 Тогда
			...
			КонецЕсли;
		Иначе
			ЗаключениеФактора = мсСтр[0];
			Если ЗаключениеФактора.Заключение.УИДЕГИСЗ = 71 Тогда
				ТекстИмеетПротивопоказания	= ТекстИмеетПротивопоказания + ?(ПустаяСтрока(ТекстИмеетПротивопоказания), "", Символы.ПС)
										+ Символы.Таб + СтрокаВредныйФактор.Фактор["НомерПП"] + " " + СтрокаВредныйФактор.Фактор.НаименованиеПолное;
			Иначе
				ТекстНеИмеетПротивопоказания = ТекстНеИмеетПротивопоказания + ?(ПустаяСтрока(ТекстНеИмеетПротивопоказания), "", Символы.ПС)
...
Функция СформироватьТекстРезультатаОбследования(СсылкаНаОбъект, ВредныеФакторы = Неопределено)
