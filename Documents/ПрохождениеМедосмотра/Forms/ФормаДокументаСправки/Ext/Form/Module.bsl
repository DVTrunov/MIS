...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Заключение
	Элементы.ГруппаЗаключение.Видимость = Объект.ПодписанЭП Или ЗаключениеСделано(Объект.ДатаЗавершения, Объект.ПредседательМедКомиссии);
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ПроверитьЗаполнениеОбязательныхПараметров(ТекущийОбъект.ПараметрыПриема);
...
Процедура ДатаЗавершенияПриИзменении(Элемент)
	...
	УстановитьВидимостьГруппыЭП(Элементы.ГруппаЭП, ИспользуютсяЭП, Объект.ДатаЗавершения, Объект.ПредседательМедКомиссии, ИспользуетсяРЭМД, ШаблонРЭМД, Объект.Номенклатура);
	
КонецПроцедуры

&НаКлиенте
Процедура ПредседательМедКомиссииПриИзменении(Элемент)
...
&НаСервереБезКонтекста
Процедура ПроверитьЗаполнениеОбязательныхПараметров(ПараметрыПриема)
	
	Для Каждого ПараметрПМО Из ПараметрыПриема Цикл
		Если ПараметрПМО.Параметр.ОбязательноДляЗаполнения
			И ПараметрПМО.Параметр.ТипЗначения <> Тип("Булево")
			И Не ЗначениеЗаполнено(ПараметрПМО.Значение)
		Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru='Параметр %1 не заполнен. Данный параметр обязателен к заполнению'"), РаботаСШаблонамиHTML.ПредставлениеПараметраШаблона(ПараметрПМО.Параметр)));
		КонецЕсли;
	КонецЦикла;
...
Функция ЗаключениеСделано(ДатаЗавершения, ПредседательМедКомиссии)
	
	Возврат ЗначениеЗаполнено(ДатаЗавершения) И ЗначениеЗаполнено(ПредседательМедКомиссии);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьГруппыЭП(ГруппаЭП, ИспользуютсяЭП, ДатаЗавершения, ПредседательМедКомиссии, ИспользуетсяРЭМД, ШаблонРЭМД, Номенклатура)
	
	ГруппаЭП.Видимость = ИспользуютсяЭП И ЗаключениеСделано(ДатаЗавершения, ПредседательМедКомиссии) И (ИспользуетсяРЭМД Или Не ПустойТабДок(ШаблонРЭМД, Номенклатура));
