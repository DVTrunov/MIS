...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаключениеПоФакторам = ЗаключениеПоФакторам();
	
	ОбновитьТаблицуЗаключенийПоФакторам();
	ОбновитьСвязанныеДокументы();
	
	Если ТипЗнч(Объект.ВидПолиса) = Неопределено Тогда
		Объект.ВидПолиса = Справочники.ВидыПолисов.ПустаяСсылка();
	ИначеЕсли ТипЗнч(Объект.ВидПолиса) = Тип("ДокументСсылка.ДоговорСКлиентом") Тогда
		ИсточникОплаты = 1;
	КонецЕсли;
	
	мПараметрыВыбора = Новый Массив;
	мПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.ПометкаУдаления", Ложь));
	Элементы.ВидПолиса.ПараметрыВыбора = Новый ФиксированныйМассив(мПараметрыВыбора);
	
	ОпределитьВидимостьПолей230();
	
	УстановитьВидимостьПроверкиКорректности230();
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьВидимостьЭП();
	УстановитьВидимостьПолейСвязанныхСПМО();
	ОбновитьВидПолисаПоИсточникуОплат();
	УстановитьПредставлениеИнфекционныйАнамнезИСведенияОПрививках();
	Элементы.ФормаОбщаяКомандаПечать.Видимость = Ложь;
...
Асинх Процедура ФакторыИУсловияРаботыФакторПриИзменении(Элемент)
	...
	ПроверяемыйНаУникальностьФактор = Элементы.ФакторыИУсловияРаботы.ТекущиеДанные.Фактор;
	Если ЗначениеЗаполнено(ПроверяемыйНаУникальностьФактор) Тогда
		ФакторУникален = ДобавляемыйФакторУникален(ПроверяемыйНаУникальностьФактор, Элементы.ФакторыИУсловияРаботы.ТекущиеДанные.НомерСтроки);
		Если Не ФакторУникален Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='В таблице уже присутствует выбранный фактор.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	Если Объект.ДействияМедосмотра.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru='Список вредных факторов был изменен, перезаполнить действия медосмотра?'");
...
&НаКлиенте
Процедура ЗаключенияМедосмотровПоФакторамПриАктивизацииСтроки(Элемент)
	
	Если Элементы.ЗаключенияМедосмотровПоФакторам.ТекущиеДанные = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	Фактор = Элементы.ЗаключенияМедосмотровПоФакторам.ТекущиеДанные.Фактор;
	
	Если ЕстьКритичныеПротивопоказанияПоФактору(Фактор) Тогда
		Элементы.ЗаключенияМедосмотровПоФакторамЗаключение.ТолькоПросмотр = Истина;
	Иначе
		Элементы.ЗаключенияМедосмотровПоФакторамЗаключение.ТолькоПросмотр = Ложь;
	КонецЕсли;
...
Процедура Заключения(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПерейтиКФормеЗаключенийМедосмотра", ЭтотОбъект);
...
&НаКлиенте
Процедура ПроверкаКорректностиЗаполнения(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПерейтиКФормеПроверкиКорректности", ЭтотОбъект);
	РаботаСДиалогамиКлиент.ПроверитьМодифицированностьВФорме(ЭтаФорма, Истина, ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура Противопоказания(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПерейтиКФормеПротивопоказаний", ЭтотОбъект);
	РаботаСДиалогамиКлиент.ПроверитьМодифицированностьВФорме(ЭтаФорма, Истина, ОписаниеОповещения);
...
Процедура ПоляЗаключенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли Элемент.Имя = "ИнфекционныйАнамнез" Тогда
		Если Элементы.ПрофильРаботы.Видимость = Истина 
			И Не ЗначениеЗаполнено(Объект.ПрофильРаботы) 
		Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Заполнение инфекционного анамнеза возможно только после выбора профиля работы.'"));
			Возврат;
		КонецЕсли;
	...
	Если Элемент.Имя = "ИнфекционныйАнамнез" Или Элемент.Имя = "СведенияОПрививках" Тогда
		...
		Если МассивФакторов.Количество() = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Заполнение инфекционного анамнеза возможно только после выбора факторов.'"));
			Возврат;
		КонецЕсли;
		ПараметрыФормы = Новый Структура();
		ПараметрыФормы.Вставить("ВредныеФакторы", МассивФакторов);
		ПараметрыФормы.Вставить("ПрофильРаботы", Объект.ПрофильРаботы);
		
		Если Элемент.Имя = "ИнфекционныйАнамнез" Тогда
			ДанныеИнфекционныхЗаболеваний = Новый Массив;
			Для Каждого Эл Из Объект.ИнфекционныеЗаболевания Цикл
				СтруктураДанных = Новый Структура("Диагноз, Дата", Эл.Диагноз, Эл.Дата);
				ДанныеИнфекционныхЗаболеваний.Добавить(СтруктураДанных);
			КонецЦикла;
			ПараметрыФормы.Вставить("ИнфекционныеЗаболевания", ДанныеИнфекционныхЗаболеваний);
			
			ОткрытьФорму("Обработка.ИнтерфейсВводаПолейЭМД.Форма.ИнфекционныйАнамнез",ПараметрыФормы,ЭтотОбъект,,,,ОповещениеОВыборе,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
		Иначе
			ДанныеПрофилактическихПрививок = Новый Массив;
			Для Каждого Эл Из Объект.ПрофилактическиеПрививки Цикл
				СтруктураДанных = Новый Структура();
				СтруктураДанных.Вставить("ГруппаЗаболеваний", Эл.ГруппаЗаболеваний);
				СтруктураДанных.Вставить("ДатаВакцинации", Эл.ДатаВакцинации);
				СтруктураДанных.Вставить("НаименованиеПрививки", Эл.НаименованиеПрививки);
				СтруктураДанных.Вставить("Повторная", Эл.Повторная);
				СтруктураДанных.Вставить("ПорядковыйНомер", Эл.ПорядковыйНомер);
				ДанныеПрофилактическихПрививок.Добавить(СтруктураДанных);
			КонецЦикла;
			ПараметрыФормы.Вставить("ПрофилактическиеПрививки", ДанныеПрофилактическихПрививок);
			
			ОткрытьФорму("Обработка.ИнтерфейсВводаПолейЭМД.Форма.СведенияОПрививках",ПараметрыФормы,ЭтотОбъект,,,,ОповещениеОВыборе,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	Иначе
...
&НаКлиенте
Процедура ВидПолисаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Если ТипЗнч(Объект.ВидПолиса) = Тип("ДокументСсылка.ДоговорСКлиентом") Тогда
		СтандартнаяОбработка = Ложь;
		ПараметрыВыбораДоговора = Новый Структура("ДополнительныеПараметры", Новый Структура("КонтекстВыбораСоздания", ЭтотОбъект.ИмяФормы));
		ПараметрыВыбораДоговора.Вставить("Отбор", Новый Структура("Клиент, ПометкаУдаления", Объект.Клиент, Ложь));
		ОткрытьФорму("Документ.ДоговорСКлиентом.ФормаВыбора", ПараметрыВыбораДоговора, Элемент, ЭтотОбъект);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВидПолисаОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	
	Если ВыбранноеЗначение <> Неопределено
		И ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.ВидыПолисов")
	Тогда
		ДанныеРаботодателя = ПолучитьДанныеРаботодателяПоВидуПолиса(ВыбранноеЗначение);
...
Асинх Процедура СписокНаПрохождениеМедосмотраПриИзменении(Элемент)
	...
	// Перезаполнение при изменении списка (возможно, другой вид медосмотра - набор приказов медосмотров)
	Если ЗначениеЗаполнено(Объект.СписокНаПрохождениеМедосмотра) Тогда
	...
	// Изменение видимости элементов формы от заполненности списка на прохождение
	РаботаСФормамиКлиентСервер.УстановитьВидимостьЭлементаФормы(Элементы, "ГруппаСохраненныеДанные", Не ЗначениеЗаполнено(Объект.СписокНаПрохождениеМедосмотра));
	УстановитьВидимостьПолейСвязанныхСПМО();
...
&НаКлиенте
Процедура ФакторыИУсловияРаботыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле = Элементы.ФакторыИУсловияРаботыФакторПриказ Тогда
		ТекущийЭлемент = Элементы.ФакторыИУсловияРаботыФактор;
		Элементы.ФакторыИУсловияРаботы.ИзменитьСтроку();
	КонецЕсли;
...
&НаКлиенте
Процедура УстановитьПредставлениеИнфекционныйАнамнезИСведенияОПрививках()
	
	Если ЗначениеЗаполнено(Объект.ИнфекционныйАнамнез) Тогда
		ИнфекционныйАнамнез = Объект.ИнфекционныйАнамнез;
	Иначе
		ИнфекционныйАнамнез = МедосмотрыКлиент.ПолучитьПредставлениеИнфекционныеЗаболевания(Объект.ИнфекционныеЗаболевания);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.СведенияОПрививках) Тогда
		СведенияОПрививках = Объект.СведенияОПрививках;
	Иначе
		СведенияОПрививках = МедосмотрыКлиент.ПолучитьПредставлениеСведенияОПривиках(Объект.ПрофилактическиеПрививки);
	КонецЕсли;
...
Функция ПолучитьСписокУникальныхПриказовИСправок(ВидМО)
	...
	УникальныеПриказы		 = ОбщегоНазначенияБИТКлиентСервер.СортироватьМассивПоЗначению(УникальныеПриказы);
	УникальныеВредныеФакторы = ОбщегоНазначенияБИТКлиентСервер.СортироватьМассивПоЗначению(УникальныеВредныеФакторы);
	
	// Соединяем
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(УникальныеПриказы, УникальныеВредныеФакторы, Истина);
...
Процедура ДобавитьРеквизитыИПоляДляСправкиЗаключение(ДопРеквизит, Сч, ЭлементПоле, ЭлементГруппы, ПриСозданииФормы) 
	...
	Если ГруппаСправок = Неопределено Тогда
		...
		ЭлементПоле.Вид = ВидПоляФормы.ПолеВвода;
		ЭлементПоле.КнопкаВыбора = Истина;
		ЭлементПоле.КнопкаОчистки = Истина;
		ЭлементПоле.АвтоОтметкаНезаполненного = Истина;
		...
		ГруппаСправокСтраницы.Ширина = 80;
	...
	ДобавитьКомандуНаФорму("ОбновитьПредпросмотр" + Сч, КоманднаяПанельСправки, "Обновить предпросмотр", "ОбновитьПредпросмотр", БиблиотекаКартинок.Обновить);
	ДобавитьКомандуНаФорму("ПечатьСправки" + Сч, КоманднаяПанельСправки, "Печать", "ПечатьСправки", БиблиотекаКартинок.Печать);
...
Процедура ДобавитьКомандуНаФорму(ИмяКоманды, Родитель, Заголовок, Действие, Картинка)
...
Функция ВычислитьЗначениеПредзаполнения(СтрокаАлгоритма, Знач ОбъектСсылка)
	...
	Если ЗначениеЗаполнено(СтрокаАлгоритма) Тогда
		...
		НачатьТранзакцию();
		Попытка
			Выполнить(Справочники.ВычисляемыеЗначенияПараметров.ПолучитьАлгоритмВычисленияЗначения(СтрокаАлгоритма));
			Возврат Результат;
...
Процедура ВыбораЗначенияПараметраЗавершение(Значение, ИмяРеквизитаОбъекта) Экспорт 
	...
	Если ИмяРеквизитаОбъекта = "ИнфекционныйАнамнез" Тогда
		Объект.ИнфекционныеЗаболевания.Очистить();
		Для Каждого Эл Из Значение Цикл
			НоваяСтрока = Объект.ИнфекционныеЗаболевания.Добавить();
			НоваяСтрока.Диагноз = Эл.Диагноз;
			НоваяСтрока.Дата = Эл.Дата;
		КонецЦикла;
		Объект.ИнфекционныйАнамнез = "";
		УстановитьПредставлениеИнфекционныйАнамнезИСведенияОПрививках();
	ИначеЕсли ИмяРеквизитаОбъекта = "СведенияОПрививках" Тогда
		Объект.ПрофилактическиеПрививки.Очистить();
		Для Каждого Эл Из Значение Цикл
			НоваяСтрока = Объект.ПрофилактическиеПрививки.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Эл);
		КонецЦикла;
		Объект.СведенияОПрививках = "";
		УстановитьПредставлениеИнфекционныйАнамнезИСведенияОПрививках();
	Иначе
...
&НаКлиенте
Процедура УстановитьВидимостьЭП()
	
	ВыводитьГруппуЭП = Ложь;
	Если ИспользуютсяЭП
		И ЗначениеЗаполнено(Объект.ДатаЗавершения)
		И (ИспользуетсяРЭМД
			И ВидМедосмотраПоПриказу29н(Объект.ВидМО)
			Или МедосмотрыСервер.ЕстьЗаполненныеМакетыПриказов(Объект.ФакторыИУсловияРаботы))
	Тогда
		ВыводитьГруппуЭП = Истина;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ВидМедосмотраПоПриказу29н(ВидМедосмотра)
	
	Возврат ВидМедосмотра.Приказы.НайтиСтроки(Новый Структура("Приказ", Справочники.ПриказыМедосмотров.Приказ29н)).Количество() > 0;
...
Процедура ОбработкаОповещнияВыбораМестаРаботы(ВыбранныйКонтрагент, ИсточникВыбора) Экспорт
	...
	Если ВыбранныйКонтрагент <> Неопределено Тогда
		...
		// Автоподстановка программы обслуживания по контрагенту
		Если ИсточникОплаты = 0 Тогда
			ПрограммаОбслуживанияПоКонтрагенту = ВидПолисаПоКонтрагентуДляАвтоподстановки(ВыбранныйКонтрагент, Объект.ВидПолиса, Объект.Филиал);
			Если ПрограммаОбслуживанияПоКонтрагенту <> Неопределено Тогда
				Объект.ВидПолиса = ПрограммаОбслуживанияПоКонтрагенту;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ВидПолисаПоКонтрагентуДляАвтоподстановки(Контрагент, ТекущийВидПолиса, ФилиалДокумента)
	
	ТекущийВидПолиса = Справочники.ВидыПолисов.ПустаяСсылка();
	Если ТекущийВидПолиса.Контрагент = Контрагент Тогда
		ВидПолиса = Неопределено; // Не изменять текущий в документе.
	Иначе
		ВидПолиса = Справочники.ВидыПолисов.ПустаяСсылка(); // По умолчанию очищаем.
		
		// Поиск с отбором по организации филиала документа
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Контрагент", Контрагент);
		Запрос.УстановитьПараметр("Организация", ФилиалДокумента.Организация);
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВидыПолисовДоговоры.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.ВидыПолисов.Договоры КАК ВидыПолисовДоговоры
		|ГДЕ
		|	ВидыПолисовДоговоры.Ссылка.Контрагент = &Контрагент
		|	И ВидыПолисовДоговоры.Договор.Организация = &Организация
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ВидыПолисов.Контрагент = &Контрагент"
		;
		...
		Для Каждого РезультатЗапроса Из Результаты Цикл
			...
			Если Выб.Количество() = 1 Тогда
				Выб.Следующий();
				ВидПолиса = Выб.Ссылка;
			ИначеЕсли Выб.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
			Прервать; // Если найден или если их более 1.
		КонецЦикла;
	КонецЕсли;
...
Функция ПерезаполнитьПараметрыПриема(ВредныйФакторСправки, ТребуетсяАвтозаполнение)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ЗначенияВрачей = МедосмотрыСервер.ЗначенияПараметровИзДокументовПоМедосмотру(Объект.Ссылка, Объект.Клиент);
	Иначе
...
&НаСервере
Процедура ОбновитьТаблицуЗаключенийПоФакторам()
	
	// Только для медосмотров по 29н
	Если Не (ИспользуютсяЭП
			И ИспользуетсяРЭМД
			И ЗначениеЗаполнено(Объект.Ссылка)
			И ВидМедосмотраПоПриказу29н(Объект.ВидМО))
		Или Не ЗаключениеПоФакторам
	Тогда
		Элементы.ГруппаЗаключенияПоФакторам.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	Элементы.ГруппаЗаключенияПоФакторам.Видимость = Истина;
	
	ЗаключенияМедосмотровПоФакторамКопия = Объект.ЗаключенияМедосмотровПоФакторам.Выгрузить();
	Объект.ЗаключенияМедосмотровПоФакторам.Очистить();
	
	Для Каждого ФакторДокумента Из Объект.ФакторыИУсловияРаботы Цикл
		
		СтрокаЗаключения = Объект.ЗаключенияМедосмотровПоФакторам.Добавить();
		СтрокаЗаключения.Фактор = ФакторДокумента.Фактор;
		
		ЕстьКритичныеПротивопоказания = ЕстьКритичныеПротивопоказанияПоФактору(ФакторДокумента.Фактор);
		
		// Если есть противопоказания, то ставим выявленность принудительно
		Если ЕстьКритичныеПротивопоказания Тогда
			СтрокаЗаключения.Заключение = ИнтеграцияЕГИСЗ_СЭМД.ЗаключениеМедосмотра(71);
			Продолжить;
		КонецЕсли;
		
		// Заполнение ранее заполненными значениями
		СтрокаКопииПоФактору = ЗаключенияМедосмотровПоФакторамКопия.Найти(ФакторДокумента.Фактор, "Фактор");
		Если СтрокаКопииПоФактору <> Неопределено 
			И ЗначениеЗаполнено(СтрокаКопииПоФактору.Заключение)
		Тогда
			СтрокаЗаключения.Заключение = СтрокаКопииПоФактору.Заключение;
		КонецЕсли;
...
&НаСервере
Функция ЕстьКритичныеПротивопоказанияПоФактору(ВредныйФактор)
	
	Для Каждого КритичноеПротивопоказание Из Объект.КритичныеПротивопоказания Цикл
		Если СтрНайти(КритичноеПротивопоказание.ВредныеФакторы, ВредныйФактор.НомерПП) <> 0 Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ОбновитьВидПолисаПоИсточникуОплат()
	
	Если ИсточникОплаты = 1 Тогда
		ВидПолисаПоУмолчанию = ПредопределенноеЗначение("Документ.ДоговорСКлиентом.ПустаяСсылка");
		Элементы.ВидПолиса.Заголовок = НСтр("ru='Договор с клиентом'");
	Иначе
		ВидПолисаПоУмолчанию = ПредопределенноеЗначение("Справочник.ВидыПолисов.ПустаяСсылка");
		Элементы.ВидПолиса.Заголовок = НСтр("ru='Программа обслуживания'");
	КонецЕсли;
	
	Если ТипЗнч(Объект.ВидПолиса) <> ТипЗнч(ВидПолисаПоУмолчанию)
		Или НевернаяПринадлежностьВидаПолиса(Объект.ВидПолиса, Объект.Клиент)
	Тогда
		Объект.ВидПолиса = ВидПолисаПоУмолчанию;
		Модифицированность = Истина;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция НевернаяПринадлежностьВидаПолиса(ВидПолиса, Клиент)
	
	Возврат ТипЗнч(ВидПолиса) = Тип("ДокументСсылка.ДоговорСКлиентом") И ВидПолиса.Клиент <> Клиент;
	
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьДанныеРаботодателяПоВидуПолиса(ВидПолиса)
	
	ДанныеРаботодателя = Новый Структура("НаименованиеПолное, Представление, ИНН");
	ЗаполнитьЗначенияСвойств(ДанныеРаботодателя, ВидПолиса.Контрагент);
	
	Возврат ДанныеРаботодателя;
	
КонецФункции

&НаСервере
Процедура ОпределитьВидимостьПолей230()
	
	Если Не ВидМедосмотраПоПриказу29н(Объект.ВидМО) Тогда
		Возврат;
	КонецЕсли;
	
	ВредныеФакторыДляМедкнижки = МедосмотрыСервер.ПунктыВредныхФакторовДляМедкнижки();
	НайденФактор23По29Н = Ложь;
	Для Каждого ФакторПМО Из Объект.ФакторыИУсловияРаботы Цикл
		Если ВредныеФакторыДляМедкнижки.Найти(ФакторПМО.Фактор.НомерПП) <> Неопределено Тогда
			ПрохождениеМедосмотраДляМедКнижки = Истина;
			Если ФакторПМО.Фактор.НомерПП = "23" Тогда
				НайденФактор23По29Н = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Элементы.НомерЛичнойМедКнижки.Видимость = Ложь;
	Элементы.ПрофильРаботы.Видимость = Ложь;
	Если ПрохождениеМедосмотраДляМедКнижки Тогда
		Элементы.НомерЛичнойМедКнижки.Видимость = Истина;
		Если НайденФактор23По29Н Тогда
			Элементы.ПрофильРаботы.Видимость = Истина;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УбратьЛишниеВредности()
...
&НаСервере
Процедура УстановитьВидимостьПроверкиКорректности230()
	
	Если Не ВидМедосмотраПоПриказу29н(Объект.ВидМО) Тогда
		ВидимостьПроверкиКорректности230 = Ложь;
	Иначе
		ВидимостьПроверкиКорректности230 = МедосмотрыСервер.МедосмотрДляМедицинскихКнижек(Объект.Ссылка);
	КонецЕсли;
	
	РаботаСФормамиКлиентСервер.УстановитьВидимостьЭлементаФормы(Элементы, "ПроверкаКорректностиЗаполнения", ВидимостьПроверкиКорректности230);
	
КонецПроцедуры

&НаСервере
Функция ДобавляемыйФакторУникален(ДобавляемыйФактор, НомерСтроки)
	
	ФакторыДубли = Объект.ФакторыИУсловияРаботы.НайтиСтроки(Новый Структура("Фактор", ДобавляемыйФактор));
	Для Каждого ФакторДубль Из ФакторыДубли Цикл
		Если ФакторДубль.НомерСтроки <> НомерСтроки Тогда
			
			// Очищаем введенный фактор
			ВведеннаяСтрокаМассив = Объект.ФакторыИУсловияРаботы.НайтиСтроки(Новый Структура("Фактор, НомерСтроки", ДобавляемыйФактор, НомерСтроки));
			Если ВведеннаяСтрокаМассив.Количество() > 0 Тогда
				ВведеннаяСтрока = ВведеннаяСтрокаМассив[0];
				ВведеннаяСтрока.Фактор = Справочники.ПереченьВредныхФакторовИРабот.ПустаяСсылка();
			КонецЕсли;
...
&НаКлиенте
Процедура ПроверитьКорректностьЗаполненияТаблицыИдентификаторов()
	
	Если Объект.ПодписанЭП
		Или Объект.ВидПрохожденияМО <> ПредопределенноеЗначение("Перечисление.ВидыМедосмотров.ПериодическийПредварительный")
	Тогда
		Возврат;
	КонецЕсли;
	
	ПриказыВредныеФакторы = МедосмотрыСервер.ПолучитьПриказыВредныеФакторыДляТаблицыИдентификаторовПоСсылке(Объект.Ссылка);
	
	ТаблицаИдентификаторовЗаполненаКорректно = Истина;
	
	Если ПриказыВредныеФакторы.Количество() <> Объект.ИдентификаторыПриказов.Количество() Тогда
		ТаблицаИдентификаторовЗаполненаКорректно = Ложь;
	Иначе
		Для Каждого ПриказВредныйФактор Из ПриказыВредныеФакторы Цикл
			Если Объект.ИдентификаторыПриказов.НайтиСтроки(Новый Структура("ПриказВредныйФактор", ПриказВредныйФактор)).Количество() = 0 Тогда
				ТаблицаИдентификаторовЗаполненаКорректно = Ложь;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	Если Не ТаблицаИдентификаторовЗаполненаКорректно Тогда
		МедосмотрыКлиентСервер.ОбновитьИдентификаторыПриказов(Объект, Модифицированность);
	КонецЕсли;
...
&НаКлиенте
Процедура ПерейтиКФормеПроверкиКорректности(Результат, ДопПараметры) Экспорт
	
	Если Не Результат Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура("ПрохождениеМедосмотра", Объект.Ссылка);
	ОткрытьФорму("Документ.ПрохождениеМедосмотра.Форма.ФормаПроверкиКорректностиЗаполненияСЭМД230", ПараметрыФормы, ЭтаФорма);
	
КонецПроцедуры

&НаКлиенте
Процедура ПерейтиКФормеЗаключенийМедосмотра(Результат, ДопПараметры) Экспорт
...
&НаСервере
Функция ЗаключениеПоФакторам()
	
	Возврат Объект.ЭМД.НайтиСтроки(Новый Структура("ВидДокументаЭМД", ИнтеграцияЕГИСЗОбщегоНазначенияПовтИсп.РегистрируемыеЭлектронныеМедДокументы(194))).Количество() = 0;
...
Процедура ПодписатьДокумент(Команда)
	...
	Если МожноПерейтиКПодписанию() Тогда
		ПроверитьКорректностьЗаполненияТаблицыИдентификаторов();
...
Процедура ОбновитьСводкуПодписей(Ссылка, СводкаHTML, Знач ЭлектронныеПодписи, Знач ФакторыИУсловияРаботы)
	...
	доп_СертификатыЭЦП.ЗаполнитьСводкуПодписей(Ссылка, СводкаHTML, ЭлектронныеПодписи,, Новый Структура("ТаблицаФакторовИУсловийРабот", ФакторыИУсловияРаботы));
...
Процедура УстановитьДоступностьФакторовИУсловийРабот()
	...
	Элементы.ФакторыИУсловияРаботы.ТолькоПросмотр = ЗначениеЗаполнено(Объект.ДатаЗавершения);
	Элементы.ВидМедосмотра.ТолькоПросмотр = ЗначениеЗаполнено(Объект.ДатаЗавершения);
	
КонецПроцедуры

&НаКлиенте
Процедура ПерейтиКФормеПротивопоказаний(Результат, ДопПараметры) Экспорт
...
&НаКлиенте
Процедура ВыявленныеПротивопоказанияВредныеФакторыПриИзменении(Элемент)
	ОбновитьТаблицуЗаключенийПоФакторам();
КонецПроцедуры
