...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	///////////////////////////////////////////////////
	// ОМГ    
	Если  ЭтотОбъект.омг_ВидОплаты = ПредопределенноеЗначение("Перечисление.омг_ВидыОплаты.Предоплата") и не ЭтотОбъект.НомерЧекаККМ = 0 Тогда
	Менеджер = РегистрыСведений.омг_ВозвратПредоплаты.СоздатьМенеджерЗаписи();	
	Менеджер.Заявка = ЭтотОбъект.ДокументОснование;		
	Менеджер.Сумма = ЭтотОбъект.Сумма;  
	Менеджер.Статус = Перечисления.омг_СтатусыПредоплаты.ПроизведенВозврат;
	Менеджер.ПлатежныйДокумент = ЭтотОбъект.ДокументОснование.Ссылка;
	Менеджер.ПлатежныйДокументВозврата = ЭтотОбъект.Ссылка;
	ГУИД = ЭтотОбъект.Ссылка.УникальныйИдентификатор();
	ГУИДСтрокой = СокрЛП(ГУИД);
	Менеджер.InvoiceId = ГУИДСтрокой;
	Менеджер.Ссылка = "";
	Менеджер.Записать(); 
	КонецЕсли;
	// ОМГ
