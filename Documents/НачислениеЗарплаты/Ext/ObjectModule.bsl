...
//
Процедура ЗаполнитьБазуРасчета(ПостроительОтчета = Неопределено,ТекСотрудник = Неопределено) Экспорт
	...
	Иначе 
		СтрПоиска = Новый Структура("Сотрудник",ТекСотрудник);
		СтрокиПоОтборуТекСотрудник = БазаРасчета.НайтиСтроки(СтрПоиска);
		Для каждого ТекСТрока из СтрокиПоОтборуТекСотрудник Цикл
			БазаРасчета.Удалить(ТекСТрока);
		КонецЦикла;	
		СтрокиПоОтборуТекСотрудник = Начисления.НайтиСтроки(СтрПоиска);
		Для каждого ТекСТрока из СтрокиПоОтборуТекСотрудник Цикл
			Начисления.Удалить(ТекСТрока);
		КонецЦикла;	
		СписокСотрудников = Новый Массив; 
		СписокСотрудников.Добавить(ТекСотрудник);
	КонецЕсли;
...
Процедура ЗаполнитьНачисления(ТекСотрудник = Неопределено) Экспорт
	
	Если ТекСотрудник = Неопределено Тогда	
		Начисления.Очистить();
		БазаРасчетаСОтборм = БазаРасчета;
	Иначе 
		СтрПоиска = Новый Структура("Сотрудник",ТекСотрудник);
		СтрокиПоОтборуТекСотрудник = Начисления.НайтиСтроки(СтрПоиска);
		Для каждого ТекСТрока из СтрокиПоОтборуТекСотрудник Цикл
			Начисления.Удалить(ТекСТрока);
		КонецЦикла;
		БазаРасчетаСОтборм = БазаРасчета.НайтиСтроки(СтрПоиска);
	КонецЕсли;
	...
	Для Каждого ПорядокРасчета Из ПорядкиРасчета Цикл
		...
		Для Каждого Строка Из БазаРасчетаСОтборм Цикл
		...
		РассчитатьЗначениеМинимальногоОклада(ПорядокРасчета,ТекСотрудник);
...
Процедура РассчитатьЗначениеМинимальногоОклада(ПорядокРасчета,ТекСотрудник = Неопределено)
	...
	Если ТекСотрудник = Неопределено Тогда
		...
		БазаРасчетаСОтбором = БазаРасчета;
	иначе      
		
		СтрПоиска = Новый Структура("Сотрудник",ТекСотрудник);
		НачисленияВДокументе = Начисления.Выгрузить(СтрПоиска);
		НачисленияВДокументе.Свернуть("Сотрудник, ВидРасчета, ПериодНачисления", "Результат");
		
		МинимальныйОклад = Перечисления.ФормулыРасчетаЗаработнойПлаты.МинимальныйОклад;
		
		БазаРасчетаСОтбором = БазаРасчета.НайтиСтроки(СтрПоиска);
	КонецЕсли;
	
	// Подготовка кеша
	БазаРасчетаМинОклады = БазаРасчета.ВыгрузитьКолонки(); 
		
	Для Каждого Строка Из БазаРасчетаСОтбором Цикл
		Если Строка.ВидРасчета.ПорядокРасчета = ПорядокРасчета
