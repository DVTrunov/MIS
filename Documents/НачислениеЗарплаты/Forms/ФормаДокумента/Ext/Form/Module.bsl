...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если  Не ЗначениеЗаполнено(Объект.НачалоПериода)
			И Не ЗначениеЗаполнено(Объект.КонецПериода)
		Тогда
			Объект.КонецПериода = НачалоМесяца(ТекущаяДата()) - 1;
...
&НаСервере
Процедура ВызватьМодульОбъектаЗаполнитьБазуРасчета(ТекСотрудник = Неопределено)
	
	СКД = Новый СхемаКомпоновкиДанных;
	ДокументОбъект = РеквизитФормыВЗначение("Объект");
	ДокументОбъект.ЗаполнитьБазуРасчета(КомпоновщикНастроек, ТекСотрудник);
	ЗначениеВРеквизитФормы(ДокументОбъект, "Объект");
	
КонецПроцедуры

&НаСервере
Процедура ВызватьМодульОбъектаЗаполнитьНачисления(ТекСотрудник = Неопределено )
	
	ДокументОбъект = РеквизитФормыВЗначение("Объект");
	ДокументОбъект.ЗаполнитьНачисления(ТекСотрудник);
	ЗначениеВРеквизитФормы(ДокументОбъект, "Объект");
...
&НаКлиенте
Процедура ЗаполнитьИРассчитатьПоСотруднику(Команда)
	
	Если ЗначениеЗаполнено(Элементы.Начисления.ТекущиеДанные.Сотрудник) Тогда
		
		ТекСотрудник = Элементы.Начисления.ТекущиеДанные.Сотрудник;
		...
		Если Объект.БазаРасчета.Количество() + Объект.Начисления.Количество() <> 0 Тогда
			...
			ТекстВопроса = НСтр("ru='Все таблицы документа по сотруднику " + ТекСотрудник + " будут очищены. Продолжить?'");
			Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да,);
		...
		ВызватьМодульОбъектаЗаполнитьБазуРасчета(ТекСотрудник);
		ВызватьМодульОбъектаЗаполнитьНачисления(ТекСотрудник);
...
Процедура НачисленияПриАктивизацииСтроки(Элемент)
	...
	Если не Элементы.Начисления.ТекущиеДанные = Неопределено Тогда
		
		ТекСотрудник = Элементы.Начисления.ТекущиеДанные.Сотрудник;
		
		Если ЗначениеЗаполнено(ТекСотрудник) Тогда
			ИзменитьЗаголовокКоманды(ТекСотрудник)
		КонецЕсли;
...
&НаСервере
Процедура ИзменитьЗаголовокКоманды(ТекСотрудник)
	
	КоманадаЗаполнитьИРассчитатьПоСотруднику = ЭтаФорма.Команды.Найти("ЗаполнитьИРассчитатьПоСотруднику");
	КоманадаЗаполнитьИРассчитатьПоСотруднику.Заголовок = "Заполнить и рассчитать " + ТекСотрудник;	
