...
//  ТабличныйДокумент.
//
Функция Печать(Ссылка, ИмяМакета) Экспорт
	
	ТабДок = Новый ТабличныйДокумент;
	
	Макет = Документы.Рецепт.ПолучитьМакет("омг_РецептФормаПечати");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Рецепт.Дата КАК ДатаВремя,
	|	Рецепт.Номер КАК НомерРецепта,
	|	Рецепт.омг_Препараты.(
	|		Номенклатура КАК Номенклатура,
	|		Количество КАК Количество,
	|		СпособПрименения КАК СпособПрименения,
	|		РазВдень КАК РазВдень,
	|		СколькоДней КАК СколькоДней,
	|		Дозировка КАК Дозировка,
	|		Дополнение КАК Дополнение
	|	) КАК омг_Препараты,
	|	Рецепт.Взрослый КАК ЭтоВзрослый,
	|	"""" + Рецепт.Клиент.ИНН + "" "" + Рецепт.Клиент.Наименование + "" "" + СТРОКА(Рецепт.Клиент.ДатаРождения) КАК ИИНФИОДР,
	|	Рецепт.Врач.Наименование КАК Врач,
	|	Рецепт.омг_ДействителенДо КАК РецептДо,
	|	Рецепт.Врач КАК Врач1,
	|	Рецепт.омг_ключ КАК омг_ключ
	|ИЗ
	|	Документ.Рецепт КАК Рецепт
	|ГДЕ
	|	Рецепт.Ссылка В(&Ссылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО Рецепт.Врач = КонтактнаяИнформация.Объект
	|ГДЕ
	|	Рецепт.Ссылка В(&Ссылка)
	|	И КонтактнаяИнформация.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.ТелефонСотовый)";
	Запрос.Параметры.Вставить("Ссылка", Ссылка);
	ВыборкаОбщая = Запрос.ВыполнитьПакет();
	Выборка = ВыборкаОбщая[0].Выбрать();
    ВыборкаТел = ВыборкаОбщая[1].Выбрать();
	
	Шапка = Макет.ПолучитьОбласть("Шапка");
	НоменклатураМакет = Макет.ПолучитьОбласть("Номенклатура");
	КоличествоМакет = Макет.ПолучитьОбласть("Количество");  
	DSМакет         = Макет.ПолучитьОбласть("DS");
	ПодвалМакет     = Макет.ПолучитьОбласть("Подвал");
	ТабДок.Очистить();
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ЭтоВзрослый = 1 Тогда 
			
			Шапка.Параметры.Взрослый = "Х"
		
		Иначе
			
			Шапка.Параметры.Ребенок  = "Х"		
		КонецЕсли;
		ТабДок.Вывести(Шапка, Выборка.Уровень());

		Выборкаомг_Препараты = Выборка.омг_Препараты.Выбрать();
		Сч = 1;
		Пока Выборкаомг_Препараты.Следующий() Цикл
			 НоменклатураМакет.Параметры.Номенклатура = ""+ Сч + ") Rp: " + Выборкаомг_Препараты.Номенклатура; 
			 ТабДок.Вывести(НоменклатураМакет, Выборкаомг_Препараты.Уровень());
			 КоличествоМакет.Параметры.Количество =  "D.T.D: " + Выборкаомг_Препараты.Количество;  
			 ТабДок.Вывести(КоличествоМакет, Выборкаомг_Препараты.Уровень());
			 DSМакет.Параметры.Прием =  "D.S. " + Выборкаомг_Препараты.СпособПрименения + ", в день " + Выборкаомг_Препараты.РазВдень +" раз, "+ " дней: " + Выборкаомг_Препараты.СколькоДней ;
			 ТабДок.Вывести(DSМакет, Выборкаомг_Препараты.Уровень());  
			 Сч = Сч + 1;
		КонецЦикла;
		
		ПодвалМакет.Параметры.РецептДо = Выборка.РецептДо;
		СтрПоиска = Новый Структура("Объект",Выборка.Врач1);
		ВыборкаТел.Сбросить();
		
		Если ВыборкаТел.НайтиСледующий(СтрПоиска) Тогда
			 ПодвалМакет.Параметры.Телефон = ВыборкаТел.Представление
		КонецЕсли;    
		
		Если ЗначениеЗаполнено(Выборка.омг_ключ) Тогда   
			ПодвалМакет.Параметры.КРАптеки = Новый Картинка(ИнтеграцияWebKassaВызовСервераПереопределяемый.ПолучитьQRКод(Выборка.омг_ключ, 1, 160));
		КонецЕсли;	
		
		ТабДок.Вывести(ПодвалМакет, Выборка.Уровень());
...
Функция ПечатьРецепта(СсылкаНаОбъект,ПечататьНоменклатуру,ДвусторонняяПечать, НоменклатураВРодительномПадеже)
	...
	ИначеЕсли ФормаРецепта = пчФормыРецептов.Форма148_1_у_04_л Тогда
		...
		ЗаполнитьПараметрПосимвольно(Макет, "КодКатегорииГраждан", Прав(Формат(СсылкаНаОбъект.ЛьготнаяКатегория.УИДЕГИСЗ, "ЧГ="), 3), 5);
		ЗаполнитьПараметрПосимвольно(Макет, "КодФормыМКБ", СсылкаНаОбъект.Диагноз.КодДиагноза, 5);
