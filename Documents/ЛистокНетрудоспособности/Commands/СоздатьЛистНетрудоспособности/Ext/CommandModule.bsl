...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды, ОткрытьФорму)
	...
	АктуальныеЛН = ПолучитьАктуальныеЛНКлиента(ПараметрКоманды);
	
	Если АктуальныеЛН.Количество() > 0 Тогда
		Оповещение = Новый ОписаниеОповещения("ОткрытьЛН", ЭтотОбъект, Новый Структура("Документ, Прием, ОткрытьФорму, ПараметрыВыполненияКоманды", АктуальныеЛН[0], ПараметрКоманды, ОткрытьФорму, ПараметрыВыполненияКоманды));
		ТекстВопроса = "У пациента уже есть открытый ЛН:
					   |(%1)
					   |Перейти к его редактированию?";
		
		ПоказатьВопрос(Оповещение, СтрШаблон(ТекстВопроса,АктуальныеЛН[0]),РежимДиалогаВопрос.ДаНет);
	Иначе
		ОткрытьЛН(КодВозвратаДиалога.Нет, Новый Структура("Прием, ОткрытьФорму, ПараметрыВыполненияКоманды", ПараметрКоманды, ОткрытьФорму, ПараметрыВыполненияКоманды));
	КонецЕсли;

КонецПроцедуры 

&НаКлиенте
Процедура ОткрытьЛН(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе 
		...
		ЗначенияЗаполнения = ПолучитьЗначенияЗаполненияДляЛистаНетрудоспособности(ДополнительныеПараметры.Прием, ОткрытьФорму);
...
&НаСервере
Функция ПолучитьАктуальныеЛНКлиента(Прием)

	Возврат Документы.ЛистокНетрудоспособности.ЛН_Пациента(Прием.Клиент).Актуальные;
