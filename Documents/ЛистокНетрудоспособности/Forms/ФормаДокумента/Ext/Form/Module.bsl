...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		Объект.ЛПУАдрес	 = Реквизиты.АдресФактический;
		...
		Если ЗначениеЗаполнено(Объект.Клиент) Тогда
			Если ОбщегоНазначенияКлиентСервер.ПолучитьПолноеЧислоЛетСДаты(Объект.Клиент.ДатаРождения, Объект.Дата) < 18 Тогда
				...
				КонецЕсли;
			ИначеЕсли ДанныеОКлиентеНеЗаполнены() Тогда
...
Процедура КлиентПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Клиент) Тогда
		АктуальныеЛНКлиента = ОбщегоНазначенияСервер.ФункцияМенеджера("Документы.ЛистокНетрудоспособности", "ЛН_Пациента", Объект.Клиент, Объект.Ссылка).Актуальные;
		
		Если АктуальныеЛНКлиента.Количество() > 0 Тогда
			Оповещение = Новый ОписаниеОповещения("ОткрытьЛН_Дубль", ЭтотОбъект, Новый Структура("Документ", АктуальныеЛНКлиента[0]));
			ТекстВопроса = "У пациента уже есть открытый ЛН:
						   |(%1)
						   |Перейти к его редактированию?";
		
			ПоказатьВопрос(Оповещение, СтрШаблон(ТекстВопроса,АктуальныеЛНКлиента[0]),РежимДиалогаВопрос.ДаНет);
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьПодписиПродолжение(Результат, ИмяСтроки) Экспорт 
	...
	Если МассивСтрок.Количество() > 0 Тогда
		ДополнительныеДанные = Новый Структура("ИмяСтроки, ИдентификаторФормы", ИмяСтроки, УникальныйИдентификатор); 
		УдалитьПодписьНаСервере(Объект.Ссылка, ДополнительныеДанные);
		Прочитать();
...
&НаСервереБезКонтекста
Процедура УдалитьПодписьНаСервере(ДокументСсылка, ДополнительныеПараметры)
	
	ИмяСтроки = ДополнительныеПараметры.ИмяСтроки;
	...
	Для Каждого СтрокаПодписи ИЗ СтрокиПодписи Цикл
		ЗаписьПодписи = доп_СертификатыЭЦП.ПолучитьЭлектроннуюПодпись(ДокументСсылка,,,СтрокаПодписи.ИдентификаторПодписи);
		Если ЗаписьПодписи <> Неопределено Тогда
			ЭлектроннаяПодпись.УдалитьПодпись(ДокументСсылка,ЗаписьПодписи.ПорядковыйНомер, ИдентификаторФормы);
		КонецЕсли;
	КонецЦикла;
...
Функция ДанныеПодписи(ДокументСсылка, Идентификатор)
	ЗаписьПодписи = доп_СертификатыЭЦП.ПолучитьЭлектроннуюПодпись(ДокументСсылка,,,Идентификатор);
...
&НаКлиенте
Процедура ОткрытьЛН_Дубль(Результат, ДополнительныеПараметры) Экспорт

	Если Результат = КодВозвратаДиалога.Да Тогда
		ПараметрыФормы = Новый Структура("Ключ", ДополнительныеПараметры.Документ);
