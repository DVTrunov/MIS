...
Процедура ЗаполнитьДанныеКлиентаПоПредставителю(Документ, Представитель) Экспорт
	...
	ДанныеПоУходу.ДатаНачалаУхода = Объект.Запись1ДатаНачала;
	
	// Степень родства.
	СтепеньРодства = Справочники.СтепениРодства.ПустаяСсылка();
	Если ТипЗнч(Представитель.СтепеньРодства) = Тип("Строка") Тогда
		// Попытка найти ЗакП среди родственников для подставноки степени родства.
		Если ТипЗнч(Представитель.Объект) = Тип("СправочникСсылка.Клиенты") Тогда
			...
			Для Каждого СтрокаПредставитель Из Представители Цикл
				Если СтрокаПредставитель.Представитель = Представитель.Объект
					И ТипЗнч(СтрокаПредставитель.СтепеньРодства) = Тип("СправочникСсылка.СтепениРодства")
				Тогда
					Если ЗначениеЗаполнено(СтрокаПредставитель.СтепеньРодства.КодФСС) Тогда
						СтепеньРодства = СтрокаПредставитель.СтепеньРодства;
					КонецЕсли;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(Представитель.СтепеньРодства.КодФСС) Тогда
		СтепеньРодства = Представитель.СтепеньРодства;
	КонецЕсли;
	
	ДанныеПоУходу.СтепеньРодства = СтепеньРодства;
...
//
Функция ЛН_Пациента(Клиент, Исключения = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("Исключения", ?(Исключения = Неопределено, Новый Массив, Исключения));
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|	И НЕ ЛистокНетрудоспособности.Ссылка В (&Исключения)
...
#Область СводкаЛНдляМедкарт // Информация об ЛН по медкарте (о первичном, продлениях, дубликате).

// Данные листков нетрудоспособности по медкарте.
//
// Параметры:
//  МедКарта - СправочникСсылка.МедицинскиеКарты - медицинская карта пациента, на которую оформлялись листки.
// 
// Возвращаемое значение:
//  Структура.
//
Функция ДанныеЛистковМедкарты(МедКарта) Экспорт
	
	ПервыйЛН = ДанныеПервогоЛисткаМедкарты(МедКарта);
	
	СтруктураВозврата = ПустаяСтруктураДанныхЛистковМедкарты();
	
	Если ПервыйЛН.Следующий() Тогда
		
		СтруктураВозврата.НомерПервичногоЛН = Формат(ПервыйЛН.Номер,"ЧГ=");
		СтруктураВозврата.ДатаПервичногоЛН = ПервыйЛН.Дата;
		СтруктураВозврата.ОсвобождениеОтРаботыНачало = ПервыйЛН.Запись1ДатаНачала;
		СтруктураВозврата.ОсвобождениеОтРаботыОкончание = Макс(ПервыйЛН.Запись1ДатаОкончания, ПервыйЛН.Запись2ДатаОкончания, ПервыйЛН.Запись3ДатаОкончания);
		СтруктураВозврата.ПриступитьКРаботе = ?(ПервыйЛН.ДатаНачалаРаботы = Дата(1,1,1),СтруктураВозврата.ОсвобождениеОтРаботыОкончание, ПервыйЛН.ДатаНачалаРаботы);
		
		ДанныеОпекаемогоРодственника = ПервыйЛН.ДанныеПоУходуЗаРодственниками.Выбрать();
		Если ДанныеОпекаемогоРодственника.Следующий() Тогда
			СтруктураВозврата.ФИО_ОпекаемогоЧленаСемьи = СокрЛП(ДанныеОпекаемогоРодственника.Фамилия + " " + ДанныеОпекаемогоРодственника.Имя + " " + ДанныеОпекаемогоРодственника.Отчество);
		КонецЕсли;
		
		Если ПервыйЛН.СледующийЛН <> "" Тогда
			ДанныеСледующегоЛистка(1, ПервыйЛН.СледующийЛН,СтруктураВозврата); 
		КонецЕсли;
		
		ЛНДубликат = НайтиДубликатЛистка(Формат(ПервыйЛН.Номер,"ЧГ="));
		Если ЛНДубликат <> Неопределено Тогда
			СтруктураВозврата.НомерЛНДубликата = Формат(ЛНДубликат.Номер,"ЧГ=");
			СтруктураВозврата.ДатаЛНДубликата = ЛНДубликат.Дата;
			Если ПервыйЛН.СледующийЛН = "" Тогда
				ДанныеСледующегоЛистка(1, ЛНДубликат.СледующийЛН,СтруктураВозврата);	
			КонецЕсли;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтруктураВозврата.НомерЛНПродолжения2) Тогда
			ДанныеСледующегоЛистка(2, СтруктураВозврата.НомерЛНПродолжения2,СтруктураВозврата);	
		КонецЕсли;
...
Функция ПустаяСтруктураДанныхЛистковМедкарты()
	
	СтруктураВозврата = Новый Структура;
	
	СтруктураВозврата.Вставить("НомерПервичногоЛН");
	СтруктураВозврата.Вставить("ДатаПервичногоЛН");
	СтруктураВозврата.Вставить("ОсвобождениеОтРаботыНачало");
	СтруктураВозврата.Вставить("ОсвобождениеОтРаботыОкончание");
	СтруктураВозврата.Вставить("НомерЛНДубликата");
	СтруктураВозврата.Вставить("ДатаЛНДубликата");
	СтруктураВозврата.Вставить("НомерЛНПродолжения1");
	СтруктураВозврата.Вставить("НомерЛНПродолжения2");
	СтруктураВозврата.Вставить("ОсвобождениеОтРаботыПродолжение1Начало");
	СтруктураВозврата.Вставить("ОсвобождениеОтРаботыПродолжение1Окончание");
	СтруктураВозврата.Вставить("ОсвобождениеОтРаботыПродолжение2Начало");
	СтруктураВозврата.Вставить("ОсвобождениеОтРаботыПродолжение2Окончание");
	СтруктураВозврата.Вставить("ПриступитьКРаботе", Дата(1,1,1));
	СтруктураВозврата.Вставить("ФИО_ОпекаемогоЧленаСемьи");
	
	Возврат СтруктураВозврата;
	
КонецФункции

Функция ДанныеПервогоЛисткаМедкарты(МедКарта)
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |	ЛистокНетрудоспособности.Ссылка КАК Ссылка,
	               |	ЛистокНетрудоспособности.ДатаНачалаРаботы КАК ДатаНачалаРаботы,
	               |	ЛистокНетрудоспособности.ДанныеПоУходуЗаРодственниками.(
	               |		Фамилия КАК Фамилия,
	               |		Имя КАК Имя,
	               |		Отчество КАК Отчество
	               |	) КАК ДанныеПоУходуЗаРодственниками
	               |ИЗ
	               |	Документ.ЛистокНетрудоспособности КАК ЛистокНетрудоспособности
	               |ГДЕ
...
Функция ДанныеЛисткаПоНомеру(Номер)
	...
	СтруктураЛН = ПустаяСтруктураДанныхЛистковМедкарты();
	
	Если Выборка.Следующий() Тогда
		
		СтруктураЛН.НомерПервичногоЛН = Выборка.Номер;
		СтруктураЛН.ОсвобождениеОтРаботыНачало = Выборка.Запись1ДатаНачала;
		СтруктураЛН.ОсвобождениеОтРаботыОкончание = Макс(Выборка.Запись1ДатаОкончания, Выборка.Запись2ДатаОкончания, Выборка.Запись3ДатаОкончания);
		СтруктураЛН.НомерЛНПродолжения1 = Выборка.СледующийЛН;
		
		СтруктураЛН.ПриступитьКРаботе = Макс(СтруктураЛН.ОсвобождениеОтРаботыОкончание, Выборка.ДатаНачалаРаботы); 
...
Функция НайтиДубликатЛистка(НомерЛН)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Документ.ЛистокНетрудоспособности КАК ЛистокНетрудоспособности
	               |ГДЕ
	               |	ЛистокНетрудоспособности.ВзаменРанееВыданного = &НомерЛН
	               |	И ЛистокНетрудоспособности.Дубликат";
...
Процедура ДанныеСледующегоЛистка(ЛНПоСчету, НомерЛН, СтруктураВозврата)
	
	ЛНПродолжение = ДанныеЛисткаПоНомеру(Число(НомерЛН));
	
	СтруктураВозврата["НомерЛНПродолжения" + ЛНПоСчету] = Формат(ЛНПродолжение.НомерПервичногоЛН,"ЧГ=");
	СтруктураВозврата["ОсвобождениеОтРаботыПродолжение" + ЛНПоСчету + "Начало"] = ЛНПродолжение.ОсвобождениеОтРаботыНачало;
	СтруктураВозврата["ОсвобождениеОтРаботыПродолжение" + ЛНПоСчету + "Окончание"] = ЛНПродолжение.ОсвобождениеОтРаботыОкончание;
	СтруктураВозврата["ПриступитьКРаботе"] = Макс(СтруктураВозврата.ПриступитьКРаботе, ЛНПродолжение.ПриступитьКРаботе);
	Если СтруктураВозврата.Свойство("НомерЛНПродолжения" + Строка(ЛНПоСчету + 1)) Тогда
		СтруктураВозврата["НомерЛНПродолжения" + Строка(ЛНПоСчету + 1)] = ЛНПродолжение.НомерЛНПродолжения1;	
	КонецЕсли;
