...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ЭтоПриемВСтационаре") Тогда
		ЭтоПриемВСтационаре = Параметры.ЭтоПриемВСтационаре;
	Иначе
		ЭтоПриемВСтационаре = ОпределитьЭтоПриемВСтационаре(Объект.МедицинскаяКарта);
	КонецЕсли;
	НастроитьЭлементыФормыСтационараСервер();
	...
	Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.МедицинскаяКарта) И Не Объект.омг_ЭтоОперация Тогда
			Объект.МедицинскаяКарта = КлиентПриИзмененииСервер(Объект.Клиент, ЭтоПриемВСтационаре);
			ЭтоПриемВСтационаре = ОпределитьЭтоПриемВСтационаре(Объект.МедицинскаяКарта);
			// Добавлено Макеев (
			Если ЭтоПриемВСтационаре Тогда
				омг_ЗаполнитьДокументОснованиеПоМедКарте();
			КонецЕсли;
			// )
		КонецЕсли;
	...
	// Добавлено Макеев (
	Если РольДоступна("омг_Врач") Тогда 
		Элементы.ГруппаШапка.Видимость = Ложь;
	КонецЕсли;
	// )     
	...
	//НовыйЭлемент = ЭтаФорма.Элементы.Добавить("СтраховаяПрограмма", Тип("ПолеФормы"),Элементы.ГруппаВрачКлиент);
	//НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	//НовыйЭлемент.Заголовок = "Страх. прогр.";
	//НовыйЭлемент.ПутьКДанным = "Объект.омг_СтраховаяПрограмма";	 
	Если Объект.Ссылка.Пустая() и ТипЗнч(ЭтаФорма.Параметры.Основание) = Тип("ДокументСсылка.Заявка") Тогда
		Объект.омг_МедПерсонал.Загрузить(омг_МедицинскаяДеятельность.ВернутьМедПерсоналПоИсточникам(ЭтаФорма.Параметры.Основание,ЭтаФорма.Параметры.ДополнительныеПараметры));
	КонецЕсли; 
	
	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("омг_Набор", Тип("ПолеФормы"),Элементы.Работы);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "Объект.Работы.омг_Набор";	 
	// ОМГ
	//////////////////////////////////////////////////////////////////
	
   УстановитьВидПланаЛечения();
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	НастроитьЭлементыФормыСтационара(); // УМЦ.КОРП
	
	/////////////////////////////////////////////////////////
	// ОМГ  
	омг_ЧетноеПроведение = Ложь;
	Если омг_МедицинскаяДеятельность.ЭтоВрач() и Объект.Ссылка.Пустая() и ЗначениеЗаполнено(Объект.ДокументОснование) Тогда 
		СтрПоиска = Новый Структура;
		СтрПоиска.Вставить("Роль",ПредопределенноеЗначение("Перечисление.омг_РолиПерсонала.Медсестра"));
		
		СтрокиМедСестра = Объект.омг_МедПерсонал.НайтиСтроки(СтрПоиска);   
		Если СтрокиМедСестра.Количество() = 0 и ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.Заявка") Тогда
			
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаМедСестра",ЭтотОбъект);	
 
   		  ПоказатьВопрос(Оповещение,
        "В заявке не указана  мед.сестра. Указать мед.сестру ?",
	...
	// Добавлено Карсаков (
	ОбновитьДоступностьТелемедКоманд ();
	// ) Конец добавленного Карсаков
	
	//Добавлено для автосохранения Карсаков
	ПодключитьОбработчикОжидания("омг_ФоновоеАвтосохранение", 45);
	//Конец добавления Карсаков 
	
	омгДокументОткрыт = Ложь; 
	
	Если не Объект.Ссылка.Пустая() Тогда	
		омгДокументОткрыт = ПриОткрытииНаСервере()
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	/////////////////////////////////////////////////////////
	// ОМГ
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение и омг_МедицинскаяДеятельность.ЭтоВрач() и не ЗначениеЗаполнено(Объект.Ссылка) Тогда   
		Если Не омг_ЧетноеПроведение Тогда
			Отказ = Истина;
    ...
    ПоказатьВопрос(Оповещение,
        ...
        "Проверьте корректность состава услуг!" // (необ.) заголовок
    );    
		КонецЕсли;	
	КонецЕсли;  
	// ОМГ
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	//Добавлено для автосохранения Карсаков
	ЭтоАвтосохранение = Ложь;
	Если ПараметрыЗаписи.Свойство("ЭтоАвтосохранение", ЭтоАвтосохранение) И ЭтоАвтосохранение Тогда
		ТекущийОбъект.ДополнительныеСвойства.Вставить("ЭтоАвтосохранение", Истина);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ИспользуютсяЭП Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ЗаписатьЭтапыНазначенийПланаЛечения(ТекущийОбъект.Ссылка, СписокНазначений); 
	ЗаписатьПозицииПланаМаршрута(ТекущийОбъект.Ссылка, СписокНазначений);
КонецПроцедуры
			...
			Если ЗначениеЗаполнено(Параметр.Диагноз) Тогда
				...
				Если НайденныеСтроки.Количество() = 0 Тогда
					...
					Если ТекущиеДанные <> Неопределено
						И Не ЗначениеЗаполнено(ТекущиеДанные.Диагноз)
					Тогда
...
&НаКлиенте
Процедура ПодборНазначенийУслуг(Команда)
	ВидНазначения = ?(Команда.Имя = "ПодборПроцедур", ПредопределенноеЗначение("Перечисление.ВидыНазначений.Процедура"), ПредопределенноеЗначение("Перечисление.ВидыНазначений.Обследование"));
	СтруктураПараметров = Новый Структура("ВидНазначения, ЗакрыватьПриВыборе, МножественныйВыбор, ЗакрыватьПриЗакрытииВладельца", ВидНазначения, Ложь, Истина, Истина);
	ОткрытьФорму("Документ.Прием.Форма.ФормаПодбора", СтруктураПараметров, Элементы.Назначения,,,,, РежимОткрытияОкнаФормы.Независимый);
КонецПроцедуры
...
Функция ПервичныйПриемДокумента(ОбъектФормы)
	...
	Если ЗначениеЗаполнено(ОбъектФормы.ПервичныйПрием) Тогда
		Возврат ОбъектФормы.ПервичныйПрием;
	КонецЕсли;
		...
		Тогда
			...
			Иначе
				...
				Если Не СтрокаШаблона.НеИспользоватьШапкуШаблона Тогда
					ТекстШаблонаПолный = МодульРаботыСШаблонами.УстановитьШапкуШаблона(ТекстШаблонаПолный, РежимПолучателяПечатнойВерсииОсмотра, Объект);
				КонецЕсли;
				ДокументHTML = РаботаСDOMКлиентСервер.СоздатьДокументDOM(ТекстШаблонаПолный);
				...
				ТекстШаблонаПолный = РаботаСDOMКлиентСервер.ПолучитьТекстHTMLДокументаDOM(ДокументHTML);
			...
			Если СтрокаСтраницыШаблона <> Неопределено Тогда
				...
				Иначе
					Если ПустаяСтрока(СтрокаШаблона.ТекстДляПечати) Тогда
						СтрокаШаблона.ТекстДляПечати = РаботаСШаблонамиHTMLКлиентСервер.ПолучитьТекстШаблонаHTMLПоУмолчанию();
					КонецЕсли;
					ДокументHTML = РаботаСDOMКлиентСервер.СоздатьДокументDOM(СтрокаШаблона.ТекстДляПечати);
					
					РаботаСШаблонамиHTMLКлиентСервер.ЗагрузитьИзображенияПриема(ДокументHTML, соотвКешИзображенийШаблонов, ЭтаФорма.УникальныйИдентификатор);
					
					СтрокаШаблона.ТекстДляПечати = РаботаСDOMКлиентСервер.ПолучитьТекстHTMLДокументаDOM(ДокументHTML);
...
Процедура КомандаПросмотрЭМК(Команда)
	...
	ОткрытьФорму("Обработка.ПросмотрМедКарт.Форма.ФормаАмбулаторнаяМК85", ПараметрыФормы);
...
&НаКлиенте
Процедура омг_ЭтоОперацияПриИзменении(Элемент)
	
	Объект.ЭтоПервичный = Ложь;
	Объект.ПервичныйПрием = "";
...
Процедура КлиентПриИзменении(Элемент)
	...
	Клиент = Объект.Клиент;
	УстановитьВидПланаЛеченияНаСервере();
КонецПроцедуры

&НаСервереБезКонтекста 
Функция КлиентПриИзмененииСервер(Клиент, ЭтоПриемВСтационаре)
...
&НаСервере
Процедура омг_ЗаполнитьДокументОснованиеПоМедКарте()
	...
	|ГДЕ
	|	Прием.Ссылка <> &Ссылка
	|	И Прием.МедицинскаяКарта = &МедицинскаяКарта
	|
	|УПОРЯДОЧИТЬ ПО
	|	Прием.Дата УБЫВ";
	...
	Если Объект.ДокументОснование.Пустая() И Выборка.Следующий() Тогда
...
Процедура ПерезаполнитьПодменюШаблоновЗаполненияОсмотра()
	...
	ТекПользователь = Пользователи.ТекущийПользователь().Сотрудник;
	Запрос.УстановитьПараметр("ВрачВладелец",ТекПользователь);  
	Запрос.Текст =
	//омг_Трунов
	"ВЫБРАТЬ
	...
	|	ШаблоныЗаполненияОсмотра.омг_ВрачВладелец КАК омг_ВрачВладелец
	|ИЗ
	...
	|	И ШаблоныЗаполненияОсмотра.омг_ВрачВладелец = &ВрачВладелец"
	;
...
&НаКлиенте
Процедура ЗаполнитьПланЛеченияИзСтандартаЛечения(СтандартыЛечения, ВыбранныеИдентификаторы = Неопределено)
	
	ВидНазначенияМедикаменты = ПредопределенноеЗначение("Перечисление.ВидыНазначений.Медикаменты");
	...
	Для Каждого РеквизитыСтандарта Из РеквизитыСтандартов Цикл
		...
		Назначения = РеквизитыСтандарта.Назначения;
		Для Каждого Назначение Из Назначения Цикл
			Если ВыбранныеИдентификаторы <> Неопределено
				И ВыбранныеИдентификаторы.Найти(Строка(Назначение.ИдентификаторНазначения)) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			// Определение необходимости добавлять новую позицию плана:
			//	- анализируются только позиции, где выполнено менее, чем назначено (только "актуальные")
			//	- если количество в плане (общее, вкл. уже выполненные) - меньше, чем в стандарте, то увеличиваем кол-во в строке.
			НайденныеНазначения = НайтиНазначенияВПлане(СписокНазначений, Назначение.Назначение, Назначение.ВидНазначения);
			
			НеДобавлятьНовуюСтрокуПлана = Ложь;
			Для Каждого СтрокаПлана Из НайденныеНазначения Цикл
				Если СтрокаПлана.Количество > СтрокаПлана.Факт Тогда // Ещё не вся выполнена
					НеДобавлятьНовуюСтрокуПлана = Истина;
					
					// Увеличиваем план, если в стандарте больше
					Если СтрокаПлана.Количество < Назначение.Количество Тогда
						СтрокаПлана.Количество = Назначение.Количество
					КонецЕсли;
					
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если НеДобавлятьНовуюСтрокуПлана Тогда
				Продолжить;
			КонецЕсли;
			
			// Добавление новой позиции плана лечения из стандарта
			...
			ЭлементГруппы.ЭтапМаршрутаПациента = Назначение.ЭтапМаршрутаПациента;
			ЭлементГруппы.СтандартЛечения = РеквизитыСтандарта.Ссылка;
			
			Если Назначение.ВидНазначения = ВидНазначенияМедикаменты Тогда
				...
				Если Назначение.Количество > 0 И Назначение.ПриемовВДень > 0 Тогда
					ЭлементГруппы.ДатаОкончания = ЭлементГруппы.Дата + (Назначение.Количество/Назначение.ПриемовВДень-1) * 86400;
				КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция НайтиНазначенияВПлане(ПланЛеченияНаФорме, Назначение, ВидНазначения = Неопределено, АнатомическаяОбласть = Неопределено)
	
	НайденныеНазначения = Новый Массив;
	
	Для Каждого СтрокаПлана Из ПланЛеченияНаФорме.ПолучитьЭлементы() Цикл
...
Процедура СтандартыЛеченияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.ЭтоГруппа Тогда
		...
		КонецЕсли;
	Иначе
		Если ТекущиеДанные.ВидСтандарта Тогда
			// Регламентированный стандарт — добавляем в используемые сразу.
			ДобавитьСтандартВИспользуемые(ТекущиеДанные.СтандартЛечения);
		Иначе
			// Внутренний стандарт — открываем форму выбора услуг.
			ПараметрыФормы = Новый Структура("СтандартЛечения", ТекущиеДанные.СтандартЛечения);
			Оповещение = Новый ОписаниеОповещения("СтандартыЛеченияВыбор_ПослеВыбораУслуг",
				ЭтотОбъект, ТекущиеДанные.СтандартЛечения);
			ОткрытьФорму("ОбщаяФорма.ВыборУслугСтандартаЛечения", ПараметрыФормы, ЭтотОбъект,
...
Процедура НазначенияНазначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Тогда
		Оповещение = Новый ОписаниеОповещения("НазначенияНазначениеНачалоВыбора_ПослеВыбораУслуги", ЭтотОбъект);
		НоваяРабота = РаботаСФормамиКлиент.ВыбратьЭлементНоменклатурыПоВиду(Истина, Элементы.Назначения.ТекущиеДанные.Назначение, Элементы.НазначенияНазначение, Ложь, Оповещение);
	Иначе
		Возврат; // Вид назначения "Прием" устарел. Редактирование больше не поддерживается. Пустой вид назначения также недопустим.
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура НазначенияНазначениеНачалоВыбора_ПослеВыбораУслуги(НоваяРабота, ДополнительныеПараметры) Экспорт
	
	Если НоваяРабота = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	НазначенияТекущиеДанные = Элементы.Назначения.ТекущиеДанные;
	
	Если НазначенияТекущиеДанные.Назначение <> НоваяРабота Тогда
		НазначенияТекущиеДанные.ДокументРегистратор = Объект.Ссылка;
	КонецЕсли;
	НазначенияТекущиеДанные.Назначение = НоваяРабота;
	
	Если Не ЗначениеЗаполнено(НазначенияТекущиеДанные.ИдентификаторНазначения) Тогда
		НазначенияТекущиеДанные.ИдентификаторНазначения = Новый УникальныйИдентификатор;
	КонецЕсли;
	Модифицированность = Истина;
...
Процедура ПервичныйПриемПриИзменении(Элемент)
	...
	Если Объект.Диагнозы.Количество() = 0
		И Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаДиагнозы
	Тогда
...
Процедура МедицинскаяКартаПриИзменении(Элемент)
	...
	ЭтоПриемВСтационаре = ОпределитьЭтоПриемВСтационаре(Объект.МедицинскаяКарта);
	НастроитьЭлементыФормыСтационара();
	УстановитьВидПланаЛеченияНаСервере();
КонецПроцедуры
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.СтраницаДиагнозы Тогда
		...
		КонецЕсли;
	ИначеЕсли ТекущаяСтраница = Элементы.ГруппаЭП Тогда
		// РЭМД
		ИнтеграцияЕГИСЗ_РЭМДКлиент.НастроитьЭлементыФормы(ЭтотОбъект);
	Иначе
		ОчиститьПустуюТаблицуДиагнозов();
...
Процедура ПодборАнализов(Команда)
	...
	ТекущиеИД = Новый Массив;
	Для Каждого СтрокаРабота Из Объект.Работы Цикл
		Если ЗначениеЗаполнено(СтрокаРабота.ИД) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьВМассивБезДублей(ТекущиеИД, СтрокаРабота.ИД);
		КонецЕсли;
	КонецЦикла;
	...
	ОтборыПодбора.Вставить("ТекущиеИД", ТекущиеИД);
	...
	Для Каждого ИдентификаторСтрокиДерева Из Элементы.Назначения.ВыделенныеСтроки Цикл
		...
		Если ТипЗнч(ЭлементГруппы.Назначение) = Тип("СправочникСсылка.Номенклатура") Тогда
			Если ЭлементГруппы.ВидНазначения <> ПредопределенноеЗначение("Перечисление.ВидыНазначений.Медикаменты") Тогда
				...
				КонецЕсли;
				// Добавлено Макеев (
				СтрокаРаботы.Количество = ЭлементГруппы.Количество;
				// )
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьПланЛеченияНаСервере()
	...
	Для Каждого Назначение Из мНазначения Цикл
		ЗаполнитьЗначенияСвойств(СписокНазначений.ПолучитьЭлементы().Добавить(), Назначение);
	КонецЦикла;
	ЗагрузитьЭтапыНазначенийПланаЛечения();
КонецПроцедуры
...
Процедура ДобавитьВОчередь(Команда)
	...
	Если РаботаСДиалогамиКлиент.ЗаписатьНовыйОбъектВФорме(ЭтаФорма) Тогда
		...
		Тогда
			ОткрытьФорму("Документ.ЛистОжидания.Форма.ФормаБыстрогоСоздания85", ПараметрыФормы);
		Иначе
			// Показываем список уже созданных листов ожидания по этому приёму
			ОткрытьФорму("Документ.ЛистОжидания.ФормаСписка85", ПараметрыФормы);
		КонецЕсли;
...
Процедура НазначенияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	// Изменено Макеев (
	Если ТипЗнч(ДанныеСтроки.Назначение) = Тип("СправочникСсылка.Номенклатура")
	Тогда
		СтандартнаяОбработка = Ложь;
...
Процедура ПолеДиагнозПриИзменении(Элемент)
	...
	ДиагнозПриИзменении();
	НеобходимоОбновитьСтандартыЛечения = Элементы.СтандартыЛеченияПоДиагнозам.Пометка;
	РаботаСШаблонамиHTMLКлиент.ПересчетПараметровОтЗависимыхРеквизитов("Диагнозы", ЭтаФорма, Объект);
КонецПроцедуры
...
Процедура ДиагнозПриИзменении()
	Если ЭтоМедосмотр Тогда
		...
		КонецЕсли;
	КонецЕсли;
//Начало ОМГ ТРунов *обновление шаблона при изменении диагноза*	
	//Если Не Элементы.КнопкаВерсияДляПечати.Пометка Тогда
	//	ПеренестиЗначенияТекстовыхПолейОсмотраВТаблицуПараметров();
	//	
	//	ЗначенияПоУмолчанию = РаботаСШаблонамиHTML.ПолучитьЗначенияПараметровПоУмолчанию(ТаблицаПараметров, КешЗависимыхПараметров, Объект);
...
&НаКлиенте
Процедура ВыбратьПротивопоказание()
	...
	Если ЭтоМедосмотр Тогда
		...
		Если ТаблицаДействий.Колонки.Найти("Действие") <> Неопределено Тогда
			...
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |	И ОтметкиВыполненияДействийСрезПоследних.ДокументРегистратор <> &ДокументРегистратор";
			
			Запрос.УстановитьПараметр("ПрохождениеМедОсмотра", Объект.ПервичныйПрием);
			Запрос.УстановитьПараметр("ДокументРегистратор", Объект.Ссылка);
			
			Результат = Запрос.Выполнить().Выгрузить();
			
			Если Результат.Количество() <> 0 Тогда
				ВыполненныеРанееДействия = Результат.ВыгрузитьКолонку("Действие");
			КонецЕсли;
		КонецЕсли;
		...
		Для Каждого СтрокаДействия Из ТаблицаДействий Цикл
			...
			// Если выполнено другим приемом, то недоступно
			Если ВыполненныеРанееДействия.Найти(СтрокаДействия.Действие) <> Неопределено Тогда
				НоваяСтрока.Выполнено = Ложь;
				НоваяСтрока.ВыполненоРанее = Истина;
			КонецЕсли;
...
Процедура Заключения(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		ОбщегоНазначения.СообщитьПользователю(нСтр("ru='Для редактирования заключений необходимо записать документ.'", "ru"));
		Возврат;
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФормуЗаключенийМедосмотра", ЭтотОбъект);
	РаботаСДиалогамиКлиент.ПроверитьМодифицированностьВФорме(ЭтаФорма, Истина, ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФормуЗаключенийМедосмотра(Результат, ДопПараметры) Экспорт
	...
	Для Каждого СтрокаОтметок Из ТаблицаОтметок Цикл
		СписокДействий.Добавить(СтрокаОтметок.Действие);
	КонецЦикла;
	
	Если ТипЗнч(Объект.ПервичныйПрием) <> Тип("ДокументСсылка.ПрохождениеМедосмотра") Или СписокДействий.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Нет действий медосмотра для выполнения в этом документе'"));
		Возврат;	
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура("Действия, ПрохождениеМедосмотра, ПриемИзКоторогоОткрытаФорма",СписокДействий, Объект.ПервичныйПрием, Объект.Ссылка);
...
&НаКлиенте
Процедура УстановитьРежимВСтационаре(Команда)
	
	Если РаботаСДиалогамиКлиент.ПроверитьМодифицированностьВФорме(ЭтаФорма) Тогда

		ЗначенияЗаполнения = Новый Структура("ДокументОснование, Врач, Клиент, МедицинскаяКарта",
	                                          Объект.Ссылка, 
	                                          Объект.Врач, 
	                                          Объект.Клиент, 
	                                          Объект.МедицинскаяКарта);
...
&НаКлиенте
Процедура ОткрытьСписокНазначений(Команда)
	
	ПараметрыФормы = Новый Структура("Отбор", Новый Структура("МедицинскаяКарта", Объект.МедицинскаяКарта));
	ОткрытьФорму("Документ.ИзменениеРежима.ФормаСписка", ПараметрыФормы, ЭтотОбъект);
	
КонецПроцедуры

&НаКлиенте
Процедура ДинамикаПоказателей(Команда)
	
	Если ЗначениеЗаполнено(Объект.МедицинскаяКарта) Тогда 
		ПараметрыФормы = Новый Структура("Филиал, МедКарта, Клиент", Объект.Филиал, Объект.МедицинскаяКарта, Объект.Клиент);
		ОткрытьФорму("ОбщаяФорма.ДинамикаПоказателейПациента",ПараметрыФормы);
	Иначе 
		ПоказатьПредупреждение(, "Необходимо выбрать медицинскую карту!");
	КонецЕсли;

КонецПроцедуры

&НаСервереБезКонтекста
Функция ОпределитьЭтоПриемВСтационаре(МедицинскаяКарта)
	
	Возврат ЗначениеЗаполнено(МедицинскаяКарта) И Стационар.ЭтоМедКартаСтационара(МедицинскаяКарта);
	
КонецФункции

&НаКлиенте
Процедура НастроитьЭлементыФормыСтационара()
	
	Элементы.ГруппаНазначенияСтационара.Видимость	= ЭтоПриемВСтационаре;
	Элементы.УчитываетсяВОбщемРасчете.Видимость		= ЭтоПриемВСтационаре;
	
	Если КоманднаяПанель.ПодчиненныеЭлементы.Найти("ФормаСоздать") <> Неопределено Тогда
		РаботаСФормамиКлиентСервер.УстановитьВидимостьЭлементаФормы(КоманднаяПанель.ПодчиненныеЭлементы.ФормаСоздать.ПодчиненныеЭлементы, "ФормаДокументОбращениеВСтационарСоздатьНаОсновании", ЭтоПриемВСтационаре);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура НастроитьЭлементыФормыСтационараСервер()
...
&НаКлиенте
Процедура СоздатьДокументРасчетФакторовРиска(Команда)
	ПараметрыФормы = Новый Структура("Клиент", Объект.Клиент);
	ОткрытьФорму("Документ.РасчетФакторовРиска.Форма.ФормаДокумента", ПараметрыФормы);
КонецПроцедуры
...
&НаКлиенте
Процедура омг_ОбработкаВыбораМедСестры(Результат,ДополнительныеПараметры) Экспорт
...
иначе  
	СтрПоиска = Новый Структура;   
	СтрПоиска.Вставить("Роль",ПредопределенноеЗначение("Перечисление.омг_РолиПерсонала.Медсестра"));
	НайдСтроки = Объект.омг_МедПерсонал.НайтиСтроки(СтрПоиска);
	Если НайдСтроки.Количество() > 0 Тогда
	СтрМедПерсонала = НайдСтроки[0];
	СтрМедПерсонала.Должность = ПредопределенноеЗначение("Справочник.Должности.ПустаяСсылка");
	иначе
	СтрМедПерсонала = Объект.омг_МедПерсонал.Добавить();
    КонецЕсли;	
	СтрМедПерсонала.Сотрудник = Результат;
	СтрМедПерсонала.Роль = ПредопределенноеЗначение("Перечисление.омг_РолиПерсонала.Медсестра");
...
&НаКлиенте
Процедура ПослеЗакрытияВопросаМедСестра(Результат, Параметры) Экспорт
 
    Если Результат = КодВозвратаДиалога.Да Тогда
        Если ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.Заявка") Тогда
			ОткрытьФорму("ОбщаяФорма.омг_ВыборМедсестры",,,,,,Новый ОписаниеОповещения("омг_ОбработкаВыбораМедСестры",ЭтотОбъект));
		КонецЕсли; 
    КонецЕсли;	
...
Процедура ЗаявкиДокументаСсылкаНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("Сотрудник",	Объект.Врач);
...
Процедура ОбработкаВыбораЗаявок(ВыбранныеЗаявки, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Для Каждого Заявка Из ВыбранныеЗаявки Цикл
		Если Заявка <> Объект.ДокументОснование Тогда
			...
			Иначе
				Объект.омг_Заявки.Добавить().Заявка = Заявка;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьЗаявкиДокумента()
	...
	Заявки = Новый Массив;
	Для Каждого СтрокаТЧ Из Объект.омг_Заявки Цикл
...
Процедура ОбновитьТекстГиперссылкиЗаявок()
	...
	Элементы.ЗаявкиДокументаСсылка.Видимость = Истина;
	
	Если Элементы.ЗаявкиДокументаСсылка.Видимость Тогда
		КоличествоДопЗаявок = Объект.омг_Заявки.Количество();
...
&НаСервере
Процедура омг_ЗаполнитьСоставУслугИзЗаявокНаСервере()
	
	Заявки = Новый Массив;
	Заявки.Добавить(Объект.ДокументОснование);
	Для каждого строкаЗаявка из Объект.омг_Заявки Цикл
		
		Заявки.Добавить(строкаЗаявка.Заявка);	
		
	КонецЦикла;	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	омг_НоменклатураЗаявокДляПриемовОстатки.Номенклатура КАК Номенклатура,
		|	омг_НоменклатураЗаявокДляПриемовОстатки.КоличествоОстаток КАК Количество,
		|	омг_НоменклатураЗаявокДляПриемовОстатки.омг_Набор КАК омг_Набор
		|ИЗ
		|	РегистрНакопления.омг_НоменклатураЗаявокДляПриемов.Остатки(, Заявка В (&Заявки)) КАК омг_НоменклатураЗаявокДляПриемовОстатки";
	
	Запрос.УстановитьПараметр("Заявки",Заявки);
	РезультатЗапроса = Запрос.Выполнить();
	Объект.Работы.Очистить();
	Объект.Работы.Загрузить(РезультатЗапроса.Выгрузить());
	
КонецПроцедуры
&НаКлиенте
Процедура омг_ЗаполнитьСоставУслугИзЗаявок(Команда) 
	
	Оповещение = Новый ОписаниеОповещения("омг_ПослеЗакрытияВопроса",
	ЭтотОбъект,Истина);	
	
	ПоказатьВопрос(Оповещение,
	"Документ будет снят с проведения и существующие услуги будут очищены и заменены на услуги из связанных заявок, продолжить?",
...
Процедура омг_ПослеЗакрытияВопроса(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда 
		...
		омг_ЗаполнитьСоставУслугИзЗаявокНаСервере();
...
&НаСервере
Процедура омг_ЗаполнитьСоставУслугВЗаявкахНаСервере()
	...
	ТабИсходныеРаботы = Объект.Работы.Выгрузить(,"Номенклатура,Количество");
	ТабИсходныеРаботы.Свернуть("Номенклатура","Количество");
	
	ЗаявкаОбъект = Объект.ДокументОснование.ПолучитьОбъект();
	ЕстьИзменения = Ложь;
	Для каждого строкаТабИсходныеРаботы из ТабИсходныеРаботы Цикл
		ВыборкаДетальныеЗаписи.Сбросить();
		СтрПоиска = Новый Структура(); 
		СтрПоиска.Вставить("Номенклатура",строкаТабИсходныеРаботы.Номенклатура);
		СтрПоиска.Вставить("Сотрудник",Объект.Врач);
		Если ВыборкаДетальныеЗаписи.НайтиСледующий(СтрПоиска) Тогда 
			Если ВыборкаДетальныеЗаписи.Количество < строкаТабИсходныеРаботы.Количество Тогда 
				МасНайденых = ЗаявкаОбъект.Работы.НайтиСтроки(СтрПоиска);
				Если МасНайденых.Количество() > 0 Тогда
					МасНайденых[0].Количество = строкаТабИсходныеРаботы.Количество;
					ЕстьИзменения = Истина;
				КонецЕсли;	 
			КонецЕсли;
		иначе
			НоваяСтрокаЗаявки = ЗаявкаОбъект.Работы.Добавить();
			НоваяСтрокаЗаявки.Номенклатура = строкаТабИсходныеРаботы.Номенклатура;
			НоваяСтрокаЗаявки.Сотрудник = Объект.Врач;
			НоваяСтрокаЗаявки.Количество = строкаТабИсходныеРаботы.Количество; 
			ЕстьИзменения = Истина;
		КонецЕсли;	
	КонецЦикла;	
	...
	Если ЕстьИзменения Тогда 
		...
		Попытка
			...
			Сообщить("Зявка " +ЗаявкаОбъект.Ссылка+" успешно обновлена!",статусСообщения.ОченьВажное);
		Исключение
			Сообщить("Не удалось обновить услуги в "+ЗаявкаОбъект.Ссылка,статусСообщения.ОченьВажное);
		КонецПопытки;	
...
&НаКлиенте
Процедура омг_ЗаполнитьСоставУслугВЗаявках(Команда)
	
	Если ЗначениеЗаполнено(Объект.ДокументОснование) и ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.Заявка") Тогда 
		Оповещение = Новый ОписаниеОповещения("омг_ПослеЗакрытияВопроса2",
		ЭтотОбъект,Истина);	
		
		ПоказатьВопрос(Оповещение,
		"Дозаполнить " +Объект.ДокументОснование+ " услугами?",
		РежимДиалогаВопрос.ДаНетОтмена,
		0,         
		КодВозвратаДиалога.Да,    
		"Предупреждение"); 
...
&НаКлиенте
Процедура омг_ПослеЗакрытияВопроса2(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда 
		...
		Если ЭтаФорма.Модифицированность Тогда
			Попытка
				ЭтаФорма.Записать()
			Исключение
				Сообщить("Не удалось записать прием!",статусСообщения.ОченьВажное);  
				Возврат;
			КонецПопытки;
		КонецЕсли;	
		
		омг_ЗаполнитьСоставУслугВЗаявкахНаСервере();
...
&НаКлиенте
Процедура омг_ФоновоеАвтосохранение() Экспорт
	
	Если Не ВерсияДляПечати И Модифицированность И Не Объект.Ссылка.Пустая() Тогда
		
		ПараметрыАвто = Новый Структура;
		ПараметрыАвто.Вставить("РежимЗаписи", РежимЗаписиДокумента.Запись);
		ПараметрыАвто.Вставить("ЭтоАвтосохранение", Истина);
		
		Попытка
			Записать(ПараметрыАвто);
		Исключение
		КонецПопытки;
	КонецЕсли;
...
&НаКлиенте
Процедура омг_ЭтоТелемедПриемПриИзменении(Элемент)
	ОбновитьДоступностьТелемедКоманд ();
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьДоступностьТелемедКоманд ()
	Если Объект.омг_ЭтоТелемедПрием Тогда
		Элементы.ГруппаТелемед.Видимость = Истина;
		Если Объект.омг_КонференцияТелемедЗавершена Тогда
			Элементы.НадписьЧатНеСоздан.Видимость = Ложь;
			Элементы.НадписьЧатСоздан.Видимость = Ложь;
			Элементы.НадписьЧатБылЗавершен.Видимость = Истина;
			Элементы.СоздатьКомнатуКонференцииСПациентом.Доступность = Ложь;
			Элементы.ПерейтиВКомнатуКонференции.Доступность = Ложь;
			Элементы.ВыкинутьПользователейИзКонференции.Доступность = Ложь;
			Элементы.НапомнитьПациентуОКонференции.Доступность = Ложь;
		Иначе
				Если ЗначениеЗаполнено(Объект.омг_ВнутренняяСсылкаНаОбсуждениеТелемед) Тогда
					Элементы.НадписьЧатНеСоздан.Видимость = Ложь;
					Элементы.НадписьЧатСоздан.Видимость = Истина;
					Элементы.НадписьЧатБылЗавершен.Видимость = Ложь;
					Элементы.СоздатьКомнатуКонференцииСПациентом.Доступность = Ложь;
					Элементы.ПерейтиВКомнатуКонференции.Доступность = Истина;
					Элементы.ВыкинутьПользователейИзКонференции.Доступность = Истина;
					Элементы.НапомнитьПациентуОКонференции.Доступность = Истина;
				Иначе
					Элементы.НадписьЧатНеСоздан.Видимость = Истина;
					Элементы.НадписьЧатСоздан.Видимость = Ложь;
					Элементы.НадписьЧатБылЗавершен.Видимость = Ложь;
					Элементы.СоздатьКомнатуКонференцииСПациентом.Доступность = Истина;
					Элементы.ПерейтиВКомнатуКонференции.Доступность = Ложь;
					Элементы.ВыкинутьПользователейИзКонференции.Доступность = Ложь;
					Элементы.НапомнитьПациентуОКонференции.Доступность = Ложь;
				КонецЕсли;
		КонецЕсли;
	Иначе
		Элементы.ГруппаТелемед.Видимость = Ложь;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура СоздатьКомнатуКонференцииСПациентом(Команда)
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
        ПоказатьПредупреждение (,"Документ не сохранен!!!! Сначала сохраните документ ""Прием"" нажав на кнопку ""Записать""",,"Внимание");
        Возврат;
	КонецЕсли;
	ДанныеЧата = СоздатьТелемедОбсуждение(); 
	Объект.омг_ВнутренняяСсылкаНаОбсуждениеТелемед = ПолучитьНавигационнуюСсылку(ДанныеЧата.Идентификатор);
	ПараметрыЗаписи = Новый Структура("РежимЗаписи", РежимЗаписиДокумента.Запись);
	ЭтаФорма.Записать(ПараметрыЗаписи);
	
	ТипДействияСКонференцией = Новый Структура;
	ТипДействияСКонференцией.Вставить("ТипДействия", "Конференция открыта");
	
	ПараметрыФормы = Новый Структура("Данные");
	ПараметрыФормы.Данные = ТипДействияСКонференцией;
	
	ОткрытьФорму("ОбщаяФорма.омг_ФормаПоказаСтатусаДействийТелемед", ПараметрыФормы);
	
	//ПоказатьПредупреждение(,"Комната конференции с пациентом создана! Вы можете перейти в нее нажав на кнопку ""Перейти в чат""",,"Команата создана");
    ОбновитьДоступностьТелемедКоманд ();
КонецПроцедуры


&НаСервере
Функция СоздатьТелемедОбсуждение()
    
    СтроковойКлюч = "Телемед_" + Строка(Объект.Ссылка.УникальныйИдентификатор());
    
    Обсуждение = СистемаВзаимодействия.ПолучитьОбсуждение(СтроковойКлюч);
    ФлагНовыйЧат = Ложь;
    Если Обсуждение = Неопределено Тогда
        Обсуждение = СистемаВзаимодействия.СоздатьОбсуждение();
        Обсуждение.Ключ = СтроковойКлюч; 
        Обсуждение.Заголовок = "Онлайн прием пациента: " + Объект.Клиент.Наименование;
        
        // Обязательно добавляем врача (иначе он не сможет войти по внутренней ссылке)
        УИДВрача = ПользователиИнформационнойБазы.ТекущийПользователь().УникальныйИдентификатор;
        Обсуждение.Участники.Добавить(СистемаВзаимодействия.ПолучитьИдентификаторПользователя(УИДВрача));
        Обсуждение.Записать();
		ФлагНовыйЧат = Истина;
    КонецЕсли;
    
    СсылкаДляПациента = СистемаВзаимодействия.ПолучитьНавигационнуюСсылкуПриглашенияВОбсуждение(Обсуждение.Идентификатор);
    Объект.омг_ВнешняяСсылкаНаКонференциюТелемед = СсылкаДляПациента;
	Сообщение = СистемаВзаимодействия.СоздатьСообщение(Обсуждение.Идентификатор);
    
	Если ФлагНовыйЧат Тогда
	    Сообщение.Текст = "Комната для видеозвонка готова. Ссылка для пациента сгенерирована: " + СсылкаДляПациента;
	Иначе
	    Сообщение.Текст = "Врач запросил ссылку повторно. Текущая ссылка для подключения: " + СсылкаДляПациента;
	КонецЕсли;
	
	Сообщение.Записать();
    Возврат Новый Структура("Идентификатор, СсылкаВнешняя", Обсуждение.Идентификатор, СсылкаДляПациента);		
КонецФункции

&НаКлиенте
Процедура ПерейтиВКомнатуКонференции(Команда)
    Если ПустаяСтрока(Объект.омг_ВнутренняяСсылкаНаОбсуждениеТелемед) Тогда
        ПоказатьПредупреждение(, "Комната для видеозвонка еще не создана.");
        Возврат;
    КонецЕсли;
    
    Попытка
        ПерейтиПоНавигационнойСсылке(Объект.омг_ВнутренняяСсылкаНаОбсуждениеТелемед);
    Исключение
        ТекстСообщения = "Доступ закрыт." + Символы.ПС + 
                         "Вы не можете читать этот чат и участвовать в видеозвонке, так как это прием другого врача.";
        ПоказатьПредупреждение(, ТекстСообщения,, "Внимание");
    КонецПопытки;
КонецПроцедуры

&НаКлиенте
Процедура ВыкинутьПользователейИзКонференции(Команда)
	 ЗакрытьКонференциюТелемедНаСервере ();
	 ПараметрыЗаписи = Новый Структура("РежимЗаписи", РежимЗаписиДокумента.Запись);
	 ЭтаФорма.Записать(ПараметрыЗаписи);
	 ОбновитьДоступностьТелемедКоманд ();
	 
	 ТипДействияСКонференцией = Новый Структура;
	 ТипДействияСКонференцией.Вставить("ТипДействия", "Конференция завершена");
 ...
 КонецПроцедуры
 &НаКлиенте
Процедура НазначенияСтандартЛеченияНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	
	СписокВыбора = Новый СписокЗначений;
	Для Каждого СтрокаСтандарта Из ИспользуемыеСтандартыЛечения Цикл
		Если СписокВыбора.НайтиПоЗначению(СтрокаСтандарта.СтандартЛечения) = Неопределено Тогда
			СписокВыбора.Добавить(СтрокаСтандарта.СтандартЛечения);
		КонецЕсли;
	КонецЦикла;
	
	Если СписокВыбора.Количество() = 0 Тогда
		ПоказатьПредупреждение(, "В документе ещё не выбран ни один стандарт лечения.");
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("НазначенияСтандартЛеченияВыбор_Завершение", ЭтотОбъект);
	ПоказатьВыборИзСписка(Оповещение, СписокВыбора, Элемент);
КонецПроцедуры  
&НаКлиенте
Процедура НазначенияСтандартЛеченияВыбор_Завершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	ТекущиеДанные = Элементы.Назначения.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
		ТекущиеДанные.СтандартЛечения = ВыбранныйЭлемент.Значение;
		Модифицированность = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура СтандартыЛеченияВыбор_ПослеВыбораУслуг(Результат, СтандартЛечения) Экспорт
	
	Если ТипЗнч(Результат) <> Тип("Массив") Тогда
		Возврат; // Врач отменил выбор.
	КонецЕсли;
	Если Результат.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДобавитьСтандартВИспользуемые(СтандартЛечения);
	
	мсСтандартыЛечения = Новый Массив;
	мсСтандартыЛечения.Добавить(СтандартЛечения);
	ЗаполнитьПланЛеченияИзСтандартаЛечения(мсСтандартыЛечения, Результат);
	
	ОтсортироватьПланЛеченияПоЭтапам();
	Модифицированность = Истина;
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьСтандартВИспользуемые(СтандартЛечения)
	
	Если ИспользуемыеСтандартыЛечения.НайтиСтроки(
			Новый Структура("СтандартЛечения", СтандартЛечения)).Количество() = 0 Тогда
		НоваяСтрока = ИспользуемыеСтандартыЛечения.Добавить();
		НоваяСтрока.СтандартЛечения = СтандартЛечения;
	КонецЕсли;
	
КонецПроцедуры 
&НаКлиенте
Процедура ОтсортироватьПланЛеченияПоЭтапам()
	
	СтрокиПлана = СписокНазначений.ПолучитьЭлементы();
	
	Массив = Новый Массив;
	Для Каждого СтрокаПлана Из СтрокиПлана Цикл
		Массив.Добавить(СтрокаПлана);
	КонецЦикла;
	
	Для Сч1 = 0 По Массив.ВГраница() - 1 Цикл
		Для Сч2 = 0 По Массив.ВГраница() - Сч1 - 1 Цикл
			Если ИндексЭтапа(Массив[Сч2].ЭтапМаршрутаПациента)
					> ИндексЭтапа(Массив[Сч2 + 1].ЭтапМаршрутаПациента) Тогда
				Буфер = Массив[Сч2];
	...
	Для Инд = 0 По Массив.ВГраница() Цикл
		ТекущийИндекс = СтрокиПлана.Индекс(Массив[Инд]);
		Если ТекущийИндекс <> Инд Тогда
			СтрокиПлана.Сдвинуть(ТекущийИндекс, Инд - ТекущийИндекс);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры
&НаКлиенте
Процедура ВедениеПланаЛечения(Команда)
	Если Не ЗначениеЗаполнено(Объект.Клиент) Тогда   // СВЕРИТЬ: реквизит клиента документа Прием
		ПоказатьПредупреждение(, "В приёме не заполнен клиент.");
		Возврат;
	КонецЕсли;
	Маршруты = МаршрутыПациентовСервер.МаршрутыПациентаДляВыбора(Объект.Клиент);
	Если Маршруты.Количество() = 0 Тогда
		ПоказатьПредупреждение(, "У пациента нет назначенных планов лечения.");
		Возврат;
	ИначеЕсли Маршруты.Количество() = 1 Тогда
		ОткрытьВедениеМаршрута(Маршруты[0].Значение);
	Иначе
		Оповещение = Новый ОписаниеОповещения("ВедениеПланаЛечения_ВыборМаршрута", ЭтотОбъект);
		ПоказатьВыборИзСписка(Оповещение, Маршруты, Команда);
	КонецЕсли;
КонецПроцедуры 
&НаСервере
Процедура УстановитьВидПланаЛеченияНаСервере()
	УстановитьВидПланаЛечения();
КонецПроцедуры
&НаСервере
Процедура УстановитьВидПланаЛечения()
	
	Элементы.Назначения.Видимость = Истина; // СВЕРИТЬ: имя элемента таблицы плана.
	Элементы.ГруппаПлашкаМаршрутов.Видимость = Ложь;
	ТекстПлашкиМаршрутов = "";
	
	Если Не ЭтоАмбулаторнаяКарта() Тогда
		Возврат;
	КонецЕсли;
	
	Если МаршрутыПациентовСервер.ЕстьСобственныйМаршрут(Объект.Ссылка) Тогда
		Возврат;
	КонецЕсли;
	...
	ЧужиеМаршруты = МаршрутыПациентовСервер.МаршрутыПациентаДляПлашки(
		Объект.Клиент, Объект.Ссылка);
	Если ЧужиеМаршруты.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	СписокНазначений.ПолучитьЭлементы().Очистить();
	
	Текст = "У пациента есть назначенные планы лечения:" + Символы.ПС;
	Для Каждого Элемент Из ЧужиеМаршруты Цикл
		Текст = Текст + "• " + Элемент.Представление + Символы.ПС;
	КонецЦикла;
	ТекстПлашкиМаршрутов = Текст;
	
	Элементы.Назначения.Видимость = Ложь;
	Элементы.ГруппаПлашкаМаршрутов.Видимость = Истина;
	
КонецПроцедуры

&НаСервере
Функция ЭтоАмбулаторнаяКарта()
	
	Если Не ЗначениеЗаполнено(Объект.МедицинскаяКарта) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ТипКарты = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		Объект.МедицинскаяКарта, "ВидКарты.ТипКарты");
	
	Возврат ТипКарты = Перечисления.ТипыМедицинскихКарт.Амбулаторная;
	
КонецФункции

&НаКлиенте
Процедура НазначитьОтдельныйПлан(Команда)
	Элементы.Назначения.Видимость = Истина;
	Элементы.ГруппаПлашкаМаршрутов.Видимость = Ложь;
КонецПроцедуры

&НаКлиенте
Процедура ПерейтиКВедениюИзПлашки(Команда)
	
	Если Не ЗначениеЗаполнено(Объект.Клиент) Тогда
		Возврат;
	КонецЕсли;
	
	Маршруты = МаршрутыПациентовСервер.МаршрутыПациентаДляПлашки(
		Объект.Клиент, Объект.Ссылка);
	
	Если Маршруты.Количество() = 0 Тогда
		ПоказатьПредупреждение(, "Незавершённых маршрутов не найдено.");
		Возврат;
	ИначеЕсли Маршруты.Количество() = 1 Тогда
		ОткрытьВедениеМаршрута(Маршруты[0].Значение);
	Иначе
		Оповещение = Новый ОписаниеОповещения("ПерейтиКВедениюИзПлашки_Выбор", ЭтотОбъект);
		ПоказатьВыборИзСписка(Оповещение, Маршруты, Команда);
	КонецЕсли;
...
&НаКлиенте
Процедура ПерейтиКВедениюИзПлашки_Выбор(ВыбранныйЭлемент, ДопПарам) Экспорт
	Если ВыбранныйЭлемент <> Неопределено Тогда
		ОткрытьВедениеМаршрута(ВыбранныйЭлемент.Значение);
	КонецЕсли;
КонецПроцедуры
&НаКлиенте
Процедура ВедениеПланаЛечения_ВыборМаршрута(ВыбранныйЭлемент, ДопПарам) Экспорт
	Если ВыбранныйЭлемент <> Неопределено Тогда
		ОткрытьВедениеМаршрута(ВыбранныйЭлемент.Значение);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ОткрытьВедениеМаршрута(Маршрут)
	ЗначенияЗаполнения = Новый Структура;
	ЗначенияЗаполнения.Вставить("ПервичныйПрием", Маршрут.ПервичныйПрием);
	ЗначенияЗаполнения.Вставить("СтандартЛечения", Маршрут.СтандартЛечения);
	ПараметрыФормы = Новый Структура("ЗначенияЗаполнения", ЗначенияЗаполнения);
...
&НаКлиентеНаСервереБезКонтекста
Функция ИндексЭтапа(Этап)
	
	Если Этап = ПредопределенноеЗначение("Перечисление.ЭтапыМаршрутаПациента.Консультация") Тогда
		Возврат 1;
	ИначеЕсли Этап = ПредопределенноеЗначение("Перечисление.ЭтапыМаршрутаПациента.ДиагностикаЛабораторная") Тогда
		Возврат 2;
	ИначеЕсли Этап = ПредопределенноеЗначение("Перечисление.ЭтапыМаршрутаПациента.ДиагностикаИнструментальная") Тогда
		Возврат 3;
	ИначеЕсли Этап = ПредопределенноеЗначение("Перечисление.ЭтапыМаршрутаПациента.Лечение") Тогда
		Возврат 4;
	ИначеЕсли Этап = ПредопределенноеЗначение("Перечисление.ЭтапыМаршрутаПациента.ДинамическийКонтроль") Тогда
		Возврат 5;
	Иначе
		Возврат 99;
	КонецЕсли;
...
КонецФункции  
&НаСервереБезКонтекста
Процедура ЗаписатьЭтапыНазначенийПланаЛечения(СсылкаПриема, СписокНазначений)
	
	НаборЗаписей = РегистрыСведений.ЭтапыНазначенийПланаЛечения.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ПервичныйПрием.Установить(СсылкаПриема);
	
	Для Каждого СтрокаПлана Из СписокНазначений.ПолучитьЭлементы() Цикл
		Если Не ЗначениеЗаполнено(СтрокаПлана.ЭтапМаршрутаПациента)
			ИЛИ Не ЗначениеЗаполнено(СтрокаПлана.СтандартЛечения) Тогда
			Продолжить;
		КонецЕсли;
		Запись = НаборЗаписей.Добавить();
		Запись.ПервичныйПрием			= СсылкаПриема;
		Запись.ИдентификаторНазначения	= Новый УникальныйИдентификатор(СтрокаПлана.ИдентификаторНазначения);
		Запись.ЭтапМаршрутаПациента		= СтрокаПлана.ЭтапМаршрутаПациента;
		Запись.СтандартЛечения			= СтрокаПлана.СтандартЛечения;
	КонецЦикла;
...
КонецПроцедуры
&НаСервереБезКонтекста
Процедура ЗаписатьПозицииПланаМаршрута(СсылкаПриема, СписокНазначений)
	
	НаборЗаписей = РегистрыСведений.ПозицииПланаМаршрута.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ПервичныйПрием.Установить(СсылкаПриема);
	...
	Для Каждого СтрокаПлана Из СписокНазначений.ПолучитьЭлементы() Цикл
		...
		Запись.СтандартЛечения			= СтрокаПлана.СтандартЛечения;
		Запись.ИдентификаторНазначения	= Новый УникальныйИдентификатор(СтрокаПлана.ИдентификаторНазначения);
		Запись.Номенклатура				= СтрокаПлана.Назначение; // СВЕРИТЬ имя реквизита: возможно "Номенклатура"
		Запись.ВидНазначения			= СтрокаПлана.ВидНазначения;
		Запись.ДатаНачала				= СтрокаПлана.Дата;
		Запись.ДатаОкончания			= СтрокаПлана.ДатаОкончания;
		Запись.Количество				= СтрокаПлана.Количество;
		Запись.Дозировка				= СтрокаПлана.Дозировка;
		Запись.ПриемовВДень				= СтрокаПлана.ПриемовВДень;
		Запись.ЕдиницаИзмерения			= СтрокаПлана.ЕдиницаИзмерения;
	КонецЦикла;
...
&НаСервере
Процедура ЗагрузитьЭтапыНазначенийПланаЛечения()
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЭтапыНазначений.ИдентификаторНазначения КАК ИдентификаторНазначения,
	|	ЭтапыНазначений.ЭтапМаршрутаПациента КАК ЭтапМаршрутаПациента,
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Соответствие.Вставить(Строка(Выборка.ИдентификаторНазначения),
			Новый Структура("Этап, Стандарт",
				Выборка.ЭтапМаршрутаПациента, Выборка.СтандартЛечения));
	КонецЦикла;
	
	Для Каждого СтрокаПлана Из СписокНазначений.ПолучитьЭлементы() Цикл
		Данные = Соответствие.Получить(Строка(СтрокаПлана.ИдентификаторНазначения));
		Если Данные <> Неопределено Тогда
			СтрокаПлана.ЭтапМаршрутаПациента = Данные.Этап;
			СтрокаПлана.СтандартЛечения = Данные.Стандарт;
		КонецЕсли;
	КонецЦикла;	
КонецПроцедуры

&НаКлиенте
Процедура ЗафиксироватьИсходЛечения(Команда)
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(, "Сначала запишите приём.");
		Возврат;
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ПервичныйПрием", Объект.Ссылка);
	ПараметрыФормы.Вставить("ДокументЗакрытия", Объект.Ссылка);
	
	ОткрытьФорму("ОбщаяФорма.ФиксацияИсходаЛечения", ПараметрыФормы, ЭтотОбъект,
		, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры


&НаСервере
Процедура ЗакрытьКонференциюТелемедНаСервере ()
	    СтроковойКлюч = "Телемед_" + Строка(Объект.Ссылка.УникальныйИдентификатор());
	Обсуждение = СистемаВзаимодействия.ПолучитьОбсуждение(СтроковойКлюч);
	Если Обсуждение <> Неопределено Тогда
		 УИДВрача = ПользователиИнформационнойБазы.ТекущийПользователь().УникальныйИдентификатор;
         IDВрачаВОбсуждении = СистемаВзаимодействия.ПолучитьИдентификаторПользователя(УИДВрача);
		 МассивНаУдаление = Новый Массив;
   		 Для Каждого Участник Из Обсуждение.Участники Цикл
      		  Если Участник <> IDВрачаВОбсуждении Тогда
      		      МассивНаУдаление.Добавить(Участник);
     		  КонецЕсли;
   		 КонецЦикла;
        
   		 Для Каждого УчастникКУдалению Из МассивНаУдаление Цикл
      		  Обсуждение.Участники.Удалить(УчастникКУдалению);
    	 КонецЦикла;
    
    	 Обсуждение.Заголовок = "[Завершен] " + Обсуждение.Заголовок;
    	 Сообщение = СистемаВзаимодействия.СоздатьСообщение(Обсуждение.Идентификатор);
    	 Сообщение.Текст = "Видеоприем завершен. Пациент отключен от комнаты.";
    	 Сообщение.Записать();
         Объект.омг_КонференцияТелемедЗавершена = Истина;
    	 Обсуждение.Записать();
	КонецЕсли;
...
Функция ПриОткрытииНаСервере()
	...
	СписокПросматривающих = омг_ДополнительныеАлгоритмы.ПолучитьСписокПросматривающих(Объект.Ссылка); 
	
	Если не СписокПросматривающих = "" Тогда 
		
		ОбщегоНазначения.СообщитьПользователю("Документ уже открыт у следующих пользователей: " + СписокПросматривающих);
	...
	Иначе	                 
		...
		МенеджерЗаписи.ДатаОткрытия = ТекущаяДата();
		МенеджерЗаписи.НачалоСеанса = омг_ДополнительныеАлгоритмы.ДатаНачалаСеанса(ТекущийНомерСеанса);
		МенеджерЗаписи.НомерСеанса  = ТекущийНомерСеанса;
		МенеджерЗаписи.УникальныйИдентификаторПользователяИБ = ПользователиИнформационнойБазы.ТекущийПользователь().УникальныйИдентификатор;		МенеджерЗаписи.Записать(Истина); 
...
Процедура ПриЗакрытииНаСервере()
	...
	Набор = РегистрыСведений.омг_ОткрытыеДокументы.СоздатьНаборЗаписей();
