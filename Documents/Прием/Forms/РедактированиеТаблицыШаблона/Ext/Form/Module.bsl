...
&НаКлиенте
Перем ДанныеТекущегоЭлементаHTML;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстШаблона					= Параметры.ТекстШаблона;
	ИдентификаторРодительскойСтроки	= Параметры.ИдентификаторРодительскойСтроки;
	ИдентификаторСтрокиШаблона		= Параметры.ИдентификаторСтрокиШаблона;
	ИдентификаторТаблицыШаблонов	= Параметры.ИдентификаторТаблицыШаблонов;
	КешЗначенийВыбора				= Параметры.КешЗначенийВыбора;
	
	Если Параметры.ЭталонныеСтрокиТаблиц = Неопределено Тогда
		// Тип параметра произвольный, чтобы попадать в эту ветку, если параметр при открытии формы не задан
		СписокЭталонныхСтрок = РаботаСШаблонамиHTML.ЭталонныеСтрокиТаблиц(ТекстШаблона);
	Иначе
		СписокЭталонныхСтрок = Параметры.ЭталонныеСтрокиТаблиц;
	КонецЕсли;
	
	ЭталонныеСтрокиТаблиц.ЗагрузитьЗначения(СписокЭталонныхСтрок.ВыгрузитьЗначения());
	
	ЗаполнитьТаблицыПараметров(Ложь);
	
	Если Не СформироватьПолеHTML(ИдентификаторТаблицыШаблонов, ТекстШаблона, МассивПараметров, МассивПараметровСВычисляемымЗначением) Тогда
		ПолеHTMLОсмотр = "";
		Возврат;
	КонецЕсли;
	...
	Для Каждого Параметр Из МассивПараметров Цикл
		...
		НоваяСтрока.НаименованиеДляВрача		 = РаботаСШаблонамиHTML.ПредставлениеПараметраШаблона(НоваяСтрока.Параметр);
	...
	ЗаполнитьТаблицыПараметров(Истина);
	...
	РаботаСШаблонамиHTMLКлиентСервер.УстановитьСкриптыРедактируемогоПоляHTML(HTMLДокумент);
...
Процедура ПриОткрытии(Отказ)
	...
	ЭлементТаблицы = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML, ИдентификаторТаблицыШаблонов, "table", "data-bit-dyntable-id", Истина);
	ТаблицаИзШаблона = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(РаботаСDOMКлиентСервер.СоздатьДокументDOM(ТекстШаблона), ИдентификаторТаблицыШаблонов, "table", "data-bit-dyntable-id", Истина);
	
	РаботаСШаблонамиHTMLКлиентСервер.РекурсивноЗаполнитьВложенныеТаблицы(ТаблицаПараметров, ЭлементТаблицы, ТаблицаИзШаблона, ИдентификаторСтрокиШаблона, , ИдентификаторРодительскойСтроки, ИдентификаторТаблицыШаблонов, ЭталонныеСтрокиТаблиц);
...
Процедура Готово(Команда)
	...
	Для Каждого СтрокаПараметра Из ТаблицаПараметров Цикл
		...
		Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru='Параметр %1 не заполнен. Данный параметр обязателен к заполнению'"), СтрокаПараметра.НаименованиеДляВрача));
	...
	ПередаваемыеПараметры.Вставить("ИдентификаторТаблицы", ИдентификаторТаблицыШаблонов);
	ПередаваемыеПараметры.Вставить("ИдентификаторСтрокиШаблона", ИдентификаторСтрокиШаблона);
...
Процедура ПолеHTMLДокументаОчистить(Команда)
	...
	мсСтрокаПараметра = ТаблицаПараметров.НайтиСтроки(Новый Структура("ИдентификаторТаблицы, СтрокаУидПараметра, ИдентификаторСтрокиШаблона, НомерСтрокиТаблицы", ИдентификаторТаблицыШаблонов, ДанныеЭлементаHTMLДляКонтекстногоМеню.id, ИдентификаторСтрокиШаблона, ДанныеЭлементаHTMLДляКонтекстногоМеню.НомерЭлемента));
	...
	РаботаСШаблонамиHTMLКлиент.ОчиститьЗначениеПараметраHTMLИнтерактивно(ЭтотОбъект, "ПолеHTMLДокумента", ЭлементHTMLДляКонтекстногоМеню, мсСтрокаПараметра[0], КешЗначенийВыбора, , , ДополнительныеПараметры);
...
Процедура ПолеHTMLДокументаПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	ДанныеТаблицы = Новый Структура("ИндексЭталоннойСтроки, ИдентификаторТаблицыШаблонов", ИндексЭталоннойСтроки, ИдентификаторТаблицыШаблонов);
	РаботаСШаблонамиHTMLКлиент.ОбработатьИзмененныеПоляВвода(ДанныеСобытия.Document, ЭтотОбъект, Элемент.Имя, ИдентификаторСтрокиШаблона, ТаблицаПараметров, КешЗначенийВыбора,, ДанныеТаблицы);
	
	Если ВРег(ДанныеСобытия.Element.TagName) = "BUTTON"
		Или ВРег(ДанныеСобытия.Element.TagName) = "IMG"
	Тогда
		...
		Если ЗначениеЗаполнено(ИдентификаторТаблицы) Тогда
			...
			КонецЕсли;
			РаботаСШаблонамиHTMLКлиент.ОбработатьНажатиеКнопкиРедактированияВложеннойТаблицы(ЭтотОбъект, ИдентификаторСтрокиШаблона, ИдентификаторТаблицы, ИдентификаторСтроки);
			
			Возврат;
		ИначеЕсли ДанныеСобытия.Element.getAttribute("data-bit-addrow") <> Неопределено Тогда
			ДобавитьСтрокуВТаблицу(ТаблицаПараметров.НайтиСтроки(Новый Структура("ИдентификаторТаблицы", ИдентификаторТаблицыШаблонов)).Количество() / ПараметрыЭталоннойСтроки.Количество());
		...
		ИначеЕсли ДанныеСобытия.Element.getAttribute("data-bit-deleterow") <> Неопределено Тогда
			...
			СохранитьДанныеТекущегоЭлемента(ДанныеСобытия.Element, Элементы.ПолеHTMLДокумента, ДанныеТекущегоЭлементаHTML);
			
			ОповещениеПодтверждение = Новый ОписаниеОповещения("УдалитьСтрокуТаблицыПослеПодтверждения", ЭтотОбъект);
	...
	ТекущаяСтрокаТаблицы = РаботаСШаблонамиHTMLКлиент.ПолучитьТекущуюСтрокуМногострочнойТаблицы(ЭлементHTML, ИдентификаторТаблицыШаблонов);
	...
	мсСтрокаПараметра = ТаблицаПараметров.НайтиСтроки(Новый Структура("ИдентификаторТаблицы, СтрокаУидПараметра, НомерСтрокиТаблицы", ИдентификаторТаблицыШаблонов, ЭлементHTML.id, ИндексТекущейСтроки - ИндексЭталоннойСтроки));
	Если мсСтрокаПараметра.Количество() > 0 Тогда
		...
		РаботаСШаблонамиHTMLКлиент.ВыбратьЗначениеПараметраHTMLИнтерактивно(ЭтаФорма, Элемент.Имя, ЭлементHTML, СтрокаПараметра, КешЗначенийВыбора, ОсновнойВладелец.Объект,, ДополнительныеПараметры);
	КонецЕсли;
...
Процедура РедактированиеВложеннойТаблицыШаблоновДокументаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	РаботаСШаблонамиHTMLКлиент.РедактированиеВложеннойТаблицыШаблоновДокументаЗавершение(Результат, ДополнительныеПараметры, ПолеHTMLОсмотр, ТаблицаПараметров, ИдентификаторРодительскойСтроки, ТекстШаблона, ЭталонныеСтрокиТаблиц);
...
Процедура НастройкаРеквизитовФормыПриСоздании()
	...
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Значение", ТипЗначенияПараметраHTML, "ПараметрыНеэталонныхСтрок", "Значение"));
...
Процедура ДобавитьСтрокуВТаблицу(КоличествоДобавленныхСтрок, СдвинутьНомераПараметров = Ложь, НеДобавлятьПараметр = Ложь, ИдентификаторСтроки = Неопределено)
	...
	ДокументHTML = РаботаСDOMКлиентСервер.СоздатьДокументDOM(ПолеHTMLОсмотр);
	Таблица = ПолучитьТаблицуDOM(ДокументHTML, ИдентификаторТаблицыШаблонов);
	...
	Для Каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
		Если СтрокаТаблицы.ИндексСтроки = ИндексЭталоннойСтроки Тогда
			...
			Если СдвинутьНомераПараметров Тогда // Вставить строку.
				Для Каждого СтрокаПараметра Из ТаблицаПараметров.НайтиСтроки(Новый Структура("ИдентификаторТаблицы", ИдентификаторТаблицыШаблонов)) Цикл
...
Процедура ДобавитьСтрокиТаблицы()
	...
	Если ПараметрыЭталоннойСтроки.Количество() <> 0 Тогда
		...
		Для НомерСтроки = 1 По НомераСтрок.Количество() Цикл
			...
			ПараметрыСтроки = ТаблицаПараметров.НайтиСтроки(Новый Структура("ИдентификаторТаблицы, НомерСтрокиТаблицы", ИдентификаторТаблицыШаблонов, НомерСтроки));
...
&НаКлиенте
Процедура УдалитьСтрокуТаблицыПослеПодтверждения(Ответ, ДополнительныеПараметры) Экспорт
	
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		
		ТекущийЭлементHTML = ПолучитьТекущийЭлементШаблонаHTMLИмеющийTag(ДанныеТекущегоЭлементаHTML, Элементы.ПолеHTMLДокумента);
		
		ЭлементСтроки = РаботаСHTMLКлиентСервер.ПолучитьБлижайшийУзел(ТекущийЭлементHTML, "tr");
		...
		ДокументHTML = РаботаСDOMКлиентСервер.СоздатьДокументDOM(ПолеHTMLОсмотр);
		УдаляемаяСтрока = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML, ИдентификаторУдаляемойСтроки, "tr", "data-bit-dyntable-tr-id", Истина);
		УдаляемаяСтрока.РодительскийУзел.УдалитьДочерний(УдаляемаяСтрока);
		ПолеHTMLОсмотр = РаботаСDOMКлиентСервер.ПолучитьТекстHTMLДокументаDOM(ДокументHTML);
	КонецЕсли;
	
	ДанныеТекущегоЭлементаHTML = Неопределено;
...
&НаСервере
Процедура ЗаполнитьТаблицыПараметров(ПараметрыТекущейТаблицы)
	...
	Для Каждого ДанныеПараметра Из Параметры.МассивДанныхТаблицы Цикл
		...
		Если ПараметрыТекущейТаблицы <> (ДанныеПараметра.ИдентификаторТаблицы = ИдентификаторТаблицыШаблонов) Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого СтруктураПараметра Из Параметры.ПараметрыТаблицы Цикл
		ЗаполнитьЗначенияСвойств(ПараметрыНеэталонныхСтрок.Добавить(), СтруктураПараметра);
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Функция СформироватьПолеHTML(ИдентификаторЭлемента, ТекстHTML, МассивПараметров, МассивПараметровСВычисляемымЗначением)
	...
	НомерЭталоннойСтроки = РаботаСШаблонамиHTML.НомерЭталоннойСтрокиТаблицыШаблонов(ИдентификаторТаблицыШаблонов);
	...
	Если ТелоТаблицы.Количество() <> 0 Тогда
		Для Каждого СтрокаТаблицы Из ТелоТаблицы[0].ДочерниеУзлы Цикл
			...
			Иначе
				ЗаполнитьНеэталоннуюСтроку(СтрокаТаблицы);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого СтруктураПараметра Из ПараметрыНеэталонныхСтрок Цикл
	...
	Для Каждого ВложеннаяТаблица Из ВложенныеТаблицыСтроки Цикл
		...
		Пока Родитель <> Неопределено Цикл
			...
			Если Родитель.ЕстьАтрибут("data-bit-dyntable-id") Тогда
				Если Родитель.ПолучитьАтрибут("data-bit-dyntable-id") = ИдентификаторТаблицыШаблонов Тогда
					...
					РаботаСШаблонамиHTMLКлиентСервер.ПодготовитьТаблицуШаблоновКДобавлениюВДокументHTML(ВложеннаяТаблица, УидТаблицыШаблонов, ЭталонныеСтрокиТаблиц, , Истина);
	...
	ТаблицаНовая = ПолучитьТаблицуDOM(НовыйДокументHTML, ИдентификаторТаблицыШаблонов);
...
&НаСервере
Процедура ЗаполнитьНеэталоннуюСтроку(СтрокаТаблицы)
	
	ВложенныеТаблицыСтроки = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(СтрокаТаблицы,, "table", "data-bit-dyntable-id");
	...
	Для Каждого СтруктураЭталоннойСтроки Из ЭталонныеСтрокиТаблиц.ВыгрузитьЗначения() Цикл
		Если Не СтруктураЭталоннойСтроки.Свойство("НомерЭталоннойСтроки") Тогда
			Продолжить;
		КонецЕсли;
	...
	ИсходныеТекстыСтрок = Новый Соответствие;
	Для Каждого ВложеннаяТаблица Из ВложенныеТаблицыСтроки Цикл
		ИдентификаторТаблицы = ВложеннаяТаблица.ПолучитьАтрибут("data-bit-dyntable-id");
		ТелоТаблицы = ВложеннаяТаблица.ПолучитьТелоТаблицы()[0];
		Попытка
			ИндексЭталоннойСтроки = НомераЭталонныхСтрок.Получить(ИдентификаторТаблицы) - 1;
			ЭталоннаяСтрока = ТелоТаблицы.ДочерниеУзлы.Элемент(ИндексЭталоннойСтроки);
		Исключение
			Продолжить;
		КонецПопытки;
		
		Идентификатор = Строка(Новый УникальныйИдентификатор);
		ИсходныеТекстыСтрок.Вставить(Идентификатор, РаботаСDOMКлиентСервер.ПолучитьТекстHTMLДокументаDOM(ЭталоннаяСтрока));
		ЭталоннаяСтрока.УстановитьАтрибут("data-bit-dyntable-original-reference-row", Идентификатор);
	КонецЦикла;
	
	РаботаСШаблонамиHTMLКлиентСервер.ЗаполнитьЗначенияПараметровHTMLДокумента(СтрокаТаблицы, ПараметрыНеэталонныхСтрок, ИдентификаторСтрокиШаблона, КешЗначенийВыбора, Истина,, ТаблицаПараметров);
	
	Если ИсходныеТекстыСтрок.Количество() > 0 Тогда
		Для Каждого ИсходнаяЭталоннаяСтрока Из РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(СтрокаТаблицы,, "tr", "data-bit-dyntable-original-reference-row") Цикл
			ИдентификаторЭлемента = ИсходнаяЭталоннаяСтрока.ПолучитьАтрибут("data-bit-dyntable-original-reference-row");
			ТекстИсходнойСтроки = ИсходныеТекстыСтрок.Получить(ИдентификаторЭлемента);
			ИсходнаяЭталоннаяСтрока.УдалитьАтрибут("data-bit-dyntable-original-reference-row");
			Если ТекстИсходнойСтроки <> Неопределено Тогда
				РаботаСHTMLКлиентСервер.ЗаменитьЭлементНаПроизвольныйHTML(ИсходнаяЭталоннаяСтрока, ТекстИсходнойСтроки);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ПолучитьПараметры(Элемент, МассивПараметров, МассивПараметровСВычисляемымЗначением, ДополнительныеЗначенияПараметров, ВложенныеТаблицы = Неопределено, ДанныеВложеннойТаблицы = Неопределено)
	...
	ПрерватьОбработкуЭлемента = Ложь;
	ОбработатьВложеннуюТаблицу(Элемент, МассивПараметров, МассивПараметровСВычисляемымЗначением, ДополнительныеЗначенияПараметров, ВложенныеТаблицы, ДанныеВложеннойТаблицы, ПрерватьОбработкуЭлемента);
	Если ПрерватьОбработкуЭлемента Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ОбработатьВложеннуюТаблицу(Элемент, МассивПараметров, МассивПараметровСВычисляемымЗначением, ДополнительныеЗначенияПараметров, ВложенныеТаблицы, ДанныеВложеннойТаблицы, ПрерватьОбработкуЭлемента)
	
	Если ДанныеВложеннойТаблицы = Неопределено Тогда
		Если ВложенныеТаблицы = Неопределено
			Или ВложенныеТаблицы.Найти(Элемент) = Неопределено
		Тогда
			Возврат;
		КонецЕсли;
		...
		Если ИдентификаторРодительскойТаблицы = ИдентификаторТаблицыШаблонов Тогда
		...
		ПрерватьОбработкуЭлемента = Истина;
	Иначе
		Если ДанныеВложеннойТаблицы.ЭталоннаяСтрокаВложеннойТаблицы = Элемент
...
Процедура ЗаполнитьТаблицуПараметрами()
	...
	КоличествоДобавленныхСтрок = ТаблицаПараметров.НайтиСтроки(Новый Структура("ИдентификаторТаблицы", ИдентификаторТаблицыШаблонов)).Количество() / ПараметрыЭталоннойСтроки.Количество();
	...
	Для Каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
		Если СтрокаТаблицы.ИндексСтроки > ИндексЭталоннойСтроки И СтрокаТаблицы.ИндексСтроки <= ИндексЭталоннойСтроки + КоличествоДобавленныхСтрок Тогда
			...
			Для Каждого Элемент Из РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(Элементы.ПолеHTMLДокумента.Документ, ИдентификаторТаблицыШаблонов, "table", "data-bit-dyntable-id") Цикл
				ТекущаяСтрокаТаблицы = Элемент.rows[Строка(СтрокаТаблицы.ИндексСтроки)];
			КонецЦикла;
			
			Для Каждого Параметр Из ТаблицаПараметров.НайтиСтроки(Новый Структура("ИдентификаторТаблицы, НомерСтрокиТаблицы", ИдентификаторТаблицыШаблонов, СтрокаТаблицы.ИндексСтроки - ИндексЭталоннойСтроки)) Цикл
				РаботаСШаблонамиHTMLКлиент.УстановитьЗначениеПредставлениеПараметраHTMLИнтерактивно(Параметр, ТекущаяСтрокаТаблицы, КешЗначенийВыбора);
			КонецЦикла;
...
Процедура ДобавитьПараметры(НомерСтрокиТаблицы, ИдентификаторСтроки)
	...
	Для Каждого Параметр Из ПараметрыЭталоннойСтроки Цикл
		...
		НоваяСтрока.ИдентификаторТаблицы			= ИдентификаторТаблицыШаблонов;
		НоваяСтрока.ИдентификаторСтроки				= ИдентификаторСтроки;
		НоваяСтрока.ИдентификаторРодительскойСтроки	= ИдентификаторРодительскойСтроки;
	КонецЦикла;
...
Функция ПолучитьТаблицуDOM(Знач ДокументHTML, Знач ИдентификаторТаблицыШаблонов)
	...
	Возврат РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(ДокументHTML, ИдентификаторТаблицыШаблонов,, "data-bit-dyntable-id", Истина);
...
Процедура СохранитьЗначенияАктивныхТекстовыхПолей()
	...
	АктивныеТекстовыеПоля = РаботаСHTMLКлиентСервер.ПолучитьЭлементыПоЗначениюАтрибута(Элементы.ПолеHTMLДокумента.Документ,,, "data-bit-isOnChange");
...
&НаКлиентеНаСервереБезКонтекста
Процедура СохранитьДанныеТекущегоЭлемента(ТекущийЭлемент, ПолеHTMLДокумента, ДанныеТекущегоЭлементаHTML)
	
	ЭлементыПоТегу = ПолеHTMLДокумента.Документ.getElementsByTagName(ТекущийЭлемент.tagName);
	Для НомерЭлемента = 1 По ЭлементыПоТегу.length Цикл
		ИндексЭлемента = НомерЭлемента - 1;
		Если ЭлементыПоТегу[ИндексЭлемента] = ТекущийЭлемент Тогда
			ДанныеТекущегоЭлементаHTML = Новый Структура("ИмяТега, НомерЭлемента", ТекущийЭлемент.tagName, ИндексЭлемента);
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьТекущийЭлементШаблонаHTMLИмеющийTag(ДанныеТекущегоЭлементаHTML, ПолеHTMLДокумента)
	
	Если ДанныеТекущегоЭлементаHTML = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ЭлементыПоТегу = ПолеHTMLДокумента.Документ.getElementsByTagName(ДанныеТекущегоЭлементаHTML.ИмяТега);
	
	Если ЭлементыПоТегу.length < ДанныеТекущегоЭлементаHTML.НомерЭлемента + 1 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат ЭлементыПоТегу[ДанныеТекущегоЭлементаHTML.НомерЭлемента];
