...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Если не РольДоступна("ПолныеПрава") Тогда
		...
		КонецЕсли;
	//КонецЕсли;  
	НовыйЭлемент.УстановитьДействие("ПриИзменении","омг_СтраховаяПрограммаПриИзменении");
...
Процедура ПриОткрытии(Отказ)
	...
	ПодключитьОбработчикОжидания("омг_ФоновоеАвтосохранение", 30);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ИспользуютсяЭП Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ЗаписатьЭтапыНазначенийПланаЛечения(Объект.Ссылка, СписокНазначений);	
	...
	Если СписокПросматривающих = "" Тогда 
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	//////////////////////////////////////////////////////////////////
	// ОМГ 
	Если омгДокументОткрыт или (не Объект.Ссылка.Пустая() и не ЭтотОбъект.ТолькоПросмотр) Тогда
		ПриЗакрытииНаСервере();
	КонецЕсли;
	// ОМГ
	//////////////////////////////////////////////////////////////////
...
Процедура ОтсортироватьПланЛеченияПоЭтапам()
	...
	// Сортировка пузырьком по индексу этапа (строк в плане немного).
	...
	// Переставляем строки дерева в нужный порядок.
...
Процедура ГруппаСтраницыОсмотраПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	ТекСтраница = СтраницыШаблоновПриема.НайтиСтроки(Новый Структура("ИмяСтраницыФормы", Элементы.ГруппаСтраницыОсмотра.ТекущаяСтраница.Имя));	
	СтрШаблонПриема = ТекСтраница[0];
	Элементы.ШаблоныПриема.ТекущаяСтрока  =  Объект.ШаблоныПриема[СтрШаблонПриема.ИдентификаторСтрокиШаблона].НомерСтроки - 1  
...
Процедура ЗаявкиДокументаСсылкаНажатие(Элемент)
	...
	ОткрытьФорму("Документ.Прием.Форма.ЗаявкиДокумента", ПараметрыФормы, ЭтаФорма,,,,Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОбработкаВыбораЗаявок(ВыбранныеЗаявки, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Тогда
		Объект.омг_Заявки.Добавить().Заявка = Объект.ДокументОснование;
	КонецЕсли;
...
Процедура ОбновитьТекстГиперссылкиЗаявок()
	...
	Если Элементы.ЗаявкиДокументаСсылка.Видимость Тогда
		КоличествоДопЗаявок = Объект.омг_Заявки.Количество()-1;
...
&НаСервере
Процедура омг_ЗаполнитьСоставУслугВЗаявкахНаСервере(НужноПровести = Ложь)
	...
	Для каждого строкаТабИсходныеРаботы из ТабИсходныеРаботы Цикл
		...
		Если ЗаявкаОбъект.НесколькоСотрудников Тогда
	...
	Если ЕстьИзменения или НужноПровести Тогда 
...
Процедура омг_ПослеЗакрытияВопроса2(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда 
		...
		омг_ЗаполнитьСоставУслугВЗаявкахНаСервере(НужноПровести);
...
Процедура омг_СтраховаяПрограммаПриИзменении(Элемент)
	...
	Если не омг_ДополнительныеАлгоритмы.ВернутьАктуальностьСоглашения(Объект.омг_СтраховаяПрограмма,Объект.Дата) Тогда
...
Процедура омг_ФоновоеАвтосохранение() Экспорт
	...
	Если Не ВерсияДляПечати И Модифицированность И Не Объект.Ссылка.Пустая() и не ЭтаФорма.ТолькоПросмотр Тогда
...
Функция ПриОткрытииНаСервере()
	...
	Если не СписокПросматривающих = "" Тогда 
		...
		Элементы.СтраницаОсмотр.Доступность = ложь;
		ВозЗнач = Ложь;
...
Процедура ЗаписатьПозицииПланаМаршрута(СсылкаПриема, СписокНазначений)
	...
	Для Каждого СтрокаПлана Из СписокНазначений.ПолучитьЭлементы() Цикл
		...
		Запись.СтандартЛечения			= СтрокаПлана.СтандартЛечения;
		Запись.ИдентификаторНазначения	= Новый УникальныйИдентификатор(СтрокаПлана.ИдентификаторНазначения);
		Запись.Номенклатура				= СтрокаПлана.Назначение; 
...
Процедура ЗафиксироватьИсходЛечения(Команда)
	ПервичныйПрием = Объект.Ссылка;;
	
	Если Не ЗначениеЗаполнено(ПервичныйПрием) Тогда
		ПоказатьПредупреждение(, "Сначала запишите приём — у маршрута должен быть первичный приём.");
		Возврат;
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ПервичныйПрием", ПервичныйПрием);
...
Процедура УстановитьВидПланаЛечения()
	...
	// По умолчанию — таблица плана видна, плашка скрыта.
	Элементы.Назначения.Видимость = Истина;
	Элементы.ГруппаПлашкаМаршрутов.Видимость = Ложь;
	ТекстПлашкиМаршрутов = "";
	
	// Механизм работает только для амбулаторных медкарт.
	Если Не ЭтоАмбулаторнаяКарта() Тогда
		Возврат;
	КонецЕсли;
	
	// Если у приёма есть свой маршрут — это его собственный план, показываем таблицу.
	Если МаршрутыПациентовСервер.ЕстьСобственныйМаршрут(Объект.Ссылка) Тогда
		Возврат;
	КонецЕсли;
	
	// Своего маршрута нет — проверяем чужие маршруты пациента.
	Если Не ЗначениеЗаполнено(Объект.Клиент) Тогда
		Возврат;
	КонецЕсли;
	...
	// Чужие маршруты есть — показываем плашку, таблицу очищаем (типовое заполнение снимаем).
	СписокНазначений.ПолучитьЭлементы().Очистить();
...
&НаКлиенте
Процедура омг_МедПерсоналСотрудникПриИзменении(Элемент)
	
	ТекСотрудник = Элементы.омг_МедПерсонал.ТекущиеДанные.Сотрудник;
	
	Если ЗначениеЗаполнено(ТекСотрудник) Тогда
		
		СтрДолжность = омг_ДополнительныеАлгоритмы.ВернутьДолжностьИРоль(ТекСотрудник); 
		Если не СтрДолжность = Неопределено	Тогда
			ЗаполнитьЗначенияСвойств(Элементы.омг_МедПерсонал.ТекущиеДанные,СтрДолжность)	
		КонецЕсли;	
	КонецЕсли;
