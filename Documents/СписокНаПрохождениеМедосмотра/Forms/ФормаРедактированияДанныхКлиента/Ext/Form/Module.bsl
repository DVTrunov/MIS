...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Попытка
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Параметры.ИмяРеквизита) Тогда
				ЭлементРеквизита = Элементы.Найти(ИмяРеквизита);
				Если ЭлементРеквизита <> Неопределено Тогда
					ТекущийЭлемент = ЭлементРеквизита;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	Исключение
...
Процедура ЗаписатьНовыеДанныеПМОИСПМО(ДанныеДляЗаписи, ТекстОшибки, СписокИзмененыхПМО)
	...
	МедосмотрыКлиентСервер.ЗаполнитьСПМОЧисленностьРаботниковПериодическогоМедосмотра(СПМООбъект);
	СПМООбъект.Записать();
