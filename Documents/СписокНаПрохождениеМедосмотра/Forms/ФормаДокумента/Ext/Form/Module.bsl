...
Процедура ПриОткрытии(Отказ)
	...
	ОбновитьДоступностьЧисленностей(Объект.РучнаяКорректировка);
...
&НаКлиенте
Процедура ОбновитьДоступностьЧисленностей(Доступность)
	
	Элементы.ГруппаРаботникиНаТяжРаботах.Доступность = Доступность;
	Элементы.ГруппаЧисленностьНиз.Доступность		 = Доступность;
...
Процедура УстановитьДоступностьЭлементов()
	...
	доп_СертификатыЭЦПКлиент.УстановитьДоступностьЭлементов(ЭтотОбъект, СписокНередактируемыхЭлеменовПоЭП(), Объект.ПодписанЭП);
...
Процедура ОбновитьСписокПодписей()
	...
	доп_СертификатыЭЦП.ЗаполнитьСписокПодписей(Объект.Ссылка, УникальныйИдентификатор, Неопределено, ЭлектронныеПодписи,, СводкаHTML);
...
Асинх Процедура ИзменитьСтроку(Команда)
	...
	ТекущаяКолонкаСписка = Элементы.Клиенты.ТекущийЭлемент.Имя;
	ИмяТекущегоРеквизита = Сред(ТекущаяКолонкаСписка, СтрДлина("Клиенты")+1);
	ПараметрыФормы.Вставить("ИмяРеквизита", ИмяТекущегоРеквизита);
...
Процедура КлиентыДобавлениеЗавершение(Результат, ТекущаяСтрока) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		КонецЕсли;
		Модифицированность = Истина;
		МедосмотрыКлиентСервер.ЗаполнитьСПМОЧисленностьРаботниковПериодическогоМедосмотра(Объект);
		КоличествоКлиентовВСмете = ПолучитьКоличествоКлиентовСметы();
	КонецЕсли;
