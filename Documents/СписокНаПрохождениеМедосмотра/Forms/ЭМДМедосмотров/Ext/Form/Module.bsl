...
Функция ПолучитьДДФайла(ПМО)
	...
	ДанныеФайловЭМД = доп_РаботаСФайлами.ПолучитьДанныеФайловЭМД(ПМО, , ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор());
...
Процедура ПодписатьФайлЭМДВРоли()
	...
	Для Каждого ОбъектМ Из МассивОбъектов Цикл
		...
		ЭлементДанных.Вставить("Объект",
			Новый ОписаниеОповещения("ПриПолученииПодписи", ОбщегоНазначенияКлиент.ОбщийМодуль("доп_СертификатыЭЦПКлиент"), ТекущиеПараметрыВыполнения));
...
Функция СформироватьФайлыДляСохранения()
	...
	Для Каждого ПодписанныйПМО Из ДокументыПМО.НайтиСтроки(Новый Структура("ПодписанМО, ПодписанПредседателем", Истина, Истина)) Цикл
		...
		ДанныеФайловЭМД = доп_РаботаСФайлами.ПолучитьДанныеФайловЭМД(ПодписанныйПМО.ПМО, Истина, ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор());
		Если ДанныеФайловЭМД.Количество() > 0 Тогда
		...
		Пока ВыборкаПодписи.Следующий() Цикл
			Если Не ЭМДДобавлен Тогда
				...
				Если ЗначениеЗаполнено(ВыборкаПодписи.ПутьКФайлу) Тогда
					...
					Если Не доп_РаботаСФайлами.ФайлСуществует(ПутьДоЭМДВАрхиве) Тогда
