...
Функция ПечатьЗаключительныйАкт(СсылкаНаОбъект, МВТ = Неопределено, СохранениеЭМД = Ложь)
	...
	// 3
	ОбластьЧислОбязОсм.Параметры.Всего			= СсылкаНаОбъект.ЧисленностьРаботниковОбязМедосмотрВсего;
	ОбластьЧислОбязОсм.Параметры.ВТЧЖенщин		= СсылкаНаОбъект.ЧисленностьРаботниковОбязМедосмотрЖенщин;
	ОбластьЧислОбязОсм.Параметры.РабДо18		= СсылкаНаОбъект.ЧисленностьРаботниковОбязМедосмотрДо18Лет;
	ОбластьЧислОбязОсм.Параметры.РабУтрТруд		= СсылкаНаОбъект.ЧисленностьРаботниковОбязМедосмотрУстУтратаНетрудоспособности;
	...
	// 4
	ОбластьЧислПодлежОсм.Параметры.Всего		= СсылкаНаОбъект.ЧисленностьРаботниковПодлежМедосмотруВсего;
	ОбластьЧислПодлежОсм.Параметры.ВТЧЖенщин	= СсылкаНаОбъект.ЧисленностьРаботниковПодлежМедосмотруЖенщин;
	ОбластьЧислПодлежОсм.Параметры.РабДо18		= СсылкаНаОбъект.ЧисленностьРаботниковПодлежМедосмотруДо18Лет;
	ОбластьЧислПодлежОсм.Параметры.РабУтрТруд	= СсылкаНаОбъект.ЧисленностьРаботниковПодлежМедосмотруУстУтратаНетрудоспособности;
	...
	// Процент охвата. Расширенный вариант с отдельно Ж и до 18лет сохранен в неиспользуемой области макета "ПроцентОхватаРасш".
	// 6.
	...
	#Область НеИспользуется // Детализация "Имеющих противопоказания" на "Временную" и "Постоянную". Не используется, т.к. отсутствует в пр.29н, но сохранено в макете как облать ИмПротПодробно
	//// Временные
