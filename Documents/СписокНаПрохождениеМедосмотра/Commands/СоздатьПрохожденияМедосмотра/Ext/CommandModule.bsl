	...
	Если ТипЗнч(ПараметрКоманды) = Тип("Массив") Тогда
		...
		Если спПМО.Количество() > 0 Тогда
			...
			Если Ответ = КодВозвратаДиалога.Да Тогда
				...
				Иначе
					мсПФ = УниверсальныеМеханизмыКлиент.ОткрытьФормуВыбораПечатныхФормОбъектаМножественнаяПечать(спПМО[0].СсылкаПМО, ПараметрыВыполненияКоманды.Источник, "Справочник.ПриказыМедосмотров.Форма.ВыборПечатнойФормы");
				КонецЕсли;
...
Функция ОткрытьФормуВыбораПечатныхФормОбъектаМножественнаяПечать(Объект, ФормаВладелец) Экспорт
	...
	Попытка
		...
		Форма = ПолучитьФорму("Справочник.ПриказыМедосмотров.Форма.ВыборПечатнойФормы", ,ФормаВладелец);
