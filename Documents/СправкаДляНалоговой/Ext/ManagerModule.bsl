...
Функция ПолучитьПаспортныеДанныеКлиента(Клиент, Дата = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ПаспДанныеРегистр.ДокументВид) Тогда
		...
		ПаспортныеДанные.СерияДУЛ		 = СтрЗаменить(ПаспДанныеРегистр.ДокументСерия, " ", "");
		ПаспортныеДанные.НомерДУЛ		 = СтрЗаменить(ПаспДанныеРегистр.ДокументНомер, " ", "");
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаОбъекта" Тогда
		...
		ИначеЕсли Параметры.Свойство("ЗначениеКопирования")
			И Параметры.ЗначениеКопирования.ОтчетныйГод >= 2024
		Тогда
			Возврат; // При копировании обр.24 открываем сразу форму 24.
