...
Процедура ЗаполнитьПоПодразделению(Команда)
	Если Не ЗначениеЗаполнено(Объект.Подразделение) Тогда
		ПоказатьПредупреждение(,"Не заполнено подразделение", 30);
		Возврат;
	КонецЕсли;	
...
Процедура ЗаполнитьПоГрафику(Сотрудники, Сообщать = Истина, Создавать = Ложь, ОтборФилиалы = Неопределено)
	...
	Для Каждого Сотрудник Из Сотрудники Цикл
		...
		СтрокаТабеля = ОбщегоНазначенияБИТКлиентСервер.ПерваяСтрокаОтбораТаблицы(Объект.Табель, Новый Структура("Сотрудник",Сотрудник));
