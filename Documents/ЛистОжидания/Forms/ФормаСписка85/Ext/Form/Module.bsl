...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Корректировка 8.5 Карсаков
	СобытияПоЛистуОжидания.Параметры.УстановитьЗначениеПараметра("ЛистОжидания", Документы.ЛистОжидания.ПустаяСсылка());
	//Конец корректировки
...
&НаКлиенте
Процедура омг_УстановитьФильтры(Команда)
	Элементы.ГруппаПериод.Показать();
КонецПроцедуры

&НаКлиенте
Процедура омг_СкрытьФильтры(Команда)
	Элементы.ГруппаПериод.Скрыть();
КонецПроцедуры

&НаКлиенте
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "ИконкаЗвонка" Тогда
		...
		НомерТелефона = ПолучитьТелефонКлиентаНаСервере(ТекущиеДанные.Клиент);
		...
		доп_ТелефонияКлиентПереопределяемый.Позвонить(НомерТелефона, , ВыбраннаяСтрока);
		
	КонецЕсли;

КонецПроцедуры
&НаСервереБезКонтекста
Функция ПолучитьТелефонКлиентаНаСервере(КлиентСсылка)
