...
Функция Печать(СсылкаНаОбъект, ИмяМакета) Экспорт
	...
	Если ИмяМакета = "ПечатьЗаказНаряда" Тогда
		ТабДокумент = ПечатьЗаказНаряда(СсылкаНаОбъект, ЗначениеЗаполнено(СсылкаНаОбъект.Вид.ЭтоЗуботехническийЗаказНаряд));
	КонецЕсли;
...
Процедура ЗаполнитьСоставПоВиду(Объект, УдалитьСтарые = Истина) Экспорт
	...
	Если УправлениеЗаказами.ЭтоЗаказНаПроизводство(Объект.Вид) И Объект.Вид.ЭтоЗуботехническийЗаказНаряд Тогда
		...
		Если ЗначениеЗаполнено(Объект.ДокументОснование) И 
			ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.Прием") Тогда 
			РедактированныеЗубы = стомСтоматология.ПолучитьКонструкцииЗубыРедактированныеВПриеме(
				Объект.ДокументОснование, Объект.Клиент, Объект.ЗубнаяКарта);
			Если РедактированныеЗубы.Количество() > 0 Тогда 
				Объект.КонструкцияЗуб = РедактированныеЗубы[0];
			КонецЕсли;
		КонецЕсли;
...
Функция ПечатьЗаказНаряда(СсылкаНаОбъект, ВыводитьЗубнуюКарту = Ложь)
	...
	#Область Стоматология
	// Получаем данные о зубе или конструкции
	Если СсылкаНаОбъект.Вид.ЭтоЗуботехническийЗаказНаряд Тогда 
		ОбластьЗФШапка = МакетНаряда.ПолучитьОбласть("ОбластьЗФШапка");
		ТабДок.Вывести(ОбластьЗФШапка);	
		ОбластьЗФ = МакетНаряда.ПолучитьОбласть("ОбластьЗФ");
		АдресЗК = стомЗубнаяКартаИнициализация.ИнициализироватьЗубнуюКарту(СсылкаНаОбъект.ЗубнаяКарта, 
			СсылкаНаОбъект.Клиент, Новый УникальныйИдентификатор, СсылкаНаОбъект.Дата, СсылкаНаОбъект.ВариантЗубнойКарты, СсылкаНаОбъект);
		Если ЗначениеЗаполнено(АдресЗК) Тогда 
			
			// Обозначения и варианты зубов
			...
			Для Каждого кзЗуб Из СтруктураЗК.Зубы Цикл
				...
				ИмяПоляОбозначение = "З" + СтруктураЗуб.Расположение.Номер;
				ИмяПоляВариант = "З" + СтруктураЗуб.Расположение.Номер + "В";
				ИмяПоляКонструкция = "З" + СтруктураЗуб.Расположение.Номер + "К";
				
				ОбластьЗФ.Параметры[ИмяПоляОбозначение] = СтруктураЗуб.Обозначение;
				ОбластьЗФ.Параметры[ИмяПоляВариант] = стомЗубнаяКартаМетоды.ПолучитьОбозначениеЗуба(СтруктураЗК, СтруктураЗуб.ЗубВКарте, Ложь);
				
				Если СтруктураЗуб.ЗубВКарте = СсылкаНаОбъект.КонструкцияЗуб Тогда 
					ОбластьЗуб = ОбластьЗФ.Область(ИмяПоляОбозначение);
					ОбластьЗуб.Обвести(ЖирнаяЛиния, ЖирнаяЛиния, ЖирнаяЛиния, ЖирнаяЛиния);
				КонецЕсли;
			...
			Для Каждого кзКонструкция Из СтруктураЗК.Конструкции Цикл
				...
				НомерПервойКолонки = Неопределено;
				НомерПоследнейСтроки = 0;
				НомерПоследнейКолонки = 0;
				Для Каждого ЗубВКарте Из СтруктураКонструкция.СоставЗубов Цикл 
					
					СтруктураЗуб = стомЗубнаяКартаМетоды.НайтиЗубВКарте(СтруктураЗК, ЗубВКарте);
					Если СтруктураЗуб = Неопределено Или СтруктураЗуб.Расположение = Неопределено Тогда 
						Продолжить;
					КонецЕсли;
					
					ИмяПоляКонструкция = "З" + СтруктураЗуб.Расположение.Номер + "К";
					ОбластьЗФ.Параметры[ИмяПоляКонструкция] = СтруктураКонструкция.Обозначение;
					
					ОбластьЗубКонструкции = ОбластьЗФ.Область(ИмяПоляКонструкция);
					НомерПервойСтроки = ?(НомерПервойСтроки = Неопределено, ОбластьЗубКонструкции.Верх, Мин(НомерПервойСтроки, ОбластьЗубКонструкции.Верх));
					НомерПервойКолонки = ?(НомерПервойКолонки = Неопределено, ОбластьЗубКонструкции.Лево, Мин(НомерПервойКолонки, ОбластьЗубКонструкции.Лево));
					НомерПоследнейСтроки = Макс(НомерПоследнейСтроки, ОбластьЗубКонструкции.Низ);
					НомерПоследнейКолонки = Макс(НомерПоследнейКолонки, ОбластьЗубКонструкции.Право);
					
				КонецЦикла;
				
				ОбластьКонструкция = ОбластьЗФ.Область(НомерПервойСтроки, НомерПервойКолонки, НомерПоследнейСтроки, НомерПоследнейКолонки);
				ОбластьКонструкция.Объединить();
				Если СсылкаНаОбъект.КонструкцияЗуб = СтруктураКонструкция.Конструкция Тогда 
					ОбластьКонструкция.Обвести(ЖирнаяЛиния, ЖирнаяЛиния, ЖирнаяЛиния, ЖирнаяЛиния); 
				КонецЕсли;
