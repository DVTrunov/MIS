...
Функция ПечатьАвансовыйОтчет(СсылкаНаОбъект)
	...
	|	Сумма(АвансовыйОтчетВыданныеАвансы.СуммаВыдано) КАК СуммаВыдано, 
	...
	ВсегоРанееВыдано = 0;
	ВсегоРанееИзрасходовано = 0;
	
	Если РезультатАвансы.Следующий() Тогда
		
		ВсегоРанееВыдано = ВсегоРанееВыдано
						   + ? (ЗначениеЗаполнено(РезультатАвансы.СуммаВыдано), РезультатАвансы.СуммаВыдано, 0);
		ВсегоРанееИзрасходовано = ВсегоРанееИзрасходовано
						   + ? (ЗначениеЗаполнено(РезультатАвансы.СуммаИзрасходовано), РезультатАвансы.СуммаИзрасходовано, 0);
	...
	ОбластьМакета.Параметры.НачальныйОстаток = ВсегоРанееВыдано; 
	ОбластьМакета.Параметры.НачальныйПерерасход = ВсегоРанееИзрасходовано;
	
	Остаток = ВсегоРанееВыдано - ВсегоРанееИзрасходовано + Получено - Израсходовано;
	
	Если Остаток < 0 Тогда 
		ОбластьМакета.Параметры.КонечныйПерерасход = -Остаток;
	ИначеЕсли Остаток > 0 Тогда
		ОбластьМакета.Параметры.КонечныйОстаток = Остаток;
	КонецЕсли;	
