...
&НаКлиенте
Процедура РедактироватьАвансВСписке(Команда)
	ПеренестиАвансыИзШапкиВТабличнуюЧасть(Истина);
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ПеренестиАвансыИзШапкиВТабличнуюЧасть()
...
&НаКлиенте
Процедура НастроитьАвансВШапке()

	КоличествоАвансов = Объект.ВыданныеАвансы.Количество();
	
	Если КоличествоАвансов = 1 Тогда
		АвансЗапись = Объект.ВыданныеАвансы[0];

		НетДокумента = Не ЗначениеЗаполнено(АвансЗапись.ДокументАванса);
		ВыданоИзрасходовано =	ЗначениеЗаполнено(АвансЗапись.СуммаВыдано)
							  И ЗначениеЗаполнено(АвансЗапись.СуммаИзрасходовано);
	КонецЕсли;

	ПоказыватьАвансВШапке = (КоличествоАвансов = 0 Или
						(КоличествоАвансов = 1 И НетДокумента И Не ВыданоИзрасходовано));

	Элементы.ГруппаОстатокПерерасход.Видимость = ПоказыватьАвансВШапке;
	Элементы.ГруппаАвансы.Видимость = НЕ ПоказыватьАвансВШапке;

	Если ПоказыватьАвансВШапке И КоличествоАвансов = 1 Тогда
		АвансСумма = АвансЗапись.СуммаАванса;
		ОстатокИлиПерерасход = АвансЗапись.СуммаВыдано >  0;
		ОстатокПерерасходСумма = АвансЗапись.СуммаВыдано + АвансЗапись.СуммаИзрасходовано;;
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ПеренестиАвансыИзШапкиВТабличнуюЧасть(ПриПереключенииВТабЧасть = Ложь)
	
	// Переносим аванс и нач. остаток в табличную часть документа
	Если Элементы.ГруппаОстатокПерерасход.Видимость Тогда
		Объект.ВыданныеАвансы.Очистить();
		АвансЗапись = Объект.ВыданныеАвансы.Добавить();

		АвансЗапись.СуммаАванса = АвансСумма;
		АвансЗапись.СуммаВыдано = ? (ОстатокИлиПерерасход, ОстатокПерерасходСумма, 0);
		АвансЗапись.СуммаИзрасходовано = ? (ОстатокИлиПерерасход, 0, ОстатокПерерасходСумма);
		
		Если ПриПереключенииВТабЧасть Тогда
			Элементы.ГруппаОстатокПерерасход.Видимость = Ложь;
			Элементы.ГруппаАвансы.Видимость = Истина;
			ТекущийЭлемент = Элементы.ГруппаАвансы;
		КонецЕсли;
	КонецЕсли;
