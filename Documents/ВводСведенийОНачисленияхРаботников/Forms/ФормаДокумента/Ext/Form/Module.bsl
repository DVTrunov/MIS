...
Процедура ЗаполнитьСведениямиПоСотрудникамСервер(Сотрудники, Получатель = Неопределено, Дата = Неопределено)
	...
	ТаблицаНачислений.Свернуть("Владелец,Действие,ИсточникБазы,Начисление,Параметр1,Параметр2,Параметр3,Период,ПериодПрекращения,Персональное,ПорядокРасчета,Сотрудник,ТрудоваяРоль,ФормулаРасчета ");
