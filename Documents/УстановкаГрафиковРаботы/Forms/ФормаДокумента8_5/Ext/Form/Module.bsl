...
Процедура ПриОткрытии(Отказ)
	//Добавлено Трунов
	Если ЭтоВебКлиентСервер() Тогда
		Элементы.ДеревоГрафиков.ВысотаВСтрокахТаблицы = 7;
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	омг_Idoctor.омг_ОбновитьРасписаниеIDoctor();
...
Процедура УчастникиПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		ОбновитьОсновныеВидыЗаполненияГрафиков(ОбщегоНазначенияБИТКлиентСервер.НовыйМассив(ТекущиеДанные.Участник), Истина);
...
Процедура СформироватьДеревоГрафикаКлиент()
	...
	//Изменено Трунов. Добавлена проверка на версию клиента
	ВариантИнтерфейса = ПолучитьНастройкиКлиентскогоПриложения();
	Если ВариантИнтерфейса = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_5 Тогда
		Возврат;
	Иначе
...
КонецПроцедуры
&НаСервере
Функция ПолучитьНастройкиКлиентскогоПриложения()
...
Процедура ДеревоГрафиковПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		РезультатВвода = ОткрытьФормуМодально("Документ.УстановкаГрафиковРаботы.Форма.ФормаВводаВремени", ПараметрыФормы);
...
&НаКлиенте
Процедура ОбновитьОсновныеВидыЗаполненияГрафиков(Знач ЗаполняемыеУчастники = Неопределено, ТолькоДобавлятьНовых = Ложь)
	
	Если ЗаполняемыеУчастники = Неопределено Тогда
		ЗаполняемыеУчастники = Новый Массив;
		Для Каждого СтрокаУчастник Из Объект.Участники Цикл
			ЗаполняемыеУчастники.Добавить(СтрокаУчастник.Участник)
		КонецЦикла;
		
		ОсновныеВидыЗаполненияГрафиков.Очистить(); // Полное перезаполнение.
	КонецЕсли;
	
	ОтборУчастник = Новый Структура("Сотрудник");
	
	Если ТолькоДобавлятьНовых Тогда
		мОбрИнд = ЗаполняемыеУчастники.Количество()-1;
		Для Сч = 0 По мОбрИнд Цикл
			
			ОтборУчастник.Сотрудник = ЗаполняемыеУчастники[мОбрИнд-Сч];
			
			Участник = ЗаполняемыеУчастники[мОбрИнд-Сч];
			Если ОсновныеВидыЗаполненияГрафиков.НайтиСтроки(ОтборУчастник).Количество() <> 0 Тогда
				ЗаполняемыеУчастники.Удалить(мОбрИнд-Сч);
			КонецЕсли;
			
		КонецЦикла;
	КонецЕсли;
	
	Если ЗаполняемыеУчастники.Количество() <> 0 Тогда
		ОсновныеВидыМассив = ДобавитьСотрудниковВТаблицуОсновныеВидыЗаполненияГрафиков(ЗаполняемыеУчастники);
		
		Для Каждого СтрокаВидУчастника Из ОсновныеВидыМассив Цикл
			
			СтарыеСтрокиУчастника = ОсновныеВидыЗаполненияГрафиков.НайтиСтроки(Новый Структура("Сотрудник", СтрокаВидУчастника.Участник));
			Если СтарыеСтрокиУчастника.Количество() <> 0 Тогда
				СтрокаКэш = СтарыеСтрокиУчастника[0];
			Иначе
				СтрокаКэш = ОсновныеВидыЗаполненияГрафиков.Добавить();
				СтрокаКэш.Сотрудник = СтрокаВидУчастника.Участник;
			КонецЕсли;
			
			СтрокаКэш.ВидЗаполнения = СтрокаВидУчастника.ВидЗаполнения;	 
		КонецЦикла;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ДобавитьСотрудниковВТаблицуОсновныеВидыЗаполненияГрафиков(ЗаполняемыеУчастники)
	
	Результат = Новый Массив;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Участники", ЗаполняемыеУчастники);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Сотрудники.Ссылка КАК Участник,
	|	Сотрудники.ОсновнойСпособЗаполненияГрафика КАК ОсновнойСпособЗаполненияГрафика,
	|	Сотрудники.Подразделение.ОсновнойСпособЗаполненияГрафика КАК ПодразделениеОсновнойСпособЗаполненияГрафика
	|ИЗ
	...
	|ВЫБРАТЬ
	|	Оборудование.Ссылка,
	|	Оборудование.ОсновнойСпособЗаполненияГрафика,
	|	Оборудование.Подразделение.ОсновнойСпособЗаполненияГрафика
	|ИЗ
	|	Справочник.Оборудование КАК Оборудование
	|ГДЕ
	|	Оборудование.Ссылка В(&Участники)"
	;
	Выб = Запрос.Выполнить().Выбрать();
	Отбор = Новый Структура("Участник");
	
	Для Каждого Участник Из ЗаполняемыеУчастники Цикл
		Если ЗначениеЗаполнено(Участник) Тогда
			
			Выб.Сбросить();
			Отбор.Участник = Участник;
			Если Выб.НайтиСледующий(Отбор) Тогда
				ВидЗаполнения = ОбщегоНазначенияБИТКлиентСервер.ЗаполненноеЗначение(Выб.ОсновнойСпособЗаполненияГрафика, Выб.ПодразделениеОсновнойСпособЗаполненияГрафика);
				Результат.Добавить(Новый Структура("Участник, ВидЗаполнения",Участник,ВидЗаполнения));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьОсновнымСпособом(Команда)
	...
	// Обновить основные виды заполнения
...
&НаСервереБезКонтекста
Функция ЭтоВебКлиентСервер()
	Сеанс = ПолучитьТекущийСеансИнформационнойБазы();
	Возврат Сеанс.ИмяПриложения = "WebClient";
КонецФункции
