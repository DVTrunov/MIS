...
Функция ПодписываетсяЭП(ИсточникОтчета) Экспорт
	...
	Попытка
		Возврат МедицинскаяОтчетностьВызовСервера.МедОтчеты(ИсточникОтчета).ПодписываетсяЭП();
	Исключение
		Возврат Ложь;
	КонецПопытки;
...
Функция РолиДляПодписания(ИсточникОтчета) Экспорт
	...
	Попытка
		Возврат МедицинскаяОтчетностьВызовСервера.МедОтчеты(ИсточникОтчета).РолиДляПодписания();
	Исключение
...
Функция РольДоступнаТекущемуПользователю(МедицинскийОтчет, Роль) Экспорт
	...
	Попытка
		Возврат МедицинскаяОтчетностьВызовСервера.МедОтчеты(МедицинскийОтчет.ИсточникОтчета).РольДоступнаТекущемуПользователю(МедицинскийОтчет, Роль);
	Исключение
...
Функция СформироватьТабличныйДокументПоРазделу(ИмяТекРаздела, ОтчетОбъект, ИмяОтчета, НаименованиеФормыОтчета, СтруктураДанныхСтраниц) Экспорт
	...
	Исключение
		МакетСтраницы = ОтчетОбъект.ПолучитьМакет(НаименованиеФормыОтчета); // Некоторые старые сохраненные случаи (КОРП);
	КонецПопытки;
