...
Процедура СформироватьСтруктуруРеквизитовФормы(ИмяОтчета)
	...
	// Переопределение видимости кнопоки заполнения и расшифровки.
	Попытка
		Если ОтчетОбъект = Неопределено Тогда
			ОтчетОбъект = МедицинскаяОтчетностьВызовСервера.ОтчетФабрикаМакетов(ИмяОтчета);
		КонецЕсли;
		СтруктураРеквищитовФормыЭтогоОтчета = ОтчетОбъект.СформироватьСтруктуруРеквизитовФормы(ВыбраннаяФорма);
		ЗаполнитьЗначенияСвойств(СтруктураРеквизитовФормы, СтруктураРеквищитовФормыЭтогоОтчета);
	Исключение КонецПопытки;
...
Функция ПолучитьСоответствиеТабДоковИзАрхива(МедицинскийОтчет, ТекстыОшибок)
	...
	ДанныеФайловЭМД = доп_РаботаСФайлами.ПолучитьДанныеФайловЭМД(МедицинскийОтчет, , ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор());
...
Процедура ДобавитьВТабДокЭЦП(ТабДок, МедицинскийОтчет)
	...
	Выборка = доп_СертификатыЭЦП.ПолучитьЭлектронныеПодписи(МедицинскийОтчет);
